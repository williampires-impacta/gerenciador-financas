...
import { bodySymbol, deepQuerySymbol, formDataFileSymbol, getErrorMatchers, headerSymbol, httpBodySymbol, httpSymbol, keyDictionarySymbol, labelSymbol, querySymbol, unionCasesSymbol, } from "./trait.js";
//#region AST helpers (survive S.optional / Suspend / transforms)
export const getProps = (ast) => {
    if (ast._tag === "Objects")
...
export const getAnn = (ast, symbol) => {
    ...
    if (ast._tag === "Union") {
        const real = ast.types.filter((t) => t._tag !== "Undefined" &&
            !(t._tag === "Literal" && t.literal === null));
        if (real.length === 1)
            return getAnn(real[0], symbol);
    }
    return undefined;
};
export const getPropAnn = (prop, symbol) => getAnn(prop.type, symbol);
export const hasPropAnn = (prop, symbol) => getPropAnn(prop, symbol) !== undefined;
export const nameOf = (prop, symbol) => {
...
export const resolveNode = (ast) => {
    if (ast._tag === "Suspend")
        return resolveNode(ast.thunk());
    if (ast.encoding && ast.encoding.length > 0)
...
export const isOpaqueValue = (v) => v instanceof Blob ||
    v instanceof File ||
 ...
 */
export const mapKeysByDictionary = (dict, value, direction) => {
    ...
    const rename = direction === "decode"
        ? Object.fromEntries(Object.entries(dict).flatMap(([ts, wire]) => typeof wire === "string"
            ? [[wire, ts]]
            : wire.map((w) => [w, ts])))
        : Object.fromEntries(Object.entries(dict).map(([ts, wire]) => [
            ts,
 ...
 */
export const mapKeys = (ast, value, direction, fallback) => {
    if (value === null ||
        value === undefined ||
        typeof value !== "object" ||
        isOpaqueValue(value)) {
    ...
    const dict = getAnn(ast, keyDictionarySymbol) ??
        fallback;
    ...
    const unionCases = getAnn(ast, unionCasesSymbol);
    if (unionCases && direction === "decode" && !Array.isArray(value)) {
        const obj = (dict ? mapKeysByDictionary(dict, value, "decode") : value);
    ...
    // handled above.)
    if (node._tag === "Union") {
        const arms = node.types.map(resolveNode);
        if (Array.isArray(value)) {
            const arr = arms.find((t) => t._tag === "Arrays");
            if (arr)
                return mapKeys(arr, value, direction, dict);
        }
        else {
            let best;
            let bestScore = 0;
            ...
            if (best)
                return mapKeys(best, value, direction, dict);
        }
    ...
    if (node._tag === "Objects" && !Array.isArray(value)) {
        const props = node
            .propertySignatures;
        const isigs = node.indexSignatures;
 ...
 */
const appendQuery = (query, name, value) => {
    if (Array.isArray(value)) {
        for (const v of value)
            appendQuery(query, name, v);
    }
    else if (value !== null &&
        typeof value === "object" &&
 ...
 */
const flattenToFormPairs = (obj, prefix = "") => {
    const pairs = [];
...
export const buildRequest = ({ input, inputAst, baseUrl, headers: baseHeaders, mapMemberHeader, unknownKeyToWire, }) => {
    ...
    const rootDict = getAnn(inputAst, keyDictionarySymbol);
    const headers = { ...baseHeaders };
    const body = {};
    let rawBody; // whole-body member (T.HttpBody) — sent as-is
    const files = [];
    ...
    for (const prop of getProps(inputAst)) {
        ...
        const isBodyMember = !hasPropAnn(prop, labelSymbol) &&
        ...
        if (hasPropAnn(prop, labelSymbol)) {
            ...
            body[nameOf(prop, bodySymbol)] = mapKeys(prop.type, value, "encode", rootDict);
        }
    }
    ...
    let request = HttpClientRequest.make(http.method)(url).pipe(HttpClientRequest.setHeaders(headers));
    if (http.contentType === "multipart") {
        ...
        const parts = rawBody !== undefined && typeof rawBody === "object"
            ? rawBody
            : body;
        const isFileOrBlob = (v) => v instanceof Blob || v instanceof File;
            ...
            if (isFileOrBlob(value)) {
                ...
                for (const file of value) {
                    if (isFileOrBlob(file)) {
                        form.append(file instanceof File ? file.name : key, file, file instanceof File ? file.name : undefined);
                    }
                }
            }
            else if (typeof value === "object") {
    ...
    else if (http.contentType === "form-urlencoded" &&
        ...
        const source = rawBody !== undefined && typeof rawBody === "object"
            ? rawBody
            : body;
        const params = new URLSearchParams();
        for (const [k, v] of flattenToFormPairs(source))
            params.append(k, v);
        request = request.pipe(HttpClientRequest.setBody(HttpBody.text(params.toString(), "application/x-www-form-urlencoded")));
    }
        ...
        if (rawBody instanceof Blob || rawBody instanceof ArrayBuffer) {
            ...
            request = request.pipe(HttpClientRequest.setBody(HttpBody.raw(rawBody, { contentType: http.bodyMediaType })));
        }
        else if (rawBody instanceof Uint8Array) {
            request = request.pipe(HttpClientRequest.setBody(HttpBody.uint8Array(rawBody, http.bodyMediaType)));
        }
        else if (http.bodyMediaType) {
            request = request.pipe(HttpClientRequest.setBody(typeof rawBody === "string"
 ...
 * nothing.
 */
export const matchesExpression = (m, code, status, message) => {
    if (m.code === undefined && m.status === undefined && m.message === undefined)
        return false;
    if (m.code !== undefined && m.code !== code)
        return false;
    if (m.status !== undefined && m.status !== status)
        return false;
    if (m.message !== undefined) {
        if (typeof m.message === "string") {
            ...
            if (includes === undefined && matches === undefined)
                return false;
...
const matcherSpecificity = (m) => (m.code !== undefined ? 1 : 0) +
 ...
 */
export const matchTypedError = (errorClasses, status, errors) => {
    let best;
    for (const cls of errorClasses) {
        ...
        for (const m of matchers) {
            for (const e of errors) {
                if (!matchesExpression(m, e.code, status, e.message))
                    continue;
...
//#endregion
//# sourceMappingURL=protocol-http.js.map