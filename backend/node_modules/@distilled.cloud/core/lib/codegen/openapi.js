...
const addShape = (ctx, base, def) => {
    const id = `${ctx.ns}#${uniqueName(ctx, base)}`;
    ctx.shapes[id] = def;
    return id;
};
const detectVersion = (spec) => {
    if (spec?.swagger === "2.0")
        return "2.0";
    const v = spec?.openapi;
    if (typeof v === "string" && v.startsWith("3.1"))
        return "3.1";
    if (typeof v === "string" && v.startsWith("3.0"))
        return "3.0";
    throw new Error(`Unsupported OpenAPI version (swagger=${spec?.swagger}, openapi=${spec?.openapi})`);
};
 ...
 */
const flattenObject = (ctx, def, depth = 0) => {
    const out = {
        properties: {},
        required: new Set(),
        isObject: false,
    ...
    const seen = new Set();
    const visit = (d, dep, inAllOf, inUnion) => {
        if (dep > MAX_SCHEMA_DEPTH)
            return;
        ...
        if (Array.isArray(r.allOf)) {
            for (const sub of r.allOf)
                visit(sub, dep + 1, true, inUnion);
        }
        if (inAllOf) {
            const branches = r.oneOf ?? r.anyOf;
            if (Array.isArray(branches)) {
                for (const b of branches) {
                    if (!isNullBranch(ctx, b))
                        visit(b, dep + 1, inAllOf, true);
                }
            }
        }
...
const isNullBranch = (ctx, branch) => {
    const r = deref(ctx, branch);
    if (!r || typeof r !== "object")
        return false;
    if (r.type === "null")
        return true;
    if (Array.isArray(r.type) && r.type.every((t) => t === "null")) {
        return true;
    }
    if (Array.isArray(r.enum) &&
        r.enum.length > 0 &&
        r.enum.every((v) => v === null)) {
        return true;
    }
    return false;
};
/** Intrinsic nullability flags on a schema node (not union null branches). */
const ownNullable = (ctx, def) => {
    ...
    if (def.nullable === true)
        return true;
    if (def["x-nullable"] === true)
        return true;
    if (Array.isArray(def.type) && def.type.includes("null"))
        return true;
    return false;
};
/**
 ...
 * its `$ref` resolution.
 */
const dirExcluded = (ctx, prop, dir) => {
    const flag = dir === "in" ? "readOnly" : "writeOnly";
    if (prop && typeof prop === "object" && prop[flag] === true)
        return true;
 ...
 */
const scanInlineForFlag = (d, flag, refs) => {
    ...
    for (const sub of schemaChildren(d)) {
        if (scanInlineForFlag(sub, flag, refs))
            return true;
    }
    return false;
};
 ...
 */
const buildDirSensitiveRefs = (ctx, dir) => {
    const flag = dir === "in" ? "readOnly" : "writeOnly";
    ...
    const sensitive = new Set(flagged);
    const queue = [...flagged];
 ...
 */
const dirSensitive = (ctx, def, dir) => {
...
const typeOf = (def) => {
    ...
    if (Array.isArray(t)) {
        ...
        if (real.length === 1 && typeof real[0] === "string")
            return real[0];
    }
    return undefined;
};
 ...
 */
const isNameable = (ctx, def) => {
    if (!def || typeof def !== "object")
        return false;
    if (def.$ref)
        return isNameable(ctx, deref(ctx, def));
    if (Array.isArray(def.enum)) {
        const values = def.enum.filter((v) => v !== null);
    ...
    if (Array.isArray(branches)) {
        return branches.filter((b) => !isNullBranch(ctx, b)).length >= 2;
    }
    const t = typeOf(def);
 ...
 */
const convertSchema = (ctx, def, hint, depth, dir, reservedId) => {
    const emit = (shapeDef, base) => {
    ...
    if (def.$ref) {
        ...
        if (cached)
            return inline(cached.target, cached.nullable || siteNullable);
        const resolved = resolvePointer(ctx.spec, def.$ref);
        if (resolved === undefined)
            return inline(PRELUDE.Document, siteNullable);
        ...
        if (!isNameable(ctx, resolved)) {
            ...
            const entry = { target: PRELUDE.Document, nullable: false };
        ...
        const entry = { target: id, nullable: false };
        ctx.refs.set(cacheKey, entry);
    ...
    if (Array.isArray(branches)) {
        ...
        real.forEach((b, i) => {
            const branchName = typeof b?.$ref === "string"
            ...
            let k = 2;
            while (mn in members)
                mn = `${memberIdent(branchName)}_${k++}`;
            members[mn] = { target: r.target };
        });
        const targets = Object.values(members);
        if (targets.length === 1) {
            return inline(targets[0].target, nullable);
        }
        return {
    ...
    if (Array.isArray(def.allOf)) {
        ...
        if (Object.keys(flat.properties).length === 0) {
            ...
            if (flat.additionalProperties !== undefined) {
                const r = convertSchema(ctx, { type: "object", additionalProperties: flat.additionalProperties }, hint, depth + 1, dir, reservedId);
                return { target: r.target, nullable: nullable || r.nullable };
            }
            if (!flat.isObject)
                return inline(PRELUDE.Document, nullable);
    ...
    if (Array.isArray(def.enum) && def.enum.length > 0) {
        ...
        if (values.every((v) => typeof v === "number")) {
            ...
            for (const lit of values) {
                ...
                let k = 2;
                while (used.has(mn))
                    mn = `${enumMemberName(String(lit))}_${k++}`;
                used.add(mn);
                members[mn] = {
                    target: PRELUDE.Unit,
                    traits: { "smithy.api#enumValue": lit },
    ...
    if (t === "array") {
        ...
        return {
            target: emit({ type: "list", member: { target: item.target }, traits: docTraits }, reservedId !== undefined ? hint : `${hint}List`),
            nullable,
        };
    }
    // --- object ---------------------------------------------------------------
    if (t === "object" || def.properties || def.additionalProperties) {
        ...
        if (ap !== undefined && ap !== false) {
            const value = ap === true || (typeof ap === "object" && Object.keys(ap).length === 0)
...
const isSensitiveProperty = (ctx, name, def) => {
    ...
    if (!isString)
        return false;
    if (r["x-sensitive"] === true || def?.["x-sensitive"] === true)
        return true;
 ...
 */
const buildMembers = (ctx, properties, required, hint, depth, dir) => {
    const members = {};
    for (const [name, prop] of Object.entries(properties)) {
        if (dirExcluded(ctx, prop, dir))
            continue;
        let mn = memberIdent(name);
        let k = 2;
        while (mn in members)
            mn = `${memberIdent(name)}_${k++}`;
        const conv = convertSchema(ctx, prop, `${hint}${pascal(name)}`, depth, dir);
        const traits = {};
        const doc = prop && typeof prop === "object" && typeof prop.description === "string"
            ? prop.description
            : undefined;
        if (doc)
            traits["smithy.api#documentation"] = doc;
        if (required.has(name))
            traits["smithy.api#required"] = {};
        if (mn !== name)
            traits["smithy.api#jsonName"] = name;
        if (conv.nullable)
            traits[NULLABLE_TRAIT] = {};
 ...
 */
const collectParams = (ctx, pathItem, op) => {
    const byKey = new Map();
    for (const raw of [
    ...
    return [...byKey.values()].filter((p) => p.in === "path" ||
        p.in === "query" ||
        p.in === "body" ||
        p.in === "header");
};
 ...
 */
const labelTarget = (ctx, schema, hint) => {
    const r = deref(ctx, schema);
    if (Array.isArray(r?.enum) &&
        r.enum.length > 0 &&
        r.enum.every((v) => typeof v === "string")) {
...
const trimDescription = (desc) => {
    ...
    for (const line of desc.split(/\r?\n/)) {
        if (line.trim().startsWith("### Authorization"))
            break;
...
const opDoc = (op) => {
    ...
    const parts = [summary, desc].filter((s) => s !== undefined && s !== "");
...
const successSchema = (ctx, responses, order) => {
...
const PAGINATION_INPUT_ALIASES = {
    cursor: ["cursor", "page_token", "pageToken"],
    token: ["next_token", "NextToken", "nextToken"],
    page: ["page"],
};
const detectPagination = (ctx, params, responseSchema) => {
    if (!responseSchema)
        return undefined;
    ...
    for (const [k, v] of Object.entries(bag)) {
        if (k === "pagination" || k === "next_token" || k === "NextToken")
            continue;
 ...
 */
const OPTIONAL_HTTP_METHODS = ["head", "options"];
const DEFAULT_STATUS_TO_ERROR_CLASS = {
    "400": "BadRequest",
    "403": "Forbidden",
    "404": "NotFound",
    "409": "Conflict",
...
export const convertOpenApiToSmithy = (spec, options) => {
    const doc = spec;
    ...
    const errorIds = new Map(); // class name → shape id
    ...
    const usedErrors = new Map(); // class name → first status
    const serviceOps = [];
    for (const [rawPath, pathItem] of Object.entries(doc.paths ?? {})) {
        ...
        for (const method of httpMethods) {
            ...
            if (!op || typeof op !== "object")
                continue;
            if (skipDeprecated && op.deprecated === true)
                continue;
            const opName = pascal(typeof op.operationId === "string" && op.operationId
                ? op.operationId
                : `${method}_${rawPath}`);
            ...
            if (bodySchema !== undefined) {
                ...
                if (Object.keys(flat.properties).length > 0) {
                    const bodyMembers = buildMembers(ctx, flat.properties, flat.required, `${opName}Request`, 0, "in");
            ...
            // ---- Input shape ----
            const inputTarget = Object.keys(members).length > 0
            ...
            const { schema: respSchema } = method === "head"
                ? { schema: undefined }
            ...
            if (respSchema !== undefined) {
                ...
                if (Object.keys(flat.properties).length > 0) {
                    ...
                    const isPlainRef = typeof respSchema.$ref === "string" &&
            ...
            for (const status of Object.keys(op.responses ?? {})) {
                if (!/^[45]\d\d$/.test(status))
                    continue;
                if (defaultErrorStatuses.has(status))
                    continue;
                const cls = statusToErrorClass[status];
                if (!cls)
                    continue;
                const id = errorIds.get(cls);
                if (!usedErrors.has(cls))
                    usedErrors.set(cls, Number(status));
                if (!errors.some((e) => e.target === id))
            ...
            if (contentType === "multipart")
                httpTrait.contentType = "multipart";
            const traits = { "smithy.api#http": httpTrait };
            const documentation = opDoc(op);
            if (documentation)
                traits["smithy.api#documentation"] = documentation;
            if (pagination)
                traits["smithy.api#paginated"] = pagination;
            if (contentType)
                traits[CONTENT_TYPE_TRAIT] = contentType;
    ...
    ctx.shapes[`${ctx.ns}#${serviceName}`] = {
        ...
        traits: {
            "smithy.api#title": typeof doc.info?.title === "string"
                ? doc.info.title
                : options.serviceName,
