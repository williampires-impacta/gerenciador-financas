 ...
 */
import { isPrelude } from "./naming.js";
/**
 ...
 */
export const shapeDeps = (def) => {
    if (!def)
        return [];
    if (def.type === "structure" || def.type === "union") {
        return Object.values(def.members ?? {}).map((m) => m.target);
    }
    if (def.type === "list")
        return [def.member.target];
    if (def.type === "map") {
 ...
 */
export const reachableFrom = (shapes, roots, deps = shapeDeps) => {
    const reachable = new Set();
    const visit = (id) => {
        if (isPrelude(id) || reachable.has(id) || !shapes[id])
            return;
        reachable.add(id);
        for (const dep of deps(shapes[id]))
            visit(dep);
    };
 ...
 */
export const topoOrder = (shapes, ids, deps = shapeDeps) => {
    const order = [];
    const done = new Set();
    const stack = new Set();
    const walk = (id) => {
        if (done.has(id) || stack.has(id) || isPrelude(id) || !shapes[id])
            return;
        stack.add(id);
        for (const dep of deps(shapes[id]))
            walk(dep);
 ...
 */
export const isForwardRef = (indexOf, target, selfIdx) => {
 ...
 */
export const cyclicShapeIds = (shapes, ids, deps = shapeDeps) => {
