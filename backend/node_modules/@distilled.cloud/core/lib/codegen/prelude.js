 ...
 */
import { isPrelude, local } from "./naming.js";
import { isForwardRef } from "./graph.js";
/** Prelude shape → schema expression (JSON-wire flavor). */
export const JSON_PRELUDE = {
    String: "S.String",
    Boolean: "S.Boolean",
    Double: "S.Number",
    Float: "S.Number",
 ...
 */
export const makeSchemaRef = (prelude, indexOf) => (target, selfIdx) => {
    if (isPrelude(target))
        return prelude[local(target)] ?? "S.Unknown";
 ...
 */
export const makeTsRef = (tsPrelude) => (target) => isPrelude(target) ? (tsPrelude[local(target)] ?? "unknown") : local(target);
