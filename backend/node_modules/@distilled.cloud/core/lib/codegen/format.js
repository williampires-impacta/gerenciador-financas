...
export const runTool = (argv) => Effect.tryPromise({
    ...
    ? Effect.void
    : Effect.die(new Error(`${argv.join(" ")} exited with ${code}`))), Effect.catchCause((cause) => Effect.die(cause)));
/** Format a generated directory in place. */
export const formatGenerated = (dir) => Effect.flatMap(Console.log(`\n🧹 Formatting ${dir}`), () => runTool(["bunx", "oxfmt", dir]));
/**
 * Lint-fix then format. `oxlint --fix` can leave its rewrites unformatted,
 * so the formatter has to run after it, not before.
 */
export const lintAndFormatGenerated = (dir) => Effect.flatMap(Console.log(`\n🧹 Linting and formatting ${dir}`), () => Effect.flatMap(runTool(["bunx", "oxlint", "--fix", dir]), () => runTool(["bunx", "oxfmt", dir])));
