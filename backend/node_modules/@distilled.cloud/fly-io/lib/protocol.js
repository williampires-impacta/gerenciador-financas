...
import { makeRestProtocol, unwrapRedactedDeep, wrapSensitive, } from "@distilled.cloud/core/protocol-rest";
import { buildRequest, getAnn, mapKeys, matchTypedError, } from "@distilled.cloud/core/protocol-http";
import { HTTP_STATUS_MAP, InternalServerError, } from "@distilled.cloud/core/errors";
import { parseRetryAfterForStatus } from "@distilled.cloud/core/retry-after";
import { Credentials, DEFAULT_FLY_API_BASE_URL, DEFAULT_SPRITES_API_BASE_URL, } from "./credentials.js";
import { ConfigError as FlyConfigError, FlyIoParseError, CreateExtensionTosAgreementNotAuthorized, SpritesNotEnabled, UnknownFlyIoError, } from "./errors.js";
import { graphqlOpSymbol, httpSymbol, responsePathSymbol, } from "./traits.js";
const flyErrorEnvelope = (body) => {
...
const spritesErrorEnvelope = (body) => {
    if (body === null || typeof body !== "object")
        return undefined;
    const b = body;
    const message = typeof b.message === "string"
        ...
        : typeof b.error === "string"
            ? b.error
            : undefined;
    const code = typeof b.error === "string" && typeof b.message === "string"
        ? b.error
        : undefined;
    return { message, code };
};
...
export const FlyIoProtocol = makeRestProtocol({
...
export const FlyApiProtocol = makeRestProtocol({
    credentials: resolveFlyCreds,
 ...
 */
const parseSpritesExecFrames = (bytes) => {
    if (bytes.length === 0)
        return undefined;
    const first = bytes[0];
    if (first > 3)
...
const isMintOp = (http) => http !== undefined &&
    http.method === "POST" &&
    /\/organizations\/\{[^}]+}\/tokens$/.test(http.uri);
const SPRITES_NOT_ENABLED = /sprites not enabled/i;
const readResponseText = (response) => response.text.pipe(Effect.orDie, Effect.map((text) => text ?? ""));
...
const errorEnv = (body) => spritesErrorEnvelope(body) ?? flyErrorEnvelope(body) ?? {};
const failHttp = (status, body, text, headers) => {
...
const orgFromTokensCurrent = (body) => {
    if (body === null || typeof body !== "object")
        return undefined;
...
const orgFromGraphql = (body) => {
    ...
    const nodes = root.organizations
        ?.nodes;
...
const discoverOrgSlug = (fly) => Effect.gen(function* () {
    const http = yield* HttpClient.HttpClient;
    const token = Redacted.value(fly.apiKey);
    const machinesRoot = fly.apiBaseUrl.replace(/\/+$/, "");
    const currentReq = HttpClientRequest.make("GET")(`${machinesRoot}/tokens/current`).pipe(HttpClientRequest.setHeaders({
    ...
    const gqlReq = HttpClientRequest.make("POST")(`${DEFAULT_FLY_API_BASE_URL}/graphql`).pipe(HttpClientRequest.setHeaders({
        Authorization: `Bearer ${token}`,
        Accept: "application/json",
...
const mintOnce = (fly) => Effect.gen(function* () {
    const http = yield* HttpClient.HttpClient;
    const orgSlug = yield* discoverOrgSlug(fly);
    const macaroon = stripFlyAuthScheme(Redacted.value(fly.apiKey));
    const request = HttpClientRequest.make("POST")(`${DEFAULT_SPRITES_API_BASE_URL}/organizations/${encodeURIComponent(orgSlug)}/tokens`).pipe(HttpClientRequest.setHeaders({
    ...
    const token = json !== null && typeof json === "object"
        ? json.token
        : undefined;
    if (typeof token !== "string" || token.length === 0) {
        return yield* fail(new FlyIoParseError({
            body: json ?? text,
...
const mintSpritesToken = (fly) => Effect.gen(function* () {
    ...
    const run = mintOnce(fly).pipe(Effect.tap((token) => Effect.sync(() => {
...
const encodeSpritesRequest = ({ input, inputAst, }) => Effect.gen(function* () {
    const fly = yield* resolveFlyCreds;
    const http = getAnn(inputAst, httpSymbol);
...
const decodeSpritesResponse = ({ response, outputAst, errors: errorClasses, }) => Effect.gen(function* () {
    ...
    if (status >= 400) {
        ...
        if (StatusErrorClass) {
            return yield* fail(new StatusErrorClass({
                message,
                retryAfter: parseRetryAfterForStatus(status, headers),
            }));
        }
        if (status >= 500) {
            return yield* fail(new InternalServerError({
                message,
                retryAfter: parseRetryAfterForStatus(status, headers),
            }));
        }
        return yield* fail(new UnknownFlyIoError({
            message,
...
export const SpritesProtocol = Layer.succeed(API.Protocol, API.Protocol.of({
    encode: (args) => encodeSpritesRequest(args),
    decode: (args) => decodeSpritesResponse(args),
}));
// =============================================================================
...
const matchGraphqlError = (status, errorBody, headers, errorClasses) => {
    ...
    if (envelope._tag === "Some" && envelope.value.errors?.length) {
        ...
        return fail(new UnknownFlyIoError({
...
const graphqlEncode = ({ input, inputAst, }) => Effect.gen(function* () {
    const resolveCredentials = yield* Credentials;
    const creds = yield* resolveCredentials;
    const op = getAnn(inputAst, graphqlOpSymbol);
    if (!op) {
    ...
    return HttpClientRequest.make("POST")(url).pipe(HttpClientRequest.setHeaders({
        Authorization: `Bearer ${Redacted.value(creds.apiKey)}`,
        Accept: "application/json",
    }), HttpClientRequest.bodyJsonUnsafe({
        query: op.query,
...
const graphqlDecode = ({ response, outputAst, errors, }) => Effect.gen(function* () {
    ...
    const envelope = json;
    const hasGraphQLErrors = envelope !== null &&
    ...
    const path = getAnn(outputAst, responsePathSymbol);
    let payload = envelope !== null && typeof envelope === "object"
        ? envelope.data
        : undefined;
    if (path !== undefined) {
...
export const FlyGraphqlProtocol = Layer.succeed(API.Protocol, API.Protocol.of({
    encode: (args) => graphqlEncode(args),
