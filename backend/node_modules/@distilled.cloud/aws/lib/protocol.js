...
import { makeRequestBuilder } from "./client/request-builder.js";
import { makeResponseParser } from "./client/response-parser.js";
import * as Credentials from "./credentials.browser.js";
import * as Endpoint from "./endpoint.js";
import * as Region from "./region.js";
import { makeEndpointResolver } from "./rules-engine/endpoint-resolver.js";
import { getAwsApiService, getAwsAuthSigv2, getAwsAuthSigv4, } from "./traits.js";
import { getIdentifier } from "./util/ast.js";
// Keyed by the operation config object, which API.make memoizes per
// operation — one entry per operation per process.
const preparedOps = new WeakMap();
...
const encode = ({ input, config, }) => Effect.gen(function* () {
    const { buildRequest, sigv4, sigv2, serviceSdkId, resolveEndpoint: rulesResolver, } = prepare(config);
    ...
    if (customEndpoint) {
        ...
        const authSchemes = resolved.endpoint.properties?.authSchemes;
    ...
    if (config.endpointHostPrefix !== undefined && !customEndpoint) {
        const resolvedPrefix = config.endpointHostPrefix.replace(/\{(\w+)\}/g, (_, member) => String(input?.[member] ?? ""));
    ...
    if (sigv2) {
        ...
        pairs.push(["AWSAccessKeyId", Redacted.value(credentials.accessKeyId)], ["SignatureVersion", "2"], ["SignatureMethod", "HmacSHA256"], ["Timestamp", new Date().toISOString().replace(/\.\d{3}/, "")]);
        ...
        const signatureBytes = yield* Effect.promise(async () => {
            const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(Redacted.value(credentials.secretAccessKey)), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
        ...
        const contentTypeV2 = resolvedRequest.headers["Content-Type"] ??
            resolvedRequest.headers["content-type"] ??
            "application/x-www-form-urlencoded";
        return pipe(HttpClientRequest.make(resolvedRequest.method)(requestUrl.toString()), HttpClientRequest.setHeaders(resolvedRequest.headers), HttpClientRequest.setBody(HttpBody.text(signedBody, contentTypeV2)));
    }
    ...
    const hasContentSha256 = Object.keys(resolvedRequest.headers).some((k) => k.toLowerCase() === "x-amz-content-sha256");
    ...
    const hasServiceChecksum = Object.keys(resolvedRequest.headers).some((k) => k.toLowerCase() === "x-amz-checksum");
    ...
    const hasStreamingInput = resolvedRequest.hasStreamingInput === true &&
    ...
    const useUnsignedPayload = (isStreamingBody ||
        hasStreamingInput ||
    ...
    if ((serviceSdkId === "S3 Control" || serviceSdkId === "Glacier") &&
        !useUnsignedPayload &&
        !hasContentSha256 &&
        !isStreamingBody) {
        const bodyBytes = resolvedRequest.body === undefined
            ? new Uint8Array(0)
            : resolvedRequest.body instanceof Uint8Array
                ? resolvedRequest.body
                : new TextEncoder().encode(String(resolvedRequest.body));
        const digest = yield* Effect.promise(() => crypto.subtle.digest("SHA-256", 
    ...
    return pipe(HttpClientRequest.make(resolvedRequest.method)(signedRequest.url), HttpClientRequest.setHeaders(signedHeaders), HttpClientRequest.setBody(httpBody));
});
/**
...
const decode = ({ response, config, }) => Effect.gen(function* () {
    ...
    const isEmptyBody = response.request.method === "HEAD" ||
        contentLength === "0" ||
        response.status === 204;
 ...
 */
export const AwsProtocol = Layer.succeed(API.Protocol, API.Protocol.of({
