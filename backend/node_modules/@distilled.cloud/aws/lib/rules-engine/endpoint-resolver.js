...
import { getContextParam, getEndpointResolver, getStaticContextParams, hasHttpLabel, } from "../traits.js";
import { getPropertySignatures } from "../util/ast.js";
import { isVirtualHostableS3Bucket, parseArn, partition, } from "./aws-functions.js";
import { getAttr, isValidHostLabel, parseURL, substring, uriEncode, } from "./standard-functions.js";
/**
 * Recursively resolve template values in nested objects/arrays
 */
const resolveTemplates = (value) => {
    if (typeof value === "string")
        return value;
    if (Array.isArray(value))
        return value.map(resolveTemplates);
 ...
 */
export const makeEndpointResolver = (operation) => {
    ...
    return Effect.fn(function* (resolverInput) {
        ...
        if (staticContextParams) {
            for (const [paramName, paramDef] of Object.entries(staticContextParams)) {
                endpointParams[paramName] = paramDef.value;
            }
        }
        // Extract context parameters from the payload
        const payloadObj = input;
        ...
        const result = resolver(endpointParams, endpointResolverHelpers);
        if (result.type === "error") {
            return yield* Effect.fail(new Error(result.message));
        }
        const endpoint = {
            url: result.endpoint.url,
            properties: result.endpoint.properties,
        ...
        const adjustedRequest = adjustRequestPath(request, endpoint.url, contextParamMappings, payloadObj);
 ...
 */
function extractContextParamMappings(ast) {
 ...
 */
function adjustRequestPath(request, endpointUrl, contextParamMappings, input) {
    try {
