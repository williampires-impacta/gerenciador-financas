...
import { Auth, } from "./auth.browser.js";
import { ConflictingSSORegion, ConflictingSSOStartUrl, ExpiredSSOToken, InvalidSSOProfile, InvalidSSOToken, ProfileNotFound, regionFromEnv, SsoPortalError, SsoRegion, SsoStartUrl, } from "./credentials.js";
import { parseIni, parseSSOSessionData } from "./util/parse-ini.js";
...
export const Default = Effect.serviceOption(Auth).pipe(Effect.map(Option.getOrUndefined), Effect.flatMap((c) => (c ? Effect.succeed(c) : makeAuthService())));
export const loadProfile = (profile) => Effect.flatMap(Default, (auth) => auth.loadProfile(profile));
export const loadProfileCredentials = (profile) => Effect.flatMap(Default, (auth) => auth.loadProfileCredentials(profile));
export const makeAuthService = () => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const client = yield* HttpClient.HttpClient;
    const loadProfile = Effect.fn(function* (profileName) {
        const profiles = yield* Effect.promise(() => ini.parseKnownFiles({ profile: profileName }));
        ...
        if (profile.sso_session || profile.sso_start_url) {
            if (profile.sso_session) {
                ...
                const ssoStartUrl = Option.getOrElse(yield* Effect.serviceOption(SsoStartUrl), () => profile.sso_start_url);
                const ssoSessions = yield* fs.readFileString(configPath).pipe(Effect.flatMap((config) => Effect.promise(async () => parseIni(config))), Effect.map(parseSSOSessionData));
            ...
            if (missingFields.length > 0) {
                yield* new InvalidSSOProfile({
                    profile: profileName,
                    missingFields,
                    message: `Profile is configured with invalid SSO credentials. Required parameters "sso_account_id", ` +
    ...
    const loadProfileCredentials = Effect.fn(function* (profileName) {
        const awsDir = path.join(ini.getHomeDir(), ".aws");
        ...
        if (ssoCacheKey) {
            ...
            // `Effect.catch`.
            const cachedCreds = yield* fs.readFileString(cachedCredsFilePath).pipe(Effect.flatMap((text) => Effect.try({
            ...
            })), Effect.catch(() => new InvalidSSOToken({
            ...
            if (isExpired(ssoToken.expiresAt)) {
                yield* Console.log(`The SSO session token associated with profile=${profileName} was not found or is invalid. ${REFRESH_MESSAGE}`);
                return yield* new ExpiredSSOToken({
                    message: `The SSO session token associated with profile=${profileName} was not found or is invalid. ${REFRESH_MESSAGE}`,
                    profile: profileName,
                });
            }
            const response = yield* client.get(`https://portal.sso.${profile.sso_region}.amazonaws.com/federation/credentials?account_id=${profile.sso_account_id}&role_name=${profile.sso_role_name}`, {
                headers: {
            ...
            yield* fs.writeFileString(cachedCredsFilePath, JSON.stringify({
