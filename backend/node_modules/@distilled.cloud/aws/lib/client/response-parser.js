...
import { COMMON_ERRORS, InternalError, ParseError, UnknownAwsError, } from "../errors.js";
import { getAwsQueryError, getHttpError, getHttpHeader, getProtocol, getSyntheticError, hasErrorMessage, } from "../traits.js";
import { getIdentifier, getPropertySignatures, isBooleanAST, isNumberAST, } from "../util/ast.js";
...
const wireCodeMatches = (from, errorCode) => from === errorCode || stripErrorSuffix(from) === stripErrorSuffix(errorCode);
/**
 ...
 * — reject it.
 */
const matchesMessage = (matcher, message) => {
 ...
 */
export const makeResponseParser = (operation, options) => {
    ...
    const protocol = protocolFactory(operation);
    ...
    const registerError = (err) => {
        ...
        if (queryError?.code)
            errorSchemas.set(queryError.code, err);
    };
    ...
    // @ts-expect-error
    return Effect.fn(function* (response) {
        // Success path
        if (response.status >= 200 && response.status < 300) {
            ...
            if (lenient) {
                return yield* decode(deserialized).pipe(Effect.catch(() => Effect.succeed(deserialized)));
            }
        ...
        for (const { schema, trait } of syntheticErrors) {
            if (!wireCodeMatches(trait.from, errorCode))
                continue;
            if (!matchesMessage(trait.message, errorMessage))
                continue;
        ...
        if (errorSchema === undefined && errorCode === "") {
            const statusMatches = (operation.errors ?? []).filter((err) => getHttpError(err.ast) === response.status);
        ...
        if (errorSchema) {
            ...
            for (const prop of errorProps) {
                ...
                if (headerName) {
                    ...
                    if (headerValue !== undefined) {
                        ...
                        data[String(prop.name)] = isNumberAST(prop.type)
                            ? Number(headerValue)
            ...
            const decoded = yield* Schema.decodeUnknownEffect(errorSchema)(dataWithTag).pipe(Effect.catch(() => Effect.succeed(dataWithTag)));
