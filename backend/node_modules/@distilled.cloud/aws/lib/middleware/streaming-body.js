...
import { hasHttpPayload, hasRequiresLength, isInputEventStream, isStreamingType, } from "../traits.js";
/**
 ...
 */
export const makeStreamingBodyMiddleware = (schema) => {
    ...
    return (request) => Effect.gen(function* () {
        ...
        const hasContentLength = Object.keys(request.headers).some((k) => k.toLowerCase() === "content-length");
        // Also check for x-amz-decoded-content-length (used in aws-chunked encoding)
        const hasDecodedContentLength = Object.keys(request.headers).some((k) => k.toLowerCase() === "x-amz-decoded-content-length");
 ...
 */
const bufferStream = (stream) => Effect.promise(async (signal) => {
 ...
 */
const checkIsEventStream = (ast) => {
    if (isInputEventStream(ast))
        return true;
    // Handle S.optional() wrapping - check union members
    if (ast._tag === "Union") {
        for (const member of ast.types) {
            if (member._tag === "Undefined")
                continue;
            if (isInputEventStream(member))
                return true;
        }
    }
    return false;
 ...
 */
const getStreamingPayloadInfo = (ast) => {
...
//# sourceMappingURL=streaming-body.js.map