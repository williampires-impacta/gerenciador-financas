...
import { getMd5ChecksumAlgorithmFunction } from "../hash/md5.js";
import { toUint8Array } from "../hash/utf8.js";
import { getAwsProtocolsHttpChecksum } from "../traits.js";
import { createBufferedReadableStream } from "../util/stream.js";
export const stringHasher = (checksumAlgorithmFn, body) => Effect.promise(async () => {
 ...
 */
export const applyHttpChecksum = (schema, request) => Effect.gen(function* () {
    ...
    if (request.headers[checksumHeader] ||
    ...
    if (isStreaming) {
        ...
        const { "Content-Length": _cl, "content-length": _cl2, ...headersWithoutContentLength } = request.headers;
        return {
            ...request,
            body: chunkedBody,
            headers: {
 ...
 */
export function createAwsChunkedStream(source, checksumAlgorithmFn, checksumLocationName) {
    const hash = new checksumAlgorithmFn();
