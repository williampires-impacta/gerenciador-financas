...
import { applyApiGatewayCustomizations, isApiGateway, } from "../customizations/api-gateway.js";
import { applyGlacierCustomizations, isGlacier, } from "../customizations/glacier.js";
import { ParseError } from "../errors.js";
import { parseEventStreamToUnion } from "../eventstream/parser.js";
import { serializeInputEventStream, serializeInputEventStreamWithPayloads, } from "../eventstream/serializer.js";
import { getAwsApiService, getEventPayloadMap, getEventSchema, getHttpHeader, getOutputEventPayloadMap, getHttpPrefixHeaders, getHttpQuery, getPropAnnotations, getServiceVersion, hasHttpLabel, hasHttpPayload, hasHttpQueryParams, isInputEventStream, isOutputEventStream, isStreamingType, } from "../traits.js";
import { getEncodedPropertySignatures, getIdentifier, isBooleanAST, isNumberAST, } from "../util/ast.js";
import { extractJsonErrorCode, extractJsonErrorData, sanitizeErrorCode, } from "../util/error.js";
import { extractStaticQueryParams } from "../util/query-params.js";
import { applyHttpTrait, bindInputToRequest } from "../util/serialize-input.js";
import { convertStreamingInput, isEffectStream, readableToEffectStream, readStreamAsBytes, readStreamAsText, } from "../util/stream.js";
export const restJson1Protocol = (operation) => {
    ...
    const headerProps = [];
    const prefixHeaderProps = [];
    let outputPayloadProp;
    let responseCodePropName;
    ...
    const hasBodyCapableInputMembers = getEncodedPropertySignatures(inputAst).some((prop) => getHttpHeader(prop) === undefined &&
        !hasHttpLabel(prop) &&
        ...
        !hasHttpPayload(prop));
    return {
        serializeRequest: Effect.fn(function* (input) {
            const encoded = yield* encodeInput(input).pipe(Effect.mapError((err) => new ParseError({ message: err.message })));
            ...
            const { payloadValue, payloadAst, bodyMembers, hasBodyMembers } = bindInputToRequest(inputAst, encoded, request);
            ...
            if (payloadValue !== undefined && payloadAst !== undefined) {
                ...
                const isInputEventStreamPayload = isInputEventStream(payloadAst) && isEffectStream(payloadValue);
                if (isInputEventStreamPayload) {
                    ...
                    if (eventPayloadMap && Object.keys(eventPayloadMap).length > 0) {
                        request.body = serializeInputEventStreamWithPayloads(payloadValue, eventPayloadMap);
                    }
                    else {
                        request.body = serializeInputEventStream(payloadValue);
                    }
                    ...
                    request.body = convertStreamingInput(payloadValue);
        ...
        deserializeResponse: Effect.fn(function* (response) {
            const result = {};
            ...
            if (outputPayloadProp?.isStreaming) {
                if (outputPayloadProp.isEventStream && response.body) {
                    ...
                    // the generated types
                    result[outputPayloadProp.name] = parseEventStreamToUnion(response.body, undefined, outputPayloadProp.eventSchema, outputPayloadProp.eventPayloadMap);
                }
                else {
        ...
        deserializeError: Effect.fn(function* (response) {
            // Read body as text
            const bodyText = yield* readStreamAsText(response.body);
            ...
            const rawErrorCode = response.headers["x-amzn-errortype"] ??
...
function isRawPayload(ast) {
    if (isStreamingType(ast))
        return true;
    if (ast._tag === "String")
        return true;
    if (AST.isUnion(ast))
        return ast.types.some(isRawPayload);
    return false;
}
/**
 ...
 */
function isBlobPayload(ast) {
    if (AST.isUnion(ast))
        return ast.types.some(isBlobPayload);
