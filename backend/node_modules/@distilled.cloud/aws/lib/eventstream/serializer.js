...
import { encodeEvent, } from "./codec.js";
// ============================================================================
// Stream Serializer
// ============================================================================
/**
 ...
 */
export const serializeEventStream = (events) => {
 ...
 */
function extractEventTypeAndPayload(event) {
 ...
 */
export const serializeInputEvent = (event, contentType = "application/json") => {
 ...
 */
export const serializeInputEventStream = (events, contentType = "application/json") => {
    // Convert typed events to StreamEvents, then encode to bytes
    const byteStream = events.pipe(Stream.map((event) => serializeInputEvent(event, contentType)), Stream.mapEffect(encodeEvent));
 ...
 * @returns A MessageEvent ready for wire serialization
 */
export const serializeInputEventWithPayload = (event, payloadMemberName, contentType = "application/octet-stream") => {
    ...
    const payloadSource = innerPayload && typeof innerPayload === "object"
        ? innerPayload
        : event;
 ...
 * @returns ReadableStream of encoded event stream bytes
 */
export const serializeInputEventStreamWithPayloads = (events, eventPayloadMap) => {
    const byteStream = events.pipe(Stream.map((event) => {
