...
import { getCrc32ChecksumAlgorithmFunction } from "../hash/crc32.js";
// ============================================================================
// Constants
// ============================================================================
...
(function (HeaderType) {
    HeaderType[HeaderType["BoolTrue"] = 0] = "BoolTrue";
    HeaderType[HeaderType["BoolFalse"] = 1] = "BoolFalse";
    HeaderType[HeaderType["Byte"] = 2] = "Byte";
    HeaderType[HeaderType["Short"] = 3] = "Short";
    HeaderType[HeaderType["Int"] = 4] = "Int";
    HeaderType[HeaderType["Long"] = 5] = "Long";
    HeaderType[HeaderType["ByteArray"] = 6] = "ByteArray";
 ...
 */
const computeCrc32 = (data) => Effect.promise(async () => {
 ...
 */
export const encodeMessage = (message) => Effect.gen(function* () {
    ...
    const totalLength = PRELUDE_LENGTH +
        headersBytes.length +
        payload.length +
        MESSAGE_CRC_LENGTH;
    // Build the message
 ...
 */
function decodeHeaderValue(data, offset) {
    return Effect.gen(function* () {
        const type = data[offset];
        const view = new DataView(data.buffer, data.byteOffset + offset);
        switch (type) {
            case HeaderType.BoolTrue:
                return [{ type: HeaderType.BoolTrue, value: true }, 1];
            case HeaderType.BoolFalse:
                return [{ type: HeaderType.BoolFalse, value: false }, 1];
            case HeaderType.Byte:
                return [{ type: HeaderType.Byte, value: view.getInt8(1) }, 2];
            case HeaderType.Short:
                return [
            ...
            case HeaderType.ByteArray: {
                ...
                return [{ type: HeaderType.ByteArray, value }, 3 + length];
            }
            case HeaderType.String: {
                const length = view.getUint16(1, false);
                const value = new TextDecoder().decode(data.subarray(offset + 3, offset + 3 + length));
                return [{ type: HeaderType.String, value }, 3 + length];
            }
            case HeaderType.Timestamp: {
            ...
            case HeaderType.Uuid: {
                ...
                return [{ type: HeaderType.Uuid, value }, 17];
            }
            default:
 ...
 */
export const decodeMessage = (data) => Effect.gen(function* () {
    ...
    const messageCrc = new DataView(data.buffer, data.byteOffset + totalLength - MESSAGE_CRC_LENGTH).getUint32(0, false);
    const computedMessageCrc = yield* computeCrc32(data.subarray(0, totalLength - MESSAGE_CRC_LENGTH));
    ...
    const payload = data.subarray(PRELUDE_LENGTH + headersLength, PRELUDE_LENGTH + headersLength + payloadLength);
    const headers = yield* decodeHeaders(headersData);
    return [{ headers, payload }, totalLength];
});
/**
 * Check if we have a complete message in the buffer
 * Returns the total message length if complete, or 0 if incomplete
 */
export function getMessageLength(data) {
 ...
 */
export const parseEvent = (message) => Effect.gen(function* () {
