...
import { bodySymbol, headerSymbol, keyDictionarySymbol, responseCodeSymbol, } from "@distilled.cloud/core/trait";
import { buildRequest, getAnn, getProps, hasPropAnn, mapKeys, matchTypedError, nameOf, } from "@distilled.cloud/core/protocol-http";
import { retryableKey } from "@distilled.cloud/core/category";
import { ConfigError, Forbidden, GatewayTimeout, HTTP_STATUS_MAP, InternalServerError, TooManyRequests, Unauthorized, } from "@distilled.cloud/core/errors";
...
import { CloudflareHttpError, InvalidRoute, UnknownCloudflareError, } from "./errors.js";
import { binaryResponseBodySymbol, envelopePayloadRootSymbol, envelopePayloadSymbol, resultInfoSymbol, } from "./traits.js";
 ...
 */
const httpStatusError = (status, body, headers) => {
...
const camelToSnake = (key) => key.replace(/([a-z0-9])([A-Z])/g, "$1_$2").toLowerCase();
/**
 ...
 */
const bearerPrefixAuthorization = (name, value) => name === "authorization" && !/^Bearer\s/i.test(value)
    ? `Bearer ${value}`
    : value;
...
// annotations.
const encode = ({ input, inputAst, }) => Effect.gen(function* () {
 ...
 * `content-length`).
 */
const wantsNumber = (ast) => ast._tag === "Number" ||
    (ast._tag === "Union" &&
        ast.types?.some(wantsNumber) === true);
/**
 ...
 */
const makeDecode = (options) => ({ response, outputAst, errors: errorClasses, }) => Effect.gen(function* () {
    ...
    if (failed) {
        ...
        const isTransientAuthBlip = /authentication error|unable to authenticate request/i.test(errorMessage) ||
            (errorCode === 10001 && /internal error/i.test(errorMessage));
        const tagBlip = (error) => isTransientAuthBlip ? tagRetryable(error) : error;
        // 1. Per-operation typed error (matcher metadata on the class).
        const typed = matchTypedError(errorClasses, status, [
            { code: errorCode, message: errorMessage },
        ]);
        if (typed !== undefined) {
            return yield* Effect.fail(tagBlip(typed));
        }
        ...
        if (status === 429 || GLOBAL_RATE_LIMIT_MESSAGE.test(errorMessage)) {
            return yield* fail(new TooManyRequests({
        ...
        if (status >= 400 && status < 500) {
            ...
            if (StatusErrorClass) {
                return yield* fail(tagBlip(new StatusErrorClass({
                    message: errorMessage,
                    retryAfter: parseRetryAfterForStatus(status, headers),
                })));
            }
        ...
        return yield* fail(new UnknownCloudflareError({
            code: errorCode,
            message: errorMessage,
        }));
    }
    ...
    const payload = (nonJson ? text : "result" in json ? json.result : json);
    const rootDict = getAnn(outputAst, keyDictionarySymbol);
    ...
    for (const prop of getProps(outputAst)) {
        ...
        if (options.resultInfo && hasPropAnn(prop, resultInfoSymbol)) {
            ...
            const src = payload && typeof payload === "object"
                ...
                : undefined;
            if (src !== undefined) {
                result[key] = mapKeys(prop.type, payload[src], "decode", rootDict);
            }
        }
    }
    return result;
});
export const CloudflareProtocol = Layer.succeed(API.Protocol, API.Protocol.of({
 ...
 */
export const CloudflarePaginatedProtocol = Layer.succeed(API.Protocol, API.Protocol.of({
    encode: (args) => encode(args),
