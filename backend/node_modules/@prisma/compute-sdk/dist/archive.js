 ...
 */
export async function createArchive(directory, entrypoint, signal) {
...
async function addDirectory(packer, rootPath, fsPath, tarPrefix, signal) {
    signal?.throwIfAborted();
...
async function addFile(packer, rootPath, fsPath, tarPath, fileStat, signal) {
    signal?.throwIfAborted();
...
async function addSymlink(packer, rootPath, fsPath, tarPath, linkStat, signal) {
    signal?.throwIfAborted();
    const symlinkPath = resolvePathWithinRoot(rootPath, fsPath);
    const target = await readlink(symlinkPath);
    const linkname = await resolveArchiveSymlinkTarget(rootPath, symlinkPath, target, signal);
    packer.entry({
        name: tarPath,
        type: "symlink",
        linkname,
...
async function resolveArchiveSymlinkTarget(rootPath, symlinkPath, target, signal) {
    signal?.throwIfAborted();
...
function collectGzipped(packer, signal) {
    signal?.throwIfAborted();
    const chunks = [];
