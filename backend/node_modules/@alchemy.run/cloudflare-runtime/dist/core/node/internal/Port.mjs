...
//#region src/core/internal/Port.ts
...
*/
const viteSupportsPortZero = (version) => {
...
*/
const MAX_PORT_SEARCH_ATTEMPTS = 128;
/** The bind failed because the address is genuinely taken (scanning to the next port can help). */
const ADDRESS_IN_USE_CODES = /* @__PURE__ */ new Set(["EADDRINUSE", "EACCES"]);
/**
...
*/
const UNSUPPORTED_HOST_CODES = /* @__PURE__ */ new Set([
	"EADDRNOTAVAIL",
	"EAFNOSUPPORT",
	"EINVAL",
	"ENOTFOUND"
]);
const errorCode = (error) => typeof error === "object" && error !== null && "code" in error && typeof error.code === "string" ? error.code : void 0;
/**
...
*/
const isUnsupportedHostError = (error, host, hasIPv6Interface) => {
	const code = errorCode(error);
	if (code !== void 0) return UNSUPPORTED_HOST_CODES.has(code);
	return host !== void 0 && NodeNet.isIPv6(host) && !(hasIPv6Interface ?? Object.values(NodeOs.networkInterfaces()).some((addresses) => addresses?.some((address) => address.family === "IPv6") ?? false));
};
...
const globalReservations = /* @__PURE__ */ new Map();
const isReserved = (port) => {
...
const make = (options) => Effect.gen(function* () {
	const addressInUseError = (port) => new ConfigError({
		subtag: "AddressInUse",
	...
	const bind = (port, host) => Effect.callback((resume) => {
		...
		server.once("error", (error) => {
			server.close(() => {
				const code = errorCode(error);
				if (isUnsupportedHostError(error, host)) return resume(Effect.succeed(port));
				if (code === void 0 || ADDRESS_IN_USE_CODES.has(code)) return resume(Effect.fail(addressInUseError(port)));
				return resume(Effect.die(new SystemError({
	...
	const cache = yield* Cache.makeWith((port) => Effect.forEach(HOSTS, (host) => bind(port, host)).pipe(Effect.as(true), Effect.orElseSucceed(() => false)), {
		capacity: options.cache ? Infinity : 0,
		timeToLive: (exit) => exit._tag === "Success" && exit.value ? 0 : "30 seconds"
	});
	const reserveLocal = (port) => Cache.set(cache, port, false);
	const reserve = (port) => Effect.sync(() => {
	...
	const search = Effect.fn(function* (start) {
		let port = start;
		const limit = Math.min(MAX_PORT, start + 128 - 1);
		while (port <= limit) {
			if (!isReserved(port) && (yield* Cache.get(cache, port))) {
				yield* reserve(port);
				return port;
			}
		...
		return yield* Effect.die(new SystemError({
	...
	const probe = (port) => Effect.forEach(HOSTS, (host) => bind(port, host)).pipe(Effect.as(port));
	return {
		find: Effect.fn(function* (port) {
			if (port === 0) return yield* bind(port).pipe(Effect.tap(reserveLocal));
			return yield* search(port);
		}),
		waitFor: (port) => Effect.suspend(() => isReserved(port) ? Effect.fail(addressInUseError(port)) : probe(port)).pipe(Effect.retry({
			while: () => !isReserved(port),
			schedule: Schedule.spaced(WAIT_FOR_INTERVAL),
			times: WAIT_FOR_ATTEMPTS
		}), Effect.tap(() => reserve(port))),
		check: (port) => Effect.suspend(() => isReserved(port) ? Effect.fail(addressInUseError(port)) : Cache.get(cache, port).pipe(Effect.flatMap((available) => available ? reserve(port).pipe(Effect.as(port)) : Effect.fail(addressInUseError(port))))),
		reserve
	};
});
//#endregion
export { MAX_PORT, MAX_PORT_SEARCH_ATTEMPTS, isUnsupportedHostError, make, viteSupportsPortZero };
