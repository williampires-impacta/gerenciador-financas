...
export const groupBy: {
   ...
   * @since 2.0.0
   */
  <A, K extends string | symbol>(f: (a: A) => K): (self: Iterable<A>) => Record.ReadonlyRecord.GroupByResult<K, NonEmptyArray<A>>
  /**
   * Groups elements into a record by a key-returning function. Each key maps
   * to a `NonEmptyArray` of elements that produced that key.
   *
   ...
   * @since 2.0.0
   */
  <A, K extends string | symbol>(self: Iterable<A>, f: (a: A) => K): Record.ReadonlyRecord.GroupByResult<K, NonEmptyArray<A>>
