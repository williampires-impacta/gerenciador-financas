...
import { dual, memoize } from "./Function.js";
...
const isSourceError = u => Predicate.isTagged(u, "SourceError");
const catchSourceError = (self, hasInput) => self.pipe(Effect.catchDefect(defect => isSourceError(defect) ? Effect.fail(evaluationFailure(new ConfigError(defect), hasInput)) : Effect.die(defect)));
...
export function all(arg) {
  ...
  if (Array.isArray(configs)) {
    return make((provider, pathPrefix) => Effect.flatMapEager(Effect.all(configs.map(config => Effect.result(evaluateAt(config, provider, pathPrefix)))), resolveArray));
  } else {
    return make((provider, pathPrefix) => Effect.flatMapEager(Effect.all(Rec.map(configs, config => Effect.result(evaluateAt(config, provider, pathPrefix)))), resolveRecord));
  }
}
