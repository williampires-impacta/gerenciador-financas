 ...
 * @since 2.0.0
 */
const releaseReadFor = (self, fiberId) => Effect.gen(function* () {
  const state = yield* TxRef.get(self.stateRef);
  const currentCount = Option.getOrElse(HashMap.get(state.readers, fiberId), () => 0);
...
export const releaseRead = self => Effect.withFiber(fiber => releaseReadFor(self, fiber.id));
/**
 ...
 * @since 2.0.0
 */
const releaseWriteFor = (self, fiberId) => Effect.gen(function* () {
  const state = yield* TxRef.get(self.stateRef);
...
export const releaseWrite = self => Effect.withFiber(fiber => releaseWriteFor(self, fiber.id));
/**
 ...
 * @since 2.0.0
 */
export const readLock = self => Effect.withFiber(fiber => Effect.acquireRelease(acquireRead(self), () => releaseReadFor(self, fiber.id)));
/**
 ...
 * @since 2.0.0
 */
export const writeLock = self => Effect.withFiber(fiber => Effect.acquireRelease(acquireWrite(self), () => releaseWriteFor(self, fiber.id)));
/**
