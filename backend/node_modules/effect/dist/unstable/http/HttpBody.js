...
const fileContentLength = (size, options) => {
 ...
 * @since 4.0.0
 */
export const file = (path, options) => Effect.flatMap(FileSystem.FileSystem, fs => Effect.map(fs.stat(path), info => stream(fs.stream(path, options), options?.contentType, fileContentLength(info.size, options))));
/**
 ...
 * @since 4.0.0
 */
export const fileFromInfo = (path, info, options) => Effect.map(FileSystem.FileSystem, fs => stream(fs.stream(path, options), options?.contentType, fileContentLength(info.size, options)));
