...
import { sqlCleanupBatchSize } from "../../internal/persistence.js";
 ...
 * @since 4.0.0
 */
export class PersistenceError extends /*#__PURE__*/Schema.Error(ErrorTypeId)({
    ...
    make: Effect.fnUntraced(function* (storeId) {
      ...
      return identity({
        ...
        getMany: keys => sql`SELECT id, value FROM ${table} WHERE id IN (${sql.literal(keys.map(wrapString).join(", "))}) AND (expires IS NULL OR expires > ${clock.currentTimeMillisUnsafe()})`.unprepared.pipe(Effect.mapError(cause => new PersistenceError({
          ...
          return Effect.succeed(keys.map(key => values.get(key)));
        })),
...
export const layerBackingSql = /*#__PURE__*/Layer.effect(BackingPersistence)(/*#__PURE__*/Effect.gen(function* () {
  ...
  yield* sql.onDialectOrElse({
    pg: () => sql`CREATE INDEX IF NOT EXISTS effect_persistence_expires_idx ON ${table} (expires) WHERE expires IS NOT NULL`.pipe(Effect.orDie, Effect.asVoid),
    mysql: () => Effect.gen(function* () {
      const indexExists = sql`
      ...
      yield* sql`CREATE INDEX effect_persistence_expires_idx ON ${table} (expires)`.pipe(Effect.catch(error => Effect.flatMap(indexExists, exists => exists ? Effect.void : Effect.fail(error))));
    }).pipe(Effect.orDie),
    mssql: () => Effect.gen(function* () {
      const indexExists = sql`
        ...
        `.pipe(Effect.map(rows => Number(rows[0].count) > 0));
      yield* sql`CREATE INDEX effect_persistence_expires_idx ON ${table} (expires) WHERE expires IS NOT NULL`.pipe(Effect.catch(error => Effect.flatMap(indexExists, exists => exists ? Effect.void : Effect.fail(error))));
    }).pipe(Effect.orDie),
    // sqlite
    orElse: () => sql`CREATE INDEX IF NOT EXISTS effect_persistence_expires_idx ON ${table} (expires) WHERE expires IS NOT NULL`.pipe(Effect.orDie, Effect.asVoid)
  });
  ...
  const deleteExpiredBatch = sql.onDialectOrElse({
    ...
    mysql: () => Effect.fnUntraced(function* (expiresAtOrBefore) {
  ...
  const deleteExpired = Effect.gen(function* () {
    ...
    return yield* deleteExpiredBatch(expiresAtOrBefore).pipe(Effect.repeat({
      while: deletedCount => deletedCount === sqlCleanupBatchSize,
      schedule: Schedule.spaced(cleanupBatchDelay)
    }));
  });
  yield* deleteExpired.pipe(Effect.catch(cause => Effect.logWarning("Failed to clean up expired persistence entries", cause)), Effect.repeat(Schedule.spaced(cleanupInterval)), Effect.forkScoped);
  const upsert = sql.onDialectOrElse({
    pg: () => entries => sql`
        INSERT INTO ${table} ${sql.insert(entries)}
  ...
  return BackingPersistence.of({
    make: prefix => Effect.sync(() => {
      ...
      return identity({
        ...
        set: (key, value, ttl) => Effect.mapError(ttl === undefined ? redis.send("SET", prefixed(key), JSON.stringify(value)) : redis.send("SET", prefixed(key), JSON.stringify(value), "PX", String(Math.ceil(Duration.toMillis(ttl)))), ({
          cause
        }) => new PersistenceError({
        ...
        clear: redis.send("KEYS", `${prefix}:*`).pipe(Effect.flatMap(keys => keys.length === 0 ? Effect.void : redis.send("DEL", ...keys)), Effect.mapError(({
          cause
        }) => new PersistenceError({
