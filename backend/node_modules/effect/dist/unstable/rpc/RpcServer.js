...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (group, options) {
  ...
  const getSchemas = rpc => {
    ...
    if (!schemas) {
      ...
      schemas = {
        decode: Schema.decodeUnknownEffect(codecFor(rpc.payloadSchema)),
        encodeChunk: Schema.encodeUnknownEffect(codecFor(Schema.NonEmptyArray(Option.isSome(streamSchemas) ? streamSchemas.value.success : Schema.Any))),
        encodeExit: Schema.encodeUnknownEffect(codecFor(Rpc.exitSchema(rpc))),
        encodeDefect: Schema.encodeUnknownEffect(codecFor(rpc.defectSchema)),
  ...
  const sendDefect = (client, defect) => Effect.catchCause(send(client.id, ResponseDefectEncoded(encodeDefectUnsafe(defect))), cause => Effect.annotateLogs(Effect.logDebug(cause), {
    module: "RpcServer",
    method: "sendDefect"
  }));
  // main server loop
 ...
 * @since 4.0.0
 */
export const makeProtocolWithHttpEffect = /*#__PURE__*/Effect.fnUntraced(function* (options = {}) {
  ...
  const httpEffect = Effect.gen(function* () {
    ...
    const queue = yield* Queue.make({
      capacity: includesFraming && options.streamBufferSize !== "unbounded" ? options.streamBufferSize ?? 16 : undefined
    });
    const parser = serialization.makeUnsafe();
    const requestIds = [];
    const offer = data => typeof data === "string" ? Queue.offer(queue, encoder.encode(data)) : Queue.offer(queue, data);
    const client = {
      write: !includesFraming ? response =>
      // buffered responses cannot carry notifications, so they are dropped
      response._tag === "Request" && response.isNotification === true ? Effect.void : Queue.offer(queue, response) : response => {
        try {
...
export const makeProtocolHttp = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  const {
    httpEffect,
    protocol
  } = yield* makeProtocolWithHttpEffect(options);
  const router = yield* HttpRouter.HttpRouter;
  ...
  const onSocket = function* (socket, headers) {
    ...
    const write = response => {
      try {
        ...
        return Effect.orDie(writeRaw(parser.encode(ResponseDefectEncoded(encodeDefectUnsafe(cause)))));
      }
    };
    clients.set(id, {
      write
