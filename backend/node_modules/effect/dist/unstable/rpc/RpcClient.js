...
export const makeNoSerialization = /*#__PURE__*/Effect.fnUntraced(function* (group, options) {
  ...
  const getRpcClientMiddleware = rpc => {
    ...
    for (const tag of rpc.middlewares.values()) {
      const middleware = Context.getOrUndefinedUnsafe(services, `${tag.key}/Client`);
...
const makeRpcSchemas = codecFor => {
  ...
  return rpc => {
    ...
    entry = {
      decodeChunk: Option.map(streamSchemas, streamSchemas => Schema.decodeUnknownEffect(codecFor(Schema.NonEmptyArray(streamSchemas.success)))),
      encodePayload: Schema.encodeEffect(codecFor(rpc.payloadSchema)),
      decodeExit: Schema.decodeUnknownEffect(codecFor(Rpc.exitSchema(rpc)))
    };
    cache.set(rpc, entry);
    return entry;
  };
