...
const remoteRetrySchedule = /*#__PURE__*/Schedule.min([/*#__PURE__*/Schedule.exponential(200, 1.5), /*#__PURE__*/Schedule.spaced({
  seconds: 10
})]);
const make = /*#__PURE__*/Effect.gen(function* () {
  ...
  const runRemote = Effect.fnUntraced(function* (remote) {
    ...
    }).pipe(Effect.scoped, Effect.catchCause(Effect.logError), Effect.repeat(remoteRetrySchedule), Effect.annotateLogs({
      service: "EventLog",
    ...
    const writeUntilSuccess = write.pipe(Effect.tapCause(Effect.logDebug), Effect.retry(remoteRetrySchedule));
    yield* Effect.addFinalizer(() => Effect.ignore(write));
    const changesSub = yield* journal.changes;
    const changes = yield* Queue.dropping(1);
    yield* PubSub.takeAll(changesSub).pipe(Effect.andThen(Queue.offer(changes, undefined)), Effect.forever, Effect.forkScoped);
    yield* writeUntilSuccess;
    return yield* Queue.take(changes).pipe(Effect.andThen(writeUntilSuccess), Effect.catchCause(Effect.logError), Effect.forever);
