...
export const makeTestClient = /*#__PURE__*/Effect.fnUntraced(function* (entity, layer) {
  ...
  const sharding = shardingTag.of({
    ...
    registerEntity: (entity, handlers, options) => Effect.contextWith(context => {
      entityMap.set(entity.type, {
        ...
        build: entity.protocol.toHandlers(handlers)
      });
      return Effect.void;
    })
  });
  ...
  const map = yield* ResourceMap.make(Effect.fnUntraced(function* (entityId) {
    ...
    const handlerContext = entityEntry.context.pipe(Context.add(CurrentRunnerAddress, runnerAddress), Context.add(CurrentAddress, address), Context.add(Scope, scope));
    const handlers = yield* entityEntry.build.pipe(Effect.setContext(handlerContext));
    ...
    }).pipe(Effect.setContext(Context.merge(handlerContext, handlers)));
