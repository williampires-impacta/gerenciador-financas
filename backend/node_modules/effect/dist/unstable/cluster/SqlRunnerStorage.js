...
const postgresLockNamespace = prefix => {
...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  const acquireLockConn = sql.onDialectOrElse({
    pg: () => Effect.fnUntraced(function* (scope) {
      ...
      const pid = disableAdvisoryLocks ? 0 : (yield* conn.executeValues("SELECT pg_backend_pid()", []))[0][0];
      if (!disableAdvisoryLocks) {
        yield* Scope.addFinalizerExit(scope, () => conn.executeRaw("SELECT pg_advisory_unlock_all()", []).pipe(Effect.timeout(lockOperationInterval), Effect.interruptible, Effect.ignoreCause));
      }
      return [conn, pid];
    }, Effect.orDie),
    mysql: () => Effect.fnUntraced(function* (scope) {
      const conn = yield* Effect.orDie(sql.reserve).pipe(Scope.provide(scope));
      if (disableAdvisoryLocks) return [conn, 0];
      ...
      if (!disableAdvisoryLocks) {
        yield* Scope.addFinalizerExit(scope, () => conn.executeRaw("SELECT RELEASE_ALL_LOCKS()", []).pipe(Effect.timeout(lockOperationInterval), Effect.interruptible, Effect.ignoreCause));
      }
      return [conn, pid];
    }, Effect.orDie),
    orElse: () => undefined
  ...
  const withDeadline = Effect.fnUntraced(function* (operation) {
    const fiber = yield* Effect.forkIn(operation, layerScope, {
      startImmediately: true
    });
    return yield* Fiber.join(fiber).pipe(Effect.timeout(lockOperationInterval), Effect.ensuring(Effect.suspend(() => fiber.pollUnsafe() !== undefined ? Effect.void : Fiber.interrupt(fiber).pipe(Effect.forkIn(layerScope, {
      startImmediately: true
    }), Effect.asVoid))));
  });
  ...
  const rebuildLockConn = generation => {
    if (!lockConn || lockConnRebuilding || generation !== lockConnGeneration || lockConn.state.current._tag === "Closed") return Effect.void;
    ...
    return withDeadline(lockConn.rebuildUnsafe()).pipe(Effect.exit, Effect.tap(exit => Effect.sync(() => {
  ...
  // replaced again.
  const onErrorRebuildLockConn = effect => Effect.suspend(() => {
  ...
  yield* sql.onDialectOrElse({
    mssql: () => sql`
      ...
      `,
    mysql: () => disableAdvisoryLocks ? sql`
  ...
  const execWithLockConn = effect => {
    ...
    return lockConn.await.pipe(Effect.flatMap(([conn]) => conn.executeRaw(query, params)), onErrorRebuildLockConn);
  };
  const execWithLockConnUnprepared = effect => {
    if (!lockConn) return effect.values;
    const [query, params] = effect.compile();
    return lockConn.await.pipe(Effect.flatMap(([conn]) => conn.executeUnprepared(query, params, undefined)), onErrorRebuildLockConn);
  };
  const execWithLockConnValues = effect => {
    if (!lockConn) return effect.values;
    const [query, params] = effect.compile();
    return lockConn.await.pipe(Effect.flatMap(([conn]) => conn.executeValues(query, params)), onErrorRebuildLockConn);
  };
...
`.pipe(execWithLockConn, Effect.andThen(acquiredLocks(address, shardIds)));
        };
      }
      return Effect.fnUntraced(function* (_address, shardIds) {
        ...
        const takenLocks = yield* conn.executeValues(`SELECT objid FROM pg_locks WHERE locktype = 'advisory' AND granted = true AND classid = ${pgLockNamespaceOid} AND objsubid = 2 AND pid = ${pid} ORDER BY objid`, []);
...
`.pipe(execWithLockConnUnprepared, Effect.andThen(acquiredLocks(address, shardIds)));
        };
      }
      return Effect.fnUntraced(function* (_address, shardIds) {
        const [conn, pid] = yield* lockConn.await;
  ...
  const pgLocks = shardIdsMap => Array.from(shardIdsMap.entries(), ([lockNum, shardId]) => `pg_try_advisory_lock(${pgLockNamespace}, ${lockNum}) AS "${shardId}"`).join(", ");
  ...
  const withLockOperationDeadline = operation => onErrorRebuildLockConn(withDeadline(operation));
  const releaseShard = sql.onDialectOrElse({
    pg: () => {
      if (disableAdvisoryLocks) {
        return (address, shardId) => sql`DELETE FROM ${locksTableSql} WHERE address = ${address} AND shard_id = ${shardId}`.pipe(execWithLockConn);
      }
      return Effect.fnUntraced(function* (_address, shardId) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          const takenLocks = yield* conn.executeValues(`SELECT 1 FROM pg_locks WHERE locktype = 'advisory' AND granted = true AND classid = ${pgLockNamespaceOid} AND objid = ${lockNum} AND objsubid = 2 AND pid = pg_backend_pid()`, []);
          if (takenLocks.length === 0) return;
        }
        const [conn] = yield* lockConn.await;
        yield* conn.executeRaw(`SELECT pg_advisory_unlock_all()`, []);
      }, onErrorRebuildLockConn, Effect.asVoid);
    },
    mysql: () => {
      if (disableAdvisoryLocks) {
        return (address, shardId) => sql`DELETE FROM ${locksTableSql} WHERE address = ${address} AND shard_id = ${shardId}`.pipe(execWithLockConn);
    ...
    orElse: () => (address, shardId) => sql`DELETE FROM ${locksTableSql} WHERE address = ${address} AND shard_id = ${shardId}`
  });
  const releaseAllShards = sql.onDialectOrElse({
    pg: () => address => disableAdvisoryLocks ? sql`DELETE FROM ${locksTableSql} WHERE address = ${address}`.pipe(execWithLockConn) : sql`SELECT pg_advisory_unlock_all()`.pipe(execWithLockConn, Effect.asVoid),
    mysql: () => address => disableAdvisoryLocks ? sql`DELETE FROM ${locksTableSql} WHERE address = ${address}`.pipe(execWithLockConn) : sql`SELECT RELEASE_ALL_LOCKS()`.pipe(execWithLockConn, Effect.asVoid),
    orElse: () => address => sql`DELETE FROM ${locksTableSql} WHERE address = ${address}`
  });
  return RunnerStorage.makeEncoded({
    ...
    acquire: (address, shardIds) => withLockOperationDeadline(acquireLock(address, shardIds)).pipe(PersistenceError.refail, withTracerDisabled),
    refresh: (address, shardIds) => withLockOperationDeadline(sql`UPDATE ${runnersTableSql} SET last_heartbeat = ${sqlNow} WHERE address = ${address}`.pipe(execWithLockConn, shardIds.length > 0 ? Effect.andThen(refreshShards(address, shardIds)) : Effect.as([]))).pipe(PersistenceError.refail, withTracerDisabled),
    release: (address, shardId) => withLockOperationDeadline(releaseShard(address, shardId)).pipe(Effect.asVoid, PersistenceError.refail, withTracerDisabled),
    releaseAll: address => withLockOperationDeadline(releaseAllShards(address)).pipe(Effect.asVoid, PersistenceError.refail, withTracerDisabled)
  });
}, withTracerDisabled);
/**
