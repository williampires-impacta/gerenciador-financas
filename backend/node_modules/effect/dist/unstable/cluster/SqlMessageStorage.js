 ...
 * @since 4.0.0
 */
export const makeEncoded = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  const limitFragment = sql.onDialectOrElse({
    mssql: () => limit => sql.literal(`OFFSET 0 ROWS FETCH NEXT ${Math.floor(limit)} ROWS ONLY`),
    orElse: () => limit => sql.literal(`LIMIT ${Math.floor(limit)}`)
  });
  const groupAddresses = addresses => {
    ...
    return Array.from(byShard, ([shardId, byEntityType]) => Array.from(byEntityType, ([entityType, addresses]) => ({
      shardId,
      entityType,
      addresses
    }))).flat();
  };
  const unprocessedFilters = options => ({
    addressFilter: options?.addresses !== undefined ? sql`AND (${sql.or(groupAddresses(options.addresses).map(group => sql.and([sql`m.shard_id = ${group.shardId}`, sql`m.entity_type = ${group.entityType}`, sql.in("m.entity_id", group.addresses.map(address => address.entityId))])))})` : emptyFragment,
  ...
  const getUnprocessedMessagesForDialect = sql.onDialectOrElse({
    pg: () => (shardIds, now, filters) => sql`
      ...
      `,
    orElse: () => (shardIds, now, filters) => sql`
  ...
  const getUnprocessedMessages = (shardIds, now, options) => options?.addresses?.length === 0 ? Effect.succeed([]) : getUnprocessedMessagesForDialect(shardIds, now, unprocessedFilters(options));
  const encoded = {
    saveEnvelope: ({
      deliverAt,
      envelope,
    ...
    repliesForUnfiltered: requestIds => sql`
        ...
        ORDER BY rowid ASC
      `.unprepared.pipe(Effect.provideService(SqlClient.SafeIntegers, true), Effect.map(Arr.map(replyFromRow)), PersistenceError.refail, withTracerDisabled),
    unprocessedMessages: Effect.fnUntraced(function* (shardIds, now, options) {
    ...
    resetAddresses: addresses => addresses.length === 0 ? Effect.void : sql`
        UPDATE ${messagesTableSql}
        SET last_read = NULL
        WHERE processed = ${sqlFalse}
        AND (${sql.or(groupAddresses(addresses).map(group => sql.and([sql`shard_id = ${group.shardId}`, sql`entity_type = ${group.entityType}`, sql.in("entity_id", group.addresses.map(address => address.entityId))])))})
        `.pipe(Effect.asVoid, PersistenceError.refail, withTracerDisabled),
 ...
 * @since 4.0.0
 */
export const make = options => Effect.flatMap(makeEncoded(options), MessageStorage.makeEncoded);
/**
