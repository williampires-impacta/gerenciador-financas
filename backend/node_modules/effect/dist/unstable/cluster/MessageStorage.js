...
export const makeEncoded = /*#__PURE__*/Effect.fnUntraced(function* (encoded) {
  ...
  const storage = yield* make({
    ...
    unprocessedMessages(shardIds, options) {
      ...
      return Effect.flatMap(Effect.suspend(() => encoded.unprocessedMessages(shards, clock.currentTimeMillisUnsafe(), options)), messages => decodeMessages(storage, messages));
    },
...
export class MemoryDriver extends /*#__PURE__*/Context.Service()("effect/cluster/MessageStorage/MemoryDriver", {
  make: /*#__PURE__*/Effect.gen(function* () {
    ...
    const journal = [];
    const addressKey = address => `${address.shardId.group}/${address.shardId.id}/${address.entityType}/${address.entityId}`;
    const resetAddresses = addresses => addresses.length === 0 ? Effect.void : Effect.sync(() => {
    ...
    const encoded = {
      ...
      resetShards: shardIds => Effect.sync(() => {
