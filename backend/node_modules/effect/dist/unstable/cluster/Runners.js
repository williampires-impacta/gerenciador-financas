...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  return Runners.of({
    ...
    sendLocal(options) {
      ...
      return Message.serialize(message, codecFor).pipe(Effect.flatMap(encoded => Message.deserializeLocal(message, encoded, codecFor)), Effect.flatMap(options.send), Effect.catchTag("MalformedMessage", error => {
        if (message._tag === "OutgoingEnvelope") {
...
export const makeRpc = /*#__PURE__*/Effect.gen(function* () {
  ...
  const clients = yield* RcMap.make({
    lookup: address => Effect.flatMap(clientProtocol.make(address), protocol => Effect.map(Effect.provideService(makeRpcClient, RpcClient_.Protocol, protocol), client => ({
      client,
  ...
  return yield* make({
    ...
    ping(address) {
      return RcMap.get(clients, address).pipe(Effect.flatMap(({
        client
      }) => client.Ping()), Effect.catchCause(() => Effect.andThen(RcMap.invalidate(clients, address), Effect.fail(new RunnerUnavailable({
        address
      })))), Effect.scoped);
    },
    send({
      ...
      const respondMalformed = error => message.respond(new Reply.WithExit({
        id: snowflakeGen.nextUnsafe(),
        requestId: message.envelope.requestId,
        exit: Exit.die(error)
      }));
      ...
      if (!isStream) {
        return RcMap.get(clients, address).pipe(Effect.flatMap(({
          ...
          onSuccess: request => client.Effect({
            request,
            persisted: isPersisted
          }).pipe(Effect.flatMap(reply => Schema.decodeEffect(Reply.Reply(message.rpc, codecFor))(reply).pipe(Effect.provideContext(message.context), Effect.orDie)), Effect.flatMap(message.respond), Effect.catchTag("RpcClientError", () => Effect.fail(new RunnerUnavailable({
            address
    ...
    notify({
      ...
      const notify = RcMap.get(clients, address.value).pipe(Effect.flatMap(({
        client,
        codecFor
      }) => {
        const encode = message._tag === "OutgoingRequest" ? Effect.orDie(Message.serializeRequest(message, codecFor)) : Effect.succeed(envelope);
        return Effect.flatMap(encode, envelope => client.Notify({
          envelope,
          persisted: isPersisted
        }));
      }), Effect.scoped, Effect.catchTag("RpcClientError", () => Effect.fail(new RunnerUnavailable({
