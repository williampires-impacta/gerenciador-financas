 ...
 * @since 4.0.0
 */
export const fromGlob = migrations => pipe(Object.keys(migrations), Arr.flatMapNullishOr(_ => _.match(/^(?:.*\/)?(\d+)_([^.]+)\.(js|ts|mjs|mts)$/)), Arr.map(([key, id, name]) => [Number(id), name, Effect.promise(() => migrations[key]())]), Arr.sort(migrationOrder), Effect.succeed);
/**
 ...
 * @since 4.0.0
 */
export const fromBabelGlob = migrations => pipe(Object.keys(migrations), Arr.flatMapNullishOr(_ => _.match(/^_(\d+)_([^.]+?)(Js|Ts|Mjs|Mts)?$/)), Arr.map(([key, id, name]) => [Number(id), name, Effect.succeed(migrations[key])]), Arr.sort(migrationOrder), Effect.succeed);
/**
...
export const fromFileSystem = /*#__PURE__*/Effect.fnUntraced(function* (directory) {
  ...
  return files.map(file => Option.fromNullishOr(file.match(/^(?:.*\/)?(\d+)_([^.]+)\.(js|ts|mjs|mts)$/))).flatMap(Option.match({
    onNone: () => [],
    onSome: ([basename, id, name]) => [[Number(id), name,
    ...
    Effect.flatMap(Effect.orDie(path.toFileUrl(path.join(directory, basename))), url => Effect.promise(() => import(__rewriteRelativeImportExtension(/* @vite-ignore */
    /* webpackIgnore: true */
    url.href))))]]
  })).sort(([a], [b]) => a - b);
});
