...
const makeTable = () => Object.create(null);
const isTable = value => typeof value === "object" && value !== null && !Array.isArray(value) && !(value instanceof Date);
class TomlParser {
  ...
  parseDate(token) {
    ...
    if (new RegExp(`^${date}[Tt]${time}$`).test(token) || new RegExp(`^${date}$`).test(token) || new RegExp(`^${time}$`).test(token)) {
 ...
 * @since 4.0.0
 */
export const parse = input => new TomlParser(input.replace(/^\uFEFF/, "")).parse();
