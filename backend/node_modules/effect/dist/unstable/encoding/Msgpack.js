...
export const decode = () => Channel.fromTransform((upstream, _scope) => Effect.sync(() => {
  ...
  const pull = Effect.suspend(() => Pull.matchEffect(upstream, {
    onSuccess: loop,
    onFailure: Effect.failCause,
    onDone: done => incomplete === undefined ? Cause.done(done) : Effect.fail(new MsgPackError({
      kind: "Unpack",
  ...
  function loop(chunk) {
    ...
    for (let i = 0; i < chunk.length; i++) {
      ...
      try {
        ...
        if (error.incomplete) {
          incomplete = {
            bytes: buf.subarray(error.lastPosition),
            cause
          };
          if (error.values) {
