...
export const makeCommand = options => {
  ...
  const buildHelpDoc = commandPath => {
    ...
    for (const arg of config.arguments) {
      ...
      for (const single of singles) {
        args.push({
          ...
          required: !metadata.isOptional && (!metadata.isVariadic || Option.exists(metadata.variadicMin, min => min > 0)),
    ...
    for (const group of subcommands) {
      ...
      subcommandDocs.push({
        group: group.group,
        commands: Arr.map(visible, subcommand => ({
 ...
 */
export const toFlagDoc = (single, metadata) => {
 ...
 */
export const makeParser = (cfg, options) => Effect.fnUntraced(function* (input) {
  const parsedArgs = {
    flags: input.flags,
  ...
  if (options?.allowLeftovers !== true && remainingArguments.length > 0) {
    return yield* new CliError.UnexpectedArgument({
