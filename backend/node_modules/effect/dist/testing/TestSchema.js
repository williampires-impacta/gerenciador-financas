...
export class Asserts {
  ...
  make(options) {
    ...
    async function succeed(input, expected) {
      const r = await Effect.runPromise(makeEffect(input, options).pipe(Effect.mapErrorEager(SchemaIssue.defaultFormatter), Effect.result));
  ...
  verifyLosslessTransformation(options) {
    ...
    return FastCheck.assert(FastCheck.asyncProperty(arbitrary, async t => {
      const r = await Effect.runPromise(encodeEffect(t).pipe(Effect.flatMapEager(e => decodeUnknownEffect(e)), Effect.mapErrorEager(SchemaIssue.defaultFormatter), Effect.result));
...
export class Decoding {
  ...
  async succeed(input, expected) {
    const r = await Effect.runPromise(this.decodeUnknownEffect(input, this.options?.parseOptions).pipe(Effect.mapErrorEager(SchemaIssue.defaultFormatter), Effect.result));
    expected = arguments.length === 1 ? input : expected;
    assert.deepStrictEqual(r, Result.succeed(expected));
  }
  /**
  ...
  provide(service, implementation) {
    return new Decoding(pipe(this.schema, Schema.middlewareDecoding(Effect.provideService(service, implementation))), this.options);
  }
}
/**
...
export class Encoding {
  ...
  async succeed(input, expected) {
    const r = await Effect.runPromise(this.encodeUnknownEffect(input, this.options?.parseOptions).pipe(Effect.mapErrorEager(SchemaIssue.defaultFormatter), Effect.result));
    expected = arguments.length === 1 ? input : expected;
    assert.deepStrictEqual(r, Result.succeed(expected));
  }
  /**
   ...
   */
  provide(service, implementation) {
    return new Encoding(pipe(this.schema, Schema.middlewareEncoding(Effect.provideService(service, implementation))), this.options);
  }
}
