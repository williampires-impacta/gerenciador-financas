 ...
 * @since 4.0.0
 */
export function get(oldValue, newValue) {
  const patches = [];
 ...
 * @since 4.0.0
 */
export function apply(patch, oldValue) {
  let doc = oldValue;
...
function applyOperation(doc, op) {
  ...
  if (Array.isArray(parent)) {
    ...
    if (op.op === "add") updated.splice(index, 0, op.value);else if (op.op === "remove") updated.splice(index, 1);else updated[index] = op.value;
  ...
  if (isJsonObject(parent)) {
    ...
    if (op.op === "remove") delete updated[lastToken];else InternalRecord.assignProperty(updated, lastToken, op.value);
    return rebuildFromStack(stack, updated);
  }
  throw new Error(`Cannot ${op.op} at "${op.path}" (parent not found or not a container).`);
}
...
function rebuildFromStack(stack, newParent) {
  ...
  for (let i = stack.length - 1; i >= 0; i--) {
    ...
    if (Array.isArray(container)) {
      ...
      InternalRecord.assignProperty(copy, token, acc);
      acc = copy;
    }
  }
  return acc;
