...
import * as internal from "./internal/graph.js";
import * as csr from "./internal/graphCsr.js";
...
/** @internal */
const traversalRadius = (radius, defaultRadius) => {
  const value = radius ?? defaultRadius;
  if (value !== Infinity && (!Number.isInteger(value) || value < 0)) {
    throw new GraphError({
      message: "Traversal radius must be a non-negative integer or Infinity"
    });
  }
  return value;
};
...
/** @internal */
const getMutableImplForMutation = graph => {
  assertMutable(graph);
  if (internal.isTransforming(graph)) {
    throw new GraphError({
      message: "Cannot mutate graph during a transformation"
    });
  }
...
/** @internal */
const withMutationGuard = (graph, evaluate) => graph.mutable ? internal.withTransformation(graph, evaluate) : evaluate();
// =============================================================================
// Constructors
// =============================================================================
/**
...
export const fromSnapshot = snapshot => {
  if (snapshot.type !== "directed" && snapshot.type !== "undirected") {
    throw new GraphError({
      message: "Snapshot type must be directed or undirected"
    });
  }
  let previous = -1;
  ...
  for (let i = 0; i < snapshot.nodes.length; i++) {
    ...
    if (node === undefined || node === null) {
      throw new GraphError({
        message: `Node at position ${i} must be defined`
      });
    }
    const index = node.index;
    if (!Number.isSafeInteger(index) || index < 0) {
      throw new GraphError({
        message: `Node index at position ${i} must be a non-negative safe integer`
      });
    }
    if (index <= previous) {
      throw new GraphError({
        message: "Node indexes must be strictly increasing"
      });
    }
    previous = index;
  ...
  for (let i = 0; i < snapshot.edges.length; i++) {
    ...
    if (edge === undefined || edge === null) {
      throw new GraphError({
        message: `Edge at position ${i} must be defined`
      });
    }
    if (!Number.isSafeInteger(edge.index) || edge.index < 0) {
      throw new GraphError({
        message: `Edge index at position ${i} must be a non-negative safe integer`
      });
    }
    if (edge.index <= previous) {
      throw new GraphError({
        message: "Edge indexes must be strictly increasing"
      });
    }
    previous = edge.index;
    if (!Number.isSafeInteger(edge.source) || edge.source < 0) {
      throw new GraphError({
        message: `Edge source at position ${i} must be a non-negative safe integer`
      });
    }
    ...
    if (!Number.isSafeInteger(edge.target) || edge.target < 0) {
      throw new GraphError({
        message: `Edge target at position ${i} must be a non-negative safe integer`
      });
    }
...
export const make = type => mutate => {
  if (type !== "directed" && type !== "undirected") {
    throw new GraphError({
      message: "Graph type must be directed or undirected"
    });
  }
  if (mutate === undefined) {
    return internal.make(type, false);
  }
  const graph = internal.make(type, true);
...
export const complement = /*#__PURE__*/dual(2, (self, createEdge) => {
  ...
  return make(self.type)(mutable => {
    ...
    for (let i = 0; i < cache.nodeIds.length; i++) {
      newIndices[i] = addNode(mutable, cache.nodeData[i]);
    }
    for (let i = 0; i < cache.nodeIds.length; i++) {
      ...
      for (let j = start; j < cache.nodeIds.length; j++) {
        ...
        addEdge(mutable, newIndices[i], newIndices[j], createEdge(cache.nodeData[i], cache.nodeData[j]));
      }
    }
  });
});
 ...
 * @since 4.0.0
 */
export const inducedSubgraph = /*#__PURE__*/dual(2, (self, nodeIndices) => {
...
export const addNode = (mutable, data) => {
  ...
  if (!Number.isSafeInteger(nodeIndex)) {
    throw new GraphError({
      message: "Graph has exhausted safe node indexes"
    });
  }
  // Add node data
 ...
 * @since 3.18.0
 */
export const hasNode = /*#__PURE__*/dual(2, (graph, nodeIndex) => internal.toImpl(graph).nodes.has(nodeIndex));
/**
 ...
 * @since 3.18.0
 */
export const nodeCount = graph => internal.toImpl(graph).nodes.size;
/**
...
export const addEdge = (mutable, source, target, data) => {
  ...
  if (!Number.isSafeInteger(edgeIndex)) {
    throw new GraphError({
      message: "Graph has exhausted safe edge indexes"
    });
  }
  // Create edge data
  const edgeData = {
 ...
 * @since 4.0.0
 */
export const removeNodes = (mutable, nodeIndices) => {
  assertMutable(mutable);
  if (internal.isTransforming(mutable)) {
    throw new GraphError({
      message: "Cannot mutate graph during a transformation"
...
/** @internal */
const removeNodeInternal = (impl, nodeIndex) => {
 ...
 * @since 4.0.0
 */
export const removeEdges = (mutable, edgeIndices) => {
  assertMutable(mutable);
  if (internal.isTransforming(mutable)) {
    throw new GraphError({
      message: "Cannot mutate graph during a transformation"
 ...
 * @since 3.18.0
 */
export const getEdge = /*#__PURE__*/dual(2, (graph, edgeIndex) => {
 ...
 * @since 3.18.0
 */
export const edgeCount = graph => internal.toImpl(graph).edges.size;
/**
 ...
 * @since 4.0.0
 */
export const incidentEdges = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(nodeIndex)) {
    throw missingNode(nodeIndex);
  }
 ...
 * @since 4.0.0
 */
export const outgoingEdges = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot get outgoing edges of undirected graph"
    });
  }
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(nodeIndex)) {
 ...
 * @since 4.0.0
 */
export const incomingEdges = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot get incoming edges of undirected graph"
    });
  }
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(nodeIndex)) {
 ...
 * @since 4.0.0
 */
export const edgesBetween = /*#__PURE__*/dual(3, (graph, source, target) => {
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(source)) {
    throw missingNode(source);
  }
 ...
 * @since 4.0.0
 */
export const degree = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
  if (graph.type === "directed") {
    throw new GraphError({
      message: "Cannot get degree of directed graph"
    });
  }
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(nodeIndex)) {
 ...
 * @since 4.0.0
 */
export const outDegree = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot get outgoing edges of undirected graph"
    });
 ...
 * @since 4.0.0
 */
export const inDegree = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot get incoming edges of undirected graph"
    });
...
const getDirectedNeighbors = (graph, nodeIndex, direction) => {
  ...
  if (!graph.mutable) {
    ...
    if (cache !== undefined) {
      ...
      const adjacency = direction === "incoming" ? csr.getIncoming(cache) : csr.getOutgoing(cache);
      const start = adjacency.rowOffsets[node];
      const result = new Array(adjacency.rowOffsets[node + 1] - start);
...
const getUniqueDirectedNeighbors = (graph, nodeIndex, direction) => Array.from(new Set(getDirectedNeighbors(graph, nodeIndex, direction)));
/**
...
export const neighbors = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
  ...
  return getUniqueDirectedNeighbors(graph, nodeIndex, "outgoing");
});
/**
...
export const predecessors = /*#__PURE__*/dual(2, (graph, nodeIndex) => {
  ...
  return getUniqueDirectedNeighbors(graph, nodeIndex, "incoming");
});
/**
...
export const neighborsDirected = /*#__PURE__*/dual(3, (graph, nodeIndex, direction) => {
  ...
  return getUniqueDirectedNeighbors(graph, nodeIndex, direction);
});
...
export const findCycle = graph => {
  ...
  const makeCycle = (ancestor, current, closingEdge) => {
    const path = [current];
    const edges = [];
    let cursor = current;
...
export const isBipartite = graph => {
  if (graph.type === "directed") {
    throw new GraphError({
      message: "Cannot determine bipartite status of directed graph"
    });
  }
  const cache = csr.get(graph);
  const outgoing = csr.getOutgoing(cache);
...
/** @internal */
const bipartiteColors = graph => {
  if (graph.type === "directed") {
    throw new GraphError({
      message: "Cannot find bipartite matching of directed graph"
    });
  }
  const cache = csr.get(graph);
  const outgoing = csr.getOutgoing(cache);
    ...
    while (head < tail) {
      ...
      for (let i = outgoing.rowOffsets[node]; i < outgoing.rowOffsets[node + 1]; i++) {
        ...
        if (colors[neighbor] === -1) {
          ...
          throw new GraphError({
            message: "Cannot find bipartite matching of non-bipartite graph"
          });
        }
      }
    }
 ...
 * @since 4.0.0
 */
export const maximumBipartiteMatching = graph => {
  const {
    cache,
    colors
  } = bipartiteColors(graph);
...
const getUnweightedDistances = (graph, source, direction, target) => {
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(source)) {
    throw missingNode(source);
  }
 ...
 * @since 4.0.0
 */
export const unweightedDistances = /*#__PURE__*/dual(args => isGraph(args[0]), (graph, source, options) => getUnweightedDistances(graph, source, options?.direction ?? "outgoing"));
/**
 ...
 * @since 4.0.0
 */
export const hasPath = /*#__PURE__*/dual(args => isGraph(args[0]), (graph, source, target, options) => {
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(source)) {
    throw missingNode(source);
  }
  ...
  const adjacencies = csr.getAdjacencies(cache, graph.type === "undirected" ? "outgoing" : options?.direction ?? "outgoing");
  const visited = new Uint8Array(cache.nodeIds.length);
  const queue = new Uint32Array(cache.nodeIds.length);
  let head = 0;
  let tail = 1;
...
export const connectedComponents = graph => {
  if (graph.type === "directed") {
    throw new GraphError({
      message: "Cannot find connected components of directed graph"
    });
  }
  const cache = csr.get(graph);
  const outgoing = csr.getOutgoing(cache);
...
/** @internal */
const analyzeLowLinks = graph => {
  if (graph.type === "directed") {
    throw new GraphError({
      message: "Cannot analyze undirected connectivity of directed graph"
    });
  }
  const cache = csr.get(graph);
  const outgoing = csr.getOutgoingWithEdges(cache);
    ...
    while (stack.length > 0) {
      ...
      if (frame.position < end) {
        ...
        if (discovered[neighbor] === -1) {
          ...
          stack.push({
            node: neighbor,
            position: outgoing.rowOffsets[neighbor]
          });
  ...
  const resultArticulationPoints = [];
 ...
 * @since 4.0.0
 */
export const bridges = graph => analyzeLowLinks(graph).bridges;
/**
 ...
 * @since 4.0.0
 */
export const articulationPoints = graph => analyzeLowLinks(graph).articulationPoints;
/**
 ...
 * @since 4.0.0
 */
export const biconnectedComponents = graph => analyzeLowLinks(graph).biconnectedComponents;
/** @internal */
const solveMaximumFlow = (graph, config) => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot compute flow of undirected graph"
    });
  }
  const cache = csr.get(graph);
  ...
  if (source === target) {
    throw new GraphError({
      message: "Flow source and target must be different nodes"
    });
  }
  ...
  withMutationGuard(graph, () => {
    for (let edge = 0; edge < edges.length; edge++) {
      ...
      if (!Number.isFinite(capacity) || capacity < 0) {
        throw new GraphError({
          message: `Edge ${edgeIds[edge]} capacity must be a finite non-negative number`
        });
      }
  ...
  while (true) {
    ...
    if (!Number.isFinite(value + amount)) {
      throw new GraphError({
        message: "Maximum flow exceeds the finite number range"
      });
    }
    for (let node = target; node !== source;) {
  ...
  const cut = [];
  for (let edge = 0; edge < edgeIds.length; edge++) {
    if (endpoints.sources[edge] !== endpoints.targets[edge] && visited[endpoints.sources[edge]] !== 0 && visited[endpoints.targets[edge]] === 0) {
 ...
 * @since 4.0.0
 */
export const maximumFlow = /*#__PURE__*/dual(2, (graph, config) => {
  const {
    cut,
    flows,
    value
  } = solveMaximumFlow(graph, config);
  return {
    value,
    flows,
    cut
 ...
 * @since 4.0.0
 */
export const minimumCut = /*#__PURE__*/dual(2, (graph, config) => {
 ...
 * @since 4.0.0
 */
export const weaklyConnectedComponents = graph => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot find weakly connected components of undirected graph"
    });
  }
  const cache = csr.get(graph);
  const {
    primary,
    secondary
  } = csr.getAdjacencies(cache, "undirected");
...
/** @internal */
const csrReachesAll = (nodeCount, primary, secondary) => {
  if (nodeCount === 0) {
    return true;
  }
  const visited = new Uint8Array(nodeCount);
 ...
 * @since 4.0.0
 */
export const isWeaklyConnected = graph => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot find weakly connected components of undirected graph"
    });
 ...
 * @since 4.0.0
 */
export const isStronglyConnected = graph => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot find strongly connected components of undirected graph"
    });
  ...
  const cache = csr.get(graph);
  return csrReachesAll(cache.nodeIds.length, csr.getOutgoing(cache)) && csrReachesAll(cache.nodeIds.length, csr.getIncoming(cache));
};
/**
 ...
 * @since 4.0.0
 */
export const isTree = graph => {
  if (graph.type === "directed") {
    throw new GraphError({
      message: "Cannot determine tree status of directed graph"
    });
  }
 ...
 * @since 4.0.0
 */
export const minimumSpanningForest = /*#__PURE__*/dual(2, (graph, cost) => {
  if (graph.type === "directed") {
    throw new GraphError({
      message: "Cannot find minimum spanning forest of directed graph"
    });
  }
  const impl = internal.toImpl(graph);
  const nodes = [];
  ...
  withMutationGuard(graph, () => {
    for (const [index, edge] of impl.edges) {
      ...
      if (Number.isNaN(weight) || weight === -Infinity) {
        throw new GraphError({
          message: "Minimum spanning forest does not support NaN or -Infinity edge weights"
        });
      }
      if (weight !== Infinity) {
        weightedEdges.push({
 ...
 * @since 4.0.0
 */
export const transitiveReduction = graph => {
  if (graph.type === "undirected") {
    throw new GraphError({
      message: "Cannot transitively reduce undirected graph"
    });
  }
  if (!isAcyclic(graph)) {
    throw new GraphError({
      message: "Cannot transitively reduce cyclic graph"
    });
  }
  const impl = internal.toImpl(graph);
  const nodes = [];
...
const denseMinHeapMake = (capacity, indexed = false) => {
...
const denseMinHeapPush = (heap, node, priority, sequence) => {
  ...
  while (index > 0) {
    ...
    if (priority > heap.priorities[parent] || priority === heap.priorities[parent] && sequence >= heap.sequences[parent]) {
      break;
    }
...
const denseMinHeapPop = heap => {
  ...
  while (true) {
    ...
    const right = left + 1;
    let child = left;
    if (right < last && (heap.priorities[right] < heap.priorities[left] || heap.priorities[right] === heap.priorities[left] && heap.sequences[right] < heap.sequences[left])) {
      child = right;
    }
    if (heap.priorities[child] > priority || heap.priorities[child] === priority && heap.sequences[child] >= sequence) {
      break;
    }
  ...
  while (priorityQueue.size > 0) {
    ...
    for (let i = outgoing.rowOffsets[currentNode]; i < outgoing.rowOffsets[currentNode + 1]; i++) {
      ...
      if (edgeWeights[edge] !== Infinity && !Number.isFinite(nextDistance)) {
        throw new GraphError({
          message: "Dijkstra distance calculation exceeded the finite number range"
        });
      }
...
export const floydWarshall = /*#__PURE__*/dual(2, (graph, cost) => {
    ...
    for (let i = 0; i < size; i++) {
      ...
      for (let j = 0; j < size; j++) {
        ...
        if (!Number.isFinite(candidate)) {
          throw new GraphError({
            message: "Floyd-Warshall distance calculation exceeded the finite number range"
          });
        }
  ...
  for (let i = 0; i < size; i++) {
    if (distancesMatrix[i * size + i] < 0) {
      throw new GraphError({
        message: `Negative cycle detected involving node ${cache.nodeIds[i]}`
      });
    }
  }
  ...
  for (let i = 0; i < size; i++) {
    ...
    const distanceRow = new Map();
    const pathRow = new Map();
    const edgePathRow = new Map();
    const costRow = new Map();
...
export const astar = /*#__PURE__*/dual(2, (graph, config) => {
  ...
  const source = csr.getNodeIndex(cache, config.source);
  const target = csr.getNodeIndex(cache, config.target);
  const sourceNodeData = cache.nodeData[source];
  const targetNodeData = cache.nodeData[target];
  const edgeWeights = new Float64Array(cachedEdges.length);
  withMutationGuard(graph, () => {
    for (let i = 0; i < cachedEdges.length; i++) {
      const weight = config.cost(cachedEdges[i].data);
  ...
  if (config.source === config.target) {
    if (!Number.isFinite(withMutationGuard(graph, () => config.heuristic(sourceNodeData, targetNodeData)))) {
      throw new GraphError({
        message: "A* algorithm requires finite heuristic values"
      });
    }
    return Option.some({
      path: [config.source],
  ...
  while (openSet.size > 0) {
    ...
    for (let i = outgoing.rowOffsets[current]; i < outgoing.rowOffsets[current + 1]; i++) {
      ...
      if (edgeWeights[edge] !== Infinity && !Number.isFinite(tentativeScore)) {
        throw new GraphError({
          message: "A* distance calculation exceeded the finite number range"
        });
      }
      if (tentativeScore < scores[neighbor]) {
        ...
        previousEdge[neighbor] = edge;
        const priority = tentativeScore + getHeuristic(cache.nodeData[neighbor]);
        if (!Number.isFinite(priority)) {
          throw new GraphError({
            message: "A* priority calculation exceeded the finite number range"
          });
        }
...
export const bellmanFord = /*#__PURE__*/dual(2, (graph, config) => {
  ...
  const addWeight = (distance, weight) => {
    ...
    if (!Number.isFinite(candidate)) {
      throw new GraphError({
        message: "Bellman-Ford distance calculation exceeded the finite number range"
      });
    }
    return candidate;
  };
  ...
  for (let edge = 0; edge < edges.length; edge++) {
    ...
    if (graph.type === "undirected" && edgeSource !== edgeTarget && addWeight(distances[edgeTarget], weight) < distances[edgeSource]) {
  ...
  if (affected[target] !== 0) {
    throw new GraphError({
      message: `Negative cycle affects path to node ${config.target}`
    });
  }
  if (distances[target] === Infinity) {
    return Option.none();
...
const pathEnumerationLimit = limit => {
  const value = limit ?? Infinity;
  if (value !== Infinity && (!Number.isInteger(value) || value < 0)) {
    throw new GraphError({
      message: "Path enumeration limit must be a non-negative integer or Infinity"
    });
  }
  return value;
};
const pathWalker = iterator => ({
 ...
 * @since 4.0.0
 */
export const simplePaths = /*#__PURE__*/dual(2, (graph, config) => {
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(config.source)) {
    throw missingNode(config.source);
  }
 ...
 * @since 4.0.0
 */
export const allShortestPaths = /*#__PURE__*/dual(2, (graph, config) => {
  const impl = internal.toImpl(graph);
  if (!impl.nodes.has(config.source)) {
    throw missingNode(config.source);
  }
  ...
  return pathWalker(function* () {
    ...
    withMutationGuard(graph, () => {
      for (let edge = 0; edge < graphEdges.length; edge++) {
        ...
        if (Number.isNaN(weight) || weight < 0) {
          throw new GraphError({
            message: "All shortest paths requires non-negative edge weights"
          });
        }
        weights[edge] = weight;
      }
    ...
    distances.fill(Infinity);
    distances[source] = 0;
    const previous = new Array(cache.nodeIds.length);
    ...
    while (queue.size > 0) {
      ...
      for (let i = outgoing.rowOffsets[currentNode]; i < outgoing.rowOffsets[currentNode + 1]; i++) {
        ...
        if (weights[edge] !== Infinity && !Number.isFinite(nextDistance)) {
          throw new GraphError({
            message: "All shortest paths distance calculation exceeded the finite number range"
          });
        }
    ...
    while (stack.length > 0 && emitted < limit) {
      ...
      if (frame.node === source) {
        ...
        yield {
          ...
          distance,
          costs: reverseEdges.map(edge => graphEdges[edge].data).reverse()
        };
        backtrack();
        continue;
      }
...
const makeCsrNodeWalker = (graph, makeIterator) => {
  return new Walker(f => ({
    // Capture CSR at iterator creation so invalidation cannot change an in-flight mutable traversal.
    [Symbol.iterator]: () => makeIterator(csr.get(graph), (index, data) => withMutationGuard(graph, () => f(index, data)))
  }));
};
const traversalStarts = (graph, start) => {
  if (start === undefined) {
    return [];
  }
...
const traversalStartPositions = (cache, start) => {
  const positions = new Array(start.length);
...
export const dfs = /*#__PURE__*/dual(args => isGraph(args[0]), (graph, config = {}) => {
  ...
  return makeCsrNodeWalker(graph, (cache, f) => {
    ...
    if (radius === Infinity) {
      ...
      const pushNeighbors = (targets, offsets, current) => {
        ...
        next() {
          while (stack.length > 0) {
            ...
            return {
              ...
              value: f(cache.nodeIds[current], cache.nodeData[current])
            };
          }
          return {
            done: true,
    ...
    const enqueue = (targets, offsets, current, depth) => {
...
export const bfs = /*#__PURE__*/dual(args => isGraph(args[0]), (graph, config = {}) => {
  ...
  return makeCsrNodeWalker(graph, (cache, f) => {
    ...
    if (radius === Infinity) {
      return {
        next() {
          ...
          if (view.secondary !== undefined) {
            enqueue(view.secondary.columnIndices, view.secondary.rowOffsets[current], view.secondary.rowOffsets[current + 1]);
          }
          return {
            done: false,
            value: f(cache.nodeIds[current], cache.nodeData[current])
    ...
    const depths = new Uint32Array(cache.nodeIds.length);
    const enqueueBounded = (targets, from, to, depth) => {
      for (let i = from; i < to; i++) {
        const neighbor = targets[i];
        if (discovered[neighbor] === 0) {
          discovered[neighbor] = 1;
    ...
    return {
      next() {
        ...
        const depth = depths[head++];
        if (depth < radius) {
          enqueueBounded(view.primary.columnIndices, view.primary.rowOffsets[current], view.primary.rowOffsets[current + 1], depth);
          if (view.secondary !== undefined) {
            enqueueBounded(view.secondary.columnIndices, view.secondary.rowOffsets[current], view.secondary.rowOffsets[current + 1], depth);
          }
        }
        return {
          done: false,
...
export const topo = /*#__PURE__*/dual(args => isGraph(args[0]), (graph, config = {}) => {
  ...
  return makeCsrNodeWalker(graph, (cache, f) => {
    ...
    for (const initial of initials) {
      ...
      if (inDegree[node] !== 0) {
        throw new GraphError({
          message: `Initial node ${initial} has incoming edges`
        });
      }
      initialSet[node] = 1;
      queue.push(node);
    ...
    return {
      next() {
        ...
        return {
          ...
          value: withMutationGuard(graph, () => f(nodeIndex, nodeData))
        };
      }
    };
  }
    ...
    return {
      next() {
        ...
        return {
          ...
          value: withMutationGuard(graph, () => f(edgeIndex, copyEdge(edgeData)))
        };
      }
    };
  }
