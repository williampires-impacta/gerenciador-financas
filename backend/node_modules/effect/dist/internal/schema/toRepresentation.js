...
/** @internal */
export function toRepresentation(ast, options) {
  const {
    references,
    representations
...
/** @internal */
export function toRepresentations(asts, options) {
  const references = {};
  ...
  function annotateReference(ast, candidate, reference) {
    ...
    if (fallback !== undefined) {
      return InternalAnnotations.resolveIdentifierFallback(ast) === fallback ? ast : SchemaAST.annotate(ast, {
        [InternalAnnotations.IDENTIFIER_FALLBACK_KEY]: fallback
      });
    }
    return reference === candidate.identifier ? ast : SchemaAST.annotate(ast, {
  ...
  function getCandidate(input) {
    ...
    const fallback = identifier === undefined ? (ast !== input ? InternalAnnotations.resolveIdentifier(input) : undefined) ?? InternalAnnotations.resolveIdentifierFallback(ast) : undefined;
...
//# sourceMappingURL=toRepresentation.js.map