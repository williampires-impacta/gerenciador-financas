...
function compileJsonSchema(representations, rootPaths, references, options) {
  ...
  function compileCheck(check, type, path) {
    ...
    if (callback !== undefined) {
      ...
      const schema = ordinary === undefined ? fragment : {
        ...fragment,
        ...ordinary
      };
      const allowed = ordinary === undefined ? inlineableCheckKeywords : inlineableAnnotatedCheckKeywords;
      return check._tag === "Filter" && hasOnlyKeywords(schema, allowed) && (ordinary === undefined || hasOnlyKeywords(ordinary, promotableAnnotationKeywords)) ? [schema, true] : [schema];
    }
          ...
          for (let index = 0; index < representation.indexSignatures.length; index++) {
            ...
            if (patterns.length === 0) {
              ...
              for (const pattern of patterns) {
                ...
                InternalRecord.assignProperty(patternProperties, pattern, previous === undefined ? type : previous === false || type === false ? false : appendJsonSchema(previous, type));
              }
            }
          }
