...
/** @internal */
export const toEquivalence = /*#__PURE__*/memoize(ast => {
  return recur(ast, []);
});
function recur(ast, path) {
  ...
  // handle annotations
  // ---------------------------------------------
  const annotation = InternalAnnotations.resolve(ast)?.["toEquivalence"];
  if (annotation) {
    ...
    case "Union":
      {
        ...
        const compiled = new Map(types.map((candidate, i) => [candidate, [SchemaParser._is(candidate), recur(ast.types[i], path)]]));
        return Equivalence.make((a, b) => {
...
//# sourceMappingURL=toEquivalence.js.map