...
import { afterAll as vitestAfterAll, afterEach as vitestAfterEach, beforeAll as vitestBeforeAll, beforeEach as vitestBeforeEach, } from "vitest";
import * as Core from "./Core.js";
export { executeWhenReady, getWhenReady, guardContentType, guardedFetchLayer, rpcClientLayer, WorkerNotReady, } from "./Http.js";
const timeoutOf = (opts) => typeof opts === "number" ? opts : opts?.timeout;
...
export const make = (options) => {
    ...
    const wrap = (eff) => Core.toEffect(eff, options, sharedScope, sidecar);
    const runEff = (eff) => Core.run(eff, options, sharedScope, sidecar);
    ...
    test.only = (name, eff, opts) => {
        it.only(name, () => wrap(eff), timeoutOf(opts));
    };
    test.todo = (name, _eff, _opts) => {
        it.todo(name);
    };
    const wrapProvider = (name, fn) => {
        ...
        const body = Core.withProviders(fn(scratch), options, scratch.name).pipe(Effect.ensuring(scratch.destroy().pipe(Effect.ignore)));
    ...
    return {
        ...
        destroy: (stack, callOpts) => Core.destroy(options, stack, { ...callOpts, scope: sharedScope }).pipe(Effect.ensuring(closeScope)),
    };
};
