 ...
 */
const isColdStartStatus = (status) => status === 404 || status >= 500;
/**
 ...
 */
export const executeWhenReady = (request, options) => Effect.gen(function* () {
    const client = yield* HttpClient.HttpClient;
    return yield* client.execute(request).pipe(Effect.flatMap((response) => isColdStartStatus(response.status)
        ? Effect.fail(new WorkerNotReady({ status: response.status }))
        : Effect.succeed(response)), Effect.retry({
 ...
 */
export const getWhenReady = (url, options) => executeWhenReady(HttpClientRequest.get(url), options);
 ...
 */
export const guardContentType = (contentType, options) => (client) => client.pipe(HttpClient.transform((effect, request) => Effect.flatMap(effect, (response) => {
    const observed = response.headers["content-type"] ?? "";
    if (observed.includes(contentType)) {
        return Effect.succeed(response);
    }
    return response.text.pipe(Effect.orElseSucceed(() => "<unreadable body>"), Effect.flatMap((body) => Effect.fail(new HttpClientError({
 ...
 */
export const guardedFetchLayer = (contentType, options) => Layer.effect(HttpClient.HttpClient)(Effect.gen(function* () {
    const client = yield* HttpClient.HttpClient;
 ...
 * ```
 */
export const rpcClientLayer = (url, options) => {
    const serialization = options?.serialization ?? RpcSerialization.ndjson;
    return RpcClient.layerProtocolHttp({ url }).pipe(Layer.provide(options?.guard === false
        ? FetchHttpClient.layer
        : guardedFetchLayer(serialization.contentType, options)), Layer.provide(Layer.succeed(RpcSerialization.RpcSerialization, serialization)));
};
