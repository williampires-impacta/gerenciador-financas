 ...
 * @resource
 */
export const Bucket = Resource("Prisma.Bucket");
/**
...
const attrsFrom = (bucket) => ({
    bucketId: bucket.id,
    name: bucket.name,
    projectId: bucket.project.id,
    createdAt: bucket.createdAt,
});
const ProviderLive = () => Provider.effect(Bucket, Effect.gen(function* () {
...
const ProviderLocal = () => devProvider(Bucket, ["bucketId"], ({ id, news }) => ({
    bucketId: devId("bucket", id),
    name: news.name ?? id,
    projectId: attrOrString(news.project, "projectId") ?? devId("project", id),
    createdAt: DEV_TIMESTAMP,
}));
export const BucketProvider = () => ProviderLayer.dual(Bucket, {
    local: () => ProviderLocal(),
    live: () => ProviderLive(),
});
