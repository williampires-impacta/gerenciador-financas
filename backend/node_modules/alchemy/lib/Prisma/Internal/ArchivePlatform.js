...
import { verifiedFileChunks } from "./ArtifactFile.js";
export const closeDirectoryHandle = async (handle) => {
    try {
        await handle.close();
    }
    catch {
...
const entryLimitError = (maxEntries) => new Error(`Compute artifact exceeds the ${maxEntries} entry safety limit. Narrow the artifact directory or add ignore patterns.`);
export const readDirectoryEntriesSecure = (options) => Effect.tryPromise({
    try: async () => {
        const before = await lstat(options.directory, { bigint: true });
        if (!before.isDirectory() || before.isSymbolicLink()) {
            throw new Error(`Compute artifact directory changed before traversal: ${options.relativePrefix || "."}`);
        }
        ...
        try {
            for await (const entry of handle) {
                if (entry.name.includes("\\")) {
                    throw new Error(`Compute artifact entry contains an unsupported backslash: ${options.relativePrefix || "."}`);
                }
                ...
                const relativeName = options.relativePrefix
                    ...
                    : name;
                observedEntries += 1;
                if (options.entriesAlreadyObserved + observedEntries >
                    options.maxEntries) {
        ...
        if (!after.isDirectory() ||
            ...
            throw new Error(`Compute artifact directory changed during traversal: ${options.relativePrefix || "."}`);
        }
        return {
...
export const writeCompressedArchiveSecure = (archivePath, entries, manifest, maxCompressedBytes) => Effect.tryPromise({
    try: (signal) => {
        ...
        const compressedByteLimit = new Transform({
            transform(chunk, _encoding, callback) {
                ...
                if (compressedBytes > maxCompressedBytes) {
                    callback(new Error(`Prisma compute archive exceeds the ${maxCompressedBytes} byte compressed upload safety limit.`));
                    return;
                }
                callback(null, chunk);
            },
        });
        return pipeline(Readable.from(tarChunks(entries, manifest), { objectMode: false }), createGzip(), compressedByteLimit, createWriteStream(archivePath, { flags: "w", mode: 0o600 }), { signal });
    },
    catch: (error) => error instanceof Error ? error : new Error(String(error)),
});
export const isArchivedRegularFile = (entries, entrypointName) => {
...
async function* tarChunks(entries, manifest) {
    for (const entry of entries) {
        ...
        if (emitted !== entry.file.size) {
            throw new Error(`Compute artifact file '${entry.name}' changed while the archive was being created.`);
        }
    ...
    if (manifestPadding > 0)
        yield new Uint8Array(manifestPadding);
...
const writeChecksum = (buffer, checksum) => {
    const text = checksum.toString(8).padStart(6, "0").slice(0, 6);
    writeString(buffer, 148, 8, `${text}\0 `);
};
const paddingLength = (size) => (512 - (size % 512)) % 512;
//# sourceMappingURL=ArchivePlatform.js.map