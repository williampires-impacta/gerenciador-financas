...
const toError = (message, cause) => new Error(message, { cause });
const isSameIdentity = (stat, identity) => stat.isFile() &&
...
const validateLimit = (maxBytes) => {
    if (!Number.isSafeInteger(maxBytes) || maxBytes <= 0) {
        throw new Error("Artifact maxBytes must be a positive safe integer.");
    }
};
const validateStat = (stat, maxBytes, description, allowEmpty) => {
    if (!stat.isFile()) {
...
async function* readVerifiedChunks(artifact) {
    ...
    try {
        ...
        if (!isSameIdentity(before, artifact.identity)) {
            throw new Error("Prisma deployment artifact changed after it was validated; rebuild or reselect the artifact before deploying.");
        }
        ...
        if (trailingBytes !== 0 || !isSameIdentity(after, artifact.identity)) {
            throw new Error("Prisma deployment artifact changed while it was being read; rebuild or reselect the artifact before deploying.");
        }
    }
    finally {
        await handle.close();
...
const inspectVerifiedFilePromise = async (inputPath, maxBytes, options) => {
    ...
    if (inputStat.isSymbolicLink()) {
        throw new Error(`${description} must not be a symbolic link; provide the regular file directly.`);
    }
    ...
    if (!isSameIdentity(stat, identityFrom(inputStat))) {
        throw new Error(`${description} changed while it was being validated; retry with a stable file.`);
    }
    return {
        _tag: "VerifiedFile",
        path: canonicalPath,
...
export const inspectVerifiedFile = (inputPath, maxBytes, options) => Effect.tryPromise({
...
export const inspectArtifactFile = (inputPath, maxBytes, options) => Effect.tryPromise({
    try: async (signal) => {
...
export const artifactFileStream = (artifact) => Stream.fromAsyncIterable(readVerifiedChunks(artifact), (cause) => cause instanceof Error
    ? cause
    : toError("Failed to stream Prisma deployment artifact.", cause));
export const verifiedFileChunks = (file) => readVerifiedChunks(file);
export const readArtifactFile = (artifact) => Effect.gen(function* () {
    const bytes = new Uint8Array(artifact.size);
    let offset = 0;
    yield* Stream.runForEach(artifactFileStream(artifact), (chunk) => Effect.sync(() => {
        bytes.set(chunk, offset);
        offset += chunk.byteLength;
    }));
    if (offset !== artifact.size) {
        return yield* Effect.fail(new Error("Prisma deployment artifact length changed while it was being read."));
    }
    return bytes;
});
//# sourceMappingURL=ArtifactFile.js.map