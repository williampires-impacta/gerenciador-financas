...
export const waitForAppDeploymentTarget = Effect.fn(function* (client, appId, deploymentId, options = {}) {
    ...
    while (true) {
        ...
        const observationOption = yield* client
            .getApp(appId)
            .pipe(Effect.result, Effect.timeoutOption(Duration.millis(remainingBeforeObservation)));
    ...
    return yield* Effect.fail(new AggregateError(lastError === undefined ? [] : [lastError], `Timed out waiting for Prisma App '${appId}' to report deployment '${deploymentId}' as latest (last observed latest deployment: '${lastLatestDeploymentId ?? "none"}').`));
});
/**
 ...
 * target deletion.
 */
export const promoteAppObserved = Effect.fn(function* (client, appId, deploymentId, options = {}) {
    const promoted = yield* client
        .promoteApp(appId, { deploymentId })
        .pipe(Effect.result);
    if (Result.isSuccess(promoted)) {
        const observation = yield* waitForAppDeploymentTarget(client, appId, deploymentId, options).pipe(Effect.result);
        if (Result.isFailure(observation)) {
            return yield* Effect.fail(new AggregateError([observation.failure], `Prisma App '${appId}' promotion returned success for deployment '${deploymentId}', but the App did not converge to that target. The deployment was preserved because promotion commit state is ambiguous.`));
        }
        return {
            ...promoted.success,
    ...
    const repairedObservation = Result.isSuccess(repaired)
        ? yield* waitForAppDeploymentTarget(client, appId, deploymentId, options).pipe(Effect.result)
        : yield* client.getApp(appId).pipe(Effect.result);
    if (Result.isSuccess(repairedObservation) &&
    ...
    return yield* Effect.fail(new AggregateError([
        promoted.failure,
        ...
        ...(Result.isFailure(repairedObservation)
            ...
            : [
                new Error(`Prisma App '${appId}' still reports deployment '${repairedObservation.success.latestDeploymentId ?? "none"}' as latest after canonical recovery targeted '${deploymentId}'.`),
            ]),
...
//# sourceMappingURL=AppPromotion.js.map