...
import { DEV_TIMESTAMP, devId, devProvider } from "./Internal/DevStub.js";
...
import { destroyProjectApps } from "./ComputeLifecycle.js";
...
import { isInputObject, isPrismaDevId } from "./Refs.js";
/**
 ...
 * @resource
 */
export const Project = Resource("Prisma.Project");
const createName = (id, name) => name === undefined ? createPhysicalName({ id }) : Effect.succeed(name);
const findProjectByName = (client, name) => client.listProjects().pipe(Effect.flatMap((projects) => {
    const matches = projects.filter((p) => p.name === name);
    return matches.length > 1
        ? Effect.fail(new Error(`Multiple Prisma projects are named '${name}'; use a unique project name before managing it with Alchemy.`))
...
const recoverGeneratedProjectAfterConflict = (client, name) => findProjectByName(client, name).pipe(Effect.flatMap((project) => project
    ? Effect.succeed(project)
    : Effect.fail(new GeneratedProjectNotVisible(`Generated Prisma project '${name}' already exists but is not visible yet.`))), Effect.retry({
    while: (error) => error instanceof GeneratedProjectNotVisible,
    schedule: generatedProjectRecoverySchedule,
}));
const defaultDatabase = (client, projectId) => client.listProjectDatabases(projectId, { limit: 100 }).pipe(Effect.flatMap((databases) => {
...
const requireDefaultDatabaseInRegion = (database, projectName, desiredRegion) => database?.region?.id === desiredRegion
    ? Effect.succeed(database)
    : Effect.fail(new DefaultDatabaseConsistencyError(database
...
const observeDesiredDefaultDatabase = (client, projectName, projectId, expectedDatabaseId, desiredRegion) => Effect.gen(function* () {
    const project = yield* client
        .getProject(projectId)
        .pipe(Effect.catchIf(isNotFound, () => Effect.fail(new DefaultDatabaseConsistencyError(`Prisma project '${projectName}' (${projectId}) is not visible yet while verifying its new default database.`))));
    const database = yield* requireDefaultDatabaseInRegion(yield* defaultDatabase(client, projectId).pipe(Effect.catchIf(isNotFound, () => Effect.fail(new DefaultDatabaseConsistencyError(`Prisma project '${projectName}' default database list is not visible yet.`)))), projectName, desiredRegion);
    if (database.id !== expectedDatabaseId) {
        return yield* Effect.fail(new DefaultDatabaseConsistencyError(`Prisma project '${projectName}' exposes default database '${database.id}', but newly created database '${expectedDatabaseId}' was expected. Retry after the in-progress default database promotion completes.`));
    }
    if (project.defaultRegion !== desiredRegion) {
        return yield* Effect.fail(new DefaultDatabaseConsistencyError(`Prisma project '${projectName}' still reports default region '${project.defaultRegion ?? "unknown"}', but region '${desiredRegion}' was requested. Retry after the in-progress default database promotion completes.`));
    }
...
const attrsFrom = (project, database, secrets) => ({
...
const ProviderLive = () => Provider.effect(Project, Effect.gen(function* () {
    const client = yield* PrismaClient;
    return {
        stables: ["projectId"],
        list: Effect.fn(function* () {
            const projects = yield* client.listProjects();
            return yield* Effect.forEach(projects, Effect.fn(function* (project) {
                const database = yield* defaultDatabase(client, project.id).pipe(Effect.catchIf(isNotFound, () => Effect.succeed(undefined)));
        ...
        diff: Effect.fn(function* ({ id, olds = {}, news = {}, output }) {
            ...
            const desiredRegion = isResolved(news.region)
                ...
                : undefined;
            const hadDatabase = output?.databaseId !== undefined ||
        ...
        read: Effect.fn(function* ({ id, output, olds = {} }) {
            ...
            const project = projectId
                ...
                : yield* findProjectByName(client, yield* createName(id, olds.name));
            if (!project)
                return undefined;
            const database = yield* defaultDatabase(client, project.id).pipe(Effect.catchIf(isNotFound, () => Effect.succeed(undefined)));
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, olds, output }) {
            ...
            if (!project) {
                const result = yield* client
                    ...
                    secrets: extractConnectionSecrets(project.database?.connections[0]),
                    created: true,
                    recoverSecrets: true,
                })), Effect.catchIf(isConflict, () => news.name === undefined
                    ? recoverGeneratedProjectAfterConflict(client, name).pipe(Effect.map((project) => ({
                        project,
                        database: undefined,
                        secrets: {},
                    ...
                    : Effect.fail(new Error(`Prisma project '${name}' appeared after the adoption check. Refusing to take it over; rerun with adoption enabled if it is the intended project.`))));
            ...
            if (!project) {
                return yield* Effect.fail(new Error(`Prisma project '${name}' could not be observed after create recovery.`));
            }
            const ownedGeneratedIdentity = news.name === undefined && project.name === name;
            if (news.createDatabase === false && createdDatabase) {
                return yield* Effect.fail(new Error(`Prisma created unexpected default database '${createdDatabase.name}' (${createdDatabase.id}) for project '${project.name}' even though createDatabase was false. Refusing to persist a state the Management API cannot remove in place.`));
            }
            if (news.createDatabase === false && !createdProject) {
                const existingDefault = yield* defaultDatabase(client, project.id).pipe(Effect.catchIf(isNotFound, () => Effect.succeed(undefined)));
                if (existingDefault) {
                    return yield* Effect.fail(new Error(`Prisma project '${project.name}' already owns default database '${existingDefault.name}' (${existingDefault.id}), which cannot be removed in place. Refusing to adopt or reconcile it as createDatabase: false; replace the Prisma.Project instead.`));
                }
            }
            ...
            if (news.createDatabase !== false) {
                ...
                if (!database) {
                    ...
                    const created = yield* client
                        ...
                        .pipe(Effect.catchIf(isConflict, () => defaultDatabase(client, projectId).pipe(Effect.flatMap((database) => requireDefaultDatabaseInRegion(database, name, desiredRegion)))));
            ...
            if (defaultDatabaseChanged) {
                ...
                if (changedDatabaseId === undefined) {
                    return yield* Effect.fail(new DefaultDatabaseConsistencyError(`Prisma project '${name}' did not return an identifier for its new default database.`));
                }
                const observed = yield* observeDesiredDefaultDatabase(client, name, project.id, changedDatabaseId, news.region ?? "us-east-1");
                project = observed.project;
                database = observed.database;
            }
            const persistedSecrets = output &&
                database !== undefined &&
...
const ProviderLocal = () => devProvider(Project, ["projectId"], ({ id, news }) => ({
    projectId: devId("project", id),
    projectName: news.name ?? id,
    workspaceId: devId("workspace", "local"),
    createdAt: DEV_TIMESTAMP,
    defaultRegion: news.createDatabase === false
        ? (news.region ?? null)
        : (news.region ?? "us-east-1"),
...
export const ProjectProvider = () => ProviderLayer.dual(Project, {
    local: () => ProviderLocal(),
    live: () => ProviderLive(),
});
