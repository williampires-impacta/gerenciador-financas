...
import { destroyApp } from "./ComputeLifecycle.js";
import { ensureAppImmutableIdentity } from "./Internal/AppIdentity.js";
 ...
 * @resource
 */
export const App = Resource("Prisma.App");
const desiredBranchId = Effect.fn(function* (client, projectId, props) {
    if (props.branchId !== undefined && !isPrismaDevId(props.branchId)) {
        return { resolved: true, id: props.branchId };
    }
    if (props.branchGitName !== undefined) {
        ...
        if (branches.length > 1) {
            return yield* Effect.fail(new Error(`Prisma returned multiple branches named '${props.branchGitName}' in project '${projectId}'; refusing an ambiguous App match.`));
        }
        return branches[0]
            ? { resolved: true, id: branches[0].id }
            : { resolved: false };
    }
    const branches = yield* client.listBranches(projectId, { limit: 100 });
    const defaults = branches.filter((branch) => branch.isDefault);
    if (defaults.length > 1) {
        return yield* Effect.fail(new Error(`Prisma returned multiple default branches for project '${projectId}'; refusing an ambiguous App match.`));
    }
    const defaultBranch = defaults[0];
    return defaultBranch
        ? { resolved: true, id: defaultBranch.id }
        : { resolved: false };
});
const createDisplayName = (id, displayName) => displayName === undefined
    ? createPhysicalName({ id })
    : Effect.succeed(displayName);
const findApp = Effect.fn(function* (client, projectId, displayName, props) {
    ...
    if (matches.length > 1) {
        return yield* Effect.fail(new Error(`Prisma returned multiple Apps named '${displayName}' on branch '${branch.id}' in project '${projectId}'; refusing an ambiguous ownership match.`));
    }
    return matches[0];
});
const attrsFrom = (app) => ({
...
const branchNeedsSync = Effect.fn(function* (client, projectId, app, props) {
    if (props.branchId !== undefined && !isPrismaDevId(props.branchId)) {
...
const validateAppProps = (props) => Effect.gen(function* () {
    ...
    if (props.branchId === null || props.branchGitName === null) {
        return yield* Effect.fail(new Error("Prisma.App requires an attached branch because the Management API cannot create an unassigned App atomically. Omit both fields to use the project default branch, or provide branchId/branchGitName."));
    }
});
const ProviderLive = () => Provider.effect(App, Effect.gen(function* () {
    const client = yield* PrismaClient;
    return {
        stables: ["appId"],
        list: () => client.listApps().pipe(Effect.map((apps) => apps.map(attrsFrom))),
        diff: Effect.fn(function* ({ id, olds, news, output }) {
            ...
            if (isResolved(news.regionId) && news.regionId !== undefined) {
                ...
                if (currentRegionId !== undefined &&
                    ...
                    return yield* Effect.fail(new Error(`Prisma App region is immutable and the Management API cannot atomically move an App without deleting its serving endpoint first. Create a second App with a different display name in the target region, cut traffic over, then remove this App.`));
                }
            }
            const updateProps = {
            ...
            const resolvedUpdateProps = {
                ...updateProps,
                displayName: yield* createDisplayName(id, updateProps.displayName),
            };
            if (!output) {
            ...
            const branch = yield* desiredBranchId(client, newProjectId ?? output.projectId, resolvedUpdateProps);
        ...
        read: Effect.fn(function* ({ id, output, olds }) {
            ...
            const app = appId
                ...
                : yield* Effect.gen(function* () {
                    ...
                    return projectId
                        ? yield* findApp(client, projectId, yield* createDisplayName(id, olds.displayName), olds)
                        : undefined;
                });
            if (!app)
                return undefined;
        ...
        reconcile: Effect.fn(function* ({ id, news, output }) {
            ...
            if (!branch.resolved) {
                return yield* Effect.fail(new Error(news.branchGitName === undefined
            ...
            if (!app) {
                const result = yield* client
                    ...
                    app,
                    created: true,
                })), Effect.catchIf(isConflict, (conflict) => findApp(client, projectId, displayName, news).pipe(Effect.flatMap((app) => app &&
                    output?.appId !== undefined &&
                    app.id === output.appId
                    ? Effect.succeed({ app, created: false })
                    : Effect.fail(new Error(`Prisma app '${displayName}' already exists on the requested branch but is not owned by this App resource. Import it with explicit adoption or choose a different display name.`, { cause: conflict }))))));
                app = result.app;
            }
            yield* ensureAppImmutableIdentity(app, projectId, news.regionId ?? output?.regionId ?? app.region.id);
            const needsBranchSync = yield* branchNeedsSync(client, projectId, app, news);
            ...
            if (app.name !== displayName || app.branchId !== branch.id) {
                return yield* Effect.fail(new Error(`Prisma App '${app.id}' did not converge to display name '${displayName}' and branch '${branch.id ?? "null"}'. Refusing to persist mismatched App state.`));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if (!app)
                return;
            if (app.projectId !== output.projectId ||
                app.region.id !== output.regionId) {
                return yield* Effect.fail(new Error(`Prisma App '${app.id}' no longer matches its persisted immutable project and region identity. Refusing to delete a mismatched App.`));
            }
            yield* destroyApp(client, output.appId);
        }),
    };
}));
const ProviderLocal = () => devProvider(App, ["appId"], ({ id, news }) => ({
    appId: devId("app", id),
...
export const AppProvider = () => ProviderLayer.dual(App, {
    local: () => ProviderLocal(),
    live: () => ProviderLive(),
});
