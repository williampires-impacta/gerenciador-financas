...
import { attrOrRedactedString, attrOrString, devId, devProvider, } from "./Internal/DevStub.js";
...
import { concreteIdsChanged, isInputObject, isPrismaDevId, resolveBucketId, unresolvedBucketIdOf, } from "./Refs.js";
/**
 ...
 * @resource
 */
export const BucketAccessKey = Resource("Prisma.BucketAccessKey");
 ...
 */
export class AmbiguousBucketAccessKeyError extends Data.TaggedError("AmbiguousBucketAccessKeyError") {
}
const listKeys = (client, bucketId) => client
    .listBucketKeys(bucketId, { limit: 100 })
    .pipe(Effect.catchIf(isNotFound, () => Effect.succeed([])));
const uniqueKeyNamed = (client, bucketId, expectedName) => listKeys(client, bucketId).pipe(Effect.flatMap((keys) => {
    const matches = keys.filter((key) => key.name === expectedName);
    return matches.length > 1
        ? Effect.fail(new AmbiguousBucketAccessKeyError({
            bucketId,
            name: expectedName,
...
const ProviderLive = () => Provider.effect(BucketAccessKey, Effect.gen(function* () {
    ...
    return {
        ...
        read: Effect.fn(function* ({ output }) {
            ...
            if (!output || isPrismaDevId(output.bucketAccessKeyId))
                return output;
            const keys = yield* listKeys(client, output.bucketId);
            return keys.some((key) => key.id === output.bucketAccessKeyId)
                ? output
                : undefined;
        }),
        reconcile: Effect.fn(function* ({ id, instanceId, news, output }) {
            const persisted = output && !isPrismaDevId(output.bucketAccessKeyId)
                ? output
                : undefined;
            if (persisted) {
                ...
                if (keys.some((key) => key.id === persisted.bucketAccessKeyId)) {
                    return persisted;
                }
            }
...
const ProviderLocal = () => devProvider(BucketAccessKey, BUCKET_ACCESS_KEY_STABLES, ({ id, news, output }) => ({
    ...
    secretAccessKey: attrOrRedactedString(output, "secretAccessKey") ??
        Redacted.make(devId("secret-access-key", id)),
    endpoint: "http://localhost",
    bucketName: `dev-${id}`,
}));
export const BucketAccessKeyProvider = () => ProviderLayer.dual(BucketAccessKey, {
    local: () => ProviderLocal(),
    live: () => ProviderLive(),
});
