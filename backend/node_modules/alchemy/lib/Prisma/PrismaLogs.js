...
const WEBSOCKET_HANDSHAKE_TIMEOUT = "10 seconds";
...
export const parseDeploymentLogRecord = (message, timestamp) => {
    ...
    return Effect.try({
        ...
        catch: () => new PrismaLogStreamError({
            message: `Failed to decode Prisma deployment log record (${byteLength} bytes; type: ${recordType})`,
        }),
    });
};
export const tailDeploymentLogs = (client, deploymentId, query) => Stream.callback((queue) => Effect.gen(function* () {
    const WebSocket = yield* loadWebSocketConstructor;
    const sockets = new Set();
    let latestCursor = query?.cursor;
    let latestByteEnd = query?.cursor !== undefined &&
    ...
    const connect = (cursor) => Effect.gen(function* () {
        if (stopped)
            return;
        const request = yield* client
            .getDeploymentLogsRequest(deploymentId, cursor === undefined ? query : { ...query, cursor })
            .pipe(Effect.mapError(() => new PrismaLogStreamError({
        ...
        const scheduleReconnect = (reconnectCursor, code, closeMode) => {
            ...
            Effect.runFork(Effect.sleep(Duration.millis(Math.min(RECONNECT_BACKOFF_MS * 2 ** noProgressReconnects, 1_000))).pipe(Effect.andThen(connect(reconnectCursor)), Effect.catch((error) => Effect.sync(() => {
                fail(error);
            }))));
        };
        socket.once("open", () => {
        ...
        socket.on("message", (raw) => {
            if (terminalHandled || stopped)
                return;
            const decoded = Effect.runSyncExit(parseDeploymentLogRecord(rawDataToString(raw), new Date()));
            ...
            if (record.raw.retryable) {
                const reconnectCursor = record.raw.cursor && record.raw.cursor.length > 0
                    ? record.raw.cursor
                    : latestCursor;
        ...
        socket.on("error", (cause) => {
            if (terminalHandled || stopped)
                return;
            const oversized = cause instanceof Error &&
        ...
        yield* Deferred.await(opened).pipe(Effect.timeoutOrElse({
...
const loadWebSocketConstructor = Effect.tryPromise({
    ...
    catch: (cause) => new PrismaLogStreamError({
        message: "Prisma deployment log tailing requires the optional `ws` package.",
        cause,
    }),
});
const rawDataToString = (raw) => {
    if (typeof raw === "string")
        return raw;
    if (raw instanceof ArrayBuffer)
        return new TextDecoder().decode(raw);
    if (Array.isArray(raw))
        return raw.map(rawDataToString).join("");
    return raw.toString();
};
//# sourceMappingURL=PrismaLogs.js.map