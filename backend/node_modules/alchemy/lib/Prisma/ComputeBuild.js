...
import { normalizeEntrypoint } from "./ComputeArchive.js";
...
const importNft = () => import("@vercel/nft").catch((cause) => {
    throw new Error("Failed to load '@vercel/nft'. Install the optional peer dependency '@vercel/nft' to build NestJS apps for Prisma Compute.", { cause });
});
...
const collectBoundedOutput = (stream, label, limit) => Stream.runFoldEffect(stream, () => ({ chunks: [], bytes: 0 }), (state, chunk) => {
    const bytes = state.bytes + chunk.byteLength;
    return bytes > limit
        ? Effect.fail(new Error(`Build ${label} exceeded the ${limit} byte output safety limit. Reduce build verbosity or raise outputLimitBytes explicitly.`))
        : Effect.sync(() => {
            state.chunks.push(chunk);
            return { chunks: state.chunks, bytes };
        });
}).pipe(Effect.map(({ chunks, bytes }) => {
 ...
 */
export const runBuildCommand = Effect.fn(function* ({ command, cwd, env, outputLimitBytes = DEFAULT_BUILD_OUTPUT_LIMIT_BYTES, timeoutSeconds = DEFAULT_BUILD_TIMEOUT_SECONDS, }) {
    ...
    const resultOption = yield* Effect.scoped(Effect.gen(function* () {
    ...
    if (Option.isNone(resultOption)) {
        return yield* Effect.fail(new Error(`Build command timed out after ${timeoutSeconds} seconds.`));
    }
    const result = resultOption.value;
    if (result.exitCode !== 0) {
        return yield* Effect.fail(new Error(`Build command failed with exit code ${result.exitCode} (stdout: ${result.stdout.bytes} bytes; stderr: ${result.stderr.bytes} bytes).`));
    }
...
const buildProcessEnvironment = (explicitEnv = {}) => ({
...
const strategies = [
    {
        name: "nextjs",
        canBuild: (appPath) => Effect.gen(function* () {
            return ((yield* hasRootFile(appPath, NEXT_CONFIG_FILENAMES)) ||
                (yield* hasPackageDependency(appPath, ["next"])));
        }),
        execute: (options) => buildFramework({
            ...
            missingMessage: "Could not find an installed Next.js CLI. Install `next` in the application or workspace before deploying.",
            sourceDir: ".next/standalone",
            entrypoint: "server.js",
            defaultPort: 3000,
            allowNestedEntrypoint: true,
            extrasRelativeToEntrypoint: true,
            missingOutputMessage: 'Next.js build did not produce standalone output. Add output: "standalone" to your next.config file.',
            extras: [
    ...
    },
    {
        name: "nuxt",
        canBuild: (appPath) => Effect.gen(function* () {
            return ((yield* hasRootFile(appPath, NUXT_CONFIG_FILENAMES)) ||
        ...
        execute: (options) => buildFramework({
            ...
            args: ["build"],
            failurePrefix: "Nuxt",
            missingMessage: "Could not find an installed Nuxt CLI. Install `nuxt` in the application or workspace before deploying.",
            sourceDir: ".output",
            entrypoint: "server/index.mjs",
            defaultPort: 3000,
            requiredFile: ".output/server/index.mjs",
            missingOutputMessage: "Nuxt build did not produce a Nitro node server entrypoint at .output/server/index.mjs. Ensure nitro.preset is 'node-server' (the default).",
        }),
    },
    {
        name: "astro",
        canBuild: (appPath) => Effect.gen(function* () {
            return ((yield* hasRootFile(appPath, ASTRO_CONFIG_FILENAMES)) ||
        ...
        execute: (options) => buildFramework({
            ...
            args: ["build"],
            failurePrefix: "Astro",
            missingMessage: "Could not find an installed Astro CLI. Install `astro` in the application or workspace before deploying.",
            sourceDir: "dist",
            entrypoint: "server/entry.mjs",
            defaultPort: 4321,
            requiredFile: "dist/server/entry.mjs",
            missingOutputMessage: 'Astro build did not produce a standalone server entrypoint. Install @astrojs/node and configure it with adapter: node({ mode: "standalone" }) in your astro.config file.',
        }),
    },
    {
        name: "nestjs",
        canBuild: (appPath) => Effect.gen(function* () {
            return ((yield* hasRootFile(appPath, [NEST_CLI_FILENAME])) ||
    ...
    },
    {
        ...
        execute: (options) => buildFramework({
            ...
            missingMessage: "Could not find an installed Vite CLI. Install `vite` in the application or workspace before deploying.",
            sourceDir: ".output",
            entrypoint: "server/index.mjs",
            defaultPort: 3000,
            requiredFile: ".output/server/index.mjs",
            missingOutputMessage: "TanStack Start build did not produce a Nitro node server entrypoint at .output/server/index.mjs. Ensure your vite.config includes the TanStack Start and Nitro plugins with the default node preset.",
        }),
    },
    {
        name: "bun",
...
export const runComputeAutoBuild = Effect.fn(function* (options) {
...
const buildFramework = Effect.fn(function* (options) {
    ...
    yield* runBuildCommand({
        command: yield* packageCliCommand(options.appPath, options.cliName, options.args, options.missingMessage),
        cwd: options.appPath,
    ...
    const build = Effect.gen(function* () {
        return yield* withStagingDeadline(Effect.gen(function* () {
            yield* copyDirectoryPreserveSymlinks(path.join(options.appPath, options.sourceDir), temp.artifactDir, temp.artifactDir, options.appPath, budget);
            yield* materializeBunNodeModuleAliases(temp.artifactDir, path.join(options.appPath, options.sourceDir), budget);
            const entrypoint = yield* resolveFrameworkEntrypoint(temp.artifactDir, options.entrypoint, options.allowNestedEntrypoint ?? false);
            ...
            for (const extra of options.extras ?? []) {
                ...
                if (yield* directoryExists(extraSource)) {
                    ...
                    yield* copyDirectoryPreserveSymlinks(extraSource, extraTarget, temp.artifactDir, options.appPath, budget);
                }
            }
            return {
    ...
    return yield* build.pipe(Effect.catch((error) => temp.cleanup.pipe(Effect.andThen(Effect.fail(error)))));
});
const buildNestjs = Effect.fn(function* (options) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const buildScript = yield* readPackageScript(options.appPath, "build");
    yield* runBuildCommand({
        command: buildScript === undefined
            ? yield* packageCliCommand(options.appPath, "nest", ["build"], "Could not find an installed Nest CLI. Add a `build` script to package.json or install `@nestjs/cli` in the application or workspace before deploying.")
    ...
    const budget = makeStagingBudget();
    const build = Effect.gen(function* () {
        const entrypoint = yield* withStagingDeadline(stageTracedNestjsArtifact({
            appPath: options.appPath,
    ...
    return yield* build.pipe(Effect.catch((error) => fs
        .remove(path.dirname(temp.artifactDir), { recursive: true })
        .pipe(Effect.ignore, Effect.andThen(Effect.fail(error)))));
});
const resolveNestjsCompiledEntrypoint = Effect.fn(function* (appPath) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const candidates = [
    ...
    return yield* Effect.fail(new Error(`NestJS build did not produce a compiled entrypoint. Looked for ${candidates.join(", ")} under ${appPath}. Ensure \`nest build\` ran and check package.json main, nest-cli.json, or tsconfig outDir.`));
});
const configuredNestjsCompiledEntrypoint = Effect.fn(function* (appPath) {
    const path = yield* Path.Path;
...
const stageTracedNestjsArtifact = Effect.fn(function* (options) {
    ...
    if (sortedFiles.length > STAGING_MAX_ENTRIES) {
        return yield* Effect.fail(new Error(`Compute artifact staging exceeded the ${STAGING_MAX_ENTRIES} entry safety limit while tracing NestJS dependencies.`));
    }
    for (const file of sortedFiles) {
        const normalized = normalizeRelativePath(file);
        if (normalized === undefined || normalized === ".") {
            return yield* Effect.fail(new Error(`NestJS dependency trace returned an unsafe path: ${file}`));
        }
        yield* stageTracedPath(path.join(sourceRoot, normalized), path.join(options.artifactDir, normalized), [realSourceRoot], artifactRoot, options.budget);
    }
    return entrypoint;
});
const copyDirectoryPreserveSymlinks = Effect.fn(function* (sourceDir, targetDir, targetRoot, sourceBoundary, budget) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    ...
    yield* copyDirectoryWithinRoots(sourceDir, targetDir, [realSourceRoot], path.resolve(targetRoot), budget);
});
const copyDirectoryWithinRoots = Effect.fn(function* (sourceDir, targetDir, allowedRoots, targetRoot, budget) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    yield* assertRealPathWithinRoots(sourceDir, allowedRoots);
    yield* assertSafeStagingDestination(targetDir, targetRoot);
    yield* fs.makeDirectory(targetDir, { recursive: true });
    const entries = (yield* readDirectoryEntries(sourceDir)).sort((a, b) => a.name.localeCompare(b.name));
    for (const entry of entries) {
        ...
        if (entry.type === "SymbolicLink") {
            ...
            yield* assertPathWithinRoots(path.resolve(path.dirname(target), symlinkTarget), [targetRoot], target);
        ...
        if (entry.type === "File") {
            ...
            if (copied.type !== "File" || Number(copied.size) !== size) {
                ...
                return yield* Effect.fail(new Error(`Compute artifact file changed while staging: ${source}`));
            }
            yield* fs.chmod(target, stat.mode & 0o777);
            continue;
        }
        yield* accountStagingEntry(budget, source, 0);
        return yield* Effect.fail(new Error(`Unsupported filesystem entry in compute artifact: ${source}`));
    }
});
const readDirectoryEntries = Effect.fn(function* (directory) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const names = yield* fs.readDirectory(directory);
    if (names.length > STAGING_MAX_ENTRIES) {
        return yield* Effect.fail(new Error(`Compute artifact directory exceeds the ${STAGING_MAX_ENTRIES} entry safety limit: ${directory}`));
    }
    return yield* Effect.forEach(names, (name) => Effect.gen(function* () {
        const file = path.join(directory, name);
        const isSymlink = yield* fs.readLink(file).pipe(Effect.as(true), Effect.catch(() => Effect.succeed(false)));
        if (isSymlink) {
            return { name, type: "SymbolicLink" };
        }
        const stat = yield* fs.stat(file);
        return {
            name,
            type: stat.type === "Directory" || stat.type === "File"
                ? stat.type
                : "Other",
        };
    }));
});
const materializeBunNodeModuleAliases = Effect.fn(function* (artifactDir, sourceDir, budget) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const nodeModules = path.join(artifactDir, "node_modules");
    const aliasRoot = path.join(nodeModules, ".bun", "node_modules");
    if (!(yield* directoryExists(aliasRoot)))
        return;
    const allowedRoots = [
    ...
    for (const entry of yield* fs.readDirectory(aliasRoot)) {
        ...
        if (entry.startsWith("@")) {
            ...
            for (const packageName of yield* fs.readDirectory(source)) {
                yield* copyAliasIfMissing(allowedRoots, path.join(source, packageName), path.join(targetScope, packageName), targetRoot, budget);
            }
        }
        else {
            yield* copyAliasIfMissing(allowedRoots, source, path.join(nodeModules, entry), targetRoot, budget);
        }
    }
});
const copyAliasIfMissing = Effect.fn(function* (allowedRoots, source, target, targetRoot, budget) {
    const fs = yield* FileSystem.FileSystem;
...
const resolveFrameworkEntrypoint = Effect.fn(function* (artifactDir, entrypoint, allowNested) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    if (!allowNested) {
        return entrypoint;
    ...
    const candidates = (yield* fs.readDirectory(artifactDir, { recursive: true }))
        ...
        return (path.basename(file) === entrypointFile &&
    ...
    if (candidates.length === 1)
        return candidates[0];
    if (candidates.length > 1) {
        return yield* Effect.fail(new Error(`Next.js standalone output has multiple ${entrypointFile} files (${candidates.join(", ")}). Cannot determine the application entrypoint.`));
    }
    return yield* Effect.fail(new Error(`Could not find framework entrypoint ${entrypoint} inside ${artifactDir}.`));
});
function buildBun(options) {
...
const resolveBunEntrypoint = Effect.fn(function* (appPath, entrypoint) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const candidate = entrypoint ?? (yield* readPackageMain(appPath));
    if (!candidate) {
        return yield* Effect.fail(new Error("Prisma Compute auto-build needs an entrypoint for Bun apps. Set `entrypoint` or package.json `main`."));
    }
...
const hasRootFile = Effect.fn(function* (appPath, filenames) {
    const fs = yield* FileSystem.FileSystem;
...
const hasPackageDependency = Effect.fn(function* (appPath, packageNames) {
...
const readPackageMain = Effect.fn(function* (appPath) {
    const parsed = yield* readPackageJson(appPath);
    return typeof parsed?.main === "string" ? parsed.main : undefined;
});
const readPackageScript = Effect.fn(function* (appPath, scriptName) {
    const parsed = yield* readPackageJson(appPath);
...
const readPackageJson = Effect.fn(function* (appPath) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const packagePath = path.join(appPath, "package.json");
    const text = yield* fs
        .readFileString(packagePath)
        .pipe(Effect.catch((error) => error._tag === "PlatformError" && error.reason._tag === "NotFound"
...
const readNestCliConfig = Effect.fn(function* (appPath) {
...
const readNestjsOutDir = Effect.fn(function* (appPath) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
const parseTsconfigOutDir = Effect.fn(function* (content) {
    const parsed = yield* Effect.try({
        try: () => JSON.parse(stripJsonComments(content)),
        catch: (cause) => cause instanceof Error ? cause : new Error(String(cause)),
    }).pipe(Effect.catch(() => Effect.succeed(undefined)));
    const outDir = parsed?.compilerOptions?.outDir;
    return typeof outDir === "string" ? normalizeRelativePath(outDir) : undefined;
});
const readJsonObjectFile = Effect.fn(function* (directory, fileName) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const text = yield* fs
...
const directoryExists = Effect.fn(function* (dirPath) {
    const fs = yield* FileSystem.FileSystem;
    const stat = yield* fs
        .stat(dirPath)
        .pipe(Effect.catch(() => Effect.succeed(undefined)));
...
const stageTracedPath = Effect.fn(function* (sourcePath, destinationPath, allowedRoots, targetRoot, budget) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    ...
    if (stat === undefined && symlinkTarget === undefined) {
        return yield* Effect.fail(new Error(`NestJS dependency disappeared while staging: ${sourcePath}`));
    }
    ...
    if (symlinkTarget !== undefined) {
        ...
        yield* assertPathWithinRoots(path.resolve(path.dirname(destinationPath), validatedTarget), [targetRoot], destinationPath);
    ...
    if (stat?.type === "File") {
        ...
        if (copied.type !== "File" || Number(copied.size) !== size) {
            ...
            return yield* Effect.fail(new Error(`Compute artifact file changed while staging: ${sourcePath}`));
        }
        yield* fs.chmod(destinationPath, stat.mode & 0o777);
        return;
    }
    yield* accountStagingEntry(budget, sourcePath, 0);
    return yield* Effect.fail(new Error(`Unsupported filesystem entry in NestJS dependency trace: ${sourcePath}`));
});
const makeStagingBudget = () => ({
    entries: 0,
    totalBytes: 0,
    deadline: Date.now() + STAGING_TIMEOUT_SECONDS * 1000,
});
const withStagingDeadline = (effect, budget) => effect.pipe(Effect.timeoutOption(Duration.millis(Math.max(1, budget.deadline - Date.now()))), Effect.flatMap((result) => Option.isSome(result)
    ? Effect.succeed(result.value)
    : Effect.fail(new Error(`Compute artifact staging timed out after ${STAGING_TIMEOUT_SECONDS} seconds.`))));
const accountStagingEntry = (budget, source, size) => {
    if (Date.now() > budget.deadline) {
        return Effect.fail(new Error(`Compute artifact staging timed out after ${STAGING_TIMEOUT_SECONDS} seconds.`));
    }
    if (size > STAGING_MAX_FILE_BYTES) {
        return Effect.fail(new Error(`Compute artifact file exceeds the ${STAGING_MAX_FILE_BYTES} byte per-file safety limit: ${source}`));
    }
    const entries = budget.entries + 1;
    if (entries > STAGING_MAX_ENTRIES) {
        return Effect.fail(new Error(`Compute artifact staging exceeded the ${STAGING_MAX_ENTRIES} entry safety limit.`));
    }
    const totalBytes = budget.totalBytes + size;
    if (!Number.isSafeInteger(totalBytes) ||
        totalBytes > STAGING_MAX_TOTAL_BYTES) {
        return Effect.fail(new Error(`Compute artifact staging exceeded the ${STAGING_MAX_TOTAL_BYTES} byte total safety limit.`));
    }
    budget.entries = entries;
    budget.totalBytes = totalBytes;
    return Effect.void;
};
const safeFileSize = (source, rawSize) => {
    const size = Number(rawSize);
    return Number.isSafeInteger(size) && size >= 0
        ? Effect.succeed(size)
        : Effect.fail(new Error(`Compute artifact has an invalid file size: ${source}`));
};
const assertRealPathWithinRoots = Effect.fn(function* (source, allowedRoots) {
    const fs = yield* FileSystem.FileSystem;
    const realSource = yield* fs.realPath(source);
    yield* assertPathWithinRoots(realSource, allowedRoots, source);
    return realSource;
});
const assertSafeStagingDestination = Effect.fn(function* (destination, targetRoot) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    ...
    while (!(yield* fs.exists(existingAncestor))) {
        const isDanglingSymlink = yield* fs.readLink(existingAncestor).pipe(Effect.as(true), Effect.catch(() => Effect.succeed(false)));
        if (isDanglingSymlink) {
            return yield* Effect.fail(new Error(`Compute artifact destination contains a dangling symlink: ${existingAncestor}`));
        }
        const parent = path.dirname(existingAncestor);
        if (parent === existingAncestor) {
            return yield* Effect.fail(new Error(`Could not establish a safe compute artifact destination: ${destination}`));
        }
...
const assertPathWithinRoots = Effect.fn(function* (realSource, allowedRoots, displaySource) {
    const path = yield* Path.Path;
    ...
    return yield* Effect.fail(new Error(`Compute artifact path escapes its staging root: ${displaySource}`));
});
const validateStagedSymlink = Effect.fn(function* (source, allowedRoots) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const target = yield* fs.readLink(source);
    if (path.isAbsolute(target) || /^[a-zA-Z]:[\\/]/.test(target)) {
        return yield* Effect.fail(new Error(`Compute artifact contains an absolute symlink: ${source}`));
    }
    yield* assertRealPathWithinRoots(source, allowedRoots);
    return target;
});
const validateStagedDirectory = Effect.fn(function* (root, budget) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const realRoot = yield* fs.realPath(root);
    const visit = Effect.fn(function* (directory) {
        yield* assertRealPathWithinRoots(directory, [realRoot]);
        const entries = (yield* readDirectoryEntries(directory)).sort((a, b) => a.name.localeCompare(b.name));
        for (const entry of entries) {
...
const resolveSourceRoot = Effect.fn(function* (startPath) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
const packageJsonDeclaresWorkspaces = Effect.fn(function* (directory) {
    const parsed = yield* readJsonObjectFile(directory, "package.json");
    return parsed?.workspaces !== undefined;
});
const packageCliCommand = Effect.fn(function* (appPath, cliName, args, missingMessage) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const sourceRoot = yield* resolveSourceRoot(appPath);
    const executable = process.platform === "win32" ? `${cliName}.cmd` : cliName;
    const candidates = Array.from(new Set([appPath, sourceRoot].map((root) => path.join(root, "node_modules", ".bin", executable))));
    for (const candidate of candidates) {
        const stat = yield* fs
            .stat(candidate)
            .pipe(Effect.catch(() => Effect.succeed(undefined)));
...
const packageScriptCommand = (scriptName) => [
...
const processBuildEnv = (env = {}) => Object.fromEntries(Object.entries(env).flatMap(([key, value]) => value === undefined
    ? []
...
//# sourceMappingURL=ComputeBuild.js.map