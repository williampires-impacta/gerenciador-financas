 ...
 * @resource
 */
export const SourceRepository = Resource("Prisma.SourceRepository");
const findRepository = (client, projectId) => client.listSourceRepositories({ projectId, limit: 100 }).pipe(Effect.flatMap((repos) => {
    const active = repos.filter((repo) => repo.status === "active");
    return active.length > 1
        ? Effect.fail(new Error(`Prisma project '${projectId}' has multiple active source repositories; refusing to select one arbitrarily.`))
        : Effect.succeed(active[0]);
}));
const matchesDesiredRepository = (repo, projectId, props) => repo.status === "active" &&
...
const verifyRepositoryLink = Effect.fn(function* (client, repo, expectedAppIds = [], expectedDatabaseIds = []) {
    ...
    if (observed.status !== "active" ||
        ...
        return yield* Effect.fail(new SourceRepositoryLinkNotReady(`Prisma source repository link '${repo.id}' has not converged to its requested immutable identity.`));
    }
    const branches = yield* client.listBranches(repo.projectId, {
        gitName: observed.defaultBranch,
        limit: 100,
    });
    const defaults = branches.filter((branch) => branch.gitName === observed.defaultBranch && branch.isDefault === true);
    if (defaults.length !== 1) {
        return yield* Effect.fail(new SourceRepositoryLinkNotReady(`Prisma source repository link '${repo.id}' did not produce exactly one default branch named '${observed.defaultBranch}'.`));
    }
    const branchId = defaults[0].id;
    yield* Effect.forEach(expectedAppIds, (appId) => client
        .getApp(appId)
        .pipe(Effect.flatMap((app) => app.branchId === branchId
        ? Effect.void
        : Effect.fail(new SourceRepositoryLinkNotReady(`Prisma App '${appId}' was not attached to repository branch '${branchId}'.`)))), { concurrency: 8 });
    yield* Effect.forEach(expectedDatabaseIds, (databaseId) => client
        .getDatabase(databaseId)
        .pipe(Effect.flatMap((database) => database.branchId === branchId
        ? Effect.void
        : Effect.fail(new SourceRepositoryLinkNotReady(`Prisma database '${databaseId}' was not attached to repository branch '${branchId}'.`)))), { concurrency: 8 });
    return observed;
});
const ProviderLive = () => Provider.effect(SourceRepository, Effect.gen(function* () {
    const client = yield* PrismaClient;
    return {
        stables: ["sourceRepositoryId"],
        list: Effect.fn(function* () {
            const projects = yield* client.listProjects();
            const repositories = yield* Effect.forEach(projects, (project) => client
        ...
        diff: Effect.fn(function* ({ olds, news, output }) {
            ...
            if (concreteIdsChanged(oldProjectId, newProjectId) ||
                ...
                observedMismatch) {
                return yield* Effect.fail(new Error(`Prisma source repository links cannot be replaced atomically: unlinking mutates branch/resource attachments and a failed relink cannot restore them. Create a new project or explicitly unlink and relink in separate reviewed deployments.`));
            }
            return undefined;
        }),
        ...
        reconcile: Effect.fn(function* ({ news, output }) {
            ...
            if (repo && !matchesDesiredRepository(repo, projectId, news)) {
                return yield* Effect.fail(new Error(`Prisma project '${projectId}' is already linked to '${repo.repoFullName}'. The Management API has no atomic relink or rollback operation, so this provider will not delete the live link during reconcile. Explicitly unlink it in a separate reviewed deployment before linking repository '${news.providerRepositoryId}'.`));
            }
            if (!repo) {
                ...
                const previouslyUnassignedDatabases = (yield* client.listProjectDatabases(projectId, { limit: 100 }))
                ...
                repo = yield* client
                    ...
                    .pipe(Effect.catchIf(isConflict, () => Effect.fail(new Error(`Prisma source repository '${news.providerRepositoryId}' appeared after the adoption check. Refusing to take over the project link; rerun with adoption enabled if it is the intended repository.`))));
                repo = yield* verifyRepositoryLink(client, repo, previouslyUnassignedApps, previouslyUnassignedDatabases).pipe(Effect.retry({
                    while: (error) => error instanceof SourceRepositoryLinkNotReady,
                    schedule: sourceRepositoryConsistencySchedule,
                }), Effect.mapError((error) => new Error(`Prisma source repository link '${repo.id}' was created but its branch/resource side effects did not converge. The link was not deleted because unlinking cannot roll back partial attachments; inspect it and adopt after repair.`, { cause: error })));
            }
            else {
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if (!repo)
                return;
            if (repo.projectId !== output.projectId ||
                repo.repoId !== output.repoId ||
                repo.provider !== output.provider ||
                repo.installationId !== output.installationId) {
                return yield* Effect.fail(new Error(`Prisma source repository '${repo.id}' no longer matches its persisted project/repository/installation identity. Refusing to delete a mismatched link.`));
            }
            yield* client
...
const ProviderLocal = () => devProvider(SourceRepository, ["sourceRepositoryId"], ({ id, news }) => ({
...
export const SourceRepositoryProvider = () => ProviderLayer.dual(SourceRepository, {
    local: () => ProviderLocal(),
    live: () => ProviderLive(),
});
