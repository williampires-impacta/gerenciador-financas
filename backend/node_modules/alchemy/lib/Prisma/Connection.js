...
import { DEV_TIMESTAMP, attrOrNullableString, attrOrRedactedString, attrOrString, devId, devProvider, } from "./Internal/DevStub.js";
...
import { deriveConnectionAttrs, hasCanonicalConnectionSecrets, } from "./Internal/DatabaseSecrets.js";
import { physicalInstanceName } from "./Internal/EnvName.js";
import { concreteIdsChanged, isInputObject, isPrismaDevId, resolveDatabaseId, unresolvedDatabaseIdOf, } from "./Refs.js";
/**
 ...
 * @resource
 */
export const Connection = Resource("Prisma.Connection");
const findConnection = (client, databaseId, predicate) => client
...
class AmbiguousPrismaConnectionError extends Error {
    _tag = "AmbiguousPrismaConnectionError";
    constructor(databaseId, name, count) {
        super(`Prisma database '${databaseId}' has ${count} connections named '${name}'; use a unique connection name before importing it into Alchemy`);
    }
}
class InvalidPrismaConnectionNameError extends Error {
    _tag = "InvalidPrismaConnectionNameError";
    constructor() {
...
const uniqueConnection = (client, databaseId, description, predicate) => findConnection(client, databaseId, predicate).pipe(Effect.flatMap((connections) => connections.length <= 1
    ? Effect.succeed(connections[0])
    : Effect.fail(new AmbiguousPrismaConnectionError(databaseId, description, connections.length))));
...
const recoverGeneratedConnectionAfterConflict = (client, databaseId, expectedName) => uniqueConnection(client, databaseId, expectedName, (candidate) => candidate.name === expectedName).pipe(Effect.flatMap((connection) => connection
    ? Effect.succeed(connection)
    : Effect.fail(new GeneratedConnectionNotVisible(`Generated Prisma connection '${expectedName}' is not visible yet.`))), Effect.retry({
    while: (error) => error instanceof GeneratedConnectionNotVisible,
    schedule: generatedConnectionRecoverySchedule,
}));
const physicalConnectionPrefix = (name) => `${name.trim().slice(0, 52)}-`;
const isGeneratedPhysicalConnectionName = (physicalName, logicalName) => {
...
const isAdoptablePhysicalConnectionName = (physicalName, logicalName) => physicalName === logicalName.trim() ||
    isGeneratedPhysicalConnectionName(physicalName, logicalName);
const attrsFrom = (connection, secrets) => ({
...
const ProviderLive = () => Provider.effect(Connection, Effect.gen(function* () {
    const client = yield* PrismaClient;
    return {
        stables: ["connectionId"],
        list: () => client
            .listConnections()
            .pipe(Effect.map((connections) => connections.map((c) => attrsFrom(c, {})))),
        ...
        read: Effect.fn(function* ({ id, instanceId, output, olds }) {
            ...
            if (connectionId && output) {
                ...
                if (!connection)
                    return undefined;
                if (connection.database.id !== output.databaseId ||
                    connection.name !== output.connectionName) {
                    return yield* Effect.fail(new Error(`Prisma connection '${connection.id}' no longer matches persisted database '${output.databaseId}' and name '${output.connectionName}'. Refusing to refresh a mismatched connection.`));
                }
            ...
            const owned = yield* uniqueConnection(client, databaseId, expectedName, (connection) => connection.name === expectedName);
            if (owned)
                return attrsFrom(owned, {});
            const prefix = physicalConnectionPrefix(name);
            const generated = yield* uniqueConnection(client, databaseId, `${prefix}<instance-id>`, (connection) => connection.name !== expectedName &&
                isGeneratedPhysicalConnectionName(connection.name, name));
            const connection = generated ??
                (yield* uniqueConnection(client, databaseId, name, (connection) => connection.name === name));
            if (!connection)
                return undefined;
        ...
        reconcile: Effect.fn(function* ({ id, instanceId, news, olds, output, }) {
            ...
            if (connectionId &&
                ...
                return yield* Effect.fail(new Error(`Persisted Prisma connection '${connectionId}' has physical name '${physicalName}', which does not match requested logical name '${name}'. Refusing to adopt or persist a mismatched connection.`));
            }
            if (connection &&
                (connection.database.id !== databaseId ||
                    connection.name !== physicalName)) {
                return yield* Effect.fail(new Error(`Prisma connection '${connection.id}' resolves to database '${connection.database.id}' with name '${connection.name}', not requested database '${databaseId}' and physical name '${physicalName}'. Refusing to rotate or persist mismatched identity; replace the connection instead.`));
            }
            if (!connection && !connectionId) {
                connection = yield* uniqueConnection(client, databaseId, expectedName, (candidate) => candidate.name === expectedName);
            }
            ...
            if (!connection) {
                ...
                connection = yield* physicalName === expectedName
                    ? create.pipe(Effect.catchIf(isConflict, () => recoverGeneratedConnectionAfterConflict(client, databaseId, expectedName)))
                    : create;
                secrets = extractConnectionSecrets(connection);
            }
            const missingCanonicalSecrets = secrets.directConnectionString === undefined &&
            ...
            const recoveringOwnedGeneratedSecrets = physicalName === expectedName && missingCanonicalSecrets;
            if (recoveringOwnedGeneratedSecrets ||
                (news.rotate === true && olds?.rotate !== true)) {
                const rotated = yield* client.rotateConnection(connection.id);
                if (rotated.id !== connection.id ||
                    rotated.database.id !== databaseId ||
                    rotated.name !== physicalName) {
                    return yield* Effect.fail(new Error(`Prisma rotated connection '${rotated.id}' for database '${rotated.database.id}' with name '${rotated.name}', but connection '${connection.id}' for database '${databaseId}' with name '${physicalName}' was requested. Refusing to persist mismatched credentials.`));
                }
                connection = rotated;
                secrets = extractConnectionSecrets(rotated);
                if (!hasCanonicalConnectionSecrets(secrets)) {
                    return yield* Effect.fail(new Error(`Prisma rotated connection '${connection.id}' for database '${databaseId}' without returning a canonical connection URL. Refusing to persist missing or stale credentials.`));
                }
            }
            return attrsFrom(connection, {
                directConnectionString: secrets.directConnectionString ?? output?.directConnectionString,
                pooledConnectionString: secrets.pooledConnectionString ?? output?.pooledConnectionString,
                accelerateConnectionString: secrets.accelerateConnectionString ??
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if (connection.database.id !== output.databaseId ||
                connection.name !== output.connectionName) {
                return yield* Effect.fail(new Error(`Prisma connection '${connection.id}' no longer matches persisted database '${output.databaseId}' and name '${output.connectionName}'. Refusing to delete a mismatched connection.`));
            }
            yield* client
...
const ProviderLocal = () => devProvider(Connection, ["connectionId"], ({ id, news }) => {
    const secrets = {
