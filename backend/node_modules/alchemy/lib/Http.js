...
import { causeResponse, } from "effect/unstable/http/HttpServerError";
...
export const isScopeEjected = (scope) => scopeEjected in scope;
export const serve = (handler) => Effect.serviceOption(HttpServer).pipe(Effect.map(Option.getOrUndefined), Effect.flatMap((http) => http
...
export const safeHttpEffect = (handler) => Effect.catchCause(handler.pipe(
// @ts-expect-error
Effect.flatMap((response) => HttpServerResponse.isHttpServerResponse(response)
...
// file paths).
causeResponse(cause).pipe(Effect.flatMap(([response, reportableCause]) => Effect.withFiber((fiber) => fiber.getRef(ErrorReporter.CurrentErrorReporters).size > 0
    ? ErrorReporter.report(reportableCause)
    : logUnreportedCause(reportableCause)).pipe(Effect.as(response)))));
/**
 ...
 * and are skipped.
 */
const logUnreportedCause = (cause) => {
    const failures = cause.reasons.filter((reason) => reason._tag !== "Interrupt" &&
...
export const resolvePort = (options) => options?.port !== undefined
    ? Effect.succeed(options.port)
    : Config.number("PORT").pipe(Config.withDefault(3000));
export const BunHttpServer = (serverOptions) => Layer.effect(HttpServer, Effect.gen(function* () {
    const BunHttpServerPlatform = yield* Effect.promise(() => import("@effect/platform-bun/BunHttpServer"));
    return {
        serve: (handler, options) => Effect.gen(function* () {
...
export const NodeHttpServer = () => Layer.effect(HttpServer, Effect.gen(function* () {
    const NodeHttpServerPlatform = yield* Effect.promise(() => import("@effect/platform-node/NodeHttpServer"));
    const NodeHttp = yield* Effect.promise(() => import("node:http"));
    return {
        serve: (handler, options) => Effect.gen(function* () {
            const port = yield* resolvePort(options);
            const server = yield* NodeHttpServerPlatform.make(NodeHttp.createServer, { port });
            yield* server.serve(safeHttpEffect(handler));
        }).pipe(Effect.orDie),
    };
}));
