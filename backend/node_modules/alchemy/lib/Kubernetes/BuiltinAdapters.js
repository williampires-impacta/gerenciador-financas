...
import { ClusterAdapter, } from "./ClusterAdapter.js";
import { mintUserCredentials, resolveKubeContext, runExecCredential, } from "./internal/kubeconfig.js";
const toAuthHeaders = (credentials) => credentials.token ? { Authorization: `Bearer ${credentials.token}` } : {};
const requireEndpoint = (connection) => connection.endpoint !== undefined
    ? Effect.succeed(connection.endpoint)
    : Effect.fail(new Error(`A '${connection.auth.kind}' Kubernetes connection requires an ` +
 ...
 * plugins.
 */
export const KubeConfigAdapter = Layer.effect(ClusterAdapter("kubeconfig"), Effect.gen(function* () {
    ...
    return {
        ...
        connect: Effect.fn(function* (connection) {
            if (connection.auth.kind !== "kubeconfig") {
                return yield* Effect.die(new Error(`kubeconfig adapter received auth kind '${connection.auth.kind}'`));
            }
            ...
            // here.
            const initial = yield* mintUserCredentials(resolved.user).pipe(Effect.provideContext(context));
            const isExec = resolved.user.exec?.command !== undefined;
            const headers = isExec
                ? mintUserCredentials(resolved.user).pipe(Effect.map(toAuthHeaders), Effect.provideContext(context))
...
export const TokenAdapter = Layer.succeed(ClusterAdapter("token"), {
    kind: "Kubernetes.ClusterAdapter",
    connect: Effect.fn(function* (connection) {
        if (connection.auth.kind !== "token") {
            return yield* Effect.die(new Error(`token adapter received auth kind '${connection.auth.kind}'`));
        }
...
export const ClientCertAdapter = Layer.succeed(ClusterAdapter("client-cert"), {
    kind: "Kubernetes.ClusterAdapter",
    connect: Effect.fn(function* (connection) {
        if (connection.auth.kind !== "client-cert") {
            return yield* Effect.die(new Error(`client-cert adapter received auth kind '${connection.auth.kind}'`));
        }
        return {
            endpoint: yield* requireEndpoint(connection),
            certificateAuthorityData: connection.certificateAuthorityData,
 ...
 */
export const ExecAdapter = Layer.effect(ClusterAdapter("exec"), Effect.gen(function* () {
    const context = yield* Effect.context();
    return {
        kind: "Kubernetes.ClusterAdapter",
        connect: Effect.fn(function* (connection) {
            if (connection.auth.kind !== "exec") {
                return yield* Effect.die(new Error(`exec adapter received auth kind '${connection.auth.kind}'`));
            }
            ...
            return {
                ...
                certificateAuthorityData: connection.certificateAuthorityData,
                insecureSkipTlsVerify: connection.insecureSkipTlsVerify,
                headers: runExecCredential(exec).pipe(Effect.map(toAuthHeaders), Effect.provideContext(context)),
                clientCert: initial.clientCert,
            };
        }),
    };
...
export const builtinAdapters = () => Layer.mergeAll(KubeConfigAdapter, TokenAdapter, ClientCertAdapter, ExecAdapter);
//# sourceMappingURL=BuiltinAdapters.js.map