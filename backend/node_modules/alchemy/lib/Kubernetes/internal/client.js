...
import { findClusterAdapter, } from "../ClusterAdapter.js";
import { buildKubernetesObjectPathWithSpec, chunkByApplyRank, DEFAULT_APPLY_RANK, kubernetesObjectKey, lookupKubernetesKindSpec, sortRefsForDelete, toKubernetesObjectRef, } from "./objects.js";
export class KubernetesApiError extends Data.TaggedError("KubernetesApiError") {
    get message() {
        return `${this.method} ${this.path} responded ${this.statusCode}: ${this.body.length > 0 ? this.body.slice(0, 1000) : "(empty body)"}`;
    }
}
 ...
 * registered adapter.
 */
export const connectCluster = (connection) => findClusterAdapter(connection.auth.kind).pipe(Effect.flatMap((adapter) => adapter.connect(connection)));
const requestJson = Effect.fn(function* ({ transport, method, path, body, }) {
    ...
    return yield* Effect.tryPromise({
        try: () => new Promise((resolve, reject) => {
            ...
            ? error
            : new Error(`Failed Kubernetes ${method} ${path}: ${error instanceof Error ? error.message : String(error)}`),
    }).pipe(
    ...
    // callers.
    Effect.retry({
        while: (e) => !(e instanceof KubernetesApiError),
 ...
 */
export const resolveKindSpec = Effect.fn(function* ({ transport, input, }) {
    ...
    const resource = listed.resources?.find((candidate) => candidate.kind === input.kind &&
        typeof candidate.name === "string" &&
        !candidate.name.includes("/"));
    if (!resource?.name) {
        return yield* Effect.fail(new KubernetesApiError({
            method: "GET",
            path: discoveryPath,
            statusCode: 404,
...
const buildPath = Effect.fn(function* ({ transport, object, }) {
...
// ─────────────────────────────────────────────────────────── object ops ──
export const readObject = Effect.fn(function* ({ transport, object, }) {
...
export const applyObject = Effect.fn(function* ({ transport, object, }) {
...
export const deleteObject = Effect.fn(function* ({ transport, object, }) {
    yield* buildPath({ transport, object }).pipe(Effect.flatMap((path) => requestJson({
        transport,
        method: "DELETE",
        path,
    })), Effect.catchIf((error) => error instanceof KubernetesApiError, (error) => (error.statusCode === 404 ? Effect.void : Effect.fail(error))));
});
export const reconcileObjects = Effect.fn(function* ({ transport, previousObjects, desiredObjects, }) {
    const desiredRefs = desiredObjects.map(toKubernetesObjectRef);
    const desiredKeys = new Set(desiredRefs.map(kubernetesObjectKey));
    const removedObjects = previousObjects.filter((object) => !desiredKeys.has(kubernetesObjectKey(object)));
    ...
    for (const chunk of chunkByApplyRank(desiredObjects)) {
        yield* Effect.forEach(chunk, (object) => applyObject({
            transport,
            object,
        }), {
...
export const deleteObjects = Effect.fn(function* ({ transport, objects, }) {
