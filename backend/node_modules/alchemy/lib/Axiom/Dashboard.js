 ...
 * @resource
 */
export const Dashboard = Resource("Axiom.Dashboard");
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ news, output }) {
            ...
            const observed = output?.uid
                ? yield* get({ uid: output.uid }).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)))
                : undefined;
        ...
        read: Effect.fn(function* ({ output }) {
            if (!output?.uid)
                return undefined;
            return yield* get({ uid: output.uid }).pipe(Effect.map(toAttrsFromGet), Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
        }),
