 ...
 * @resource
 */
export const View = Resource("Axiom.View");
export const ViewProvider = () => Provider.effect(View, Effect.gen(function* () {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ news, output }) {
            ...
            const observed = yield* get({ id: viewId }).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
        ...
        read: Effect.fn(function* ({ output }) {
            if (!output?.id)
                return undefined;
            return yield* get({ id: output.id }).pipe(Effect.map((current) => ({ ...current, id: output.id })), Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
        }),
    };
}));
