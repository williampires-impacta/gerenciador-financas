 ...
 * @resource
 */
export const Dataset = Resource("Axiom.Dataset");
/**
...
const buildMarker = (stack, stage, id) => `[alchemy:stack=${stack};stage=${stage};id=${id}]`;
const augmentDescription = (description, marker) => {
    const base = stripMarker(description ?? "");
    return base.length > 0 ? `${base}\n${marker}` : marker;
};
const stripMarker = (description) => description.replace(MARKER_RE, "").trimEnd();
...
const buildOtelAttrs = (apiBaseUrl, name) => {
    ...
    return {
        ...
        otelHeaders: { "X-Axiom-Dataset": name },
    };
};
export const DatasetProvider = () => Provider.effect(Dataset, Effect.gen(function* () {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output }) {
            ...
            const observed = yield* get({ dataset_id: datasetId }).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
            ...
            if (current === undefined) {
                ...
                }).pipe(Effect.catchIf((e) => e._tag === "Conflict" || e._tag === "UnprocessableEntity", () => update({
            ...
            const needsSync = current.description !== desiredDescription ||
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* del({ dataset_id: output.id }).pipe(Effect.catchTag("NotFound", () => Effect.void));
        }),
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            if (!datasetId)
                return undefined;
            const existing = yield* get({ dataset_id: datasetId }).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
            if (!existing)
                return undefined;
