 ...
 * @resource
 */
export const ApiToken = Resource("Axiom.ApiToken");
export const ApiTokenProvider = () => Provider.effect(ApiToken, Effect.gen(function* () {
    ...
    return {
        ...
        read: Effect.fn(function* ({ output }) {
            if (!output?.id)
                return undefined;
            return yield* get({ id: output.id }).pipe(Effect.map((current) => ({ ...current, token: output.token })), Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
        }),
    };
}));
