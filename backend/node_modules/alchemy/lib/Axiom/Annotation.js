 ...
 * @resource
 */
export const Annotation = Resource("Axiom.Annotation");
export const AnnotationProvider = () => Provider.effect(Annotation, Effect.gen(function* () {
    ...
    return {
        ...
        list: () => listAnnotations({}).pipe(Effect.map((annotations) => annotations.map((a) => ({ ...a })))),
        ...
        read: Effect.fn(function* ({ output }) {
            if (!output?.id)
                return undefined;
            return yield* get({ id: output.id }).pipe(Effect.map((current) => ({
                ...current,
