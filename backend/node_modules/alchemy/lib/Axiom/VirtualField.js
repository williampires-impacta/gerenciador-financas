 ...
 * @resource
 */
export const VirtualField = Resource("Axiom.VirtualField");
export const VirtualFieldProvider = () => Provider.effect(VirtualField, Effect.gen(function* () {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const perDataset = yield* Effect.forEach(datasets, (ds) => listVirtualFields({ dataset: ds.name }), { concurrency: 10 });
        ...
        reconcile: Effect.fn(function* ({ news, output }) {
            ...
            const observed = output?.id
                ? yield* get({ id: output.id }).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)))
                : undefined;
        ...
        read: Effect.fn(function* ({ output }) {
            if (!output?.id)
                return undefined;
            return yield* get({ id: output.id }).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
        }),
    };
}));
//# sourceMappingURL=VirtualField.js.map