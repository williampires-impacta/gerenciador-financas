...
import { PlanetscaleConflict, pollUntil } from "../Util.js";
/**
 * Polls keyspaces in a branch until the named keyspace reports
 * `resizing === false` (i.e. any in-flight resize has completed).
 */
export const waitForKeyspaceReady = Effect.fn(function* (organization, database, branch, keyspace) {
    yield* pollUntil(`keyspace "${keyspace}" not resizing`, ps.listKeyspaces({ organization, database, branch }), (page) => {
 ...
 * matching.
 */
const observeDefaultKeyspace = Effect.fn(function* (organization, database, branch) {
    ...
    return (keyspaces.data.find((x) => x.default) ??
 ...
 */
export const observeDefaultKeyspaceReplicas = Effect.fn(function* (organization, database, branch) {
    return yield* observeDefaultKeyspace(organization, database, branch).pipe(Effect.map((keyspace) => keyspace?.replicas), Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
});
/**
 ...
 */
export const ensureMySQLProductionBranchClusterSize = Effect.fn(function* (organization, database, branch, expectedClusterSize, expectedReplicas) {
    ...
    if (expectedReplicas !== undefined &&
        ...
        if (extraReplicas < 0) {
            return yield* Effect.fail(new PlanetscaleConflict({
                message: `Cannot set replicas to ${expectedReplicas} on keyspace "${keyspace.name}": ` +
        ...
        Effect.retry({
            while: (e) => e._tag === "UnprocessableEntity" &&
        ...
        yield* pollUntil(`keyspace "${keyspace.name}" resize completed`, ps.listKeyspaceResizeRequests({
            organization,
            database,
            branch,
            keyspace: keyspace.name,
...
//# sourceMappingURL=MySQLClusterSize.js.map