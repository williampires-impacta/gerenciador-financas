 ...
 * of this primitive.
 */
export const pollUntil = (description, fn, predicate, schedule = defaultSchedule) => fn.pipe(Effect.flatMap((value) => predicate(value)
    ? Effect.succeed(value)
    : Effect.fail(new NotReady({ description }))), Effect.retry({
    while: (e) => e?._tag === "Planetscale::NotReady",
    schedule,
}));
/**
 ...
 * retry loop.
 */
export const waitForBranchReady = Effect.fn(function* (organization, database, branch, session) {
    const startedAt = yield* Clock.currentTimeMillis;
    return yield* pollUntil(`branch "${branch}" ready`, Effect.gen(function* () {
        if (session) {
            const now = yield* Clock.currentTimeMillis;
            const seconds = Math.floor((now - startedAt) / 1000);
            yield* session.note(`Waiting for branch to be ready... (${seconds} seconds elapsed; this can take a few minutes)`);
        }
        return yield* ps.getBranch({ organization, database, branch });
    }).pipe(Effect.catchTag("NotFound", () => Effect.fail(new NotReady({ description: `branch "${branch}" not found yet` })))), (data) => data.ready);
});
/**
 ...
 * retry loop.
 */
export const waitForDatabaseReady = Effect.fn(function* (organization, database, session) {
    const startedAt = yield* Clock.currentTimeMillis;
    return yield* pollUntil(`database "${database}" ready`, Effect.gen(function* () {
        if (session) {
            const now = yield* Clock.currentTimeMillis;
            const seconds = Math.floor((now - startedAt) / 1000);
            yield* session.note(`Waiting for database to be ready... (${seconds} seconds elapsed; this can take a few minutes)`);
        }
        return yield* ps.getDatabase({ organization, database });
    }).pipe(Effect.catchTag("NotFound", () => Effect.fail(new NotReady({
        description: `database "${database}" not found yet`,
    })))), (data) => data.ready);
});
export const isKnownError = (tag, message) => (error) => typeof error === "object" &&
    error !== null &&
    error._tag === tag &&
