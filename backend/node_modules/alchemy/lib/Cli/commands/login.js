 ...
 * folder.
 */
const loginMain = Argument.file("main").pipe(Argument.withDescription("Stack entrypoint whose providers() to log in with, defaults to alchemy.run.ts (falls back to all built-in providers when absent)"), Argument.optional);
export const loginCommand = Command.make("login", {
    ...
    if (explicitMain != null && !mainExists) {
        return yield* Effect.fail(new CliError.InvalidValue({
            option: "main",
            value: explicitMain,
    ...
    if (mainExists) {
        ...
        yield* Console.log("No alchemy.run.ts found — using Alchemy's built-in providers.");
    ...
    const providers = Object.values(authProviders).filter((provider) => selected == null || selected.includes(provider.name));
    if (providers.length === 0) {
        yield* Console.log(selected != null
