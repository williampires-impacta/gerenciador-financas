...
import { envFile, importStack, instrumentCommand, profile, script, yes, } from "./_shared.js";
/**
...
const stackFlag = Flag.string("stack").pipe(Flag.withDescription("Stack name (e.g. AlchemyEffectWebsite)"));
const stageFlag = Flag.string("stage").pipe(Flag.withDescription("Stage name (e.g. dev, prod)"));
const fqnFlag = Flag.string("fqn").pipe(Flag.withDescription("Fully-qualified resource name"));
/**
...
const withStateService = (args, body) => Effect.gen(function* () {
    ...
    const services = Layer.mergeAll(Layer.succeed(AuthProviders, {}), ConfigProvider.layer(withProfileOverride(yield* loadConfigProvider(args.envFile), args.profile)), Logger.layer([fileLogger("out")], { mergeWithExisting: true }), Layer.succeed(Stage, "placeholder"), 
...
const stacksCommand = Command.make("stacks", { main: script, envFile, profile, local: localFlag }, instrumentCommand("state.stacks")(Effect.fn(function* (args) {
...
const stagesCommand = Command.make("stages", { stack: stackFlag, main: script, envFile, profile, local: localFlag }, instrumentCommand("state.stages")(Effect.fn(function* ({ stack: stackName, ...rest }) {
...
const resourcesCommand = Command.make("resources", {
    ...
    profile,
    local: localFlag,
}, instrumentCommand("state.resources")(Effect.fn(function* ({ stack: stackName, stageName, ...rest }) {
    yield* withStateService(rest, (state) => Effect.gen(function* () {
...
const getCommand = Command.make("get", {
    ...
    profile,
    local: localFlag,
}, instrumentCommand("state.get")(Effect.fn(function* ({ stack: stackName, stageName, fqn, ...rest }) {
    yield* withStateService(rest, (state) => Effect.gen(function* () {
...
const treeCommand = Command.make("tree", { main: script, envFile, profile, local: localFlag }, instrumentCommand("state.tree")(Effect.fn(function* (args) {
    yield* withStateService(args, (state) => Effect.gen(function* () {
        ...
        // output.
        const tree = yield* Effect.forEach(stacks, (stk) => Effect.gen(function* () {
            const stages = [...(yield* state.listStages(stk))].sort();
            const stageEntries = yield* Effect.forEach(stages, (stg) => Effect.map(state.list({ stack: stk, stage: stg }), (fqns) => ({ stage: stg, fqns: [...fqns].sort() })), { concurrency: "unbounded" });
...
const optionalStackFlag = Flag.string("stack").pipe(Flag.withDescription("Stack name to export. Omit to export ALL stacks in the store."), Flag.optional, Flag.map(Option.getOrUndefined));
const optionalStageFlag = Flag.string("stage").pipe(Flag.withDescription("Stage to export within the stack. Omit to export all stages."), Flag.optional, Flag.map(Option.getOrUndefined));
/**
...
const exportCommand = Command.make("export", {
    ...
    profile,
    local: localFlag,
}, instrumentCommand("state.export")(Effect.fn(function* ({ stack: stackName, stageName, ...rest }) {
    if (stageName !== undefined && stackName === undefined) {
        yield* Console.log("Error: cannot specify --stage without --stack. Pass the stack name via --stack.");
...
const clearStackFlag = Flag.string("stack").pipe(Flag.withDescription("Stack name to clear. Omit to clear ALL stacks in the store."), Flag.optional, Flag.map(Option.getOrUndefined));
const clearStageFlag = Flag.string("stage").pipe(Flag.withDescription("Stage to clear within the stack. Omit to clear all stages in the stack."), Flag.optional, Flag.map(Option.getOrUndefined));
/**
...
const clearCommand = Command.make("clear", {
    ...
    local: localFlag,
    yes,
}, instrumentCommand("state.clear")(Effect.fn(function* ({ stack: stackName, stageName, yes: yesFlag, ...rest }) {
    if (stageName !== undefined && stackName === undefined) {
        yield* Console.log("Error: cannot specify --stage without --stack. Pass the stack name via --stack.");
        return yield* Effect.fail(new Error("missing stack"));
    }
    yield* withStateService(rest, (state) => Effect.gen(function* () {
        const targets = stackName === undefined
        ...
        const scope = stackName === undefined
...
export const stateCommand = Command.make("state", {}).pipe(Command.withSubcommands([
    stacksCommand,
    stagesCommand,
    resourcesCommand,
    getCommand,
