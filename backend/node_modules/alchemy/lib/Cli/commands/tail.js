...
import { envFile, formatLocalTimestamp, importStack, instrumentCommand, parseResourceFilter, profile, resourceFilter, script, stage, TAIL_COLORS, TAIL_RESET, } from "./_shared.js";
export const tailCommand = Command.make("tail", {
    main: script,
    envFile,
    stage,
    profile,
    ...
    const services = Layer.mergeAll(ConfigProvider.layer(withProfileOverride(yield* loadConfigProvider(envFile), profile)), Layer.succeed(AuthProviders, {}), Layer.succeed(Stage, stage), Logger.layer([fileLogger("out")], { mergeWithExisting: true }), State.localState());
    yield* Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
            ...
            if (filterSet) {
                for (const id of filterSet) {
                    if (!availableIds.includes(id)) {
                        return yield* Effect.die(new Error(`Unknown resource '${id}' in --filter. Available: ${availableIds.join(", ") || "(none)"}`));
                    }
                }
            }
            ...
            for (const fqn of fqns) {
                ...
                if (filterSet && !filterSet.has(resource.LogicalId))
                    continue;
            ...
            if (tailable.length === 0) {
                if (filterSet) {
                    yield* Console.log("No tailable resources match --filter (deploy first, or selected resources may not support tail).");
                }
                else {
                    yield* Console.log("No tailable resources found. Deploy first, then run tail.");
                }
                return;
            }
            ...
            const taggedStreams = tailable.map(({ logicalId, stream }, i) => {
                ...
                return stream.pipe(Stream.map(({ timestamp, message }) => {
