...
import { buildBuiltinAuthProviders, buildStackProviders, envFile, instrumentCommand, printProfile, profile, } from "./_shared.js";
/**
 ...
 */
const mainFile = Argument.file("main").pipe(Argument.withDescription("Stack entrypoint whose providers() should be used, defaults to alchemy.run.ts"), Argument.withDefault("alchemy.run.ts"));
/**
...
export const collectAuthProviders = Effect.fn("collectAuthProviders")(function* (options) {
    ...
    yield* buildStackProviders({ ...options, registry: authProviders }).pipe(Effect.catchCause((cause) => Effect.logDebug("profile show: could not load user stack providers", {
        cause,
    })));
    return authProviders;
});
const showCommand = Command.make("show", { profile, envFile, main: mainFile }, instrumentCommand("profile.show", (a) => ({
...
const clearCommand = Command.make("clear", { profile }, instrumentCommand("profile.clear", (a) => ({
    "alchemy.profile": a.profile,
    ...
    if (removed) {
        yield* Console.log(`Cleared profile '${profile}' and all its credentials.`);
    }
    else {
        yield* Console.log(`Profile '${profile}' not found in profiles.json; removed any stray credentials directory.`);
    }
})));
export const profileCommand = Command.make("profile", {}).pipe(Command.withSubcommands([showCommand, clearCommand]));
