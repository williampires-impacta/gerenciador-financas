...
import { isNonInteractive } from "../selectCli.js";
import * as NukeUI from "../tui/components/Nuke.js";
import { buildStackProviders, dryRun, envFile, instrumentCommand, profile, script, yes, } from "./_shared.js";
const includeFlag = Flag.string("include").pipe(Flag.withDescription("Glob of provider IDs to include (e.g. 'Cloudflare.*' or 'Cloudflare.Worker'). " +
    "Repeatable; when omitted, every provider is included."), Flag.atLeast(0));
const excludeFlag = Flag.string("exclude").pipe(Flag.withDescription("Glob of provider IDs to exclude (applied after --include). Repeatable."), Flag.atLeast(0));
const filterFlag = Flag.string("filter").pipe(Flag.withDescription("JavaScript expression evaluated with `resource` in scope " +
    '(e.g. \'resource.Type === "Cloudflare.Worker" && ' +
    'resource.workerName.startsWith("alchemy-")\'). Any resource for which ' +
    "an expression is truthy is SPARED. Repeatable."), Flag.atLeast(0));
const verboseFlag = Flag.boolean("verbose").pipe(Flag.withAlias("v"), Flag.withDescription("List every individual resource that will be deleted, not just per-provider counts."), Flag.withDefault(false));
const concurrencyFlag = Flag.integer("concurrency").pipe(Flag.withDescription("Max number of providers scanned/deleted in parallel (resources within a " +
    "provider are always deleted concurrently). Use 0 for unbounded. " +
    "Default: 16 — unbounded tends to be slower because it triggers " +
    "provider-side rate limiting and retry backoff."), Flag.withDefault(16), Flag.map((n) => (n <= 0 ? "unbounded" : n)));
const timeoutFlag = Flag.integer("timeout").pipe(Flag.withDescription("Per-provider timeout (seconds) for each list/delete call, so one slow or " +
    "hanging provider can't stall the whole run. Default: 120."), Flag.withDefault(120));
const independentFlag = Flag.boolean("independent").pipe(Flag.withDescription("Delete every resource independently instead of in coordinated passes. " +
    ...
    "resources are deleted concurrently."), Flag.withDefault(false));
const retriesFlag = Flag.integer("retries").pipe(Flag.withDescription("With --independent: retries per resource after the initial delete " +
    "attempt (each attempt still bounded by --timeout). Default: 10."), Flag.withDefault(10));
const localFlag = Flag.boolean("local").pipe(Flag.withDescription("Enumerate and delete LOCAL (emulated) resources instead of real cloud " +
...
const nukeCommand = Command.make("nuke", {
    ...
    filter: filterFlag,
    local: localFlag,
}, instrumentCommand("unsafe.nuke", (a) => ({
    "alchemy.profile": a.profile,
    "alchemy.main": a.main,
}))(Effect.fn(function* ({ main, envFile, profile, yes, dryRun, verbose, concurrency, timeout, independent, retries, include, exclude, filter, local, }) {
    ...
    // ---- Delete phase ------------------------------------------------
    const totals = [...groupBy(targets, (t) => t.id).entries()].map(([id, items]) => ({ id, total: items.length }));
    ...
    if (remaining.length > 0) {
        yield* Console.log(`${remaining.length} resource(s) could not be deleted.`);
    }
}))).pipe(
// hide the command because it's dangerous and we don't want agents to discover and use it
Command.unlisted, Command.withDescription("Enumerate every live resource across the stack's providers and delete " +
...
export const unsafeCommand = Command.make("unsafe", {}).pipe(Command.withDescription("Dangerous, irreversible operations."), Command.withSubcommands([nukeCommand]));
