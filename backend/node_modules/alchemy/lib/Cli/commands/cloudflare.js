...
import { bootstrap as bootstrapCloudflare, teardownStateStore, } from "../../Cloudflare/StateStore/State.js";
...
import { envFile, formatLocalTimestamp, instrumentCommand, parseSince, profile, } from "./_shared.js";
/**
 ...
 * account out-of-band.
 */
const cloudflareLayers = (envFileOpt, profileName) => Effect.gen(function* () {
    const authProviders = {};
    ...
    const cf = Layer.provideMerge(Layer.mergeAll(CloudflareCredentials.fromAuthProvider(), CloudflareEnvironment.fromProfile(), CloudflareAccess.AccessLive), authLayer);
    const logger = Logger.layer([fileLogger("cloudflare.txt")], {
        mergeWithExisting: true,
    });
    return Layer.mergeAll(cf, ConfigProvider.layer(withProfileOverride(yield* loadConfigProvider(envFileOpt), profileName)), logger);
});
const cloudflareForce = Flag.boolean("force").pipe(Flag.withDescription("Force a full redeploy even if the state-store worker already exists. " +
    "Without this flag, an existing worker is adopted and only its credentials are refreshed."), Flag.withDefault(false));
const cloudflareWorkerName = Flag.string("worker-name").pipe(Flag.withDescription("Override the default state-store worker name (advanced; only needed for multiple state stores per account)."), Flag.optional, Flag.map(Option.getOrUndefined));
const bootstrapCommand = Command.make("bootstrap", {
    envFile,
    profile,
    force: cloudflareForce,
    workerName: cloudflareWorkerName,
}, instrumentCommand("cloudflare.bootstrap", (a) => ({
...
const teardownCommand = Command.make("teardown", {
    envFile,
    profile,
    workerName: cloudflareWorkerName,
}, instrumentCommand("cloudflare.teardown", (a) => ({
    "alchemy.profile": a.profile,
    "alchemy.worker_name": a.workerName ?? "",
...
}))).pipe(Command.unlisted, Command.withDescription("Tear down the cloudflare state store"));
/**
 ...
 */
const buildTokenPolicies = (accountIds, groups) => {
    const buckets = {
        "com.cloudflare.api.account": {
            effect: "allow",
            permissionGroups: [],
            resources: Object.fromEntries(accountIds.map((id) => [`com.cloudflare.api.account.${id}`, "*"])),
        },
 ...
 */
const selectAccountIds = (defaultAccountId) => Effect.gen(function* () {
    ...
    if (accounts.length === 0) {
        ...
        return yield* Effect.die("No Cloudflare accounts found for these credentials.");
    }
    ...
    return yield* Clank.multiselect({
        message: "Select the Cloudflare account(s) to scope the token to " +
...
const allPermissionsFlag = Flag.boolean("all-permissions").pipe(Flag.withDescription("Grant the token EVERY Cloudflare permission group (a 'god token'). " +
    "Use with care — it has full access to your account."), Flag.withDefault(false));
const tokenNameFlag = Flag.string("name").pipe(Flag.withDescription("Name for the API token. Defaults to 'alchemy' (or 'alchemy-all-permissions')."), Flag.optional, Flag.map(Option.getOrUndefined));
const tokenAccountIdFlag = Flag.string("account-id").pipe(Flag.withDescription("Cloudflare account ID(s) to scope the token to (comma-separated for " +
    "multiple). If omitted, you'll be prompted to select from your accounts."), Flag.optional, Flag.map(Option.match({
    onNone: () => undefined,
    onSome: (v) => v
        .split(",")
        .map((s) => s.trim())
...
const createTokenCommand = Command.make("create-token", {
...
}, instrumentCommand("cloudflare.create-token", (a) => ({
    "alchemy.all_permissions": a.allPermissions,
}))(Effect.fn(function* ({ envFile, allPermissions, name, accountId }) {
    const configProvider = ConfigProvider.layer(yield* loadConfigProvider(envFile));
    yield* Effect.gen(function* () {
        ...
        const credentials = Effect.succeed(apiKeyCredentials({ apiKey, email }));
        const withCreds = (self) => self.pipe(Effect.provideService(CloudflareCredentials.Credentials, credentials));
        const resolvedAccountIds = accountId ?? (yield* withCreds(selectAccountIds(undefined)));
        const tokenName = name ??
            (yield* Clank.text({
                message: "Token name",
                placeholder: allPermissions ? "alchemy-superuser" : "alchemy",
                validate: (v) => v.trim().length === 0 ? "Token name is required" : undefined,
            }));
        ...
        const pgResponse = yield* http.get("https://api.cloudflare.com/client/v4/user/tokens/permission_groups", {
            headers: {
                "X-Auth-Key": apiKey,
                "X-Auth-Email": email,
        ...
        const pgBody = (yield* pgResponse.json);
        const liveGroups = (pgBody.result ?? []).flatMap((g) => g.id && g.scopes && g.scopes.length > 0
        ...
        if (allPermissions) {
            ...
            const chosenIds = yield* Clank.multiselect({
                message: "Select the permission groups to grant (space to toggle, enter to confirm)",
        ...
        if (policies.length === 0) {
            return yield* Effect.die("No permission groups resolved for this account; cannot create a token.");
        }
        if (allPermissions) {
            yield* Clank.warn("This token will have FULL access to your Cloudflare account. " +
        ...
        const result = yield* withCreds(user.createToken({ name: tokenName, policies }));
        if (!result.value) {
            return yield* Effect.die("Cloudflare did not return a token value. Try again.");
        }
        ...
        const granted = (result.policies ?? []).reduce((n, p) => n + (p.permissionGroups?.length ?? 0), 0);
        ...
        const status = yield* http
            ...
            .pipe(Effect.flatMap((r) => r.json), Effect.map((b) => b.result?.status), Effect.catch(() => Effect.succeed(undefined)));
        yield* Console.log("");
        yield* Console.log(`Created Cloudflare API token "${result.name ?? tokenName}" (${result.id ?? "unknown id"}).`);
        yield* Console.log(`Granted ${granted} permission group(s) across ${result.policies?.length ?? 0} policy(ies)` +
            (status ? `; token status: ${status}.` : "."));
        yield* Console.log("");
        yield* Console.log(result.value);
        yield* Console.log("");
        yield* Console.log("Store this value now — Cloudflare only shows it once. " +
            "Use it as CLOUDFLARE_API_TOKEN.");
        if (granted === 0) {
            yield* Clank.warn("Cloudflare granted 0 permissions. A token can only carry permissions " +
            ...
            yield* Clank.info("Heads up: the Cloudflare dashboard often renders API-created tokens " +
...
const tailFlag = Flag.boolean("tail").pipe(Flag.withDescription("Stream logs in real time via the Cloudflare tail websocket instead of fetching past entries."), Flag.withDefault(false));
const limitFlag = Flag.integer("limit").pipe(Flag.withDescription("Number of log entries to fetch (ignored with --tail)"), Flag.withDefault(100));
const sinceFlag = Flag.string("since").pipe(Flag.withDescription("Fetch logs since this time (e.g. '1h', '30m', '2024-01-01T00:00:00Z')"), Flag.optional, Flag.map(Option.getOrUndefined));
/**
 ...
 */
const stateLogsCommand = Command.make("logs", {
    envFile,
    profile,
    workerName: cloudflareWorkerName,
    tail: tailFlag,
    limit: limitFlag,
    since: sinceFlag,
}, instrumentCommand("cloudflare.state.logs", (a) => ({
    "alchemy.profile": a.profile,
    ...
    yield* Effect.gen(function* () {
        ...
        const formatLine = (line) => `${formatLocalTimestamp(line.timestamp)} [${scriptName}] ${line.message}`;
        if (tail) {
        ...
        for (const line of lines.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime())) {
            yield* Console.log(formatLine(line));
        }
    }).pipe(Effect.provide(services));
})));
const stateCommand = Command.make("state", {}).pipe(Command.withSubcommands([stateLogsCommand]));
export const cloudflareCommand = Command.make("cloudflare", {}).pipe(Command.withSubcommands([
    bootstrapCommand,
    teardownCommand,
    createTokenCommand,
    stateCommand,
