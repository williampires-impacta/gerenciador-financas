 ...
 */
const pickDistTag = (current, distTags) => {
    ...
    if (channelPick === undefined)
        return latest;
    if (latest === undefined)
        return channelPick;
    return compareVersions(latest, channelPick) > 0 ? latest : channelPick;
};
const readCache = Effect.fn(function* (cachePath) {
...
const writeCache = Effect.fn(function* (cachePath, distTags) {
    const fs = yield* FileSystem.FileSystem;
    const checkedAt = yield* Clock.currentTimeMillis;
    const cache = { checkedAt, distTags };
...
const fetchDistTags = Effect.gen(function* () {
    ...
    const distTags = (yield* response.json);
 ...
 */
const refreshDistTags = Effect.fn(function* (cachePath) {
    ...
    Effect.tap((distTags) => writeCache(cachePath, distTags)), Effect.tapError(() => writeCache(cachePath)), Effect.forkScoped);
    return yield* Fiber.join(fetch).pipe(Effect.timeout(SYNC_WAIT), Effect.catch(() => Effect.succeed(undefined)));
});
/**
...
export const checkLatestVersion = Effect.gen(function* () {
    ...
    const distTags = cached !== undefined && now - cached.checkedAt <= CACHE_TTL_MILLIS
        ? cached.distTags
    ...
    // prompt a "downgrade".
    if (latest === undefined || compareVersions(latest, current) <= 0)
        return;
    const installCmd = typeof process !== "undefined" && process.versions?.bun
    ...
    const message = `alchemy ${latest} is available (you're on ${current}). ` +
