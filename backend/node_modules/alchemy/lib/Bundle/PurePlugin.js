 ...
 */
export const DEFAULT_PURE_PACKAGES = [
    "effect",
    "@effect/*",
    "alchemy",
    "@alchemy.run/*",
 ...
 */
export const purePlugin = (options = {}) => {
    ...
    return {
        ...
        options(opts) {
            for (const input of inputFilePaths(opts))
                entryPaths.add(input);
            return null;
        },
        transform: {
...
const stripIdSuffix = (id) => id.replace(/[?#].*$/, "");
/**
 ...
 */
function inputFilePaths(opts) {
    const raw = typeof opts.input === "string"
        ? [opts.input]
    ...
    return raw
        .filter((entry) => typeof entry === "string" && !entry.startsWith("\0"))
 ...
 */
export async function resolvePackageInfo(startDir, cache) {
 ...
 *     // => "@effect/cluster"
 */
export function packageNameFromId(id) {
...
const collectPureAnchorsCached = async (code, filename) => {
 ...
 */
export async function collectPureAnchors(code, filename) {
    ...
    const bound = [];
    const discarded = [];
    const visitCall = (call, isDiscarded) => {
        if (isIIFE(call))
            return;
        ...
        if (alreadyAnnotated(code, anchor))
            return;
        (isDiscarded ? discarded : bound).push(anchor);
    };
    const visitExpression = (expr, isDiscarded) => {
        if (!expr)
            return;
        switch (expr.type) {
            case "CallExpression":
    ...
    const visitTopLevel = (node) => {
        switch (node.type) {
            case "ExpressionStatement":
                visitExpression(node.expression, true);
                return;
