...
import { BundleError, bundleErrorFromUnknown, bundleOutputFromFiles, } from "./Bundle.js";
 ...
 */
export const viteBuildOutputPlugin = Effect.fn(function* ({ entryEnvironment = "ssr", }) {
    ...
    const maybeExternalWorkspaces = new Set();
    const findUp = yield* cachedFunction((dir, filenames) => Effect.filter(filenames.map((filename) => path.join(dir, filename)), fs.exists, { concurrency: "unbounded" }).pipe(Effect.flatMap(([match]) => {
        if (match) {
    ...
    const collectExternalWorkspaces = () => Effect.forEach(maybeExternalWorkspaces, (directory) => findUp(directory, ["package.json"])).pipe(Effect.map((paths) => new Set(paths.filter((file) => file !== undefined).map(path.dirname))));
    const plugin = {
        ...
        async writeBundle(_, bundle) {
            ...
            await Promise.all(files.map(async (file) => {
                if (file.type === "chunk") {
                    file.imports
                        .filter((self) => self in RSC_MANIFEST)
                        .forEach((id) => {
                        ...
                        serverChunks.set(fileName(RSC_MANIFEST[id], this.environment), readRscManifestChunk(id, this.environment));
                    });
                }
                const name = fileName(file.fileName, this.environment);
                const content = file.type === "chunk" ? file.code : file.source;
                serverChunks.set(name, sha256(content).pipe(Effect.map((hash) => ({ path: name, content, hash }))));
            }));
        },
    };
    ...
    const fileName = (name, environment) => {
    ...
    const makeServerBundle = () => {
        if (!serverEntry && !serverChunks.size)
            return Effect.undefined;
        if (!serverEntry)
        ...
        return Effect.all(server, { concurrency: "unbounded" }).pipe(Effect.flatMap(bundleOutputFromFiles));
    };
    const getChunk = (path) => {
