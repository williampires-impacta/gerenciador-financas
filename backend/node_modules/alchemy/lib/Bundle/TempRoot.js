 ...
 */
export const resolveMainPath = Effect.fn(function* (main) {
 ...
 */
export const createTempBundleDir = (entry, dotAlchemy, id) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
 ...
 */
export const getStableContextDir = (entry, dotAlchemy, id) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const stack = yield* Stack;
    const stage = yield* Stage;
 ...
 */
export const cleanupBundleTempDir = (tempDir) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    yield* fs.remove(tempDir, { recursive: true }).pipe(Effect.ignore);
});
const findBundleTempRoot = (entry, dotAlchemy) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
export const findCwdForBundle = Effect.fn(function* (entry) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    let current = path.dirname(entry);
    while (true) {
...
//# sourceMappingURL=TempRoot.js.map