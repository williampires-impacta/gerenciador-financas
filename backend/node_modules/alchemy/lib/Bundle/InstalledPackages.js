 ...
 * imports, etc.).
 */
export function parsePackageRoot(specifier) {
 ...
 */
export function parsePackageRootFromSpecifier(specifier) {
...
export function matchesPackageRoot(moduleId, root) {
    return moduleId === root || moduleId.startsWith(`${root}/`);
}
export function npmInstallArgs(architecture) {
    return npmCommandArgs("ci", architecture);
}
export function npmLockfileArgs(architecture) {
    return [
...
export function npmPlainInstallArgs(architecture) {
    return npmCommandArgs("install", architecture);
}
const npmCommandArgs = (command, architecture) => {
 ...
 */
export function normalizeInstallTargets(install) {
    if (!install)
        return Effect.succeed({});
    const entries = Array.isArray(install)
        ? install.map((dep) => [dep, "*"])
...
export function resolvePackageInstallPlan(options) {
    ...
    return Effect.gen(function* () {
        ...
        for (const packageName of packageNames) {
            resolved[packageName] = yield* resolveInstallVersion(options.cwd, sourcePackageJson, packageName, options.requested[packageName]);
        }
 ...
 */
export function resolvePackageInstallIdentity(options) {
    return Effect.gen(function* () {
...
export function hashPackageInstallIdentity(options) {
    return sha256Object({
 ...
 */
export function installResolvedPackages(options) {
    const packageNames = Object.keys(options.resolved).sort();
    if (packageNames.length === 0)
        return Effect.succeed([]);
    const runInstall = (directory, args) => options.runNpmInstall === undefined
    ...
    return Effect.gen(function* () {
        ...
        return yield* Effect.acquireUseRelease(fileSystem.makeTempDirectory({ prefix: "alchemy-lambda-packages-" }), (directory) => Effect.gen(function* () {
            ...
            yield* fileSystem.writeFileString(pathService.join(directory, "package.json"), `${manifest}\n`);
            if (hasOverrides) {
...
const runNpmInstall = (directory, args) => Effect.sync(() => ChildProcess.setCwd(ChildProcess.make("npm", args, {
    shell: false,
    env: { ...process.env },
}), directory)).pipe(Effect.flatMap(exec), Effect.scoped, Effect.mapError((cause) => {
...
}), Effect.flatMap(({ exitCode, stderr }) => exitCode === 0
    ? Effect.void
    : Effect.fail(new BundleError({
        message: `npm install for build.install failed with exit code ${exitCode}: ${stderr}`,
    }))));
const printBunBinaryLockfile = (lockfilePath) => Effect.sync(() => ChildProcess.make("bun", [lockfilePath], {
    shell: false,
    env: { ...process.env },
})).pipe(Effect.flatMap(exec), Effect.scoped, Effect.mapError((cause) => {
    const message = cause instanceof Error ? cause.message : String(cause);
    return new BundleError({
        message: message.includes("ENOENT")
...
const pinInstallVersionsFromLockfile = (options) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    ...
    const content = lockfileName === "bun.lockb"
        ? yield* printBunBinaryLockfile(options.lockfilePath)
        : yield* fs.readFileString(options.lockfilePath).pipe(Effect.mapError((cause) => new BundleError({
            message: `Failed to read package-manager lockfile for Lambda externals from '${options.cwd}'`,
            cause,
        })));
    const plan = yield* Effect.try({
        try: () => parseLockedInstallPlan({
            name: lockfileName === "bun.lockb" ? "yarn.lock" : lockfileName,
            content,
            importer,
            candidates,
            packageJson: options.packageJson,
            resolved: Object.fromEntries(Object.entries(options.resolved).filter(([packageName]) => !candidates.includes(packageName))),
        }),
        catch: (cause) => new BundleError({
 ...
 */
const addLockedGraphOverrides = (overrides, roots, nodeId, dependencies) => {
    ...
    const asOverrideMap = (parent, dependencyName) => {
    ...
    const resolveConflicts = (out, node, conflicted, path) => {
        const still = new Set();
        ...
        for (const edge of edgesOf(node)) {
            ...
            if (![...still].some((name) => childSpecs.has(name)))
                continue;
            const record = asOverrideMap(out, edge.dependencyName);
            if (record["."] === undefined)
                record["."] = edge.installSpec;
    ...
    for (const { node, rootSelector } of roots) {
        ...
        const rootMap = typeof existing === "object" ? existing : {};
        if (typeof existing === "string")
            rootMap["."] = existing;
        const specs = specsOf(node);
        if (specs.size === 0)
...
const parseLockedInstallPlan = (options) => {
    switch (options.name) {
...
const parsePackageLock = (options) => {
    ...
    for (const packageName of options.candidates) {
        ...
        if (resolved[packageName] === options.resolved[packageName] &&
            importerKey === "") {
    ...
    if (packages !== undefined) {
        addLockedGraphOverrides(overrides, roots, (packagePath) => packagePath, (packagePath) => {
            const value = packages[packagePath];
            const entry = asRecord(value);
            if (entry === undefined)
                return [];
            return Object.keys(dependencySpecifiers(entry)).flatMap((dependencyName) => {
                const childPath = resolvePackageLockDependencyPath(packages, packagePath, dependencyName);
...
const parsePnpmLock = (options) => {
    ...
    const importerKey = options.importer === "" ? "." : options.importer;
    const importer = asRecord(importers?.[importerKey]) ??
    ...
    addLockedGraphOverrides(overrides, roots, (packageKey) => packageKey, (packageKey) => {
        const value = packages[packageKey];
        const parent = parsePnpmPackageKey(packageKey);
        const entry = asRecord(value);
        if (parent === undefined || entry === undefined)
            return [];
        return Object.entries(dependencySpecifiers(entry)).flatMap(([dependencyName, dependencyEntryValue]) => {
            const rawVersion = typeof dependencyEntryValue === "string"
...
const parseBunLock = (options) => {
    ...
    for (const packageName of options.candidates) {
        ...
        const direct = findBunPackageDescriptor(packages, [
    ...
    addLockedGraphOverrides(overrides, roots, (packageKey) => packageKey, (packageKey) => {
        ...
        if (parent === undefined || metadata === undefined)
            return [];
        return Object.entries(dependencySpecifiers(metadata)).flatMap(([dependencyName, dependencySpecifier]) => {
            if (typeof dependencySpecifier !== "string")
                return [];
            const expectedName = npmAliasName(dependencyName, dependencySpecifier);
            const child = findBunPackageDescriptor(packages, [`${packageKey}/${dependencyName}`, dependencyName], expectedName);
            if (child === undefined)
                return [];
            return [
                {
...
const parseYarnLock = (options) => {
    ...
    addLockedGraphOverrides(overrides, roots, (entry) => entry, (entry) => Object.entries(entry.dependencies).flatMap(([dependencyName, dependencySpecifier]) => {
...
const dependencySpecifiers = (entry) => ({
    ...asRecord(entry.dependencies),
    ...asRecord(entry.optionalDependencies),
    ...asRecord(entry.peerDependencies),
});
const lockedInstallSpec = (dependencyName, actualName, version) => actualName === dependencyName ? version : `npm:${actualName}@${version}`;
const npmLockedInstallSpec = (dependencyName, entry, version) => lockedInstallSpec(dependencyName, stringValue(entry?.name) ?? dependencyName, version);
const resolvePackageLockDependencyPath = (packages, packagePath, dependencyName) => {
...
const addPackageLockV1Overrides = (dependencies, overrides, rootNames) => {
    const roots = rootNames.flatMap((rootName) => {
...
const pnpmInstallSpec = (dependencyName, rawVersion) => {
...
const findPnpmPackageKey = (packages, dependencyName, rawVersion) => {
    if (rawVersion === undefined)
        return undefined;
    let version = rawVersion.startsWith("/") ? rawVersion.slice(1) : rawVersion;
...
const parsePnpmPackageKey = (packageKey) => {
    ...
    if (peerSuffix !== -1)
        descriptor = descriptor.slice(0, peerSuffix);
    return parsePackageDescriptor(descriptor);
};
const parsePackageDescriptor = (descriptor) => {
...
const npmAliasName = (dependencyName, specifier) => {
    if (!specifier.startsWith("npm:"))
        return dependencyName;
    return (parsePackageDescriptor(specifier.slice("npm:".length))?.name ??
        dependencyName);
};
const findBunPackageDescriptor = (packages, candidateKeys, expectedName) => {
    ...
    const matches = Object.entries(packages)
        ...
        .filter((entry) => entry?.name === expectedName);
...
const parseYarnV1Lockfile = (content) => {
    const root = {};
    const stack = [
        { indent: -1, node: root },
    ];
...
const stripYarnQuotes = (value) => value.startsWith('"') && value.endsWith('"') && value.length >= 2
    ? value.slice(1, -1)
    : value;
const parseYarnEntries = (content) => {
    return Object.entries(parseYarnLockfile(content)).flatMap(([selectorList, value]) => {
        if (selectorList === "__metadata")
            return [];
        const entry = asRecord(value);
        const version = stringValue(entry?.version);
        if (version === undefined)
...
const findYarnEntry = (entries, dependencyName, specifier) => {
    const expected = `${dependencyName}@${specifier}`;
    const berryExpected = specifier.startsWith("npm:")
        ? expected
        : `${dependencyName}@npm:${specifier}`;
    return entries.find((entry) => entry.selectors.some((selector) => selector === expected || selector === berryExpected));
};
const yarnResolutionName = (resolution) => {
    if (resolution === undefined)
        return undefined;
    const npmMarker = resolution.indexOf("@npm:");
    return npmMarker === -1 ? undefined : resolution.slice(0, npmMarker);
};
const dependencyEntry = (importer, packageName) => asRecord(importer?.dependencies)?.[packageName] ??
    asRecord(importer?.optionalDependencies)?.[packageName] ??
    asRecord(importer?.devDependencies)?.[packageName];
const dependencyValue = (importer, packageName) => stringValue(dependencyEntry(importer, packageName));
const declaredPackageVersion = (packageJson, packageName) => packageJson.dependencies?.[packageName] ??
    packageJson.optionalDependencies?.[packageName] ??
    packageJson.devDependencies?.[packageName];
const nodeModulesLookupPaths = (importer, packageName) => {
    const paths = [];
...
const parseBunPackageDescriptor = (value) => {
 ...
 */
const parseJsonc = (content) => JSON.parse(stripJsonc(content));
...
const resolveCatalogVersion = (cwd, packageName, version) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
...
const findUp = (cwd, filenames) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
const readNearestLockfileFingerprint = (cwd) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
const resolveBunCatalogVersion = (cwd, packageName, version) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    let currentDir = cwd;
    while (true) {
...
const parseBunCatalogSource = (manifest) => {
    ...
    const catalogs = manifest.catalogs !== undefined || workspaceSource?.catalogs !== undefined
...
const parseBunWorkspacesCatalogSource = (workspaces) => {
...
const resolveCatalogEntry = (packageName, version, source) => {
...
const readArtifactFiles = (directory) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
