 ...
 */
export class RemovalPolicy extends Context.Service()("RemovalPolicy") {
}
/**
 ...
 */
export const retain = ((enabled = true) => (eff) => eff.pipe(typeof enabled === "boolean"
    ? Effect.provideService(RemovalPolicy, enabled ? "retain" : "destroy")
    : Effect.provideServiceEffect(RemovalPolicy, enabled.pipe(Effect.map((a) => (a ? "retain" : "destroy"))))));
/**
 ...
 *   May also be an `Effect` producing the boolean.
 */
export const destroy = ((enabled = true) => (eff) => eff.pipe(typeof enabled === "boolean"
    ? Effect.provideService(RemovalPolicy, enabled ? "destroy" : "retain")
    : Effect.provideServiceEffect(RemovalPolicy, enabled.pipe(Effect.map((a) => (a ? "destroy" : "retain"))))));
//# sourceMappingURL=RemovalPolicy.js.map