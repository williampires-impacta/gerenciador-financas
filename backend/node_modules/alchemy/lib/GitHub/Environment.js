 ...
 * @resource
 */
export const Environment = Resource("GitHub.Environment");
...
export const resolveEnvironmentName = (environment) => {
...
export const EnvironmentProvider = () => Provider.succeed(Environment, {
    ...
    reconcile: Effect.fn(function* ({ news }) {
        ...
        const reviewers = yield* Effect.tryPromise({
            try: async () => {
                if (news.reviewers === undefined)
                    return null;
                const users = await Promise.all((news.reviewers.users ?? []).map(async (username) => {
                    const { data } = await octokit.rest.users.getByUsername({
                        username,
                    });
                    return { type: "User", id: data.id };
                }));
                const teams = await Promise.all((news.reviewers.teams ?? []).map(async (team_slug) => {
        ...
        const environment = yield* Effect.tryPromise({
            try: async () => {
                const { data } = await octokit.rest.repos.createOrUpdateEnvironment({
                    ...
                    deployment_branch_policy: news.deploymentBranchPolicy === undefined
                        ? null
        ...
        if (news.deploymentBranchPolicy !== undefined &&
            "customBranchPolicies" in news.deploymentBranchPolicy) {
            const desired = news.deploymentBranchPolicy.customBranchPolicies;
            yield* Effect.tryPromise({
                try: async () => {
                    const observed = await octokit.paginate(octokit.rest.repos.listDeploymentBranchPolicies, {
                        owner: news.owner,
                        repo: news.repository,
                        environment_name: news.name,
                        per_page: 100,
            ...
            try: async () => {
                try {
                    ...
                    for (let page = 1;; page++) {
                        ...
                        environments.push(...batch.map((environment) => ({
                            environmentId: environment.id,
                            nodeId: environment.node_id,
                            name: environment.name,
                            htmlUrl: environment.html_url,
