...
export const RpcServerBun = RpcServer.layerServer(Effect.fn(function* ({ parentConnected, parentDisconnected, createRpcSession, }) {
    const server = yield* Effect.sync(() => Bun.serve({
        port: 0,
        fetch: (request, server) => {
            const url = new URL(request.url);
            if (server.upgrade(request, {
                data: url.pathname === "/parent"
                    ? { type: "parent" }
                    : {
                        type: "pending",
        ...
        websocket: {
            open: (ws) => {
                if (ws.data && ws.data.type === "parent") {
                    ...
                    const sessionEnv = ws.data && ws.data.type === "pending"
                        ? ws.data.sessionEnv
                        : undefined;
                    ws.data = {
                        type: "session",
...
//# sourceMappingURL=RpcServerBun.js.map