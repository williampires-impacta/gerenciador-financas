 ...
 * @resource
 */
export const Image = Resource("Docker.Image");
export const ImageProvider = () => Provider.effect(Image, Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const docker = yield* Docker;
    const buildAndInspectImage = Effect.fn(function* (id, props, instanceId) {
    ...
    const resolveBuildPaths = Effect.fn(function* (build) {
    ...
    return Image.Provider.of({
        ...
        read: Effect.fn(function* ({ id, instanceId, olds, output }) {
            ...
            const ref = output?.imageRef ??
                (yield* dockerPhysicalName(id, olds, instanceId).pipe(Effect.map((name) => `${name}:${olds.tag ?? "latest"}`)));
        ...
        delete: Effect.fn(({ olds, output }) => docker.image
