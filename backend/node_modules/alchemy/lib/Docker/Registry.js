...
export const repositoryFromImageRef = (imageRef) => {
 ...
 */
export const withRegistryHost = (imageRef, registry) => {
    const registryHost = registry.server.replace(/\/$/, "");
    const firstSegment = imageRef.split("/")[0];
    const hasRegistryPrefix = imageRef.includes("/") &&
...
export const parseRepoDigest = (imageRef, output) => {
 ...
 */
export const parseCreatedAt = (created) => {
