 ...
 * @resource
 */
export const Volume = Resource("Docker.Volume");
export const VolumeProvider = () => Provider.effect(Volume, Effect.gen(function* () {
    ...
    return Volume.Provider.of({
        ...
        delete: Effect.fn(({ olds, output }) => docker.volume
            .remove(output.name, dockerContextName(olds.context))
            .pipe(Effect.catchReason("PlatformError", "NotFound", () => Effect.void))),
    });
}));
const makeVolumeArgs = (id, props, instanceId) => dockerPhysicalName(id, props, instanceId).pipe(Effect.map((name) => ({
    name,
    driver: props.driver ?? "local",
    opt: props.driverOpts,
    label: props.labels,
})));
export const toVolumeAttributes = (info) => ({
    id: info.Name,
    name: info.Name,
    driver: info.Driver,
