 ...
 * @resource
 */
export const ChannelAssociation = Resource("AWS.Notifications.ChannelAssociation");
export const ChannelAssociationProvider = () => Provider.effect(ChannelAssociation, Effect.gen(function* () {
    // The association has no ARN of its own — identity is the
    // (configuration, channel) pair, observed via ListChannels.
    const findAssociation = Effect.fn(function* (notificationConfigurationArn, channelArn) {
        return yield* pinNotificationsRegion(notifications.listChannels
            .items({ notificationConfigurationArn })
            .pipe(Stream.filter((arn) => arn === channelArn), Stream.runHead, Effect.map(Option.getOrUndefined), 
    ...
    return ChannelAssociation.Provider.of({
        ...
        read: Effect.fn(function* ({ olds, output }) {
            const configArn = output?.notificationConfigurationArn ??
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            const existing = yield* findAssociation(news.notificationConfigurationArn, news.channelArn);
