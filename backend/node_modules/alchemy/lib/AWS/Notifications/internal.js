...
export const pinNotificationsRegion = (effect) => effect.pipe(Effect.provideService(Region, Effect.succeed(US_EAST_1)));
/**
 * Read the observed tags on a User Notifications resource, tolerating a
 * not-yet-visible resource (returns `{}`).
 */
export const readNotificationsTags = Effect.fn(function* (arn) {
    return yield* pinNotificationsRegion(notifications.listTagsForResource({ arn }).pipe(Effect.map((r) => (r.tags ?? {})), Effect.catchTag(["ResourceNotFoundException", "ValidationException"], () => Effect.succeed({}))));
});
/**
 ...
 */
export const syncNotificationsTags = Effect.fn(function* (arn, id, userTags) {
    ...
    if (upsert.length > 0) {
        yield* pinNotificationsRegion(notifications.tagResource({
            arn,
