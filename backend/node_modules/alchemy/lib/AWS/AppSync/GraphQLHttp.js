...
import { GraphQL, GraphQLApiError, } from "./GraphQL.js";
/**
 ...
 */
export const GraphQLHttp = Layer.effect(GraphQL, Effect.gen(function* () {
    ...
    return Effect.fn(function* (api) {
        ...
        const execute = Effect.fn(`AWS.AppSync.GraphQL(${api.LogicalId})`)(function* (request) {
            ...
            const { credentials, region } = yield* Effect.gen(function* () {
                ...
                const region = /\.appsync-api\.([a-z0-9-]+)\.amazonaws\.com$/.exec(url.hostname)?.[1] ?? (yield* yield* Region.Region);
            ...
            const toError = (status) => (cause) => new GraphQLApiError({
                status,
