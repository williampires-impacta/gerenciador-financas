 ...
 * @resource
 */
export const GroupingConfiguration = Resource("AWS.ApplicationSignals.GroupingConfiguration");
...
const sameDefinitions = (desired, observed) => desired.length === observed.length &&
    desired.every((d, i) => JSON.stringify(normalizeDefinition(d)) ===
        JSON.stringify(normalizeDefinition(observed[i])));
export const GroupingConfigurationProvider = () => Provider.effect(GroupingConfiguration, Effect.gen(function* () {
    /**
     ...
     */
    const observe = appsignals.listGroupingAttributeDefinitions({}).pipe(Effect.map((response) => response.UpdatedAt === undefined &&
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, session }) {
            ...
            const fresh = observed !== undefined &&
...
//# sourceMappingURL=GroupingConfiguration.js.map