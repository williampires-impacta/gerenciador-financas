 ...
 * @resource
 */
export const Connection = Resource("AWS.CodeConnections.Connection");
export const ConnectionProvider = () => Provider.effect(Connection, Effect.gen(function* () {
    const toName = (id, props) => props.connectionName
    ...
    const findByName = Effect.fn(function* (name) {
        const connections = yield* codeconnections.listConnections
            .pages({})
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Connections ?? [])));
        return connections.find((c) => c.ConnectionName === name);
    });
    const toAttrs = (connection, name) => ({
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            if (connection?.ConnectionArn === undefined)
                return undefined;
        ...
        list: () => codeconnections.listConnections.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.Connections ?? [])
            .flatMap((c) => c.ConnectionArn !== undefined
            ? [
                {
