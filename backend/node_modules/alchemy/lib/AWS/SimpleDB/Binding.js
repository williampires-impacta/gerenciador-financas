 ...
 */
export const registerSimpleDbBinding = (operation, domain) => Effect.gen(function* () {
 ...
 */
export const makeSimpleDbBinding = (options) => Effect.gen(function* () {
    ...
    return Effect.fn(function* (domain) {
        ...
        return Effect.fn(`AWS.SimpleDB.${options.operation}(${domain.LogicalId})`)(function* (request) {
            const name = yield* domainName;
            return yield* call({ ...request, DomainName: name });
        });
    });
});
