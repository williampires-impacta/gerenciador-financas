 ...
 * @resource
 */
export const ClusterSubnetGroup = Resource("AWS.Redshift.ClusterSubnetGroup");
/**
 ...
 */
const retryWhileSubnetGroupInUse = (self) => Effect.retry(self, {
    while: (e) => e._tag === "InvalidClusterSubnetGroupStateFault",
    schedule: Schedule.max([Schedule.fixed("15 seconds"), Schedule.recurs(40)]),
});
export const ClusterSubnetGroupProvider = () => Provider.effect(ClusterSubnetGroup, Effect.gen(function* () {
    const toName = (id, props) => props.clusterSubnetGroupName
    ...
    const readGroup = Effect.fn(function* (name) {
        const response = yield* redshift
            ...
            .pipe(Effect.catchTag("ClusterSubnetGroupNotFoundFault", () => Effect.succeed(undefined)));
    ...
    const toAttrs = Effect.fn(function* (group) {
        ...
        return {
            ...
            clusterSubnetGroupArn: redshiftArn(region, accountId, "subnetgroup", group.ClusterSubnetGroupName),
            description: group.Description,
            vpcId: group.VpcId,
            subnetIds: (group.Subnets ?? []).flatMap((subnet) => subnet.SubnetIdentifier ? [subnet.SubnetIdentifier] : []),
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!observed?.ClusterSubnetGroupName) {
                ...
                if (!observed?.ClusterSubnetGroupName) {
                    return yield* Effect.fail(new Error(`Failed to create cluster subnet group '${name}'`));
                }
            }
            // 3. Sync — diff OBSERVED description/subnets against desired;
            //    modifyClusterSubnetGroup replaces the whole subnet list.
            const observedSubnets = (observed.Subnets ?? []).flatMap((subnet) => subnet.SubnetIdentifier ? [subnet.SubnetIdentifier] : []);
            if (observed.Description !== description ||
                ...
                observed = yield* readGroup(name);
                if (!observed?.ClusterSubnetGroupName) {
                    return yield* Effect.fail(new Error(`Cluster subnet group '${name}' not found after update`));
                }
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryWhileSubnetGroupInUse(redshift
        ...
        redshift.describeClusterSubnetGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.ClusterSubnetGroups ?? []).filter((group) => group.ClusterSubnetGroupName !== undefined))), Effect.flatMap(Effect.forEach((group) => toAttrs(group), { concurrency: 4 }))),
    };
}));
//# sourceMappingURL=ClusterSubnetGroup.js.map