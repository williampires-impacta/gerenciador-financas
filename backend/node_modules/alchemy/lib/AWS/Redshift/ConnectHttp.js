    ...
    return Effect.fn(function* (cluster, options = {}) {
        ...
        if (!globalThis.__ALCHEMY_RUNTIME__) {
            ...
            if (isBindingHost(host)) {
                ...
                const arnBase = Output.map(cluster.clusterArn, (arn) => arn.slice(0, arn.lastIndexOf(":cluster:")));
                ...
                yield* host.bind `Allow(${host}, AWS.Redshift.Connect(${cluster}))`({
                    policyStatements: options.dbUser !== undefined
                        ? [
                            {
                                Effect: "Allow",
                                Action: [
                                ...
                                ? [
                                    {
                                        ...
                                        Resource: options.dbGroups.map((group) => Output.interpolate `${arnBase}:dbgroup:${cluster.clusterIdentifier}/${group}`),
                                    },
                                ]
                                : []),
                        ]
        ...
        return Effect.gen(function* () {
            ...
            if (!host) {
                return yield* Effect.die(`Redshift cluster endpoint for '${cluster.LogicalId}' is not available yet`);
            }
            const credentials = options.dbUser !== undefined
