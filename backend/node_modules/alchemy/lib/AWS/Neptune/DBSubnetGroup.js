 ...
 * @resource
 */
export const DBSubnetGroup = Resource("AWS.Neptune.DBSubnetGroup");
export const DBSubnetGroupProvider = () => Provider.effect(DBSubnetGroup, Effect.gen(function* () {
    const toName = (id, props) => props.dbSubnetGroupName
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.dbSubnetGroupName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!observed?.DBSubnetGroupName) {
                ...
                if (!observed?.DBSubnetGroupName) {
                    return yield* Effect.fail(new Error(`Failed to create DB subnet group '${dbSubnetGroupName}'`));
                }
            }
            else {
                ...
                observed = yield* readGroup(dbSubnetGroupName);
                if (!observed?.DBSubnetGroupName) {
                    return yield* Effect.fail(new Error(`DB subnet group '${dbSubnetGroupName}' not found after update`));
                }
            }
            ...
            return {
                ...
                subnetIds: (observed.Subnets ?? []).flatMap((subnet) => subnet.SubnetIdentifier ? [subnet.SubnetIdentifier] : []),
        ...
        neptune.describeDBSubnetGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.DBSubnetGroups ?? []).flatMap((group) => group.DBSubnetGroupName
            ? [
                {
                    dbSubnetGroupName: group.DBSubnetGroupName,
                    dbSubnetGroupArn: group.DBSubnetGroupArn,
                    vpcId: group.VpcId,
                    subnetIds: (group.Subnets ?? []).flatMap((subnet) => subnet.SubnetIdentifier
