 ...
 * @resource
 */
export const Table = Resource("AWS.DynamoDB.Table");
export const TableProvider = () => Provider.effect(Table, Effect.gen(function* () {
    ...
    const toKeyAttributeNames = (key) => key === undefined ? [] : typeof key === "string" ? [key] : key;
    ...
    const toIndexKeySchema = (partitionKey, sortKey) => [
    ...
    const isLegacyWireIndex = (index) => typeof index === "object" && index !== null && "KeySchema" in index;
    const toWireGlobalSecondaryIndex = (index) => isLegacyWireIndex(index)
        ? index
        : {
    ...
    const toWireLocalSecondaryIndex = (tablePartitionKey, index) => isLegacyWireIndex(index)
        ? index
        : {
            IndexName: index.indexName,
    ...
    const toWireGlobalSecondaryIndexes = (indexes) => indexes === undefined || indexes.length === 0
        ? undefined
        : indexes.map(toWireGlobalSecondaryIndex);
    const toWireLocalSecondaryIndexes = (tablePartitionKey, indexes) => indexes === undefined || indexes.length === 0
        ? undefined
        : indexes.map((index) => toWireLocalSecondaryIndex(tablePartitionKey, index));
    const toAttributeDefinitions = (attrs) => Object.entries(attrs)
        .map(([name, type]) => ({
        AttributeName: name,
        AttributeType: type,
    }))
        .sort((a, b) => a.AttributeName.localeCompare(b.AttributeName));
    const adoptExistingTable = (tableName) => 
    ...
    const normalizeStreamSpecification = (streamSpecification) => streamSpecification?.StreamEnabled === true
        ? {
            StreamEnabled: true,
            StreamViewType: streamSpecification.StreamViewType,
        }
        : undefined;
    const resolveStreamSpecification = (bindings) => Effect.gen(function* () {
        const requested = bindings
            .flatMap((binding) => binding.data?.streamSpecification
            ?.StreamEnabled === true
            ? [
                normalizeStreamSpecification(binding.data
                    ?.streamSpecification),
            ]
            : binding.streamSpecification
                ?.StreamEnabled === true
                ? [
                    normalizeStreamSpecification(binding.streamSpecification),
                ]
                : [])
        ...
        for (const spec of rest) {
            if (spec.StreamViewType !== first.StreamViewType) {
                return yield* Effect.fail(new ConflictingStreamViewTypes({
    ...
    const isRetryableControlPlaneError = (error) => error._tag === "InternalServerError" ||
    ...
    const formatPollingElapsed = (elapsedSeconds) => `${elapsedSeconds}s elapsed`;
    const formatGlobalSecondaryIndexStatuses = (indexes) => JSON.stringify((indexes ?? []).map((index) => ({
        name: index.IndexName,
        status: index.IndexStatus,
        backfilling: index.Backfilling,
    })));
    const updateTimeToLive = (tableName, timeToLiveSpecification) => dynamodb
        .updateTimeToLive({
        TableName: tableName,
    ...
    const updateContinuousBackups = (tableName, pointInTimeRecoverySpecification) => dynamodb
        .updateContinuousBackups({
        TableName: tableName,
        PointInTimeRecoverySpecification: pointInTimeRecoverySpecification,
    })
        .pipe(Effect.retry({
        while: (e) => e._tag === "ContinuousBackupsUnavailableException" ||
    ...
    const canonicalPolicyDocument = (policy) => {
        if (policy === undefined)
            return undefined;
        try {
    ...
    const readTableResourcePolicy = (tableArn) => dynamodb.getResourcePolicy({ ResourceArn: tableArn }).pipe(Effect.map((response) => response.Policy), Effect.catchTag("PolicyNotFoundException", () => Effect.succeed(undefined)), Effect.retry({
        while: isRetryableControlPlaneError,
        schedule: Schedule.max([
            Schedule.exponential(250),
            Schedule.recurs(15),
    ...
    const putTableResourcePolicy = (tableArn, policy) => dynamodb
    ...
    const deleteTableResourcePolicy = (tableArn) => dynamodb.deleteResourcePolicy({ ResourceArn: tableArn }).pipe(Effect.catchTag("PolicyNotFoundException", () => Effect.void), Effect.retry({
        while: isRetryableControlPlaneError,
        schedule: Schedule.max([
            Schedule.exponential(250),
            Schedule.recurs(15),
    ...
    const isKinesisDestinationTransitioning = (destination) => destination.DestinationStatus === "ENABLING" ||
    ...
    const waitForKinesisDestinationsSettled = (session, tableName) => {
        ...
        return Effect.gen(function* () {
            ...
            while: (error) => error._tag === "KinesisDestinationNotSettled" ||
                isRetryableControlPlaneError(error),
            schedule: waitForKinesisDestinationsConvergence.pipe(Schedule.tap(({ attempt }) => {
                elapsedSeconds = (attempt + 1) * 5;
                return session.note(`DynamoDB Table provider: waiting for Kinesis streaming destinations on ${tableName} to settle (${formatPollingElapsed(elapsedSeconds)})`);
            })),
        }));
    };
    const disableKinesisDestination = (tableName, streamArn) => dynamodb
    ...
    const enableKinesisDestination = (tableName, destination) => dynamodb
        .enableKinesisStreamingDestination({
        TableName: tableName,
        StreamArn: destination.streamArn,
        EnableKinesisStreamingConfiguration: destination.approximateCreationDateTimePrecision !== undefined
            ? {
                ApproximateCreationDateTimePrecision: destination.approximateCreationDateTimePrecision,
            }
            : undefined,
    })
        .pipe(Effect.retry({
    ...
    const updateKinesisDestinationPrecision = (tableName, destination) => dynamodb
    ...
    const waitForContributorInsightsSettled = (session, tableName) => Effect.gen(function* () {
        ...
        while: (error) => error._tag === "ContributorInsightsNotSettled" ||
            isRetryableControlPlaneError(error),
        schedule: Schedule.max([
            Schedule.fixed("2 seconds"),
            Schedule.recurs(45),
        ]).pipe(Schedule.tap(({ attempt }) => session.note(`DynamoDB Table provider: waiting for Contributor Insights on ${tableName} to settle (${formatPollingElapsed((attempt + 1) * 2)})`))),
    }));
    ...
    const listContributorInsightsRules = (tableName) => cloudwatch.describeInsightRules.pages({}).pipe(Stream.runFold(() => [], (acc, page) => [
        ...acc,
        ...(page.InsightRules ?? [])
            .map((rule) => rule.Name)
    ...
    const waitForContributorInsightsRulesDeleted = (session, tableName) => Effect.gen(function* () {
        ...
        schedule: Schedule.max([
            Schedule.fixed("2 seconds"),
            Schedule.recurs(20),
        ]).pipe(Schedule.tap(({ attempt }) => session.note(`DynamoDB Table provider: waiting for Contributor Insights rules of ${tableName} to be cleaned up (${formatPollingElapsed((attempt + 1) * 2)})`))),
    }), 
    ...
    Effect.catchTag("ContributorInsightsNotSettled", () => session.note(`DynamoDB Table provider: Contributor Insights rules for ${tableName} were not cleaned up by DynamoDB; they may be stranded from an earlier enable session (deletable only via AWS support)`)));
    const updateTableContributorInsights = (tableName, enabled) => dynamodb
    ...
    const waitForTableActive = (session, tableName) => {
        let elapsedSeconds = 0;
        ...
        return Effect.gen(function* () {
            ...
            yield* session.note(`DynamoDB Table provider: table ${tableName} is ACTIVE (${formatPollingElapsed(elapsedSeconds)})`);
            return response.Table;
        }).pipe(Effect.retry({
            while: (error) => error._tag === "TableNotActive" ||
                isRetryableControlPlaneError(error),
            schedule: waitForTableActivationConvergence.pipe(Schedule.tap(({ attempt }) => {
                elapsedSeconds = attempt * 10;
                return session.note(`${progressMessage} (${formatPollingElapsed(elapsedSeconds)})`);
            })),
        }));
    };
    const waitForGlobalSecondaryIndexesStable = (session, tableName, expectedIndexNames) => {
        let elapsedSeconds = 0;
        ...
        return Effect.gen(function* () {
            ...
            const allActive = (table?.GlobalSecondaryIndexes ?? []).every((index) => index.IndexStatus === "ACTIVE");
            if (JSON.stringify(actualIndexNames) !== JSON.stringify(expected) ||
                !allActive) {
            ...
            yield* session.note(`DynamoDB Table provider: GSIs for ${tableName} stabilized (${JSON.stringify(actualIndexNames)}) (${formatPollingElapsed(elapsedSeconds)})`);
            return table;
        }).pipe(Effect.retry({
            while: (error) => error._tag === "TableIndexesNotStable" ||
                isRetryableControlPlaneError(error),
            schedule: waitForGlobalSecondaryIndexesConvergence.pipe(Schedule.tap(({ attempt }) => {
                elapsedSeconds = attempt * 10;
                return session.note(`${progressMessage} (${formatPollingElapsed(elapsedSeconds)})`);
            })),
        }));
    };
    const waitForTableDeleted = (session, tableName) => {
        let elapsedSeconds = 0;
        ...
        }).pipe(Effect.catchTag("ResourceNotFoundException", () => {
            return session.note(`DynamoDB Table provider: table ${tableName} deletion confirmed (${formatPollingElapsed(elapsedSeconds)})`);
        }), Effect.retry({
            while: (error) => error._tag === "TableStillDeleting" ||
                isRetryableControlPlaneError(error),
            schedule: waitForDeletionConvergence.pipe(Schedule.tap(({ attempt }) => {
                elapsedSeconds = attempt;
                return session.note(`${progressMessage} (${formatPollingElapsed(elapsedSeconds)})`);
            })),
        }));
    };
    const deleteGlobalSecondaryIndexes = (session, tableName) => Effect.gen(function* () {
        ...
        }), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({ Table: undefined })));
        const indexNames = [...(response.Table?.GlobalSecondaryIndexes ?? [])]
            .map((index) => index.IndexName)
            .sort();
        if (indexNames.length === 0) {
            yield* session.note(`DynamoDB Table provider: no GSIs remain on ${tableName}; retrying table deletion`);
            return;
        }
        yield* session.note(`DynamoDB Table provider: deleting GSIs before deleting table ${tableName} (${indexNames.join(", ")})`);
        ...
        for (const indexName of indexNames) {
            ...
            yield* session.note(`DynamoDB Table provider: deleting GSI ${indexName} from ${tableName}`);
            yield* dynamodb
                .updateTable({
                TableName: tableName,
                ...
                .pipe(Effect.timeout(1000), Effect.tap(() => session.note(`DynamoDB Table provider: delete accepted for GSI ${indexName} on ${tableName} (${formatPollingElapsed(elapsedSeconds)})`)), Effect.retry({
                while: (error) => {
    ...
    const isRetryableReadError = (error) => error._tag === "InternalServerError" ||
        error._tag === "LimitExceededException" ||
        ...
        error._tag === "ProvisionedThroughputExceededException" ||
        error._tag === "RequestLimitExceeded";
    const readTableState = (tableName) => Effect.gen(function* () {
        const response = yield* dynamodb
            .describeTable({
            TableName: tableName,
        })
        ...
        const [tagsResult, continuousBackupsResult, ttlResult] = yield* Effect.all([
            ...
            }), Effect.catchTag("TableNotFoundException", () => Effect.succeed({ ContinuousBackupsDescription: undefined }))),
            dynamodb
                .describeTimeToLive({
                TableName: tableName,
            })
            ...
            }), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({ TimeToLiveDescription: undefined }))),
        ]);
        return {
            table,
            tags: Object.fromEntries((tagsResult.Tags ?? []).map((tag) => [tag.Key, tag.Value])),
            pointInTimeRecoveryDescription: continuousBackupsResult.ContinuousBackupsDescription
    ...
    const toAttrs = (state) => ({
        ...
        tableArn: state.table.TableArn,
        partitionKey: state.table.KeySchema?.find((key) => key.KeyType === "HASH")
            ?.AttributeName ?? "",
    ...
    const indexesByName = (indexes) => Object.fromEntries((indexes ?? []).map((index) => [index.IndexName, index]));
    ...
    const normalizeKeySchema = (keySchema) => {
    ...
    const isSameGsiDefinition = (left, right) => JSON.stringify(normalizeKeySchema(left.KeySchema)) ===
    ...
    const diffGlobalSecondaryIndexes = (olds, news) => {
        ...
        for (const [indexName, oldIndex] of Object.entries(oldByName)) {
            ...
            const provDiff = newIndex.ProvisionedThroughput !== undefined &&
    ...
    return Table.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            const names = yield* dynamodb.listTables.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
            const states = yield* Effect.forEach(names, (tableName) => readTableState(tableName).pipe(
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const tableName = output?.tableName ??
                (olds ? yield* createTableName(id, olds) : undefined);
            if (!tableName)
                return undefined;
            const state = yield* readTableState(tableName).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
            if (!state)
                return undefined;
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            ...
            const replace = news.tableName !== undefined
            ...
            // the new prop shape.
            if (havePropsChanged({
                localSecondaryIndexes: toWireLocalSecondaryIndexes(olds.partitionKey, olds.localSecondaryIndexes) ?? [],
            }, {
                localSecondaryIndexes: toWireLocalSecondaryIndexes(news.partitionKey, news.localSecondaryIndexes) ?? [],
            })) {
                return replace;
            }
            const { requiresReplacement } = diffGlobalSecondaryIndexes(toWireGlobalSecondaryIndexes(olds.globalSecondaryIndexes), toWireGlobalSecondaryIndexes(news.globalSecondaryIndexes));
        ...
        reconcile: Effect.fn(function* ({ id, output, news, session, bindings, }) {
            ...
            if (state === undefined) {
                yield* session.note(`Table ${tableName}: creating with stream specification ${JSON.stringify(desiredStreamSpecification)}`);
                yield* dynamodb
                    .createTable({
                    TableName: tableName,
                ...
                if ((news.globalSecondaryIndexes?.length ?? 0) > 0) {
                    yield* waitForGlobalSecondaryIndexesStable(session, tableName, news.globalSecondaryIndexes?.map((index) => index.indexName) ??
                        []);
                }
            ...
            const currentStreamSpecification = normalizeStreamSpecification(state.table.StreamSpecification);
            const streamViewTypeChanged = currentStreamSpecification?.StreamEnabled === true &&
            ...
            if (havePropsChanged({ streamSpecification: currentStreamSpecification }, { streamSpecification: desiredStreamSpecification })) {
            ...
            const { updates: globalSecondaryIndexUpdates } = diffGlobalSecondaryIndexes(state.table.GlobalSecondaryIndexes, toWireGlobalSecondaryIndexes(news.globalSecondaryIndexes));
            ...
            if (globalSecondaryIndexUpdates.length > 0) {
                const expectedNames = news.globalSecondaryIndexes?.map((index) => index.indexName) ??
                    [];
                yield* session.note(`Table ${tableName}: waiting for GSIs to stabilize (${expectedNames.join(", ") || "none"})`);
            ...
            const observedBillingMode = state.table.BillingModeSummary?.BillingMode ??
            ...
            const baseChanged = havePropsChanged({
                ...
                provisionedThroughput: desiredBillingMode === "PROVISIONED"
            ...
            const currentTtlEnabled = currentTtl?.TimeToLiveStatus === "ENABLED" ||
            ...
            const currentPitrEnabled = state.pointInTimeRecoveryDescription?.PointInTimeRecoveryStatus ===
                "ENABLED";
            const desiredPitrEnabled = news.pointInTimeRecoverySpecification?.pointInTimeRecoveryEnabled ??
                false;
            const currentPitrPeriod = state.pointInTimeRecoveryDescription?.RecoveryPeriodInDays;
            const desiredPitrPeriod = toWireDays(news.pointInTimeRecoverySpecification?.recoveryPeriod);
            if (currentPitrEnabled !== desiredPitrEnabled ||
            ...
            if (canonicalPolicyDocument(observedPolicy) !==
            ...
            for (const destination of kinesisDestinations) {
                ...
                yield* session.note(`Table ${tableName}: disabling Kinesis streaming destination ${destination.StreamArn}`);
            ...
            if (desiredKinesisDestination !== undefined) {
                const activeDestination = kinesisDestinations.find((destination) => destination.StreamArn === desiredKinesisDestination.streamArn &&
                    destination.DestinationStatus === "ACTIVE");
                if (activeDestination === undefined) {
                    yield* session.note(`Table ${tableName}: enabling Kinesis streaming destination ${desiredKinesisDestination.streamArn}`);
                    yield* enableKinesisDestination(tableName, desiredKinesisDestination);
                    kinesisDestinations = yield* waitForKinesisDestinationsSettled(session, tableName);
                }
                else if (desiredKinesisDestination.approximateCreationDateTimePrecision !==
                    undefined &&
                    (activeDestination.ApproximateCreationDateTimePrecision ??
                        "MILLISECOND") !==
                        desiredKinesisDestination.approximateCreationDateTimePrecision) {
                    yield* session.note(`Table ${tableName}: updating Kinesis streaming destination precision to ${desiredKinesisDestination.approximateCreationDateTimePrecision}`);
                    yield* updateKinesisDestinationPrecision(tableName, desiredKinesisDestination);
                    kinesisDestinations = yield* waitForKinesisDestinationsSettled(session, tableName);
                }
                const settledDestination = kinesisDestinations.find((destination) => destination.StreamArn === desiredKinesisDestination.streamArn);
                if (settledDestination?.DestinationStatus !== "ACTIVE") {
                    return yield* Effect.fail(new KinesisStreamingDestinationFailed({
                        tableName,
            ...
            const observedInsightsStatus = yield* waitForContributorInsightsSettled(session, tableName);
            const observedInsightsEnabled = observedInsightsStatus === "ENABLED";
            if (observedInsightsEnabled !== desiredInsightsEnabled) {
                yield* session.note(`Table ${tableName}: ${desiredInsightsEnabled ? "enabling" : "disabling"} Contributor Insights`);
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            yield* Effect.gen(function* () {
                const insightsStatus = yield* waitForContributorInsightsSettled(session, output.tableName);
                if (insightsStatus !== "DISABLED") {
                    yield* session.note(`Table ${output.tableName}: disabling Contributor Insights before delete`);
            ...
            while (true) {
                ...
                const deleteResult = yield* dynamodb
                    ...
                    .pipe(Effect.timeout(1000), Effect.as("accepted"), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed("already-deleted")), Effect.catchTag("ResourceInUseException", () => Effect.succeed("delete-gsis-first")), Effect.retry({
                    while: (error) => error._tag === "InternalServerError" ||
                        error._tag === "TimeoutError",
                    schedule: waitForDeletionConvergence.pipe(Schedule.tap(({ attempt }) => session.note(`DynamoDB Table provider: deleteTable transient failure for ${output.tableName} on attempt ${deleteAttempt} (${formatPollingElapsed(attempt)})`))),
                }));
                if (deleteResult === "accepted") {
                    yield* session.note(`DynamoDB Table provider: deleteTable accepted for ${output.tableName}`);
                    break;
                }
                if (deleteResult === "already-deleted") {
                    yield* session.note(`DynamoDB Table provider: table ${output.tableName} already deleted`);
                    return;
                }
                yield* session.note(`DynamoDB Table provider: deleteTable blocked for ${output.tableName}; deleting GSIs first`);
...
class KinesisStreamingDestinationFailed extends Data.TaggedError("KinesisStreamingDestinationFailed") {
}
