 ...
 */
export const readDomainTags = Effect.fn(function* (arn) {
 ...
 */
export const subsetDiffers = (desired, observed) => {
    ...
    if (typeof desired === "object" && desired !== null) {
        if (typeof observed !== "object" || observed === null)
            return true;
        return Object.entries(desired).some(([key, value]) => value !== undefined &&
            subsetDiffers(value, observed[key]));
    }
 ...
 */
export const repeatUntilDomainState = (read, done) => Effect.repeat(read, {
...
export const isDomainActive = (domain) => domain === undefined ||
    (domain.Created === true &&
        domain.Processing !== true &&
        domain.Deleted !== true);
/** A domain can be deleted once it is no longer applying a config change. */
export const isDomainDeletable = (domain) => domain === undefined || domain.Deleted === true || domain.Processing !== true;
