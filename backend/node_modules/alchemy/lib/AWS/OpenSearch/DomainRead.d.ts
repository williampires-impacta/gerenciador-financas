...
import type { CountRequest, CountResponse, GetDocumentResponse, OpenSearchApiError, SearchRequest, SearchResponse } from "./DataPlaneTypes.ts";
...
export interface ReadDomainClient {
     ...
     */
    search<TDoc = unknown>(request?: SearchRequest): Effect.Effect<SearchResponse<TDoc>, OpenSearchApiError | Credentials.CredentialsError>;
    /** Count documents matching a Query-DSL body (`GET …/_count`). */
    count(request?: CountRequest): Effect.Effect<CountResponse, OpenSearchApiError | Credentials.CredentialsError>;
    /**
     * Fetch one document by id (`GET /{index}/_doc/{id}`). A missing document
     * is not an error — the response carries `found: false`.
     */
    getDocument<TDoc = unknown>(index: string, id: string): Effect.Effect<GetDocumentResponse<TDoc>, OpenSearchApiError | Credentials.CredentialsError>;
    /** Check whether a document exists (`HEAD /{index}/_doc/{id}`). */
    existsDocument(index: string, id: string): Effect.Effect<boolean, OpenSearchApiError | Credentials.CredentialsError>;
    /**
     * Raw read-only escape hatch — a SigV4-signed `GET` against any data-plane
     * path (e.g. `_cluster/health`, `_cat/indices?format=json`).
     */
    get(path: string, query?: Record<string, string | undefined>): Effect.Effect<unknown, OpenSearchApiError | Credentials.CredentialsError>;
}
/**
 ...
 * @binding
 */
export interface DomainRead extends Binding.Service<DomainRead, "AWS.OpenSearch.DomainRead", (domain: Domain) => Effect.Effect<ReadDomainClient>> {
}
export declare const DomainRead: DomainRead;
//# sourceMappingURL=DomainRead.d.ts.map