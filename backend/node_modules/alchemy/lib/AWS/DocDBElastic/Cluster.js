 ...
 * @resource
 */
export const Cluster = Resource("AWS.DocDBElastic.Cluster");
...
export const ClusterProvider = () => Provider.effect(Cluster, Effect.gen(function* () {
    ...
    const findByName = Effect.fn(function* (name) {
        const matches = yield* docdbelastic.listClusters.items({}).pipe(Stream.filter((cluster) => cluster.clusterName === name), Stream.take(1), Stream.runCollect);
        const summary = Array.from(matches)[0];
        if (summary === undefined)
            return undefined;
        return yield* getByArn(summary.clusterArn);
    });
    const readCluster = Effect.fn(function* (name, arn) {
        if (arn !== undefined) {
    ...
    const waitForActive = Effect.fn(function* (name, arn) {
        const policy = Schedule.max([
            Schedule.fixed("15 seconds"),
            Schedule.recurs(80),
        ]);
        return yield* readCluster(name, arn).pipe(Effect.flatMap((cluster) => {
            if (cluster === undefined) {
                return Effect.fail(new Error(`Elastic cluster '${name}' not found`));
            }
            if (cluster.status !== "ACTIVE") {
                return Effect.fail(new Error(`Elastic cluster '${name}' not active (status: ${cluster.status})`));
            }
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, output, session }) {
            ...
            if (observed === undefined) {
                return yield* Effect.fail(new Error(`Elastic cluster '${name}' not found after create`));
            }
            ...
            const update = { clusterArn: arn };
            let mutated = false;
            if (props.shardCapacity !== undefined &&
            ...
            if (props.preferredBackupWindow !== undefined &&
        ...
        list: () => docdbelastic.listClusters.items({}).pipe(Stream.runCollect, Effect.flatMap((summaries) => Effect.forEach(Array.from(summaries), (summary) => getByArn(summary.clusterArn).pipe(Effect.flatMap((cluster) => cluster === undefined
            ? Effect.succeed(undefined)
            : toAttrs(cluster))), { concurrency: 4 })), Effect.map((attrs) => attrs.filter((a) => a !== undefined))),
    };
}));
