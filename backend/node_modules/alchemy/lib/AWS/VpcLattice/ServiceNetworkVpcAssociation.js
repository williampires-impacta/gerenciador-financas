 ...
 * @resource
 */
export const ServiceNetworkVpcAssociation = Resource("AWS.VpcLattice.ServiceNetworkVpcAssociation");
export const ServiceNetworkVpcAssociationProvider = () => Provider.effect(ServiceNetworkVpcAssociation, Effect.gen(function* () {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let assoc = output?.associationId
                ...
                : undefined;
            if (!assoc?.arn || !assoc.id) {
                const created = yield* vpclattice.createServiceNetworkVpcAssociation({
                ...
                if (!created.arn || !created.id) {
                    return yield* Effect.fail(new Error("Failed to create service network VPC association"));
                }
            ...
            if (!assoc?.arn || !assoc.id) {
                return yield* Effect.fail(new Error("service network VPC association is missing its id/arn"));
            }
        ...
        list: () => Effect.gen(function* () {
            ...
            const serviceNetworkIds = yield* vpclattice.listServiceNetworks
                ...
                .flatMap((network) => network.id === undefined ? [] : [network.id])));
            const summaries = (yield* Effect.forEach(serviceNetworkIds, (serviceNetworkIdentifier) => vpclattice.listServiceNetworkVpcAssociations
                .pages({ serviceNetworkIdentifier })
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.items ?? []))), { concurrency: 10 })).flat();
