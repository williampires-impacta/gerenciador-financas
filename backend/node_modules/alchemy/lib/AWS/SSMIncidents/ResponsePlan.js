 ...
 * ```
 */
const ResponsePlanResource = Resource("AWS.SSMIncidents.ResponsePlan");
...
export const ResponsePlanProvider = () => Provider.effect(ResponsePlanResource, Effect.gen(function* () {
    ...
    const findArnByName = (name) => incidents.listResponsePlans.items({}).pipe(Stream.filter((summary) => summary.name === name), Stream.take(1), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)[0]?.arn));
    const getPlan = (arn) => incidents
        .getResponsePlan({ arn })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    const observe = Effect.fn(function* (name, arn) {
        if (arn !== undefined) {
    ...
    return ResponsePlanResource.Provider.of({
        stables: ["arn", "name"],
        list: () => incidents.listResponsePlans.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).map((summary) => ({
            arn: summary.arn,
            name: summary.name,
        })))),
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (plan === undefined) {
                ...
                const arn = yield* incidents
                    ...
                    .pipe(Effect.map((r) => r.arn), Effect.catchTag("ConflictException", () => findArnByName(name).pipe(Effect.map((found) => found))));
            ...
            const chatChannelDelta = news.chatChannel !== undefined &&
