 ...
 * @resource
 */
export const DatasetGroup = Resource("AWS.Personalize.DatasetGroup");
export const DatasetGroupProvider = () => Provider.effect(DatasetGroup, Effect.gen(function* () {
    ...
    const waitActive = Effect.fn(function* (datasetGroupArn) {
        const group = yield* describe(datasetGroupArn).pipe(Effect.repeat({
        ...
        if (group?.status !== "ACTIVE") {
            return yield* Effect.fail(new Error(`Personalize dataset group ${datasetGroupArn} did not become ACTIVE (status: ${group?.status}, reason: ${group?.failureReason})`));
        }
        return group;
    });
    ...
    const listChildren = Effect.fn(function* (datasetGroupArn) {
        const [trackers, filters, solutions, datasets] = yield* Effect.all([
            personalize.listEventTrackers.pages({ datasetGroupArn }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.eventTrackers ?? []))),
            personalize.listFilters.pages({ datasetGroupArn }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Filters ?? []))),
            personalize.listSolutions.pages({ datasetGroupArn }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.solutions ?? []))),
            personalize.listDatasets.pages({ datasetGroupArn }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.datasets ?? []))),
        ], { concurrency: 4 });
     ...
     */
    const drainChildren = Effect.fn(function* (datasetGroupArn) {
        const { trackers, filters, solutions, datasets } = yield* listChildren(datasetGroupArn);
        yield* Effect.forEach(trackers, (t) => personalize
            .deleteEventTracker({ eventTrackerArn: t.eventTrackerArn })
            .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void), Effect.retry(inUseRetry)), { concurrency: 4 });
        yield* Effect.forEach(filters, (f) => personalize.deleteFilter({ filterArn: f.filterArn }).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void), Effect.retry(inUseRetry)), { concurrency: 4 });
        // Campaigns block their solution's deletion, so drain them per
        // solution before deleting the solution.
        yield* Effect.forEach(solutions, Effect.fn(function* (s) {
            const campaigns = yield* personalize.listCampaigns
                .pages({ solutionArn: s.solutionArn })
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.campaigns ?? [])));
            yield* Effect.forEach(campaigns, (c) => personalize
        ...
        yield* Effect.forEach(datasets, (d) => personalize.deleteDataset({ datasetArn: d.datasetArn }).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void), Effect.retry(inUseRetry)), { concurrency: 4 });
        ...
        const remaining = yield* listChildren(datasetGroupArn).pipe(Effect.repeat({
            schedule: Schedule.max([
                Schedule.fixed("3 seconds"),
                Schedule.recurs(40),
            ]),
            until: (children) => children.trackers.length === 0 &&
                children.filters.length === 0 &&
                children.solutions.length === 0 &&
                children.datasets.length === 0,
        }));
        if (remaining.trackers.length +
            remaining.filters.length +
            remaining.solutions.length +
            remaining.datasets.length >
            0) {
            return yield* Effect.fail(new Error(`Personalize dataset group ${datasetGroupArn} still has children after drain (trackers: ${remaining.trackers.length}, filters: ${remaining.filters.length}, solutions: ${remaining.solutions.length}, datasets: ${remaining.datasets.length})`));
        }
    });
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
            ...
            if (group === undefined) {
                const arn = yield* personalize
                    ...
                    .pipe(Effect.map((created) => created.datasetGroupArn), Effect.catchTag("ResourceAlreadyExistsException", (error) => findArnByName(name).pipe(Effect.flatMap((existing) => existing === undefined
                    ? Effect.fail(error)
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            const remaining = yield* describe(output.datasetGroupArn).pipe(Effect.repeat({
                schedule: Schedule.max([
                    Schedule.fixed("3 seconds"),
                    Schedule.recurs(40),
                ]),
