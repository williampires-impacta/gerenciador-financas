 ...
 * @resource
 */
export const Dataset = Resource("AWS.Personalize.Dataset");
export const DatasetProvider = () => Provider.effect(Dataset, Effect.gen(function* () {
    ...
    const waitActive = Effect.fn(function* (datasetArn) {
        const dataset = yield* describe(datasetArn).pipe(Effect.repeat({
        ...
        if (dataset?.status !== "ACTIVE") {
            return yield* Effect.fail(new Error(`Personalize dataset ${datasetArn} did not become ACTIVE (status: ${dataset?.status})`));
        }
        return dataset;
    });
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (dataset === undefined) {
                const arn = yield* personalize
                    ...
                    .pipe(Effect.map((created) => created.datasetArn), Effect.catchTag("ResourceAlreadyExistsException", (error) => findArnByName(name, news.datasetGroupArn).pipe(Effect.flatMap((existing) => existing === undefined
                    ? Effect.fail(error)
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            const remaining = yield* describe(output.datasetArn).pipe(Effect.repeat({
                schedule: Schedule.max([
                    Schedule.fixed("3 seconds"),
                    Schedule.recurs(40),
                ]),
            ...
            if (remaining !== undefined) {
                return yield* Effect.fail(new Error(`Personalize dataset ${output.datasetArn} was not deleted (status: ${remaining.status})`));
            }
        }),
        ...
        list: () => personalize.listDatasetGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.datasetGroups ?? [])), Effect.flatMap(Effect.forEach((group) => personalize.listDatasets
            .pages({ datasetGroupArn: group.datasetGroupArn })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.datasets ?? []))), { concurrency: 4 })), Effect.map((groups) => groups.flat()), Effect.flatMap(Effect.forEach((summary) => describe(summary.datasetArn).pipe(Effect.map((d) => (d ? toAttrs(d) : undefined))), { concurrency: 4 })), Effect.map((items) => items.filter((item) => item !== undefined))),
    };
}));
