 ...
 * @resource
 */
export const Schema = Resource("AWS.Personalize.Schema");
export const SchemaProvider = () => Provider.effect(Schema, Effect.gen(function* () {
    ...
    const describe = Effect.fn(function* (schemaArn) {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let schema = output?.schemaArn !== undefined
            ...
            if (schema === undefined) {
                const arn = yield* personalize
                    ...
                    .pipe(Effect.map((created) => created.schemaArn), Effect.catchTag("ResourceAlreadyExistsException", (error) => findArnByName(name).pipe(Effect.flatMap((existing) => existing === undefined
                    ? Effect.fail(error)
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* personalize.deleteSchema({ schemaArn: output.schemaArn }).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void), 
        ...
        list: () => personalize.listSchemas.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.schemas ?? [])), Effect.flatMap(Effect.forEach((summary) => describe(summary.schemaArn).pipe(Effect.map((s) => s
            ? {
            ...
            : undefined)), { concurrency: 4 })), Effect.map((items) => items.filter((item) => item !== undefined))),
    };
}));
