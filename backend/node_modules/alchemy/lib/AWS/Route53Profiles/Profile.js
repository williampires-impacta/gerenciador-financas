 ...
 * @resource
 */
export const Profile = Resource("AWS.Route53Profiles.Profile");
/**
 ...
 * @internal
 */
const retryDeleteConflict = (self) => Effect.retry(self, {
 ...
 * @internal
 */
const untilProfileSettled = (self) => self.pipe(Effect.repeat({
...
export const ProfileProvider = () => Provider.effect(Profile, Effect.gen(function* () {
    ...
    const observe = (profileId) => profiles.getProfile({ ProfileId: profileId }).pipe(Effect.map((r) => r.Profile === undefined ||
        r.Profile.Status === "DELETING" ||
        r.Profile.Status === "DELETED"
        ? undefined
        : r.Profile), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    const findByName = (name) => profiles.listProfiles.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((summary) => summary.Name === name)));
    const readTags = (profileArn) => profiles.listTagsForResource({ ResourceArn: profileArn }).pipe(Effect.map((r) => {
        const tags = {};
    ...
    const syncTags = Effect.fn(function* (id, profileArn, userTags) {
    ...
    return Profile.Provider.of({
        ...
        list: () => profiles.listProfiles.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((summary) => summary.Id !== undefined &&
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const profileId = output?.profileId ??
                (yield* createName(id, olds ?? {}).pipe(Effect.flatMap(findByName), Effect.map((summary) => summary?.Id)));
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryDeleteConflict(profiles.deleteProfile({ ProfileId: output.profileId })).pipe(Effect.asVoid, Effect.catchTag("ResourceNotFoundException", () => Effect.void));
        }),
    });
}));
