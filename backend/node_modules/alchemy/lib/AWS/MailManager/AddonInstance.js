...
import { readMailManagerTags, syncMailManagerTags } from "./internal.js";
/**
 ...
 * @resource
 */
export const AddonInstance = Resource("AWS.MailManager.AddonInstance");
export const AddonInstanceProvider = () => Provider.effect(AddonInstance, Effect.gen(function* () {
    const getById = (addonInstanceId) => mm
        .getAddonInstance({ AddonInstanceId: addonInstanceId })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    const listAll = mm.listAddonInstances.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.AddonInstances ?? [])));
    // Instances have no user-supplied name — recover identity after a lost
    // state write by searching for our ownership tags.
    const findByAlchemyTags = Effect.fn(function* (id) {
        const instances = yield* listAll;
        for (const instance of instances) {
            if (instance.AddonInstanceArn === undefined)
                continue;
    ...
    const toAttrs = (instance) => instance.AddonInstanceId !== undefined &&
        ...
        : undefined;
    const observe = Effect.fn(function* (id, output) {
    ...
    return AddonInstance.Provider.of({
        ...
        list: () => listAll.pipe(Effect.map((instances) => instances.flatMap((i) => {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            //    the tag search.
            if (instance === undefined) {
                yield* session.note(`creating Add On instance for subscription ${news.addonSubscriptionId}`);
            ...
            if (attrs === undefined) {
                return yield* Effect.fail(new Error(`Mail Manager Add On instance for subscription '${news.addonSubscriptionId}' not found after create`));
            }
