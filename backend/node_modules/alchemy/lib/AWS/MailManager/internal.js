...
import { diffTags, tagRecord } from "../../Tags.js";
/**
 ...
 */
export const readMailManagerTags = (arn) => mm.listTagsForResource({ ResourceArn: arn }).pipe(Effect.map((r) => tagRecord(r.Tags ?? [])), Effect.catch(() => Effect.succeed({})));
/**
 * Converge the tags on a Mail Manager resource to `desired`, diffing against
 * the OBSERVED cloud tags so adoption converges.
 */
export const syncMailManagerTags = Effect.fn(function* (arn, desired) {
 ...
 */
export const retryWhileMailManagerConflict = (self) => Effect.retry(self, {
 ...
 */
export const repeatUntilMailManagerStable = (self, done, times = 24) => Effect.repeat(self, {
...
export const sameShape = (l, r) => JSON.stringify(normalize(l)) === JSON.stringify(normalize(r));
