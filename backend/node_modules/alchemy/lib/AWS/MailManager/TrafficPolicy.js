...
import { readMailManagerTags, retryWhileMailManagerConflict, sameShape, syncMailManagerTags, } from "./internal.js";
/**
 ...
 * @resource
 */
export const TrafficPolicy = Resource("AWS.MailManager.TrafficPolicy");
export const TrafficPolicyProvider = () => Provider.effect(TrafficPolicy, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.trafficPolicyName ??
            (yield* createPhysicalName({ id, maxLength: 63 })));
    });
    const getById = (trafficPolicyId) => mm
        .getTrafficPolicy({ TrafficPolicyId: trafficPolicyId })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    const findByName = (name) => mm.listTrafficPolicies.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    ...
    const observe = Effect.fn(function* (output, name) {
        ...
        if (summary?.TrafficPolicyId === undefined)
            return undefined;
        return yield* getById(summary.TrafficPolicyId);
    });
    const toAttrs = Effect.fn(function* (policy) {
        if (policy.TrafficPolicyArn === undefined) {
            return yield* Effect.fail(new Error(`Mail Manager traffic policy '${policy.TrafficPolicyId}' returned without an ARN`));
        }
        return {
    ...
    return TrafficPolicy.Provider.of({
        ...
        list: () => mm.listTrafficPolicies.pages({}).pipe(Stream.runCollect, Effect.flatMap((chunk) => Effect.forEach(Array.from(chunk)
            .flatMap((page) => page.TrafficPolicies ?? [])
            .flatMap((p) => p.TrafficPolicyId !== undefined ? [p.TrafficPolicyId] : []), (trafficPolicyId) => getById(trafficPolicyId))), Effect.flatMap((results) => Effect.forEach(results.flatMap((p) => (p === undefined ? [] : [p])), (p) => toAttrs(p)))),
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (policy === undefined) {
                return yield* Effect.fail(new Error(`Mail Manager traffic policy '${name}' not found after create`));
            }
            // 3. SYNC — diff OBSERVED state against desired; apply only the
            //    delta.
            if (policy.TrafficPolicyName !== name ||
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryWhileMailManagerConflict(mm.deleteTrafficPolicy({
