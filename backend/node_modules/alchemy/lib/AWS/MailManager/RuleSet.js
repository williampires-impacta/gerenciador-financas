 ...
 * @resource
 */
export const RuleSet = Resource("AWS.MailManager.RuleSet");
export const RuleSetProvider = () => Provider.effect(RuleSet, Effect.gen(function* () {
    ...
    // write.
    const findByName = (name) => mm.listRuleSets.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    ...
    const observe = Effect.fn(function* (output, name) {
        ...
        if (summary?.RuleSetId === undefined)
            return undefined;
        return yield* getById(summary.RuleSetId);
    });
    const toAttrs = (ruleSet) => ({
        ruleSetId: ruleSet.RuleSetId,
    ...
    return RuleSet.Provider.of({
        ...
        list: () => mm.listRuleSets.pages({}).pipe(Stream.runCollect, Effect.flatMap((chunk) => Effect.forEach(Array.from(chunk)
            .flatMap((page) => page.RuleSets ?? [])
            .flatMap((r) => r.RuleSetId !== undefined ? [r.RuleSetId] : []), (ruleSetId) => getById(ruleSetId))), Effect.map((results) => results.flatMap((r) => (r === undefined ? [] : [toAttrs(r)])))),
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (ruleSet === undefined) {
                return yield* Effect.fail(new Error(`Mail Manager rule set '${name}' not found after create`));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryWhileMailManagerConflict(mm.deleteRuleSet({ RuleSetId: output.ruleSetId })).pipe(Effect.asVoid);
        }),
    });
}));
