 ...
 * @resource
 */
export const AddonSubscription = Resource("AWS.MailManager.AddonSubscription");
export const AddonSubscriptionProvider = () => Provider.effect(AddonSubscription, Effect.gen(function* () {
    const getById = (addonSubscriptionId) => mm
        .getAddonSubscription({ AddonSubscriptionId: addonSubscriptionId })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    const listAll = mm.listAddonSubscriptions.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.AddonSubscriptions ?? [])));
    ...
    const findByAlchemyTags = Effect.fn(function* (id) {
        ...
        for (const subscription of subscriptions) {
            if (subscription.AddonSubscriptionArn === undefined)
                continue;
    ...
    const toAttrs = (subscription) => subscription.AddonSubscriptionId !== undefined &&
    ...
    return AddonSubscription.Provider.of({
        ...
        list: () => listAll.pipe(Effect.map((subscriptions) => subscriptions.flatMap((s) => {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (attrs === undefined) {
                return yield* Effect.fail(new Error(`Mail Manager Add On subscription for '${news.addonName}' not found after create`));
            }
...
//# sourceMappingURL=AddonSubscription.js.map