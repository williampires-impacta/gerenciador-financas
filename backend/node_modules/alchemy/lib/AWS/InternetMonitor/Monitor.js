 ...
 * @resource
 */
export const Monitor = Resource("AWS.InternetMonitor.Monitor");
...
export const MonitorProvider = () => Provider.effect(Monitor, Effect.gen(function* () {
    ...
    const readMonitor = Effect.fn(function* (name) {
        return yield* im
    ...
    const waitUntilSettled = Effect.fn(function* (name) {
        ...
        return yield* readMonitor(name).pipe(Effect.flatMap((monitor) => {
    ...
    return Monitor.Provider.of({
        ...
        list: () => im.listMonitors.items({}).pipe(Stream.runCollect, Effect.flatMap((monitors) => Effect.forEach(Array.from(monitors), (monitor) => im.getMonitor({ MonitorName: monitor.MonitorName }).pipe(Effect.map(toAttrs), 
        // Tolerate delete races between list and get.
        Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined))), { concurrency: 4 })), Effect.map((attrs) => attrs.flatMap((a) => (a === undefined ? [] : [a])))),
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (props.maxCityNetworksToMonitor !== undefined &&
            ...
            if (props.trafficPercentageToMonitor !== undefined &&
            ...
            if (props.internetMeasurementsLogDelivery !== undefined &&
                !sameJson(props.internetMeasurementsLogDelivery, observed.InternetMeasurementsLogDelivery)) {
                update.InternetMeasurementsLogDelivery =
                    props.internetMeasurementsLogDelivery;
                mutated = true;
            }
            if (props.healthEventsConfig !== undefined &&
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if (observed.Status !== "INACTIVE") {
                ...
                yield* readMonitor(name).pipe(Effect.flatMap((monitor) => monitor !== undefined && monitor.Status !== "INACTIVE"
                    ? Effect.fail(new Error(`Internet Monitor monitor '${name}' is not yet INACTIVE (status: ${monitor.Status})`))
                    : Effect.succeed(monitor)), Effect.retry({
            ...
            const reapLogGroups = logs
                ...
                .pipe(Effect.map((r) => r.logGroups ?? []), Effect.flatMap((groups) => Effect.forEach(groups.flatMap((g) => 
            ...
            g.logGroupName !== undefined &&
                ...
                .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void)), { concurrency: 4 })));
            ...
            yield* reapLogGroups.pipe(Effect.repeat({
