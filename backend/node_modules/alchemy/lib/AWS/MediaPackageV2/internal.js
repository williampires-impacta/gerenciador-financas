 ...
 */
export const toMpTagRecord = (tags) => Object.fromEntries(Object.entries(tags ?? {}).filter((entry) => typeof entry[1] === "string"));
/**
 * Sync tags on a MediaPackage resource: diff the OBSERVED cloud tags against
 * the desired set and apply only the delta.
 */
export const syncMpTags = Effect.fn(function* (arn, observedTags, desiredTags) {
 ...
 */
export const retryWhileMpConflict = (self) => Effect.retry(self, {
...
export const listAllChannelGroups = Effect.fn(function* () {
    return yield* mediapackagev2.listChannelGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Items ?? [])));
});
/**
 ...
 */
export const listGroupChannels = Effect.fn(function* (channelGroupName) {
 ...
 */
export const listChannelEndpoints = Effect.fn(function* (channelGroupName, channelName) {
 ...
 */
export const deleteChannelWithEndpoints = Effect.fn(function* (channelGroupName, channelName) {
    const endpoints = yield* listChannelEndpoints(channelGroupName, channelName);
    yield* Effect.forEach(endpoints, (endpoint) => mediapackagev2.deleteOriginEndpoint({
 ...
 */
export const policiesEqual = (a, b) => {
 ...
 */
export const matchesDesired = (desired, observed) => {
    ...
    if (typeof desired === "object") {
        ...
        return Object.entries(desired).every(([key, value]) => value === undefined
            ? true
            : matchesDesired(value, observed[key]));
    }
