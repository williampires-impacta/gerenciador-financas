...
import { deleteChannelWithEndpoints, listGroupChannels, retryWhileMpConflict, syncMpTags, toMpTagRecord, } from "./internal.js";
/**
 ...
 * @resource
 */
export const ChannelGroup = Resource("AWS.MediaPackageV2.ChannelGroup");
export const ChannelGroupProvider = () => Provider.effect(ChannelGroup, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.channelGroupName ??
    ...
    const getGroup = Effect.fn(function* (channelGroupName) {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (group === undefined) {
                group = yield* mediapackagev2
                    ...
                    .pipe(Effect.catchTag("ConflictException", () => mediapackagev2.getChannelGroup({ ChannelGroupName: name })));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* Effect.forEach(channels, (channel) => deleteChannelWithEndpoints(channelGroupName, channel.ChannelName), { concurrency: 5, discard: true });
        ...
        list: () => Effect.gen(function* () {
            ...
            const groups = yield* Effect.forEach(items, (item) => mediapackagev2
                .getChannelGroup({ ChannelGroupName: item.ChannelGroupName })
                .pipe(Effect.map(toAttrs), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined))), { concurrency: 5 });
