 ...
 * ```
 */
const SubscriberNotificationResource = Resource("AWS.SecurityLake.SubscriberNotification");
...
const toWireConfiguration = (props) => props.httpsNotificationConfiguration !== undefined
...
const configFingerprint = (props) => {
    ...
    return https !== undefined
        ? JSON.stringify({
            ...
            authorizationApiKeyValue: https.authorizationApiKeyValue !== undefined
...
const getSubscriberEndpoint = (subscriberId) => securitylake
    .getSubscriber({ subscriberId })
    .pipe(Effect.map((response) => response.subscriber?.subscriberEndpoint));
export const SubscriberNotificationProvider = () => Provider.effect(SubscriberNotificationResource, Effect.gen(function* () {
    return {
        read: Effect.fn(function* ({ olds, output }) {
            ...
            const endpoint = yield* getSubscriberEndpoint(subscriberId).pipe(Effect.catchTag(["ResourceNotFoundException", "UnauthorizedException"], () => Effect.succeed(undefined)));
        ...
        list: () => securitylake.listSubscribers.items({}).pipe(Stream.filter((subscriber) => subscriber.subscriberEndpoint !== undefined), Stream.runCollect, Effect.map((subscribers) => [...subscribers].map((subscriber) => ({
            subscriberId: subscriber.subscriberId,
        ...
        reconcile: Effect.fn(function* ({ news, olds, output, session }) {
            ...
            if (observedEndpoint === undefined) {
                ...
                endpoint = yield* securitylake
                    ...
                    .pipe(Effect.map((response) => response.subscriberEndpoint), Effect.catchTag("ConflictException", () => securitylake
                    .updateSubscriberNotification({
                    subscriberId: news.subscriberId,
                    configuration,
                })
                    .pipe(retryWhileConflict, Effect.map((response) => response.subscriberEndpoint))));
            }
                ...
                endpoint = yield* securitylake
                    ...
                    .pipe(retryWhileConflict, Effect.map((response) => response.subscriberEndpoint ?? endpoint));
            }
