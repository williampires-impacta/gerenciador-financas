...
export const pinEcrPublic = (effect) => effect.pipe(Effect.provideService(Region, Effect.succeed(US_EAST_1)));
/**
 ...
 */
export const makePublicRepositoryHttpBinding = (options) => Effect.gen(function* () {
    ...
    return Effect.fn(function* (repository) {
        ...
        return Effect.fn(`AWS.ECRPublic.${options.capability}(${repository.LogicalId})`)(function* (request) {
 ...
 * `Resource: ["*"]`).
 */
export const makePublicRegistryHttpBinding = (options) => Effect.gen(function* () {
    const op = yield* pinEcrPublic(options.operation);
    return Effect.fn(function* () {
        ...
        return Effect.fn(`AWS.ECRPublic.${options.capability}`)(function* (request) {
            // Call-site region pin — see makePublicRepositoryHttpBinding above.
            return yield* pinEcrPublic(op((request ?? {})));
        });
    });
});
