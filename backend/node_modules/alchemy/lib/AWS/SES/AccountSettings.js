 ...
 * @resource
 */
export const AccountSettings = Resource("AWS.SES.AccountSettings");
const sameReasons = (a, b) => {
    const key = (reasons) => JSON.stringify([...(reasons ?? [])].sort());
    return key(a) === key(b);
};
export const AccountSettingsProvider = () => Provider.effect(AccountSettings, Effect.gen(function* () {
    const observe = sesv2.getAccount({}).pipe(Effect.map((response) => ({
        ...
        // into props.
        suppressedReasons: (response.SuppressionAttributes?.SuppressedReasons ?? []).filter((reason) => reason === "BOUNCE" || reason === "COMPLAINT"),
        dashboardEngagementMetrics: response.VdmAttributes?.DashboardAttributes?.EngagementMetrics,
        guardianOptimizedSharedDelivery: response.VdmAttributes?.GuardianAttributes?.OptimizedSharedDelivery,
    })));
            ...
            if (news.vdm !== undefined &&
                ...
                yield* sesv2.putAccountVdmAttributes({
                    VdmAttributes: {
                        ...
                        DashboardAttributes: news.vdm.dashboardEngagementMetrics !== undefined
                            ? { EngagementMetrics: news.vdm.dashboardEngagementMetrics }
                            : undefined,
                        GuardianAttributes: news.vdm.guardianOptimizedSharedDelivery !== undefined
                            ? {
