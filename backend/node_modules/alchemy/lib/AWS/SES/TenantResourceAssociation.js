 ...
 * @resource
 */
export const TenantResourceAssociation = Resource("AWS.SES.TenantResourceAssociation");
export const TenantResourceAssociationProvider = () => Provider.effect(TenantResourceAssociation, Effect.gen(function* () {
    // Every resource associated with the tenant, or undefined when the
    // tenant itself is gone.
    const listResources = Effect.fn(function* (tenantName) {
        const pages = yield* sesv2.listTenantResources
            .pages({ TenantName: tenantName })
            .pipe(Stream.runCollect, Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
        if (pages === undefined)
            return undefined;
        return Array.from(pages).flatMap((page) => page.TenantResources ?? []);
    });
    const isAssociated = Effect.fn(function* (tenantName, resourceArn) {
    ...
    return TenantResourceAssociation.Provider.of({
        ...
        list: Effect.fn(function* () {
            ...
            const nested = yield* Effect.forEach(tenantNames, (tenantName) => listResources(tenantName).pipe(Effect.map((resources) => (resources ?? []).flatMap((resource) => resource.ResourceArn
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return undefined;
            if (news.tenantName !== olds.tenantName ||
