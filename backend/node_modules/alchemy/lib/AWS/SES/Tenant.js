 ...
 * @resource
 */
export const Tenant = Resource("AWS.SES.Tenant");
...
export const TenantProvider = () => Provider.effect(Tenant, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.tenantName ?? (yield* createPhysicalName({ id, maxLength: 64 })));
    });
    const getTenant = Effect.fn(function* (name) {
        return yield* sesv2.getTenant({ TenantName: name }).pipe(Effect.map((response) => response.Tenant), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
    });
    return Tenant.Provider.of({
        ...
        list: Effect.fn(function* () {
            ...
            return Array.from(pages)
                ...
                .flatMap((entry) => entry.TenantName && entry.TenantId && entry.TenantArn
                ? [
                    {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            if (!found || !found.TenantId || !found.TenantArn)
                return undefined;
            const attrs = {
                tenantName: name,
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (observed === undefined) {
                ...
                observed = yield* getTenant(name).pipe(Effect.repeat({
