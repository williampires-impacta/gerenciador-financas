 ...
 * @resource
 */
export const DedicatedIpPool = Resource("AWS.SES.DedicatedIpPool");
const DEFAULT_SCALING_MODE = "STANDARD";
...
const dedicatedIpPoolArnOf = (region, accountId, name) => `arn:aws:ses:${region}:${accountId}:dedicated-ip-pool/${name}`;
export const DedicatedIpPoolProvider = () => Provider.effect(DedicatedIpPool, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.poolName ??
            (yield* createPhysicalName({ id, maxLength: 64, lowercase: true })));
    });
    const getPool = Effect.fn(function* (name) {
        return yield* sesv2.getDedicatedIpPool({ PoolName: name }).pipe(Effect.map((response) => response.DedicatedIpPool), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
    });
    ...
    const getPoolTags = Effect.fn(function* (name) {
    ...
    return DedicatedIpPool.Provider.of({
        ...
        list: Effect.fn(function* () {
            ...
            const pools = yield* Effect.forEach(poolNames, (poolName) => getPool(poolName).pipe(Effect.map((pool) => pool ? [{ poolName, scalingMode: pool.ScalingMode }] : [])), { concurrency: 2 });
            return pools.flat();
        }),
        ...
        reconcile: Effect.fn(function* ({ id, news, output }) {
            ...
            if (observed === undefined) {
                ...
                observed = yield* getPool(name).pipe(Effect.repeat({
