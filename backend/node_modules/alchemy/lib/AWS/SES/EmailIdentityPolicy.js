 ...
 * @resource
 */
export const EmailIdentityPolicy = Resource("AWS.SES.EmailIdentityPolicy");
export const EmailIdentityPolicyProvider = () => Provider.effect(EmailIdentityPolicy, Effect.gen(function* () {
    ...
    const getPolicies = Effect.fn(function* (emailIdentity) {
        return yield* sesv2
            .getEmailIdentityPolicies({ EmailIdentity: emailIdentity })
            .pipe(Effect.map((response) => response.Policies ?? {}), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
    });
    return EmailIdentityPolicy.Provider.of({
        ...
        list: Effect.fn(function* () {
            ...
            const nested = yield* Effect.forEach(identities, (emailIdentity) => getPolicies(emailIdentity).pipe(Effect.map((policies) => Object.keys(policies ?? {}).map((policyName) => ({
                emailIdentity,
                policyName,
            })))), { concurrency: 2 });
            return nested.flat();
        ...
        reconcile: Effect.fn(function* ({ id, news, output }) {
            ...
            if (existing === undefined) {
                ...
                yield* sesv2
                    ...
                    .pipe(Effect.catchTag("AlreadyExistsException", () => sesv2.updateEmailIdentityPolicy({
                    EmailIdentity: emailIdentity,
                    PolicyName: policyName,
                    Policy: desiredPolicy,
                })));
                ...
                const changed = yield* Effect.sync(() => normalizePolicyDocument(existing) !==
