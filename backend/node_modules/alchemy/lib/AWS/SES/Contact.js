 ...
 * @resource
 */
export const Contact = Resource("AWS.SES.Contact");
...
const stringifyAttributes = (attributes) => attributes === undefined ? undefined : JSON.stringify(attributes);
...
const samePreferences = (a, b) => {
    const key = (prefs) => JSON.stringify([...(prefs ?? [])]
        .map((p) => ({
...
export const ContactProvider = () => Provider.effect(Contact, Effect.gen(function* () {
    const getContact = Effect.fn(function* (contactListName, emailAddress) {
        return yield* sesv2
            .getContact({
    ...
    return Contact.Provider.of({
        ...
        list: Effect.fn(function* () {
            ...
            const nested = yield* Effect.forEach(contactListNames, (contactListName) => sesv2.listContacts
                .pages({ ContactListName: contactListName })
                .pipe(Stream.runCollect, Effect.map((pages) => Array.from(pages)
                .flatMap((page) => page.Contacts ?? [])
                .flatMap((contact) => contact.EmailAddress
                ? [
                    {
                        contactListName,
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return undefined;
            if (news.contactListName !== olds.contactListName ||
        ...
        reconcile: Effect.fn(function* ({ news, output }) {
            ...
            if (observed === undefined) {
                ...
                yield* sesv2
                    ...
                    .pipe(Effect.catchTag("AlreadyExistsException", () => sesv2.updateContact({
                    ContactListName: contactListName,
                    EmailAddress: emailAddress,
                    TopicPreferences: news.topicPreferences,
                    UnsubscribeAll: news.unsubscribeAll,
                ...
                const preferencesChanged = managesPreferences &&
                    !samePreferences(observed.TopicPreferences, news.topicPreferences);
                const unsubscribeChanged = managesUnsubscribe &&
                    (observed.UnsubscribeAll ?? false) !== news.unsubscribeAll;
                const attributesChanged = managesAttributes &&
                    (yield* Effect.sync(() => normalizeAttributes(observed.AttributesData) !==
