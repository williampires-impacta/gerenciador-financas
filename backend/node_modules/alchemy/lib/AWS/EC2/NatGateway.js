...
export const NatGatewayId = (id) => `nat-${id}`;
/**
 ...
 * @resource
 */
export const NatGateway = Resource("AWS.EC2.NatGateway");
export const NatGatewayProvider = () => Provider.effect(NatGateway, Effect.gen(function* () {
    ...
    const toAttrs = Effect.fn(function* (gw) {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const gateways = Array.from(pages).flatMap((page) => (page.NatGateways ?? []).filter((gw) => gw.NatGatewayId != null && gw.State !== "deleted"));
...
const waitForNatGatewayAvailable = (natGatewayId, session) => Effect.gen(function* () {
    ...
    schedule: Schedule.max([Schedule.fixed(5000), Schedule.recurs(60)]).pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for NAT Gateway to be available... (${attempt * 5}s)`))),
}));
...
const waitForNatGatewayDeleted = (natGatewayId, session) => Effect.gen(function* () {
    ...
    schedule: Schedule.max([Schedule.fixed(5000), Schedule.recurs(60)]).pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for NAT Gateway deletion... (${attempt * 5}s)`))),
}));
