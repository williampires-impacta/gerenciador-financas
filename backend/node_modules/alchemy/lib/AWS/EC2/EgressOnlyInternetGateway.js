...
export const EgressOnlyInternetGatewayId = (id) => `eigw-${id}`;
/**
 ...
 * @resource
 */
export const EgressOnlyInternetGateway = Resource("AWS.EC2.EgressOnlyInternetGateway");
...
export const EgressOnlyInternetGatewayProvider = () => Provider.effect(EgressOnlyInternetGateway, Effect.gen(function* () {
    ...
    const describeEgressOnlyInternetGateway = (eigwId) => ec2
        .describeEgressOnlyInternetGateways({
        EgressOnlyInternetGatewayIds: [eigwId],
    })
        .pipe(Effect.map((r) => r.EgressOnlyInternetGateways?.[0]), Effect.flatMap((gw) => gw
        ? Effect.succeed(gw)
        : Effect.fail(new Error(`Egress-Only Internet Gateway ${eigwId} not found`))));
    const toAttrs = (gw) => AWSEnvironment.current.pipe(Effect.map((env) => ({
        egressOnlyInternetGatewayId: gw.EgressOnlyInternetGatewayId,
        egressOnlyInternetGatewayArn: `arn:aws:ec2:${env.region}:${env.accountId}:egress-only-internet-gateway/${gw.EgressOnlyInternetGatewayId}`,
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const items = yield* ec2.describeEgressOnlyInternetGateways
                .pages({})
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.EgressOnlyInternetGateways ?? [])
                .filter((gw) => gw.EgressOnlyInternetGatewayId != null)
                .map((gw) => ({
                egressOnlyInternetGatewayId: gw.EgressOnlyInternetGatewayId,
                egressOnlyInternetGatewayArn: `arn:aws:ec2:${env.region}:${env.accountId}:egress-only-internet-gateway/${gw.EgressOnlyInternetGatewayId}`,
                attachments: gw.Attachments?.map((a) => ({
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (output?.egressOnlyInternetGatewayId) {
                const lookup = yield* ec2
                    ...
                    .pipe(Effect.catchTag("InvalidEgressOnlyInternetGatewayId.NotFound", () => Effect.succeed({ EgressOnlyInternetGateways: [] })));
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            // takes).
            Effect.flatMap((result) => result.ReturnCode === false
                ? Effect.fail(new EigwDeleteFailed({ eigwId }))
                : Effect.void), Effect.catchTag("InvalidGatewayID.NotFound", () => Effect.void), Effect.catchTag("InvalidEgressOnlyInternetGatewayId.NotFound", () => Effect.void), 
            ...
            yield* ec2
                ...
                .pipe(Effect.flatMap((r) => (r.EgressOnlyInternetGateways ?? []).length === 0
                ? Effect.void
                : Effect.fail(new EigwStillExists({ eigwId }))), Effect.retry({
