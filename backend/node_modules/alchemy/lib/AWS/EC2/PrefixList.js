...
export const PrefixListId = (id) => `pl-${id}`;
/**
 ...
 * @resource
 */
export const PrefixList = Resource("AWS.EC2.PrefixList");
...
const isStableState = (state) => state === undefined || state.endsWith("-complete");
export const PrefixListProvider = () => Provider.effect(PrefixList, Effect.gen(function* () {
    ...
    const describePrefixList = (prefixListId) => ec2.describeManagedPrefixLists({ PrefixListIds: [prefixListId] }).pipe(Effect.map((r) => r.PrefixLists?.[0]), Effect.catchTag("InvalidPrefixListID.NotFound", () => Effect.succeed(undefined)));
    // Poll until the list leaves any "-in-progress" state.
    const waitStable = (prefixListId) => describePrefixList(prefixListId).pipe(Effect.flatMap((pl) => isStableState(pl?.State)
        ? Effect.succeed(pl)
        : Effect.fail(new PrefixListNotStable())), Effect.retry({
        while: (e) => e instanceof PrefixListNotStable,
        schedule: Schedule.max([Schedule.fixed(2000), Schedule.recurs(30)]),
    }));
    const getEntries = (prefixListId) => ec2.getManagedPrefixListEntries
        .pages({ PrefixListId: prefixListId })
        .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Entries ?? [])));
    const toAttrs = (pl) => AWSEnvironment.current.pipe(Effect.map((env) => ({
        prefixListId: pl.PrefixListId,
        prefixListArn: pl.PrefixListArn ??
            `arn:aws:ec2:${env.region}:${env.accountId}:prefix-list/${pl.PrefixListId}`,
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const items = yield* ec2.describeManagedPrefixLists.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.PrefixLists ?? [])
                // Skip AWS-managed lists (e.g. com.amazonaws.*), which
                // are owned by "AWS" and have no PrefixListName we set.
                .filter((pl) => pl.PrefixListId != null && pl.OwnerId !== "AWS")
                .map((pl) => ({
                prefixListId: pl.PrefixListId,
                prefixListArn: pl.PrefixListArn ??
                    `arn:aws:ec2:${env.region}:${env.accountId}:prefix-list/${pl.PrefixListId}`,
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const addEntries = desiredEntries
                .filter((e) => !currentByCidr.has(e.cidr) ||
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            yield* ec2
                ...
                .pipe(Effect.catchTag("InvalidPrefixListID.NotFound", () => Effect.void), 
