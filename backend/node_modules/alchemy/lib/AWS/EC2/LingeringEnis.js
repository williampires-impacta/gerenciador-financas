 ...
 */
const isReapableEni = (eni) => eni.InterfaceType === "lambda" ||
 ...
 */
const reapLingeringEnis = Effect.fn(function* (scope, session) {
    ...
    for (const eni of lingering) {
        ...
        Effect.catchTag("InvalidNetworkInterfaceID.NotFound", () => Effect.succeed("deleted")), Effect.catch(() => Effect.succeed("pending")));
        if (outcome === "deleted") {
 ...
 */
export const retryWhileLingeringEnis = (deleteCall, options) => Effect.gen(function* () {
    ...
    for (let attempt = 1;; attempt++) {
        ...
        yield* options.session.note(pendingReapable > 0
...
//# sourceMappingURL=LingeringEnis.js.map