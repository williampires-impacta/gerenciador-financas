...
export const SubnetId = (id) => `subnet-${id}`;
/**
 ...
 * @resource
 */
export const Subnet = Resource("AWS.EC2.Subnet");
export const SubnetProvider = () => Provider.effect(Subnet, Effect.gen(function* () {
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (subnet === undefined) {
                const newSubnetId = yield* ec2
                    ...
                    const newSubnetId = createResult.Subnet
                        .SubnetId;
                    return newSubnetId;
                }), Effect.tap((newSubnetId) => session.note(`Subnet created: ${newSubnetId}`)), Effect.catchTag("InvalidSubnet.Conflict", (error) => Effect.gen(function* () {
                    ...
                    const existing = yield* ec2.describeSubnets
                        ...
                        .pipe(Stream.filter((s) => hasTags(alchemyTags, Object.fromEntries((s.Tags ?? []).map((t) => [t.Key ?? "", t.Value])))), Stream.runHead, Effect.map(Option.getOrUndefined));
                    if (existing?.SubnetId === undefined) {
                        return yield* Effect.fail(error);
                    }
                    const existingId = existing.SubnetId;
                    yield* session.note(`Recovered subnet ${existingId} at ${news.cidrBlock} owned by this stack (left by an interrupted run)`);
                    return existingId;
                })));
            ...
            const observedDnsAAAA = subnet.PrivateDnsNameOptionsOnLaunch
                ?.EnableResourceNameDnsAAAARecord;
            if (observedHostnameType !== news.hostnameType ||
                observedDnsA !== news.enableResourceNameDnsARecordOnLaunch ||
                observedDnsAAAA !== news.enableResourceNameDnsAAAARecordOnLaunch) {
                if (news.enableResourceNameDnsARecordOnLaunch !== undefined ||
                    ...
                    yield* ec2.modifySubnetAttribute({
                        ...
                        EnableResourceNameDnsARecordOnLaunch: news.enableResourceNameDnsARecordOnLaunch !== undefined
                            ? { Value: news.enableResourceNameDnsARecordOnLaunch }
                            : undefined,
                        EnableResourceNameDnsAAAARecordOnLaunch: news.enableResourceNameDnsAAAARecordOnLaunch !== undefined
            ...
            const currentTags = Object.fromEntries((subnet.Tags ?? []).map((t) => [t.Key, t.Value]));
        ...
        list: () => ec2.describeSubnets.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Subnets ?? [])
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            yield* retryWhileLingeringEnis(ec2
                ...
                .pipe(Effect.tapError(Effect.logDebug), Effect.catchTag("InvalidSubnetID.NotFound", () => Effect.void)), {
 ...
 */
const toSubnetAttributes = (subnet) => ({
 ...
 */
const waitForSubnetAvailable = (subnetId, session) => Effect.gen(function* () {
 ...
 */
const waitForSubnetDeleted = (subnetId, session) => Effect.gen(function* () {
    ...
    schedule: Schedule.max([Schedule.fixed(2000), Schedule.recurs(15)]).pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for subnet deletion... (${attempt * 2}s)`))),
}));
