...
export const SecurityGroupId = (id) => `sg-${id}`;
/**
 ...
 * @resource
 */
export const SecurityGroup = Resource("AWS.EC2.SecurityGroup");
export const SecurityGroupProvider = () => Provider.effect(SecurityGroup, Effect.gen(function* () {
    ...
    const describeSecurityGroup = (groupId) => ec2.describeSecurityGroups({ GroupIds: [groupId] }).pipe(Effect.map((r) => r.SecurityGroups?.[0]), Effect.flatMap((sg) => sg
        ? Effect.succeed(sg)
        : Effect.fail(new Error(`Security Group ${groupId} not found`))));
    const describeSecurityGroupRules = (groupId) => ec2.describeSecurityGroupRules
        .items({
        Filters: [{ Name: "group-id", Values: [groupId] }],
    })
        .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
    const toAttrs = Effect.fn(function* (sg, rules) {
        const { accountId, region } = yield* AWSEnvironment.current;
        return {
            groupId: sg.GroupId,
            groupArn: `arn:aws:ec2:${region}:${accountId}:security-group/${sg.GroupId}`,
    ...
    const toIpPermission = (rule) => ({
        ...
        UserIdGroupPairs: rule.referencedGroupId
            ? [
                {
                    GroupId: rule.referencedGroupId,
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            const groups = yield* ec2.describeSecurityGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.SecurityGroups ?? []).filter((sg) => sg.GroupId != null &&
                // Every VPC's `default` group is AWS-managed and can
                // never be deleted (CannotDelete) — don't enumerate it.
                sg.GroupName !== "default"))));
            return yield* Effect.forEach(groups, (sg) => Effect.gen(function* () {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (output?.groupId) {
                const lookup = yield* ec2
                    ...
                    .pipe(Effect.catchTag("InvalidGroup.NotFound", () => Effect.succeed({ SecurityGroups: [] })));
            ...
            const currentTags = Object.fromEntries((sg.Tags ?? []).map((t) => [t.Key, t.Value]));
            ...
            if (currentIngress.length > 0) {
                yield* ec2
                    ...
                    GroupId: groupId,
                    SecurityGroupRuleIds: currentIngress.map((r) => r.SecurityGroupRuleId),
                    DryRun: false,
                })
                    .pipe(Effect.catchTag("InvalidPermission.NotFound", () => Effect.void));
            }
            if (currentEgress.length > 0) {
                yield* ec2
                    .revokeSecurityGroupEgress({
                    GroupId: groupId,
                    SecurityGroupRuleIds: currentEgress.map((r) => r.SecurityGroupRuleId),
                    DryRun: false,
                })
                    .pipe(Effect.catchTag("InvalidPermission.NotFound", () => Effect.void));
            }
