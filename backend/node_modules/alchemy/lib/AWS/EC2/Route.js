 ...
 * @resource
 */
export const Route = Resource("AWS.EC2.Route");
export const RouteProvider = () => Provider.effect(Route, Effect.gen(function* () {
    return {
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            const dest = news.destinationCidrBlock ||
            ...
            if (observed === undefined) {
                ...
                yield* ec2.replaceRoute(targetParams).pipe(Effect.tapError(Effect.log), Effect.retry({
                    while: (e) => e._tag === "InvalidRouteTableID.NotFound",
                    schedule: Schedule.exponential(100),
                }));
        ...
        list: () => Effect.gen(function* () {
            ...
            return yield* ec2.describeRouteTables.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.RouteTables ?? []).flatMap((rt) => {
                const routeTableId = rt.RouteTableId;
                if (routeTableId === undefined)
                    return [];
                if (defaultVpc.vpcId !== undefined &&
        ...
        delete: Effect.fn(function* ({ output, session }) {
            const dest = output.destinationCidrBlock ||
            ...
            yield* ec2
                ...
                .pipe(Effect.tapError(Effect.logDebug), Effect.catchTag("InvalidRoute.NotFound", () => Effect.void), Effect.catchTag("InvalidRouteTableID.NotFound", () => Effect.void));
 ...
 */
const describeRoute = (routeTableId, props) => Effect.gen(function* () {
