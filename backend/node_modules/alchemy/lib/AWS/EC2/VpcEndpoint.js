...
export const VpcEndpointId = (id) => `vpce-${id}`;
/**
 ...
 * @resource
 */
export const VpcEndpoint = Resource("AWS.EC2.VpcEndpoint");
export const VpcEndpointProvider = () => Provider.effect(VpcEndpoint, Effect.gen(function* () {
    ...
    const describeVpcEndpoint = (vpcEndpointId) => ec2.describeVpcEndpoints({ VpcEndpointIds: [vpcEndpointId] }).pipe(
    ...
    }), Effect.map((r) => r.VpcEndpoints?.[0]), Effect.flatMap((ep) => ep
        ? Effect.succeed(ep)
        : Effect.fail(new Error(`VPC Endpoint ${vpcEndpointId} not found`))));
    const toAttrs = Effect.fn(function* (ep) {
        const { accountId, region } = yield* AWSEnvironment.current;
        return {
            vpcEndpointId: ep.VpcEndpointId,
            vpcEndpointArn: `arn:aws:ec2:${region}:${accountId}:vpc-endpoint/${ep.VpcEndpointId}`,
            ...
            creationTimestamp: ep.CreationTimestamp instanceof Date
            ...
            dnsOptions: ep.DnsOptions
                ? {
                    ...
                    privateDnsOnlyForInboundResolverEndpoint: ep.DnsOptions.PrivateDnsOnlyForInboundResolverEndpoint,
                }
                : undefined,
    ...
    return {
        ...
        read: Effect.fn(function* ({ output }) {
            ...
            const lookup = yield* ec2
                ...
                .pipe(Effect.catchTag("InvalidVpcEndpointId.NotFound", () => Effect.succeed({ VpcEndpoints: [] })));
        ...
        list: () => Effect.gen(function* () {
            const endpoints = yield* ec2.describeVpcEndpoints.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.VpcEndpoints ?? []).filter((ep) => ep.VpcEndpointId != null))));
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (ep === undefined) {
                ...
                const result = yield* ec2.createVpcEndpoint({
                    ...
                    RouteTableIds: news.routeTableIds,
                    ...
                    DnsOptions: news.dnsOptions
                        ? {
                            ...
                            PrivateDnsOnlyForInboundResolverEndpoint: news.dnsOptions.privateDnsOnlyForInboundResolverEndpoint,
                        }
                        : undefined,
                    TagSpecifications: [
                        {
            ...
            if (observedType === "Gateway") {
                ...
                const desiredRtIds = new Set(news.routeTableIds ?? []);
            ...
            if (observedType === "Interface" ||
                ...
                const observedSgIds = new Set((ep.Groups ?? [])
                    .map((g) => g.GroupId)
                    .filter((g) => Boolean(g)));
                const desiredSgIds = new Set(news.securityGroupIds ?? []);
                ...
                if (news.privateDnsEnabled !== undefined &&
            ...
            if (news.dnsOptions !== undefined) {
                ...
                if ((news.dnsOptions.dnsRecordIpType !== undefined &&
            ...
            const currentTags = Object.fromEntries((ep.Tags ?? []).map((t) => [t.Key, t.Value]));
 ...
 */
const waitForVpcEndpointAvailable = (vpcEndpointId, session) => Effect.gen(function* () {
    ...
    schedule: Schedule.max([Schedule.fixed(3000), Schedule.recurs(60)]).pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for VPC Endpoint to be available... (${attempt * 3}s)`))),
}));
/**
 * Wait for VPC Endpoint to be deleted
 */
const waitForVpcEndpointDeleted = (vpcEndpointId, session) => Effect.gen(function* () {
    const result = yield* ec2
        .describeVpcEndpoints({ VpcEndpointIds: [vpcEndpointId] })
        .pipe(Effect.catchTag("InvalidVpcEndpointId.NotFound", () => Effect.succeed({ VpcEndpoints: [] })));
    const ep = result.VpcEndpoints?.[0];
    ...
    schedule: Schedule.max([Schedule.fixed(3000), Schedule.recurs(60)]).pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for VPC Endpoint deletion... (${attempt * 3}s)`))),
}));
/**
 ...
 */
const waitForEndpointEnisReleased = (networkInterfaceIds, session) => Effect.gen(function* () {
    const remaining = yield* ec2.describeNetworkInterfaces
        ...
        .pipe(Stream.map((eni) => eni.NetworkInterfaceId), Stream.filter((id) => Boolean(id)), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
    ...
    schedule: Schedule.max([Schedule.fixed(3000), Schedule.recurs(20)]).pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for endpoint network interfaces to release... (${attempt * 3}s)`))),
}));
/**
 ...
 */
const isFullAccessEndpointPolicy = (doc) => {
    if (!doc)
        return true;
    try {
 ...
 */
const policyDocumentEquals = (a, b) => {
    if (a === b)
        return true;
