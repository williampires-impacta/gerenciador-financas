...
export const VpcPeeringConnectionId = (id) => `pcx-${id}`;
 ...
 * @resource
 */
export const VpcPeeringConnection = Resource("AWS.EC2.VpcPeeringConnection");
class PeeringNotSettled {
    _tag = "PeeringNotSettled";
}
export const VpcPeeringConnectionProvider = () => Provider.effect(VpcPeeringConnection, Effect.gen(function* () {
    ...
    const describePeering = (pcxId) => ec2
        .describeVpcPeeringConnections({ VpcPeeringConnectionIds: [pcxId] })
        .pipe(Effect.map((r) => r.VpcPeeringConnections?.[0]), Effect.catchTag("InvalidVpcPeeringConnectionID.NotFound", () => Effect.succeed(undefined)), Effect.catchTag("InvalidVpcPeeringConnectionId.NotFound", () => Effect.succeed(undefined)));
    ...
    const waitFor = (pcxId, target) => describePeering(pcxId).pipe(Effect.flatMap((pcx) => {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            const items = yield* ec2.describeVpcPeeringConnections
                .pages({})
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.VpcPeeringConnections ?? [])
                .filter((pcx) => pcx.VpcPeeringConnectionId != null &&
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (autoAccept) {
                ...
                if (settled?.Status?.Code === "pending-acceptance") {
                    ...
                    Effect.catchTag("InvalidVpcPeeringConnectionID.NotFound", () => Effect.succeed(undefined)), Effect.catchTag("InvalidVpcPeeringConnectionId.NotFound", () => Effect.succeed(undefined)));
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            yield* ec2
                ...
                .pipe(Effect.catchTag("InvalidVpcPeeringConnectionID.NotFound", () => Effect.void), Effect.catchTag("InvalidVpcPeeringConnectionId.NotFound", () => Effect.void));
        }),
    };
}));
