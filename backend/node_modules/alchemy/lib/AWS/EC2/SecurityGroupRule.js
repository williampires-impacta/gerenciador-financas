...
import { getDefaultVpcDefaultSecurityGroupId, getDefaultVpcScope, } from "./defaultVpcScope.js";
export const SecurityGroupRuleId = (id) => `sgr-${id}`;
/**
 ...
 * @resource
 */
export const SecurityGroupRule = Resource("AWS.EC2.SecurityGroupRule");
export const SecurityGroupRuleProvider = () => Provider.effect(SecurityGroupRule, Effect.gen(function* () {
    ...
    const describeRule = (ruleId) => ec2.describeSecurityGroupRules({ SecurityGroupRuleIds: [ruleId] }).pipe(Effect.map((r) => r.SecurityGroupRules?.[0]), Effect.flatMap((rule) => rule
        ? Effect.succeed(rule)
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            return yield* ec2.describeSecurityGroupRules.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.SecurityGroupRules ?? [])
                .filter((rule) => rule.SecurityGroupRuleId != null)
                .filter((rule) => defaultSgId === undefined ||
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const ipPermission = {
                ...
                UserIdGroupPairs: news.referencedGroupId
                    ? [
                        {
                            GroupId: news.referencedGroupId,
                ...
                PrefixListIds: news.prefixListId
                    ? [
                        {
                            PrefixListId: news.prefixListId,
                            Description: news.description,
                        },
                    ]
                    : undefined,
            ...
            if (output?.securityGroupRuleId) {
                const lookup = yield* ec2
                    ...
                    .pipe(Effect.catchTag("InvalidSecurityGroupRuleId.NotFound", () => Effect.succeed({ SecurityGroupRules: [] })));
            ...
            if (observed === undefined) {
                ...
                const result = news.type === "ingress"
                    ...
                    SecurityGroupRules: [
                        {
                            ...
                            SecurityGroupRule: {
                                ...
                                ReferencedGroupId: news.referencedGroupId,
                                PrefixListId: news.prefixListId,
                                Description: news.description,
                            },
                        },
            ...
            const currentTags = Object.fromEntries((observed.Tags ?? []).map((t) => [t.Key, t.Value]));
        ...
        delete: Effect.fn(function* ({ olds, output, session }) {
            ...
            const groupId = (output.groupId ?? olds?.groupId);
            ...
            if (isEgress) {
                yield* ec2
                    ...
                    .pipe(Effect.catchTag("InvalidPermission.NotFound", () => Effect.void), Effect.catchTag("InvalidGroup.NotFound", () => Effect.void));
            }
            else {
                yield* ec2
