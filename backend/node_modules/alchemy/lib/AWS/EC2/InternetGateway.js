...
import { retryWhileLingeringEnis } from "./LingeringEnis.js";
export const InternetGatewayId = (id) => `igw-${id}`;
/**
 ...
 * @resource
 */
export const InternetGateway = Resource("AWS.EC2.InternetGateway");
export const InternetGatewayProvider = () => Provider.effect(InternetGateway, Effect.gen(function* () {
    const findInternetGatewayByTags = Effect.fn(function* (id) {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            return yield* ec2.describeInternetGateways.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.InternetGateways ?? [])
                .filter((igw) => igw.InternetGatewayId != null)
                .filter((igw) => defaultVpc.vpcId === undefined ||
                !igw.Attachments?.some((a) => a.VpcId === defaultVpc.vpcId))
                .map((igw) => {
                const internetGatewayId = igw.InternetGatewayId;
                const attachedVpcId = igw.Attachments?.find((a) => a.State === "available" || a.State === "attaching")?.VpcId;
                return {
                    internetGatewayId,
        ...
        reconcile: Effect.fn(function* ({ id, instanceId, news = {}, output, session, }) {
            ...
            if (output?.internetGatewayId) {
                const lookup = yield* ec2
                    ...
                    .pipe(Effect.catchTag("InvalidInternetGatewayID.NotFound", () => Effect.succeed({ InternetGateways: [] })));
            ...
            const currentTags = Object.fromEntries((igw.Tags ?? []).map((t) => [t.Key, t.Value]));
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            if (attachments.length > 0) {
                for (const attachment of attachments) {
                    yield* retryWhileLingeringEnis(ec2
                        .detachInternetGateway({
                        InternetGatewayId: internetGatewayId,
                        VpcId: attachment.VpcId,
                    })
                        .pipe(Effect.tapError(Effect.logDebug), Effect.catchTag("Gateway.NotAttached", () => Effect.void), Effect.catchTag("InvalidInternetGatewayID.NotFound", () => Effect.void)), {
            ...
            yield* ec2
                ...
                .pipe(Effect.tapError(Effect.logDebug), Effect.catchTag("InvalidInternetGatewayID.NotFound", () => Effect.void), 
 ...
 */
const findInternetGateway = (internetGatewayId) => Effect.gen(function* () {
    const result = yield* ec2
        .describeInternetGateways({ InternetGatewayIds: [internetGatewayId] })
        .pipe(Effect.catchTag("InvalidInternetGatewayID.NotFound", () => Effect.succeed({ InternetGateways: [] })));
    return result.InternetGateways?.[0];
});
const describeInternetGateway = (internetGatewayId) => findInternetGateway(internetGatewayId).pipe(Effect.flatMap((igw) => igw
    ? Effect.succeed(igw)
    : Effect.fail(new InternetGatewayNotVisible({ internetGatewayId }))), Effect.retry({
...
class InternetGatewayAttachmentPending extends Data.TaggedError("InternetGatewayAttachmentPending") {
}
const waitForInternetGatewayAttached = (internetGatewayId, vpcId, session) => Effect.gen(function* () {
    ...
    schedule: Schedule.max([Schedule.fixed(1000), Schedule.recurs(30)]).pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for internet gateway attachment to become available... (${attempt}s)`))),
}));
/**
 * Wait for internet gateway to be deleted
 */
const waitForInternetGatewayDeleted = (internetGatewayId, session) => Effect.gen(function* () {
    yield* Effect.retry(Effect.gen(function* () {
        const result = yield* ec2
            .describeInternetGateways({ InternetGatewayIds: [internetGatewayId] })
            .pipe(Effect.tapError(Effect.logDebug), Effect.catchTag("InvalidInternetGatewayID.NotFound", () => Effect.succeed({ InternetGateways: [] })));
        ...
        schedule: Schedule.max([
            Schedule.fixed(2000),
            Schedule.recurs(15),
        ]).pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for internet gateway deletion... (${attempt * 2}s)`))),
    });
});
