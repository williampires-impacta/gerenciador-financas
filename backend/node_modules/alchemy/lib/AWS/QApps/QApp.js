 ...
 * @resource
 */
export const QApp = Resource("AWS.QApps.QApp");
const createTitle = (id, props) => props.title
...
const readQAppById = Effect.fn(function* (instanceId, appId) {
...
const findQAppByTitle = Effect.fn(function* (instanceId, title) {
    const apps = yield* qapps.listQApps.pages({ instanceId }).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.apps ?? [])));
 ...
 */
const isSubsetOf = (desired, observed) => {
...
const definitionDrifted = (desired, observed) => {
    if (desired.initialPrompt !== undefined &&
        desired.initialPrompt !== observed.initialPrompt) {
        return true;
    }
    return !isSubsetOf(desired.cards, observed.appDefinition.cards);
};
export const QAppProvider = () => Provider.effect(QApp, Effect.gen(function* () {
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const needsUpdate = title !== observed.title ||
