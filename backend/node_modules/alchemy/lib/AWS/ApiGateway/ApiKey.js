...
import { apiKeyArn, syncTags } from "./common.js";
/**
 ...
 * ```
 */
const ApiKeyResource = Resource("AWS.ApiGateway.ApiKey");
export { ApiKeyResource as ApiKey };
const resolvedValue = (value) => value ? Redacted.value(value) : undefined;
...
const readByName = Effect.fn(function* (id, name) {
...
export const ApiKeyProvider = () => Provider.effect(ApiKeyResource, Effect.gen(function* () {
    return {
        ...
        list: () => ag.getApiKeys.pages({ limit: 500, includeValues: false }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.items ?? [])
            .filter((k) => !!k.id)
            .map((k) => ({
            id: k.id,
            name: k.name,
        ...
        reconcile: Effect.fn(function* ({ id, news: newsIn, output, session }) {
            ...
            if (!observed?.id) {
                const created = yield* ag
                    ...
                    if (existing)
                        return existing;
                    return yield* Effect.fail(new ag.ConflictException({
            ...
            if (!final.id)
                return yield* Effect.die("getApiKey missing id");
            return {
                id: final.id,
                name: final.name,
