 ...
 */
const isApiStatusUpdatingError = (error) => {
    if (!error || typeof error !== "object")
        return false;
    if (error._tag !== "BadRequestException")
        return false;
    const message = error.message ?? "";
    return (message.includes("apiStatus is UPDATING") ||
 ...
 * ```
 */
export const retryOnApiStatusUpdating = (effect) => Effect.retry(effect, {
    schedule: apiGatewayMutationSchedule,
    while: isApiStatusUpdatingError,
});
const isRestApiDeleteRetryable = (error) => {
    if (!error || typeof error !== "object")
        return false;
    const tagged = error;
    return (tagged._tag === "TooManyRequestsException" ||
 ...
 */
export const deleteRestApiAndWait = Effect.fn(function* (restApiId) {
    yield* ag.deleteRestApi({ restApiId }).pipe(Retry.policy({
        while: isRestApiDeleteRetryable,
        schedule: restApiDeleteSchedule,
    }), Effect.catchTag("NotFoundException", () => Effect.void), Semaphore.withPermits(restApiDeleteMutex, 1));
    ...
    yield* observe.pipe(Effect.flatMap(() => Effect.fail(new RestApiStillExists({ restApiId }))), Effect.retry({
        while: (error) => error._tag === "TooManyRequestsException" ||
            error._tag === "RestApiStillExists",
        schedule: restApiGoneSchedule,
    }), Effect.catchTag("NotFoundException", () => Effect.void));
});
export const restApiArn = (region, restApiId) => `arn:aws:apigateway:${region}::/restapis/${restApiId}`;
export const stageArn = (region, restApiId, stageName) => `arn:aws:apigateway:${region}::/restapis/${restApiId}/stages/${stageName}`;
export const apiKeyArn = (region, apiKeyId) => `arn:aws:apigateway:${region}::/apikeys/${apiKeyId}`;
export const usagePlanArn = (region, usagePlanId) => `arn:aws:apigateway:${region}::/usageplans/${usagePlanId}`;
...
export const syncTags = Effect.fn(function* ({ resourceArn, oldTags, newTags, }) {
    ...
    if (removed.length > 0) {
        yield* ag
            ...
            .pipe(Effect.catchTag("NotFoundException", () => Effect.void), Effect.catchTag("BadRequestException", () => Effect.void));
    }
    if (upsert.length > 0) {
