 ...
 * ```
 */
const GatewayResponseResource = Resource("AWS.ApiGateway.GatewayResponse");
export { GatewayResponseResource as GatewayResponse };
export const GatewayResponseProvider = () => Provider.effect(GatewayResponseResource, Effect.gen(function* () {
    return {
        stables: ["restApiId", "responseType"],
        ...
        list: () => Effect.gen(function* () {
            const restApiIds = yield* ag.getRestApis.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.items ?? [])
                .map((a) => a.id)
                .filter((id) => id != null))));
            const rows = yield* Effect.forEach(restApiIds, (restApiId) => ag.getGatewayResponses({ restApiId }).pipe(Effect.map((res) => (res.items ?? [])
                .filter((g) => g.responseType != null)
                .map((g) => ({
                restApiId,
                responseType: g.responseType,
                statusCode: g.statusCode,
        ...
        reconcile: Effect.fn(function* ({ news: newsIn, output, session }) {
            ...
            // `output` exists.
            yield* retryOnApiStatusUpdating(ag.putGatewayResponse({
                restApiId,
                responseType,
