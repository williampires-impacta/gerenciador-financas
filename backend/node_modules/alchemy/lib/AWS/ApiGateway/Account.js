 ...
 * ```
 */
const AccountResource = Resource("AWS.ApiGateway.Account");
export { AccountResource as Account };
export const AccountProvider = () => Provider.effect(AccountResource, Effect.gen(function* () {
    return {
        ...
        list: () => ag.getAccount({}).pipe(Effect.map((a) => [
            {
                cloudwatchRoleArn: a.cloudwatchRoleArn,
        ...
        reconcile: Effect.fn(function* ({ news: newsIn, session }) {
            ...
            if (manages) {
                ...
                if (desired !== observedRoleArn) {
                    yield* retryOnApiStatusUpdating(ag.updateAccount({
