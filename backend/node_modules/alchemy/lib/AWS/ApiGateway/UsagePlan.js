...
import { syncTags, usagePlanArn } from "./common.js";
/**
 ...
 * ```
 */
const UsagePlanResource = Resource("AWS.ApiGateway.UsagePlan");
...
const apiStageKey = (stage) => `${stage.apiId}:${stage.stage}`;
const parseThrottleKey = (key) => {
...
const buildApiStageThrottlePatches = (stageKey, prev, next) => {
    const keys = new Set([
        ...Object.keys(prev ?? {}),
        ...Object.keys(next ?? {}),
    ]);
    ...
    for (const key of keys) {
        ...
        if (current?.burstLimit !== old?.burstLimit) {
            patches.push(current?.burstLimit === undefined
        ...
        if (current?.rateLimit !== old?.rateLimit) {
            patches.push(current?.rateLimit === undefined
...
const buildApiStagePatches = (prev, next) => {
...
const buildThrottlePatches = (prev, next) => {
    const patches = [];
    if (next?.burstLimit !== prev?.burstLimit) {
        patches.push(next?.burstLimit === undefined
    ...
    if (next?.rateLimit !== prev?.rateLimit) {
        patches.push(next?.rateLimit === undefined
...
const buildQuotaPatches = (prev, next) => {
    const patches = [];
    if (next?.limit !== prev?.limit) {
        patches.push(next?.limit === undefined
    ...
    if (next?.offset !== prev?.offset) {
        patches.push(next?.offset === undefined
    ...
    if (next?.period !== prev?.period) {
        patches.push(next?.period === undefined
            ? { op: "remove", path: "/quota/period" }
            : { op: "replace", path: "/quota/period", value: next.period });
    }
    return patches;
};
export const UsagePlanProvider = () => Provider.effect(UsagePlanResource, Effect.gen(function* () {
    return {
        stables: ["id"],
        ...
        list: () => ag.getUsagePlans.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.items ?? [])
            .filter((p) => p.id != null)
            .map((p) => ({
            id: p.id,
            name: p.name,
