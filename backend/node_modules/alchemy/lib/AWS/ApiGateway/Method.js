 ...
 * @resource
 */
export const MethodResource = Resource("AWS.ApiGateway.Method");
const MethodImpl = (id, props) => Effect.gen(function* () {
    ...
    if (!restApiId || !resourceId) {
        return yield* Effect.die("Method requires either `restApi` (preferred) or explicit " +
...
const putIntegrationRequest = (restApiId, resourceId, httpMethod, integration) => ({
    restApiId,
    resourceId,
    httpMethod,
...
const putMethod = (news) => ag.putMethod({
...
const deleteIntegrationSafe = (p) => ag
    .deleteIntegration({
...
const deleteMethodSafe = (p) => ag
    ...
    .pipe(Effect.catchTag("NotFoundException", () => Effect.void));
const readMethodSnapshot = (p) => Effect.gen(function* () {
...
export const MethodProvider = () => Provider.effect(MethodResource, Effect.gen(function* () {
    return {
        stables: ["restApiId", "resourceId", "httpMethod"],
        ...
        list: () => Effect.gen(function* () {
            const restApis = yield* ag.getRestApis.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.items ?? []).filter((api) => api.id != null))));
            const perApi = yield* Effect.forEach(restApis, (api) => Effect.gen(function* () {
                ...
                const keys = resources.flatMap((resource) => resource.id
                    ? Object.keys(resource.resourceMethods ?? {}).map((httpMethod) => ({
                        resourceId: resource.id,
                        httpMethod,
                    }))
                    : []);
                const snaps = yield* Effect.forEach(keys, (key) => readMethodSnapshot({
                    restApiId: api.id,
                    resourceId: key.resourceId,
                    httpMethod: key.httpMethod,
                }), { concurrency: 10 });
                return snaps.filter((snap) => snap !== undefined);
            }), { concurrency: 5 });
            return perApi.flat();
        }),
        reconcile: Effect.fn(function* ({ news: newsIn, output, session }) {
            ...
            const resourceId = (output?.resourceId ?? news.resourceId);
            ...
            const needsRecreate = observed !== undefined &&
            ...
            if (observed === undefined || needsRecreate) {
                ...
                if (news.integration) {
                    yield* ag.putIntegration(putIntegrationRequest(restApiId, resourceId, httpMethod, news.integration));
                }
                yield* session.note(needsRecreate
