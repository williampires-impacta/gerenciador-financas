...
import { deleteRestApiAndWait, restApiArn, retryOnApiStatusUpdating, syncTags, } from "./common.js";
/**
 ...
 * @resource
 */
export const RestApi = Resource("AWS.ApiGateway.RestApi");
...
const patchReplace = (path, value) => ({
    op: "replace",
    path,
    value,
});
const encodeJsonPointerSegment = (s) => s.replace(/~/g, "~0").replace(/\//g, "~1");
const binaryMediaTypePath = (mediaType) => `/binaryMediaTypes/${encodeJsonPointerSegment(mediaType)}`;
const buildBinaryMediaTypePatches = (prev, next) => {
...
const buildUpdatePatches = (news, prev) => {
    const patches = [];
    ...
    if (news.minimumCompressionSize !== prev.minimumCompressionSize) {
        patches.push(patchReplace("/minimumCompressionSize", String(news.minimumCompressionSize ?? "")));
    }
    ...
    if (news.disableExecuteApiEndpoint !== prev.disableExecuteApiEndpoint) {
        patches.push(patchReplace("/disableExecuteApiEndpoint", String(!!news.disableExecuteApiEndpoint)));
    }
...
export const RestApiProvider = () => Provider.effect(RestApi, Effect.gen(function* () {
    return {
        stables: ["restApiId", "rootResourceId"],
        diff: Effect.fn(function* ({ news: newsIn, olds }) {
            ...
            !deepEqual(news.endpointConfiguration?.types, olds.endpointConfiguration?.types) ||
                !deepEqual(news.endpointConfiguration?.vpcEndpointIds, olds.endpointConfiguration?.vpcEndpointIds) ||
                !deepEqual(news.endpointConfiguration?.ipAddressType, olds.endpointConfiguration?.ipAddressType)) {
        ...
        list: () => ag.getRestApis.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.items ?? [])
        ...
        precreate: Effect.fn(function* ({ id, news: newsIn, session }) {
            ...
            const created = yield* retryOnApiStatusUpdating(ag.createRestApi({
                name,
        ...
        reconcile: Effect.fn(function* ({ id, news: newsIn, output, session }) {
            ...
            const observed = yield* ag
                ...
                .pipe(Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
            if (!observed?.id) {
                return yield* Effect.die(`RestApi ${output.restApiId} disappeared between precreate and reconcile`);
            }
            ...
            if (patches.length > 0) {
                yield* retryOnApiStatusUpdating(ag.updateRestApi({
