 ...
 * ```
 */
export const Key = Resource("AWS.KMS.Key");
const defaultKeyUsage = "ENCRYPT_DECRYPT";
const defaultKeySpec = "SYMMETRIC_DEFAULT";
const defaultDeletionWindowInDays = 30;
export const KeyProvider = () => Provider.succeed(Key, {
    stables: ["keyId", "keyArn"],
    list: () => Effect.gen(function* () {
        const keys = yield* kms.listKeys.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Keys ?? [])
            .map((key) => key.KeyId)
            .filter((keyId) => keyId !== undefined))));
        const hydrated = yield* Effect.forEach(keys, (keyId) => readKey({
            keyId,
            deletionWindowInDays: defaultDeletionWindowInDays,
        }), { concurrency: 5 });
        return hydrated.filter((key) => key !== undefined && key.keyState !== "PendingDeletion");
    }),
    read: Effect.fn(function* ({ olds = {}, output }) {
        ...
        return yield* readKey({
            keyId: output.keyId,
            deletionWindowInDays: output.deletionWindowInDays ??
    ...
    diff: Effect.fn(function* ({ news = {}, olds = {} }) {
        if (!isResolved(news))
            return;
        if ((news.keyUsage ?? defaultKeyUsage) !==
    ...
    reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
        ...
        if (enabled && !state.enabled) {
            yield* kms.enableKey({ KeyId: state.keyId }).pipe(Effect.retry({
                while: isKmsEventuallyConsistent,
                schedule: kmsRetrySchedule,
            }));
        }
        ...
        if (enableKeyRotation &&
            ...
            yield* kms.disableKeyRotation({ KeyId: state.keyId }).pipe(Effect.retry({
                while: isKmsEventuallyConsistent,
                schedule: kmsRetrySchedule,
            }));
        }
        if (!enabled && state.enabled) {
            yield* kms.disableKey({ KeyId: state.keyId }).pipe(Effect.retry({
                while: isKmsEventuallyConsistent,
                schedule: kmsRetrySchedule,
            }));
        }
    ...
    delete: Effect.fn(function* ({ output, session }) {
        yield* kms
            ...
            .pipe(Effect.catchTag("NotFoundException", () => Effect.void), Effect.catchTag("KMSInvalidStateException", () => Effect.void));
        const remaining = yield* Effect.repeat(kms.describeKey({ KeyId: output.keyId }).pipe(Effect.map((response) => response.KeyMetadata?.KeyState), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined))), {
        ...
        if (remaining !== undefined && remaining !== "PendingDeletion") {
            yield* Effect.die(new Error(`KMS key ${output.keyId} remained ${remaining} after scheduling deletion`));
        }
        yield* session.note(`Scheduled KMS key deletion: ${output.keyId}`);
    }),
});
const readKey = Effect.fn(function* ({ keyId, deletionWindowInDays, }) {
...
const readConvergedKey = Effect.fn(function* ({ keyId, deletionWindowInDays, desiredDescription, desiredEnabled, desiredKeyRotationEnabled, desiredPolicy, desiredRotationPeriodInDays, desiredTags, }) {
    const observeConverged = Effect.gen(function* () {
        ...
        if (!Object.entries(desiredTags).every(([name, value]) => key.tags[name] === value) ||
    ...
    return yield* Effect.gen(function* () {
        ...
        while: (error) => error._tag === "KmsKeyNotConverged" || isKmsEventuallyConsistent(error),
        schedule: kmsRetrySchedule,
    }));
});
const listKeyTags = Effect.fn(function* (keyId) {
    const pages = yield* kms.listResourceTags.pages({ KeyId: keyId }).pipe(Stream.runCollect, Effect.catchTag("NotFoundException", () => Effect.succeed([])));
    return toTagRecord(Array.from(pages).flatMap((page) => page.Tags ?? []));
});
const readKeyRotation = Effect.fn(function* (keyId) {
    return yield* kms.getKeyRotationStatus({ KeyId: keyId }).pipe(Effect.map((response) => ({
        enabled: response.KeyRotationEnabled,
        periodInDays: response.RotationPeriodInDays,
    })), Effect.catchTag("UnsupportedOperationException", () => Effect.succeed(undefined)), Effect.catchTag("KMSInvalidStateException", () => Effect.succeed(undefined)), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
});
const readKeyPolicy = Effect.fn(function* (keyId) {
    return yield* kms.getKeyPolicy({ KeyId: keyId, PolicyName: "default" }).pipe(Effect.map((response) => response.Policy), Effect.catchTag("KMSInvalidStateException", () => Effect.succeed(undefined)), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
});
const toAttrs = ({ metadata, tags, rotation, policy, deletionWindowInDays, }) => ({
...
const toTagRecord = (tags) => Object.fromEntries((tags ?? [])
    .filter((tag) => typeof tag.TagKey === "string" && typeof tag.TagValue === "string")
    .map((tag) => [tag.TagKey, tag.TagValue]));
const toKmsTags = (tags) => Object.entries(tags).map(([TagKey, TagValue]) => ({ TagKey, TagValue }));
/**
 ...
 */
const samePolicy = (observed, desired) => {
    if (observed === desired)
        return true;
    if (observed === undefined || desired === undefined)
        return false;
