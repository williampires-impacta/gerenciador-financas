 ...
 */
export const toSqlParameter = (name, value) => {
 ...
 */
export const fromField = (field, typeName) => {
    const f = field;
    if (f.isNull) {
        return null;
    }
...
export const toRows = (response) => {
    ...
    return (response.records ?? []).map((record) => {
        ...
        record.forEach((field, index) => {
            ...
            row[column?.label ?? column?.name ?? `column${index}`] = fromField(field, column?.typeName ?? undefined);
        });
        return row;
    });
};
