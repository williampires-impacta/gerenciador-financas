...
import { StartSyncExecution } from "./StartSyncExecution.js";
/**
...
const startSyncExecutionOnSyncEndpoint = Effect.gen(function* () {
    ...
    const syncStatesEndpoint = Effect.map(regionEffect, (region) => `https://sync-states.${region}.amazonaws.com`);
    ...
    return (input) => op(input).pipe(Effect.provideService(Endpoint.Endpoint, syncStatesEndpoint));
});
export const StartSyncExecutionHttp = Layer.effect(StartSyncExecution, makeStateMachineArnHttpBinding({
    tag: "AWS.StepFunctions.StartSyncExecution",
    operation: startSyncExecutionOnSyncEndpoint,
    actions: ["states:StartSyncExecution"],
}));
//# sourceMappingURL=StartSyncExecutionHttp.js.map