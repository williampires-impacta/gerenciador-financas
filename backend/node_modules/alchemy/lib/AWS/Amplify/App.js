 ...
 * @resource
 */
export const App = Resource("AWS.Amplify.App");
...
export const AppProvider = () => Provider.effect(App, Effect.gen(function* () {
    const toName = (id, props = {}) => props.name
        ? Effect.succeed(props.name)
        : createPhysicalName({ id, maxLength: 100 });
    const observe = (appId) => amplify.getApp({ appId }).pipe(Effect.map((r) => r.app), 
    ...
    // up on the response.
    const findOwnApp = Effect.fn(function* (id, name) {
        ...
        for (const app of apps) {
            if (app.name !== name)
                continue;
            if (yield* hasAlchemyTags(id, app.tags))
                return app;
        }
        return undefined;
    });
    const syncTags = Effect.fn(function* (appArn, desiredTags, observedTags) {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!app) {
                ...
                app = yield* findOwnApp(id, name).pipe(Effect.flatMap((existing) => existing
                    ...
                    : amplify
                        ...
                        .pipe(Effect.map((r) => r.app))), Effect.retry({
                    while: (e) => e._tag === "TimeoutException" ||
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* amplify.deleteApp({ appId: output.appId }).pipe(Effect.retry({
                while: (e) => e._tag === "TimeoutException" ||
                    (e._tag === "BadRequestException" &&
                        (e.message ?? "").includes("Rate exceeded")),
                schedule: amplifyWriteRetrySchedule,
