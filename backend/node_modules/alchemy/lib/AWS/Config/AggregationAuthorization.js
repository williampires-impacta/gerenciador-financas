 ...
 * @resource
 */
export const AggregationAuthorization = Resource("AWS.Config.AggregationAuthorization");
export const AggregationAuthorizationProvider = () => Provider.effect(AggregationAuthorization, Effect.gen(function* () {
    // There is no Get API — enumerate and match on the (account, region)
    // identity pair.
    const observeAuthorization = Effect.fn(function* (accountId, region) {
        return yield* config.describeAggregationAuthorizations.items({}).pipe(Stream.filter((auth) => auth.AuthorizedAccountId === accountId &&
            auth.AuthorizedAwsRegion === region), Stream.runHead, Effect.map((head) => (head._tag === "Some" ? head.value : undefined)));
    });
    ...
    return AggregationAuthorization.Provider.of({
        ...
        list: () => config.describeAggregationAuthorizations.items({}).pipe(Stream.runCollect, Effect.map((auths) => Array.from(auths).flatMap((auth) => auth.AggregationAuthorizationArn &&
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return undefined;
            if (olds.authorizedAccountId !== news.authorizedAccountId ||
        ...
        reconcile: Effect.fn(function* ({ id, news, session }) {
            ...
            let live = yield* observeAuthorization(news.authorizedAccountId, news.authorizedAwsRegion);
