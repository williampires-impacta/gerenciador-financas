 ...
 * @resource
 */
export const DeliveryChannel = Resource("AWS.Config.DeliveryChannel");
/**
 ...
 * @internal
 */
const retryChannelPut = (self) => Effect.retry(self, {
    while: (e) => e._tag === "NoAvailableConfigurationRecorderException" ||
 ...
 * @internal
 */
const retryChannelDelete = (self) => Effect.retry(self, {
    while: (e) => e._tag === "LastDeliveryChannelDeleteFailedException",
    schedule: Schedule.max([Schedule.fixed("2 seconds"), Schedule.recurs(20)]),
});
export const DeliveryChannelProvider = () => Provider.effect(DeliveryChannel, Effect.gen(function* () {
    const createChannelName = Effect.fn(function* (id, props) {
        return (props.name ?? (yield* createPhysicalName({ id, maxLength: 256 })));
    });
    const toWireChannel = (name, props) => ({
        name,
        s3BucketName: props.s3BucketName,
        s3KeyPrefix: props.s3KeyPrefix,
        s3KmsKeyArn: props.s3KmsKeyArn,
        snsTopicARN: props.snsTopicArn,
        configSnapshotDeliveryProperties: props.snapshotDeliveryFrequency === undefined
            ? undefined
            : { deliveryFrequency: props.snapshotDeliveryFrequency },
    });
    const observeChannel = Effect.fn(function* (name) {
        const response = yield* config
            .describeDeliveryChannels({ DeliveryChannelNames: [name] })
            .pipe(Effect.catchTag("NoSuchDeliveryChannelException", () => Effect.succeed({ DeliveryChannels: [] })));
        return (response.DeliveryChannels ?? []).at(0);
    });
    return DeliveryChannel.Provider.of({
        stables: ["deliveryChannelName"],
        list: () => config.describeDeliveryChannels({}).pipe(Effect.map((response) => (response.DeliveryChannels ?? []).flatMap((channel) => channel.name && channel.s3BucketName
            ? [
                {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.deliveryChannelName ??
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* retryChannelDelete(config.deleteDeliveryChannel({
