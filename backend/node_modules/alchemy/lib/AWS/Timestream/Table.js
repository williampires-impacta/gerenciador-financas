 ...
 * @resource
 */
export const Table = Resource("AWS.Timestream.Table");
 ...
 */
const toWireRetention = (retention) => retention === undefined
    ? undefined
    : {
...
const readTable = Effect.fn(function* (databaseName, tableName) {
    const response = yield* withWriteEndpoint(TSW.describeTable({ DatabaseName: databaseName, TableName: tableName })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    if (!response?.Table) {
        return undefined;
    }
    const table = response.Table;
    const tagsResponse = yield* withWriteEndpoint(TSW.listTagsForResource({ ResourceARN: table.Arn })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    if (!tagsResponse) {
        return undefined;
    }
    return {
...
export const TableProvider = () => Provider.effect(Table, Effect.gen(function* () {
    return {
        ...
        list: () => Effect.gen(function* () {
            const refs = yield* withWriteEndpoint(TSW.listTables.pages({}).pipe(EffectStream.runCollect)).pipe(Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Tables ?? []).flatMap((table) => table.DatabaseName && table.TableName
                ? [
                    {
                        databaseName: table.DatabaseName,
                        tableName: table.TableName,
                ...
                : []))));
            const hydrated = yield* Effect.forEach(refs, (ref) => readTable(ref.databaseName, ref.tableName), { concurrency: 10 });
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const tableArn = `arn:aws:timestream:${region}:${accountId}:database/${databaseName}/table/${tableName}`;
            ...
            if (state === undefined) {
                yield* withWriteEndpoint(TSW.createTable({
            ...
            // entirely on a no-op.
            const retentionDrifted = desiredRetention !== undefined &&
            ...
            if (retentionDrifted ||
                ...
                yield* withWriteEndpoint(TSW.updateTable({
                    DatabaseName: databaseName,
                    TableName: tableName,
                    RetentionProperties: desiredRetention,
                    MagneticStoreWriteProperties: news.magneticStoreWriteProperties,
            ...
            if (removed.length > 0) {
                yield* withWriteEndpoint(TSW.untagResource({ ResourceARN: tableArn, TagKeys: removed }));
            }
            if (upsert.length > 0) {
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* withWriteEndpoint(TSW.deleteTable({
