 ...
 * @resource
 */
export const FileSystem = Resource("AWS.FSx.FileSystem");
const fsxTagsToRecord = (tags) => Object.fromEntries((tags ?? []).map((t) => [t.Key, t.Value]));
const deleteConfigFor = (type) => {
    switch (type) {
        case "LUSTRE":
...
export const FileSystemProvider = () => Provider.effect(FileSystem, Effect.gen(function* () {
    ...
    const listAll = fsx.describeFileSystems.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.FileSystems ?? [])));
    ...
    return FileSystem.Provider.of({
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return undefined;
            const replaced = news.fileSystemType !== olds.fileSystemType ||
