...
const bucketNameOf = (bucket) => typeof bucket === "string" ? bucket : bucket.bucketName;
/**
 ...
 * @resource
 */
export const LayerVersion = Resource("AWS.Lambda.LayerVersion");
/**
...
export const LayerVersionProvider = () => Provider.effect(LayerVersion, Effect.gen(function* () {
    const createLayerName = (id, layerName) => layerName
    ...
    const readDirectoryEntries = Effect.fn(function* (directory) {
        ...
        const entries = yield* Effect.forEach(names, Effect.fn(function* (name) {
     ...
     */
    const packageContent = Effect.fn(function* (props) {
        const config = {
        ...
        if (props.s3 && props.path) {
            return yield* Effect.die("AWS.Lambda.LayerVersion accepts either `path` or `s3`, not both.");
        }
        if (props.s3) {
        ...
        if (info.type !== "Directory" && path.extname(resolved) !== ".zip") {
            return yield* Effect.die(`AWS.Lambda.LayerVersion \`path\` must be a directory or a .zip archive: ${resolved}`);
        }
        const archive = info.type === "Directory"
        ...
        if (archive.byteLength > MAX_INLINE_ARCHIVE_SIZE) {
            return yield* Effect.die(`AWS.Lambda.LayerVersion archive is ${archive.byteLength} bytes, above Lambda's ${MAX_INLINE_ARCHIVE_SIZE} byte direct-upload limit. Upload it to S3 and use \`s3\` instead.`);
        }
        return {
            content: {
    ...
    const snapshot = (layerName, sourceHash, 
    ...
    version) => {
        ...
        const layerArn = ("LayerArn" in version ? version.LayerArn : undefined) ??
    ...
    const getVersion = (layerName, version) => Lambda.getLayerVersion({
        LayerName: layerName,
    ...
    return {
        ...
        diff: Effect.fn(function* ({ olds, news, output }) {
            ...
            if (sourceHash !== output.sourceHash ||
        ...
        list: () => Effect.gen(function* () {
            ...
            // account/region.
            const layerNames = yield* Lambda.listLayers.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
                .map((layer) => layer.LayerName)
                .filter((name) => name !== undefined)));
            const versions = yield* Effect.forEach(layerNames, (layerName) => Lambda.listLayerVersions.items({ LayerName: layerName }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((version) => {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!attrs) {
                return yield* Effect.die(`Lambda layer ${layerName} did not return complete attributes.`);
            }
