...
import { deepEqual, havePropsChanged, isResolved } from "../../Diff.js";
...
import * as Serverless from "../../Serverless/index.js";
import { buildEventTelemetry } from "../../Telemetry.js";
import { Stack } from "../../Stack.js";
import { Stage } from "../../Stage.js";
import { createInternalTags, createTagsList, hasAlchemyTags, hasTags, } from "../../Tags.js";
import { sha256 } from "../../Util/sha256.js";
import { zipCode } from "../../Util/zip.js";
import { Assets } from "../Assets.js";
import { AWSEnvironment } from "../Environment.js";
import * as IAM from "../IAM/index.js";
import { syncEventInvokeConfig, } from "./EventInvokeConfig.js";
import { makeFunctionBundler } from "./FunctionBundle.js";
import { makeFunctionHttpHandler } from "./HttpServer.js";
export const FunctionTypeId = "AWS.Lambda.Function";
export class HandlerContext extends Context.Service()("AWS.Lambda.HandlerContext") {
}
 ...
 */
export const isBindingHost = (value) => {
    return (typeof value === "object" &&
        value !== null &&
        "Type" in value &&
        (value.Type === "AWS.Lambda.Function" ||
...
export const layerVersionArnOf = (layer) => typeof layer === "string" ? layer : layer.layerVersionArn;
/**
 * Resolve an {@link AccessPointRef} (or the legacy raw-`arn` field) on a
 * `fileSystemConfigs` entry to the access point ARN.
 */
const accessPointArnOf = (config) => typeof config.accessPoint === "string"
    ? config.accessPoint
    : typeof config.accessPoint
        ?.accessPointArn === "string"
        ? config.accessPoint.accessPointArn
        : config.arn;
/**
...
const handlerStringOf = (props) => {
    ...
    const base = props.bundle === false && typeof props.main === "string"
        ? props.main
 ...
 */
export const toTimeoutSeconds = (timeout) => {
    if (timeout === undefined)
        return undefined;
    const json = timeout;
    const input = json._id === "Duration"
        ? json._tag === "Millis"
            ? json.millis
            : json._tag === "Nanos"
                ? BigInt(json.nanos)
...
export const normalizeFunctionUrl = (url = true) => {
    if (url === false) {
        return undefined;
    }
 ...
 * @resource
 */
export const Function = Platform(FunctionTypeId, {
        ...
        const ctx = {
            ...
            exports: Effect.sync(() => ({
                ...
                handler: Effect.gen(function* () {
                    ...
                    const services = Context.omit(Layer.CurrentMemoMap)(yield* Effect.context());
                    return async (event, context) => {
                        for (const handler of handlers) {
                            ...
                            if (Effect.isEffect(eff)) {
                                ...
                                const exit = await eff.pipe(Effect.provide(Layer.mergeAll(Layer.succeed(HandlerContext, context), Layer.succeed(Scope.Scope, scope), 
                                ...
                                // below.
                                Layer.effectContext(buildEventTelemetry(services, scope, ctx.telemetry))).pipe(Layer.provideMerge(Layer.succeedContext(services)))), Effect.tap(Effect.logDebug), Effect.runPromiseExit);
...
export const FunctionProvider = () => Provider.effect(Function, Effect.gen(function* () {
    ...
    const createFunctionName = (id, functionName) => Effect.gen(function* () {
    ...
    const hashBundle = (code) => sha256(code);
    const createNames = (id, functionName) => Effect.gen(function* () {
    ...
    const attachBindings = Effect.fn(function* ({ roleName, policyName, 
    // functionArn,
    // functionName,
    bindings, }) {
        ...
        const policyStatements = activeBindings.flatMap((binding) => binding?.data?.policyStatements?.map((stmt) => ({
            ...stmt,
        ...
        // `reconcile`.
        const vpcRequests = activeBindings.flatMap((binding) => binding?.data?.vpc ? [binding.data.vpc] : []);
        const vpc = vpcRequests.length > 0
            ? {
                subnetIds: [
        ...
        const fileSystemConfigs = [
            ...new Map(activeBindings
                .flatMap((binding) => binding?.data?.fileSystemConfigs ?? [])
                .map((config) => [config.localMountPath, config])).values(),
        ];
     ...
     */
    const syncTracingPolicy = Effect.fn(function* ({ roleName, tracing, mayHaveBeenActive, }) {
        if (tracing === "Active") {
    ...
    const createRoleIfNotExists = Effect.fn(function* ({ id, roleName, vpc, }) {
    ...
    const getReservedConcurrentExecutions = Effect.fn(function* (functionName) {
        return yield* Lambda.getFunctionConcurrency({
            FunctionName: functionName,
        }).pipe(Effect.map((config) => config.ReservedConcurrentExecutions), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    });
    const syncReservedConcurrentExecutions = Effect.fn(function* ({ functionName, reservedConcurrentExecutions, }) {
        ...
        if (reservedConcurrentExecutions === undefined) {
            yield* Lambda.deleteFunctionConcurrency({
                FunctionName: functionName,
            }).pipe(retryFunctionMutation, Effect.catchTag("ResourceNotFoundException", () => Effect.void));
            return undefined;
        }
    ...
    const createOrUpdateFunction = Effect.fn(function* ({ id, news, roleArn, archive, hash, env, functionName, preferUpdate, fileSystemConfigs, vpc, session, }) {
        yield* Effect.logDebug(`creating function ${id}`);
        const waitStartedAt = Date.now();
        const isRolePropagationError = (e) => e._tag === "InvalidParameterValueException" &&
        ...
        const noteRolePropagationWait = () => session.note(`Waiting for Lambda execution role to become assumable: ${functionName} (${Math.ceil((Date.now() - waitStartedAt) / 1000)}s)`);
        const tags = yield* createInternalTags(id);
        ...
        Effect.flatMap((f) => f.Tags !== undefined
            ? Effect.succeed(hasTags(tags, f.Tags))
            : Lambda.listTags({
                Resource: f.Configuration?.FunctionArn ?? "",
            }).pipe(Effect.map((r) => hasTags(tags, r.Tags ?? {})), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(false)))), Effect.filterOrFail((owned) => owned, () => 
        // TODO(sam): add custom
        new Error("Function tags do not match expected values")), Effect.flatMap(() => Effect.gen(function* () {
            ...
            }).pipe(Effect.tapError((e) => isRolePropagationError(e)
        ...
        const create = Lambda.createFunction(createFunctionRequest).pipe(Effect.tapError((e) => Effect.gen(function* () {
            yield* Effect.logDebug(e);
        })), Effect.retry({
            while: (e) => isRolePropagationError(e),
            schedule: Schedule.fixed(1000).pipe(Schedule.tap(() => noteRolePropagationWait())),
        }), Effect.catchTags({
            ResourceConflictException: () => getAndUpdate,
        }));
        if (preferUpdate) {
            yield* getAndUpdate.pipe(Effect.catchTags({
    ...
    const removePublicFunctionUrlPermissions = Effect.fn(function* (functionName) {
        yield* Effect.all([
    ...
    const upsertPermission = (permission) => Lambda.addPermission(permission).pipe(Effect.catchTag("ResourceConflictException", () => Effect.gen(function* () {
    ...
    const upsertPublicFunctionUrlPermissions = Effect.fn(function* (functionName) {
        yield* Effect.all([
            upsertPermission({
                FunctionName: functionName,
                StatementId: publicUrlAccessStatementId,
    ...
    const createOrUpdateFunctionUrl = Effect.fn(function* ({ functionName, url, oldUrl, currentFunctionUrl, }) {
        ...
        if (desired) {
            ...
            const { FunctionUrl } = yield* Lambda.createFunctionUrlConfig(config).pipe(Effect.catchTag("ResourceConflictException", () => Lambda.updateFunctionUrlConfig(config)), retryFunctionMutation);
    ...
    const summary = ({ archive }) => `${archive.length >= 1024 * 1024
    ...
    return {
        ...
        diff: Effect.fn(function* ({ id, olds, news, output }) {
            ...
            if (!deepEqual(normalizeFunctionUrl(olds.functionUrl), normalizeFunctionUrl(news.functionUrl))) {
            ...
            if ((olds.architecture ?? "x86_64") !== (news.architecture ?? "x86_64")) {
                return { action: "update" };
            }
            if (olds.reservedConcurrentExecutions !==
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            const fn = yield* Lambda.getFunction({
                FunctionName: functionName,
            }).pipe(Effect.map((r) => r.Configuration), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
            if (!fn?.FunctionArn || !fn.FunctionName || !fn.Role) {
                return undefined;
            }
            const tagsResult = yield* Lambda.listTags({
                Resource: fn.FunctionArn,
            }).pipe(Effect.map((r) => r.Tags ?? {}), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})));
            const functionUrl = yield* Lambda.getFunctionUrlConfig({
                FunctionName: fn.FunctionName,
            }).pipe(Effect.map((f) => f.FunctionUrl), Effect.retry({
                while: (e) => e._tag === "ResourceConflictException",
                schedule: Schedule.exponential(100),
            }), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
            const reservedConcurrentExecutions = yield* getReservedConcurrentExecutions(fn.FunctionName);
        ...
        list: () => Effect.gen(function* () {
            const configs = yield* Lambda.listFunctions.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
            const rows = yield* Effect.forEach(configs, (fn) => Effect.gen(function* () {
                if (!fn.FunctionArn || !fn.FunctionName || !fn.Role) {
                    return undefined;
                }
                const functionUrl = yield* Lambda.getFunctionUrlConfig({
                    FunctionName: fn.FunctionName,
                }).pipe(Effect.map((f) => f.FunctionUrl), 
        ...
        precreate: Effect.fn(function* ({ id, news, session }) {
            ...
            const code = new TextEncoder().encode(`export default () => ({ statusCode: 503, headers: { "content-type": "application/json" }, body: JSON.stringify({ error: "function initializing" }) })`);
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, bindings, output, session, }) {
            ...
            const { env, vpc: bindingVpc, fileSystemConfigs: bindingFileSystemConfigs, } = yield* attachBindings({
                roleName,
                policyName,
                functionArn,
                functionName,
            ...
            yield* syncTracingPolicy({
                ...
                mayHaveBeenActive: olds?.tracing === "Active" ||
            ...
            const desiredFileSystemConfigs = [
                ...new Map([
                    ...(news.fileSystemConfigs ?? []).map((c) => {
                        ...
                        if (arn === undefined) {
                            throw new Error(`Function(${id}): fileSystemConfigs entry for ${c.localMountPath} needs an access point — set \`accessPoint\` (an AWS.EFS.AccessPoint resource or its ARN) or \`arn\``);
                        }
            ...
            yield* createOrUpdateFunction({
                ...
                fileSystemConfigs: desiredFileSystemConfigs.length > 0
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* iam
                ...
                .pipe(Effect.flatMap((policies) => Effect.all((policies.PolicyNames ?? []).map((policyName) => iam
                .deleteRolePolicy({
                RoleName: output.roleName,
                PolicyName: policyName,
            })
            ...
            yield* iam
                ...
                .pipe(Effect.flatMap((policies) => Effect.all((policies.AttachedPolicies ?? []).map((policy) => iam
                .detachRolePolicy({
                RoleName: output.roleName,
            ...
            yield* iam.deleteRole({ RoleName: output.roleName }).pipe(Effect.catchTag("NoSuchEntityException", () => Effect.void), Effect.retry({
                while: (error) => error._tag === "DeleteConflictException" ||
            ...
            const reapLogGroup = logs.deleteLogGroup({ logGroupName }).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void), 
            ...
            const lastIngestion = yield* logs
                ...
                .pipe(Effect.map((r) => r.logStreams?.[0]?.lastIngestionTime), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)), Effect.timeoutOrElse({
                duration: "5 seconds",
                orElse: () => Effect.gen(function* () {
                    yield* Effect.logWarning(`Timed out inspecting Lambda log group ${logGroupName}`);
                    return undefined;
                }),
            }));
            ...
            const observeLogGroup = describeLogGroup.pipe(Effect.timeoutOrElse({
                duration: "30 seconds",
                orElse: () => Effect.logWarning(`Timed out observing Lambda log group ${logGroupName} — assuming still present`).pipe(Effect.as(true)),
            }));
            // Final authoritative observation: here a timeout must fail loudly,
            // since we are about to declare the delete converged.
            const observeLogGroupOrDie = describeLogGroup.pipe(Effect.timeoutOrElse({
                duration: "30 seconds",
                orElse: () => Effect.die(new Error(`Timed out confirming Lambda log group deletion: ${logGroupName}`)),
            }));
            ...
            }), Effect.catchTag("ResourceNotFoundException", () => Effect.void), Effect.timeoutOrElse({
                duration: "45 seconds",
                orElse: () => Effect.die(new Error(`Timed out deleting Lambda log group ${logGroupName}`)),
            }));
            ...
            const observedAtBudgetEnd = yield* reapIfObserved.pipe(Effect.repeat({
            ...
            if (observedAtBudgetEnd && (yield* observeLogGroupOrDie)) {
                yield* Effect.die(new Error(`Lambda log group ${logGroupName} remained observable after delete`));
            }
            return null;
        }),
        tail: ({ output }) => {
            const runTailSession = Effect.gen(function* () {
                ...
                return response.responseStream.pipe(Stream.flatMap((event) => {
                    if ("sessionUpdate" in event && event.sessionUpdate) {
                        const lines = (event.sessionUpdate.sessionResults ?? []).flatMap((result) => {
                            if (!result.message)
                                return [];
                            return [
                                {
        ...
        logs: ({ output, options, }) => logs
            .filterLogEvents({
            logGroupName: `/aws/lambda/${output.functionName}`,
            startTime: options.since?.getTime(),
            limit: options.limit ?? 100,
        })
            .pipe(Effect.map((response) => (response.events ?? []).flatMap((event) => {
            if (!event.message)
                return [];
            return [
                {
