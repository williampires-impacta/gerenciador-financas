...
import { DeploymentEventSource as AppConfigDeploymentEventSourceContract, } from "../AppConfig/DeploymentEventSource.js";
import { Extension } from "../AppConfig/Extension.js";
import { ExtensionAssociation } from "../AppConfig/ExtensionAssociation.js";
import { Role } from "../IAM/Role.js";
import * as Lambda from "./Function.js";
const ALL_DEPLOYMENT_EVENTS = [
    "ON_DEPLOYMENT_START",
    "ON_DEPLOYMENT_STEP",
...
const eventSetSuffix = (events) => [...events]
    .sort()
    .map((event) => event
 ...
 */
export const isAppConfigDeploymentEvent = (event) => typeof event?.InvocationId === "string" &&
    typeof event?.Type === "string" &&
    typeof event?.Application === "object";
const isEnvironment = (target) => target.Type === "AWS.AppConfig.Environment";
/**
 ...
 * @binding
 */
export const AppConfigDeploymentEventSource = Layer.effect(AppConfigDeploymentEventSourceContract, 
...
Effect.gen(function* () {
    ...
    return Effect.fn(function* (target, props, process) {
                            ...
                            Statement: [
                                {
                                    ...
                                    Resource: [host.functionArn],
                                },
                            ],
                        },
                    },
                });
                const extension = yield* DeploymentExtension(`${target.LogicalId}-${suffix}-DeploymentEvents`, {
                    description: `Deployment notifications for ${target.LogicalId} -> ${host.LogicalId}`,
                    actions: Object.fromEntries(events.map((event) => [
                        event,
                        [
                            {
                                name: event,
        ...
        yield* host.listen(Effect.gen(function* () {
            ...
            event
                ...
                .join("")));
            return (event) => {
                if (isAppConfigDeploymentEvent(event) &&
...
//# sourceMappingURL=AppConfigDeploymentEventSource.js.map