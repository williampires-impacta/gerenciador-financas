...
import { UserPoolTriggerEventSource as CognitoUserPoolTriggerEventSource, userPoolTriggerSourcePrefixes, } from "../Cognito/UserPoolTriggerEventSource.js";
 ...
 */
export const isUserPoolTriggerEvent = (event) => typeof event?.version === "string" &&
 ...
 * @binding
 */
export const UserPoolTriggerEventSource = Layer.effect(CognitoUserPoolTriggerEventSource, Effect.gen(function* () {
    // this layer can only be used in a Lambda Function
    const host = yield* Lambda.Function;
    const Permission = yield* LambdaPermission;
    return Effect.fn(function* (userPool, props, handler) {
        ...
        yield* host.listen(Effect.gen(function* () {
            ...
            return (event) => {
                if (isUserPoolTriggerEvent(event) &&
                    ...
                    // as-is.
                    return handler(event).pipe(Effect.map((result) => result ?? event), Effect.orDie);
                }
            };
        }));
    });
