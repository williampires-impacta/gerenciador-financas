...
import { Assets, AssetsLive } from "../Assets.js";
import { flociSidecarEntry, makeDevWatchProvider, } from "../Local/DevWatchProvider.js";
import { Function, FunctionProvider, layerVersionArnOf, } from "./Function.js";
import { makeFunctionBundler, } from "./FunctionBundle.js";
export const FlociFunctionProvider = () => makeDevWatchProvider(Function, flociSidecarEntry(), {
    ...
    replaceOn: ({ olds, news, output }) => Effect.sync(() => {
    ...
    startWatch: (ctx) => Effect.gen(function* () {
        ...
        const swap = Effect.fn(function* (result) {
            if (result.identityHash === lastHash)
                return;
            ...
            yield* Effect.logInfo(`[alchemy dev] ${functionName}: code swapped in ${Date.now() - startedAt}ms`);
        });
        if (props.bundle === false) {
            ...
            yield* fs.watch(dir, { recursive: true }).pipe(Stream.debounce("200 millis"), Stream.runForEach(() => Effect.gen(function* () {
                startedAt = Date.now();
                const result = yield* bundler.prebuiltCode(realMain);
                yield* swap(result);
            }).pipe(Effect.catchCause((cause) => Effect.logWarning(`[alchemy dev] ${functionName}: code swap failed`, cause)))));
        }
        else {
            ...
            yield* Bundle.watch(plan.inputOptions, plan.outputOptions, plan.extra).pipe(Stream.runForEach((event) => Effect.gen(function* () {
                switch (event._tag) {
                    case "Start": {
                        startedAt = Date.now();
                        return;
                    ...
                    case "Error": {
                        yield* Effect.logWarning(`[alchemy dev] ${functionName}: rebuild failed: ${event.error.message}`);
                        return;
                    }
                    case "Success": {
...
//# sourceMappingURL=FlociFunctionProvider.js.map