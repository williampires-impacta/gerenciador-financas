 ...
 * @resource
 */
export const EventSourceMapping = Resource("AWS.Lambda.EventSourceMapping");
export const EventSourceMappingProvider = () => Provider.effect(EventSourceMapping, Effect.gen(function* () {
    const createEventSourceMappingTags = Effect.fn(function* (id) {
    ...
    const toCreateRequest = (props, tags) => ({
    ...
    const toUpdateRequest = (uuid, props) => ({
        UUID: uuid,
        FunctionName: props.functionName,
        Enabled: props.enabled ?? true,
        BatchSize: props.batchSize,
    ...
    return {
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return;
            if (news.eventSourceArn !== olds.eventSourceArn) {
                return { action: "replace" };
            }
            if (news.startingPosition !== olds.startingPosition) {
                return { action: "replace" };
            }
            if (news.startingPositionTimestamp?.getTime() !==
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const functionName = news.functionName;
            const eventSourceArn = news.eventSourceArn;
            ...
            if (!config?.UUID) {
                config = yield* lambda.listEventSourceMappings
                    ...
                    const mapping = page.EventSourceMappings?.find((m) => m.EventSourceArn === eventSourceArn);
                    if (mapping?.UUID) {
                ...
                })), Stream.filter((item) => item !== undefined), Stream.runHead, Effect.map(Option.getOrUndefined));
            }
            ...
            if (!config?.UUID) {
                config = yield* lambda
                    ...
                    ResourceConflictException: () => lambda.listEventSourceMappings
                        .pages({ FunctionName: functionName })
                        .pipe(Stream.mapEffect(Effect.fn(function* (page) {
                        const mapping = page.EventSourceMappings?.find((m) => m.EventSourceArn === eventSourceArn);
                        if (mapping?.UUID) {
                    ...
                    })), Stream.filter((item) => item !== undefined), Stream.runHead, Effect.map(Option.getOrUndefined), Effect.flatMap((mapping) => mapping
                        ? Effect.succeed(mapping)
                        : Effect.die(new Error(`EventSourceMapping(${id}) not found on function ${functionName}`)))),
                }), retryPermissionsPropagation, retryTransient);
            }
            if (!config?.UUID) {
                return yield* Effect.die(new Error(`EventSourceMapping(${id}) could not be reconciled`));
            }
            ...
            if (config.State !== desiredState) {
                config = yield* lambda.getEventSourceMapping({ UUID: uuid }).pipe(Effect.flatMap((observed) => observed.State === desiredState
                    ? Effect.succeed(observed)
                    : Effect.fail(new MappingNotSettled({ state: observed.State }))), Effect.retry({
                    while: (e) => e._tag === "MappingNotSettled",
                    schedule: Schedule.max([
                        Schedule.spaced("2 seconds"),
                        Schedule.recurs(45),
                    ]).pipe(Schedule.tap(({ attempt }) => session.note(`EventSourceMapping ${uuid}: waiting to become ${desiredState} (${(attempt + 1) * 2}s elapsed)`))),
                }), 
                // Never wedge a deploy on a mapping stuck in transition —
                // surface the observed state in the attributes instead.
                Effect.catchTag("MappingNotSettled", () => lambda.getEventSourceMapping({ UUID: uuid })));
            }
            ...
            const observedTagsResp = yield* lambda
                ...
                .pipe(retryTransient);
            const observedTags = Object.fromEntries(Object.entries(observedTagsResp.Tags ?? {}).filter((entry) => typeof entry[1] === "string"));
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* lambda.deleteEventSourceMapping({ UUID: output.uuid }).pipe(Effect.retry({
                while: (e) => e._tag === "ResourceInUseException" ||
                    e._tag === "ResourceConflictException",
                schedule: Schedule.max([
                    Schedule.exponential(100),
        ...
        list: () => lambda.listEventSourceMappings.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.EventSourceMappings ?? [])
            .filter((config) => config.UUID != null &&
...
const retryTransient = Effect.retry({
    while: (e) => e._tag === "InternalFailure" ||
...
const sanitizeAwsTagValue = (value) => value.replace(/[^\p{L}\p{Z}\p{N}_.:/=+\-@]/gu, "-");
