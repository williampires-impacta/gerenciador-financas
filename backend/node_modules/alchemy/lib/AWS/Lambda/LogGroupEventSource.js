...
import { LogGroupEventSource as LogsLogGroupEventSource, } from "../Logs/LogGroupEventSource.js";
import { SubscriptionFilter } from "../Logs/SubscriptionFilter.js";
 ...
 * subscription event.
 */
export const isCloudWatchLogsEvent = (event) => typeof event?.awslogs?.data === "string";
/**
 * Decode the gzipped, base64-encoded CloudWatch Logs subscription payload.
 */
export const decodeCloudWatchLogsEvent = (event) => Effect.try({
    try: () => JSON.parse(gunzipSync(Buffer.from(event.awslogs.data, "base64")).toString()),
    catch: (cause) => new Error("failed to decode CloudWatch Logs subscription payload", {
        cause,
    }),
});
/**
 ...
 * @binding
 */
export const LogGroupEventSource = Layer.effect(LogsLogGroupEventSource, 
...
Effect.gen(function* () {
    ...
    return Effect.fn(function* (logGroup, props, process) {
            ...
            return (event) => {
                if (isCloudWatchLogsEvent(event)) {
                    return Effect.gen(function* () {
                        ...
                        yield* process(Stream.fromArray(payload.logEvents.map((logEvent) => ({
                            id: logEvent.id,
