 ...
 * ```
 */
const FilterResource = Resource("AWS.GuardDuty.Filter");
export { FilterResource as Filter };
const filterArn = (region, accountId, detectorId, name) => `arn:aws:guardduty:${region}:${accountId}:detector/${detectorId}/filter/${name}`;
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            for (const detectorId of DetectorIds ?? []) {
                ...
                for (const name of names) {
                    const f = yield* getFilter(detectorId, name);
                    if (f)
                        out.push(yield* buildAttrs(detectorId, name, f));
                }
            }
            return out;
        }),
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!live) {
                ...
                const drift = live.Action !== desiredAction ||
