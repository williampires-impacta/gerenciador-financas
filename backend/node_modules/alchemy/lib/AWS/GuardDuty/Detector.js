 ...
 * ```
 */
const DetectorResource = Resource("AWS.GuardDuty.Detector");
export { DetectorResource as Detector };
export const detectorArn = (region, accountId, detectorId) => `arn:aws:guardduty:${region}:${accountId}:detector/${detectorId}`;
const buildAttrs = Effect.fn(function* (detectorId, d) {
...
export const DetectorProvider = () => Provider.effect(DetectorResource, Effect.gen(function* () {
    ...
    const getDetector = (detectorId) => guardduty
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            for (const detectorId of DetectorIds ?? []) {
                const d = yield* getDetector(detectorId);
                if (d)
                    out.push(yield* buildAttrs(detectorId, d));
            }
            return out;
        }),
        reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
            ...
            if (!detectorId || !live) {
                ...
                const settingsChanged = observedEnabled !== desiredEnable ||
