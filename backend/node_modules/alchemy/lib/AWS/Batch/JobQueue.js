 ...
 * @resource
 */
export const JobQueue = Resource("AWS.Batch.JobQueue");
const toAttributes = (q, tags) => ({
    jobQueueName: q.jobQueueName,
    jobQueueArn: q.jobQueueArn,
    state: (q.state ?? "ENABLED"),
...
const observedTagsOf = (q) => Object.fromEntries(Object.entries(q.tags ?? {}).filter((e) => typeof e[1] === "string"));
const toOrder = (computeEnvironments) => computeEnvironments.map((computeEnvironment, index) => ({
    order: index + 1,
    computeEnvironment,
}));
export const JobQueueProvider = () => Provider.effect(JobQueue, Effect.gen(function* () {
    const toName = (id, props = {}) => props.jobQueueName
    ...
    const awaitSettled = (name) => pollBatch(describeOne(name), (q) => q === undefined ||
        (q.status !== "CREATING" &&
            q.status !== "UPDATING" &&
            q.status !== "DELETING"));
    const awaitDisabled = (name) => pollBatch(describeOne(name), (queue) => queue === undefined ||
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            return Array.from(pages)
                ...
                .flatMap((q) => q.jobQueueArn && q.status !== "DELETED"
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const arn = `arn:aws:batch:${region}:${accountId}:job-queue/${name}`;
            ...
            const create = retryBatch(batch.createJobQueue({
                jobQueueName: name,
                state: desiredState,
                priority: desiredPriority,
                computeEnvironmentOrder: desiredOrder,
                tags: desiredTags,
            }), (e) => e._tag === "ComputeEnvironmentNotValid").pipe(Effect.catchTag("JobQueueAlreadyExists", () => Effect.void));
            ...
            if (dirty) {
                yield* retryBatch(batch.updateJobQueue(update), (e) => e._tag === "JobQueueBeingModified");
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if ((existing.state ?? "ENABLED") !== "DISABLED" &&
                ...
                yield* retryBatch(batch.updateJobQueue({ jobQueue: name, state: "DISABLED" }), (e) => e._tag === "JobQueueBeingModified").pipe(Effect.catchTag("JobQueueNotFound", () => Effect.void));
            }
            ...
            if (disabled.status !== "DELETING") {
                yield* retryBatch(batch.deleteJobQueue({ jobQueue: name }), (e) => e._tag === "JobQueueBeingModified").pipe(Effect.catchTag("JobQueueNotFound", () => Effect.void));
            }
            ...
            const final = yield* pollBatch(describeOne(name), (q) => q === undefined || q.status === "DELETED");
            if (final && final.status !== "DELETED") {
                return yield* Effect.fail(new JobQueueDeleteTimeoutError({
                    jobQueueName: name,
                    status: final.status,
