 ...
 */
export declare const makeBatchJobHttpBinding: <I, A, E, R>(options: {
...
}) => Effect.Effect<(queue: JobQueue) => Effect.Effect<(request: I) => Effect.Effect<A, E, never>, never, never>, never, R>;
/**
 ...
 * resource-level IAM).
 */
export declare const makeBatchQueueHttpBinding: <I extends {
    jobQueue?: string;
}, A, E, R>(options: {
    ...
    wildcardIam?: boolean;
}) => Effect.Effect<(queue: JobQueue) => Effect.Effect<(request?: Omit<I, "jobQueue"> | undefined) => Effect.Effect<A, E, never>, never, never>, never, R>;
