...
import { pollBatch, retryBatch } from "./internal.js";
/**
 ...
 */
export class ComputeEnvironmentInvalidError extends Data.TaggedError("ComputeEnvironmentInvalidError") {
}
/**
 ...
 * @resource
 */
export const ComputeEnvironment = Resource("AWS.Batch.ComputeEnvironment");
const toAttributes = (ce, tags) => ({
    ...
    managementType: (ce.type ?? "MANAGED"),
    type: (ce.computeResources?.type ?? "FARGATE"),
    state: (ce.state ?? "ENABLED"),
...
const observedTagsOf = (ce) => Object.fromEntries(Object.entries(ce.tags ?? {}).filter((e) => typeof e[1] === "string"));
/**
 ...
 */
const isAuthPropagationInvalid = (ce) => ce?.status === "INVALID" &&
...
const resolveDefaultNetwork = Effect.gen(function* () {
    ...
    const vpcId = vpcs.Vpcs?.find((vpc) => vpc.State === undefined || vpc.State === "available")?.VpcId;
    if (!vpcId) {
        return yield* Effect.fail(new NoDefaultVpcError({
    ...
    const network = {
        subnets: subnets.flatMap((s) => s.SubnetId && (s.State === undefined || s.State === "available")
            ? [s.SubnetId]
            : []),
    ...
    if (network.subnets.length === 0 || network.securityGroupIds.length === 0) {
        return yield* Effect.fail(new NoDefaultVpcError({
            message: "Default VPC networking is not ready — pass `subnets` and `securityGroupIds` explicitly",
        }));
    }
    return network;
}).pipe(Effect.retry({
...
export const ComputeEnvironmentProvider = () => Provider.effect(ComputeEnvironment, Effect.gen(function* () {
    const toName = (id, props = {}) => props.computeEnvironmentName
    ...
    const describeOne = (name) => batch
        .describeComputeEnvironments({ computeEnvironments: [name] })
        .pipe(Effect.map((res) => res.computeEnvironments?.find((ce) => ce.status !== "DELETED") ?? res.computeEnvironments?.[0]));
    /** Wait until any CREATING/UPDATING/DELETING transition settles. */
    const awaitSettled = (name) => pollBatch(describeOne(name), (ce) => ce === undefined ||
        (ce.status !== "CREATING" &&
            ce.status !== "UPDATING" &&
            ce.status !== "DELETING"));
    const awaitDisabled = (name) => pollBatch(describeOne(name), (ce) => ce === undefined ||
        ce.status === "DELETED" ||
        ce.status === "DELETING" ||
        (ce.state === "DISABLED" &&
            ce.status !== "CREATING" &&
            ce.status !== "UPDATING"));
    const invalid = (name, ce) => {
     ...
     */
    const restoreServiceRoleIfMissing = Effect.fn(function* (serviceRoleArn) {
        if (!serviceRoleArn || serviceRoleArn.includes("/aws-service-role/")) {
            return undefined;
        }
        const roleName = serviceRoleArn.split("/").pop();
        const exists = yield* iam.getRole({ RoleName: roleName }).pipe(Effect.map(() => true), Effect.catchTag("NoSuchEntityException", () => Effect.succeed(false)));
        if (exists)
            return undefined;
        yield* iam
            .createRole({
            ...
            Description: "Temporarily restored by alchemy so AWS Batch can tear down a compute environment whose service role was deleted",
        })
            .pipe(
    ...
    const dropRestoredServiceRole = Effect.fn(function* (roleName) {
        yield* iam
            .detachRolePolicy({
            RoleName: roleName,
            PolicyArn: "arn:aws:iam::aws:policy/service-role/AWSBatchServiceRole",
     ...
     */
    const deleteRelatedJobQueues = (computeEnvironment) => Effect.gen(function* () {
        const pages = yield* batch.describeJobQueues
            .pages({})
            .pipe(Stream.runCollect);
        const queues = Array.from(pages)
            .flatMap((page) => page.jobQueues ?? [])
            .filter((queue) => queue.jobQueueName &&
            queue.status !== "DELETED" &&
            queue.computeEnvironmentOrder?.some((entry) => entry.computeEnvironment === computeEnvironment ||
                entry.computeEnvironment?.endsWith(`:compute-environment/${computeEnvironment}`)));
        yield* Effect.forEach(queues, (queue) => Effect.gen(function* () {
            const queueName = queue.jobQueueName;
            const describeQueue = batch
                .describeJobQueues({ jobQueues: [queueName] })
                .pipe(Effect.map((response) => response.jobQueues?.find((candidate) => candidate.status !== "DELETED")));
            let observed = yield* describeQueue;
            if (!observed)
                return;
            if (observed.status !== "DELETING" &&
                (observed.state ?? "ENABLED") !== "DISABLED") {
                yield* retryBatch(batch.updateJobQueue({
                    jobQueue: queueName,
                    state: "DISABLED",
                }), (error) => error._tag === "JobQueueBeingModified").pipe(Effect.catchTag("JobQueueNotFound", () => Effect.void));
            }
            observed = yield* pollBatch(describeQueue, (candidate) => candidate === undefined ||
            ...
            if (!observed || observed.status === "DELETED")
                return;
            if (observed.status !== "DELETING") {
                yield* retryBatch(batch.deleteJobQueue({ jobQueue: queueName }), (error) => error._tag === "JobQueueBeingModified").pipe(Effect.catchTag("JobQueueNotFound", () => Effect.void));
            }
            yield* pollBatch(describeQueue, (candidate) => candidate === undefined || candidate.status === "DELETED");
    ...
    return {
        ...
        diff: Effect.fn(function* ({ id, olds, news }) {
            ...
            if ((olds?.managementType ?? "MANAGED") !==
        ...
        list: () => Effect.gen(function* () {
            ...
            return Array.from(pages)
                ...
                .flatMap((ce) => ce.computeEnvironmentArn && ce.status !== "DELETED"
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
            ...
            const arn = `arn:aws:batch:${region}:${accountId}:compute-environment/${name}`;
            ...
            const fallbackNetwork = desiredManagementType === "MANAGED" &&
                (!news.subnets || !news.securityGroupIds)
                ? yield* resolveDefaultNetwork
                : undefined;
            let desiredNetwork = desiredManagementType === "MANAGED"
                ? {
            ...
            const matchesDesired = (candidate) => (candidate.type ?? "MANAGED") === desiredManagementType &&
                ...
                (desiredManagementType === "UNMANAGED"
                    ...
                    : (candidate.computeResources?.maxvCpus ?? 0) ===
                        ...
                        sameSet(candidate.computeResources?.securityGroupIds ?? [], desiredNetwork.securityGroupIds));
            /**
            ...
            const awaitValid = Effect.fn(function* () {
                ...
                for (let attempt = 0; attempt < 8; attempt++) {
                    ...
                    if (desiredManagementType === "MANAGED" &&
                        (!news.subnets || !news.securityGroupIds)) {
                    ...
                    yield* retryBatch(batch.updateComputeEnvironment({
                        computeEnvironment: name,
                        state: desiredState,
                        unmanagedvCpus: desiredManagementType === "UNMANAGED"
                            ? desiredUnmanagedvCpus
                            : undefined,
                        computeResources: desiredManagementType === "MANAGED"
                            ? {
            ...
            const reapInvalidCreate = Effect.fn(function* () {
                yield* retryBatch(batch.updateComputeEnvironment({
                    computeEnvironment: name,
                    state: "DISABLED",
                }), (e) => e._tag === "ComputeEnvironmentBeingModified").pipe(Effect.catchTag("ComputeEnvironmentNotFound", () => Effect.void));
                const disabled = yield* awaitDisabled(name);
                if (!disabled || disabled.status === "DELETED")
                    return;
                const requestDelete = retryBatch(batch.deleteComputeEnvironment({ computeEnvironment: name }), (e) => e._tag === "ComputeEnvironmentInUse" ||
                    e._tag === "ComputeEnvironmentBeingModified").pipe(Effect.catchTag("ComputeEnvironmentNotFound", () => Effect.void));
                ...
                const awaitGone = pollBatch(describeOne(name), (c) => c === undefined ||
                    c.status === "DELETED" ||
                    c.status === "INVALID");
            ...
            if (desiredManagementType === "UNMANAGED") {
                ...
                if (!sameSet(ce.computeResources?.securityGroupIds ?? [], desiredNetwork.securityGroupIds)) {
            ...
            if (dirty) {
                yield* retryBatch(batch.updateComputeEnvironment(update), (e) => e._tag === "ComputeEnvironmentBeingModified");
                ce = yield* awaitValid();
            }
            ...
            return toAttributes({ ...ce, computeEnvironmentName: name, computeEnvironmentArn: arn }, desiredTags);
        }),
        delete: Effect.fn(function* ({ output, force }) {
            ...
            const requestDelete = retryBatch(batch.deleteComputeEnvironment({ computeEnvironment: name }), (e) => e._tag === "ComputeEnvironmentInUse" ||
                e._tag === "ComputeEnvironmentBeingModified", 24).pipe(Effect.catchTag("ComputeEnvironmentNotFound", () => Effect.void));
            if (disabled.status !== "DELETING") {
                yield* requestDelete;
            ...
            const awaitGone = pollBatch(describeOne(name), (ce) => ce === undefined ||
                ce.status === "DELETED" ||
                ce.status === "INVALID");
            let final = yield* awaitGone;
            if (final && final.status === "INVALID") {
