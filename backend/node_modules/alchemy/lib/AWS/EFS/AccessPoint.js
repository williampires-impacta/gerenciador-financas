 ...
 * @resource
 */
export const AccessPoint = Resource("AWS.EFS.AccessPoint");
/**
 ...
 * (see PATTERNS §7).
 */
const retryUntilAccessPointAvailable = (self) => Effect.retry(Effect.flatMap(self, (ap) => ap.LifeCycleState === "available"
    ? Effect.succeed(ap)
    : Effect.fail(new AccessPointNotAvailable({
...
const efsTagsToRecord = (tags) => Object.fromEntries((tags ?? []).map((t) => [t.Key, t.Value]));
const toWirePosixUser = (posixUser) => posixUser === undefined
    ? undefined
    : {
        Uid: posixUser.uid,
        Gid: posixUser.gid,
...
const toWireRootDirectory = (rootDirectory) => rootDirectory === undefined
    ? undefined
    : {
...
export const AccessPointProvider = () => Provider.effect(AccessPoint, Effect.gen(function* () {
    ...
    const findById = Effect.fn(function* (accessPointId) {
        return yield* efs
            .describeAccessPoints({ AccessPointId: accessPointId })
            .pipe(Effect.map((r) => r.AccessPoints?.[0]), Effect.catchTag("AccessPointNotFound", () => Effect.succeed(undefined)));
    });
    const findByToken = Effect.fn(function* (fileSystemId, token) {
        return yield* efs
            .describeAccessPoints({ FileSystemId: fileSystemId })
            .pipe(Effect.map((r) => r.AccessPoints?.find((ap) => ap.ClientToken === token)), Effect.catchTag("FileSystemNotFound", () => Effect.succeed(undefined)));
    });
    return AccessPoint.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            const perSystem = yield* Effect.forEach(systems, (fs) => efs
                ...
                .pipe(Effect.map((r) => (r.AccessPoints ?? [])
                .filter((ap) => ap.AccessPointId !== undefined &&
                ap.LifeCycleState !== "deleted")
                .map((ap) => ({
                accessPointId: ap.AccessPointId,
                accessPointArn: ap.AccessPointArn,
                fileSystemId: ap.FileSystemId,
            }))), Effect.catchTag("FileSystemNotFound", () => Effect.succeed([]))), { concurrency: 5 });
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (ap === undefined) {
                ap = yield* efs
                    ...
                    Tags: Object.entries({ ...news.tags, ...internalTags }).map(([Key, Value]) => ({ Key, Value })),
                })
                    .pipe(Effect.catchTag("AccessPointAlreadyExists", (e) => efs
                    .describeAccessPoints({ AccessPointId: e.AccessPointId })
                    .pipe(Effect.map((r) => r.AccessPoints[0]))));
            }
            const accessPointId = ap.AccessPointId;
            ap = yield* retryUntilAccessPointAvailable(efs
                .describeAccessPoints({ AccessPointId: accessPointId })
                .pipe(Effect.map((r) => r.AccessPoints[0])));
