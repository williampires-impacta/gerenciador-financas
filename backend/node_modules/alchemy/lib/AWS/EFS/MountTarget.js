 ...
 * @resource
 */
export const MountTarget = Resource("AWS.EFS.MountTarget");
/**
 ...
 */
const retryUntilMountTargetAvailable = (self) => Effect.retry(Effect.flatMap(self, (mt) => mt.LifeCycleState === "available"
    ? Effect.succeed(mt)
    : Effect.fail(new MountTargetNotAvailable({
 ...
 */
const retryUntilMountTargetGone = (self) => Effect.retry(self, {
 ...
 */
const retryWhileMountTargetSettling = (self) => Effect.retry(self, {
    while: (e) => e._tag === "IncorrectMountTargetState" || e._tag === "DependencyTimeout",
    schedule: Schedule.max([Schedule.fixed("3 seconds"), Schedule.recurs(20)]),
});
export const MountTargetProvider = () => Provider.effect(MountTarget, Effect.gen(function* () {
    const findById = Effect.fn(function* (mountTargetId) {
        return yield* efs
            .describeMountTargets({ MountTargetId: mountTargetId })
            .pipe(Effect.map((r) => r.MountTargets?.[0]), Effect.catchTag("MountTargetNotFound", () => Effect.succeed(undefined)));
    });
    const findBySubnet = Effect.fn(function* (fileSystemId, subnetId) {
        return yield* efs
            .describeMountTargets({ FileSystemId: fileSystemId })
            .pipe(Effect.map((r) => r.MountTargets?.find((mt) => mt.SubnetId === subnetId)), Effect.catchTag("FileSystemNotFound", () => Effect.succeed(undefined)));
    });
    const toAttributes = (mt) => ({
        mountTargetId: mt.MountTargetId,
    ...
    return MountTarget.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            const perSystem = yield* Effect.forEach(systems, (fs) => efs
                .describeMountTargets({ FileSystemId: fs.FileSystemId })
                .pipe(Effect.map((r) => (r.MountTargets ?? [])
                .filter((mt) => mt.LifeCycleState !== "deleted")
                .map(toAttributes)), Effect.catchTag("FileSystemNotFound", () => Effect.succeed([]))), { concurrency: 5 });
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news) || olds === undefined)
                return undefined;
            if (news.fileSystemId !== olds.fileSystemId ||
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            if (mt === undefined) {
                mt = yield* efs
                    ...
                    .pipe(Effect.catchTag("MountTargetConflict", () => findBySubnet(news.fileSystemId, news.subnetId).pipe(Effect.flatMap((existing) => existing !== undefined
            ...
            mt = yield* retryUntilMountTargetAvailable(efs
                .describeMountTargets({ MountTargetId: mountTargetId })
            ...
            if (news.securityGroups !== undefined) {
                ...
                if (JSON.stringify(observed) !== JSON.stringify(desired)) {
                    yield* retryWhileMountTargetSettling(efs.modifyMountTargetSecurityGroups({
                        MountTargetId: mountTargetId,
                        SecurityGroups: news.securityGroups,
                    }));
                }
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            yield* retryUntilMountTargetGone(efs
                ...
                return state === undefined || state === "deleted"
                    ? Effect.void
                    : Effect.fail(new MountTargetStillDeleting({
