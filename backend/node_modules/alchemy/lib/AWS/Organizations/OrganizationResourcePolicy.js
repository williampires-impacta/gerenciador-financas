...
import { retryOrganizations } from "./common.js";
/**
 ...
 * @resource
 */
export const OrganizationResourcePolicy = Resource("AWS.Organizations.OrganizationResourcePolicy");
const readResourcePolicy = () => retryOrganizations(organizations.describeResourcePolicy({}).pipe(Effect.map((response) => response.ResourcePolicy), Effect.catchTag("ResourcePolicyNotFoundException", () => Effect.succeed(undefined)), Effect.map((policy) => {
    ...
    return summary?.Id && summary.Arn
        ? {
            ...
            document: JSON.parse(policy?.Content ?? "{}"),
        }
        : undefined;
})));
export const OrganizationResourcePolicyProvider = () => Provider.effect(OrganizationResourcePolicy, Effect.gen(function* () {
    return {
        ...
        list: () => readResourcePolicy().pipe(Effect.map((policy) => (policy ? [policy] : [])), Effect.catchTag("AWSOrganizationsNotInUseException", () => Effect.succeed([]))),
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (observedContent !== desiredContent) {
                yield* retryOrganizations(organizations.putResourcePolicy({
                    Content: desiredContent,
                }));
                state = yield* readResourcePolicy();
            }
            if (!state) {
                return yield* Effect.fail(new Error("organization resource policy not found after reconcile"));
            }
...
//# sourceMappingURL=OrganizationResourcePolicy.js.map