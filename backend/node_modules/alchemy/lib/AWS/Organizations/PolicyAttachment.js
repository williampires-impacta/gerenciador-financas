 ...
 * @resource
 */
export const PolicyAttachment = Resource("AWS.Organizations.PolicyAttachment");
export const PolicyAttachmentProvider = () => Provider.effect(PolicyAttachment, Effect.gen(function* () {
    return {
        stables: ["policyId", "targetId"],
        diff: Effect.fn(function* ({ olds, news }) {
            if (!isResolved(news))
                return;
            if (olds?.policyId !== news.policyId ||
        ...
        list: () => Effect.gen(function* () {
            const perType = yield* Effect.forEach(POLICY_TYPES, (Filter) => Effect.gen(function* () {
                const policies = yield* retryOrganizations(collectPages((NextToken) => organizations.listPolicies({ Filter, NextToken }), (page) => page.Policies));
                const policyIds = policies
                    .map((policy) => policy.Id)
                    .filter((id) => id != null);
                const perPolicy = yield* Effect.forEach(policyIds, (policyId) => listAttachmentsForPolicy(policyId), { concurrency: 10 });
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (!state) {
                ...
                if (!state) {
                    return yield* Effect.fail(new Error(`policy attachment '${news.policyId}' -> '${news.targetId}' not found after create`));
                }
            }
...
const listAttachmentsForPolicy = (policyId) => retryOrganizations(collectPages((NextToken) => organizations.listTargetsForPolicy({ PolicyId: policyId, NextToken }), (page) => page.Targets)).pipe(Effect.map((targets) => targets
...
Effect.catchTag("PolicyNotFoundException", () => Effect.succeed([])));
const readAttachment = Effect.fn(function* ({ policyId, targetId, }) {
    const targets = yield* retryOrganizations(collectPages((NextToken) => organizations.listTargetsForPolicy({ PolicyId: policyId, NextToken }), (page) => page.Targets));
