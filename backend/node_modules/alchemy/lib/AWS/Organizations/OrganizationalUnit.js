 ...
 * @resource
 */
export const OrganizationalUnit = Resource("AWS.Organizations.OrganizationalUnit");
export const OrganizationalUnitProvider = () => Provider.effect(OrganizationalUnit, Effect.gen(function* () {
    return {
        stables: ["ouId", "ouArn"],
        ...
        list: () => Effect.gen(function* () {
            ...
            const hydrated = yield* Effect.forEach(ouIds, (ouId) => readOUById(ouId), { concurrency: 10 });
            const result = hydrated.filter((ou) => ou !== undefined);
            return result;
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!state) {
                ...
                if (!state) {
                    return yield* Effect.fail(new Error(`organizational unit '${name}' not found after create`));
                }
            }
            // Sync name — observed ↔ desired. `parentId` is replacement-only,
            // so the diff has handled any cross-parent move.
            if (state.name !== name) {
                yield* retryOrganizations(organizations.updateOrganizationalUnit({
            ...
            if (!updated) {
                return yield* Effect.fail(new Error(`organizational unit '${state.ouId}' not found after reconcile`));
            }
...
const listOUsForParent = (parentId) => collectPages((NextToken) => organizations.listOrganizationalUnitsForParent({
    ParentId: parentId,
    NextToken,
}), (page) => page.OrganizationalUnits).pipe(retryOrganizations);
const listAllRoots = () => collectPages((NextToken) => organizations.listRoots({ NextToken }), (page) => page.Roots).pipe(retryOrganizations);
...
const collectDescendantOUIds = (parentIds) => Effect.gen(function* () {
    if (parentIds.length === 0)
        return [];
    const childLists = yield* Effect.forEach(parentIds, (parentId) => listOUsForParent(parentId), { concurrency: 10 });
    const childIds = childLists
        .flat()
        .map((ou) => ou.Id)
        .filter((ouId) => ouId !== undefined);
    const descendants = yield* collectDescendantOUIds(childIds);
    return [...childIds, ...descendants];
});
const readParentId = (childId) => collectPages((NextToken) => organizations.listParents({ ChildId: childId, NextToken }), (page) => page.Parents).pipe(retryOrganizations, Effect.map((parents) => parents[0]?.Id));
const readOUById = Effect.fn(function* (ouId) {
    const described = yield* retryOrganizations(organizations
        .describeOrganizationalUnit({
        OrganizationalUnitId: ouId,
    })
        .pipe(Effect.map((response) => response.OrganizationalUnit), Effect.catchTag("OrganizationalUnitNotFoundException", () => Effect.succeed(undefined))));
    if (!described?.Id || !described.Arn || !described.Name) {
        return undefined;
    }
    const [parentId, tags] = yield* Effect.all([
        readParentId(described.Id).pipe(Effect.catchTag("ChildNotFoundException", () => Effect.succeed(undefined))),
        readResourceTags(described.Id).pipe(Effect.catchTag("TargetNotFoundException", () => Effect.succeed({}))),
    ]);
    return {
        ouId: described.Id,
        ouArn: described.Arn,
...
const readOUByParentAndName = Effect.fn(function* ({ parentId, name, }) {
