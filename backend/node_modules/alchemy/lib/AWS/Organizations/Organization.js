 ...
 * @resource
 */
export const Organization = Resource("AWS.Organizations.Organization");
export const OrganizationProvider = () => Provider.effect(Organization, Effect.gen(function* () {
    return {
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (!org) {
                org = yield* retryOrganizations(organizations.createOrganization({
                    FeatureSet: desiredFeatureSet,
                })).pipe(Effect.map((response) => response.Organization), Effect.catchTag("AlreadyInOrganizationException", () => readOrganization()));
            }
            if (!org?.Id || !org.Arn) {
...
const readOrganization = () => retryOrganizations(organizations.describeOrganization({}).pipe(Effect.map((response) => response.Organization), Effect.catchTag("AWSOrganizationsNotInUseException", () => Effect.succeed(undefined))));
const ensureFeatureSet = Effect.fn(function* ({ desired, current, }) {
    const desiredFeatureSet = desired ?? "ALL";
    if (current.FeatureSet === desiredFeatureSet) {
        return current;
    }
    if (desiredFeatureSet === "ALL" &&
        ...
        return yield* Effect.fail(new Error("Organization upgrade to ALL features requires handshake completion before deployment can converge"));
    }
    return yield* Effect.fail(new Error(`Organization feature set cannot be changed from '${current.FeatureSet}' to '${desiredFeatureSet}'`));
});
