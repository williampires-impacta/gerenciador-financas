 ...
 */
export const retryWhileSettling = (self) => Effect.retry(self, {
 ...
 * reason as above.
 */
export const retryWhileNotFound = (self) => Effect.retry(self, {
 ...
 */
const untilConverged = (self, done, times = 30) => Effect.repeat(self, {
 ...
 */
export const waitForStreamActive = Effect.fn("AWS.KinesisVideo.waitForStreamActive")(function* (streamName, previousVersion) {
    const done = (info) => info?.Status === "ACTIVE" &&
        (previousVersion === undefined || info.Version !== previousVersion);
    const info = yield* untilConverged(retryWhileNotFound(kv.describeStream({ StreamName: streamName })).pipe(Effect.map((r) => r.StreamInfo)), done);
    if (!done(info)) {
        return yield* Effect.fail(new KinesisVideoNotConverged({
            resource: streamName,
            status: info?.Status,
        }));
    }
 ...
 */
export const waitForChannelActive = Effect.fn("AWS.KinesisVideo.waitForChannelActive")(function* (channelName, previousVersion) {
    const done = (info) => info?.ChannelStatus === "ACTIVE" &&
        (previousVersion === undefined || info.Version !== previousVersion);
    const info = yield* untilConverged(retryWhileNotFound(kv.describeSignalingChannel({ ChannelName: channelName })).pipe(Effect.map((r) => r.ChannelInfo)), done);
    if (!done(info)) {
        return yield* Effect.fail(new KinesisVideoNotConverged({
 ...
 */
export const waitForStreamGone = Effect.fn("AWS.KinesisVideo.waitForStreamGone")(function* (streamName) {
    const info = yield* untilConverged(kv.describeStream({ StreamName: streamName }).pipe(Effect.map((r) => r.StreamInfo), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined))), (i) => i === undefined, 
 ...
 */
export const waitForChannelGone = Effect.fn("AWS.KinesisVideo.waitForChannelGone")(function* (channelName) {
    const info = yield* untilConverged(kv.describeSignalingChannel({ ChannelName: channelName }).pipe(Effect.map((r) => r.ChannelInfo), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined))), (i) => i === undefined, 45);
    if (info !== undefined) {
        return yield* Effect.fail(new KinesisVideoNotConverged({
            resource: channelName,
            status: info.ChannelStatus,
...
const dataEndpointCache = new Map();
/**
 ...
 * requirement-free.
 */
export const discoverDataEndpoint = (streamArn, apiName, getDataEndpoint) => Effect.gen(function* () {
...
const signalingEndpointCache = new Map();
/**
 ...
 */
export const discoverSignalingEndpoint = (channelArn, protocol, role, getSignalingChannelEndpoint) => Effect.gen(function* () {
    ...
    const endpoint = (response.ResourceEndpointList ?? []).find((item) => item.Protocol === protocol)?.ResourceEndpoint;
