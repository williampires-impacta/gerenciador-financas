...
import { discoverDataEndpoint, discoverSignalingEndpoint } from "./internal.js";
/**
 ...
 * stream's ARN.
 */
export const makeStreamMediaHttpBinding = (options) => Effect.gen(function* () {
    ...
    return Effect.fn(function* (stream) {
        ...
        return Effect.fn(`${options.tag}(${stream.LogicalId})`)(function* (request) {
            ...
            return yield* op({ ...request, StreamARN: streamArn }).pipe(Effect.provideService(Endpoint.Endpoint, Effect.succeed(endpoint)));
        });
    });
});
/**
 ...
 */
export const makeChannelSignalingHttpBinding = (options) => Effect.gen(function* () {
    // Yield-first captures the operations' services (Credentials/Region/
    // HttpClient) at layer init so the runtime callable is requirement-free.
    ...
    return Effect.fn(function* (channel) {
        ...
        return Effect.fn(`${options.tag}(${channel.LogicalId})`)(function* (request) {
            ...
            const endpoint = yield* discoverSignalingEndpoint(channelArn, options.protocol, options.role, getSignalingChannelEndpoint);
            return yield* op({
                ...request,
                [options.key]: channelArn,
            }).pipe(Effect.provideService(Endpoint.Endpoint, Effect.succeed(endpoint)));
        });
    });
});
