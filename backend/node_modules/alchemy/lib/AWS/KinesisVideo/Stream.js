...
import { compactTags, retryWhileResourceInUse, retryWhileSettling, waitForStreamActive, waitForStreamGone, } from "./internal.js";
/**
 ...
 * @resource
 */
export const Stream = Resource("AWS.KinesisVideo.Stream");
export const StreamProvider = () => Provider.effect(Stream, Effect.gen(function* () {
    ...
    const observeStream = Effect.fn(function* (streamName) {
        return yield* kv.describeStream({ StreamName: streamName }).pipe(Effect.map((r) => r.StreamInfo), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    });
    const fetchObservedTags = Effect.fn(function* (streamArn) {
        return yield* kv.listTagsForStream({ StreamARN: streamArn }).pipe(Effect.map((r) => compactTags(r.Tags)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})));
    });
    return Stream.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (info === undefined) {
                yield* retryWhileResourceInUse(kv.createStream({
                    ...
                    Tags: desiredTags,
                })).pipe(Effect.catchTag("ResourceInUseException", () => Effect.succeed({ StreamARN: undefined })));
            ...
            if (desiredRetention !== undefined &&
                ...
                yield* retryWhileSettling(kv.updateDataRetention({
                    StreamARN: streamArn,
                    CurrentVersion: info.Version,
                    Operation: desiredRetention > observedRetention
            ...
            if (upsert.length > 0) {
                yield* retryWhileSettling(kv.tagStream({
                    StreamARN: streamArn,
                    Tags: Object.fromEntries(upsert.map((t) => [t.Key, t.Value])),
                }));
            }
            if (removed.length > 0) {
                yield* retryWhileSettling(kv.untagStream({
                    StreamARN: streamArn,
                    TagKeyList: removed,
                }));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryWhileResourceInUse(kv.deleteStream({ StreamARN: output.streamArn })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void));
