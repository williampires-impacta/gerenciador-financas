 ...
 * @resource
 */
export const DBProxyTargetGroup = Resource("AWS.RDS.DBProxyTargetGroup");
const toTargetGroupName = (props) => props.targetGroupName ?? "default";
export const DBProxyTargetGroupProvider = () => Provider.effect(DBProxyTargetGroup, Effect.gen(function* () {
    const readGroup = Effect.fn(function* ({ dbProxyName, targetGroupName, }) {
        const response = yield* rds
            .describeDBProxyTargetGroups({
            DBProxyName: dbProxyName,
            TargetGroupName: targetGroupName,
        })
            .pipe(Effect.catchTag("DBProxyTargetGroupNotFoundFault", () => Effect.succeed(undefined)));
        return response?.TargetGroups?.[0];
    });
    const toAttrs = ({ group, props, }) => ({
    ...
    const readTargets = Effect.fn(function* ({ dbProxyName, targetGroupName, }) {
        const response = yield* rds
            .describeDBProxyTargets({
            DBProxyName: dbProxyName,
            TargetGroupName: targetGroupName,
        })
            .pipe(Effect.catchTag("DBProxyTargetNotFoundFault", () => Effect.succeed(undefined)), Effect.catchTag("DBProxyTargetGroupNotFoundFault", () => Effect.succeed(undefined)), Effect.catchTag("DBProxyNotFoundFault", () => Effect.succeed(undefined)));
        const targets = response?.Targets ?? [];
        const observedClusters = [];
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const perProxy = yield* Effect.forEach(proxies, (proxy) => Effect.gen(function* () {
                const dbProxyName = proxy.DBProxyName;
                if (!dbProxyName)
                    return [];
                const groups = yield* rds.describeDBProxyTargetGroups
                    .pages({ DBProxyName: dbProxyName })
                    .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.TargetGroups ?? [])), Effect.catchTag("DBProxyNotFoundFault", () => Effect.succeed([])), Effect.catchTag("DBProxyTargetGroupNotFoundFault", () => Effect.succeed([])));
                return yield* Effect.forEach(groups, (group) => Effect.gen(function* () {
                    const targetGroupName = group.TargetGroupName ?? "default";
                    const { observedClusters, observedInstances } = yield* readTargets({ dbProxyName, targetGroupName });
                    return {
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            const removeInstances = [...observedInstanceSet].filter((id) => !desiredInstances.has(id));
            ...
            if (!observedGroup?.TargetGroupName) {
                return yield* Effect.fail(new Error(`DB proxy target group '${targetGroupName}' not found`));
            }
            yield* session.note(observedGroup.TargetGroupArn ??
                output?.targetGroupArn ??
                observedGroup.TargetGroupName);
            return toAttrs({ group: observedGroup, props: news });
        }),
        delete: Effect.fn(function* ({ output }) {
            if (output.dbClusterIdentifiers.length > 0 ||
                output.dbInstanceIdentifiers.length > 0) {
                yield* rds
                    .deregisterDBProxyTargets({
                    DBProxyName: output.dbProxyName,
                    TargetGroupName: output.targetGroupName,
                    DBClusterIdentifiers: output.dbClusterIdentifiers.length > 0
                        ? output.dbClusterIdentifiers
                        : undefined,
                    DBInstanceIdentifiers: output.dbInstanceIdentifiers.length > 0
...
//# sourceMappingURL=DBProxyTargetGroup.js.map