 ...
 */
const rdsDbUserArn = (sourceArn, resourceId, username) => {
 ...
 */
const dbUserArn = (resource, username) => {
    switch (resource.Type) {
        case "AWS.RDS.DBCluster":
            return Output.map(Output.all(resource.dbClusterArn, resource.dbClusterResourceId), ([arn, resourceId]) => rdsDbUserArn(arn, resourceId ?? "*", username));
        case "AWS.RDS.DBProxy":
            return Output.map(resource.dbProxyArn, (arn) => rdsDbUserArn(arn, arn.split(":")[6] ?? "*", username));
        default:
    ...
    return Effect.fn(function* (resource, options) {
        ...
        if (!globalThis.__ALCHEMY_RUNTIME__) {
            ...
            if (isBindingHost(host)) {
                ...
                const vpc = options.subnetIds !== undefined ||
