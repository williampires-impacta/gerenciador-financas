import * as aa from "@distilled.cloud/aws/accessanalyzer";
import * as Layer from "effect/Layer";
import { makeAnalyzerScopedHttpBinding } from "./BindingHttp.js";
import { CreateAccessPreview } from "./CreateAccessPreview.js";
export const CreateAccessPreviewHttp = Layer.effect(CreateAccessPreview, makeAnalyzerScopedHttpBinding({
    tag: "AWS.AccessAnalyzer.CreateAccessPreview",
    operation: aa.createAccessPreview,
    actions: ["access-analyzer:CreateAccessPreview"],
}));
//# sourceMappingURL=CreateAccessPreviewHttp.js.map