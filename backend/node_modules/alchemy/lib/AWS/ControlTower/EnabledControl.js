 ...
 * @resource
 */
export const EnabledControl = Resource("AWS.ControlTower.EnabledControl");
/**
...
// `AWS.providers()`.
const retryWhileControlOperationPending = (self) => Effect.retry(self, {
...
// bounded schedule.
const retryWhileControlConflict = (self) => Effect.retry(self, {
...
const waitForControlOperation = (operationIdentifier) => retryWhileControlOperationPending(Effect.gen(function* () {
    ...
    if (controlOperation.status === "FAILED") {
        return yield* Effect.fail(new ControlOperationFailed({
            operationIdentifier,
            status: controlOperation.status,
            statusMessage: controlOperation.statusMessage,
        }));
    }
    return yield* Effect.fail(new ControlOperationPending({
        operationIdentifier,
        status: controlOperation.status,
    }));
}));
const readEnabledControl = (enabledControlArn) => controltower
    .getEnabledControl({ enabledControlIdentifier: enabledControlArn })
    .pipe(Effect.map((r) => r.enabledControlDetails), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
...
// ARN cached.
const findEnabledControlArn = (controlIdentifier, targetIdentifier) => controltower.listEnabledControls.pages({ targetIdentifier }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    .flatMap((page) => page.enabledControls)
    .find((summary) => summary.controlIdentifier === controlIdentifier)
    ?.arn), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
export const EnabledControlProvider = () => Provider.effect(EnabledControl, Effect.gen(function* () {
    return EnabledControl.Provider.of({
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const arn = output?.enabledControlArn ??
                (olds !== undefined
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return undefined;
            if (news.controlIdentifier !== olds.controlIdentifier ||
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (details === undefined) {
                ...
                const enabled = yield* retryWhileControlConflict(controltower.enableControl({
                ...
                if (arn === undefined) {
                    return yield* Effect.fail(new EnabledControlArnUnavailable({
                        controlIdentifier: news.controlIdentifier,
                        targetIdentifier: news.targetIdentifier,
                    }));
                }
                ...
                if (observed !== desired) {
                    const updated = yield* retryWhileControlConflict(controltower.updateEnabledControl({
        ...
        delete: Effect.fn(function* ({ output, session }) {
            const result = yield* retryWhileControlConflict(controltower.disableControl({
...
//# sourceMappingURL=EnabledControl.js.map