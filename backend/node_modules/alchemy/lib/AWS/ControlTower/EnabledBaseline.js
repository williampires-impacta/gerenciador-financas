...
import { canonicalParameters, observeControlTowerTags, syncControlTowerTags, } from "./internal.js";
/**
 ...
 * @resource
 */
export const EnabledBaseline = Resource("AWS.ControlTower.EnabledBaseline");
/**
...
// `AWS.providers()`.
const retryWhileBaselineOperationPending = (self) => Effect.retry(self, {
...
// bounded schedule.
const retryWhileBaselineConflict = (self) => Effect.retry(self, {
...
const waitForBaselineOperation = (operationIdentifier) => retryWhileBaselineOperationPending(Effect.gen(function* () {
    ...
    if (baselineOperation.status === "FAILED") {
        return yield* Effect.fail(new BaselineOperationFailed({
            operationIdentifier,
            status: baselineOperation.status,
            statusMessage: baselineOperation.statusMessage,
        }));
    }
    return yield* Effect.fail(new BaselineOperationPending({
        operationIdentifier,
        status: baselineOperation.status,
    }));
}));
const readEnabledBaseline = (enabledBaselineArn) => controltower
    .getEnabledBaseline({ enabledBaselineIdentifier: enabledBaselineArn })
    .pipe(Effect.map((r) => r.enabledBaselineDetails), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
...
// ARN cached.
const findEnabledBaselineArn = (baselineIdentifier, targetIdentifier) => controltower.listEnabledBaselines
    .pages({
    filter: {
    ...
    .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.enabledBaselines)[0]?.arn));
const toAttributes = (arn, details, fallback) => ({
...
export const EnabledBaselineProvider = () => Provider.effect(EnabledBaseline, Effect.gen(function* () {
    return EnabledBaseline.Provider.of({
        ...
        list: () => controltower.listEnabledBaselines.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const arn = output?.enabledBaselineArn ??
                (olds !== undefined
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return undefined;
            if (news.baselineIdentifier !== olds.baselineIdentifier ||
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (details === undefined || arn === undefined) {
                ...
                const enabled = yield* retryWhileBaselineConflict(controltower.enableBaseline({
                ...
                const parametersChanged = news.parameters !== undefined &&
                    canonicalParameters(details.parameters) !==
                        canonicalParameters(news.parameters);
                if (versionChanged || parametersChanged) {
                    const updated = yield* retryWhileBaselineConflict(controltower.updateEnabledBaseline({
        ...
        delete: Effect.fn(function* ({ output, session }) {
            const result = yield* retryWhileBaselineConflict(controltower.disableBaseline({
...
//# sourceMappingURL=EnabledBaseline.js.map