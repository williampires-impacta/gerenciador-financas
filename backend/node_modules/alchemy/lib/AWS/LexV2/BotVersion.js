...
import { retryWhileConflict, waitForBotSettled, waitForLocaleBuilt, } from "./internal.js";
/**
 ...
 * @resource
 */
export const BotVersion = Resource("AWS.LexV2.BotVersion");
const describeVersion = Effect.fn(function* (botId, botVersion) {
    return yield* lexm
        .describeBotVersion({ botId, botVersion })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
});
export const BotVersionProvider = () => Provider.effect(BotVersion, Effect.gen(function* () {
    return {
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            const created = yield* retryWhileConflict(lexm.createBotVersion({
                botId: news.botId,
                description: news.description,
                botVersionLocaleSpecification: Object.fromEntries(news.localeIds.map((localeId) => [
                    localeId,
            ...
            if (observed === undefined) {
                return yield* Effect.fail(new Error(`failed to read created Lex bot version ${news.botId}/${botVersion}`));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            // PreconditionFailed.
            yield* retryWhileConflict(lexm.deleteBotVersion({
                botId: output.botId,
