...
import { readLexTags, toLexName, retryWhileConflict, syncLexTags, waitForBotSettled, } from "./internal.js";
/**
 ...
 * @resource
 */
export const Bot = Resource("AWS.LexV2.Bot");
const createBotName = (id, props) => Effect.gen(function* () {
    if (props.botName)
        return props.botName;
    return toLexName(yield* createPhysicalName({ id, maxLength: 100 }));
});
const describeBot = Effect.fn(function* (botId) {
    return yield* lexm
        .describeBot({ botId })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
});
/** Find a bot by exact name. The physical name embeds app/stage/id. */
const findBotByName = Effect.fn(function* (botName) {
    ...
    if (summary?.botId === undefined)
        return undefined;
    return yield* describeBot(summary.botId);
});
const botArnOf = Effect.fn(function* (botId) {
    const { accountId, region } = yield* AWSEnvironment.current;
    return `arn:aws:lex:${region}:${accountId}:bot/${botId}`;
});
const attributesOf = Effect.fn(function* (bot) {
...
export const BotProvider = () => Provider.effect(Bot, Effect.gen(function* () {
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const hydrated = yield* Effect.forEach(ids, (id) => Effect.flatMap(describeBot(id), (bot) => bot === undefined
                ? Effect.succeed(undefined)
                : attributesOf(bot)), { concurrency: 5 });
            return hydrated.filter(Predicate.isNotUndefined);
        }),
        read: Effect.fn(function* ({ id, olds, output }) {
            const observed = output?.botId !== undefined
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let observed = output?.botId !== undefined
                ? yield* describeBot(output.botId)
                : undefined;
            if (observed === undefined) {
            ...
            if (observed === undefined) {
                const created = yield* lexm
                    ...
                    .pipe(Effect.map((r) => r.botId), Effect.catchTag("ConflictException", () => Effect.map(findBotByName(botName), (bot) => bot?.botId)));
            ...
            if (observed.botName !== botName ||
                ...
                yield* retryWhileConflict(lexm.updateBot({
                    botId,
                    botName,
                    roleArn: news.roleArn,
                    dataPrivacy: desiredDataPrivacy,
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryWhileConflict(lexm.deleteBot({
                botId: output.botId,
