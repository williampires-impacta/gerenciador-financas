...
export interface LexAliasHttpBindingConfig<Req extends object, Out, Err> {
    ...
    operation: Effect.Effect<(input: Req & {
        botId: string;
        botAliasId: string;
    }) => Effect.Effect<Out, Err>, never, LexRequirements>;
}
/**
 ...
 */
export declare const makeLexAliasHttpBinding: <Req extends object, Out, Err>(config: LexAliasHttpBindingConfig<Req, Out, Err>) => Effect.Effect<(alias: BotAlias) => Effect.Effect<(request: Req) => Effect.Effect<Out, Err, never>, never, never>, never, LexRequirements>;
export {};
