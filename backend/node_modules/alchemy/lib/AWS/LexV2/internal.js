 ...
 */
export const toLexName = (name) => name
 ...
 */
const untilConverged = (self, done, options) => Effect.repeat(self, {
 ...
 */
export const waitForBotSettled = Effect.fn("AWS.LexV2.waitForBotSettled")(function* (botId) {
    const bot = yield* untilConverged(lexm.describeBot({ botId }), (b) => !transientBotStatuses.has(b.botStatus ?? ""));
    if (bot.botStatus === "Failed" ||
        transientBotStatuses.has(bot.botStatus ?? "")) {
        return yield* Effect.fail(new LexOperationFailed({
            resourceId: botId,
 ...
 */
export const waitForLocaleSettled = Effect.fn("AWS.LexV2.waitForLocaleSettled")(function* (botId, localeId) {
    const locale = yield* untilConverged(lexm.describeBotLocale({ botId, botVersion: "DRAFT", localeId }), (l) => !transientLocaleStatuses.has(l.botLocaleStatus ?? ""));
    if (locale.botLocaleStatus === "Failed" ||
 ...
 */
export const waitForLocaleBuilt = Effect.fn("AWS.LexV2.waitForLocaleBuilt")(function* (botId, localeId) {
    const locale = yield* untilConverged(lexm.describeBotLocale({ botId, botVersion: "DRAFT", localeId }), (l) => l.botLocaleStatus === "Built" || l.botLocaleStatus === "Failed", { intervalSeconds: 5, times: 18 });
 ...
 * `Failed`.
 */
export const waitForAliasSettled = Effect.fn("AWS.LexV2.waitForAliasSettled")(function* (botId, botAliasId) {
    const alias = yield* untilConverged(lexm.describeBotAlias({ botId, botAliasId }), (a) => a.botAliasStatus !== "Creating");
    if (alias.botAliasStatus === "Failed" ||
 ...
 */
export const readLexTags = Effect.fn("AWS.LexV2.readLexTags")(function* (arn) {
 ...
 */
export const syncLexTags = Effect.fn("AWS.LexV2.syncLexTags")(function* (arn, observed, desired) {
