 ...
 */
export const asPlain = (value) => value === undefined
    ? undefined
...
export const deadlineArnOf = Effect.gen(function* () {
    ...
    return (path) => `arn:aws:deadline:${region}:${accountId}:${path}`;
});
/**
 ...
 */
export const fetchDeadlineTags = Effect.fn(function* (arn) {
    const response = yield* deadline
        .listTagsForResource({ resourceArn: arn })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    return Object.fromEntries(Object.entries(response?.tags ?? {}).filter((entry) => entry[1] !== undefined));
});
/**
 * Converge a Deadline resource's tags to the desired set, diffing against
 * the OBSERVED cloud tags (never olds/output).
 */
export const syncDeadlineTags = Effect.fn(function* (arn, desired) {
 ...
 */
export const reapDeadlineLogGroups = Effect.fn(function* (prefix) {
    const groups = yield* logs.describeLogGroups
        .pages({ logGroupNamePrefix: prefix })
        .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)
        .flatMap((page) => page.logGroups ?? [])
        .map((group) => group.logGroupName)
        .filter((name) => name != null)));
    yield* Effect.forEach(groups, (logGroupName) => logs
        .deleteLogGroup({ logGroupName })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void)), { concurrency: 4, discard: true });
});
/**
 ...
 */
export const reapFarmChildren = Effect.fn(function* (farmId) {
    ...
    const queueFleetAssociations = yield* deadline.listQueueFleetAssociations
        .items({ farmId })
        .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([])));
    yield* Effect.forEach(queueFleetAssociations, (assoc) => Effect.gen(function* () {
        ...
        yield* retryWhileConflict(deadline.deleteQueueFleetAssociation({
            farmId,
            queueId: assoc.queueId,
            fleetId: assoc.fleetId,
    ...
    const queueLimitAssociations = yield* deadline.listQueueLimitAssociations
        .items({ farmId })
        .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([])));
    yield* Effect.forEach(queueLimitAssociations, (assoc) => Effect.gen(function* () {
        if (assoc.status !== "STOPPED") {
            yield* deadline
        ...
        yield* retryWhileConflict(deadline.deleteQueueLimitAssociation({
            farmId,
            queueId: assoc.queueId,
            limitId: assoc.limitId,
        })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void));
    ...
    // 3. Budgets.
    const budgets = yield* deadline.listBudgets.items({ farmId }).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([])));
    yield* Effect.forEach(budgets, (budget) => deadline
    ...
    const queues = yield* deadline.listQueues.items({ farmId }).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([])));
    yield* Effect.forEach(queues, (queue) => retryWhileConflict(deadline.deleteQueue({ farmId, queueId: queue.queueId })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void)), { concurrency: 4, discard: true });
    // 5. Fleets (deletion drains workers asynchronously).
    const fleets = yield* deadline.listFleets.items({ farmId }).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([])));
    yield* Effect.forEach(fleets, (fleet) => retryWhileConflict(deadline.deleteFleet({ farmId, fleetId: fleet.fleetId })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void)), { concurrency: 4, discard: true });
    // 6. Limits (their queue-limit associations were deleted above).
    const limits = yield* deadline.listLimits.items({ farmId }).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([])));
    yield* Effect.forEach(limits, 
    ...
    (limit) => deadline.deleteLimit({ farmId, limitId: limit.limitId }), { concurrency: 4, discard: true });
    ...
    const storageProfiles = yield* deadline.listStorageProfiles
        .items({ farmId })
        .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([])));
    yield* Effect.forEach(storageProfiles, (profile) => retryWhileConflict(deadline.deleteStorageProfile({
        farmId,
 ...
 */
export const retryWhileFarmSettling = (self) => Effect.retry(self, {
    while: (e) => e._tag === "ConflictException" ||
 ...
 */
export const retryThroughIamPropagation = (self) => Effect.retry(self, {
