 ...
 * @resource
 */
export const DeliveryStream = Resource("AWS.Firehose.DeliveryStream");
/**
 ...
 */
export class DeliveryStreamValidationError extends Data.TaggedError("DeliveryStreamValidationError") {
}
/**
 ...
 */
export class DeliveryStreamEncryptionFailed extends Data.TaggedError("DeliveryStreamEncryptionFailed") {
}
...
const defaultCompressionFormat = "UNCOMPRESSED";
const createDeliveryStreamName = (id, props) => Effect.gen(function* () {
...
const retryThroughRolePropagation = (self) => Effect.retry(self, {
    while: (e) => e._tag === "InvalidArgumentException" &&
        /role|assume|trust/i.test(e.message ?? ""),
 ...
 */
const retryConcurrentModification = (self) => Effect.retry(self, {
...
const toAttrs = ({ description, tags, roleName, }) => {
    ...
    return {
        ...
        deliveryStreamArn: description.DeliveryStreamARN,
        deliveryStreamStatus: description.DeliveryStreamStatus,
        ...
        encryptionKeyType: encryption?.Status === "ENABLED"
            ? encryption.KeyType
            : undefined,
...
const describeDeliveryStream = Effect.fn(function* (deliveryStreamName) {
...
const readDeliveryStream = Effect.fn(function* ({ deliveryStreamName, roleName, }) {
...
const waitForEncryptionSettled = (deliveryStreamName) => Effect.gen(function* () {
    const description = yield* describeDeliveryStream(deliveryStreamName);
    const encryption = description?.DeliveryStreamEncryptionConfiguration;
    if (encryption?.Status === "ENABLING" ||
        encryption?.Status === "DISABLING") {
        return yield* Effect.fail(new DeliveryStreamEncryptionPending());
    }
    return encryption;
}).pipe(Effect.retry({
    while: (e) => e._tag === "DeliveryStreamEncryptionPending",
    schedule: Schedule.max([
        Schedule.fixed("2 seconds"),
        Schedule.recurs(45),
    ]),
...
const waitForDeliveryStreamDeleted = (deliveryStreamName) => Effect.gen(function* () {
    const description = yield* describeDeliveryStream(deliveryStreamName);
    if (description !== undefined) {
        return yield* Effect.fail(new DeliveryStreamStillExists());
    }
}).pipe(Effect.retry({
    while: (e) => e._tag === "DeliveryStreamStillExists",
    schedule: Schedule.max([
...
export const DeliveryStreamProvider = () => Provider.effect(DeliveryStream, Effect.gen(function* () {
    ...
    const ensureRole = Effect.fn(function* ({ id, roleName, bucketArn, kinesisStreamArn, }) {
    ...
    return DeliveryStream.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            while (true) {
                ...
                if (!page.HasMoreDeliveryStreams ||
                    page.DeliveryStreamNames.length === 0) {
                    break;
                }
                exclusiveStart = names[names.length - 1];
            }
            const hydrated = yield* Effect.forEach(names, (deliveryStreamName) => readDeliveryStream({ deliveryStreamName, roleName: undefined }), { concurrency: 10 });
            return hydrated.filter((attrs) => attrs !== undefined);
        }),
        read: Effect.fn(function* ({ id, olds, output }) {
            const deliveryStreamName = output?.deliveryStreamName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!news?.destination?.bucketArn) {
                return yield* Effect.fail(new DeliveryStreamValidationError({
            ...
            if (news.encryption?.keyType === "CUSTOMER_MANAGED_CMK" &&
            ...
            const needsRole = !news.destination.roleArn ||
            ...
            if (observed &&
                ...
                yield* session.note(`Delivery stream ${deliveryStreamName} is ${observed.DeliveryStreamStatus}; deleting and recreating`);
                yield* firehose
            ...
            const desiredBufferingHints = {
                IntervalInSeconds: toWireSeconds(news.destination.bufferingInterval) ??
            ...
            if (observed === undefined) {
                yield* retryThroughRolePropagation(firehose
            ...
            const syncDestination = Effect.gen(function* () {
                ...
                if (!destination || !s3)
                    return;
                const delta = s3.RoleARN !== destinationRoleArn ||
            ...
            if (news.encryption) {
                const keyMatches = encryptionEnabled &&
                ...
                if (!keyMatches) {
                    yield* retryWhileInUse(firehose.startDeliveryStreamEncryption({
                        DeliveryStreamName: deliveryStreamName,
                    ...
                    if (settled?.Status !== "ENABLED") {
                        return yield* Effect.fail(new DeliveryStreamEncryptionFailed({
                            deliveryStreamName,
                            status: settled?.Status ?? "UNKNOWN",
                            details: settled?.FailureDescription?.Details,
                        }));
                    }
                    yield* session.note(`Enabled SSE (${news.encryption.keyType}) on ${deliveryStreamName}`);
                }
            }
            else if (encryptionEnabled) {
                yield* retryWhileInUse(firehose.stopDeliveryStreamEncryption({
                    DeliveryStreamName: deliveryStreamName,
                }));
                const settled = yield* waitForEncryptionSettled(deliveryStreamName);
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryWhileInUse(firehose
                .deleteDeliveryStream({
