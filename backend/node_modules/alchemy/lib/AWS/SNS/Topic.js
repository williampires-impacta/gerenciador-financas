 ...
 * @resource
 */
export const Topic = Resource("AWS.SNS.Topic");
export const TopicProvider = () => Provider.succeed(Topic, {
    ...
    list: Effect.fn(function* () {
        const topicArns = yield* sns.listTopics.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Topics ?? [])
            .map((topic) => topic.TopicArn)
            .filter((arn) => typeof arn === "string"))));
        const rows = yield* Effect.forEach(topicArns, (topicArn) => readTopic({
            id: "",
            topicArn,
    ...
    diff: Effect.fn(function* ({ id, news = {}, olds = {} }) {
        ...
        if (olds.dataProtectionPolicy !== undefined &&
    ...
    reconcile: Effect.fn(function* ({ id, news = {}, olds, output, session }) {
        ...
        if (!isFifo) {
            observedPolicy = yield* sns
                ...
                .pipe(Effect.map((r) => r.DataProtectionPolicy), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)), Effect.catchTag("InvalidParameterException", () => Effect.succeed(undefined)));
        }
        if (!isFifo &&
            news.dataProtectionPolicy !== undefined &&
...
const toTopicName = Effect.fn(function* (id, props) {
    if (props.topicName) {
...
const toTagMap = (tags) => Object.fromEntries((tags ?? [])
...
const findTopicArnByName = Effect.fn(function* (topicName) {
    const match = yield* sns.listTopics.items({}).pipe(Stream.filter((topic) => topic.TopicArn?.split(":").at(-1) === topicName), Stream.runHead, Effect.map(Option.getOrUndefined));
    return match?.TopicArn;
});
const readTopic = Effect.fn(function* ({ id, topicArn, topicName, }) {
    ...
    const topicState = yield* Effect.all([
        ...
        sns
            ...
            .pipe(Effect.map((response) => response.DataProtectionPolicy), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined))),
    ], { concurrency: "unbounded" }).pipe(Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)), Effect.catchTag("InvalidParameterException", () => Effect.succeed(undefined)), 
