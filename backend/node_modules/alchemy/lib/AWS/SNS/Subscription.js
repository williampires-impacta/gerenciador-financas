...
export const SubscriptionProvider = () => Provider.succeed(Subscription, {
    ...
    list: Effect.fn(function* () {
        const subscriptions = yield* sns.listSubscriptions.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Subscriptions ?? [])));
        const concrete = subscriptions.filter((subscription) => typeof subscription.SubscriptionArn === "string" &&
            !isPendingConfirmation(subscription.SubscriptionArn));
        const rows = yield* Effect.forEach(concrete, (subscription) => readSubscription({
    ...
    diff: Effect.fn(function* ({ news, olds }) {
        ...
        if (typeof news.topicArn === "string" &&
            typeof olds.topicArn === "string" &&
            news.topicArn !== olds.topicArn) {
            return { action: "replace" };
        }
        if (typeof news.endpoint === "string" &&
    ...
    reconcile: Effect.fn(function* ({ news, output, session }) {
        const topicArn = news.topicArn;
        const protocol = news.protocol;
        const endpoint = news.endpoint;
        ...
        let subscriptionArn;
        if (output?.subscriptionArn &&
            !isPendingConfirmation(output.subscriptionArn)) {
            const observed = yield* sns
                .getSubscriptionAttributes({
                SubscriptionArn: output.subscriptionArn,
            })
                .pipe(Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)), Effect.catchTag("InvalidParameterException", () => Effect.succeed(undefined)));
            if (observed) {
                subscriptionArn = output.subscriptionArn;
            }
        }
    ...
    delete: Effect.fn(function* ({ olds, output }) {
        ...
        yield* sns
            ...
            .pipe(Effect.catchTag("NotFoundException", () => Effect.void), Effect.catchTag("InvalidParameterException", () => Effect.void));
    }),
});
const isPendingConfirmation = (subscriptionArn) => subscriptionArn === undefined ||
...
const findSubscription = Effect.fn(function* ({ topicArn, protocol, endpoint, }) {
    if (!topicArn || !protocol) {
        return undefined;
    }
    const match = yield* sns.listSubscriptionsByTopic
        .items({ TopicArn: topicArn })
        .pipe(Stream.filter((subscription) => subscription.Protocol === protocol &&
        subscription.Endpoint === endpoint &&
        !!subscription.SubscriptionArn), Stream.runHead, Effect.map(Option.getOrUndefined));
    return match?.SubscriptionArn;
});
const readSubscription = Effect.fn(function* ({ subscriptionArn, topicArn, protocol, endpoint, }) {
    const resolvedSubscriptionArn = subscriptionArn && !isPendingConfirmation(subscriptionArn)
        ? subscriptionArn
        : yield* findSubscription({
            topicArn,
            protocol,
    ...
    return {
        ...
        pendingConfirmation: attributes.PendingConfirmation === "true" ||
