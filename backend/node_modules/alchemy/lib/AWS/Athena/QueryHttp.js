...
import { AthenaQueryFailed, Query, } from "./Query.js";
...
const glueBase = (workGroupArn) => workGroupArn.pipe(Output.map((arn) => {
    const [, partition, , region, account] = arn.split(":");
    return `arn:${partition}:glue:${region}:${account}`;
}));
const glueCatalogArn = (workGroupArn) => glueBase(workGroupArn).pipe(Output.map((base) => `${base}:catalog`));
const glueDatabaseArn = (workGroupArn) => glueBase(workGroupArn).pipe(Output.map((base) => `${base}:database/*`));
const glueTableArn = (workGroupArn) => glueBase(workGroupArn).pipe(Output.map((base) => `${base}:table/*/*`));
export const QueryHttp = Layer.effect(Query, Effect.gen(function* () {
    ...
    return Effect.fn(function* (workGroup, resultsBucket) {
        ...
        return Effect.fn(`AWS.Athena.Query(${workGroup.LogicalId})`)(function* (request) {
            ...
            }).pipe(Effect.map((r) => r.QueryExecution), Effect.repeat({
            ...
            const rows = (results.ResultSet?.Rows ?? []).map((row) => (row.Data ?? []).map((d) => d.VarCharValue ?? ""));
            return {
                queryExecutionId,
                state,
