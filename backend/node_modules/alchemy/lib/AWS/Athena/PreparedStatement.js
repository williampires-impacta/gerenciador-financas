 ...
 * @resource
 */
export const PreparedStatement = Resource("AWS.Athena.PreparedStatement");
const toAttributes = (ps) => ({
    statementName: ps.StatementName,
    workGroup: ps.WorkGroupName ?? "primary",
    queryStatement: ps.QueryStatement ?? "",
    description: ps.Description,
});
export const PreparedStatementProvider = () => Provider.effect(PreparedStatement, Effect.gen(function* () {
    // Statement names must match [a-zA-Z_][a-zA-Z0-9_@:]* — generate the
    // engine name and fold every other character to `_`.
    const toName = (id, props) => props.statementName
        ? Effect.succeed(props.statementName)
        : createPhysicalName({ id, maxLength: 128 }).pipe(Effect.map((n) => n.replace(/[^a-zA-Z0-9_]/g, "_")), Effect.map((n) => (/^[a-zA-Z_]/.test(n) ? n : `_${n}`)));
    const getOne = (workGroup, statementName) => athena
        .getPreparedStatement({
        WorkGroup: workGroup,
        StatementName: statementName,
    })
        .pipe(Effect.map((res) => res.PreparedStatement), Effect.catchTag(["ResourceNotFoundException", "WorkGroupNotFound"], () => Effect.succeed(undefined)));
    return {
        stables: ["statementName", "workGroup"],
        diff: Effect.fn(function* ({ id, olds, news }) {
            if (!isResolved(news))
                return;
            if ((yield* toName(id, olds ?? { queryStatement: "" })) !==
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            const name = output?.statementName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            else if (ps.QueryStatement !== news.queryStatement ||
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* athena
                ...
                .pipe(Effect.catchTag(["ResourceNotFoundException", "WorkGroupNotFound"], () => Effect.void));
        }),
    };
}));
