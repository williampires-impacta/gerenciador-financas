 ...
 * @resource
 */
export const DataCatalog = Resource("AWS.Athena.DataCatalog");
const observedTagsOf = (tags) => Object.fromEntries((tags ?? []).flatMap((t) => t.Key !== undefined && t.Value !== undefined ? [[t.Key, t.Value]] : []));
const paramsOf = (params) => Object.fromEntries(Object.entries(params ?? {}).flatMap((e) => typeof e[1] === "string" ? [[e[0], e[1]]] : []));
export const DataCatalogProvider = () => Provider.effect(DataCatalog, Effect.gen(function* () {
    const getOne = (name) => 
    ...
    athena.getDataCatalog({ Name: name, WorkGroup: "primary" }).pipe(Effect.map((res) => res.DataCatalog), Effect.catchTag("DataCatalogNotFound", () => Effect.succeed(undefined)));
    ...
    return {
        ...
        read: Effect.fn(function* ({ olds, output }) {
            ...
            if (!dc)
                return undefined;
            const arn = `arn:aws:athena:${region}:${accountId}:datacatalog/${name}`;
            return {
                name,
                dataCatalogArn: arn,
                type: dc.Type,
        ...
        list: () => Effect.gen(function* () {
            ...
            return Array.from(pages)
                ...
                .flatMap((dc) => dc.CatalogName
                ? [
                    {
                        name: dc.CatalogName,
                        dataCatalogArn: `arn:aws:athena:${region}:${accountId}:datacatalog/${dc.CatalogName}`,
                        type: dc.Type ?? "LAMBDA",
                        parameters: {},
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!dc) {
                ...
                const paramsDrift = Object.entries(desiredParams).some(([k, v]) => observedParams[k] !== v);
