...
import { AgentCoreProvisioningFailed, createAgentCoreName, readAgentCoreTags, retryWhileConflict, retryWhileValidation, syncAgentCoreTags, unredact, } from "./internal.js";
/**
 ...
 * @resource
 */
export const Runtime = Resource("AWS.BedrockAgentCore.Runtime");
/** Statuses indicating an in-flight transition to wait out. */
const RUNTIME_TRANSIENT = new Set(["CREATING", "UPDATING"]);
export const RuntimeProvider = () => Provider.effect(Runtime, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return props.agentRuntimeName ?? (yield* createAgentCoreName(id));
    });
    const getRuntimeOrUndefined = Effect.fn(function* (agentRuntimeId) {
    ...
    const waitForSettled = Effect.fn(function* (agentRuntimeId) {
        return yield* getRuntimeOrUndefined(agentRuntimeId).pipe(Effect.repeat({
            schedule: Schedule.fixed("5 seconds"),
            until: (r) => r === undefined || !RUNTIME_TRANSIENT.has(r.status),
            times: 60,
        }));
    });
    const toAttributes = (runtime) => ({
    ...
    return Runtime.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            const hydrated = yield* Effect.forEach(summaries, (s) => getRuntimeOrUndefined(s.agentRuntimeId), { concurrency: 5 });
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (runtime.status === "CREATE_FAILED" ||
            ...
            const drifted = (unredact(runtime.description) ?? undefined) !==
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* control
                ...
                .pipe(Effect.map((r) => r.status), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed("GONE")), Effect.repeat({
                schedule: Schedule.fixed("5 seconds"),
