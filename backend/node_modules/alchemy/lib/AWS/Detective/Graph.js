 ...
 * ```
 */
const GraphResource = Resource("AWS.Detective.Graph");
export { GraphResource as Graph };
const buildAttrs = (g) => ({
    graphArn: g.Arn,
    createdTime: g.CreatedTime?.toISOString(),
});
export const GraphProvider = () => Provider.effect(GraphResource, Effect.gen(function* () {
    ...
    const readTags = (arn) => detective.listTagsForResource({ ResourceArn: arn }).pipe(Effect.map((r) => (r.Tags ?? {})), Effect.catch(() => Effect.succeed({})));
    return {
