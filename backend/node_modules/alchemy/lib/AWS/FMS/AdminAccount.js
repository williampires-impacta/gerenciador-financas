...
export const pinFms = (effect) => effect.pipe(Effect.provideService(Region, Effect.succeed(FMS_ADMIN_REGION)));
/**
 ...
 * ```
 */
const AdminAccountResource = Resource("AWS.FMS.AdminAccount");
...
const getAdmin = pinFms(fms.getAdminAccount({})).pipe(Effect.map((r) => r), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
export const AdminAccountProvider = () => Provider.effect(AdminAccountResource, Effect.gen(function* () {
    // Association is asynchronous; the FMS role transitions CREATING → READY.
    const waitUntilReady = getAdmin.pipe(Effect.repeat({
    ...
    return {
        ...
        list: () => getAdmin.pipe(Effect.map((a) => a?.AdminAccount
        ...
        reconcile: Effect.fn(function* ({ news = {}, session }) {
            ...
            if (!admin?.AdminAccount) {
                ...
                yield* pinFms(fms.associateAdminAccount({ AdminAccount: desired })).pipe(Effect.retry({
        ...
        delete: Effect.fn(function* () {
            ...
            yield* pinFms(fms.disassociateAdminAccount({})).pipe(Effect.retry({
