 ...
 * @resource
 */
export const DeviceProfile = Resource("AWS.IoTWireless.DeviceProfile");
export const DeviceProfileProvider = () => Provider.effect(DeviceProfile, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.name ?? (yield* createPhysicalName({ id, maxLength: 256 })));
    });
    const getById = (deviceProfileId) => iotw
    ...
    // lost state write.
    const findByName = (name) => iotw.listDeviceProfiles.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    ...
    const toAttrs = Effect.fn(function* (profile, name) {
        if (profile.Id === undefined || profile.Arn === undefined) {
            return yield* Effect.fail(new Error(`IoT Wireless device profile '${name}' returned without Id/Arn`));
        }
        return {
    ...
    return DeviceProfile.Provider.of({
        ...
        list: () => iotw.listDeviceProfiles.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.DeviceProfileList ?? [])
            .flatMap((p) => p.Id !== undefined && p.Arn !== undefined
            ? [
                {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (profile === undefined) {
                return yield* Effect.fail(new Error(`IoT Wireless device profile '${name}' not found after create`));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* iotw.deleteDeviceProfile({ Id: output.deviceProfileId }).pipe(Effect.asVoid, Effect.catchTag("ResourceNotFoundException", () => Effect.void));
        }),
    });
}));
