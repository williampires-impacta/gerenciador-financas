 ...
 * @resource
 */
export const ServiceProfile = Resource("AWS.IoTWireless.ServiceProfile");
export const ServiceProfileProvider = () => Provider.effect(ServiceProfile, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.name ?? (yield* createPhysicalName({ id, maxLength: 256 })));
    });
    const getById = (serviceProfileId) => iotw
    ...
    // lost state write.
    const findByName = (name) => iotw.listServiceProfiles.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    ...
    const toAttrs = Effect.fn(function* (profile, name) {
        if (profile.Id === undefined || profile.Arn === undefined) {
            return yield* Effect.fail(new Error(`IoT Wireless service profile '${name}' returned without Id/Arn`));
        }
        return {
    ...
    return ServiceProfile.Provider.of({
        ...
        list: () => iotw.listServiceProfiles.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.ServiceProfileList ?? [])
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (profile === undefined) {
                return yield* Effect.fail(new Error(`IoT Wireless service profile '${name}' not found after create`));
            }
