 ...
 */
export const readIotWirelessTags = (arn) => iotw.listTagsForResource({ ResourceArn: arn }).pipe(Effect.map((r) => tagRecord(r.Tags ?? [])), Effect.catch(() => Effect.succeed({})));
/**
 * Converge the tags on an IoT Wireless resource to `desired`, diffing against
 * the OBSERVED cloud tags so adoption converges.
 */
export const syncIotWirelessTags = Effect.fn(function* (arn, desired) {
