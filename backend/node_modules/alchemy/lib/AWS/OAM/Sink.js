...
import { deleteSinkAndWait, readOamTags, retryOamMutation, syncOamTags, } from "./internal.js";
/**
 ...
 * @resource
 */
export const Sink = Resource("AWS.OAM.Sink");
/** Normalize a `policy` prop to a JSON string for `putSinkPolicy`. */
const toPolicyString = (policy) => typeof policy === "string" ? policy : JSON.stringify(policy);
/**
...
export const SinkProvider = () => Provider.effect(Sink, Effect.gen(function* () {
    ...
    const findByName = Effect.fn(function* (sinkName) {
    ...
    return Sink.Provider.of({
        ...
        list: () => oam.listSinks.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Items.filter((item) => item.Name != null && item.Arn != null && item.Id != null).map((item) => ({
            sinkName: item.Name,
            sinkArn: item.Arn,
            sinkId: item.Id,
        }))))),
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (live?.Arn == null) {
                live = yield* retryOamMutation(oam.createSink({ Name: sinkName, Tags: news?.tags })).pipe(Effect.catchTag("ConflictException", () => findByName(sinkName)));
            }
            ...
            if (desiredPolicy !== undefined) {
                ...
                if (!samePolicy(observed, desired)) {
                    yield* retryOamMutation(oam.putSinkPolicy({
