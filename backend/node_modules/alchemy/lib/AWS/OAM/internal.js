...
const isRetryableOamMutationError = (error, conflict) => {
    if (!error || typeof error !== "object")
        return false;
    const tag = error._tag;
    return (tag === "TooManyRequestsException" ||
...
export const retryOamMutation = (effect, options) => Retry.policy({
 ...
 */
export const deleteSinkAndWait = Effect.fn(function* (sinkArn) {
    yield* oam.listAttachedLinks.pages({ SinkIdentifier: sinkArn }).pipe(Stream.runCollect, Effect.map((pages) => Array.from(pages).reduce((count, page) => count + page.Items.length, 0)), Effect.flatMap((count) => count === 0
        ? Effect.void
        : Effect.fail(new SinkLinksStillAttached({ sinkArn, count }))), Effect.retry({
    ...
    yield* observe.pipe(Effect.flatMap(() => Effect.fail(new OamResourceStillExists({ arn: sinkArn }))), Effect.retry({
        while: (error) => error._tag === "OamResourceStillExists" ||
...
export const deleteLinkAndWait = Effect.fn(function* (linkArn) {
    yield* retryOamMutation(oam.deleteLink({ Identifier: linkArn })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void));
    const observe = Retry.none(oam.getLink({ Identifier: linkArn }));
    yield* observe.pipe(Effect.flatMap(() => Effect.fail(new OamResourceStillExists({ arn: linkArn }))), Effect.retry({
        while: (error) => error._tag === "OamResourceStillExists" ||
            error._tag === "TooManyRequestsException",
        schedule: goneSchedule,
    }), Effect.catchTag("ResourceNotFoundException", () => Effect.void));
 ...
 */
export const readOamTags = Effect.fn(function* (resourceArn) {
    return yield* oam.listTagsForResource({ ResourceArn: resourceArn }).pipe(Effect.map((r) => (r.Tags ?? {})), Effect.catchTag(["ResourceNotFoundException", "ValidationException"], () => Effect.succeed({})));
});
/**
 ...
 */
export const syncOamTags = Effect.fn(function* (resourceArn, id, userTags) {
    ...
    if (upsert.length > 0) {
        yield* retryOamMutation(oam.tagResource({
