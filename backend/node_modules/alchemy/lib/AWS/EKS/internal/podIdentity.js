...
import { createInternalTags, hasTags } from "../../../Tags.js";
export const toClientRequestToken = (id, action) => createPhysicalName({
    id: `${id}-${action}`,
    maxLength: 64,
    delimiter: "-",
});
 ...
 */
export const ensurePodRole = Effect.fn(function* ({ id, roleName, managedPolicyArns, }) {
 ...
 * providers.
 */
export const attachPolicyStatements = Effect.fn(function* ({ roleName, policyName, bindings, }) {
    const activeBindings = bindings.filter((binding) => binding.action !== "delete");
    const policyStatements = activeBindings.flatMap((binding) => binding?.data?.policyStatements?.map((statement) => ({
        ...statement,
...
export const findAssociation = Effect.fn(function* ({ clusterName, namespace, serviceAccount, }) {
...
export const ensureAssociation = Effect.fn(function* ({ id, clusterName, namespace, serviceAccount, roleArn, }) {
    ...
    if (!state) {
        ...
        if (!state) {
            return yield* Effect.fail(new Error(`PodIdentityAssociation '${namespace}/${serviceAccount}' could not be read after creation`));
        }
    }
...
export const deleteAssociation = Effect.fn(function* ({ clusterName, associationId, }) {
    yield* eks
 ...
 */
export const deletePodRole = Effect.fn(function* (roleName) {
...
//# sourceMappingURL=podIdentity.js.map