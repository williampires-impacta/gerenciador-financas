 ...
 * @resource
 */
export const PodIdentityAssociation = Resource("AWS.EKS.PodIdentityAssociation");
export const PodIdentityAssociationProvider = () => Provider.effect(PodIdentityAssociation, Effect.gen(function* () {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const perCluster = yield* Effect.forEach(clusterNames, (clusterName) => eks.listPodIdentityAssociations.pages({ clusterName }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.associations ?? [])), Effect.flatMap((summaries) => Effect.forEach(summaries, (summary) => summary.associationId
                ? readAssociationById({
                    clusterName,
                    associationId: summary.associationId,
                })
                : Effect.succeed(undefined), { concurrency: 5 }))), { concurrency: 5 });
            return perCluster
                .flat()
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!state) {
                yield* eks
                    ...
                    targetRoleArn: news.targetRoleArn,
                    policy: news.policy,
                    tags: desiredTags,
                ...
                if (!state) {
                    return yield* Effect.fail(new Error(`PodIdentityAssociation '${news.namespace}/${news.serviceAccount}' could not be read after creation`));
                }
            }
            // Sync mutable fields — observed ↔ desired.
            const desiredRoleArn = news.roleArn;
            const desiredTargetRoleArn = news.targetRoleArn;
            const desiredDisableSessionTags = news.disableSessionTags ?? false;
            if (state.roleArn !== desiredRoleArn ||
            ...
            if (!final) {
                return yield* Effect.fail(new Error(`PodIdentityAssociation '${state.associationId}' could not be read after reconcile`));
            }
...
const mapAssociation = (association) => ({
...
const readAssociationById = Effect.fn(function* ({ clusterName, associationId, }) {
...
const findAssociation = Effect.fn(function* ({ id, clusterName, namespace, serviceAccount, }) {
    let nextToken;
    let firstMatch;
    while (true) {
