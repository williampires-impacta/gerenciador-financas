 ...
 * @resource
 */
export const Addon = Resource("AWS.EKS.Addon");
...
const readAddon = Effect.fn(function* ({ clusterName, addonName, }) {
...
export const AddonProvider = () => Provider.effect(Addon, Effect.gen(function* () {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const perCluster = yield* Effect.forEach(clusterNames, (clusterName) => eks.listAddons.pages({ clusterName }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.addons ?? [])), Effect.flatMap((addonNames) => Effect.forEach(addonNames, (addonName) => readAddon({ clusterName, addonName }), { concurrency: 5 }))), { concurrency: 5 });
            return perCluster
                .flat()
        ...
        read: Effect.fn(function* ({ id, olds }) {
            const clusterName = olds.clusterName;
        ...
        reconcile: Effect.fn(function* ({ id, news, session }) {
            ...
            if (!state) {
                yield* eks
                    ...
                    serviceAccountRoleArn: news.serviceAccountRoleArn,
            ...
            const podIdentityChanged = JSON.stringify(state.podIdentityAssociations ?? []) !==
                JSON.stringify(news.podIdentityAssociations ?? []);
            if ((news.addonVersion !== undefined &&
                state.addonVersion !== news.addonVersion) ||
                state.serviceAccountRoleArn !==
                    news.serviceAccountRoleArn ||
...
const waitForAddonActive = Effect.fn(function* ({ clusterName, addonName, }) {
    return yield* readAddon({
        clusterName,
        addonName,
    }).pipe(Effect.flatMap((addon) => {
        if (!addon) {
            return Effect.fail(new AddonNotReady({
                clusterName,
                addonName,
                status: undefined,
            }));
        ...
        switch (addon.status) {
            ...
            case "UPDATE_FAILED":
            case "DELETE_FAILED":
                return Effect.fail(new Error(`Addon '${clusterName}/${addonName}' entered terminal status '${addon.status}'`));
            default:
                return Effect.fail(new AddonNotReady({
                    clusterName,
                    addonName,
...
const waitForAddonDeleted = Effect.fn(function* ({ clusterName, addonName, }) {
    yield* readAddon({
        clusterName,
        addonName,
    }).pipe(Effect.flatMap((addon) => addon
