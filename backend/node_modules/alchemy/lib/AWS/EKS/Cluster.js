...
import { deleteObjects, reconcileObjects, } from "../../Kubernetes/internal/client.js";
import {} from "../../Kubernetes/internal/objects.js";
import { eksConnectionOf, makeEksTransport } from "./KubernetesAdapter.js";
import { createPhysicalName } from "../../PhysicalName.js";
import * as Provider from "../../Provider.js";
import { Resource } from "../../Resource.js";
import { createInternalTags, diffTags, hasAlchemyTags, hasTags, } from "../../Tags.js";
/**
 ...
 * @resource
 */
export const Cluster = Resource("AWS.EKS.Cluster");
...
const getKubernetesTransport = (state) => Effect.suspend(() => {
    if (!state.endpoint || !state.certificateAuthorityData) {
        throw new Error(`EKS cluster '${state.clusterName}' is missing endpoint or certificate authority data`);
    }
...
const regionOfClusterArn = (arn) => arn?.split(":")[3] || undefined;
const getDesiredKubernetesObjects = (bindings) => bindings
    .filter((binding) => binding.data.type === "kubernetes-object")
 ...
 */
const applyAutoModeDefaults = (news, roles) => news.compute === "auto"
    ? {
        ...news,
...
const planClusterConfigUpdates = (observed, desired) => {
    ...
    const computeDrift = desiredCompute !== undefined &&
    ...
    const storageDrift = desiredBlockStorage !== undefined &&
        desiredBlockStorage !==
    ...
    const elbDrift = desiredElb !== undefined &&
        desiredElb !==
    ...
    const endpointPublicDrift = desiredVpc.endpointPublicAccess !== undefined &&
        desiredVpc.endpointPublicAccess !== observedVpc.endpointPublicAccess;
    const endpointPrivateDrift = desiredVpc.endpointPrivateAccess !== undefined &&
        desiredVpc.endpointPrivateAccess !== observedVpc.endpointPrivateAccess;
    const publicCidrsDrift = desiredVpc.publicAccessCidrs !== undefined &&
    ...
    const subnetsDrift = desiredVpc.subnetIds !== undefined &&
        !stringSetEqual(desiredVpc.subnetIds, observedVpc.subnetIds);
    const securityGroupsDrift = desiredVpc.securityGroupIds !== undefined &&
    ...
    if (desired.logging !== undefined) {
        ...
        if (Object.entries(desiredLogging).some(([type, enabled]) => (observedLogging[type] ?? false) !== enabled)) {
            updates.push({
                category: "logging",
    ...
    // Upgrade policy.
    if (desired.upgradePolicy?.supportType !== undefined &&
    ...
    // Deletion protection.
    if (desired.deletionProtection !== undefined &&
...
const mapClusterState = (cluster, tags, kubernetesObjects, managedRoles = {}) => ({
...
export const ClusterProvider = () => Provider.effect(Cluster, Effect.gen(function* () {
    ...
    const validateProps = Effect.fn(function* (props) {
        ...
        if (!props.roleArn && props.compute !== "auto") {
            return yield* Effect.fail(new Error("AWS.EKS.Cluster requires roleArn unless compute is 'auto'"));
        }
        if (props.computeConfig?.enabled &&
            props.accessConfig?.authenticationMode === "CONFIG_MAP") {
            return yield* Effect.fail(new Error("AWS.EKS.Cluster Auto Mode requires accessConfig.authenticationMode to include API access"));
        }
    });
    ...
    const ensureManagedRole = Effect.fn(function* ({ id, roleName, service, actions, managedPolicyArns, userTags, }) {
        ...
        const role = yield* iam
            ...
            .pipe(Effect.catchTag("EntityAlreadyExistsException", () => iam.getRole({ RoleName: roleName }).pipe(Effect.filterOrFail((existing) => hasTags(tags, existing.Role?.Tags), () => new Error(`Role '${roleName}' already exists and is not managed by alchemy`)))));
    ...
    const deleteManagedRole = Effect.fn(function* (roleName) {
        yield* iam.listAttachedRolePolicies.items({ RoleName: roleName }).pipe(Stream.mapEffect((policy) => iam
            .detachRolePolicy({
            RoleName: roleName,
    ...
    const readCluster = Effect.fn(function* ({ clusterName, kubernetesObjects, managedRoles, }) {
    ...
    const waitForClusterActive = (clusterName, kubernetesObjects = [], managedRoles) => readCluster({
        clusterName,
        kubernetesObjects,
        managedRoles,
    }).pipe(Effect.flatMap((state) => {
        if (!state) {
            return Effect.fail(new ClusterNotReady({
                status: undefined,
            }));
        }
    ...
    const waitForClusterDeleted = (clusterName) => readCluster({
        clusterName,
    }).pipe(Effect.flatMap((state) => state
        ? Effect.fail(new ClusterStillExists())
        : Effect.succeed(undefined)), Effect.retry({
        while: (error) => error instanceof ClusterStillExists,
        schedule: updateRetrySchedule,
    }));
    const waitForUpdate = (clusterName, updateId) => eks
        .describeUpdate({
        name: clusterName,
        updateId,
    })
        .pipe(Effect.flatMap(({ update }) => {
        if (update?.status === "Successful") {
            return Effect.succeed(update);
        }
        if (update?.status === "Failed" ||
            update?.status === "Cancelled") {
            return Effect.fail(new Error(`EKS cluster update '${updateId}' failed with status '${update?.status}'`));
        }
        return Effect.fail(new ClusterUpdateNotComplete({
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            const names = yield* eks.listClusters.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.clusters ?? [])));
            const states = yield* Effect.forEach(names, (clusterName) => readCluster({ clusterName }), { concurrency: 8 });
        ...
        reconcile: Effect.fn(function* ({ id, news, output, bindings, session, }) {
            ...
            for (const { category, request } of planClusterConfigUpdates(state, effective)) {
                ...
                if (configUpdate.update?.id) {
                    ...
                    state =
                        (yield* waitForClusterActive(clusterName, output?.kubernetesObjects ?? [], managedRoles)) ?? state;
                }
            }
            ...
            if (!final) {
                return yield* Effect.fail(new Error(`EKS cluster '${clusterName}' could not be read after reconcile`));
            }
