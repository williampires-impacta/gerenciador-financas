 ...
 * @resource
 */
export const EventDataStore = Resource("AWS.CloudTrail.EventDataStore");
/**
 ...
 * `AWS.providers()`.
 */
const retryWhileSettling = (self) => Effect.retry(self, {
 ...
 */
const retryWhileDeleteConflict = (self) => Effect.retry(self, {
    while: (e) => e._tag === "ConflictException",
    schedule: Schedule.max([Schedule.fixed("3 seconds"), Schedule.recurs(10)]),
});
const toWireSelectors = (selectors) => selectors?.map((s) => ({
    Name: s.name,
...
export const EventDataStoreProvider = () => Provider.effect(EventDataStore, Effect.gen(function* () {
    ...
    const fetchObservedTags = (arn) => cloudtrail.listTags({ ResourceIdList: [arn] }).pipe(Effect.map((r) => {
        const tags = {};
    ...
    const findByName = (name) => cloudtrail.listEventDataStores.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
        .flatMap((page) => page.EventDataStores ?? [])
        .find((s) => s.Name === name)));
    return EventDataStore.Provider.of({
        stables: ["eventDataStoreArn", "name"],
        list: () => Effect.gen(function* () {
            const stores = yield* cloudtrail.listEventDataStores.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.EventDataStores ?? [])));
            return stores
                .filter((s) => s.EventDataStoreArn !== undefined &&
                s.Name !== undefined &&
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let store = output?.eventDataStoreArn
                ? yield* getByArn(output.eventDataStoreArn)
                : yield* findByName(name).pipe(Effect.flatMap((s) => s?.EventDataStoreArn
            ...
            if (store !== undefined && store.Status === "PENDING_DELETION") {
                ...
                yield* retryWhileSettling(cloudtrail.restoreEventDataStore({
            ...
            if (store === undefined) {
                ...
                const created = yield* cloudtrail
                    ...
                    .pipe(Effect.catchTag("EventDataStoreAlreadyExistsException", () => Effect.succeed(undefined)));
                if (created?.EventDataStoreArn) {
                    ...
                    if (existing?.Status === "PENDING_DELETION") {
                        yield* session.note(`Name ${name} is held by a pending-deletion store — restoring it`);
                        yield* retryWhileSettling(cloudtrail.restoreEventDataStore({
            ...
            if (Object.keys(updateDelta).length > 0) {
                yield* session.note(`Updating event data store: ${Object.keys(updateDelta).join(", ")}`);
                store = yield* retryWhileSettling(cloudtrail.updateEventDataStore({
                    EventDataStore: arn,
                    ...updateDelta,
                }));
            }
            ...
            if (news.ingestionEnabled !== undefined) {
                ...
                if (news.ingestionEnabled && status === "STOPPED_INGESTION") {
                    ...
                    yield* retryWhileSettling(cloudtrail.startEventDataStoreIngestion({
                        EventDataStore: arn,
                    }));
                    store = (yield* getByArn(arn)) ?? store;
                }
                else if (!news.ingestionEnabled &&
                    status !== "STOPPED_INGESTION") {
                    yield* session.note(`Stopping ingestion for ${name}`);
                    yield* retryWhileSettling(cloudtrail.stopEventDataStoreIngestion({
                        EventDataStore: arn,
                    }));
                    store = (yield* getByArn(arn)) ?? store;
                }
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* retryWhileDeleteConflict(cloudtrail.deleteEventDataStore({
