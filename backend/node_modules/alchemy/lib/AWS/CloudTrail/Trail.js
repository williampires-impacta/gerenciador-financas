 ...
 * @resource
 */
export const Trail = Resource("AWS.CloudTrail.Trail");
/**
 ...
 * downstream consumer.
 */
const retryThroughBucketPropagation = (self) => Effect.retry(self, {
    while: (e) => e._tag === "InsufficientS3BucketPolicyException" ||
...
const toWireEventSelectors = (selectors) => selectors.map((s) => ({
...
const toWireAdvancedSelectors = (selectors) => selectors.map((s) => ({
    Name: s.name,
...
export const TrailProvider = () => Provider.effect(Trail, Effect.gen(function* () {
    const createTrailName = Effect.fn(function* (id, props) {
        if (props.trailName) {
            return props.trailName;
        }
        return yield* createPhysicalName({ id, maxLength: 128 });
    });
    const fetchObservedTags = (trailArn) => cloudtrail.listTags({ ResourceIdList: [trailArn] }).pipe(Effect.map((r) => {
        const tags = {};
    ...
    const readTrail = (name) => cloudtrail.getTrail({ Name: name }).pipe(Effect.map((r) => r.Trail), Effect.catchTag("TrailNotFoundException", () => Effect.succeed(undefined)));
    const toAttributes = Effect.fn(function* (trail) {
        const name = trail.Name;
        const isLogging = yield* cloudtrail
            .getTrailStatus({ Name: trail.TrailARN ?? name })
            .pipe(Effect.map((s) => s.IsLogging ?? false), Effect.catchTag("TrailNotFoundException", () => Effect.succeed(false)));
        return {
            trailName: name,
    ...
    return Trail.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            return yield* Effect.forEach(trails.filter((t) => t.Name !== undefined && t.TrailARN !== undefined), (t) => toAttributes(t), { concurrency: 5 });
        }),
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (trail === undefined) {
                ...
                yield* retryThroughBucketPropagation(cloudtrail.createTrail({
                    Name: name,
                    ...desired,
            ...
            if (desired.CloudWatchLogsLogGroupArn !== undefined &&
            ...
            if (desired.SnsTopicName !== undefined &&
            ...
            if (Object.keys(settingsDelta).length > 0) {
                yield* session.note(`Updating trail settings: ${Object.keys(settingsDelta).join(", ")}`);
                yield* retryThroughBucketPropagation(cloudtrail.updateTrail({ Name: name, ...settingsDelta }));
            ...
            if (news.eventSelectors !== undefined ||
                ...
                if (news.advancedEventSelectors !== undefined) {
                    ...
                    if (JSON.stringify(observedSelectors.AdvancedEventSelectors ?? []) !== JSON.stringify(desiredAdvanced)) {
                        yield* session.note(`Updating advanced event selectors`);
                        yield* retryWhileConflict(cloudtrail.putEventSelectors({
                            TrailName: trailArn,
                    ...
                    if (JSON.stringify(observedSelectors.EventSelectors ?? []) !==
            ...
            if (news.insightSelectors !== undefined) {
                const observedInsights = yield* cloudtrail
                    ...
                    .pipe(Effect.map((r) => r.InsightSelectors ?? []), Effect.catchTag("InsightNotEnabledException", () => Effect.succeed([])));
                const desiredInsights = news.insightSelectors.map((s) => ({
                    InsightType: s.insightType,
                }));
                if (JSON.stringify(observedInsights.map((s) => s.InsightType).sort()) !==
                    JSON.stringify(desiredInsights.map((s) => s.InsightType).sort())) {
                    yield* session.note(`Updating Insights selectors`);
                    yield* retryWhileConflict(cloudtrail.putInsightSelectors({
                        TrailName: trailArn,
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* retryWhileConflict(cloudtrail.deleteTrail({ Name: output.trailArn })).pipe(Effect.catchTag("TrailNotFoundException", () => Effect.void), 
