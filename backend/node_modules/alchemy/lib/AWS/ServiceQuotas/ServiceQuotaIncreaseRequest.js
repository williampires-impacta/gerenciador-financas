 ...
 * ```
 */
export const ServiceQuotaIncreaseRequest = Resource("AWS.ServiceQuotas.ServiceQuotaIncreaseRequest");
...
const OPEN_STATUSES = ["PENDING", "CASE_OPENED"];
...
const getRequest = (requestId) => servicequotas.getRequestedServiceQuotaChange({ RequestId: requestId }).pipe(Effect.map((r) => r.RequestedQuota), Effect.catchTag("NoSuchResourceException", () => Effect.succeed(undefined)));
// Observe the applied quota value; quotas without an applied (account-level)
// value fall back to the AWS default value.
const observeQuota = (serviceCode, quotaCode, contextId) => servicequotas
    .getServiceQuota({
    ServiceCode: serviceCode,
    QuotaCode: quotaCode,
    ContextId: contextId,
})
    .pipe(Effect.map((r) => r.Quota), Effect.catchTag("NoSuchResourceException", () => servicequotas
    .getAWSDefaultServiceQuota({
    ServiceCode: serviceCode,
    QuotaCode: quotaCode,
})
    .pipe(Effect.map((r) => r.Quota), Effect.catchTag("NoSuchResourceException", () => Effect.succeed(undefined)))));
// Find an open (PENDING / CASE_OPENED) increase request for the quota,
// optionally constrained to an exact desired value.
const findOpenRequest = Effect.fn(function* (serviceCode, quotaCode, desiredValue) {
    for (const status of OPEN_STATUSES) {
        const page = yield* servicequotas
            ...
            .pipe(Effect.map((r) => r.RequestedQuotas ?? []), Effect.catchTag("NoSuchResourceException", () => Effect.succeed([])));
...
const buildAttrs = (identity, request, quota) => ({
...
export const ServiceQuotaIncreaseRequestProvider = () => Provider.effect(ServiceQuotaIncreaseRequest, Effect.gen(function* () {
    return {
        stables: [
            "serviceCode",
            "quotaCode",
        ...
        list: () => servicequotas.listRequestedServiceQuotaChangeHistory.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.RequestedQuotas ?? [])
            .flatMap((request) => request.Id !== undefined
            ? [
                buildAttrs({
        ...
        read: Effect.fn(function* ({ olds, output }) {
            ...
            let request = output?.requestId !== undefined
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return undefined;
            if (news.serviceCode !== olds.serviceCode ||
            ...
            if (request === undefined) {
                ...
                if (!satisfied) {
                    request = yield* servicequotas
                        ...
                        .pipe(Effect.map((r) => r.RequestedQuota), 
                    ...
                    Effect.catchTag("ResourceAlreadyExistsException", () => findOpenRequest(serviceCode, quotaCode)));
                }
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if (output.requestId !== undefined &&
                ...
                yield* Effect.logWarning(`Quota increase request ${output.requestId} for ${output.serviceCode}/${output.quotaCode} is still open and cannot be cancelled via the Service Quotas API; it remains in AWS.`);
            }
        }),
    };
}));
//# sourceMappingURL=ServiceQuotaIncreaseRequest.js.map