...
import { extensionAssociationArn, readAppConfigTags, syncAppConfigTags, toTagRecord, } from "./internal.js";
/**
 ...
 * @resource
 */
export const ExtensionAssociation = Resource("AWS.AppConfig.ExtensionAssociation");
export const ExtensionAssociationProvider = () => Provider.effect(ExtensionAssociation, Effect.gen(function* () {
    const readAssociation = Effect.fn(function* (extensionAssociationId) {
     ...
     */
    const findByIdentifiers = Effect.fn(function* (props) {
        if (props.extensionIdentifier === undefined ||
        ...
        const summary = summaries.find((item) => item.Id !== undefined &&
    ...
    const toAttrs = Effect.fn(function* (association) {
        const { accountId, region } = yield* AWSEnvironment.current;
        return {
            extensionAssociationId: association.Id,
            extensionAssociationArn: association.Arn ??
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const summaries = yield* appconfig.listExtensionAssociations
                .pages({})
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Items ?? [])));
            return summaries.flatMap((summary) => summary.Id !== undefined &&
                summary.ExtensionArn !== undefined &&
                summary.ResourceArn !== undefined
                ? [
                    {
                        extensionAssociationId: summary.Id,
                        extensionAssociationArn: extensionAssociationArn(region, accountId, summary.Id),
