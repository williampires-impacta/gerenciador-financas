 ...
 * @resource
 */
export const Deployment = Resource("AWS.AppConfig.Deployment");
...
export const DeploymentProvider = () => Provider.effect(Deployment, Effect.gen(function* () {
    const readDeployment = Effect.fn(function* (applicationId, environmentId, deploymentNumber) {
    ...
    const waitForSettled = Effect.fn(function* (applicationId, environmentId, deploymentNumber) {
        return yield* readDeployment(applicationId, environmentId, deploymentNumber).pipe(Effect.flatMap((deployment) => deployment !== undefined &&
            !TERMINAL_STATES.has(deployment.State ?? "")
            ? Effect.fail(new DeploymentNotSettled({
                deploymentNumber,
                state: deployment.State ?? "",
            }))
            : Effect.succeed(deployment)), Effect.retry({
    ...
    return {
        ...
        read: Effect.fn(function* ({ output }) {
            if (output?.deploymentNumber === undefined)
                return undefined;
            const deployment = yield* readDeployment(output.applicationId, output.environmentId, output.deploymentNumber);
            if (deployment?.DeploymentNumber === undefined)
                return undefined;
            return {
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            // deployment.
            let observed = output?.deploymentNumber !== undefined
                ? yield* readDeployment(news.applicationId, news.environmentId, output.deploymentNumber)
                : undefined;
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* appconfig
                ...
                .pipe(Effect.catchTag(["ResourceNotFoundException", "BadRequestException"], () => Effect.void));
        }),
