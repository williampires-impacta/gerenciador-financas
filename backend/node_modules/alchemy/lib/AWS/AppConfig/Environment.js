...
import { environmentArn, readAppConfigTags, syncAppConfigTags, } from "./internal.js";
/**
 ...
 * @resource
 */
export const Environment = Resource("AWS.AppConfig.Environment");
const toWireMonitors = (monitors) => monitors?.map((m) => ({
    AlarmArn: m.alarmArn,
...
export const EnvironmentProvider = () => Provider.effect(Environment, Effect.gen(function* () {
    ...
    const readEnvironment = Effect.fn(function* (applicationId, environmentId) {
        ...
        list: () => Effect.gen(function* () {
            ...
            for (const app of apps) {
                ...
                for (const env of envs) {
                    if (env.Id === undefined || env.Name === undefined)
                        continue;
                    results.push({
                        environmentId: env.Id,
                        environmentName: env.Name,
