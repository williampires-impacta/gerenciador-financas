...
import { applicationArn, readAppConfigTags, syncAppConfigTags, } from "./internal.js";
/**
 ...
 * @resource
 */
export const Application = Resource("AWS.AppConfig.Application");
export const ApplicationProvider = () => Provider.effect(Application, Effect.gen(function* () {
    ...
    const readApplication = Effect.fn(function* (applicationId) {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            const { accountId, region } = yield* AWSEnvironment.current;
            const apps = yield* appconfig.listApplications.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Items ?? [])));
            return apps.flatMap((a) => a.Id !== undefined && a.Name !== undefined
                ? [
                    {
                        applicationId: a.Id,
