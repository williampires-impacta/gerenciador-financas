 ...
 * @resource
 */
export const VirtualCluster = Resource("AWS.EMRContainers.VirtualCluster");
export const VirtualClusterProvider = () => Provider.effect(VirtualCluster, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.virtualClusterName ??
            (yield* createPhysicalName({ id, maxLength: 64 })));
    });
    const toContainerProvider = (props) => ({
        type: props.type ?? "EKS",
        id: props.id,
        info: props.info,
    });
    const toAttributes = (vc) => ({
    ...
    const observeById = Effect.fn(function* (id) {
        return yield* emrc.describeVirtualCluster({ id }).pipe(Effect.map((response) => response.virtualCluster), Effect.catchTag(["ResourceNotFoundException", "ValidationException"], 
    ...
    const live = (state) => state === "RUNNING" || state === "ARRESTED";
    const observeByName = Effect.fn(function* (name) {
        return yield* emrc.listVirtualClusters
            .items({ states: ["RUNNING", "ARRESTED"] })
            .pipe(Stream.filter((vc) => vc.name === name), Stream.runHead, Effect.map(Option.getOrUndefined));
    });
    ...
    const syncTags = Effect.fn(function* (vc, desired) {
        // Diff against OBSERVED cloud tags (adoption may bring foreign tags).
        const observed = Object.fromEntries(Object.entries(vc.tags ?? {}).filter((entry) => entry[1] !== undefined));
    ...
    return VirtualCluster.Provider.of({
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            const vc = yield* emrc
                ...
                .pipe(Effect.map((response) => response.virtualCluster), Effect.catchTag(["ResourceNotFoundException", "ValidationException"], () => Effect.succeed(undefined)));
