 ...
 * @resource
 */
export const Portfolio = Resource("AWS.ServiceCatalog.Portfolio");
export const PortfolioProvider = () => Provider.effect(Portfolio, Effect.gen(function* () {
    ...
    const findByDisplayName = Effect.fn(function* (displayName) {
        return yield* servicecatalog.listPortfolios.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.PortfolioDetails ?? [])
            .find((p) => p.DisplayName === displayName)));
    });
    const observe = Effect.fn(function* (portfolioId) {
    ...
    return Portfolio.Provider.of({
        ...
        list: () => servicecatalog.listPortfolios.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.PortfolioDetails ?? [])
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryWhileResourceInUse(servicecatalog.deletePortfolio({ Id: output.portfolioId })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void));
        }),
    });
}));
