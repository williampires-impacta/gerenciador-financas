 ...
 * @resource
 */
export const AccessPolicy = Resource("AWS.OpenSearchServerless.AccessPolicy");
// OpenSearch Serverless only defines the "data" access-policy type.
const ACCESS_POLICY_TYPE = "data";
export const AccessPolicyProvider = () => Provider.effect(AccessPolicy, Effect.gen(function* () {
    ...
    return AccessPolicy.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let detail = yield* aoss
                .getAccessPolicy({ type: ACCESS_POLICY_TYPE, name })
                .pipe(Effect.map((r) => r.accessPolicyDetail), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
            // 2. ENSURE
            if (detail === undefined) {
                detail = yield* aoss
                    .createAccessPolicy({
                    ...
                    .pipe(Effect.map((r) => r.accessPolicyDetail), Effect.catchTag("ConflictException", () => aoss
                    .getAccessPolicy({ type: ACCESS_POLICY_TYPE, name })
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* retryWhileConflict(aoss.deleteAccessPolicy({
                type: ACCESS_POLICY_TYPE,
