...
import { awaitVpcEndpointActive, retryWhileConflict } from "./internal.js";
/**
 ...
 * @resource
 */
export const VpcEndpoint = Resource("AWS.OpenSearchServerless.VpcEndpoint");
export const VpcEndpointProvider = () => Provider.effect(VpcEndpoint, Effect.gen(function* () {
    ...
    const observe = Effect.fn(function* (name, vpcEndpointId) {
    ...
    return VpcEndpoint.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (detail === undefined) {
                const created = yield* aoss
                    ...
                    .pipe(Effect.map((r) => r.createVpcEndpointDetail), Effect.catchTag("ConflictException", () => Effect.succeed(undefined)));
            ...
            const addSecurityGroupIds = (news.securityGroupIds ?? []).filter((s) => !observedSgs.has(s));
            const removeSecurityGroupIds = (detail.securityGroupIds ?? []).filter((s) => !desiredSgs.has(s));
            if (addSubnetIds.length > 0 ||
                ...
                yield* aoss.updateVpcEndpoint({
                    ...
                    addSecurityGroupIds: addSecurityGroupIds.length > 0
                        ? addSecurityGroupIds
                        : undefined,
                    removeSecurityGroupIds: removeSecurityGroupIds.length > 0
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* retryWhileConflict(aoss.deleteVpcEndpoint({ id: output.vpcEndpointId })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void));
        }),
    });
}));
