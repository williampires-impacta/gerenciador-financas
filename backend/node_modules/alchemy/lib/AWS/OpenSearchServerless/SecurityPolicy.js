 ...
 * @resource
 */
export const SecurityPolicy = Resource("AWS.OpenSearchServerless.SecurityPolicy");
export const SecurityPolicyProvider = () => Provider.effect(SecurityPolicy, Effect.gen(function* () {
    ...
    return SecurityPolicy.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            const types = ["encryption", "network"];
            const results = [];
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            // 1. OBSERVE
            let detail = yield* aoss.getSecurityPolicy({ type, name }).pipe(Effect.map((r) => r.securityPolicyDetail), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
            ...
            if (detail === undefined) {
                detail = yield* aoss
                    ...
                    .pipe(Effect.map((r) => r.securityPolicyDetail), Effect.catchTag("ConflictException", () => aoss
                    .getSecurityPolicy({ type, name })
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* retryWhileConflict(aoss.deleteSecurityPolicy({
                type: output.type,
