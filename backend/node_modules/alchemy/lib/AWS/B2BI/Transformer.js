 ...
 * @resource
 */
export const Transformer = Resource("AWS.B2BI.Transformer");
const toAttrs = (r) => ({
...
const spec = (props) => JSON.stringify({
...
export const TransformerProvider = () => Provider.effect(Transformer, Effect.gen(function* () {
    const findByName = (name) => b2bi.listTransformers.items({}).pipe(Stream.filter((s) => s.name === name), Stream.runHead, Effect.flatMap((head) => head._tag === "Some"
        ? b2bi
    ...
    return {
        ...
        diff: Effect.fn(function* ({ olds, news }) {
            ...
            if (olds.status === "active") {
                const changed = spec(olds) !== spec(news) ||
                    olds.name !== news.name ||
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (live.status === "active" && (configDrift || nameDrift)) {
                return yield* Effect.fail(new Error(`Transformer '${live.transformerId}' is active and cannot be updated in place — B2BI rejects every update to an active transformer. Replace it instead.`));
            }
        ...
        list: () => b2bi.listTransformers.items({}).pipe(Stream.mapEffect((s) => b2bi.getTransformer({ transformerId: s.transformerId }).pipe(Effect.map(toAttrs), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)))), Stream.filter((item) => item !== undefined), Stream.runCollect, Effect.map((chunk) => Array.from(chunk))),
    };
}));
