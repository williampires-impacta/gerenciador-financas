 ...
 * @resource
 */
export const Profile = Resource("AWS.B2BI.Profile");
const toAttrs = (r) => ({
...
export const ProfileProvider = () => Provider.effect(Profile, Effect.gen(function* () {
    ...
    const findByName = (name) => b2bi.listProfiles.items({}).pipe(Stream.filter((s) => s.name === name), Stream.runHead, Effect.flatMap((head) => head._tag === "Some"
        ? b2bi
    ...
    return {
        ...
        list: () => b2bi.listProfiles.items({}).pipe(Stream.mapEffect((s) => b2bi.getProfile({ profileId: s.profileId }).pipe(Effect.map(toAttrs), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)))), Stream.filter((item) => item !== undefined), Stream.runCollect, Effect.map((chunk) => Array.from(chunk))),
    };
}));
