 ...
 * @resource
 */
export const Partnership = Resource("AWS.B2BI.Partnership");
const toAttrs = (r) => ({
...
export const PartnershipProvider = () => Provider.effect(Partnership, Effect.gen(function* () {
    const findByName = (name) => b2bi.listPartnerships.items({}).pipe(Stream.filter((s) => s.name === name), Stream.runHead, Effect.flatMap((head) => head._tag === "Some"
        ? b2bi
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (live === undefined) {
                ...
                const capsDrift = JSON.stringify([...(live.capabilities ?? [])].sort()) !==
                    JSON.stringify([...(news.capabilities ?? [])].sort());
                const optsDrift = JSON.stringify(live.capabilityOptions ?? null) !==
        ...
        list: () => b2bi.listPartnerships.items({}).pipe(Stream.mapEffect((s) => b2bi.getPartnership({ partnershipId: s.partnershipId }).pipe(Effect.map(toAttrs), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)))), Stream.filter((item) => item !== undefined), Stream.runCollect, Effect.map((chunk) => Array.from(chunk))),
    };
}));
