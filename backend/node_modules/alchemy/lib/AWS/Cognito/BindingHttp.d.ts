 ...
 */
export declare const cognitoMethods: (bindingId: string, logicalId: string) => {
    /**
     ...
     */
    injecting: <I extends object>(inject: Effect.Effect<I>) => <A extends I, O, E>(name: string, op: (input: A) => Effect.Effect<O, E>) => ((request: Omit<A, keyof I>) => Effect.Effect<O, E>);
    ...
    plain: <A, O, E>(name: string, op: (input: A) => Effect.Effect<O, E>) => ((request: A) => Effect.Effect<O, E>);
};
