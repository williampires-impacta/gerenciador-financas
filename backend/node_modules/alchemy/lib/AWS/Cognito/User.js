 ...
 * @resource
 */
export const User = Resource("AWS.Cognito.User");
...
const attributeRecordOf = (attributes) => Object.fromEntries((attributes ?? []).flatMap((attribute) => {
...
export const UserProvider = () => Provider.effect(User, Effect.gen(function* () {
    ...
    const getUser = Effect.fn(function* (userPoolId, username) {
        return yield* cip
            .adminGetUser({ UserPoolId: userPoolId, Username: username })
            .pipe(Effect.catchTag(["UserNotFoundException", "ResourceNotFoundException"], () => Effect.succeed(undefined)));
    });
    const attributesOf = (username, userPoolId, observed) => ({
        username,
        userPoolId,
    ...
    return User.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, output, session }) {
            ...
            if (observed === undefined) {
                yield* cip
                    ...
                    UserAttributes: Object.entries(desiredAttributes).map(([Name, Value]) => ({ Name, Value })),
                })
                ...
                const upsert = Object.entries(desiredAttributes).filter(([name, value]) => observedAttributes[name] !== value);
                if (upsert.length > 0) {
                ...
                const removed = Object.keys(olds?.attributes ?? {}).filter((name) => desiredAttributes[name] === undefined &&
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* cip
                ...
                .pipe(Effect.catchTag(["UserNotFoundException", "ResourceNotFoundException"], () => Effect.void));
        }),
    });
}));
