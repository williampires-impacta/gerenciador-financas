...
export interface IdentityPoolAuthClient {
    ...
    getCredentialsForIdentity: (request: ci.GetCredentialsForIdentityInput) => Effect.Effect<ci.GetCredentialsForIdentityResponse, ci.GetCredentialsForIdentityError>;
    /** Exchange an identity ID (+ logins) for an OpenID Connect token
     * (basic/classic flow — requires `allowClassicFlow`). */
    getOpenIdToken: (request: ci.GetOpenIdTokenInput) => Effect.Effect<ci.GetOpenIdTokenResponse, ci.GetOpenIdTokenError>;
    /** Unlink a federated login from an identity. */
    unlinkIdentity: (request: ci.UnlinkIdentityInput) => Effect.Effect<ci.UnlinkIdentityResponse, ci.UnlinkIdentityError>;
}
/**
 ...
 * @binding
 */
export interface IdentityPoolAuth extends Binding.Service<IdentityPoolAuth, "AWS.Cognito.IdentityPoolAuth", <P extends IdentityPool>(pool: P) => Effect.Effect<IdentityPoolAuthClient>> {
}
export declare const IdentityPoolAuth: IdentityPoolAuth;
//# sourceMappingURL=IdentityPoolAuth.d.ts.map