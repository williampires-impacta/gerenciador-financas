 ...
 * @resource
 */
export const IdentityPoolRoleAttachment = Resource("AWS.Cognito.IdentityPoolRoleAttachment");
/**
...
export const IdentityPoolRoleAttachmentProvider = () => Provider.effect(IdentityPoolRoleAttachment, Effect.gen(function* () {
    const getRoles = Effect.fn(function* (identityPoolId) {
        return yield* ci
    ...
    return IdentityPoolRoleAttachment.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            const observed = yield* getRoles(identityPoolId);
            const observedRoles = Object.fromEntries(Object.entries(observed?.Roles ?? {}).filter((entry) => entry[1] !== undefined));
            // 2/3. ENSURE + SYNC — SetIdentityPoolRoles is a full PUT; skip it
            //      when the observed map already matches.
            const same = JSON.stringify(Object.entries(observedRoles).sort(([a], [b]) => a.localeCompare(b))) ===
                JSON.stringify(Object.entries(desired).sort(([a], [b]) => a.localeCompare(b)));
            if (!same) {
                yield* retryThroughIamPropagation(ci.setIdentityPoolRoles({
