 ...
 * @resource
 */
export const Thing = Resource("AWS.IoT.Thing");
export const ThingProvider = () => Provider.effect(Thing, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return props.thingName ?? (yield* createPhysicalName({ id }));
    });
    return Thing.Provider.of({
        stables: ["thingName", "thingArn"],
        list: () => iot.listThings.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.things ?? [])
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
            ...
            if (live === undefined) {
                ...
                const typeChanged = (news.thingTypeName ?? undefined) !==
                    (live.thingTypeName ?? undefined);
                if (attrsChanged || typeChanged) {
                    yield* iot.updateThing({
                        thingName,
                        thingTypeName: news.thingTypeName,
                        removeThingType: news.thingTypeName === undefined &&
                            live.thingTypeName !== undefined
                            ? true
                            : undefined,
