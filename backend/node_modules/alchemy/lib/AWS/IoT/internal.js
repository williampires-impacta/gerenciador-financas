 ...
 */
export const sanitizeRuleName = (name) => name.replace(/[^a-zA-Z0-9_]/g, "_");
/**
 * Reads the observed tag map for an IoT resource ARN. A missing resource
 * yields `{}` so callers treat it as "no tags observed".
 */
export const readIotTags = Effect.fn(function* (resourceArn) {
 ...
 */
export const syncIotTags = Effect.fn(function* (resourceArn, id, userTags) {
