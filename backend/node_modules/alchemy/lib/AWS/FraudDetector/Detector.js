 ...
 * @resource
 */
export const Detector = Resource("AWS.FraudDetector.Detector");
export const DetectorProvider = () => Provider.effect(Detector, Effect.gen(function* () {
    const createId = Effect.fn(function* (id, props) {
        return (props.detectorId ??
            (yield* createPhysicalName({ id, maxLength: 64, lowercase: true })));
    });
    const get = Effect.fn(function* (detectorId) {
        const response = yield* frauddetector
            .getDetectors({ detectorId })
            .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
        return response?.detectors?.[0];
    });
    const toAttrs = (detector) => ({
        detectorId: detector.detectorId,
        arn: detector.arn,
    ...
    return {
        ...
        list: () => frauddetector.getDetectors.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.detectors ?? []).map(toAttrs)))),
    };
}));
