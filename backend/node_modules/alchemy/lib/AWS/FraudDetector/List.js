 ...
 * @resource
 */
export const List = Resource("AWS.FraudDetector.List");
/** Coerce a distilled sensitive element (`string | Redacted<string>`) to a plain string. */
const toElement = (e) => typeof e === "string" ? e : Redacted.value(e);
export const ListProvider = () => Provider.effect(List, Effect.gen(function* () {
    ...
    const getElements = Effect.fn(function* (name) {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, session }) {
            ...
            if (list === undefined) {
                ...
                const elementsChanged = desiredSet.size !== observedSet.size ||
                ...
                const variableTypeIntroduced = list.variableType === undefined &&
        ...
        list: () => frauddetector.getListsMetadata.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.lists ?? []).map(toAttrs)))),
    };
}));
