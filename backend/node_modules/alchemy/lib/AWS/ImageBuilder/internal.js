 ...
 * immutability guard.
 */
export const immutableVersionKeysChanged = (olds, news, keys) => {
    ...
    if (Output.isExpr(news) || Effect.isEffect(news))
        return false;
    const desired = news;
 ...
 * deleted (bounded).
 */
export const retryWhileDependedOn = (self) => Effect.retry(self, {
 ...
 */
export const deleteImageBuilderLogGroup = Effect.fn(function* (logGroupName) {
    ...
    for (let attempt = 0; attempt < 20; attempt++) {
        const present = yield* logs.describeLogGroups
            ...
            .pipe(Stream.filter((group) => group.logGroupName === logGroupName), Stream.runHead, Effect.map(Option.isSome));
        if (!present)
            return;
        yield* Effect.sleep("500 millis");
    }
    return yield* Effect.die(new Error(`Image Builder log group ${logGroupName} remained observable 10 seconds after delete`));
});
/**
 ...
 */
export const readImageBuilderTags = Effect.fn(function* (arn) {
 ...
 */
export const syncImageBuilderTags = Effect.fn(function* (arn, desiredTags) {
 ...
 */
export const imageBuilderArn = Effect.fn(function* (resourceType, resourcePath) {
 ...
 * element-wise.
 */
export const driftedFrom = (observed, desired) => {
    ...
    if (typeof desired === "object" && desired !== null) {
        if (typeof observed !== "object" || observed === null)
            return true;
        return Object.entries(desired).some(([key, value]) => driftedFrom(observed[key], value));
    }
