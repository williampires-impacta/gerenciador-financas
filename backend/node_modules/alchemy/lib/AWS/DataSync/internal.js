...
export const dsTagsToRecord = (tags) => Object.fromEntries((tags ?? []).map((t) => [t.Key, t.Value ?? ""]));
/** Read the observed tags currently attached to a DataSync resource. */
export const readObservedTags = Effect.fn(function* (resourceArn) {
 ...
 */
export const syncTags = Effect.fn(function* (resourceArn, observed, desired) {
 ...
 * loss.
 */
export const findLocationArnByUri = Effect.fn(function* (expectedUri) {
    const strip = (u) => u.replace(/\/+$/, "");
    const target = strip(expectedUri);
    const locations = yield* datasync.listLocations.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((p) => p.Locations ?? [])));
    return locations.find((l) => l.LocationUri !== undefined && strip(l.LocationUri) === target)?.LocationArn;
});
