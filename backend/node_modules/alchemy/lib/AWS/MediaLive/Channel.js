...
import { MediaLiveResourcePending, ensureIdentified, ensurePresent, retryWhileConflict, retryWhilePending, syncMlTags, toTagRecord, } from "./internal.js";
...
const retryWhileRolePropagating = (self) => Effect.retry(self, {
 ...
 * @resource
 */
export const Channel = Resource("AWS.MediaLive.Channel");
export const ChannelProvider = () => Provider.effect(Channel, Effect.gen(function* () {
    ...
    const getChannel = Effect.fn(function* (channelId) {
        const channel = yield* medialive
            .describeChannel({ ChannelId: channelId })
            .pipe(Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
        if (channel === undefined || isGone(channel.State))
            return undefined;
    ...
    const findByName = Effect.fn(function* (name) {
        const channels = yield* medialive.listChannels.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
     ...
     */
    const awaitSettled = Effect.fn(function* (channelId) {
        return yield* retryWhilePending(Effect.gen(function* () {
            const channel = yield* medialive.describeChannel({
                ChannelId: channelId,
            });
            if (channel.State === "CREATE_FAILED" ||
                channel.State === "UPDATE_FAILED") {
                return yield* Effect.fail(new MediaLiveChannelFailed({
                    message: `channel ${channelId} is in state ${channel.State}`,
                }));
            }
            if (channel.State === "CREATING" || channel.State === "UPDATING") {
                return yield* Effect.fail(new MediaLiveResourcePending({
    ...
    return Channel.Provider.of({
        ...
        list: () => medialive.listChannels.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .filter((channel) => channel.Id !== undefined &&
            channel.Arn !== undefined &&
            !isGone(channel.State))
            .map(toAttrs))),
        read: Effect.fn(function* ({ id, olds, output }) {
            const channel = output?.channelId !== undefined
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, output, session }) {
            ...
            if (channel === undefined) {
                ...
                const deepDrift = olds === undefined
                    ? true
                    : JSON.stringify({
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* medialive.deleteChannel({ ChannelId: output.channelId }).pipe(retryWhileConflict, Effect.catchTag("NotFoundException", () => Effect.void), Effect.asVoid);
            ...
            yield* medialive
                ...
                .pipe(Effect.flatMap((channel) => channel.State === "DELETED"
                ? Effect.void
                : Effect.fail(new MediaLiveResourcePending({
                    message: `channel ${output.channelId} is still ${channel.State}`,
                }))), retryWhilePending, Effect.catchTag("NotFoundException", () => Effect.void), Effect.catchTag("MediaLiveResourcePending", () => Effect.void));
        }),
    });
}));
