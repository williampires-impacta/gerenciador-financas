...
import { MediaLiveResourcePending, ensureIdentified, retryWhileConflict, retryWhilePending, syncMlTags, toTagRecord, } from "./internal.js";
/**
 ...
 * @resource
 */
export const Input = Resource("AWS.MediaLive.Input");
export const InputProvider = () => Provider.effect(Input, Effect.gen(function* () {
    ...
    const findByName = Effect.fn(function* (name) {
        const inputs = yield* medialive.listInputs.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
    ...
    const awaitActive = Effect.fn(function* (inputId) {
        return yield* medialive.describeInput({ InputId: inputId }).pipe(Effect.flatMap((input) => input.State === "CREATING"
            ? Effect.fail(new MediaLiveResourcePending({
                message: `input ${inputId} is still CREATING`,
            }))
            : Effect.succeed(input)), retryWhilePending, Effect.flatMap((input) => ensureIdentified(input, "DescribeInput Id/Arn")));
    });
    return Input.Provider.of({
        stables: ["inputId", "inputArn"],
        list: () => medialive.listInputs.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .filter((input) => input.Id !== undefined &&
            input.Arn !== undefined &&
            !isGone(input.State))
            .map(toAttrs))),
        read: Effect.fn(function* ({ id, olds, output }) {
            const input = output?.inputId !== undefined
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, output, session }) {
            ...
            if (input === undefined) {
                ...
                const sameStringSet = (a, b) => JSON.stringify([...a].sort()) === JSON.stringify([...b].sort());
                const nameDrift = input.Name !== name;
                const roleDrift = news.roleArn !== undefined && input.RoleArn !== news.roleArn;
                const sgDrift = !sameStringSet(news.inputSecurityGroups ?? [], input.SecurityGroups ?? []);
                const sourceDrift = !sameStringSet((news.sources ?? []).map((s) => s.Url), (input.Sources ?? []).map((s) => s.Url));
                const flowDrift = !sameStringSet((news.mediaConnectFlows ?? []).map((f) => f.FlowArn), (input.MediaConnectFlows ?? []).map((f) => f.FlowArn));
                const destinationDrift = olds === undefined
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* medialive.deleteInput({ InputId: output.inputId }).pipe(retryWhileConflict, Effect.catchTag("NotFoundException", () => Effect.void));
        }),
    });
}));
