 ...
 * @resource
 */
export const OptIn = Resource("AWS.LakeFormation.OptIn");
export const OptInProvider = () => Provider.effect(OptIn, Effect.gen(function* () {
    const observe = Effect.fn(function* (principal, resource) {
        ...
        Effect.catchTag(["EntityNotFoundException", "InvalidLakeFormationPrincipal"], () => Effect.succeed([])));
    ...
    return OptIn.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            return Array.from(pages)
                .flatMap((page) => page.LakeFormationOptInsInfoList ?? [])
                .filter((info) => info.Principal?.DataLakePrincipalIdentifier !== undefined &&
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            Effect.catchTag(["EntityNotFoundException", "InvalidLakeFormationPrincipal"], () => Effect.void));
        }),
    });
}));
