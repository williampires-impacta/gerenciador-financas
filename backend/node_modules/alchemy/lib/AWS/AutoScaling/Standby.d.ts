 ...
 */
export interface EnterStandbyRequest extends Omit<autoscaling.EnterStandbyQuery, "AutoScalingGroupName"> {
}
/**
 * `ExitStandby` request with `AutoScalingGroupName` injected from the bound
 * {@link AutoScalingGroup}.
 */
export interface ExitStandbyRequest extends Omit<autoscaling.ExitStandbyQuery, "AutoScalingGroupName"> {
}
export interface StandbyClient {
     ...
     */
    enter: (request: EnterStandbyRequest) => Effect.Effect<autoscaling.EnterStandbyAnswer, autoscaling.EnterStandbyError>;
    /**
     * Move standby instances back into service; the desired capacity is
     * incremented accordingly.
     */
    exit: (request: ExitStandbyRequest) => Effect.Effect<autoscaling.ExitStandbyAnswer, autoscaling.ExitStandbyError>;
}
/**
 ...
 * @binding
 */
export interface Standby extends Binding.Service<Standby, "AWS.AutoScaling.Standby", (group: AutoScalingGroup) => Effect.Effect<StandbyClient>> {
}
export declare const Standby: Standby;
