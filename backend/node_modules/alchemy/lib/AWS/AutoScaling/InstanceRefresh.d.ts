 ...
 */
export interface StartInstanceRefreshRequest extends Omit<autoscaling.StartInstanceRefreshType, "AutoScalingGroupName"> {
}
/**
 * `CancelInstanceRefresh` request with `AutoScalingGroupName` injected from
 * the bound {@link AutoScalingGroup}.
 */
export interface CancelInstanceRefreshRequest extends Omit<autoscaling.CancelInstanceRefreshType, "AutoScalingGroupName"> {
}
/**
 * `DescribeInstanceRefreshes` request with `AutoScalingGroupName` injected
 * from the bound {@link AutoScalingGroup}.
 */
export interface DescribeInstanceRefreshesRequest extends Omit<autoscaling.DescribeInstanceRefreshesType, "AutoScalingGroupName"> {
}
export interface InstanceRefreshClient {
     ...
     */
    start: (request?: StartInstanceRefreshRequest) => Effect.Effect<autoscaling.StartInstanceRefreshAnswer, autoscaling.StartInstanceRefreshError>;
    /**
     * Cancel the in-progress refresh. Instances already replaced are not rolled
     * back — use `rollback` for that.
     */
    cancel: (request?: CancelInstanceRefreshRequest) => Effect.Effect<autoscaling.CancelInstanceRefreshAnswer, autoscaling.CancelInstanceRefreshError>;
    /**
     * Roll the group back to the configuration it had before the in-progress
     * refresh started.
     */
    rollback: () => Effect.Effect<autoscaling.RollbackInstanceRefreshAnswer, autoscaling.RollbackInstanceRefreshError>;
    /**
     * Page through the group's instance refreshes (newest first), including
     * per-refresh status and progress details.
     */
    describe: (request?: DescribeInstanceRefreshesRequest) => Effect.Effect<autoscaling.DescribeInstanceRefreshesAnswer, autoscaling.DescribeInstanceRefreshesError>;
}
/**
 ...
 * @binding
 */
export interface InstanceRefresh extends Binding.Service<InstanceRefresh, "AWS.AutoScaling.InstanceRefresh", (group: AutoScalingGroup) => Effect.Effect<InstanceRefreshClient>> {
}
export declare const InstanceRefresh: InstanceRefresh;
//# sourceMappingURL=InstanceRefresh.d.ts.map