 ...
 * @resource
 */
export const SecurityConfiguration = Resource("AWS.EMR.SecurityConfiguration");
/** Serialize the user-facing document prop to the wire JSON string. */
const toDocument = (doc) => typeof doc === "string" ? doc : JSON.stringify(doc);
...
export const SecurityConfigurationProvider = () => Provider.effect(SecurityConfiguration, Effect.gen(function* () {
    const toName = (id, props) => props.securityConfigurationName
        ? Effect.succeed(props.securityConfigurationName)
        : createPhysicalName({ id, maxLength: 64 });
    const readConfiguration = Effect.fn(function* (name) {
        return yield* emr
            .describeSecurityConfiguration({ Name: name })
            .pipe(Effect.catchTag("SecurityConfigurationNotFound", () => Effect.succeed(undefined)));
    });
    const toAttrs = (name, document) => ({
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.securityConfigurationName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            //      race, not a failure.
            if (observed?.SecurityConfiguration !== undefined &&
        ...
        list: () => emr.listSecurityConfigurations.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((summary) => summary.Name ? [summary.Name] : [])), Effect.flatMap(Effect.forEach((name) => readConfiguration(name).pipe(Effect.map((found) => found === undefined
            ? []
            : [toAttrs(name, found.SecurityConfiguration ?? "")])), { concurrency: 4 })), Effect.map((results) => results.flat())),
    };
}));
