 ...
 * @resource
 */
export const CertificateAuthorityPolicy = Resource("AWS.ACMPCA.CertificateAuthorityPolicy");
export const CertificateAuthorityPolicyProvider = () => Provider.effect(CertificateAuthorityPolicy, Effect.gen(function* () {
    ...
    // policy to observe.
    const observe = (certificateAuthorityArn) => acmpca.getPolicy({ ResourceArn: certificateAuthorityArn }).pipe(Effect.map((response) => response.Policy), Effect.catchTag(["ResourceNotFoundException", "InvalidStateException"], () => Effect.succeed(undefined)));
    ...
    return {
        ...
        read: Effect.fn(function* ({ olds, output }) {
            const certificateAuthorityArn = output?.certificateAuthorityArn ?? olds?.certificateAuthorityArn;
            if (certificateAuthorityArn === undefined)
                return undefined;
        ...
        list: () => Effect.gen(function* () {
            ...
            const cas = yield* acmpca.listCertificateAuthorities.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
                ...
                .map((ca) => ca.Arn)));
            const policies = yield* Effect.forEach(cas, (arn) => observe(arn).pipe(Effect.map((policy) => policy === undefined
                ? []
                : [{ certificateAuthorityArn: arn, policy }])), { concurrency: 5 });
...
//# sourceMappingURL=CertificateAuthorityPolicy.js.map