...
import { RedshiftStatementFailed, Statements, } from "./Statements.js";
/** Statement statuses that mean the run is over. */
const isTerminal = (status) => status === "FINISHED" || status === "FAILED" || status === "ABORTED";
export const StatementsHttp = Layer.effect(Statements, Effect.gen(function* () {
    ...
    return Effect.fn(function* (workgroup, options = {}) {
        ...
        const execute = Effect.fn(`AWS.RedshiftData.Statements.execute(${workgroup.LogicalId})`)(function* (request) {
        ...
        const executeBatch = Effect.fn(`AWS.RedshiftData.Statements.executeBatch(${workgroup.LogicalId})`)(function* (request) {
        ...
        const describe = Effect.fn(`AWS.RedshiftData.Statements.describe(${workgroup.LogicalId})`)(function* (id) {
            return yield* describeStatement({ Id: id });
        });
        const cancel = Effect.fn(`AWS.RedshiftData.Statements.cancel(${workgroup.LogicalId})`)(function* (id) {
            return yield* cancelStatement({ Id: id });
        });
        const getResult = Effect.fn(`AWS.RedshiftData.Statements.getResult(${workgroup.LogicalId})`)(function* (id, nextToken) {
            return yield* getStatementResult({ Id: id, NextToken: nextToken });
        });
        const getResultV2 = Effect.fn(`AWS.RedshiftData.Statements.getResultV2(${workgroup.LogicalId})`)(function* (id, nextToken) {
            return yield* getStatementResultV2({ Id: id, NextToken: nextToken });
        });
        const describeTable = Effect.fn(`AWS.RedshiftData.Statements.describeTable(${workgroup.LogicalId})`)(function* (request) {
        ...
        const listDatabases = Effect.fn(`AWS.RedshiftData.Statements.listDatabases(${workgroup.LogicalId})`)(function* (request = {}) {
        ...
        const listSchemas = Effect.fn(`AWS.RedshiftData.Statements.listSchemas(${workgroup.LogicalId})`)(function* (request = {}) {
        ...
        const listTables = Effect.fn(`AWS.RedshiftData.Statements.listTables(${workgroup.LogicalId})`)(function* (request = {}) {
        ...
        const listStatements = Effect.fn(`AWS.RedshiftData.Statements.listStatements(${workgroup.LogicalId})`)(function* (request = {}) {
            return yield* listStatementsOp({
                ...request,
                WorkgroupName: yield* workgroupName,
            });
        });
        const query = Effect.fn(`AWS.RedshiftData.Statements.query(${workgroup.LogicalId})`)(function* (sql, parameters) {
            ...
            const described = yield* describe(id).pipe(Effect.repeat({
                schedule: Schedule.spaced("5 seconds"),
                until: (r) => isTerminal(r.Status),
                times: 30,
            }));
            if (described.Status !== "FINISHED") {
                return yield* Effect.fail(new RedshiftStatementFailed({
                    statementId: id,
...
//# sourceMappingURL=StatementsHttp.js.map