...
import { toWireHours } from "../../Util/Duration.js";
 ...
 * @resource
 */
export const Stream = Resource("AWS.Kinesis.Stream");
const defaultStreamMode = "ON_DEMAND";
const defaultRetentionPeriodHours = 24;
const defaultEncryptionType = "NONE";
const createStreamName = (id, props) => Effect.gen(function* () {
    if (props.streamName) {
...
const getStreamMode = (props) => ({
    StreamMode: props.streamMode ?? defaultStreamMode,
});
const assertProvisionedProps = (props) => props.streamMode === "PROVISIONED" && props.shardCount === undefined
...
const readStream = Effect.fn(function* ({ streamName, streamArn, }) {
...
const waitForStreamActive = (streamName) => Effect.gen(function* () {
    yield* Effect.sleep("2 seconds");
    const { StreamDescriptionSummary } = yield* kinesis.describeStreamSummary({
        StreamName: streamName,
    });
    if (StreamDescriptionSummary.StreamStatus !== "ACTIVE") {
        return yield* Effect.fail({ _tag: "StreamNotActive" });
    }
    return StreamDescriptionSummary;
}).pipe(Effect.retry({
    while: (e) => e._tag === "StreamNotActive" || e._tag === "ParseError",
    schedule: Schedule.max([Schedule.exponential(500), Schedule.recurs(60)]),
}));
const waitForStreamDeleted = (streamName) => Effect.gen(function* () {
    yield* kinesis.describeStreamSummary({
        StreamName: streamName,
    });
    return yield* Effect.fail({ _tag: "StreamStillExists" });
}).pipe(Effect.retry({
    while: (e) => e._tag === "StreamStillExists" || e._tag === "ParseError",
    schedule: Schedule.max([Schedule.exponential(500), Schedule.recurs(60)]),
...
export const StreamProvider = () => Provider.effect(Stream, Effect.gen(function* () {
    return {
        ...
        list: () => Effect.gen(function* () {
            const streamNames = yield* kinesis.listStreams.pages({}).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.StreamNames ?? [])));
            const hydrated = yield* Effect.forEach(streamNames, (streamName) => readStream({ streamName }), { concurrency: 10 });
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
            ...
            if (state === undefined) {
                yield* kinesis
                    ...
                    .pipe(Effect.catchTag("ResourceInUseException", () => Effect.void), Effect.retry({
            ...
            if (state.streamMode !== desiredMode) {
                yield* kinesis.updateStreamMode({
                    StreamARN: streamArn,
                    StreamModeDetails: getStreamMode(news),
                    WarmThroughputMiBps: desiredMode === "ON_DEMAND"
            ...
            const metricsToEnable = (news.shardLevelMetrics ?? []).filter((metric) => !observedMetrics.has(metric));
            const metricsToDisable = state.shardLevelMetrics.filter((metric) => !desiredMetrics.has(metric));
            ...
            if (news.maxRecordSizeInKiB !== undefined &&
