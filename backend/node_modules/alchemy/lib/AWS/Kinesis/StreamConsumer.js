 ...
 * @resource
 */
export const StreamConsumer = Resource("AWS.Kinesis.StreamConsumer");
const createConsumerName = (id, props) => Effect.gen(function* () {
...
const toAttrs = ({ description, tags, }) => ({
...
const readConsumer = Effect.fn(function* ({ streamArn, consumerName, consumerArn, }) {
...
const adoptExistingConsumer = Effect.fn(function* (streamArn, consumerName) {
    ...
    }), Effect.catchTag("ConsumerRegistryNotConsistent", () => Effect.fail(new Error(`consumer ${consumerName} exists but could not be read after ` +
        `~10s of registry-consistency retries`))));
});
const waitForConsumerStatus = (consumerArn, expectedStatus) => Effect.gen(function* () {
    yield* Effect.sleep("2 seconds");
    const response = yield* kinesis.describeStreamConsumer({
        ConsumerARN: consumerArn,
    });
    if (response.ConsumerDescription.ConsumerStatus !== expectedStatus) {
        return yield* Effect.fail({ _tag: "ConsumerStatusNotReady" });
    }
    return response.ConsumerDescription;
}).pipe(Effect.retry({
    while: (e) => e._tag === "ConsumerStatusNotReady" || e._tag === "ParseError",
    schedule: Schedule.max([Schedule.exponential(500), Schedule.recurs(60)]),
}));
const waitForConsumerDeleted = (consumerArn) => Effect.gen(function* () {
    yield* kinesis.describeStreamConsumer({
        ConsumerARN: consumerArn,
    });
    return yield* Effect.fail({ _tag: "ConsumerStillExists" });
}).pipe(Effect.retry({
    while: (e) => e._tag === "ConsumerStillExists" || e._tag === "ParseError",
    schedule: Schedule.max([Schedule.exponential(500), Schedule.recurs(60)]),
}), Effect.catchTag("ResourceNotFoundException", () => Effect.void));
export const StreamConsumerProvider = () => Provider.succeed(StreamConsumer, {
    ...
    diff: Effect.fn(function* ({ id, news, olds }) {
        ...
        if (typeof news.streamArn === "string" &&
    ...
    reconcile: Effect.fn(function* ({ id, news, output, session }) {
        ...
        const streamArn = news.streamArn;
        ...
        if (state === undefined) {
            ...
            }), Effect.asVoid, Effect.catchTag("ResourceInUseException", () => adoptExistingConsumer(streamArn, consumerName).pipe(Effect.asVoid)));
    ...
    list: Effect.fn(function* () {
        const streamArns = yield* kinesis.listStreams.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.StreamSummaries ?? [])
            .map((summary) => summary.StreamARN)
            .filter((arn) => typeof arn === "string"))));
        const consumers = yield* Effect.forEach(streamArns, (streamArn) => kinesis.listStreamConsumers.pages({ StreamARN: streamArn }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Consumers ?? []).map((consumer) => ({
            streamArn,
            consumer,
        })))), 
        // A stream may be deleted mid-enumeration; skip it.
        Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([]))), { concurrency: 10 }).pipe(Effect.map((rows) => rows.flat()));
        const hydrated = yield* Effect.forEach(consumers, ({ streamArn, consumer }) => kinesis
            .listTagsForResource({ ResourceARN: consumer.ConsumerARN })
            .pipe(Effect.map((tagsResponse) => ({
        ...
        Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined))), { concurrency: 10 });
