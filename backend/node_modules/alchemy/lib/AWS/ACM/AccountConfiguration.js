 ...
 */
const idempotencyToken = (seed, desiredDays, attempt) => `${seed.replaceAll(/[^a-zA-Z0-9]/g, "").slice(0, 20)}d${desiredDays}a${attempt}`;
export const AccountConfigurationProvider = () => Provider.effect(AccountConfiguration, Effect.gen(function* () {
    const observe = withAcmRegion(acm.getAccountConfiguration({}).pipe(Effect.map((response) => ({
        daysBeforeExpiry: response.ExpiryEvents?.DaysBeforeExpiry ??
    ...
    const put = Effect.fn(function* (desiredDays, tokenSeed) {
        for (let attempt = 0; attempt < 3; attempt++) {
            yield* withAcmRegion(acm
        ...
        return yield* Effect.fail(new Error(`ACM account configuration did not converge to ${desiredDays} days before expiry`));
    });
    return {
...
//# sourceMappingURL=AccountConfiguration.js.map