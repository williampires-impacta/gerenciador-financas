 ...
 */
export const makeAcmCertificateHttpBinding = (config) => Effect.gen(function* () {
    const op = yield* withAcmRegion(config.operation);
    return Effect.fn(function* (certificate) {
        ...
        return Effect.fn(`AWS.ACM.${config.capability}(${certificate.LogicalId})`)(function* (request) {
 ...
 */
export const makeAcmAccountHttpBinding = (config) => Effect.gen(function* () {
    const op = yield* withAcmRegion(config.operation);
    return Effect.fn(function* () {
        ...
        return Effect.fn(`AWS.ACM.${config.capability}`)(function* (request) {
            // Call-site pin — see makeAcmCertificateHttpBinding above.
            return yield* withAcmRegion(op(request ?? {}));
        });
    });
});
