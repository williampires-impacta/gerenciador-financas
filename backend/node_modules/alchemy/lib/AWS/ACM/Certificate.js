 ...
 * @internal
 */
const resolveEffectiveSans = (declared, bindings) => {
 ...
 * @resource
 */
export const Certificate = Resource("AWS.ACM.Certificate");
export const CertificateProvider = () => Provider.effect(Certificate, Effect.gen(function* () {
    const describeCertificate = Effect.fn(function* (certificateArn) {
        return yield* acm
            .describeCertificate({ CertificateArn: certificateArn })
            .pipe(Effect.map((response) => response.Certificate), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)), withCertRegion(regionOfCertificateArn(certificateArn)));
    });
    const listCertificateTags = Effect.fn(function* (certificateArn) {
        return yield* acm
            .listTagsForCertificate({ CertificateArn: certificateArn })
            .pipe(Effect.map((response) => toTagRecord(response.Tags)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})), withCertRegion(regionOfCertificateArn(certificateArn)));
    });
    const findManagedCertificate = Effect.fn(function* (id, props) {
        ...
        return yield* withCertRegion(props.region)(acm.listCertificates
            .items({
            Includes: {
                keyTypes: props.keyAlgorithm ? [props.keyAlgorithm] : undefined,
            },
        })
            .pipe(Stream.filter((summary) => summary.DomainName === props.domainName), Stream.mapEffect((summary) => Effect.gen(function* () {
            ...
            if (detail.DomainName !== props.domainName ||
        ...
        })), Stream.filter((detail) => detail !== undefined), Stream.runHead, Effect.map(Option.getOrUndefined)));
    });
    const waitForValidationRecords = Effect.fn(function* (certificateArn) {
        return yield* describeCertificate(certificateArn).pipe(Effect.flatMap((detail) => {
    ...
    const waitForIssued = Effect.fn(function* (certificateArn) {
        return yield* describeCertificate(certificateArn).pipe(Effect.flatMap((detail) => {
            if (!detail?.CertificateArn) {
            ...
            if (isTerminalFailure(detail.Status)) {
                return Effect.fail(new Error(`Certificate issuance failed with status ${detail.Status}${detail.FailureReason ? ` (${detail.FailureReason})` : ""}`));
            }
    ...
    const upsertValidationRecords = Effect.fn(function* (hostedZoneId, certificate) {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const listPage = acm.listCertificates.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.CertificateSummaryList ?? [])));
            const summaries = [
                ...new Map([...(yield* withAcmRegion(listPage)), ...(yield* listPage)].map((summary) => [summary.CertificateArn, summary])).values(),
            ];
            const rows = yield* Effect.forEach(summaries, (summary) => Effect.gen(function* () {
                if (!summary.CertificateArn) {
                    return undefined;
                }
                const detail = yield* describeCertificate(summary.CertificateArn);
        ...
        diff: Effect.fn(function* ({ olds, news: _news, oldBindings, newBindings: _newBindings, }) {
            ...
            const news = _news;
            const newBindings = _newBindings;
            if (olds.domainName !== news.domainName ||
                ...
                !deepEqual(normalizeSanList(resolveEffectiveSans(olds.subjectAlternativeNames, oldBindings)), normalizeSanList(resolveEffectiveSans(news.subjectAlternativeNames, newBindings))) ||
        ...
        reconcile: Effect.fn(function* ({ id, instanceId, news: _news, output, session, bindings, }) {
            ...
            if (!certificate?.CertificateArn) {
                certificate = yield* withCertRegion(news.region)(acm
                    .requestCertificate({
                    ...
                    Options: news.certificateTransparencyLoggingPreference || news.export
                        ? {
                            CertificateTransparencyLoggingPreference: news.certificateTransparencyLoggingPreference,
                            Export: news.export,
                        }
                        : undefined,
                    ...
                    .pipe(Effect.flatMap((response) => response.CertificateArn
                    ? describeCertificate(response.CertificateArn).pipe(Effect.map((detail) => detail))
            ...
            const shouldAutoValidate = (news.validationMethod ?? defaultValidationMethod) === "DNS" &&
            ...
            const wantsCtChange = news.certificateTransparencyLoggingPreference !== undefined &&
                news.certificateTransparencyLoggingPreference !==
                    observedOptions.CertificateTransparencyLoggingPreference;
            if (wantsCtChange) {
                yield* withCertRegion(regionOfCertificateArn(certificateArn))(acm.updateCertificateOptions({
            ...
            if (upsert.length > 0) {
                yield* withCertRegion(regionOfCertificateArn(certificateArn))(acm.addTagsToCertificate({
                    CertificateArn: certificateArn,
                    Tags: upsert,
                }));
            }
            if (removed.length > 0) {
                yield* withCertRegion(regionOfCertificateArn(certificateArn))(acm.removeTagsFromCertificate({
                    CertificateArn: certificateArn,
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* withCertRegion(regionOfCertificateArn(output.certificateArn))(acm
                .deleteCertificate({
...
/** @internal */
export const waitForRoute53Change = Effect.fn(function* (changeId) {
    return yield* route53
        .getChange({
        Id: changeId.replace(/^\/change\//, ""),
    })
        .pipe(Effect.map((response) => response.ChangeInfo), Effect.flatMap((changeInfo) => changeInfo.Status === "INSYNC"
        ? Effect.succeed(changeInfo)
        : Effect.fail(new Error("Route53ChangePending"))), Effect.retry({
...
const defaultValidationMethod = "DNS";
/**
 * Region an existing certificate lives in, parsed from its ARN
 * (`arn:aws:acm:{region}:{account}:certificate/...`).
 */
const regionOfCertificateArn = (certificateArn) => certificateArn.split(":")[3] || ACM_REGION;
 ...
 */
const withCertRegion = (region) => (effect) => effect.pipe(Effect.provideService(AwsRegion, Effect.succeed((region ?? ACM_REGION))));
const normalizeHostedZoneId = (hostedZoneId) => hostedZoneId.replace(/^\/hostedzone\//, "");
const normalizeSanList = (names) => [...(names ?? [])].sort((a, b) => a.localeCompare(b));
const toTagRecord = (tags) => Object.fromEntries((tags ?? [])
    .filter((tag) => typeof tag.Key === "string" && typeof tag.Value === "string")
    .map((tag) => [tag.Key, tag.Value]));
const toAttrs = (props, detail, tags) => ({
    ...
    hostedZoneId: props.hostedZoneId
        ...
        : undefined,
    certificateTransparencyLoggingPreference: detail.Options?.CertificateTransparencyLoggingPreference,
    export: detail.Options?.Export,
    tags,
    issuedAt: detail.IssuedAt,
    notAfter: detail.NotAfter,
});
const isTerminalFailure = (status) => status === "FAILED" || status === "VALIDATION_TIMED_OUT";
