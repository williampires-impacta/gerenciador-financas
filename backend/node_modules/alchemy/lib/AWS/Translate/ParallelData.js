 ...
 * @resource
 */
export const ParallelData = Resource("AWS.Translate.ParallelData");
...
export const ParallelDataProvider = () => Provider.effect(ParallelData, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.parallelDataName ??
    ...
    // few minutes).
    const waitForStable = Effect.fn(function* (name) {
        const final = yield* getOne(name).pipe(Effect.repeat({
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.parallelDataName ??
                (yield* createName(id, olds ?? { s3Uri: "" }));
            const found = yield* getOne(name);
            if (found?.ParallelDataProperties === undefined)
                return undefined;
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (observedProps === undefined) {
                ...
                const drift = config?.S3Uri !== news.s3Uri ||
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* translate
                ...
                .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)), Effect.repeat({
                schedule: Schedule.spaced("5 seconds"),
                until: (r) => r === undefined,
                times: 36,
            }));
        }),
        list: () => translate.listParallelData.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.ParallelDataPropertiesList ?? [])
            .filter((props) => props.Name && props.Arn)
            .map(toAttributes))),
    };
}));
//# sourceMappingURL=ParallelData.js.map