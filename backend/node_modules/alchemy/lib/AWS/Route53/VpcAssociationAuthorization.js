 ...
 * @resource
 */
export const VpcAssociationAuthorization = Resource("AWS.Route53.VpcAssociationAuthorization");
export const VpcAssociationAuthorizationProvider = () => Provider.effect(VpcAssociationAuthorization, Effect.gen(function* () {
    ...
    const observe = Effect.fn(function* (hostedZoneId, vpcId) {
        ...
        for (let page = 0; page < 10; page++) {
            const response = yield* route53
                ...
                .pipe(Effect.catchTag("NoSuchHostedZone", () => Effect.succeed({
                HostedZoneId: hostedZoneId,
                VPCs: [],
                NextToken: undefined,
            })));
    ...
    return {
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* route53
                ...
                .pipe(Effect.catchTag("VPCAssociationAuthorizationNotFound", () => Effect.void), Effect.catchTag("NoSuchHostedZone", () => Effect.void));
        }),
    };
}));
