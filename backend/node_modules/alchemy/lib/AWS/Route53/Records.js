...
import { normalizeHostedZoneId, normalizeName, toAliasTarget, toRecordSet, } from "./Record.js";
/**
 ...
 * @resource
 */
export const Records = Resource("AWS.Route53.Records");
/**
 ...
 * @internal
 */
const resolveDesiredNames = (declared, bindings) => {
...
export const RecordsProvider = () => Provider.effect(Records, Effect.gen(function* () {
    const waitForChange = Effect.fn(function* (changeId) {
        return yield* route53
            .getChange({ Id: changeId.replace(/^\/change\//, "") })
            .pipe(Effect.map((response) => response.ChangeInfo.Status), Effect.catchTag("NoSuchChange", () => Effect.succeed("PENDING")), Effect.repeat({
    ...
    const findRecord = Effect.fn(function* (hostedZoneId, name, type) {
        ...
        return (response?.ResourceRecordSets ?? []).find((recordSet) => recordSet.Name.toLowerCase() ===
            normalizeName(name).toLowerCase() &&
            recordSet.Type === type &&
            recordSet.SetIdentifier === undefined);
    });
    const desiredRecordSet = (news, name) => toRecordSet({
        name,
        type: news.type,
    ...
    const matchesDesired = (live, desired) => {
        const normalizeDns = (value) => value?.toLowerCase().replace(/\.$/, "");
        if (desired.AliasTarget) {
            return (normalizeDns(live.AliasTarget?.DNSName) ===
    ...
    const deleteNames = Effect.fn(function* (hostedZoneId, type, names) {
        const changes = [];
        ...
        yield* route53
            ...
            .pipe(Effect.flatMap((response) => waitForChange(response.ChangeInfo.Id)), Effect.catchTag("NoSuchHostedZone", () => Effect.void), Effect.catchTag("InvalidChangeBatch", () => Effect.void));
    });
    return {
        ...
        diff: Effect.fn(function* ({ olds, news }) {
            ...
            if ((olds.hostedZoneId !== undefined &&
        ...
        reconcile: Effect.fn(function* ({ news, output, session, bindings }) {
            ...
            const stale = previous.filter((name) => !desiredKeys.has(normalizeName(name).toLowerCase()));
            ...
            yield* session.note(`${news.type} × ${desired.length} record(s)` +
                (stale.length > 0 ? ` (-${stale.length})` : ""));
            const sample = desired.length > 0
