 ...
 * @resource
 */
export const Association = Resource("AWS.Chatbot.Association");
export const AssociationProvider = () => Provider.effect(Association, Effect.gen(function* () {
    ...
    const observeAssociation = Effect.fn(function* (chatConfiguration, resource) {
        return yield* chatbot.listAssociations
            .items({ ChatConfiguration: chatConfiguration })
            .pipe(Stream.runCollect, Effect.map((listings) => Array.from(listings).find((listing) => listing.Resource === resource)), Effect.catchTag("InvalidRequestException", () => Effect.succeed(undefined)));
    });
    const toAttributes = (chatConfiguration, resource) => ({
    ...
    return Association.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (observed === undefined) {
                ...
                yield* observeAssociation(news.chatConfiguration, news.resource).pipe(Effect.flatMap((listing) => listing !== undefined
                    ? Effect.void
                    : Effect.fail(new AssociationNotVisible({
                        chatConfiguration: news.chatConfiguration,
                        resource: news.resource,
                    }))), Effect.retry({
                    while: (e) => e._tag === "AssociationNotVisible",
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            const observed = yield* observeAssociation(output.chatConfigurationArn, output.resourceArn);
            if (observed === undefined)
                return;
            yield* chatbot
            ...
            Effect.catchTag(["InvalidRequestException", "ResourceNotFoundException"], () => Effect.void));
        }),
        list: () => Effect.gen(function* () {
            const configurationArns = [];
