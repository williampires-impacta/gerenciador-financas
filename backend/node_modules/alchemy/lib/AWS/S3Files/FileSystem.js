 ...
 * @resource
 */
export const FileSystem = Resource("AWS.S3Files.FileSystem");
...
export const FileSystemProvider = () => Provider.effect(FileSystem, Effect.gen(function* () {
    const observe = (fileSystemId) => s3files
        .getFileSystem({ fileSystemId })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    const observedTags = (fileSystemId) => s3files.listTagsForResource.items({ resourceId: fileSystemId }).pipe(Stream.runCollect, Effect.map((tags) => toTagRecord(Array.from(tags))), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})));
    const toAttributes = Effect.fn(function* (fs) {
        if (fs.fileSystemId === undefined ||
    ...
    const findByAlchemyTags = Effect.fn(function* (id, bucket) {
    ...
    const syncTags = Effect.fn(function* (fileSystemId, desired) {
    ...
    const syncPolicy = Effect.fn(function* (fileSystemId, desired) {
    ...
    return FileSystem.Provider.of({
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const found = output?.fileSystemId !== undefined
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let live = output?.fileSystemId !== undefined
                ? yield* observe(output.fileSystemId)
            ...
            const settled = yield* untilSettled(observe(fileSystemId), (fs) => fs === undefined || fs.status !== "creating");
        ...
        delete: Effect.fn(function* ({ olds, output }) {
            ...
            if (live.status !== "deleting" && live.status !== "deleted") {
                yield* retryWhileConflict(s3files.deleteFileSystem({
            ...
            const remaining = yield* untilSettled(observe(output.fileSystemId), (fs) => fs === undefined);
