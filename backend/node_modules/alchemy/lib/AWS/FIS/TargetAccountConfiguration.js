 ...
 * @resource
 */
export const TargetAccountConfiguration = Resource("AWS.FIS.TargetAccountConfiguration");
export const TargetAccountConfigurationProvider = () => Provider.effect(TargetAccountConfiguration, Effect.gen(function* () {
    ...
    // exist.
    const observe = (experimentTemplateId, accountId) => fis
        .getTargetAccountConfiguration({ experimentTemplateId, accountId })
        .pipe(Effect.map((r) => r.targetAccountConfiguration), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    const toAttrs = (experimentTemplateId, config) => ({
    ...
    return TargetAccountConfiguration.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (observed === undefined) {
                ...
                observed = yield* fis
                    ...
                    .pipe(Effect.map((r) => r.targetAccountConfiguration), Effect.catchTag("ConflictException", () => observe(news.experimentTemplateId, news.accountId)));
            }
            else if (
        ...
        list: () => Effect.gen(function* () {
            const templates = yield* fis.listExperimentTemplates.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
            const results = [];
