 ...
 * @resource
 */
export const NotificationChannel = Resource("AWS.DevOpsGuru.NotificationChannel");
const sameSet = (left, right) => {
    const l = [...(left ?? [])].sort();
    const r = [...(right ?? [])].sort();
    return l.length === r.length && l.every((value, i) => value === r[i]);
};
const matchesDesired = (config, news) => config?.Sns?.TopicArn === news.topicArn &&
...
export const NotificationChannelProvider = () => Provider.effect(NotificationChannel, Effect.gen(function* () {
    ...
    const listChannels = devopsguru.listNotificationChannels.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
    const findByTopic = (channels, topicArn) => channels.find((c) => c.Config?.Sns?.TopicArn === topicArn);
    return {
        stables: ["topicArn"],
        list: () => listChannels.pipe(Effect.map((channels) => channels.flatMap((c) => c.Id !== undefined && c.Config?.Sns?.TopicArn !== undefined
        ...
        read: Effect.fn(function* ({ olds, output }) {
            ...
            const found = output?.id !== undefined
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            if (id === undefined) {
                const added = yield* devopsguru
                    ...
                    .pipe(Effect.catchTag("ConflictException", (error) => listChannels.pipe(Effect.flatMap((latest) => {
