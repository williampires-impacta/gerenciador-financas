 ...
 * @resource
 */
export const ResourceCollection = Resource("AWS.DevOpsGuru.ResourceCollection");
/**
...
export const ResourceCollectionProvider = () => Provider.effect(ResourceCollection, Effect.gen(function* () {
    ...
    const observeType = Effect.fn(function* (type) {
    ...
    const observe = Effect.gen(function* () {
        ...
        const stackNames = [
            ...new Set(cfnPages.flatMap((page) => page.ResourceCollection?.CloudFormation?.StackNames ?? [])),
        ].sort();
        const tagMap = new Map();
        for (const page of tagPages) {
            for (const tag of page.ResourceCollection?.Tags ?? []) {
                const values = tagMap.get(tag.AppBoundaryKey) ?? new Set();
    ...
    const update = Effect.fn(function* (action, filter) {
        yield* retryUpdateConflict(devopsguru.updateResourceCollection({
            Action: action,
    ...
    return {
        ...
        list: () => observe.pipe(Effect.map((observed) => observed.stackNames.length > 0 || observed.tags.length > 0
            ? [toAttrs(observed)]
            : [])),
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            const stacksToRemove = observed.stackNames.filter((name) => !desiredStacks.includes(name));
            ...
            for (const tag of observed.tags) {
                ...
                const valuesToRemove = tag.tagValues.filter((value) => !(desired?.tagValues ?? []).includes(value));
            ...
            for (const desired of desiredTags) {
                const current = observed.tags.find((t) => sameKey(t.appBoundaryKey, desired.appBoundaryKey));
                const valuesToAdd = desired.tagValues.filter((value) => !(current?.tagValues ?? []).includes(value));
            ...
            yield* session.note(final.stackNames.length > 0
