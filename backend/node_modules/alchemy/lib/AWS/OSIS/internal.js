 ...
 */
export const readPipelineTags = Effect.fn(function* (arn) {
...
const isTransitional = (status) => status === "CREATING" ||
    status === "UPDATING" ||
    status === "STARTING" ||
    status === "STOPPING" ||
    status === "DELETING";
const isFailedStatus = (status) => status === "CREATE_FAILED" ||
 ...
 */
export const waitForPipelineSettled = (pipelineName, read) => Effect.flatMap(Effect.repeat(read, {
    schedule: Schedule.max([
        Schedule.fixed("15 seconds"),
        Schedule.recurs(60),
    ]),
    until: (pipeline) => pipeline === undefined || !isTransitional(pipeline.Status),
}), (pipeline) => pipeline !== undefined && isFailedStatus(pipeline.Status)
    ? Effect.fail(new PipelineOperationFailed({
        pipelineName,
 ...
 */
export const retryWhilePipelineConflict = (self) => Effect.retry(self, {
