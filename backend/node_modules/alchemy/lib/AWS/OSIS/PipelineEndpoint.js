...
import { jsonEquals } from "./internal.js";
/**
 ...
 * @resource
 */
export const PipelineEndpoint = Resource("AWS.OSIS.PipelineEndpoint");
/**
 * A pipeline endpoint whose asynchronous create converged to a failed
 * status.
 */
export class PipelineEndpointCreateFailed extends Data.TaggedError("OsisPipelineEndpointCreateFailed") {
}
export const PipelineEndpointProvider = () => Provider.effect(PipelineEndpoint, Effect.gen(function* () {
    /**
     * There is no `getPipelineEndpoint` — observe by enumerating the
     * account's endpoints and matching the id.
     */
    const findEndpoint = Effect.fn(function* (endpointId) {
        return yield* osis.listPipelineEndpoints.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    ...
    const toAttrs = Effect.fn(function* (endpoint) {
        if (!endpoint.EndpointId || !endpoint.PipelineArn) {
            return yield* Effect.fail(new Error(`OSIS pipeline endpoint '${endpoint.EndpointId}' is missing its id or pipeline ARN`));
        }
        return {
     ...
     * `internal.ts`.
     */
    const waitForSettled = (read) => Effect.repeat(read, {
        schedule: Schedule.max([
            Schedule.fixed("15 seconds"),
            Schedule.recurs(40),
        ]),
        until: (endpoint) => endpoint === undefined || endpoint.Status !== "CREATING",
    });
    return {
        ...
        diff: Effect.fn(function* ({ olds, news }) {
            ...
            if (olds?.pipelineArn !== news.pipelineArn ||
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            let observed = output?.endpointId !== undefined
            ...
            if (observed === undefined) {
                ...
                if (created.EndpointId === undefined) {
                    return yield* Effect.fail(new Error("OSIS createPipelineEndpoint returned no EndpointId"));
                }
            ...
            if (observed === undefined) {
                return yield* Effect.fail(new Error("OSIS pipeline endpoint disappeared while waiting for create"));
            }
            if (observed.Status === "CREATE_FAILED") {
                return yield* Effect.fail(new PipelineEndpointCreateFailed({
        ...
        list: () => osis.listPipelineEndpoints.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.PipelineEndpoints ?? [])), Effect.flatMap(Effect.forEach((endpoint) => toAttrs(endpoint), {
            concurrency: 4,
        }))),
    };
}));
//# sourceMappingURL=PipelineEndpoint.js.map