 ...
 * @resource
 */
export const TrackerConsumer = Resource("AWS.Location.TrackerConsumer");
export const TrackerConsumerProvider = () => Provider.effect(TrackerConsumer, Effect.gen(function* () {
    /** Whether `consumerArn` is currently linked to `trackerName`. */
    const observeAssociation = Effect.fn(function* (trackerName, consumerArn) {
        const consumerArns = yield* location.listTrackerConsumers
            .pages({ TrackerName: trackerName })
            .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.ConsumerArns)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed([])));
        return consumerArns.includes(consumerArn);
    });
    return {
        stables: ["trackerName", "consumerArn"],
        list: () => Effect.gen(function* () {
            const trackerNames = yield* location.listTrackers.pages({}).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Entries ?? []).map((entry) => entry.TrackerName))));
            const associations = yield* Effect.forEach(trackerNames, (trackerName) => location.listTrackerConsumers
                .pages({ TrackerName: trackerName })
                .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.ConsumerArns.map((consumerArn) => ({
                trackerName,
                consumerArn,
            })))), 
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            if (!isResolved(news))
                return;
            if (olds.trackerName !== news.trackerName ||
...
//# sourceMappingURL=TrackerConsumer.js.map