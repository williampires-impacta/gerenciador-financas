 ...
 * @resource
 */
export const Tracker = Resource("AWS.Location.Tracker");
const createTrackerName = (id, props) => Effect.gen(function* () {
    if (props.trackerName)
        return props.trackerName;
    return yield* createPhysicalName({ id, maxLength: 100 });
});
const readTracker = Effect.fn(function* (trackerName) {
...
export const TrackerProvider = () => Provider.effect(Tracker, Effect.gen(function* () {
    return {
        stables: ["trackerName", "trackerArn"],
        list: () => Effect.gen(function* () {
            const names = yield* location.listTrackers.pages({}).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Entries ?? []).map((entry) => entry.TrackerName))));
            const hydrated = yield* Effect.forEach(names, (name) => readTracker(name), { concurrency: 10 });
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
            ...
            if (!final) {
                return yield* Effect.fail(new Error(`failed to read reconciled tracker ${trackerName}`));
            }
            return final;
        }),
