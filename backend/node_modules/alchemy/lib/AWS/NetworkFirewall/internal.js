...
export const nfwTagsToRecord = (tags) => Object.fromEntries((tags ?? []).map((t) => [t.Key, t.Value]));
/** Convert a plain record into a Network Firewall `Tag[]` list. */
export const recordToNfwTags = (record) => Object.entries(record).map(([Key, Value]) => ({ Key, Value }));
 ...
 * short window.
 */
export const retryWhileNfwInUse = (self) => Effect.retry(self, {
 ...
 */
export const retryWhileNfwNotFound = (self) => Effect.retry(self, {
