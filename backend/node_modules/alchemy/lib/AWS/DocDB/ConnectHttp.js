...
import { Connect, connectEnvPrefix, formatMongoConnectionUrl, } from "./Connect.js";
export const ConnectHttp = Layer.effect(Connect, Effect.gen(function* () {
    ...
    return Effect.fn(function* (cluster, options) {
            ...
            if (isBindingHost(host)) {
                ...
                const secretResource = options?.secret
                    ...
                    : Output.map(cluster.masterUserSecretArn, (arn) => {
                        if (arn === undefined) {
                            throw new Error(`AWS.DocDB.Connect(${cluster.LogicalId}): the cluster has no managed master user secret — ` +
        ...
        return Effect.gen(function* () {
            ...
            if (!host) {
                return yield* Effect.die(`DocumentDB endpoint for '${cluster.LogicalId}' is not available yet`);
            }
            if (!secretArn) {
                return yield* Effect.die(`AWS.DocDB.Connect(${cluster.LogicalId}): the cluster has no managed master user secret — ` +
                    `set manageMasterUserPassword: true on the cluster or pass options.secret`);
            }
            ...
            const password = secret.password !== undefined
