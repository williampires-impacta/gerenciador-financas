 ...
 * @resource
 */
export const ReplicationSubnetGroup = Resource("AWS.DMS.ReplicationSubnetGroup");
...
export const ReplicationSubnetGroupProvider = () => Provider.effect(ReplicationSubnetGroup, Effect.gen(function* () {
    const toName = (id, props) => props.replicationSubnetGroupIdentifier
    ...
    const toArn = Effect.fn(function* (identifier) {
        const { accountId, region } = yield* AWSEnvironment.current;
        return `arn:aws:dms:${region}:${accountId}:subgrp:${identifier}`;
    });
    const findGroup = Effect.fn(function* (identifier) {
    ...
    Effect.retry({
        while: (error) => error._tag === "NoSuchEntityException" ||
    ...
    const deleteOwnedDmsVpcRoleIfUnused = Effect.gen(function* () {
        ...
        for (let attempt = 0; attempt < 30; attempt++) {
            ...
            if ((groups.ReplicationSubnetGroups ?? []).length > 0)
                return;
            ...
            if (tags === undefined)
                return;
            const owned = (tags.Tags ?? []).some((tag) => tag.Key === DMS_VPC_ROLE_OWNER_TAG.Key &&
        ...
        return yield* Effect.die(new Error(`IAM role ${DMS_VPC_ROLE_NAME} remained observable 30 seconds after the last DMS subnet group was deleted`));
    });
    ...
    const toAttrs = Effect.fn(function* (group) {
        ...
        if (!identifier) {
            return yield* Effect.fail(new Error("DMS replication subnet group is missing its identifier"));
        }
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.replicationSubnetGroupIdentifier ??
                (yield* toName(id, olds ?? { subnetIds: [] }));
            const group = yield* findGroup(name);
            if (!group?.ReplicationSubnetGroupIdentifier)
                return undefined;
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (observed === undefined) {
                ...
                Effect.catchTag("AccessDeniedFault", (error) => ensureDmsVpcRole.pipe(Effect.andThen(Effect.fail(error)))), Effect.retry({
                ...
                const observedSubnets = (observed.Subnets ?? [])
                    .map((s) => s.SubnetIdentifier)
                    .filter((s) => typeof s === "string");
                if (description !== observed.ReplicationSubnetGroupDescription ||
            ...
            if (!observed?.ReplicationSubnetGroupIdentifier) {
                return yield* Effect.fail(new Error(`DMS replication subnet group '${name}' not found after reconcile`));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            for (let attempt = 0; attempt < 30; attempt++) {
                if ((yield* findGroup(output.replicationSubnetGroupIdentifier)) ===
                    undefined) {
            ...
            yield* Effect.die(new Error(`DMS replication subnet group ${output.replicationSubnetGroupIdentifier} remained observable 30 seconds after delete`));
        }),
        list: () => dms.describeReplicationSubnetGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.ReplicationSubnetGroups ?? []).filter((group) => group.ReplicationSubnetGroupIdentifier !== undefined))), Effect.flatMap(Effect.forEach((group) => toAttrs(group), { concurrency: 4 }))),
    };
}));
