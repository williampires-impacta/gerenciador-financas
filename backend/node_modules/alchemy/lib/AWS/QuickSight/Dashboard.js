 ...
 * @resource
 */
export const Dashboard = Resource("AWS.QuickSight.Dashboard");
export const DashboardProvider = () => Provider.effect(Dashboard, Effect.gen(function* () {
    const toId = (id, props) => props.dashboardId
        ? Effect.succeed(props.dashboardId)
        : createPhysicalName({ id, maxLength: 64 });
    const readDashboard = Effect.fn(function* (accountId, dashboardId) {
    ...
    const settle = (accountId, dashboardId) => waitForSettled(dashboardId, readDashboard(accountId, dashboardId).pipe(Effect.map((d) => d === undefined ? undefined : { ...d, status: d.Version?.Status })));
    const toAttrs = (dashboard) => ({
    ...
    return Dashboard.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (observed === undefined) {
                return yield* Effect.fail(new Error(`QuickSight dashboard '${dashboardId}' not found after reconcile`));
            }
            // 3b. Sync tags.
        ...
        list: () => Effect.gen(function* () {
            ...
            return yield* quicksight.listDashboards
                ...
                .flatMap((s) => s.DashboardId !== undefined && s.Arn !== undefined
                ? [
                    {
