...
const isInProgress = (status) => status === "CREATION_IN_PROGRESS" || status === "UPDATE_IN_PROGRESS";
const isFailed = (status) => status === "CREATION_FAILED" || status === "UPDATE_FAILED";
/**
 ...
 */
export const waitForSettled = (resourceId, read) => Effect.flatMap(Effect.repeat(read, {
    schedule: Schedule.max([
        Schedule.fixed("5 seconds"),
        Schedule.recurs(30),
    ]),
    until: (observed) => observed === undefined || !isInProgress(observed.status),
}), (observed) => observed !== undefined && isFailed(observed.status)
    ? Effect.fail(new QuickSightOperationFailed({
        resourceId,
 ...
 */
export const readQuickSightTags = Effect.fn(function* (arn) {
 ...
 */
export const syncQuickSightTags = Effect.fn(function* (arn, desiredTags) {
