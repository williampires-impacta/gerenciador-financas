 ...
 * @resource
 */
export const RunGroup = Resource("AWS.Omics.RunGroup");
export const RunGroupProvider = () => Provider.effect(RunGroup, Effect.gen(function* () {
    ...
    return RunGroup.Provider.of({
        ...
        list: () => omics.listRunGroups.items({}).pipe(Stream.map((item) => ({
            runGroupId: item.id,
            runGroupArn: item.arn,
            name: item.name ?? "",
        })), Stream.runCollect, Effect.map((chunk) => Array.from(chunk))),
        read: Effect.fn(function* ({ id, output }) {
            if (output?.runGroupId === undefined)
                return undefined;
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let group = output?.runGroupId === undefined
                ? undefined
                : yield* omics
                    .getRunGroup({ id: output.runGroupId })
                    .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
...
//# sourceMappingURL=RunGroup.js.map