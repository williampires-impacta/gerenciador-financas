...
import { SERVICE_NAMESPACES, retryWhileTargetPropagates } from "./internal.js";
/**
 ...
 * @resource
 */
export const ScheduledAction = Resource("AWS.ApplicationAutoScaling.ScheduledAction");
export const ScheduledActionProvider = () => Provider.effect(ScheduledAction, Effect.gen(function* () {
    ...
    const describe = (props) => aas
        ...
        .pipe(Effect.map((res) => res.ScheduledActions?.find((a) => a.ScheduledActionName === props.scheduledActionName)));
    ...
    return {
        ...
        list: () => Effect.forEach(SERVICE_NAMESPACES, (namespace) => aas.describeScheduledActions
            .pages({ ServiceNamespace: namespace })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.ScheduledActions ?? []).map(toAttributes)))), { concurrency: 4 }).pipe(Effect.map((groups) => groups.flat())),
        ...
        reconcile: Effect.fn(function* ({ id, news, session }) {
            ...
            yield* retryWhileTargetPropagates(aas.putScheduledAction({
            ...
            if (action === undefined) {
                return yield* Effect.fail(new aas.ObjectNotFoundException({
