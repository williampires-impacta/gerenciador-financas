...
import { diffTags, hasAlchemyTags, createInternalTags } from "../../Tags.js";
import { SERVICE_NAMESPACES } from "./internal.js";
/**
 ...
 * @resource
 */
export const ScalableTarget = Resource("AWS.ApplicationAutoScaling.ScalableTarget");
export const ScalableTargetProvider = () => Provider.effect(ScalableTarget, Effect.gen(function* () {
    const describe = (props) => aas
        .describeScalableTargets({
        ServiceNamespace: props.serviceNamespace,
        ResourceIds: [props.resourceId],
        ScalableDimension: props.scalableDimension,
    })
        .pipe(Effect.map((res) => res.ScalableTargets?.find((t) => t.ResourceId === props.resourceId &&
        t.ScalableDimension === props.scalableDimension)));
    const toAttributes = (target) => ({
    ...
    const observedTags = (scalableTargetArn) => aas.listTagsForResource({ ResourceARN: scalableTargetArn }).pipe(Effect.map((res) => Object.fromEntries(Object.entries(res.Tags ?? {}).filter((entry) => entry[1] !== undefined))), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})));
    ...
    return {
        ...
        list: () => Effect.forEach(SERVICE_NAMESPACES, (namespace) => aas.describeScalableTargets
            .pages({ ServiceNamespace: namespace })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.ScalableTargets ?? []).map(toAttributes)))), { concurrency: 4 }).pipe(Effect.map((groups) => groups.flat())),
