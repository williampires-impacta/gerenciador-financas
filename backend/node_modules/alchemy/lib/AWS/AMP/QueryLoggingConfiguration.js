 ...
 * @resource
 */
export const QueryLoggingConfiguration = Resource("AWS.AMP.QueryLoggingConfiguration");
export const QueryLoggingConfigurationProvider = () => Provider.effect(QueryLoggingConfiguration, Effect.gen(function* () {
    ...
    const waitActive = Effect.fn(function* (workspaceId) {
        return yield* amp
            .describeQueryLoggingConfiguration({ workspaceId })
            .pipe(Effect.map((r) => r.queryLoggingConfiguration), Effect.repeat({
    ...
    const toWireDestinations = (destinations) => destinations.map((destination) => ({
        cloudWatchLogs: {
    ...
    const canonical = (destinations) => JSON.stringify(destinations
        .map((d) => ({
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (existing === undefined) {
                yield* amp
                    ...
                    .pipe(Effect.catchTag("ConflictException", () => amp.updateQueryLoggingConfiguration({
                    workspaceId,
                    destinations: desired,
                })));
            }
