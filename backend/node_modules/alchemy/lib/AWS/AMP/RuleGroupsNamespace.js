...
import { decodeDefinition, encodeDefinition, syncAmpTags, toTagRecord, } from "./internal.js";
/**
 ...
 * @resource
 */
export const RuleGroupsNamespace = Resource("AWS.AMP.RuleGroupsNamespace");
export const RuleGroupsNamespaceProvider = () => Provider.effect(RuleGroupsNamespace, Effect.gen(function* () {
    const toAttrs = (ns) => ({
        workspaceId: "",
        name: ns.name,
    ...
    const describe = Effect.fn(function* (workspaceId, name) {
     ...
     */
    const waitActive = Effect.fn(function* (workspaceId, name) {
        const ns = yield* amp
            .describeRuleGroupsNamespace({ workspaceId, name })
            .pipe(Effect.map((r) => r.ruleGroupsNamespace), Effect.repeat({
        ...
        if (ns.status.statusCode !== "ACTIVE") {
            return yield* Effect.fail(new Error(`AMP rule groups namespace ${name} did not become ACTIVE (status: ${ns.status.statusCode})`));
        }
        return ns;
    });
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, session }) {
            ...
            if (ns === undefined) {
                ...
                const currentDefinition = ns.data !== undefined
...
//# sourceMappingURL=RuleGroupsNamespace.js.map