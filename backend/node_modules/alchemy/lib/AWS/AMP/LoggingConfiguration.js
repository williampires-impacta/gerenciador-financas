 ...
 * @resource
 */
export const LoggingConfiguration = Resource("AWS.AMP.LoggingConfiguration");
export const LoggingConfigurationProvider = () => Provider.effect(LoggingConfiguration, Effect.gen(function* () {
    ...
    const waitActive = Effect.fn(function* (workspaceId) {
        return yield* amp.describeLoggingConfiguration({ workspaceId }).pipe(Effect.map((r) => r.loggingConfiguration), Effect.repeat({
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (existing === undefined) {
                yield* amp
                    ...
                    .pipe(Effect.catchTag("ConflictException", () => amp.updateLoggingConfiguration({
                    workspaceId,
                    logGroupArn: desiredArn,
                })));
            }
