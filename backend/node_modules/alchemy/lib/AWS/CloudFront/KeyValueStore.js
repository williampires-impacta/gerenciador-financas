 ...
 * @resource
 */
export const KeyValueStore = Resource("AWS.CloudFront.KeyValueStore");
export const KeyValueStoreProvider = () => Provider.effect(KeyValueStore, Effect.gen(function* () {
    const getByName = Effect.fn(function* (name) {
        const store = yield* cloudfront.listKeyValueStores.pages({}).pipe(Stream.map((page) => page.KeyValueStoreList?.Items ?? []), Stream.flattenIterable, Stream.filter((item) => item.Name === name), Stream.runHead, Effect.map(Option.getOrUndefined));
        if (!store?.Name) {
            return undefined;
        }
    ...
    return {
        ...
        list: () => cloudfront.listKeyValueStores.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.KeyValueStoreList?.Items ?? []).map((store) => toAttrs(store, undefined, store.Name))))),
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!observed?.KeyValueStore) {
                const created = yield* cloudfront
                    ...
                    .pipe(Effect.catchTag("EntityAlreadyExists", () => getByName(name).pipe(Effect.flatMap((existing) => existing
                    ? Effect.succeed(existing)
                    : Effect.die(`CloudFront KeyValueStore '${name}' already exists but could not be recovered`)))));
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if (!etag) {
                yield* Effect.logInfo(`CloudFront KeyValueStore delete: ${output.keyValueStoreName} already absent`);
                return;
            }
            yield* Effect.logInfo(`CloudFront KeyValueStore delete: deleting ${output.keyValueStoreName} with etag=${etag}`);
            yield* cloudfront
                .deleteKeyValueStore({
                Name: output.keyValueStoreName,
                IfMatch: etag,
...
const toAttrs = (store, etag, fallbackName) => ({
