...
import { KVS_REGION, withKvsRegion } from "./common.js";
/**
 ...
 * distribution's ARN.
 */
export const makeDistributionScopedHttpBinding = (options) => Effect.gen(function* () {
    const op = yield* options.operation;
    return Effect.fn(function* (distribution) {
        ...
        return Effect.fn(`${options.tag}(${distribution.LogicalId})`)(function* (request) {
            return yield* op({
                ...request,
 ...
 */
export const makeKeyValueStoreScopedHttpBinding = (options) => Effect.gen(function* () {
    // The distilled Region service value is `Effect<RegionName>`, not a raw
    // string (see `withKvsRegion` in common.ts).
    const op = yield* options.operation.pipe(Effect.provideService(AwsRegion, Effect.succeed(KVS_REGION)));
