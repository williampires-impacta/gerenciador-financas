...
import { extractValue, getKvsEtag, isKvsPreconditionFailed, retryForKvsReadiness, withKvsRegionFn, } from "./common.js";
/**
 ...
 * @resource
 */
export const KvRoutesUpdate = Resource("AWS.CloudFront.KvRoutesUpdate");
const CHUNK_SIZE = 1000;
export const KvRoutesUpdateProvider = () => Provider.effect(KvRoutesUpdate, Effect.gen(function* () {
    const getRoutes = Effect.fn(function* (store, fullKey) {
        const res = yield* kvs
            .getKey({ KvsARN: store, Key: fullKey })
            .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
        if (!res) {
            return { routes: [], chunkNum: 1 };
        }
        ...
        return { routes: parsed, chunkNum: 1 };
    });
    const setRoutes = Effect.fn(function* (store, etag, fullKey, routes, oldChunkNum) {
    ...
    const deleteKey = Effect.fn(function* (store, etag, fullKey, oldChunkNum) {
        const deletes = [{ Key: fullKey }];
        if (oldChunkNum > 1) {
            for (let i = 0; i < oldChunkNum; i++) {
                deletes.push({ Key: `${fullKey}:${i}` });
            }
    ...
    const deleteOp = (props) => Effect.gen(function* () {
        ...
        while: (error) => error._tag === "ValidationException" &&
    ...
    const upsertEntry = (store, fullKey, entryToAdd, entryToRemove) => Effect.gen(function* () {
        const etag = yield* getKvsEtag(store);
        const { routes, chunkNum } = yield* getRoutes(store, fullKey);
        const filtered = entryToRemove !== undefined
    ...
    return {
        ...
        list: () => Effect.succeed([]),
        read: withKvsRegionFn(Effect.fn(function* ({ output }) {
            return output;
        })),
        reconcile: withKvsRegionFn(Effect.fn(function* ({ news, output }) {
            return yield* retryForKvsReadiness(Effect.gen(function* () {
                ...
                // new array.
                const movedLocation = output !== undefined &&
                ...
                const previousEntry = !movedLocation && output !== undefined
                    ? output.entry
                    : undefined;
        ...
        delete: withKvsRegionFn(Effect.fn(function* ({ output }) {
            yield* retryForKvsReadiness(deleteOp({
                store: output.store,
...
//# sourceMappingURL=KvRoutesUpdate.js.map