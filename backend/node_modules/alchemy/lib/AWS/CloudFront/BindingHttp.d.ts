 ...
 * distribution's ARN.
 */
export declare const makeDistributionScopedHttpBinding: <I extends {
...
}) => Effect.Effect<(distribution: Distribution) => Effect.Effect<(request: Omit<I, "DistributionId">) => Effect.Effect<A, E, never>, never, never>, never, R>;
/**
 ...
 */
export declare const makeKeyValueStoreScopedHttpBinding: <I extends {
    KvsARN: string;
}, A, E, R>(options: {
    ...
    actions: readonly string[];
}) => Effect.Effect<(store: KeyValueStore) => Effect.Effect<(request: Omit<I, "KvsARN">) => Effect.Effect<A, E, never>, never, never>, never, Exclude<R, AwsRegion>>;
