...
class DistributionFunctionAssociationPending extends Data.TaggedError("DistributionFunctionAssociationPending") {
}
class DistributionPendingDeployment extends Data.TaggedError("DistributionPendingDeployment") {
}
const isFunctionAssociationPending = (error) => {
    const message = error.message ?? "";
    return (message.includes("FunctionAssociationArn") &&
 ...
 * @internal
 */
const resolveEffectiveAliases = (declared, bindings) => {
    const bound = (bindings ?? []).flatMap((binding) => "data" in binding && binding.data !== undefined
        ? (binding.data.aliases ?? [])
        : (binding.aliases ?? []));
    if (bound.length === 0) {
        return declared;
    }
 ...
 * @resource
 */
export const Distribution = Resource("AWS.CloudFront.Distribution");
export const DistributionProvider = () => Provider.effect(Distribution, Effect.gen(function* () {
    const waitForDeployment = Effect.fn(function* (distributionId) {
        yield* Effect.logInfo(`CloudFront Distribution wait: polling deployment for ${distributionId}`);
        ...
        Effect.timeout(30_000), Effect.catchTag("TimeoutError", () => Effect.fail(new DistributionPendingDeployment({
            message: `Timed out reading distribution ${distributionId} while polling deployment`,
        }))), Effect.map((response) => response.Distribution), Effect.flatMap((distribution) => distribution?.Status === "Deployed"
            ? Effect.gen(function* () {
                yield* Effect.logInfo(`CloudFront Distribution wait: ${distributionId} deployed`);
                return distribution;
            })
            : Effect.gen(function* () {
                yield* Effect.logInfo(`CloudFront Distribution wait: ${distributionId} status=${distribution?.Status ?? "unknown"}`);
                return yield* Effect.fail(new DistributionPendingDeployment({
    ...
    const getCurrent = Effect.fn(function* (distributionId) {
        yield* Effect.logInfo(`CloudFront Distribution read: loading distribution ${distributionId}`);
        const distribution = yield* cloudfront
            .getDistribution({ Id: distributionId })
            .pipe(Effect.map((response) => response.Distribution), Effect.catchTag("NoSuchDistribution", () => Effect.succeed(undefined)));
        if (!distribution?.Id) {
            yield* Effect.logInfo(`CloudFront Distribution read: distribution ${distributionId} not found`);
            return undefined;
        }
        yield* Effect.logInfo(`CloudFront Distribution read: loading config and tags for ${distributionId}`);
        ...
        yield* Effect.logInfo(`CloudFront Distribution read: loaded ${distributionId} status=${distribution.Status} enabled=${config.DistributionConfig?.Enabled ?? "unknown"} etag=${config.ETag ?? "missing"} tags=${Object.keys(tags).length}`);
        return {
            distribution,
    ...
    const getByCallerReference = Effect.fn(function* (callerReference) {
        yield* Effect.logInfo(`CloudFront Distribution read: searching for callerReference=${callerReference}`);
        let marker;
        do {
            ...
            for (const item of listed.DistributionList?.Items ?? []) {
                ...
                if (config?.DistributionConfig?.CallerReference === callerReference) {
                    yield* Effect.logInfo(`CloudFront Distribution read: recovered ${item.Id} for callerReference=${callerReference}`);
        ...
        yield* Effect.logInfo(`CloudFront Distribution read: no distribution found for callerReference=${callerReference}`);
        return undefined;
    });
    const waitForDeletionReady = Effect.fn(function* (distributionId) {
        class DistributionPendingDeletionReadiness extends Data.TaggedError("DistributionPendingDeletionReadiness") {
        }
        yield* Effect.logInfo(`CloudFront Distribution delete: waiting for ${distributionId} to become disabled and deployed`);
        return yield* Effect.logInfo(`CloudFront Distribution delete: waiting for ${distributionId} to become disabled and deployed`).pipe(
        ...
        Effect.andThen(() => getCurrent(distributionId).pipe(Effect.timeout(30_000), Effect.catchTag("TimeoutError", () => Effect.fail(new DistributionPendingDeletionReadiness({
            message: `Timed out reading distribution ${distributionId} while waiting for deletion readiness`,
        }))))), Effect.flatMap(Effect.fn(function* (current) {
            if (!current) {
                yield* Effect.logInfo(`CloudFront Distribution delete: ${distributionId} already absent while waiting`);
                return undefined;
            }
            if (current.config.Enabled ||
                current.distribution.Status !== "Deployed") {
                yield* Effect.logInfo(`CloudFront Distribution delete: ${distributionId} not ready enabled=${current.config.Enabled} status=${current.distribution.Status}`);
                return yield* Effect.fail(new DistributionPendingDeletionReadiness({
                    message: `Distribution ${distributionId} is not yet ready for deletion`,
                }));
            }
            yield* Effect.logInfo(`CloudFront Distribution delete: ${distributionId} ready for delete with etag=${current.etag ?? "missing"}`);
            return current;
        })), Effect.retry({
    ...
    return {
        ...
        read: Effect.fn(function* ({ output }) {
            ...
            return toAttrs(current.distribution, current.etag, current.tags, yield* resolveUrl(current.distribution));
        }),
        ...
        list: () => Effect.gen(function* () {
            const summaries = yield* cloudfront.listDistributions
                .pages({})
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.DistributionList?.Items ?? [])));
            const rows = yield* Effect.forEach(summaries, (summary) => getCurrent(summary.Id).pipe(
            ...
            Effect.retry({
                ...
                ? Effect.map(resolveUrl(current.distribution), (url) => toAttrs(current.distribution, current.etag, current.tags, url))
                : Effect.succeed(undefined))), { concurrency: 10 });
            return rows.filter((row) => row !== undefined);
        }),
        reconcile: Effect.fn(function* ({ id, instanceId, news: _news, output, session, bindings, }) {
            ...
            if (!observed) {
                ...
                yield* Effect.logInfo(`CloudFront Distribution reconcile: callerReference=${callerReference} aliases=${news.aliases?.length ?? 0} origins=${news.origins.length} tags=${Object.keys(desiredTags).length}`);
                const created = yield* cloudfront
                    ...
                    .pipe(Effect.catch((error) => isAccessDenied(error)
                    ? Effect.gen(function* () {
                        yield* Effect.logInfo(`CloudFront Distribution reconcile: createDistributionWithTags denied, retrying without tags for callerReference=${callerReference}`);
                        const created = yield* cloudfront.createDistribution({
                            DistributionConfig: config,
                        });
                        if (created.Distribution?.ARN &&
                            Object.keys(desiredTags).length > 0) {
                            yield* Effect.logInfo(`CloudFront Distribution reconcile: tagging distribution ${created.Distribution.Id} after fallback`);
                    ...
                    : Effect.gen(function* () {
                        yield* Effect.logInfo(`CloudFront Distribution reconcile: createDistributionWithTags failed for callerReference=${callerReference} error=${String(error)}`);
                    ...
                    etag: created.ETag,
                    tags: desiredTags,
                })), Effect.catchTag("DistributionAlreadyExists", () => Effect.gen(function* () {
                    yield* Effect.logInfo(`CloudFront Distribution reconcile: callerReference=${callerReference} already exists, attempting recovery`);
                    const recovered = yield* getByCallerReference(callerReference);
                    if (!recovered?.distribution.Id) {
                        return yield* Effect.fail(new Error(`CloudFront distribution with caller reference '${callerReference}' already exists but could not be recovered`));
                    }
                    return {
                ...
                })), Effect.catchTag("InvalidArgument", (error) => isFunctionAssociationPending(error)
                    ? Effect.logInfo("CloudFront Distribution reconcile: function association not yet ready, retrying").pipe(Effect.andThen(Effect.fail(new DistributionFunctionAssociationPending({
                        message: error.message ??
                            "CloudFront function association pending",
                    }))))
                    : Effect.fail(error)), Effect.retry({
                ...
                yield* Effect.logInfo(`CloudFront Distribution reconcile: created ${created.distributionId} etag=${created.etag ?? "missing"}, waiting for deployment`);
                const deployed = yield* waitForDeployment(created.distributionId);
                yield* Effect.logInfo(`CloudFront Distribution reconcile: deployed ${created.distributionId} domain=${deployed.DomainName}`);
            ...
            yield* Effect.logInfo(`CloudFront Distribution reconcile: updating config for ${observed.distribution.Id} with etag=${observed.etag ?? "missing"}`);
            const updated = yield* cloudfront
                .updateDistribution({
                Id: observed.distribution.Id,
                IfMatch: observed.etag,
                DistributionConfig: mergeWithObservedConfig(toConfig(observed.config.CallerReference, news), observed.config),
            })
                .pipe(Effect.catchTag("InvalidArgument", (error) => isFunctionAssociationPending(error)
                ? Effect.logInfo("CloudFront Distribution reconcile: function association not yet ready, retrying").pipe(Effect.andThen(Effect.fail(new DistributionFunctionAssociationPending({
                    message: error.message ??
                        "CloudFront function association pending",
                }))))
            ...
            yield* Effect.logInfo(`CloudFront Distribution reconcile: distribution=${observed.distribution.Id} upsertTags=${upsert.length} removedTags=${removed.length}`);
            if (upsert.length > 0) {
                yield* Effect.logInfo(`CloudFront Distribution reconcile: tagging ${observed.distribution.Id} with ${upsert.length} tag(s)`);
                yield* cloudfront.tagResource({
            ...
            if (removed.length > 0) {
                yield* Effect.logInfo(`CloudFront Distribution reconcile: removing ${removed.length} tag(s) from ${observed.distribution.Id}`);
            ...
            yield* Effect.logInfo(`CloudFront Distribution reconcile: updated ${observed.distribution.Id} etag=${updated.ETag ?? "missing"}, waiting for deployment`);
            const deployed = yield* waitForDeployment(updated.Distribution.Id);
            yield* Effect.logInfo(`CloudFront Distribution reconcile: deployed ${observed.distribution.Id} domain=${deployed.DomainName}`);
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* Effect.logInfo(`CloudFront Distribution delete: distribution=${output.distributionId}`);
            const current = yield* getCurrent(output.distributionId);
            if (!current) {
                yield* Effect.logInfo(`CloudFront Distribution delete: ${output.distributionId} already absent`);
                return;
            }
            if (current.config.Enabled) {
                yield* Effect.logInfo(`CloudFront Distribution delete: disabling ${output.distributionId} before delete`);
            ...
            if (!latest) {
                yield* Effect.logInfo(`CloudFront Distribution delete: ${output.distributionId} disappeared before delete`);
                return;
            }
            yield* Effect.logInfo(`CloudFront Distribution delete: deleting ${output.distributionId} with etag=${latest.etag ?? "missing"}`);
            yield* cloudfront
                .deleteDistribution({
                Id: output.distributionId,
                IfMatch: latest.etag,
...
const toBehavior = (behavior) => ({
    ...
    TrustedKeyGroups: behavior.trustedKeyGroups
        ? {
            Enabled: behavior.trustedKeyGroups.length > 0,
            Quantity: behavior.trustedKeyGroups.length,
            Items: behavior.trustedKeyGroups,
        }
        : undefined,
...
const toOrigin = (origin) => {
    ...
    return {
        ...
        S3OriginConfig: isS3Origin
            ...
            : undefined,
        CustomOriginConfig: isVpcOrigin || isS3Origin
            ? undefined
            : {
                ...
                OriginKeepaliveTimeout: toWireSeconds(origin.customOriginConfig?.originKeepaliveTimeout),
                IpAddressType: origin.customOriginConfig?.ipAddressType,
                OriginMtlsConfig: origin.customOriginConfig?.originMtlsConfig
                    ? {
                        ClientCertificateArn: origin.customOriginConfig.originMtlsConfig
 ...
 */
const fillUndefined = (desired, observed) => {
    ...
    for (const key of Object.keys(observed)) {
        ...
        out[key] = fillUndefined(desiredObj[key], observed[key]);
    }
    return out;
};
/**
 ...
 */
export const mergeWithObservedConfig = (desired, observed) => {
    ...
    if (merged.Origins?.Items && observed.Origins?.Items) {
        ...
        merged.Origins = {
            ...merged.Origins,
            Items: merged.Origins.Items.map((item) => fillUndefined(item, observedOrigins.find((origin) => origin.Id === item.Id) ?? item)),
        };
    }
    if (merged.CacheBehaviors?.Items && observed.CacheBehaviors?.Items) {
        ...
        merged.CacheBehaviors = {
            ...merged.CacheBehaviors,
            Items: merged.CacheBehaviors.Items.map((item) => fillUndefined(item, observedBehaviors.find((behavior) => behavior.PathPattern === item.PathPattern) ?? item)),
        };
    }
    return merged;
};
const toConfig = (callerReference, props) => ({
    ...
    Origins: {
        Quantity: props.origins.length,
        Items: props.origins.map(toOrigin),
    },
    DefaultCacheBehavior: toBehavior(props.defaultCacheBehavior),
    CacheBehaviors: props.orderedCacheBehaviors
        ? {
            Quantity: props.orderedCacheBehaviors.length,
            Items: props.orderedCacheBehaviors.map((behavior) => toBehavior(behavior)),
        }
        : undefined,
    OriginGroups: props.originGroups
        ? {
            Quantity: props.originGroups.length,
            Items: props.originGroups.map((group) => ({
                Id: group.id,
                FailoverCriteria: {
                    StatusCodes: {
    ...
    CustomErrorResponses: props.customErrorResponses
        ? {
            Quantity: props.customErrorResponses.length,
            Items: props.customErrorResponses,
        }
        : undefined,
    ...
    ViewerCertificate: props.viewerCertificate
        ? {
            CloudFrontDefaultCertificate: props.viewerCertificate.cloudFrontDefaultCertificate,
            IAMCertificateId: props.viewerCertificate.iamCertificateId,
            ACMCertificateArn: props.viewerCertificate.acmCertificateArn,
            SSLSupportMethod: props.viewerCertificate.sslSupportMethod,
            MinimumProtocolVersion: props.viewerCertificate.minimumProtocolVersion,
            Certificate: props.viewerCertificate
                .certificate,
            CertificateSource: props.viewerCertificate.certificateSource,
        }
 ...
 */
const resolveUrl = Effect.fn("AWS.CloudFront.Distribution.url")(function* (distribution) {
    ...
    return yield* Effect.gen(function* () {
        ...
        const edge = (yield* response.json);
        if (edge === null ||
            typeof edge !== "object" ||
    ...
    }).pipe(Effect.timeout("10 seconds"), Effect.provide(FetchHttpClient.layer), 
...
const toAttrs = (distribution, etag, tags, url) => ({
    ...
    comment: typeof distribution.DistributionConfig.Comment === "string"
