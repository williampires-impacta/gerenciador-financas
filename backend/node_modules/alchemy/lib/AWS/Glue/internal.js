...
export const catalogArn = (region, accountId) => `arn:${PARTITION}:glue:${region}:${accountId}:catalog`;
/** `arn:aws:glue:{region}:{account}:database/{name}` */
export const databaseArn = (region, accountId, databaseName) => `arn:${PARTITION}:glue:${region}:${accountId}:database/${databaseName}`;
/** `arn:aws:glue:{region}:{account}:table/{database}/{table}` */
export const tableArn = (region, accountId, databaseName, tableName) => `arn:${PARTITION}:glue:${region}:${accountId}:table/${databaseName}/${tableName}`;
/** `arn:aws:glue:{region}:{account}:crawler/{name}` */
export const crawlerArn = (region, accountId, crawlerName) => `arn:${PARTITION}:glue:${region}:${accountId}:crawler/${crawlerName}`;
/** `arn:aws:glue:{region}:{account}:job/{name}` */
export const jobArn = (region, accountId, jobName) => `arn:${PARTITION}:glue:${region}:${accountId}:job/${jobName}`;
/** `arn:aws:glue:{region}:{account}:connection/{name}` */
export const connectionArn = (region, accountId, connectionName) => `arn:${PARTITION}:glue:${region}:${accountId}:connection/${connectionName}`;
/**
 ...
 */
export const fetchObservedTags = Effect.fn("AWS.Glue.fetchObservedTags")(function* (resourceArn) {
    const response = yield* glue
        .getTags({ ResourceArn: resourceArn })
        .pipe(Effect.catch(() => Effect.succeed({ Tags: undefined })));
    const tags = response.Tags ?? {};
    return Object.fromEntries(Object.entries(tags).filter((entry) => entry[1] !== undefined));
});
/**
 ...
 */
export const syncTags = Effect.fn("AWS.Glue.syncTags")(function* (resourceArn, observed, desired) {
 ...
 */
export const retryWhileCrawlerRunning = (self) => Effect.retry(self, {
 ...
 */
export const retryWhileConcurrentModification = (self) => Effect.retry(self, {
 ...
 */
export const retryWhileCrawlerTargetNotReady = (self) => Effect.retry(self, {
...
export const retryCrawlerDelete = (self) => Effect.retry(self, {
    while: (e) => e._tag === "CrawlerRunningException" ||
