 ...
 * @internal
 */
export const fetchResolverTags = (arn) => r53r.listTagsForResource.items({ ResourceArn: arn }).pipe(Stream.runCollect, Effect.map((chunk) => Object.fromEntries(Array.from(chunk).map((tag) => [tag.Key, tag.Value]))), Effect.catch(() => Effect.succeed({})));
/**
 ...
 * @internal
 */
export const syncResolverTags = Effect.fn(function* (arn, desired) {
 ...
 * @internal
 */
export const toResolverTagList = (tags) => Object.entries(tags).map(([Key, Value]) => ({ Key, Value }));
/**
