 ...
 * @resource
 */
export const ResolverRuleAssociation = Resource("AWS.Route53Resolver.ResolverRuleAssociation");
/**
 ...
 * @internal
 */
const retryAssociateRaces = (self) => Effect.retry(self, {
 ...
 * @internal
 */
const untilAssociationSettled = (self) => self.pipe(Effect.repeat({
 ...
 * @internal
 */
const retryDisassociateRaces = (self) => Effect.retry(self, {
    while: (e) => e._tag === "InvalidRequestException",
    schedule: Schedule.max([Schedule.fixed("3 seconds"), Schedule.recurs(20)]),
});
export const ResolverRuleAssociationProvider = () => Provider.effect(ResolverRuleAssociation, Effect.gen(function* () {
    const listAssociations = (resolverRuleId, vpcId) => r53r
    ...
    const observe = (resolverRuleId, vpcId) => listAssociations(resolverRuleId, vpcId).pipe(Effect.map((associations) => associations.find((association) => association.Status !== "DELETING" &&
    ...
    const observeAny = (resolverRuleId, vpcId) => listAssociations(resolverRuleId, vpcId).pipe(Effect.map((associations) => associations[0]));
    return ResolverRuleAssociation.Provider.of({
        ...
        list: () => r53r.listResolverRuleAssociations.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.ResolverRuleAssociations ?? [])
            .flatMap((association) => association.Id !== undefined &&
        ...
        diff: Effect.fn(function* ({ olds, news }) {
            if (!isResolved(news))
                return undefined;
            if (olds.resolverRuleId !== news.resolverRuleId ||
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (association === undefined) {
                association = yield* retryAssociateRaces(r53r.associateResolverRule({
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* retryDisassociateRaces(r53r.disassociateResolverRule({
