 ...
 */
export class Inspector2NotConverged extends Data.TaggedError("Inspector2NotConverged") {
}
/**
 ...
 * ```
 */
const EnablerResource = Resource("AWS.Inspector2.Enabler");
...
const statusOfType = (resourceState, type) => {
    const key = TYPE_TO_STATE_KEY[type];
    if (!resourceState || !key)
        return undefined;
    return resourceState[key]?.status;
};
const enabledTypes = (resourceState) => {
    if (!resourceState)
        return [];
    const out = [];
    for (const key of Object.keys(RESOURCE_STATE_KEYS)) {
...
const statusesOf = (resourceState, types) => Object.fromEntries(types.map((type) => [type, statusOfType(resourceState, type)]));
export const EnablerProvider = () => Provider.effect(EnablerResource, Effect.gen(function* () {
    const getAccount = (accountId) => inspector2
    ...
    const waitUntilEnabled = Effect.fn(function* (accountId, types) {
        const account = yield* getAccount(accountId).pipe(Effect.repeat({
            schedule: Schedule.spaced("5 seconds"),
            until: (account) => types.every((t) => statusOfType(account?.resourceState, t) === "ENABLED"),
            times: 10,
        }));
        if (!types.every((type) => statusOfType(account?.resourceState, type) === "ENABLED")) {
            return yield* Effect.fail(new Inspector2NotConverged({
                accountId,
                expected: "ENABLED",
    ...
    const disableManagedTypes = Effect.fn(function* (accountId, types) {
        if (types.length === 0)
            return;
        let account = yield* getAccount(accountId);
        const enabling = types.filter((type) => statusOfType(account?.resourceState, type) === "ENABLING");
        ...
        const enabled = types.filter((type) => statusOfType(account?.resourceState, type) === "ENABLED");
        if (enabled.length === 0)
            return;
        yield* inspector2
    ...
    return {
        read: Effect.fn(function* ({ output }) {
            ...
            const managed = output
                ? output.resourceTypes.filter((type) => statusOfType(account.resourceState, type) !== "DISABLED")
                : enabled;
            if (managed.length === 0)
                return undefined;
            const attrs = {
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            const toEnable = desired.filter((type) => !currentlyEnabled.includes(type) &&
            ...
            const noLongerManaged = previouslyManaged.filter((type) => !desired.includes(type));
