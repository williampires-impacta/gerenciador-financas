 ...
 * @resource
 */
export const Group = Resource("AWS.XRay.Group");
/**
...
const validateGroupName = (props) => props.groupName === "Default"
    ? Effect.fail(new XRayReservedGroupName({
...
export const GroupProvider = () => Provider.effect(Group, Effect.gen(function* () {
    ...
    const desiredInsights = (props) => ({
        InsightsEnabled: props.insightsEnabled ?? false,
        NotificationsEnabled: props.notificationsEnabled ?? false,
    });
    const observeGroup = (groupName) => xray.getGroup({ GroupName: groupName }).pipe(Effect.map((r) => r.Group), 
    // Typed synthetic tag for a missing group.
    Effect.catchTag("GroupNotFound", () => Effect.succeed(undefined)));
    const observedTags = (groupArn) => xray.listTagsForResource.items({ ResourceARN: groupArn }).pipe(Stream.runCollect, Effect.map((chunk) => Object.fromEntries(Array.from(chunk).map((t) => [t.Key, t.Value]))), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})));
    return Group.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            return Array.from(groups).flatMap((group) => group.GroupName && group.GroupARN && group.GroupName !== "Default"
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (live === undefined) {
                live = yield* xray
                    ...
                    .pipe(Effect.map((r) => r.Group), Effect.catchTag("GroupAlreadyExists", () => observeGroup(groupName)));
            }
