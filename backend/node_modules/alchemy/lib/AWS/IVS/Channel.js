 ...
 * @resource
 */
export const Channel = Resource("AWS.IVS.Channel");
/**
...
export const ChannelProvider = () => Provider.effect(Channel, Effect.gen(function* () {
    const toName = (id, props) => props.channelName
        ? Effect.succeed(props.channelName)
        : createPhysicalName({ id, maxLength: 128 });
    const toAttrs = Effect.fn(function* (channel) {
        if (!channel.arn ||
            !channel.name ||
            channel.ingestEndpoint === undefined ||
            channel.playbackUrl === undefined) {
            return yield* Effect.fail(new IvsChannelIncomplete({
    ...
    const getByArn = Effect.fn(function* (arn) {
        const response = yield* ivs.getChannel({ arn }).pipe(retryWhileThrottled, Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    ...
    const findByName = Effect.fn(function* (name) {
        const summaries = yield* ivs.listChannels
            ...
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.channels)), retryWhileThrottled);
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (news.authorized !== undefined &&
                observed.authorized !== news.authorized) {
                patch.authorized = news.authorized;
            }
            if (news.recordingConfigurationArn !== undefined &&
            ...
            if (news.insecureIngest !== undefined &&
            ...
            if (news.playbackRestrictionPolicyArn !== undefined &&
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* ivs.deleteChannel({ arn: output.channelArn }).pipe(retryWhileThrottled, retryWhileConflict, Effect.catchTag("ResourceNotFoundException", () => Effect.void));
        }),
        list: () => ivs.listChannels.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.channels)), Effect.flatMap(Effect.forEach((summary) => summary.arn === undefined
            ? Effect.succeed(undefined)
            : getByArn(summary.arn).pipe(Effect.flatMap((channel) => channel === undefined
                ? Effect.succeed(undefined)
                : toAttrs(channel))), { concurrency: 5 })), Effect.map((items) => items.filter((item) => item !== undefined))),
    };
}));
