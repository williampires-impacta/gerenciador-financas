...
import { toRedactedString } from "../IAM/common.js";
 ...
 * @resource
 */
export const StreamKey = Resource("AWS.IVS.StreamKey");
/**
...
export const StreamKeyProvider = () => Provider.effect(StreamKey, Effect.gen(function* () {
    const toAttrs = Effect.fn(function* (streamKey) {
        if (!streamKey.arn || !streamKey.channelArn) {
            return yield* Effect.fail(new IvsStreamKeyIncomplete({
    ...
    const getByArn = Effect.fn(function* (arn) {
        const response = yield* ivs.getStreamKey({ arn }).pipe(retryWhileThrottled, Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
     ...
     * key is gone with it).
     */
    const findByChannel = Effect.fn(function* (channelArn) {
        const page = yield* ivs.listStreamKeys({ channelArn }).pipe(retryWhileThrottled, Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (observed === undefined) {
                observed = yield* ivs
                    ...
                    .pipe(retryWhileThrottled, Effect.map((r) => r.streamKey), Effect.catchTag("ServiceQuotaExceededException", () => findByChannel(news.channelArn)));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* ivs.deleteStreamKey({ arn: output.streamKeyArn }).pipe(retryWhileThrottled, Effect.catchTag("ResourceNotFoundException", () => Effect.void));
        }),
