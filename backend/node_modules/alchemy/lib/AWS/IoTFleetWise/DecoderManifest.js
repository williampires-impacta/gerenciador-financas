 ...
 * @resource
 */
export const DecoderManifest = Resource("AWS.IoTFleetWise.DecoderManifest");
export const DecoderManifestProvider = () => Provider.effect(DecoderManifest, Effect.gen(function* () {
    const toName = (id, props) => props.decoderManifestName
        ? Effect.succeed(props.decoderManifestName)
        : createPhysicalName({ id, maxLength: 100 });
    const readManifest = Effect.fn(function* (name) {
        return yield* iotfleetwise.getDecoderManifest({ name }).pipe(inFleetWiseRegion, Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    });
    const observeAfterWrite = Effect.fn(function* (name) {
        return yield* readManifest(name).pipe(Effect.flatMap((manifest) => manifest === undefined
            ? Effect.fail(new Error(`Decoder manifest '${name}' not found`))
            : Effect.succeed(manifest)), retryObservation);
    });
    const readInterfaces = Effect.fn(function* (name) {
    ...
    const readDecoders = Effect.fn(function* (name) {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const observedDecoderByFqn = new Map(observedDecoders.map((d) => [d.fullyQualifiedName, d]));
            const desiredDecoderByFqn = new Map(desiredDecoders.map((d) => [d.fullyQualifiedName, d]));
            const interfacesToAdd = desiredInterfaces.filter((i) => !observedInterfaceById.has(i.interfaceId));
            ...
            const decodersToAdd = desiredDecoders.filter((d) => !observedDecoderByFqn.has(d.fullyQualifiedName));
            ...
            if (interfacesToAdd.length > 0 ||
                ...
                yield* iotfleetwise
                    ...
                    networkInterfacesToUpdate: interfacesToUpdate.length > 0
                        ? interfacesToUpdate
                        : undefined,
                    networkInterfacesToRemove: interfacesToRemove.length > 0
                        ? interfacesToRemove
                        : undefined,
        ...
        list: () => iotfleetwise.listDecoderManifests.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((summary) => summary.name !== undefined && summary.arn !== undefined
            ? [
                {
...
//# sourceMappingURL=DecoderManifest.js.map