 ...
 * @resource
 */
export const TargetGroupAttachment = Resource("AWS.ELBv2.TargetGroupAttachment");
...
const retryThroughPermissionPropagation = (self) => Effect.retry(self, {
...
export const TargetGroupAttachmentProvider = () => Provider.succeed(TargetGroupAttachment, {
    ...
    diff: Effect.fn(function* ({ olds, news }) {
        ...
        if (olds.targetGroupArn !== news.targetGroupArn ||
    ...
    read: Effect.fn(function* ({ output }) {
        ...
        const registered = health?.TargetHealthDescriptions?.some((d) => d.Target?.Id === output.targetId &&
    ...
    list: Effect.fn(function* () {
        const targetGroupArns = yield* elbv2.describeTargetGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.TargetGroups ?? []).flatMap((tg) => tg.TargetGroupArn ? [tg.TargetGroupArn] : []))));
        const rows = yield* Effect.forEach(targetGroupArns, (targetGroupArn) => elbv2.describeTargetHealth({ TargetGroupArn: targetGroupArn }).pipe(Effect.map((health) => (health.TargetHealthDescriptions ?? []).flatMap((d) => d.Target?.Id
            ? [
                {
                    targetGroupArn,
        ...
        Effect.catchTag(["TargetGroupNotFoundException"], () => Effect.succeed([]))), { concurrency: 10 });
        const result = rows.flat();
        return result;
    }),
    reconcile: Effect.fn(function* ({ news, session }) {
        const targetGroupArn = news.targetGroupArn;
        ...
        const registered = health?.TargetHealthDescriptions?.some((d) => d.Target?.Id === news.targetId &&
        ...
        if (!registered) {
            yield* retryThroughPermissionPropagation(elbv2.registerTargets({
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        Effect.catchTag(["InvalidTargetException", "TargetGroupNotFoundException"], () => Effect.void));
    }),
});
