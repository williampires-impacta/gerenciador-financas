 ...
 * @resource
 */
export const ListenerCertificate = Resource("AWS.ELBv2.ListenerCertificate");
export const ListenerCertificateProvider = () => Provider.succeed(ListenerCertificate, {
    ...
    read: Effect.fn(function* ({ output }) {
        ...
        const attached = yield* elbv2.describeListenerCertificates
            ...
            .pipe(Stream.filter((c) => !c.IsDefault && c.CertificateArn === output.certificateArn), Stream.runHead, Effect.map(Option.isSome), Effect.catchTag("ListenerNotFoundException", () => Effect.succeed(false)));
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(listenerArns.flat(), (listenerArn) => elbv2.describeListenerCertificates
            .items({ ListenerArn: listenerArn })
            .pipe(Stream.filter((c) => !c.IsDefault && c.CertificateArn != null), Stream.map((c) => ({
            listenerArn,
            certificateArn: c.CertificateArn,
        })), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ListenerNotFoundException", () => Effect.succeed([]))), { concurrency: 10 });
    ...
    reconcile: Effect.fn(function* ({ news, session }) {
        ...
        const attached = yield* elbv2.describeListenerCertificates
            .items({ ListenerArn: listenerArn })
            .pipe(Stream.filter((c) => !c.IsDefault && c.CertificateArn === news.certificateArn), Stream.runHead, Effect.map(Option.isSome));
