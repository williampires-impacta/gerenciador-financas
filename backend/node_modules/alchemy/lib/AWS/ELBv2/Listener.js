...
import { serializeActions } from "./common.js";
/**
 ...
 * @resource
 */
export const Listener = Resource("AWS.ELBv2.Listener");
// Build the default-action wire shape from desugared props.
const desiredDefaultActions = (props) => {
...
const desiredMutualAuth = (props) => props.mutualAuthentication
    ? {
...
export const ListenerProvider = () => Provider.succeed(Listener, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(loadBalancerArns, (loadBalancerArn) => elbv2.describeListeners
            .pages({ LoadBalancerArn: loadBalancerArn })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Listeners ?? [])
            .filter((l) => l.ListenerArn != null)
            .map((listener) => ({
            listenerArn: listener.ListenerArn,
            loadBalancerArn: listener.LoadBalancerArn,
            targetGroupArn: defaultForwardTargetGroup(listener.DefaultActions),
        ...
        Effect.catchTag("LoadBalancerNotFoundException", () => Effect.succeed([])), Effect.catchTag("ListenerNotFoundException", () => Effect.succeed([]))), { concurrency: 10 });
        const result = rows.flat();
        return result;
    }),
    reconcile: Effect.fn(function* ({ news, output, session }) {
        const loadBalancerArn = news.loadBalancerArn;
        ...
        if (!listener?.ListenerArn) {
            const listed = yield* elbv2
                ...
                .pipe(Effect.catchTag("LoadBalancerNotFoundException", () => Effect.succeed(undefined)), Effect.catchTag("ListenerNotFoundException", () => Effect.succeed(undefined)));
