import { AwsCredentialProviderError, Credentials, } from "@distilled.cloud/aws/Credentials";
 ...
 */
export const fromEnvironment = Layer.effect(Credentials, Effect.gen(function* () {
 ...
 */
export const makeAssumeRoleResolver = (options) => Effect.gen(function* () {
    const resolve = Effect.gen(function* () {
        ...
        Effect.retry({
            while: (error) => error.name === "AccessDeniedException" ||
    ...
    Effect.provide(options.base.pipe(Layer.provideMerge(Layer.succeed(Region, Effect.succeed(options.region ?? "us-east-1"))), Layer.provideMerge(FetchHttpClient.layer))), Effect.mapError((cause) => cause instanceof AwsCredentialProviderError
        ? cause
    ...
    const cache = yield* Ref.make(undefined);
    const refreshLock = yield* Semaphore.make(1);
    const isFresh = (creds) => creds !== undefined &&
 ...
 */
export const fromAssumeRole = (options) => Layer.effect(Credentials, makeAssumeRoleResolver({
