 ...
 * ```
 */
const InsightResource = Resource("AWS.SecurityHub.Insight");
export { InsightResource as Insight };
export const InsightProvider = () => Provider.effect(InsightResource, Effect.gen(function* () {
    const toName = (id, props) => props.name
        ? Effect.succeed(props.name)
        : createPhysicalName({ id, maxLength: 128 });
    const getInsight = (arn) => securityhub.getInsights({ InsightArns: [arn] }).pipe(Effect.map((r) => r.Insights?.[0]), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)), 
    ...
    const listInsights = securityhub.getInsights.pages({}).pipe(Stream.runCollect, Effect.map((pages) => Array.from(pages).flatMap((page) => page.Insights ?? [])), Effect.catchTag("InvalidAccessException", () => Effect.succeed([])));
    const buildAttrs = (i) => ({
    ...
    return {
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* securityhub
                ...
                .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void), Effect.catchTag("InvalidAccessException", () => Effect.void), Effect.catchTag("InvalidInputException", () => Effect.void));
        }),
    };
}));
