 ...
 * @resource
 */
export const Rule = Resource("AWS.Rbin.Rule");
/**
...
const validateLock = (props) => props.lockConfiguration !== undefined &&
    ((props.resourceTags?.length ?? 0) > 0 ||
        (props.excludeResourceTags?.length ?? 0) > 0)
    ? Effect.fail(new RbinLockUnsupported({
        message: "lockConfiguration is only supported on Region-level retention rules without excludeResourceTags — remove resourceTags/excludeResourceTags or the lock.",
    }))
    : Effect.void;
...
const resourceTagsKey = (tags) => JSON.stringify((tags ?? [])
...
const readRule = (identifier) => rbin
    .getRule({ Identifier: identifier })
    .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
/** Observed tags on the rule itself (empty on a NotFound race). */
const readRuleTags = (ruleArn) => rbin.listTagsForResource({ ResourceArn: ruleArn }).pipe(Effect.map((r) => tagRecord(r.Tags ?? [])), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})));
/** Collect all rule summaries for one resource type (exhaustive pages). */
const listRuleSummaries = (resourceType) => rbin.listRules.items({ ResourceType: resourceType }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)), 
// Tolerate a resource type the Region/partition doesn't support yet.
Effect.catchTag("ValidationException", () => Effect.succeed([])));
 ...
 */
const retryWhileRulePending = (self) => Effect.retry(self, {
...
const waitForRuleAvailable = (identifier) => retryWhileRulePending(Effect.gen(function* () {
...
const retryWhileRuleExists = (self) => Effect.retry(self, {
...
export const RuleProvider = () => Provider.effect(Rule, Effect.gen(function* () {
    return Rule.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            const summaries = yield* Effect.forEach(RULE_RESOURCE_TYPES, (resourceType) => listRuleSummaries(resourceType), { concurrency: 3 }).pipe(Effect.map((groups) => groups.flat()));
            const items = yield* Effect.forEach(summaries.filter((s) => s.Identifier !== undefined), (summary) => readRule(summary.Identifier).pipe(Effect.map((live) => live === undefined ? undefined : toAttributes(live))), { concurrency: 5 });
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            for (const resourceType of resourceTypes) {
                ...
                for (const summary of summaries) {
                    if (!summary.Identifier || !summary.RuleArn)
                        continue;
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const retentionChanged = live.RetentionPeriod?.RetentionPeriodValue !==
            ...
            const resourceTagsChanged = resourceTagsKey(live.ResourceTags) !==
                resourceTagsKey(desiredResourceTags);
            const excludeTagsChanged = resourceTagsKey(live.ExcludeResourceTags) !==
            ...
            if (news.lockConfiguration !== undefined) {
                ...
                const lockDelta = live.LockState !== "locked" ||
                ...
                yield* session.note(`Unlock initiated for ${identifier}; rule stays protected (pending_unlock) until the unlock delay expires`);
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            // a half-deleted rule.
            yield* retryWhileRuleExists(Effect.gen(function* () {
                const live = yield* readRule(output.identifier);
                if (live !== undefined) {
