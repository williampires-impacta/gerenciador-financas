 ...
 * @resource
 */
export const Pipe = Resource("AWS.Pipes.Pipe");
/**
...
const isFailedState = (state) => state.includes("FAILED");
const unwrap = (value) => value === undefined
    ? undefined
    : Redacted.isRedacted(value)
        ? Redacted.value(value)
        : value;
const unwrapTags = (tags) => {
    const out = {};
...
const retryWhilePresent = (self) => Effect.retry(self, {
 ...
 */
const retryUntilRoleAssumable = (self) => Effect.retry(self, {
    while: (e) => {
        if (e._tag !== "ValidationException")
            return false;
 ...
 */
const retryWhileConflicting = (self) => Effect.retry(self, {
...
const subsetMatches = (desired, observed) => {
    ...
    if (typeof want === "object" && want !== null) {
        if (typeof have !== "object" || have === null)
            return false;
        return Object.entries(want).every(([key, value]) => subsetMatches(value, have[key]));
    }
    return want === have;
};
const filterPatternsOf = (criteria) => (criteria?.Filters ?? [])
    .map((filter) => unwrap(filter.Pattern))
    .filter((pattern) => pattern !== undefined);
const sameFilters = (a, b) => a.length === b.length && a.every((pattern, i) => pattern === b[i]);
const startingPositionOf = (params) => params?.KinesisStreamParameters?.StartingPosition ??
 ...
 */
const toUpdateSourceParameters = (params) => ({
    ...
    KinesisStreamParameters: params?.KinesisStreamParameters
        ? {
            ...
            MaximumBatchingWindowInSeconds: params.KinesisStreamParameters.MaximumBatchingWindowInSeconds,
    ...
    DynamoDBStreamParameters: params?.DynamoDBStreamParameters
        ? {
            ...
            MaximumBatchingWindowInSeconds: params.DynamoDBStreamParameters.MaximumBatchingWindowInSeconds,
 ...
 */
const sourceParametersSansFilters = (params) => {
    if (params === undefined)
        return undefined;
    const { FilterCriteria: _filters, ...rest } = params;
    return rest;
};
export const PipeProvider = () => Provider.effect(Pipe, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.pipeName ?? (yield* createPhysicalName({ id, maxLength: 64 })));
    });
    const describeOrUndefined = (pipeName) => pipes
     ...
     */
    const awaitSettled = Effect.fn(function* (pipeName) {
        const described = yield* describeOrUndefined(pipeName).pipe(Effect.flatMap((d) => d?.CurrentState !== undefined &&
            TRANSITIONAL_STATES.has(d.CurrentState)
            ? Effect.fail(new PipeStillTransitioning({
                pipeName,
                state: d.CurrentState,
            }))
            : Effect.succeed(d)), retryWhileTransitioning, Effect.catchTag("PipeStillTransitioning", (e) => Effect.fail(new PipeStateTimeout({
            pipeName,
            state: e.state,
            message: `Pipe '${pipeName}' did not settle within the bounded wait (last state: ${e.state})`,
        }))));
        if (described?.CurrentState !== undefined &&
            isFailedState(described.CurrentState)) {
            return yield* Effect.fail(new PipeFailed({
                pipeName,
    ...
    const awaitGone = Effect.fn(function* (pipeName) {
        yield* pipes.describePipe({ Name: pipeName }).pipe(Effect.flatMap((d) => Effect.fail(new PipeStillPresent({ pipeName, state: d.CurrentState }))), Effect.catchTag("NotFoundException", () => Effect.void), retryWhilePresent, Effect.catchTag("PipeStillPresent", (e) => Effect.fail(new PipeStateTimeout({
            pipeName,
            state: e.state,
    ...
    return Pipe.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            const summaries = yield* pipes.listPipes.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Pipes ?? [])));
            return summaries
                .filter((p) => p.Name !== undefined && p.Arn !== undefined)
                .map((p) => ({
                pipeName: p.Name,
                pipeArn: p.Arn,
        ...
        diff: Effect.fn(function* ({ id, news, olds }) {
            ...
            if (startingPositionOf(olds.sourceParameters) !==
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let observed = yield* awaitSettled(pipeName).pipe(Effect.catchTag("PipeFailed", (e) => Effect.gen(function* () {
                yield* session.note(`Pipe '${pipeName}' is in failed state ${e.state} (${e.stateReason ?? "no reason"}); deleting and recreating`);
                yield* pipes.deletePipe({ Name: pipeName }).pipe(retryWhileConflicting, Effect.catchTag("NotFoundException", () => Effect.void));
            ...
            const needsUpdate = (news.description !== undefined &&
                ...
                !sameFilters(filterPatternsOf(observed.SourceParameters?.FilterCriteria), filterPatternsOf(news.sourceParameters?.FilterCriteria)) ||
                !subsetMatches(sourceParametersSansFilters(news.sourceParameters), observed.SourceParameters) ||
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            retryWhileConflicting, Effect.catchTag("NotFoundException", () => Effect.void));
