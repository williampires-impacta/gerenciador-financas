...
import { toPath } from "../../FQN.js";
import * as Namespace from "../../Namespace.js";
import * as Output from "../../Output.js";
import { Stack } from "../../Stack.js";
import { Stage } from "../../Stage.js";
import { KvEntries } from "../CloudFront/KvEntries.js";
import { KvRoutesUpdate } from "../CloudFront/KvRoutesUpdate.js";
/**
 * Narrow a normalized `domain` prop to its Router-attached shape.
 * @internal
 */
export const asRouterDomain = (domain) => domain && "router" in domain && domain.router
    ? domain
    : undefined;
/**
 ...
 */
const toHostPatternRegex = (pattern) => pattern.replace(/[.+?^${}()|[\]\\]/g, "\\$&").replace(/\*/g, ".*");
/**
 ...
 * @internal
 */
export const registerDevRouterRoute = Effect.fn("AWS.Website.DevRouterRoute")(function* (domain, 
...
devUrl) {
    ...
    const hostPatterns = [
        ...
        ...(domain.aliases ?? []).map((alias, index) => [
            `RoutesUpdateAlias${index + 1}`,
            alias,
        ]),
        ...(domain.redirects ?? []).map((redirect, index) => [
            `RoutesUpdateRedirect${index + 1}`,
            redirect,
        ]),
    ];
    yield* Effect.forEach(hostPatterns, ([routeId, pattern]) => KvRoutesUpdate(routeId, {
        store: router.kvStoreArn,
        namespace: router.kvNamespace,
        key: "routes",
        entry: [
            "site",
            kvNamespace,
    ...
    const redirect = domain.redirects?.length && domain.name
    ...
    yield* KvEntries("KvEntries", {
        ...
        entries: {
            metadata: Output.map(Output.asOutput(devUrl), (resolved) => {
                if (!resolved) {
                    throw new Error("A Router-attached site needs a dev server URL to route to during `alchemy dev` — set `dev.url` (or make the dev command print its own localhost URL).");
                }
                return JSON.stringify({
                    base: routerPathPrefix && routerPathPrefix !== "/"
                        ? routerPathPrefix
                        : undefined,
...
//# sourceMappingURL=DevRouterRoute.js.map