...
import { retryUntilListenerDeletable, withGaRegion } from "./common.js";
/**
 ...
 * @resource
 */
export const Listener = Resource("AWS.GlobalAccelerator.Listener");
const toWirePortRanges = (ranges) => ranges.map((r) => ({ FromPort: r.fromPort, ToPort: r.toPort }));
const fromWirePortRanges = (ranges) => (ranges ?? []).map((r) => ({
    fromPort: r.FromPort ?? 0,
    toPort: r.ToPort ?? 0,
}));
const normalizeRanges = (ranges) => JSON.stringify([...ranges].sort((a, b) => a.fromPort - b.fromPort || a.toPort - b.toPort));
// Listener ARNs embed the parent accelerator ARN:
// arn:aws:globalaccelerator::{account}:accelerator/{id}/listener/{id}
const acceleratorArnOf = (listenerArn) => listenerArn.split("/listener/")[0];
const toAttributes = (l, listenerArn) => ({
    listenerArn,
...
const describeListener = Effect.fn(function* (listenerArn) {
    return yield* withGaRegion(ga.describeListener({ ListenerArn: listenerArn })).pipe(Effect.map((r) => r.Listener), Effect.catchTag("ListenerNotFoundException", () => Effect.succeed(undefined)));
});
...
const findListener = Effect.fn(function* (acceleratorArn, portRanges) {
    if (!acceleratorArn || !portRanges)
        return undefined;
...
export const ListenerProvider = () => Provider.succeed(Listener, {
    ...
    read: Effect.fn(function* ({ olds, output }) {
        if (output?.listenerArn) {
            ...
            if (live)
                return toAttributes(live, output.listenerArn);
        }
    ...
    reconcile: Effect.fn(function* ({ news, output, instanceId, session }) {
        ...
        if (!listenerArn) {
            const created = yield* withGaRegion(ga.createListener({
        ...
        const drift = normalizeRanges(fromWirePortRanges(live?.PortRanges)) !==
            normalizeRanges(news.portRanges) ||
            (live?.Protocol ?? "TCP") !== news.protocol ||
            (live?.ClientAffinity ?? "NONE") !== desiredAffinity;
        if (drift) {
            const updated = yield* withGaRegion(ga.updateListener({
    ...
    delete: Effect.fn(function* ({ output }) {
        yield* retryUntilListenerDeletable(withGaRegion(ga.deleteListener({ ListenerArn: output.listenerArn }))).pipe(Effect.catchTag("ListenerNotFoundException", () => Effect.void));
    }),
});
