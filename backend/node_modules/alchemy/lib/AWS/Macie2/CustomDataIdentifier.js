 ...
 * ```
 */
const CustomDataIdentifierResource = Resource("AWS.Macie2.CustomDataIdentifier");
...
const buildIdentifierAttrs = (id, live) => ({
    id,
    arn: live.arn,
    name: live.name,
});
export const CustomDataIdentifierProvider = () => Provider.effect(CustomDataIdentifierResource, Effect.gen(function* () {
    const getIdentifier = (id) => macie2.getCustomDataIdentifier({ id }).pipe(
    // Deleting is a soft delete — a deleted identifier still answers,
    // flagged `deleted: true`. Treat it (and a disabled Macie session)
    // as gone.
    Effect.map((d) => (d.deleted ? undefined : d)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)), Effect.catchTag("AccessDeniedException", () => Effect.succeed(undefined)));
    return {
        stables: ["id", "arn", "name"],
        ...
        list: () => Effect.gen(function* () {
            const pages = yield* macie2.listCustomDataIdentifiers
                .pages({})
                .pipe(Stream.runCollect, Effect.catchTag("AccessDeniedException", () => Effect.succeed([])));
            const out = [];
            for (const page of pages) {
                for (const summary of page.items ?? []) {
                    const live = yield* getIdentifier(summary.id);
                    if (live)
                        out.push(buildIdentifierAttrs(summary.id, live));
                }
            }
            return out;
        }),
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!live || identifierId === undefined) {
                ...
                const created = yield* retryThroughEnablement(macie2.createCustomDataIdentifier({
                    name,
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* macie2.deleteCustomDataIdentifier({ id: output.id }).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void), Effect.catchTag("AccessDeniedException", () => Effect.void));
        }),
    };
}));
