...
export const CE_REGION = "us-east-1";
...
export const pinCe = (effect) => effect.pipe(Effect.provideService(Region, Effect.succeed(CE_REGION)), Effect.retry({
...
export const toResourceTags = (tags) => Object.entries(tags).map(([Key, Value]) => ({ Key, Value }));
 ...
 */
export const fetchCeTags = Effect.fn(function* (resourceArn) {
    const listed = yield* pinCe(ce.listTagsForResource({ ResourceArn: resourceArn })).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({ ResourceTags: [] })));
 ...
 */
export const syncCeTags = Effect.fn(function* (resourceArn, desiredTags) {
