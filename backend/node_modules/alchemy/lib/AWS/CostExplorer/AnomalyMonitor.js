 ...
 * @resource
 */
export const AnomalyMonitor = Resource("AWS.CostExplorer.AnomalyMonitor");
export const AnomalyMonitorProvider = () => Provider.effect(AnomalyMonitor, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.monitorName ??
            (yield* createPhysicalName({ id, maxLength: 100 })));
    });
    // Observe by ARN — the typed miss tag is UnknownMonitorException.
    const getByArn = (monitorArn) => pinCe(ce.getAnomalyMonitors({ MonitorArnList: [monitorArn] })).pipe(Effect.map((r) => r.AnomalyMonitors[0]), Effect.catchTag("UnknownMonitorException", () => Effect.succeed(undefined)));
    // Fallback observation when no ARN is cached: scan all monitors for the
    // deterministic physical name.
    const findByName = (monitorName) => pinCe(ce.getAnomalyMonitors.items({}).pipe(Stream.filter((m) => m.MonitorName === monitorName), Stream.take(1), Stream.runCollect)).pipe(Effect.map((chunk) => Array.from(chunk)[0]));
    ...
    return AnomalyMonitor.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            const monitors = yield* pinCe(ce.getAnomalyMonitors.items({}).pipe(Stream.runCollect)).pipe(Effect.map((chunk) => Array.from(chunk)));
            return yield* Effect.forEach(monitors.filter((m) => m.MonitorArn !== undefined), toAttrs, { concurrency: 10 });
        }),
        read: Effect.fn(function* ({ id, olds, output }) {
            const live = output?.monitorArn
                ? yield* getByArn(output.monitorArn)
                : yield* findByName(yield* createName(id, olds ?? {}));
            if (live?.MonitorArn === undefined)
                return undefined;
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const matchesDesired = (m) => m.MonitorType === news.monitorType &&
            ...
            const live = output?.monitorArn
                ? yield* getByArn(output.monitorArn)
                : yield* findByName(name).pipe(Effect.map((m) => m !== undefined && matchesDesired(m) ? m : undefined));
            ...
            if (monitorArn === undefined) {
                ...
                const created = yield* pinCe(ce.createAnomalyMonitor({
                    AnomalyMonitor: {
                        MonitorName: name,
                ...
                })).pipe(Effect.catchTag("AnomalyMonitorAlreadyExists", (error) => findByName(name).pipe(Effect.flatMap((m) => m?.MonitorArn !== undefined && matchesDesired(m)
                ...
                yield* pinCe(ce.updateAnomalyMonitor({
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* pinCe(ce.deleteAnomalyMonitor({ MonitorArn: output.monitorArn })).pipe(Effect.catchTag("UnknownMonitorException", () => Effect.void));
        }),
    });
}));
