 ...
 * @resource
 */
export const Flow = Resource("AWS.MediaConnect.Flow");
...
export const FlowProvider = () => Provider.effect(Flow, Effect.gen(function* () {
    ...
    const readFlow = Effect.fn(function* (flowArn) {
    ...
    const findFlowArnByName = Effect.fn(function* (name) {
    ...
    const readFlowTags = Effect.fn(function* (flowArn) {
        const response = yield* mediaconnect
    ...
    const waitUntilSettled = Effect.fn(function* (flowArn) {
        const policy = Schedule.max([
            Schedule.fixed("5 seconds"),
            Schedule.recurs(36),
        ]);
        return yield* readFlow(flowArn).pipe(Effect.flatMap((flow) => {
            if (flow === undefined) {
                return Effect.fail(new Error(`MediaConnect flow '${flowArn}' not found`));
            }
            if (flow.Status !== "STANDBY" &&
                flow.Status !== "ACTIVE" &&
                flow.Status !== "ERROR") {
                return Effect.fail(new Error(`MediaConnect flow '${flowArn}' still settling (status: ${flow.Status})`));
            }
    ...
    const waitUntilGone = Effect.fn(function* (flowArn) {
        const policy = Schedule.max([
            Schedule.fixed("5 seconds"),
            Schedule.recurs(36),
        ]);
        yield* readFlow(flowArn).pipe(Effect.flatMap((flow) => flow === undefined
            ? Effect.void
            : Effect.fail(new Error(`MediaConnect flow '${flowArn}' still exists (status: ${flow.Status})`))), Effect.retry({ schedule: policy }));
    });
    const toAttrs = Effect.fn(function* (flow) {
        const outputs = [];
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            for (const desired of props.outputs ?? []) {
                if (desired.Name === undefined) {
                    return yield* Effect.fail(new FlowOutputNameMissing({
            ...
            for (const desired of desiredOutputs) {
                ...
                if (desired.Destination !== undefined &&
                ...
                if (desired.CidrAllowList !== undefined &&
                ...
                if (desired.SmoothingLatency !== undefined &&
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            const settled = yield* waitUntilSettled(arn).pipe(Effect.catch(() => Effect.succeed(undefined)));
        ...
        list: () => Effect.gen(function* () {
            ...
            for (const flow of listed) {
                if (flow.FlowArn !== undefined)
                    arns.push(flow.FlowArn);
            }
            const flows = yield* Effect.forEach(arns, (arn) => readFlow(arn), {
                concurrency: 4,
            });
            return yield* Effect.forEach(flows.filter((flow) => flow !== undefined), (flow) => toAttrs(flow), { concurrency: 4 });
        }),
    };
}));
