 ...
 */
const untilTerminalStatus = (self) => Effect.repeat(Effect.map(self, (response) => response.Operation), {
 ...
 */
export const awaitOperation = Effect.fn("AWS.CloudMap.awaitOperation")(function* (operationId) {
    const operation = yield* untilTerminalStatus(sd.getOperation({ OperationId: operationId }));
    if (operation?.Status !== "SUCCESS") {
        return yield* Effect.fail(new CloudMapOperationFailed({
            operationId,
 ...
 */
export const deregisterAllInstances = Effect.fn("AWS.CloudMap.deregisterAllInstances")(function* (serviceId) {
    const operationIds = [];
    let nextToken;
    do {
        ...
        for (const instance of page.Instances ?? []) {
            ...
            Effect.catchTag(["InstanceNotFound", "DuplicateRequest"], () => Effect.succeed(undefined)));
...
export const findNamespaceByName = Effect.fn("AWS.CloudMap.findNamespaceByName")(function* (type, name) {
 ...
 */
export const observeNamespace = Effect.fn("AWS.CloudMap.observeNamespace")(function* (type, name, namespaceId) {
    if (namespaceId !== undefined) {
        const byId = yield* sd.getNamespace({ Id: namespaceId }).pipe(Effect.map((r) => r.Namespace), Effect.catchTag("NamespaceNotFound", () => Effect.succeed(undefined)));
        if (byId !== undefined) {
            return byId;
        }
    }
    ...
    return yield* sd.getNamespace({ Id: summary.Id }).pipe(Effect.map((r) => r.Namespace), Effect.catchTag("NamespaceNotFound", () => Effect.succeed(undefined)));
});
/**
 ...
 */
const retryWhileNamespaceNotVisible = (self) => Effect.retry(self, {
    while: (e) => e._tag === "NamespaceNotFound",
    schedule: Schedule.max([Schedule.spaced("5 seconds"), Schedule.recurs(8)]),
});
export const ensureNamespace = (type, name, create) => retryWhileNamespaceNotVisible(Effect.gen(function* () {
    const created = yield* create.pipe(Effect.catchTag(["NamespaceAlreadyExists", "DuplicateRequest"], (e) => e._tag === "NamespaceAlreadyExists"
 ...
 */
export const fetchObservedTags = Effect.fn("AWS.CloudMap.fetchObservedTags")(function* (resourceArn) {
    const tags = yield* sd
        .listTagsForResource({ ResourceARN: resourceArn })
        .pipe(Effect.map((response) => response.Tags ?? []), Effect.catch(() => Effect.succeed([])));
 ...
 */
export const syncTags = Effect.fn("AWS.CloudMap.syncTags")(function* (resourceArn, observed, desired) {
