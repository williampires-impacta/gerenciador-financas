 ...
 * @resource
 */
export const ResourceShare = Resource("AWS.RAM.ResourceShare");
const toName = (id, props = {}) => props.shareName
...
const readByArn = Effect.fn(function* (arn) {
    const shares = yield* ram
        .getResourceShares({ resourceOwner: "SELF", resourceShareArns: [arn] })
        .pipe(Effect.map((r) => r.resourceShares ?? []), Effect.catchTag("UnknownResourceException", () => Effect.succeed([])));
 ...
 */
const readAssociations = Effect.fn(function* (arn, associationType) {
    const associations = yield* ram.getResourceShareAssociations
        .pages({ resourceShareArns: [arn], associationType })
        .pipe(Stream.runCollect, Effect.map((chunk) => Array.fromIterable(chunk).flatMap((page) => page.resourceShareAssociations ?? [])), Effect.catchTag("UnknownResourceException", () => Effect.succeed([])));
    return associations
        .filter((a) => a.status === "ASSOCIATED" || a.status === "ASSOCIATING")
        .map((a) => a.associatedEntity)
        .filter((entity) => entity !== undefined);
});
export const ResourceShareProvider = () => Provider.effect(ResourceShare, Effect.gen(function* () {
    return {
        ...
        list: () => ram.getResourceShares.pages({ resourceOwner: "SELF" }).pipe(Stream.runCollect, Effect.map((chunk) => Array.fromIterable(chunk)
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!state) {
                const created = yield* ram.createResourceShare({
                    ...
                    permissionArns: news.permissionArns && news.permissionArns.length > 0
                        ? news.permissionArns
                        : undefined,
                    tags: tagList,
                });
                ...
                if (!share?.resourceShareArn) {
                    return yield* Effect.fail(new Error(`resource share '${name}' missing ARN after create`));
                }
            ...
            const principalsToRemove = currentPrincipals.filter((p) => !desiredPrincipals.includes(p));
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* ram
                ...
                .pipe(Effect.catchTag("UnknownResourceException", () => Effect.void), 
