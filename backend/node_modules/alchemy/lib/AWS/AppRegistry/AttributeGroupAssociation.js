 ...
 * @resource
 */
export const AttributeGroupAssociation = Resource("AWS.AppRegistry.AttributeGroupAssociation");
export const AttributeGroupAssociationProvider = () => Provider.effect(AttributeGroupAssociation, Effect.gen(function* () {
    ...
    const observeAttributeGroup = Effect.fn(function* (specifier) {
        return yield* appregistry
            .getAttributeGroup({ attributeGroup: specifier })
            .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    });
    const isAssociated = Effect.fn(function* (applicationId, attributeGroupId) {
        const groupIds = yield* appregistry.listAssociatedAttributeGroups
            .pages({ application: applicationId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.attributeGroups ?? [])));
    ...
    return AttributeGroupAssociation.Provider.of({
        ...
        read: Effect.fn(function* ({ olds, output }) {
            ...
            if (attributeGroup?.id === undefined)
                return undefined;
...
//# sourceMappingURL=AttributeGroupAssociation.js.map