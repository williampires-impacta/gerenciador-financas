...
import { createInternalTags, diffTags, tagRecord } from "../../Tags.js";
/**
 ...
 */
export const readSchemasTags = Effect.fn(function* (resourceArn) {
    const { Tags } = yield* schemas
        .listTagsForResource({ ResourceArn: resourceArn })
        .pipe(Effect.catchTag("NotFoundException", () => Effect.succeed({
        Tags: {},
    })));
 ...
 */
export const syncSchemasTags = Effect.fn(function* (resourceArn, id, userTags) {
 ...
 * verbatim.
 */
export const canonicalJson = (content) => {
