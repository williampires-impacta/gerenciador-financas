 ...
 * @resource
 */
export const Registry = Resource("AWS.Schemas.Registry");
export const RegistryProvider = () => Provider.effect(Registry, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.registryName ??
            (yield* createPhysicalName({ id, maxLength: 64 })));
    });
    const describe = (registryName) => schemas
    ...
    return Registry.Provider.of({
        ...
        list: () => schemas.listRegistries.pages({ Scope: "LOCAL" }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.Registries ?? [])
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const currentPolicy = yield* schemas
                ...
                .pipe(Effect.map((r) => (r.Policy ? r.Policy : undefined)), Effect.catchTag("NotFoundException", () => Effect.succeed(undefined)));
        ...
        delete: Effect.fn(function* ({ output, force }) {
            ...
            const purgeAndDelete = Effect.gen(function* () {
                ...
                yield* Effect.forEach(childSchemas, (schema) => schema.SchemaName === undefined
                    ? Effect.void
                    : schemas
                        .deleteSchema({
                        RegistryName,
