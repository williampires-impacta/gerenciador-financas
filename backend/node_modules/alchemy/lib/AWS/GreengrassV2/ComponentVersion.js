 ...
 * @resource
 */
export const ComponentVersion = Resource("AWS.GreengrassV2.ComponentVersion");
/**
 ...
 */
const parseRecipeIdentity = Effect.fn(function* (recipe) {
    ...
    if (componentName === undefined || componentVersion === undefined) {
        return yield* Effect.fail(new GreengrassInvalidRecipe({
            message: "the inline recipe must declare top-level ComponentName and ComponentVersion (JSON or YAML)",
        }));
    }
...
export const ComponentVersionProvider = () => Provider.effect(ComponentVersion, Effect.gen(function* () {
    ...
    const componentArn = Effect.fn(function* (recipe) {
    ...
    const observeComponent = (arn) => greengrassv2
        ...
        reconcile: Effect.fn(function* ({ id, news, session }) {
            ...
            if (live === undefined) {
                yield* greengrassv2
                    ...
                    .pipe(Effect.catchTag(["ConflictException", "RequestAlreadyInProgressException"], () => Effect.succeed(undefined)));
                ...
                live = yield* Effect.gen(function* () {
                    ...
                    if (state === "FAILED" || state === "DEPRECATED") {
                        return yield* Effect.fail(new GreengrassComponentFailed({
                            message: `component ${identity.componentName}@${identity.componentVersion} entered state ${state}: ${observed?.status?.message ?? "no message"}`,
                        }));
                    }
                    if (observed === undefined || state !== "DEPLOYABLE") {
                        return yield* Effect.fail(new GreengrassComponentNotReady({
...
//# sourceMappingURL=ComponentVersion.js.map