...
import { imageSourceTrigger, restartFamilyTasks } from "./EcsDevWatch.js";
import { Task, TaskProvider } from "./Task.js";
export const FlociTaskProvider = () => makeDevWatchProvider(Task, flociSidecarEntry(), {
    ...
    replaceOn: ({ olds, news }) => Effect.sync(() => (olds.taskName ?? null) !== (news.taskName ?? null)
        ? { action: "replace" }
        : undefined),
    startWatch: (ctx) => Effect.gen(function* () {
        ...
        yield* trigger.pipe(Stream.runForEach(() => Effect.gen(function* () {
            const startedAt = Date.now();
            const previous = yield* ctx.currentAttrs;
            const attrs = yield* ctx.rerunReconcile;
            if (attrs.code.hash === previous.code.hash)
                return;
            ...
            yield* Effect.logInfo(`[alchemy dev] ${attrs.taskFamily}: image swapped (${restarted} task(s) restarted) in ${Date.now() - startedAt}ms`);
        }).pipe(Effect.catchCause((cause) => Effect.logWarning(`[alchemy dev] ${ctx.id}: image swap failed`, cause)))));
    }),
});
//# sourceMappingURL=FlociTaskProvider.js.map