 ...
 * @resource
 */
export const GroupMembership = Resource("AWS.IAM.GroupMembership");
export const GroupMembershipProvider = () => Provider.succeed(GroupMembership, {
    ...
    reconcile: Effect.fn(function* ({ news, session }) {
        const groupName = news.groupName;
        const desiredUsers = news.userNames;
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(groups, (group) => iam.getGroup.pages({ GroupName: group.GroupName }).pipe(Stream.runCollect, Effect.map((chunk) => ({
