 ...
 * @resource
 */
export const SigningCertificate = Resource("AWS.IAM.SigningCertificate");
export const SigningCertificateProvider = () => Provider.succeed(SigningCertificate, {
    ...
    list: Effect.fn(function* () {
        ...
        const perUser = yield* Effect.forEach(users, (user) => iam.listSigningCertificates.pages({ UserName: user.UserName }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    ...
    read: Effect.fn(function* ({ output }) {
        ...
        const cert = listed.Certificates.find((entry) => entry.CertificateId === output.certificateId);
    ...
    reconcile: Effect.fn(function* ({ news, output, session }) {
        ...
        const observed = output
            ? yield* iam
                ...
                .pipe(Effect.map((r) => r.Certificates.find((entry) => entry.CertificateId === output.certificateId)), Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined)))
            : undefined;
