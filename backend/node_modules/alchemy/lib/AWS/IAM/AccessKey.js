 ...
 * @resource
 */
export const AccessKey = Resource("AWS.IAM.AccessKey");
export const AccessKeyProvider = () => Provider.succeed(AccessKey, {
    ...
    read: Effect.fn(function* ({ output }) {
        ...
        const metadata = listed.AccessKeyMetadata.find((entry) => entry.AccessKeyId === output.accessKeyId);
    ...
    reconcile: Effect.fn(function* ({ news, output, session }) {
        ...
        const existing = output
            ? yield* iam.listAccessKeys({ UserName: output.userName }).pipe(Effect.map((listed) => listed.AccessKeyMetadata.find((entry) => entry.AccessKeyId === output.accessKeyId)), Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined)))
            : undefined;
        ...
        let createDate = existing?.CreateDate ?? output?.createDate;
        if (!existing) {
    ...
    list: Effect.fn(function* () {
        ...
        const perUser = yield* Effect.forEach(users, (user) => iam.listAccessKeys.pages({ UserName: user.UserName }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.AccessKeyMetadata.filter((entry) => entry.AccessKeyId != null).map((entry) => ({
