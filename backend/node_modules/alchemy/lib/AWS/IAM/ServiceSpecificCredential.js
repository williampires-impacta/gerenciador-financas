...
import { toRedactedString } from "./common.js";
/**
 ...
 * @resource
 */
export const ServiceSpecificCredential = Resource("AWS.IAM.ServiceSpecificCredential");
export const ServiceSpecificCredentialProvider = () => Provider.succeed(ServiceSpecificCredential, {
    ...
    list: Effect.fn(function* () {
        ...
        const perUser = yield* Effect.forEach(users, (user) => iam.listServiceSpecificCredentials({ UserName: user.UserName }).pipe(Effect.map((response) => (response.ServiceSpecificCredentials ?? []).map((metadata) => ({
        ...
        Effect.catchTag("NoSuchEntityException", () => Effect.succeed([]))), { concurrency: 10 });
        return perUser.flat();
    }),
    ...
    read: Effect.fn(function* ({ output }) {
        ...
        const metadata = listed.ServiceSpecificCredentials?.find((entry) => entry.ServiceSpecificCredentialId ===
    ...
    reconcile: Effect.fn(function* ({ news, output, session }) {
        ...
        const observed = output
            ? yield* iam
                ...
                .pipe(Effect.map((r) => r.ServiceSpecificCredentials?.find((entry) => entry.ServiceSpecificCredentialId ===
                output.serviceSpecificCredentialId)), Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined)))
            : undefined;
        // Ensure — create when missing. The secret fields are only
        // returned on first creation, so adoption preserves the prior
        // redacted values.
        let credentialId = observed?.ServiceSpecificCredentialId ??
            output?.serviceSpecificCredentialId;
        ...
        let serviceCredentialAlias = observed?.ServiceCredentialAlias ?? output?.serviceCredentialAlias;
        ...
        if (!observed) {
            ...
            if (!credential?.ServiceSpecificCredentialId) {
                return yield* Effect.fail(new Error(`createServiceSpecificCredential returned no credential id`));
            }
        ...
        if (!credentialId) {
            return yield* Effect.fail(new Error(`ServiceSpecificCredential for user '${news.userName}' has no id`));
        }
