...
import { parsePolicyDocument, stringifyPolicyDocument } from "./common.js";
/**
 ...
 * @resource
 */
export const Group = Resource("AWS.IAM.Group");
export const GroupProvider = () => Provider.effect(Group, Effect.gen(function* () {
    const toName = (id, props) => props.groupName
        ? Effect.succeed(props.groupName)
        : createPhysicalName({ id, maxLength: 128 });
    const readInlinePolicies = Effect.fn(function* (groupName) {
    ...
    const readManagedPolicies = Effect.fn(function* (groupName) {
    ...
    const syncManagedPolicies = Effect.fn(function* ({ groupName, olds, news, }) {
    ...
    const syncInlinePolicies = Effect.fn(function* ({ groupName, olds, news, }) {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            const groups = yield* iam.listGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Groups ?? [])));
            const hydrated = yield* Effect.forEach(groups, (group) => Effect.gen(function* () {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!observed?.Group?.Arn) {
                observed = yield* iam
                    ...
                    .pipe(Effect.catchTag("EntityAlreadyExistsException", () => iam.getGroup({ GroupName: groupName })), Effect.map((response) => ({
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* iam.getGroup.items({ GroupName: output.groupName }).pipe(Stream.mapEffect((user) => user.UserName
                ? iam
                    .removeUserFromGroup({
            ...
            yield* iam.listGroupPolicies
                ...
                .pipe(Stream.mapEffect((policyName) => iam
                .deleteGroupPolicy({
                GroupName: output.groupName,
                PolicyName: policyName,
            })
            ...
            yield* iam.listAttachedGroupPolicies
                ...
                ? iam
                    ...
                    .pipe(Effect.catchTag("NoSuchEntityException", () => Effect.void))
                : Effect.void), Stream.runDrain, Effect.catchTag("NoSuchEntityException", () => Effect.void));
            yield* iam
                .deleteGroup({
                GroupName: output.groupName,
            })
