...
import { oldestNondefaultPolicyVersion, parsePolicyDocument, policyArnFromParts, stringifyPolicyDocument, toTagRecord, } from "./common.js";
export { stringifyPolicyDocument } from "./common.js";
/**
 ...
 */
export const normalizePolicyDocument = (json) => {
 ...
 * @resource
 */
export const Policy = Resource("AWS.IAM.Policy");
export const PolicyProvider = () => Provider.effect(Policy, Effect.gen(function* () {
    const toPolicyName = (id, props) => props.policyName
        ? Effect.succeed(props.policyName)
        : createPhysicalName({ id, maxLength: 128 });
    const toPolicyArn = Effect.fn(function* (id, props) {
    ...
    const readPolicy = Effect.fn(function* (policyArn) {
        const response = yield* iam
            .getPolicy({ PolicyArn: policyArn })
            .pipe(Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined)));
        return response?.Policy;
    });
    const readPolicyDocument = Effect.fn(function* ({ policyArn, versionId, }) {
        if (!versionId) {
            return undefined;
        }
        const response = yield* iam
    ...
    const prunePolicyVersions = Effect.fn(function* (policyArn) {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const policies = yield* iam.listPolicies
                .pages({ Scope: "Local" })
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Policies ?? [])));
            const rows = yield* Effect.forEach(policies, (policy) => Effect.gen(function* () {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const policyArn = output?.policyArn ??
                (yield* toPolicyArn(id, olds ?? {}));
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!observed?.Arn) {
                const created = yield* iam
                    ...
                    .pipe(Effect.catchTag("EntityAlreadyExistsException", () => Effect.gen(function* () {
                    const existing = yield* readPolicy(policyArn);
                    if (!existing?.Arn) {
                        return yield* Effect.fail(new Error(`Policy '${policyName}' already exists but could not be described`));
                    }
                    const existingTags = yield* iam.listPolicyTags({
                        PolicyArn: existing.Arn,
                    });
                    if (!hasTags(desiredTags, existingTags.Tags)) {
                        return yield* Effect.fail(new Error(`Policy '${policyName}' already exists and is not managed by alchemy`));
                    }
            ...
            if (JSON.stringify(observedDocument ?? null) !==
            ...
            const freshDocument = (yield* readPolicyDocument({
                policyArn,
            ...
            return {
                ...
                permissionsBoundaryUsageCount: fresh?.PermissionsBoundaryUsageCount ??
