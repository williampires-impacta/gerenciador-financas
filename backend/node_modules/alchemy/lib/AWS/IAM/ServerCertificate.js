 ...
 * @resource
 */
export const ServerCertificate = Resource("AWS.IAM.ServerCertificate");
export const ServerCertificateProvider = () => Provider.effect(ServerCertificate, Effect.gen(function* () {
    const toName = (id, props) => props.serverCertificateName
        ? Effect.succeed(props.serverCertificateName)
        : createPhysicalName({ id, maxLength: 128 });
    const readCertificate = Effect.fn(function* (name) {
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.serverCertificateName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!cert?.ServerCertificateMetadata?.Arn) {
                const created = yield* iam
                    ...
                    PrivateKey: typeof news.privateKey === "string"
                        ? news.privateKey
                    ...
                    if (!existing?.ServerCertificateMetadata?.Arn) {
                        return yield* Effect.fail(new Error(`Server certificate '${name}' already exists but could not be described`));
                    }
                    if (!hasTags(desiredTags, existing.Tags)) {
                        return yield* Effect.fail(new Error(`Server certificate '${name}' already exists and is not managed by alchemy`));
                    }
                    return {
            ...
            if (!metadata?.Arn || !metadata.ServerCertificateName) {
                return yield* Effect.fail(new Error(`Server certificate '${name}' was not readable after sync`));
            }
        ...
        list: () => Effect.gen(function* () {
            ...
            const metadatas = yield* iam.listServerCertificates.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.ServerCertificateMetadataList ?? [])));
            const rows = yield* Effect.forEach(metadatas, (meta) => Effect.gen(function* () {
