...
import { toTagRecord, unwrapRedactedString } from "./common.js";
/**
 ...
 * @resource
 */
export const SAMLProvider = Resource("AWS.IAM.SAMLProvider");
...
const isTransientWriteError = (error) => error._tag === "ValidationError" ||
...
export const SAMLProviderProvider = () => Provider.succeed(SAMLProvider, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(arns, (samlProviderArn) => Effect.gen(function* () {
            const response = yield* iam
    ...
    reconcile: Effect.fn(function* ({ id, news, output, session }) {
        ...
        const samlProviderArn = output?.samlProviderArn ??
        ...
        if (!observed) {
            ...
            }), Effect.catchTag("EntityAlreadyExistsException", () => Effect.gen(function* () {
                const existingTags = yield* iam.listSAMLProviderTags({
                    SAMLProviderArn: samlProviderArn,
                });
                if (!hasTags(internalTags, existingTags.Tags)) {
                    return yield* Effect.fail(new Error(`SAML provider '${news.name}' already exists and is not managed by alchemy`));
                }
            ...
            if ((observed.SAMLMetadataDocument ?? undefined) !==
                ...
                yield* iam
                    .updateSAMLProvider({
                    SAMLProviderArn: samlProviderArn,
                    SAMLMetadataDocument: (observed.SAMLMetadataDocument ?? undefined) !==
                        news.samlMetadataDocument
                        ? news.samlMetadataDocument
                        : undefined,
                    AssertionEncryptionMode: news.assertionEncryptionMode,
