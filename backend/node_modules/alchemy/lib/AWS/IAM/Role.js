...
import { parsePolicyDocument, stringifyPolicyDocument, toTagRecord, } from "./common.js";
/**
...
const invalidPrincipalRetry = {
    while: (e) => e._tag === "MalformedPolicyDocumentException" &&
 ...
 * a stable document).
 */
const mergeBoundInlinePolicies = (inlinePolicies, bindings) => {
    ...
    const deduped = Array.from(new Map(statements.map((s) => [JSON.stringify(s), s])).values()).sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b)));
    return {
        ...inlinePolicies,
 ...
 */
const mergeBoundAssumeRolePolicy = (doc, bindings) => {
 ...
 * @resource
 */
export const Role = Resource("AWS.IAM.Role");
export const RoleProvider = () => Provider.effect(Role, Effect.gen(function* () {
    const toRoleName = (id, props = {}) => props.roleName
        ? Effect.succeed(props.roleName)
        : createPhysicalName({ id, maxLength: 64 });
    const readInlinePolicies = Effect.fn(function* (roleName) {
        ...
        const entries = yield* Effect.all(policyNames.map((policyName) => iam
            .getRolePolicy({
            RoleName: roleName,
        ...
        ]), Effect.catchTag("NoSuchEntityException", () => Effect.succeed([policyName, undefined])))));
        return Object.fromEntries(entries.filter((entry) => entry[1] !== undefined));
    });
    const readManagedPolicies = Effect.fn(function* (roleName) {
        const attached = yield* iam.listAttachedRolePolicies
            .items({ RoleName: roleName })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
        return attached
            .map((policy) => policy.PolicyArn)
            .filter((policyArn) => typeof policyArn === "string");
    });
    const readTags = Effect.fn(function* (roleName) {
        const listed = yield* iam.listRoleTags({
            RoleName: roleName,
        });
        return toTagRecord(listed.Tags);
    });
    const syncManagedPolicies = Effect.fn(function* ({ roleName, olds, news, }) {
    ...
    const syncInlinePolicies = Effect.fn(function* ({ roleName, olds, news, }) {
        for (const [policyName, document] of Object.entries(news)) {
            if (JSON.stringify(olds[policyName] ?? null) !==
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const roles = yield* iam.listRoles.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            ...
            const hydrated = yield* Effect.forEach(roles, (role) => Effect.gen(function* () {
                const assumeRolePolicyDocument = parsePolicyDocument(role.AssumeRolePolicyDocument);
            ...
            Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined))), { concurrency: 8 });
            return hydrated.filter((attrs) => attrs !== undefined);
        }),
        diff: Effect.fn(function* ({ id, olds, news = {} }) {
            if (!isResolved(news))
                return;
            if ((yield* toRoleName(id, olds ?? {})) !==
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            const assumeRolePolicyDocument = parsePolicyDocument(role.Role.AssumeRolePolicyDocument) ??
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output, session, bindings, }) {
            ...
            const assumeRolePolicyDocument = mergeBoundAssumeRolePolicy(news.assumeRolePolicyDocument, bindings);
            if (assumeRolePolicyDocument.Statement.length === 0) {
                return yield* Effect.die("Role requires a trust policy: set `assumeRolePolicyDocument` or bind `assumeRolePolicyStatements`.");
            }
            const desiredTags = {
            ...
            if (!observedRole?.Role) {
                observedRole = yield* iam
                    ...
                    .pipe(Effect.retry(invalidPrincipalRetry), Effect.catchTag("EntityAlreadyExistsException", () => iam.getRole({ RoleName: roleName })));
            }
            const observedAssumePolicy = parsePolicyDocument(observedRole.Role?.AssumeRolePolicyDocument);
            ...
            const observedPermissionsBoundary = observedRole.Role?.PermissionsBoundary?.PermissionsBoundaryArn;
            // Sync assume-role policy — only call updateAssumeRolePolicy
            // when the document actually differs.
            if (JSON.stringify(observedAssumePolicy ?? null) !==
            ...
            return {
                ...
                permissionsBoundary: liveRole.Role?.PermissionsBoundary?.PermissionsBoundaryArn ??
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* iam
                ...
                .pipe(Effect.catchTag("NoSuchEntityException", () => Effect.void));
            yield* iam.listRolePolicies.items({ RoleName: output.roleName }).pipe(Stream.mapEffect((policyName) => iam
                .deleteRolePolicy({
                RoleName: output.roleName,
                PolicyName: policyName,
            })
                .pipe(Effect.catchTag("NoSuchEntityException", () => Effect.void))), Stream.runDrain, 
            // The role itself may already be gone.
            Effect.catchTag("NoSuchEntityException", () => Effect.void));
            yield* iam.listAttachedRolePolicies
                .items({ RoleName: output.roleName })
                .pipe(Stream.mapEffect((policy) => iam
                .detachRolePolicy({
                RoleName: output.roleName,
            ...
            yield* Effect.die(new Error(`IAM role ${output.roleName} remained observable 30 seconds after delete`));
        }),
    };
}));
