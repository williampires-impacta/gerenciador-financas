...
import { toTagRecord } from "./common.js";
/**
 ...
 * @resource
 */
export const InstanceProfile = Resource("AWS.IAM.InstanceProfile");
export const InstanceProfileProvider = () => Provider.effect(InstanceProfile, Effect.gen(function* () {
    const toName = (id, props) => props.instanceProfileName
        ? Effect.succeed(props.instanceProfileName)
        : createPhysicalName({ id, maxLength: 128 });
    const readInstanceProfile = Effect.fn(function* (name) {
    ...
    const syncRole = Effect.fn(function* ({ profileName, currentRoleName, nextRoleName, }) {
    ...
    return {
        ...
        list: () => iam.listInstanceProfiles.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.InstanceProfiles ?? []).map((profile) => ({
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.instanceProfileName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!profile?.Arn) {
                yield* iam
                    ...
                    if (!existing?.Arn) {
                        return yield* Effect.fail(new Error(`Instance profile '${name}' already exists but could not be described`));
                    }
                    if (!hasTags(desiredTags, existing.Tags)) {
                        return yield* Effect.fail(new Error(`Instance profile '${name}' already exists and is not managed by alchemy`));
                    }
                })));
                profile = yield* readInstanceProfile(name);
                if (!profile?.Arn || !profile.InstanceProfileName) {
                    return yield* Effect.fail(new Error(`Instance profile '${name}' was not readable after create`));
                }
            }
            ...
            const desiredRoleName = news.roleName;
            yield* syncRole({
                profileName: name,
            ...
            if (!fresh?.Arn || !fresh.InstanceProfileName) {
                return yield* Effect.fail(new Error(`Instance profile '${name}' was not readable after sync`));
            }
