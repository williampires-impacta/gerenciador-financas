...
import { unwrapRedactedString } from "./common.js";
/**
 ...
 * @resource
 */
export const LoginProfile = Resource("AWS.IAM.LoginProfile");
export const LoginProfileProvider = () => Provider.succeed(LoginProfile, {
    stables: ["userName"],
    ...
    list: Effect.fn(function* () {
        ...
        const profiles = yield* Effect.forEach(users, (user) => iam.getLoginProfile({ UserName: user.UserName }).pipe(Effect.map((response) => ({
            userName: response.LoginProfile.UserName,
            createDate: response.LoginProfile.CreateDate,
            passwordResetRequired: response.LoginProfile.PasswordResetRequired,
        })), 
        ...
        const result = profiles.filter((profile) => profile !== undefined);
        return result;
    }),
});
