 ...
 * @resource
 */
export const User = Resource("AWS.IAM.User");
export const UserProvider = () => Provider.effect(User, Effect.gen(function* () {
    ...
    const readManagedPolicies = Effect.fn(function* (userName) {
    ...
    const readInlinePolicies = Effect.fn(function* (userName) {
    ...
    const readTags = Effect.fn(function* (userName) {
        const listed = yield* iam.listUserTags({
            UserName: userName,
        });
        return toTagRecord(listed.Tags);
    });
    const syncManagedPolicies = Effect.fn(function* ({ userName, olds, news, }) {
    ...
    const syncInlinePolicies = Effect.fn(function* ({ userName, olds, news, }) {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const users = yield* iam.listUsers.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Users ?? [])));
            const hydrated = yield* Effect.forEach(users, (user) => Effect.gen(function* () {
            ...
            Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined))), { concurrency: 10 });
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const userName = output?.userName ?? (yield* toName(id, olds ?? {}));
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!observedUser?.Arn) {
                const created = yield* iam
                    ...
                    .pipe(Effect.catchTag("EntityAlreadyExistsException", () => iam.getUser({ UserName: userName }).pipe(Effect.filterOrFail((existing) => hasTags(desiredTags, existing.User?.Tags), () => new Error(`User '${userName}' already exists and is not managed by alchemy`)))));
            ...
            return {
                ...
                permissionsBoundary: user.User?.PermissionsBoundary?.PermissionsBoundaryArn ??
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* iam
                ...
                .pipe(Effect.catchTag("NoSuchEntityException", () => Effect.void));
            yield* iam.listUserPolicies.items({ UserName: output.userName }).pipe(Stream.mapEffect((policyName) => iam
                .deleteUserPolicy({
                UserName: output.userName,
                PolicyName: policyName,
            })
            ...
            yield* iam.listAttachedUserPolicies
                ...
                .pipe(Stream.mapEffect((policy) => policy.PolicyArn
                ? iam
                    .detachUserPolicy({
                    UserName: output.userName,
