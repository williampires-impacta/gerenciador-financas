 ...
 * @resource
 */
export const OpenIDConnectProvider = Resource("AWS.IAM.OpenIDConnectProvider");
export const OpenIDConnectProviderProvider = () => Provider.effect(OpenIDConnectProvider, Effect.gen(function* () {
    const oidcArnFromUrl = (url) => AWSEnvironment.current.pipe(Effect.map(({ accountId }) => `arn:aws:iam::${accountId}:oidc-provider/${url.replace(/^https?:\/\//, "")}`));
    const readProvider = Effect.fn(function* (providerArn) {
    ...
    const hydrate = (providerArn) => Effect.gen(function* () {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const result = rows.filter((row) => row !== undefined);
            return result;
        }),
        ...
        read: Effect.fn(function* ({ olds, output }) {
            const providerArn = output?.openIDConnectProviderArn ??
                (olds?.url !== undefined
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            let observedClientIds = [];
            let observedThumbprints = [];
            let observedTags = {};
            if (observed?.Url) {
