 ...
 */
export class ServiceLinkedRoleDeletionFailed extends Data.TaggedError("ServiceLinkedRoleDeletionFailed") {
    get message() {
 ...
 * @resource
 */
export const ServiceLinkedRole = Resource("AWS.IAM.ServiceLinkedRole");
/**
...
export const ServiceLinkedRoleProvider = () => Provider.effect(ServiceLinkedRole, Effect.gen(function* () {
    ...
    const findRole = Effect.fn(function* (props) {
        const roles = yield* iam.listRoles
            ...
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Roles ?? [])));
        return roles.find((role) => props.customSuffix
            ? role.RoleName.endsWith(`_${props.customSuffix}`)
            : !role.RoleName.includes("_"));
    });
    const getRole = Effect.fn(function* (roleName) {
        const response = yield* iam
            .getRole({ RoleName: roleName })
            .pipe(Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined)));
        return response?.Role;
    });
    const submitDeletion = Effect.fn(function* (roleName) {
        return yield* iam.deleteServiceLinkedRole({ RoleName: roleName }).pipe(Effect.retry({
            while: (error) => error._tag === "LimitExceededException" ||
    ...
    const deleteAndWait = Effect.fn(function* (roleName) {
        let deletionTaskId;
        ...
        for (let attempt = 0; attempt < 30; attempt++) {
            ...
            if (deletionTaskId !== undefined) {
                ...
                Effect.catchTag("NoSuchEntityException", () => Effect.succeed("TASK_NOT_FOUND")), Effect.catchTag("ServiceFailureException", () => Effect.succeed(undefined)));
        ...
        return yield* Effect.fail(new ServiceLinkedRoleDeletionFailed({
            roleName,
            status: "TIMED_OUT",
            reason: lastReason ??
    ...
    return {
        ...
        diff: Effect.fn(function* ({ olds, news }) {
            ...
            if ((olds.customSuffix ?? undefined) !==
        ...
        read: Effect.fn(function* ({ olds, output }) {
            ...
            const awsServiceName = olds?.awsServiceName ??
        ...
        reconcile: Effect.fn(function* ({ news, output, session }) {
            ...
            if (!observed) {
                const created = yield* iam
                    ...
                    .pipe(Effect.catchTag("InvalidInputException", (error) => findRole(news).pipe(Effect.flatMap((existing) => existing
                    ? Effect.succeed({ Role: existing })
                    : Effect.fail(error)))));
                observed = created.Role;
            }
            if (!observed) {
                return yield* Effect.fail(new Error(`Service-linked role for '${news.awsServiceName}' was created but could not be described`));
            }
