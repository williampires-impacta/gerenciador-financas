 ...
 * @resource
 */
export const SSHPublicKey = Resource("AWS.IAM.SSHPublicKey");
export const SSHPublicKeyProvider = () => Provider.succeed(SSHPublicKey, {
    ...
    reconcile: Effect.fn(function* ({ news, output, session }) {
        ...
        const observed = output
            ? yield* iam
                ...
                .pipe(Effect.map((r) => r.SSHPublicKey), Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined)))
            : undefined;
    ...
    list: Effect.fn(function* () {
        ...
        const perUser = yield* Effect.forEach(users, (user) => iam.listSSHPublicKeys.pages({ UserName: user.UserName }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.SSHPublicKeys ?? [])), Effect.flatMap((metas) => Effect.forEach(metas, (meta) => iam
            .getSSHPublicKey({
            UserName: meta.UserName,
            SSHPublicKeyId: meta.SSHPublicKeyId,
            Encoding: "SSH",
        })
            .pipe(Effect.map((r) => r.SSHPublicKey), 
        // The key may be deleted between enumeration and
        // hydration.
        Effect.catchTag("NoSuchEntityException", () => Effect.succeed(undefined))), { concurrency: 10 })), Effect.map((keys) => keys
            .filter((key) => key?.SSHPublicKeyId != null)
            .map((key) => ({
            userName: key.UserName,
            sshPublicKeyId: key.SSHPublicKeyId,
            fingerprint: key.Fingerprint,
