 ...
 * @resource
 */
export const RegexPatternSet = Resource("AWS.WAFv2.RegexPatternSet");
const defaultScope = "REGIONAL";
const toExpressionList = (expressions) => expressions.map((RegexString) => ({ RegexString }));
const fromExpressionList = (list) => (list ?? []).flatMap((regex) => regex.RegexString !== undefined ? [regex.RegexString] : []);
const sorted = (values) => [...values].sort((a, b) => a.localeCompare(b));
const toAttrs = (set, scope) => ({
...
export const RegexPatternSetProvider = () => Provider.effect(RegexPatternSet, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.regexPatternSetName ??
            (yield* createPhysicalName({ id, maxLength: 128 })));
    });
    const findSet = Effect.fn(function* (scope, name, cachedId) {
        ...
        for (let page = 0; page < 20; page++) {
            const listed = yield* withWafScope(scope, wafv2.listRegexPatternSets({
                Scope: scope,
                NextMarker: marker,
                Limit: 100,
            }));
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            const name = output?.regexPatternSetName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (set === undefined || arn === undefined) {
                return yield* Effect.fail(new Error(`Failed to observe RegexPatternSet '${name}' after create`));
            }
            ...
            if (drifted) {
                yield* retryOptimisticLock(Effect.gen(function* () {
                    ...
                    if (!fresh?.RegexPatternSet?.Id ||
                        fresh.LockToken === undefined) {
                        return;
                    }
                    yield* withWafScope(scope, wafv2.updateRegexPatternSet({
                        Name: name,
                        Scope: scope,
