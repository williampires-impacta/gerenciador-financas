 ...
 * @resource
 */
export const WebACL = Resource("AWS.WAFv2.WebACL");
...
const toAttrs = (acl, scope) => ({
    webAclName: acl.Name,
    webAclId: acl.Id,
    webAclArn: acl.ARN,
    scope,
});
export const WebACLProvider = () => Provider.effect(WebACL, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        return (props.webAclName ??
    ...
    const findWebACL = Effect.fn(function* (scope, name, cachedId) {
        ...
        for (let page = 0; page < 20; page++) {
            const listed = yield* withWafScope(scope, wafv2.listWebACLs({ Scope: scope, NextMarker: marker, Limit: 100 }));
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!deepEqual(observedAspects, desiredAspects, { stripNullish: true })) {
                yield* retryOptimisticLock(Effect.gen(function* () {
                    ...
                    yield* retryUnavailableEntity(withWafScope(scope, wafv2.updateWebACL({
                        Name: name,
                        Scope: scope,
                        Id: fresh.WebACL.Id,
