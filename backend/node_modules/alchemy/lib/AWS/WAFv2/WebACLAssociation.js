...
import { retryUnavailableEntity, retryUnavailableEntityLong, } from "./internal.js";
/**
 ...
 * @resource
 */
export const WebACLAssociation = Resource("AWS.WAFv2.WebACLAssociation");
export const WebACLAssociationProvider = () => Provider.effect(WebACLAssociation, Effect.gen(function* () {
    ...
    const observeAssociation = Effect.fn(function* (resourceArn) {
        const response = yield* retryUnavailableEntity(wafv2.getWebACLForResource({ ResourceArn: resourceArn })).pipe(Effect.catchTag("WAFNonexistentItemException", () => Effect.succeed({ WebACL: undefined })));
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (observed !== news.webAclArn) {
                yield* retryUnavailableEntityLong(wafv2.associateWebACL({
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryUnavailableEntity(wafv2.disassociateWebACL({ ResourceArn: output.resourceArn })).pipe(Effect.catchTag(["WAFNonexistentItemException", "WAFUnavailableEntityException"], () => Effect.void));
        }),
    };
}));
