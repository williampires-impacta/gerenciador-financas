 ...
 * @resource
 */
export const ParameterGroup = Resource("AWS.DAX.ParameterGroup");
export const ParameterGroupProvider = () => Provider.effect(ParameterGroup, Effect.gen(function* () {
    ...
    const toAttrs = (group, parameters, overrides) => ({
        ...
        parameters: Object.fromEntries(Object.keys(overrides ?? {}).map((key) => [
            key,
            parameters[key] ?? "",
        ])),
    });
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            if (group?.ParameterGroupName === undefined)
                return undefined;
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (observed === undefined) {
                return yield* Effect.fail(new Error(`DAX parameter group '${name}' not found after create`));
            }
            ...
            const delta = Object.entries(props.parameters ?? {}).filter(([key, value]) => observedParameters[key] !== value);
            if (delta.length > 0) {
