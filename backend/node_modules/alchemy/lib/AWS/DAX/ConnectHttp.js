 ...
 * descriptor.
 */
export const makeDaxConnectHttpBinding = (options) => Effect.gen(function* () {
    return Effect.fn(function* (cluster) {
            ...
            if (isBindingHost(host)) {
                ...
                yield* host.bind `Allow(${host}, ${options.tag}(${cluster}))`({
                    env: {
                        ...
                        [`${prefix}_TLS`]: Output.map(cluster.clusterEndpointEncryptionType, (encryption) => (encryption === "TLS" ? "true" : "false")),
                    },
                    policyStatements: [
                        {
                            Effect: "Allow",
        ...
        return Effect.gen(function* () {
            ...
            if (!host || port === undefined || !url) {
                return yield* Effect.die(`DAX discovery endpoint for '${cluster.LogicalId}' is not available yet`);
            }
            return {
                host,
                port,
