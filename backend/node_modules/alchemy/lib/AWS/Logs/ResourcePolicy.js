 ...
 * @resource
 */
export const ResourcePolicy = Resource("AWS.Logs.ResourcePolicy");
export const ResourcePolicyProvider = () => Provider.effect(ResourcePolicy, Effect.gen(function* () {
    ...
    const toDocumentString = (document) => typeof document === "string" ? document : JSON.stringify(document);
    ...
    const observe = Effect.fn(function* (policyName) {
        const policies = yield* describeAll;
        return policies.find((policy) => policy.policyName === policyName);
    });
    return {
        stables: ["policyName"],
        list: () => describeAll.pipe(Effect.map((policies) => policies
            .filter((policy) => policy.policyName != null)
            .map((policy) => ({
