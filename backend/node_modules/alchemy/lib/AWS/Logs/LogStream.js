 ...
 * @resource
 */
export const LogStream = Resource("AWS.Logs.LogStream");
export const LogStreamProvider = () => Provider.effect(LogStream, Effect.gen(function* () {
    const toLogStreamName = (id, props = {}) => props.logStreamName
        ? Effect.succeed(props.logStreamName)
        : createPhysicalName({ id, maxLength: 512 });
    const observe = Effect.fn(function* (logGroupName, logStreamName) {
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const perGroup = yield* Effect.forEach(groups, (logGroupName) => logs.describeLogStreams.pages({ logGroupName }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
                .flatMap((page) => page.logStreams ?? [])
                .filter((stream) => stream.logStreamName != null)
                .map((stream) => ({
        ...
        diff: Effect.fn(function* ({ id, olds, news }) {
            ...
            if ((yield* toLogStreamName(id, olds)) !==
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            const remaining = yield* Effect.repeat(observe(output.logGroupName, output.logStreamName), {
                schedule: Schedule.fixed("250 millis"),
                until: (stream) => stream === undefined,
                times: 20,
            });
            if (remaining !== undefined) {
                yield* Effect.die(new Error(`CloudWatch log stream ${output.logGroupName}:${output.logStreamName} remained observable after delete`));
            }
        }),
    };
}));
