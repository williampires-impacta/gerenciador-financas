 ...
 * @resource
 */
export const LogGroup = Resource("AWS.Logs.LogGroup");
export const LogGroupProvider = () => Provider.effect(LogGroup, Effect.gen(function* () {
    const toLogGroupName = (id, props = {}) => props.logGroupName
        ? Effect.succeed(props.logGroupName)
        : createPhysicalName({ id, maxLength: 512 });
    const toLogGroupClass = (props = {}) => props.logGroupClass ?? "STANDARD";
    ...
    const normalizeLogGroupArn = (arn) => (arn.endsWith(":*") ? arn.slice(0, -2) : arn);
    const observe = Effect.fn(function* (logGroupName) {
        return yield* logs.describeLogGroups
            .items({ logGroupNamePrefix: logGroupName })
            .pipe(Stream.filter((group) => group.logGroupName === logGroupName), Stream.runHead, Effect.map(Option.getOrUndefined));
    });
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const groups = yield* logs.describeLogGroups.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.logGroups ?? [])));
            return yield* Effect.forEach(groups.filter((group) => group.logGroupName != null && group.arn != null), (group) => Effect.gen(function* () {
                const tagArn = `arn:aws:logs:${region}:${accountId}:log-group:${group.logGroupName}`;
                const tags = yield* logs
                    .listTagsForResource({ resourceArn: tagArn })
                    .pipe(Effect.map((r) => Object.fromEntries(Object.entries(r.tags ?? {}).filter((entry) => typeof entry[1] === "string"))), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})));
                return {
        ...
        diff: Effect.fn(function* ({ id, olds, news }) {
            if (!isResolved(news))
                return;
            if ((yield* toLogGroupName(id, olds ?? {})) !==
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const observedTags = yield* logs
                ...
                .pipe(Effect.map((r) => Object.fromEntries(Object.entries(r.tags ?? {}).filter((entry) => typeof entry[1] === "string"))), Effect.catch(() => Effect.succeed({})));
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            const remaining = yield* Effect.repeat(logs.describeLogGroups
                .items({ logGroupNamePrefix: output.logGroupName })
                .pipe(Stream.filter((group) => group.logGroupName === output.logGroupName), Stream.runHead, Effect.map(Option.isSome)), {
                schedule: Schedule.fixed("250 millis"),
                until: (present) => !present,
                times: 20,
            });
            if (remaining) {
                yield* Effect.die(new Error(`CloudWatch log group ${output.logGroupName} remained observable after delete`));
            }
        }),
    };
}));
