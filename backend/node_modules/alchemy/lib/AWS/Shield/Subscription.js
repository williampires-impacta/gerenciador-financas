 ...
 * ```
 */
const SubscriptionResource = Resource("AWS.Shield.Subscription");
...
const observeSubscription = shield.describeSubscription({}).pipe(Effect.map((r) => r.Subscription), Effect.catchTag("SubscriptionNotFound", () => Effect.succeed(undefined)));
const buildAttrs = (subscription) => ({
...
export const SubscriptionProvider = () => Provider.effect(SubscriptionResource, Effect.gen(function* () {
    return {
        ...
        list: () => observeSubscription.pipe(Effect.map((subscription) => subscription ? [buildAttrs(subscription)] : [])),
        reconcile: Effect.fn(function* ({ news, session }) {
            ...
            if (!subscription) {
                ...
                if (!subscription) {
                    return yield* Effect.fail(new Error("Failed to create or read the Shield Advanced subscription"));
                }
            }
        ...
        delete: Effect.fn(function* () {
            ...
            yield* shield.deleteSubscription({}).pipe(Effect.catchTag(["SubscriptionNotFound", "ResourceNotFoundException"], () => Effect.void), Effect.catchTag("LockedSubscriptionException", (error) => Effect.logWarning("Shield Advanced subscription is locked for its 1-year commitment and cannot be cancelled yet; leaving it in place.", error)));
        }),
    };
}));
