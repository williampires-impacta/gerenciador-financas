 ...
 * @resource
 */
export const Environment = Resource("AWS.DataZone.Environment");
/** The environment deployment settled in a failed state. */
export class EnvironmentDeploymentFailed extends Data.TaggedError("AWS.DataZone.EnvironmentDeploymentFailed") {
}
...
export const EnvironmentProvider = () => Provider.effect(Environment, Effect.gen(function* () {
    ...
    // "absent".
    const getEnvironmentOrUndefined = Effect.fn(function* (domainId, environmentId) {
    ...
    const findByName = Effect.fn(function* (domainId, projectId, name) {
        const found = yield* datazone
            ...
            .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)), Effect.catchTag("AccessDeniedException", () => Effect.succeed(undefined)));
        const summary = (found?.items ?? []).find((s) => unredact(s.name) === name && s.status !== "DELETING");
        return summary?.id;
    });
    // Poll the environment to a settled (non-transient) status. Deployment
    // drives a CloudFormation stack — bounded at ~5 minutes.
    const waitForSettled = Effect.fn(function* (domainId, environmentId) {
        return yield* getEnvironmentOrUndefined(domainId, environmentId).pipe(Effect.repeat({
    ...
    const waitForGone = Effect.fn(function* (domainId, environmentId) {
        yield* getEnvironmentOrUndefined(domainId, environmentId).pipe(Effect.repeat({
            schedule: Schedule.fixed("5 seconds"),
            until: (env) => env === undefined || env.status === "DELETED",
            times: 60,
        }));
    });
    const failIfDeploymentFailed = Effect.fn(function* (env, environmentId) {
    ...
    const toAttributes = (env, environmentId) => ({
        environmentId,
    ...
    return Environment.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (env === undefined) {
                ...
                const drifted = unredact(env.name) !== name ||
