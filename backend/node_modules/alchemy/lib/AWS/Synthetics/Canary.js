 ...
 * @resource
 */
export const Canary = Resource("AWS.Synthetics.Canary");
...
const VPC_ACCESS_POLICY_ARN = "arn:aws:iam::aws:policy/service-role/AWSLambdaVPCAccessExecutionRole";
/**
...
class CanaryBackingResourceStillVisible extends Data.TaggedError("CanaryBackingResourceStillVisible") {
}
...
const parseArtifactLocation = (location) => {
...
const scriptFilePath = (runtimeVersion, handler) => {
...
const buildCode = Effect.fn(function* (script, handler, runtimeVersion) {
    ...
    const buffer = yield* Effect.promise(() => zip.generateAsync({
        type: "nodebuffer",
        compression: "DEFLATE",
        platform: "UNIX",
    }));
...
export const CanaryProvider = () => Provider.effect(Canary, Effect.gen(function* () {
    const createCanaryName = Effect.fn(function* (id, props) {
        if (props.canaryName)
            return props.canaryName;
    ...
    const canaryArnOf = Effect.fn(function* (canaryName) {
        const { accountId, region } = yield* AWSEnvironment.current;
        return `arn:aws:synthetics:${region}:${accountId}:canary:${canaryName}`;
    });
    const getCanaryOrUndefined = Effect.fn(function* (canaryName) {
        return yield* synthetics.getCanary({ Name: canaryName }).pipe(Effect.map((r) => r.Canary), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    });
    // Poll until the canary leaves its transitional state. Create typically
    // settles in 30-90s; budget 3 minutes (60 * 3s).
    const waitForSettled = Effect.fn(function* (canaryName) {
        return yield* getCanaryOrUndefined(canaryName).pipe(Effect.flatMap((canary) => canary !== undefined &&
            TRANSITIONAL_STATES.includes(canary.Status?.State ?? "")
            ? Effect.fail(new CanaryNotSettled({
                canaryName,
                state: canary.Status?.State,
            }))
            : Effect.succeed(canary)), Effect.retry({
    ...
    const failIfErrored = (canaryName, canary) => canary?.Status?.State === "ERROR"
        ? Effect.fail(new CanaryFailed({
            canaryName,
    ...
    const waitUntilGone = Effect.fn(function* (canaryName) {
        yield* getCanaryOrUndefined(canaryName).pipe(Effect.flatMap((canary) => canary === undefined
            ? Effect.void
            : Effect.fail(new CanaryNotSettled({
                canaryName,
                state: canary.Status?.State,
    ...
    const deleteCanaryAndWait = Effect.fn(function* (canaryName) {
        ...
        if (state === "RUNNING" || state === "STARTING") {
            yield* synthetics
                ...
                .pipe(Effect.catchTag(["ResourceNotFoundException", "ConflictException"], () => Effect.void));
        }
        ...
        retryWhileConflict, Effect.catchTag("ResourceNotFoundException", () => Effect.void));
        yield* waitUntilGone(canaryName);
    });
    const backingFunctionName = (canaryName, engineArn) => {
        const marker = ":function:";
        const markerIndex = engineArn.indexOf(marker);
        const functionName = markerIndex === -1
            ? undefined
    ...
    const discoverBackingFunctionNames = Effect.fn(function* (canaryName, engineArn) {
        if (engineArn) {
        ...
        const exactBackingName = new RegExp(`^cwsyn-${escapedCanaryName}-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`, "i");
        const pages = yield* lambda.listFunctions
            .pages({})
            .pipe(Stream.runCollect);
        return Array.from(pages)
            .flatMap((page) => page.Functions ?? [])
            .flatMap((fn) => fn.FunctionName && exactBackingName.test(fn.FunctionName)
            ? [fn.FunctionName]
            : []);
    });
    const backingFunctionExists = (functionName) => lambda.getFunction({ FunctionName: functionName }).pipe(Effect.as(true), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(false)));
    const exactLogGroupExists = (logGroupName) => logs
        .describeLogGroups({
        logGroupNamePrefix: logGroupName,
        limit: 50,
    })
        .pipe(Effect.map((response) => (response.logGroups ?? []).some((group) => group.logGroupName === logGroupName)));
    const waitForBackingResourceGone = (canaryName, resource, identifier, exists) => exists.pipe(Effect.flatMap((visible) => visible
        ? Effect.fail(new CanaryBackingResourceStillVisible({
            canaryName,
            resource,
            identifier,
        }))
    ...
    const deleteBackingResources = Effect.fn(function* (canaryName, functionName) {
        const logGroupName = `/aws/lambda/${functionName}`;
        yield* lambda.deleteFunction({ FunctionName: functionName }).pipe(Effect.retry({
        ...
        yield* waitForBackingResourceGone(canaryName, "LambdaFunction", functionName, backingFunctionExists(functionName));
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
            if (yield* exactLogGroupExists(logGroupName)) {
                yield* logs.deleteLogGroup({ logGroupName }).pipe(Effect.retry({
        ...
        yield* waitForBackingResourceGone(canaryName, "LogGroup", logGroupName, exactLogGroupExists(logGroupName));
    });
    /**
     ...
     */
    const ensureExecutionRole = Effect.fn(function* ({ id, roleName, artifactBucket, vpc, }) {
        const { accountId, region } = yield* AWSEnvironment.current;
        ...
        yield* iam
            ...
            .pipe(Effect.catchTag("EntityAlreadyExistsException", () => Effect.sync(() => {
            created = false;
        })));
    ...
    const deleteExecutionRole = Effect.fn(function* (roleName) {
        yield* iam
            .deleteRolePolicy({
            RoleName: roleName,
            PolicyName: ROLE_POLICY_NAME,
        ...
        yield* iam
            ...
            .pipe(Effect.catchTag("NoSuchEntityException", () => Effect.void));
        yield* iam.deleteRole({ RoleName: roleName }).pipe(Effect.retry({
        ...
        return yield* Effect.die(new Error(`Synthetics execution role ${roleName} remained observable 30 seconds after delete`));
    });
    const desiredConfigHash = (props, executionRoleArn, artifactS3Location, runtimeVersion) => sha256Object({
        script: props.script,
        ...
        runConfig: props.runConfig === undefined
            ? {}
            : {
                ...props.runConfig,
                timeout: undefined,
    ...
    const toRunConfigInput = (props) => props.runConfig === undefined
        ? undefined
        : {
            TimeoutInSeconds: toWireSeconds(props.runConfig.timeout),
    ...
    const toScheduleInput = (props) => ({
        Expression: props.schedule?.expression ?? DEFAULT_SCHEDULE_EXPRESSION,
        DurationInSeconds: toWireSeconds(props.schedule?.duration),
    });
    const toVpcConfigInput = (props) => props.vpcConfig === undefined
        ? undefined
        : {
    ...
    return Canary.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const { artifactS3Location, artifactBucket } = parseArtifactLocation(news.artifactS3Location);
            ...
            const configHash = yield* desiredConfigHash(news, executionRoleArn, artifactS3Location, runtimeVersion);
            ...
            if (observed?.Status?.State === "ERROR" &&
                observed.Status?.StateReasonCode === "CREATE_FAILED") {
                yield* session.note(`canary ${canaryName} is in CREATE_FAILED (${observed.Status?.StateReason}); recreating`);
            ...
            if (observed === undefined) {
                ...
                for (let attempt = 0;; attempt++) {
                    ...
                    if (observed?.Status?.State === "ERROR" &&
                        ...
                        yield* session.note(`canary create failed (${observed.Status?.StateReason}); deleting`);
                        yield* deleteCanaryAndWait(canaryName);
                        if (attempt < 2) {
                            yield* Effect.sleep("10 seconds");
                            continue;
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            const backingFunctions = yield* discoverBackingFunctionNames(output.canaryName, engineArn);
            yield* deleteCanaryAndWait(output.canaryName);
            for (const functionName of backingFunctions) {
                yield* session.note(`deleting exact backing Lambda ${functionName} and log group /aws/lambda/${functionName}`);
