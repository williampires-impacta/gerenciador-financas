 ...
 * @resource
 */
export const KxEnvironment = Resource("AWS.FinSpace.KxEnvironment");
const createKxEnvironmentName = (id, props) => props.name
    ? Effect.succeed(props.name)
    : createPhysicalName({ id, maxLength: 255 });
const fetchKxTags = Effect.fn(function* (arn) {
...
const toKxAttributes = Effect.fn(function* (env, fallbackId) {
...
const readKxEnvironmentById = Effect.fn(function* (environmentId) {
    const response = yield* finspace
...
const findKxEnvironmentByName = Effect.fn(function* (name) {
    const environments = yield* finspace.listKxEnvironments.items({}).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk)));
 ...
 */
export class KxEnvironmentProvisioningFailed extends Data.TaggedError("KxEnvironmentProvisioningFailed") {
}
...
const waitForKxEnvironmentStatus = (environmentId, target) => retryWhileNotReady(Effect.gen(function* () {
    const response = yield* finspace
        .getKxEnvironment({ environmentId })
        .pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
...
export const KxEnvironmentProvider = () => Provider.effect(KxEnvironment, Effect.gen(function* () {
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            return yield* Effect.forEach(environments.filter((env) => !isGone(env.status)), (env) => toKxAttributes(env, env.environmentId ?? ""), { concurrency: 5 });
        }),
        ...
        diff: Effect.fn(function* ({ news, olds }) {
            ...
            const hadNetwork = olds.transitGatewayConfiguration !== undefined ||
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (env === undefined) {
                ...
                if (!created.environmentId) {
                    return yield* Effect.fail(new Error(`CreateKxEnvironment for '${name}' returned no id`));
                }
                ...
                if (env === undefined) {
                    return yield* Effect.fail(new Error(`failed to read created kdb environment ${name}`));
                }
            }
            ...
            const wantsNetwork = news.transitGatewayConfiguration !== undefined ||
                news.customDNSConfiguration !== undefined;
            const hasNetwork = env.transitGatewayConfiguration !== undefined ||
            ...
            if (!final) {
                return yield* Effect.fail(new Error(`failed to read reconciled kdb environment ${name}`));
            }
            return yield* toKxAttributes(final, environmentId);
        }),
        delete: Effect.fn(function* ({ output }) {
            yield* retryThroughConflict(finspace
