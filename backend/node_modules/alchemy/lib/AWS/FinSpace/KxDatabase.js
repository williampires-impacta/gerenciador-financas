 ...
 * @resource
 */
export const KxDatabase = Resource("AWS.FinSpace.KxDatabase");
const createDatabaseName = (id, props) => props.databaseName
    ? Effect.succeed(props.databaseName)
    : createPhysicalName({ id, maxLength: 63 });
const fetchDatabaseTags = Effect.fn(function* (arn) {
...
const readDatabase = Effect.fn(function* (environmentId, databaseName) {
    const response = yield* finspace
...
export const KxDatabaseProvider = () => Provider.effect(KxDatabase, Effect.gen(function* () {
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (attrs === undefined) {
                ...
                if (attrs === undefined) {
                    return yield* Effect.fail(new Error(`failed to read created kdb database ${databaseName}`));
                }
            }
            ...
            if (!final) {
                return yield* Effect.fail(new Error(`failed to read reconciled kdb database ${databaseName}`));
            }
            return final;
        }),
