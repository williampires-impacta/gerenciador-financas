 ...
 * @resource
 */
export const Repository = Resource("AWS.ECR.Repository");
export const RepositoryProvider = () => Provider.effect(Repository, Effect.gen(function* () {
    ...
    const toPolicyText = (policy) => policy === undefined
        ? undefined
        : typeof policy === "string"
            ? policy
            : stringifyPolicyDocument(policy);
    const readPolicy = (repositoryName) => ecr.getRepositoryPolicy({ repositoryName }).pipe(Effect.map((response) => response.policyText), Effect.catchTag([
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!repository?.repositoryArn || !repository.repositoryUri) {
                const created = yield* ecr
                    ...
                    .pipe(Effect.catchTag("RepositoryAlreadyExistsException", () => ecr
                    .describeRepositories({
                    repositoryNames: [repositoryName],
                })
                    .pipe(Effect.map((res) => ({
                ...
                if (!repository?.repositoryArn || !repository.repositoryUri) {
                    return yield* Effect.fail(new Error(`Failed to create or read repository ${repositoryName}`));
                }
            }
            ...
            const observedTags = Object.fromEntries((listedTags.tags ?? [])
        ...
        list: () => Effect.gen(function* () {
            const repositories = yield* ecr.describeRepositories.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.repositories ?? [])));
            return yield* Effect.forEach(repositories.filter((r) => r.repositoryName != null &&
                ...
                const tags = Object.fromEntries((listedTags.tags ?? [])
                ...
                const lifecyclePolicyText = yield* ecr
                    ...
                    .pipe(Effect.map((res) => res.lifecyclePolicyText), Effect.catchTag("LifecyclePolicyNotFoundException", () => Effect.succeed(undefined)));
                return {
                    repositoryName: repository.repositoryName,
                    repositoryArn: repository.repositoryArn,
                    repositoryUri: repository.repositoryUri,
