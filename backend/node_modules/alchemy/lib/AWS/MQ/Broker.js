 ...
 * @resource
 */
export const Broker = Resource("AWS.MQ.Broker");
...
const toWireUsers = (users) => users.map((u) => ({
    Username: u.username,
...
export const BrokerProvider = () => Provider.effect(Broker, Effect.gen(function* () {
    const toName = (id, props) => props.brokerName
    ...
    const readBroker = Effect.fn(function* (brokerId) {
        return yield* mq
    ...
    const findByName = Effect.fn(function* (name) {
        const summary = yield* mq.listBrokers.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    ...
    const waitForSettled = Effect.fn(function* (brokerId) {
        return yield* readBroker(brokerId).pipe(Effect.flatMap((broker) => {
    ...
    const waitUntilGone = Effect.fn(function* (brokerId) {
        yield* readBroker(brokerId).pipe(Effect.flatMap((broker) => {
            if (broker === undefined)
                return Effect.void;
            return Effect.fail(new BrokerNotSettled({
                brokerId,
    ...
    return {
        ...
        diff: Effect.fn(function* ({ id, olds, news }) {
            ...
            if ((news.engineType ?? undefined) !== (olds?.engineType ?? undefined))
                return { action: "replace" };
            if ((news.deploymentMode ?? "SINGLE_INSTANCE") !==
                (olds?.deploymentMode ?? "SINGLE_INSTANCE"))
                return { action: "replace" };
            if ((news.publiclyAccessible ?? false) !==
            ...
            if (news.subnetIds !== undefined &&
                ...
                return { action: "replace" };
            if ((news.encryptionOptions?.kmsKeyId ?? undefined) !==
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (observed === undefined) {
                const created = yield* mq
                    ...
                    .pipe(Effect.catchTag("ConflictException", () => Effect.gen(function* () {
                    const existing = yield* findByName(name);
                    if (existing?.BrokerId) {
                        return { BrokerId: existing.BrokerId };
                    }
                    return yield* Effect.fail(new Error(`MQ broker '${name}' create conflicted but no broker was found`));
                })));
            ...
            if (settled === undefined) {
                return yield* Effect.fail(new Error(`MQ broker '${name}' disappeared while reconciling`));
            }
            observed = settled;
            if (observed.BrokerState === "CREATION_FAILED") {
                return yield* Effect.fail(new Error(`MQ broker '${name}' failed to create (state: CREATION_FAILED)`));
            }
            ...
            if (news.maintenanceWindow !== undefined &&
        ...
        list: () => mq.listBrokers.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
            .flatMap((page) => page.BrokerSummaries ?? [])
            .flatMap((s) => s.BrokerId !== undefined &&
