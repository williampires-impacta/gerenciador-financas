...
import { decodeFqn, encodeFqn } from "../../FQN.js";
import { STATE_STORE_VERSION } from "../../State/HttpStateApi.js";
import { State, StateStoreError, } from "../../State/State.js";
import { encodeState, reviveState } from "../../State/StateEncoding.js";
import { recordStateStoreInit } from "../../Telemetry/Metrics.js";
import { AwsAuth } from "../AuthProvider.js";
import * as AwsCredentials from "../Credentials.js";
import * as Endpoint from "../Endpoint.js";
import { AWSEnvironment, Default as DefaultEnvironment, } from "../Environment.js";
import * as AwsRegion from "../Region.js";
/**
 ...
 * @resource
 */
export const state = (options = {}) => Layer.effect(State, Effect.gen(function* () {
    const context = yield* Effect.context();
    const make = makeS3State(options).pipe(recordStateStoreInit, Effect.orDie, Effect.provideContext(context));
    return yield* Effect.cached(make);
})).pipe(Layer.provideMerge(AwsRegion.fromEnvironment), Layer.provideMerge(AwsCredentials.fromEnvironment), Layer.provideMerge(Endpoint.fromEnvironment), Layer.provideMerge(DefaultEnvironment), Layer.provideMerge(AwsAuth), Layer.provideMerge(CredentialsStoreLive), Layer.orDie);
/**
 ...
 */
export const makeS3State = (options = {}) => Effect.gen(function* () {
    const context = yield* Effect.context();
    const prefix = options.prefix
        ? `${options.prefix.replace(/\/+$/, "")}/`
        : "";
    const toError = (cause) => new StateStoreError({
    ...
    // of the state store.
    const bucket = yield* Effect.cached(Effect.gen(function* () {
    ...
    const run = (f) => bucket.pipe(Effect.flatMap((bucket) => f(bucket).pipe(Effect.provideContext(context), Effect.mapError(toError))));
    const stagePrefix = ({ stack, stage }) => `${prefix}${stack}/${stage}/`;
    const resourceKey = (request) => `${stagePrefix(request)}${encodeFqn(request.fqn)}.json`;
    const outputKey = (request) => `${stagePrefix(request)}${OUTPUT_FILE}`;
    /** All object keys under `keyPrefix`, across pagination. */
    const listKeys = (bucket, keyPrefix) => s3.listObjectsV2.pages({ Bucket: bucket, Prefix: keyPrefix }).pipe(Stream.flatMap((page) => Stream.fromIterable(page.Contents ?? [])), Stream.map((object) => object.Key), Stream.filter((key) => key !== undefined), Stream.runCollect, Effect.map((keys) => Array.from(keys)));
    /**
     * Immediate "subdirectory" names under `keyPrefix`, derived from
     * S3 CommonPrefixes (delimiter `/`), across pagination.
     */
    const listChildren = (bucket, keyPrefix) => s3.listObjectsV2
        .pages({ Bucket: bucket, Prefix: keyPrefix, Delimiter: "/" })
        .pipe(Stream.flatMap((page) => Stream.fromIterable(page.CommonPrefixes ?? [])), 
    // `{keyPrefix}{name}/` -> `{name}`
    Stream.map((common) => common.Prefix), Stream.filter((p) => p !== undefined), Stream.map((p) => p.slice(keyPrefix.length, -1)), Stream.runCollect, Effect.map((names) => Array.from(names)));
    /** Read and revive a JSON object; `undefined` when the key is absent. */
    const readJson = (bucket, key) => s3.getObject({ Bucket: bucket, Key: key }).pipe(Effect.flatMap((result) => result.Body === undefined
        ? Effect.succeed(undefined)
        : Stream.mkString(Stream.decodeText(result.Body)).pipe(Effect.flatMap((text) => Effect.try({
            try: () => JSON.parse(text, reviveState),
            catch: (cause) => new StateStoreError({
                message: `Failed to parse state object '${key}'`,
                cause: cause instanceof Error ? cause : undefined,
            }),
        })))), Effect.catchTag("NoSuchKey", () => Effect.succeed(undefined)));
    const writeJson = (bucket, key, value) => s3.putObject({
        Bucket: bucket,
        Key: key,
    ...
    const deleteAll = (bucket, keyPrefix) => Effect.gen(function* () {
    ...
    const state = {
        ...
        listStages: (stack) => run((bucket) => listChildren(bucket, `${prefix}${stack}/`)),
        get: (request) => run((bucket) => readJson(bucket, resourceKey(request))),
        ...
        set: (request) => run((bucket) => writeJson(bucket, resourceKey(request), request.value)).pipe(Effect.map(() => request.value)),
        delete: (request) => run((bucket) => s3.deleteObject({ Bucket: bucket, Key: resourceKey(request) })).pipe(Effect.asVoid),
        deleteStack: ({ stack, stage }) => run((bucket) => deleteAll(bucket, stage === undefined
            ? `${prefix}${stack}/`
            : stagePrefix({ stack, stage }))),
        list: (request) => run((bucket) => listKeys(bucket, stagePrefix(request))).pipe(Effect.map((keys) => keys
        ...
        setOutput: (request) => run((bucket) => writeJson(bucket, outputKey(request), request.value)).pipe(Effect.map(() => request.value)),
    };
    return state;
});
/**
 ...
 */
export const createStateBucketName = (accountId, region) => `alchemy-state-${accountId}-${region}-an`.toLowerCase();
/**
 ...
 */
const ensureStateBucket = (bucket, region, options) => Effect.gen(function* () {
    ...
    const exists = yield* s3.headBucket({ Bucket: bucket }).pipe(Effect.map(() => true), Effect.catchTag(["NotFound", "NoSuchBucket"], () => Effect.succeed(false)));
    if (!exists) {
        ...
        yield* s3.headBucket({ Bucket: bucket }).pipe(Effect.retry({
    ...
    const encryptionFingerprint = (rule) => JSON.stringify({
    ...
    if (encryptionFingerprint(observedEncryption) !==
    ...
    const observedPublicAccess = yield* s3
        ...
        .pipe(Effect.map((result) => result.PublicAccessBlockConfiguration), Effect.catchTag("NoSuchPublicAccessBlockConfiguration", () => Effect.succeed(undefined)));
    const publicAccessFingerprint = (config) => JSON.stringify({
    ...
    if (publicAccessFingerprint(observedPublicAccess) !==
    ...
    const observedOwnership = yield* s3
        ...
        .pipe(Effect.map((result) => result.OwnershipControls?.Rules?.[0]?.ObjectOwnership), Effect.catchTag("OwnershipControlsNotFoundError", () => Effect.succeed(undefined)));
