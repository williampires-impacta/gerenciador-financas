 ...
 * @resource
 */
export const CompositeAlarm = Resource("AWS.CloudWatch.CompositeAlarm");
export const CompositeAlarmProvider = () => Provider.effect(CompositeAlarm, Effect.gen(function* () {
    ...
    const readCompositeAlarm = Effect.fn(function* (alarmName) {
        const described = yield* cloudwatch.describeAlarms({
            AlarmNames: [alarmName],
            AlarmTypes: ["CompositeAlarm"],
        });
        const compositeAlarm = described.CompositeAlarms?.find((candidate) => candidate.AlarmName === alarmName);
        if (!compositeAlarm?.AlarmName || !compositeAlarm.AlarmArn) {
            return undefined;
        }
        const tags = yield* readResourceTags(compositeAlarm.AlarmArn).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})));
        return {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, output, session }) {
            ...
            yield* retryConcurrent(cloudwatch.putCompositeAlarm({
                ...news,
                AlarmName: name,
            }));
            ...
            const state = yield* readCompositeAlarm(name);
            if (!state) {
                return yield* Effect.fail(new Error(`failed to read reconciled composite alarm '${name}'`));
            }
            return {
                ...state,
                tags,
        ...
        list: () => Effect.gen(function* () {
            ...
            const alarms = yield* cloudwatch.describeAlarms
                ...
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.CompositeAlarms ?? []).filter((candidate) => candidate.AlarmName != null &&
                candidate.AlarmArn != null))));
            return yield* Effect.forEach(alarms, (compositeAlarm) => readResourceTags(compositeAlarm.AlarmArn).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})), Effect.map((tags) => ({
                alarmName: compositeAlarm.AlarmName,
                alarmArn: compositeAlarm.AlarmArn,
                stateValue: compositeAlarm.StateValue,
                stateReason: compositeAlarm.StateReason,
