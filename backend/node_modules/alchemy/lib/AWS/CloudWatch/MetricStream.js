 ...
 * @resource
 */
export const MetricStream = Resource("AWS.CloudWatch.MetricStream");
export const MetricStreamProvider = () => Provider.effect(MetricStream, Effect.gen(function* () {
    const createMetricStreamName = (id, props = {}) => createName(id, props.name, 255);
    const metricStreamArn = (name) => AWSEnvironment.current.pipe(Effect.map((env) => `arn:aws:cloudwatch:${env.region}:${env.accountId}:metric-stream/${name}`));
    const readMetricStream = Effect.fn(function* (name) {
        const output = yield* cloudwatch
            .getMetricStream({
            Name: name,
        })
            .pipe(Effect.catchTag("InvalidParameterValueException", () => Effect.succeed(undefined)), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
        if (!output?.Name || !output.Arn) {
            return undefined;
        }
        const tags = yield* readResourceTags(output.Arn).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({})), Effect.catchTag("InvalidParameterValueException", () => Effect.succeed({})));
        return {
            metricStreamName: output.Name,
            metricStreamArn: output.Arn,
            state: output.State,
            metricStream: output,
            tags,
        };
    });
    const syncMetricStreamState = Effect.fn(function* ({ name, enabled, }) {
        if (enabled === false) {
            yield* retryConcurrent(cloudwatch.stopMetricStreams({
                Names: [name],
            }));
            return;
        }
        yield* retryConcurrent(cloudwatch.startMetricStreams({
            Names: [name],
        }));
    });
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const entries = yield* cloudwatch.listMetricStreams.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Entries ?? [])));
            const results = yield* Effect.forEach(entries, (entry) => entry.Name
                ? readMetricStream(entry.Name)
                : Effect.succeed(undefined), { concurrency: 10 });
            return results.filter((state) => state !== undefined);
        }),
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.metricStreamName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, output, session }) {
            ...
            yield* retryConcurrent(cloudwatch.putMetricStream({
                ...news,
                Name: name,
            }));
            ...
            const state = yield* readMetricStream(name);
            if (!state) {
                return yield* Effect.fail(new Error(`failed to read reconciled metric stream '${name}'`));
            }
            return {
                ...state,
                tags,
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* retryConcurrent(cloudwatch.deleteMetricStream({
