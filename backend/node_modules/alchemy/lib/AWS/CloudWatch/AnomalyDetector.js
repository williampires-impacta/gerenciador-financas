...
import { detectorIdentity, matchesDetectorIdentity, retryConcurrent, } from "./common.js";
 ...
 * @resource
 */
export const AnomalyDetector = Resource("AWS.CloudWatch.AnomalyDetector");
const toDescribeRequest = (input) => {
...
const describeDetector = Effect.fn(function* (props, options) {
    ...
    const detectors = yield* cloudwatch.describeAnomalyDetectors
        .items(request)
        .pipe(Stream.takeUntil((candidate) => matchesDetectorIdentity(candidate, props)), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)));
    ...
    if (!detector && options?.logMisses) {
        ...
        yield* Effect.logInfo(`${prefix}${attempt}; request=${JSON.stringify(request)} candidates=${JSON.stringify(detectors.map((candidate) => ({
...
export const AnomalyDetectorProvider = () => Provider.succeed(AnomalyDetector, {
    ...
    cloudwatch.describeAnomalyDetectors.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.AnomalyDetectors ?? []).map((detector) => ({
    ...
    reconcile: Effect.fn(function* ({ news, session }) {
        ...
        const state = yield* Effect.suspend(() => {
            ...
            return describeDetector(news, {
                ...
                : Effect.fail(new AnomalyDetectorNotVisible({
    ...
    delete: Effect.fn(function* ({ output }) {
        yield* retryConcurrent(cloudwatch.deleteAnomalyDetector(toDeleteRequest(output.anomalyDetector))).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void));
    }),
});
