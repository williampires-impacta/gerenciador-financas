...
import { createManagedTags, createName, retryConcurrent } from "./common.js";
/**
 ...
 * @resource
 */
export const AlarmMuteRule = Resource("AWS.CloudWatch.AlarmMuteRule");
export const AlarmMuteRuleProvider = () => Provider.effect(AlarmMuteRule, Effect.gen(function* () {
    const createMuteRuleName = (id, props = {}) => createName(id, props.name, 255);
    const alarmMuteRuleArn = (name) => AWSEnvironment.current.pipe(Effect.map((env) => `arn:aws:cloudwatch:${env.region}:${env.accountId}:alarm-mute-rule:${name}`));
    const readAlarmMuteRule = Effect.fn(function* (name) {
        ...
        return {
            ...
            alarmMuteRuleArn: output.AlarmMuteRuleArn,
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.alarmMuteRuleName ??
        ...
        list: () => Effect.gen(function* () {
            const summaries = yield* cloudwatch.listAlarmMuteRules
                .pages({})
                .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.AlarmMuteRuleSummaries ?? [])));
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            yield* retryConcurrent(cloudwatch.putAlarmMuteRule({
                ...news,
                Name: name,
            ...
            if (!state) {
                return yield* Effect.fail(new Error(`failed to read reconciled alarm mute rule '${name}'`));
            }
            return {
                ...state,
                tags,
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* retryConcurrent(cloudwatch.deleteAlarmMuteRule({
