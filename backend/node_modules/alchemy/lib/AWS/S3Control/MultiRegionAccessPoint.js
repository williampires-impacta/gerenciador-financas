 ...
 * @resource
 */
export const MultiRegionAccessPoint = Resource("AWS.S3Control.MultiRegionAccessPoint");
/**
 ...
 */
export class MultiRegionAccessPointNotReady extends Data.TaggedError("MultiRegionAccessPointNotReady") {
}
/**
 ...
 */
export class MultiRegionAccessPointNotDeleted extends Data.TaggedError("MultiRegionAccessPointNotDeleted") {
}
/**
 ...
 */
const retryWhileMrapTransitions = (self) => Effect.retry(self, {
    while: (e) => e._tag === "MultiRegionAccessPointNotReady" ||
...
export const MultiRegionAccessPointProvider = () => Provider.effect(MultiRegionAccessPoint, Effect.gen(function* () {
    const createName = Effect.fn(function* (id, props) {
        // Multi-Region Access Point names: 3-50 chars, lowercase.
        return (props.multiRegionAccessPointName ??
            (yield* createPhysicalName({ id, maxLength: 50, lowercase: true })));
    });
    const mrapArn = (accountId, alias) => `arn:aws:s3::${accountId}:accesspoint/${alias ?? ""}`;
    const observeMrap = (accountId, name) => inMrapRegion(s3control
        .getMultiRegionAccessPoint({ AccountId: accountId, Name: name })
        .pipe(Effect.map((r) => r.AccessPoint), Effect.catchTag("NoSuchMultiRegionAccessPoint", () => Effect.succeed(undefined))));
    const toAttrs = (name, accountId, report) => ({
    ...
    const awaitReady = (accountId, name) => retryWhileMrapTransitions(observeMrap(accountId, name).pipe(Effect.flatMap((report) => report?.Status === "READY"
        ? Effect.succeed(report)
        : Effect.fail(new MultiRegionAccessPointNotReady({
            name,
            status: report?.Status,
        })))));
    const awaitGone = (accountId, name) => retryWhileMrapTransitions(observeMrap(accountId, name).pipe(Effect.flatMap((report) => report === undefined
        ? Effect.void
        : Effect.fail(new MultiRegionAccessPointNotDeleted({
            name,
            status: report.Status,
        })))));
    const canonProps = (props) => JSON.stringify({
    ...
    return MultiRegionAccessPoint.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            const pages = yield* inMrapRegion(s3control.listMultiRegionAccessPoints
                .pages({ AccountId: accountId })
                .pipe(Stream.runCollect));
            return Array.from(pages).flatMap((page) => (page.AccessPoints ?? []).flatMap((report) => report.Name !== undefined
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (report === undefined) {
                ...
                yield* session.note(`Creating Multi-Region Access Point ${name} (takes several minutes)...`);
                yield* inMrapRegion(s3control.createMultiRegionAccessPoint({
                    AccountId: accountId,
        ...
        delete: Effect.fn(function* ({ output, session }) {
            ...
            if (report.Status !== "DELETING") {
                const clientToken = yield* Effect.sync(() => crypto.randomUUID());
                yield* session.note(`Deleting Multi-Region Access Point ${name} (takes several minutes)...`);
                yield* inMrapRegion(s3control
