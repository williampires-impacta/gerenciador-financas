 ...
 * @resource
 */
export const AccessPointPolicy = Resource("AWS.S3Control.AccessPointPolicy");
export const AccessPointPolicyProvider = () => Provider.effect(AccessPointPolicy, Effect.gen(function* () {
    const observePolicy = (accountId, name) => s3control
        .getAccessPointPolicy({ AccountId: accountId, Name: name })
        .pipe(Effect.map((r) => r.Policy), Effect.catchTag(["NoSuchAccessPointPolicy", "NoSuchAccessPoint"], () => Effect.succeed(undefined)));
    const desiredPolicy = (props) => typeof props.policy === "string"
        ? props.policy
