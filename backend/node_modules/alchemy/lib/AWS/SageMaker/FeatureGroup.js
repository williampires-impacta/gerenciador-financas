 ...
 * @resource
 */
export const FeatureGroup = Resource("AWS.SageMaker.FeatureGroup");
const createFeatureGroupName = (id, props) => props.featureGroupName
    ? Effect.succeed(props.featureGroupName)
    : createPhysicalName({ id, maxLength: 64 });
const fetchFeatureGroupTags = Effect.fn(function* (arn) {
...
const describeFeatureGroupOrUndefined = (name) => sagemaker
...
const waitForFeatureGroup = (name, target) => retryWhileNotReady(Effect.gen(function* () {
    const described = yield* describeFeatureGroupOrUndefined(name);
    if (target === "Gone") {
        if (described === undefined)
            return;
        return yield* Effect.fail(new FeatureGroupNotReady({
            featureGroupName: name,
            status: described.FeatureGroupStatus,
        }));
    }
    if (described?.FeatureGroupStatus === "Created")
        return;
    if (described?.FeatureGroupStatus === "CreateFailed" ||
        described?.FeatureGroupStatus === "DeleteFailed") {
        return yield* Effect.fail(new FeatureGroupCreateFailed({
            featureGroupName: name,
...
export const FeatureGroupProvider = () => Provider.effect(FeatureGroup, Effect.gen(function* () {
    return {
        stables: [
            "featureGroupName",
            "featureGroupArn",
        ...
        list: () => Effect.gen(function* () {
            const summaries = yield* sagemaker.listFeatureGroups.pages({}).pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.FeatureGroupSummaries ?? [])));
            const hydrated = yield* Effect.forEach(summaries.flatMap((s) => s.FeatureGroupName !== undefined ? [s.FeatureGroupName] : []), (name) => describeFeatureGroupOrUndefined(name).pipe(Effect.map((d) => (d === undefined ? undefined : toAttrs(d)))), { concurrency: 5 });
            return hydrated.filter((attrs) => attrs !== undefined);
        }),
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.featureGroupName ??
