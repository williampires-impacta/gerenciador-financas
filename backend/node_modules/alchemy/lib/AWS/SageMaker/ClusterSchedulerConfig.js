 ...
 * @resource
 */
export const ClusterSchedulerConfig = Resource("AWS.SageMaker.ClusterSchedulerConfig");
const createConfigName = (id, props) => props.name
    ? Effect.succeed(props.name)
    : createPhysicalName({ id, maxLength: 63 });
const describeConfigOrUndefined = (configId) => sagemaker
 ...
 */
const findConfigByName = Effect.fn(function* (name) {
    const summaries = yield* sagemaker.listClusterSchedulerConfigs
        .pages({ NameContains: name })
        .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.ClusterSchedulerConfigSummaries ?? [])));
    return summaries.find((s) => s.Name === name && s.Status !== "Deleted");
});
const fetchConfigTags = Effect.fn(function* (arn) {
...
const waitForConfig = (configId, target) => retryWhileNotReady(Effect.gen(function* () {
    ...
    if (target === "Gone") {
        ...
        if (FAILED_STATUSES.includes(described.Status)) {
            return yield* Effect.fail(new SchedulerConfigFailed({
                configId,
                status: described.Status,
                message: described.FailureReason,
            }));
        }
        return yield* Effect.fail(new SchedulerConfigNotReady({ configId, status: described.Status }));
    }
...
export const ClusterSchedulerConfigProvider = () => Provider.effect(ClusterSchedulerConfig, Effect.gen(function* () {
    return {
        stables: [
        ...
        list: () => Effect.gen(function* () {
            const summaries = yield* sagemaker.listClusterSchedulerConfigs
                .pages({})
                .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.ClusterSchedulerConfigSummaries ?? [])));
            return summaries.flatMap((s) => s.ClusterSchedulerConfigId !== undefined && s.Status !== "Deleted"
                ? [
                    {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const configId = output?.clusterSchedulerConfigId ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            let described = configId !== undefined
            ...
            if (described === undefined) {
                ...
                if (configId === undefined) {
                    return yield* Effect.fail(new Error(`failed to create cluster scheduler config ${name}`));
                }
            ...
            described = yield* describeConfigOrUndefined(configId);
            if (described === undefined) {
                return yield* Effect.fail(new Error(`failed to read reconciled cluster scheduler config ${name}`));
            }
            const attrs = toAttrs(described);
...
//# sourceMappingURL=ClusterSchedulerConfig.js.map