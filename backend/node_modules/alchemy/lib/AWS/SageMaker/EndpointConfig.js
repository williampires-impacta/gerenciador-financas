 ...
 * @resource
 */
export const EndpointConfig = Resource("AWS.SageMaker.EndpointConfig");
const createConfigName = (id, props) => props.endpointConfigName
...
const readEndpointConfig = Effect.fn(function* (name) {
...
export const EndpointConfigProvider = () => Provider.effect(EndpointConfig, Effect.gen(function* () {
    return {
        stables: ["endpointConfigName", "endpointConfigArn"],
        list: () => Effect.gen(function* () {
            const summaries = yield* sagemaker.listEndpointConfigs
                .pages({})
                .pipe(EffectStream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.EndpointConfigs ?? [])));
            return summaries.flatMap((s) => s.EndpointConfigName !== undefined &&
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.endpointConfigName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (attrs === undefined) {
                ...
                if (attrs === undefined) {
                    return yield* Effect.fail(new Error(`failed to read created endpoint config ${name}`));
                }
            }
