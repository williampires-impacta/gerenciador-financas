 ...
 * @resource
 */
export const AutoScalingConfiguration = Resource("AWS.AppRunner.AutoScalingConfiguration");
export const AutoScalingConfigurationProvider = () => Provider.effect(AutoScalingConfiguration, Effect.gen(function* () {
    const toName = (id, props) => props.autoScalingConfigurationName
    ...
    const findLatest = Effect.fn(function* (name) {
        ...
        const summary = (page.AutoScalingConfigurationSummaryList ?? []).find((s) => s.AutoScalingConfigurationName === name &&
            isActiveStatus(s.Status) &&
            s.AutoScalingConfigurationArn !== undefined);
        if (!summary?.AutoScalingConfigurationArn)
            return undefined;
    ...
    const toAttrs = Effect.fn(function* (config) {
        if (!config.AutoScalingConfigurationName ||
            !config.AutoScalingConfigurationArn ||
            config.AutoScalingConfigurationRevision === undefined) {
            return yield* Effect.fail(new Error("App Runner auto scaling configuration is missing its name, ARN, or revision"));
        }
        return {
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.autoScalingConfigurationName ??
        ...
        list: () => apprunner.listAutoScalingConfigurations
            ...
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.AutoScalingConfigurationSummaryList ?? []).filter((s) => isActiveStatus(s.Status) &&
            s.AutoScalingConfigurationArn !== undefined &&
            ...
            // teardown (nuke).
            s.AutoScalingConfigurationName !== "DefaultConfiguration"))), Effect.flatMap(Effect.forEach((summary) => apprunner
            .describeAutoScalingConfiguration({
            AutoScalingConfigurationArn: summary.AutoScalingConfigurationArn,
        })
            .pipe(Effect.flatMap((r) => toAttrs(r.AutoScalingConfiguration)), 
