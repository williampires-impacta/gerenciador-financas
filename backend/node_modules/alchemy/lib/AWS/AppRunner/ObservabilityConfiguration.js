 ...
 * @resource
 */
export const ObservabilityConfiguration = Resource("AWS.AppRunner.ObservabilityConfiguration");
export const ObservabilityConfigurationProvider = () => Provider.effect(ObservabilityConfiguration, Effect.gen(function* () {
    const toName = (id, props) => props.observabilityConfigurationName
    ...
    const findLatest = Effect.fn(function* (name) {
        const page = yield* apprunner.listObservabilityConfigurations({
            ObservabilityConfigurationName: name,
            LatestOnly: true,
        });
        const summary = (page.ObservabilityConfigurationSummaryList ?? []).find((s) => s.ObservabilityConfigurationName === name &&
            s.ObservabilityConfigurationArn !== undefined);
        if (!summary?.ObservabilityConfigurationArn)
            return undefined;
    ...
    const toAttrs = Effect.fn(function* (config) {
        if (!config.ObservabilityConfigurationName ||
            !config.ObservabilityConfigurationArn ||
            config.ObservabilityConfigurationRevision === undefined) {
            return yield* Effect.fail(new Error("App Runner observability configuration is missing its name, ARN, or revision"));
        }
        return {
     ...
     */
    const deleteAllRevisions = Effect.fn(function* (name) {
        ...
        const arns = Array.from(pages).flatMap((page) => (page.ObservabilityConfigurationSummaryList ?? [])
            .filter((s) => s.ObservabilityConfigurationName === name)
            .flatMap((s) => s.ObservabilityConfigurationArn !== undefined
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const name = output?.observabilityConfigurationName ??
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
            ...
            const drifted = observed !== undefined &&
        ...
        list: () => apprunner.listObservabilityConfigurations
            ...
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.ObservabilityConfigurationSummaryList ?? []).flatMap((s) => s.ObservabilityConfigurationArn !== undefined &&
            ...
            ? [s.ObservabilityConfigurationArn]
            : []))), Effect.flatMap(Effect.forEach((arn) => apprunner
            .describeObservabilityConfiguration({
            ObservabilityConfigurationArn: arn,
        })
            .pipe(Effect.flatMap((r) => isActiveStatus(r.ObservabilityConfiguration.Status)
            ? Effect.map(toAttrs(r.ObservabilityConfiguration), (attrs) => attrs)
            : Effect.succeed(undefined)), 
        // Tolerate a delete race — drop the item.
        Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined))), { concurrency: 4 })), Effect.map((items) => items.filter((item) => item !== undefined))),
    };
}));
