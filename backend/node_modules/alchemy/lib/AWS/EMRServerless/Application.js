...
import { awaitApplicationCreated, awaitApplicationStopped, } from "./internal.js";
/** Convert the alchemy-facing auto-stop config to the wire shape (minutes). */
const toWireAutoStop = (config) => ({
 ...
 * @resource
 */
export const Application = Resource("AWS.EMRServerless.Application");
/** Normalize a possibly-undefined structural prop for drift comparison. */
const canonical = (value) => value === undefined ? "" : JSON.stringify(value);
/**
...
export const ApplicationProvider = () => Provider.effect(Application, Effect.gen(function* () {
    ...
    const toAttributes = (application) => ({
    ...
    const observeById = Effect.fn(function* (applicationId) {
        return yield* emr.getApplication({ applicationId }).pipe(Effect.map((response) => response.application), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
    });
    ...
    const observeByName = Effect.fn(function* (name) {
        const summary = yield* emr.listApplications.items({}).pipe(Stream.filter((s) => s.name === name && s.state !== "TERMINATED"), Stream.runHead, Effect.map(Option.getOrUndefined));
        if (summary === undefined) {
            return undefined;
        }
        return yield* observeById(summary.id);
    });
    const observe = Effect.fn(function* (applicationId, name) {
        const byId = applicationId !== undefined
    ...
    const syncTags = Effect.fn(function* (application, desired) {
        // Diff against OBSERVED cloud tags (adoption may bring foreign tags).
        const observed = Object.fromEntries(Object.entries(application.tags ?? {}).filter((entry) => entry[1] !== undefined));
    ...
    return Application.Provider.of({
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session, instanceId, }) {
            ...
            const desiredAutoStop = news.autoStopConfiguration !== undefined
            ...
            if (application === undefined || application.state === "TERMINATED") {
                ...
                if (applicationId === undefined) {
                    return yield* Effect.fail(new emr.ResourceNotFoundException({
        ...
        delete: Effect.fn(function* ({ output }) {
            const application = yield* emr
                ...
                .pipe(Effect.map((response) => response.application), Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
            if (application === undefined || application.state === "TERMINATED") {
                return;
            }
            ...
            yield* awaitApplicationStopped(application.applicationId).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.succeed(undefined)));
            yield* emr
