 ...
 */
export const unredact = (value) => value === undefined
    ? undefined
    : Redacted.isRedacted(value)
        ? Redacted.value(value)
        : value;
export const retryIdentityCenter = (effect) => effect.pipe(Effect.retry({
...
export const listInstances = Effect.fn(function* () {
    return yield* ssoAdmin.listInstances
        ...
        .pipe(Stream.runCollect, Effect.map((instances) => Array.from(instances)));
});
export const resolveInstance = Effect.fn(function* (instanceArn) {
    ...
    if (instanceArn) {
        ...
        if (!selected?.InstanceArn || !selected.IdentityStoreId) {
            return yield* Effect.fail(new Error(`Identity Center instance '${instanceArn}' was not found`));
        }
        return selected;
    }
    ...
    return yield* Effect.fail(new Error("Unable to resolve a single visible Identity Center instance; pass instanceArn explicitly"));
});
export const resolveIdentityStoreId = Effect.fn(function* ({ identityStoreId, instanceArn, }) {
    if (identityStoreId) {
        return identityStoreId;
    }
    const instance = yield* resolveInstance(instanceArn);
    if (!instance.IdentityStoreId) {
        return yield* Effect.fail(new Error(`Identity Center instance '${instance.InstanceArn}' is missing an identity store ID`));
    }
    return instance.IdentityStoreId;
});
export const toInstanceAttributes = (instance) => ({
...
export const listGroups = Effect.fn(function* (identityStoreId) {
    return yield* identitystore.listGroups
        .items({
        IdentityStoreId: identityStoreId,
        MaxResults: 100,
    })
        .pipe(Stream.runCollect, Effect.map((groups) => Array.from(groups)));
});
