 ...
 * @resource
 */
export const Keyspace = Resource("AWS.Keyspaces.Keyspace");
...
export const KeyspaceProvider = () => Provider.effect(Keyspace, Effect.gen(function* () {
    const toName = (id, props) => props.keyspaceName
        ? Effect.succeed(props.keyspaceName)
        : createPhysicalName({ id, maxLength: 48 }).pipe(Effect.map((n) => n.replaceAll("-", "_")));
    const readKeyspace = Effect.fn(function* (name) {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output, session }) {
            ...
            if (observed === undefined) {
                ...
                observed = yield* readKeyspace(name).pipe(Effect.flatMap((k) => k === undefined
                    ? Effect.fail(new Error(`Keyspace '${name}' not found`))
                    : Effect.succeed(k)), Effect.retry({
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* keyspaces.deleteKeyspace({ keyspaceName: name }).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void), 
            ...
            yield* readKeyspace(name).pipe(Effect.flatMap((k) => k === undefined
                ? Effect.void
                : Effect.fail(new Error(`Keyspace '${name}' still deleting`))), Effect.retry({
                schedule: Schedule.max([
                    Schedule.fixed("3 seconds"),
                    Schedule.recurs(20),
                ]),
        ...
        list: () => keyspaces.listKeyspaces.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
