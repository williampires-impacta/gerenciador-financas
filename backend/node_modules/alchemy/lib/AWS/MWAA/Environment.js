 ...
 * @resource
 */
export const Environment = Resource("AWS.MWAA.Environment");
...
const toModuleInput = (config) => config === undefined
    ? undefined
    : { Enabled: config.enabled, LogLevel: config.logLevel };
const toLoggingInput = (config) => {
    if (config === undefined)
        return undefined;
    return {
...
const configValue = (v) => (Redacted.isRedacted(v) ? Redacted.value(v) : v);
...
export const EnvironmentProvider = () => Provider.effect(Environment, Effect.gen(function* () {
    const toName = (id, props) => props.environmentName
        ? Effect.succeed(props.environmentName)
        : createPhysicalName({ id, maxLength: 80 });
    const readEnvironment = Effect.fn(function* (name) {
    ...
    const waitForAvailable = Effect.fn(function* (name) {
        ...
        return yield* readEnvironment(name).pipe(Effect.flatMap((env) => {
            if (!env?.Arn) {
                return Effect.fail(new Error(`Environment '${name}' not found`));
            }
            const status = env.Status ?? "CREATING";
            if (FAILED_STATES.has(status)) {
                return Effect.fail(new Error(`Environment '${name}' reached terminal state '${status}'`));
            }
            if (status !== "AVAILABLE") {
                return Effect.fail(new Error(`Environment '${name}' not available (status: ${status})`));
            }
    ...
    const toAttrs = Effect.fn(function* (env) {
        if (!env.Name || !env.Arn) {
    ...
    return {
        ...
        diff: Effect.fn(function* ({ id, olds, news }) {
            ...
            if ((n.endpointManagement ?? undefined) !==
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (props.pluginsS3Path !== undefined &&
            ...
            if (props.requirementsS3Path !== undefined &&
            ...
            if (props.startupScriptS3Path !== undefined &&
            ...
            if (props.airflowConfigurationOptions !== undefined &&
                !sameConfigOptions(props.airflowConfigurationOptions, observed.AirflowConfigurationOptions)) {
        ...
        list: () => mwaa.listEnvironments.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.Environments ?? [])), Effect.flatMap(Effect.forEach((name) => readEnvironment(name), {
            concurrency: 4,
        })), Effect.map((envs) => envs.filter((env) => env !== undefined && env.Name !== undefined && !!env.Arn)), Effect.flatMap(Effect.forEach((env) => toAttrs(env), { concurrency: 4 }))),
    };
}));
