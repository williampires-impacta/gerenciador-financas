...
import { retrySmsVoiceThrottled } from "./internal.js";
/**
 ...
 * @resource
 */
export const EventDestination = Resource("AWS.PinpointSMSVoiceV2.EventDestination");
/**
 ...
 */
export class SmsVoiceEventDestinationMissing extends Data.TaggedError("SmsVoiceEventDestinationMissing") {
}
...
export const EventDestinationProvider = () => Provider.effect(EventDestination, Effect.gen(function* () {
    const toName = (id, props) => props.eventDestinationName
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            if (configurationSetName === undefined)
                return undefined;
            const eventDestinationName = output?.eventDestinationName ?? (yield* toName(id, olds ?? {}));
            const set = yield* getConfigurationSet(configurationSetName);
            const observed = set?.EventDestinations?.find((ed) => ed.EventDestinationName === eventDestinationName);
            if (set === undefined || observed === undefined)
                return undefined;
        ...
        diff: Effect.fn(function* ({ id, news, olds }) {
            if (!isResolved(news))
                return undefined;
            if (olds?.configurationSetName !== undefined &&
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const set = yield* getConfigurationSet(configurationSetName);
            let observed = set?.EventDestinations?.find((ed) => ed.EventDestinationName === eventDestinationName);
            ...
            observed = finalSet?.EventDestinations?.find((ed) => ed.EventDestinationName === eventDestinationName);
            if (finalSet === undefined || observed === undefined) {
                return yield* Effect.fail(new SmsVoiceEventDestinationMissing({
            ...
            const drifted = observed.Enabled !== desiredEnabled ||
        ...
        list: () => smsvoice.describeConfigurationSets.items({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((cs) => (cs.EventDestinations ?? []).map((ed) => ({
