 ...
 * @resource
 */
export const Certificate = Resource("Fly.Certificate");
...
export class CertificateMaterialRequired extends Data.TaggedError("Fly.CertificateMaterialRequired") {
}
...
const unwrapSecret = (value) => {
    if (value === undefined)
        return undefined;
    return Redacted.isRedacted(value) ? Redacted.value(value) : value;
};
const normalizePem = (pem) => (pem ?? "").replace(/\r\n/g, "\n").trim();
const desiredKind = (props) => props.kind ?? "acme";
const kindOf = (source) => source === "custom" ? "custom" : "acme";
const sourceOf = (detail) => {
    const entries = detail.certificates ?? [];
    if (entries.some((entry) => entry.source === "custom"))
        return "custom";
    if (entries.some((entry) => entry.source === "fly"))
        return "fly";
    return detail.acme_requested === true ? "fly" : "custom";
};
const sourceFromSummary = (summary) => summary.has_custom_certificate === true ? "custom" : "fly";
const toDnsRequirements = (req) => {
    if (req === undefined)
        return undefined;
    return {
        a: req.a,
        aaaa: req.aaaa,
        acmeChallenge: req.acme_challenge === undefined
            ? undefined
            : {
        ...
        cname: req.cname,
        ownership: req.ownership === undefined
            ? undefined
            : {
...
const toAttrs = (appName, detail, fallbackHostname) => ({
    appName,
...
const toAttrsFromSummary = (appName, summary) => {
...
const getByHostname = (appName, hostname) => machines
    .getAppCertificate({ app_name: appName, hostname })
    .pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
const listAppCertificates = (appName) => Effect.gen(function* () {
    const all = [];
    let cursor;
    for (let i = 0; i < 8; i++) {
...
const waitUntilGone = (appName, hostname) => getByHostname(appName, hostname).pipe(Effect.map((found) => found === undefined), Effect.repeat({
    schedule: Schedule.spaced("1 second"),
    until: (gone) => gone,
    times: 8,
}));
const requireCustomMaterial = (hostname, fullchain, privateKey) => fullchain === undefined ||
    fullchain.length === 0 ||
    privateKey === undefined ||
    privateKey.length === 0
    ? new CertificateMaterialRequired({ hostname })
    : undefined;
const createAcme = (appName, hostname) => machines
    .createAppAcmeCertificate({
    app_name: appName,
    hostname,
})
    .pipe(Effect.catchTag("Conflict", () => Effect.succeed(undefined)));
const createCustom = (appName, hostname, fullchain, privateKey) => machines
    ...
    .pipe(Effect.catchTag("Conflict", () => Effect.succeed(undefined)));
const replaceCustom = (appName, hostname, fullchain, privateKey) => Effect.gen(function* () {
    yield* machines
...
export const CertificateProvider = () => Provider.succeed(Certificate, {
    ...
    read: Effect.fn(function* ({ olds, output }) {
        ...
        if (appName === undefined || hostname === undefined)
            return undefined;
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(apps, (app) => listAppCertificates(app.appName).pipe(Effect.map((certs) => certs.flatMap((item) => {
    ...
    reconcile: Effect.fn(function* ({ news, olds, output }) {
        ...
        let current = output?.hostname !== undefined
            ? yield* getByHostname(output.appName.length > 0 ? output.appName : appName, output.hostname)
            : undefined;
        ...
        if (kind === "custom") {
            ...
            const materialChanged = olds === undefined ||
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        if (appName.length === 0 || hostname.length === 0)
            return;
        yield* machines
