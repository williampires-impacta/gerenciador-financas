...
export const destroying = (state) => state === "destroyed" ||
    state === "pending_destroy" ||
    state === "scheduled_for_destruction";
const transientState = (state) => state === "creating" || state === "pending" || state === "extending";
export const getVolumeById = (appName, volumeId) => machines.getVolumeById({ app_name: appName, volume_id: volumeId }).pipe(Effect.map((volume) => (destroying(volume.state) ? undefined : volume)), Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
export const listVolumesByApp = (appName) => machines.listVolumes({ app_name: appName }).pipe(Effect.map((volumes) => volumes.filter((volume) => !destroying(volume.state))), Effect.catchTag(["NotFound", "Forbidden"], () => Effect.succeed([])));
...
export const listVolumeGroup = (appName, name, region) => listVolumesByApp(appName).pipe(Effect.map((volumes) => volumes
    .filter((volume) => volume.name === name && volume.region === region)
    .sort(newestFirst)));
export const waitUntilVolumeReady = (appName, volumeId) => getVolumeById(appName, volumeId).pipe(Effect.flatMap((volume) => {
    if (volume === undefined)
        return Effect.succeed(undefined);
    if (transientState(volume.state)) {
        return Effect.fail(new VolumePending({
            volumeId,
...
export const waitUntilVolumeGone = (appName, volumeId) => getVolumeById(appName, volumeId).pipe(Effect.map((volume) => volume === undefined), Effect.repeat({
...
export const volumeGroupName = (id, disk) => Effect.gen(function* () {
    if (disk.name !== undefined)
        return sanitizeFlyVolumeName(disk.name);
...
export const syncVolume = Effect.fn(function* (appName, volume, disk) {
    ...
    const backupChanged = disk.autoBackupEnabled !== undefined &&
        disk.autoBackupEnabled !== current.auto_backup_enabled;
    const retentionChanged = disk.snapshotRetention !== undefined &&
...
export const deleteVolume = Effect.fn(function* (appName, volumeId) {
    ...
    yield* machines
        ...
        .pipe(Effect.catchTag("NotFound", () => Effect.void), Effect.retry({
...
export const listOwnedVolumes = Effect.fn(function* () {
    ...
    const groups = yield* Effect.forEach(apps, (app) => listVolumesByApp(app.appName).pipe(Effect.map((volumes) => volumes.flatMap((volume) => {
        if (!matchesAlchemyPhysicalName(volume.name))
            return [];
        const volumeId = volume.id;
        if (volumeId === undefined || volumeId.length === 0)
            return [];
