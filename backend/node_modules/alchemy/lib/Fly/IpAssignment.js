...
import { listOwnedApps } from "./App.js";
const resolveIpAssignmentProps = (props) => Effect.gen(function* () {
...
const IpAssignmentResource = Resource("Fly.IpAssignment");
/**
 ...
 * @resource
 */
export const IpAssignment = Object.assign((id, props) => IpAssignmentResource(id, resolveIpAssignmentProps(props)), IpAssignmentResource);
...
const asType = (value) => value === "v4" || value === "v6" || value === "shared_v4" ? value : undefined;
const inferType = (assignment, fallback) => {
    const wire = asType(assignment.type);
    if (wire !== undefined)
        return wire;
    if (assignment.shared === true)
        return "shared_v4";
...
const toAttrs = (appName, assignment, fallbackType) => {
...
const matchesDesired = (assignment, news) => {
    if (inferType(assignment, news.type) !== news.type)
        return false;
    if (news.region !== undefined && assignment.region !== news.region) {
        return false;
    }
    if (news.serviceName !== undefined &&
        assignment.service_name !== news.serviceName) {
        return false;
    }
    return true;
};
const listAssignments = (appName) => machines.listAppIPAssignments({ app_name: appName }).pipe(Effect.map((res) => res.ips ?? []), Effect.catchTag(["NotFound", "Forbidden"], () => Effect.succeed([])));
const findByIp = (appName, ip) => listAssignments(appName).pipe(Effect.map((ips) => ips.find((item) => item.ip === ip)));
const findMatching = (appName, news) => listAssignments(appName).pipe(Effect.map((ips) => ips.find((item) => matchesDesired(item, news))));
const waitUntilGone = (appName, ip) => findByIp(appName, ip).pipe(Effect.map((found) => found === undefined), Effect.repeat({
    schedule: Schedule.spaced("1 second"),
    until: (gone) => gone,
    times: 8,
}));
export const IpAssignmentProvider = () => Provider.succeed(IpAssignment, {
    ...
    diff: Effect.fn(function* ({ news, olds, output }) {
        ...
        const serviceChanged = news.serviceName !== undefined &&
            news.serviceName !== output.serviceName;
        const networkChanged = olds !== undefined &&
    ...
    read: Effect.fn(function* ({ olds, output }) {
        ...
        if (appName === undefined)
            return undefined;
        const found = output?.ip !== undefined && output.ip.length > 0
            ? yield* findByIp(appName, output.ip)
            : olds !== undefined
                ? yield* findMatching(appName, olds)
                : undefined;
        if (found === undefined || found.ip === undefined)
            return undefined;
        return toAttrs(appName, found, output?.type ?? olds?.type);
    }),
    list: Effect.fn(function* () {
        const apps = yield* listOwnedApps();
        const rows = yield* Effect.forEach(apps, (app) => listAssignments(app.appName).pipe(Effect.map((ips) => ips
            .filter((item) => item.ip !== undefined && item.ip.length > 0)
            .map((item) => toAttrs(app.appName, item)))), { concurrency: 8 });
        return rows.flat();
    }),
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        let current = output?.ip !== undefined && output.ip.length > 0
            ? yield* findByIp(appName, output.ip)
            : undefined;
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        const ip = output.ip;
        if (appName.length === 0 || ip.length === 0)
            return;
        yield* machines
...
//# sourceMappingURL=IpAssignment.js.map