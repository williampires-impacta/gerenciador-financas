...
import { FlyKmsError } from "./Errors.js";
 ...
 * machine identity.
 */
export const flyKmsPost = (appName, secretName, op, body) => Effect.tryPromise({
    try: async () => {
        const res = await fetch(`http://localhost/v1/apps/${encodeURIComponent(appName)}/secretkeys/${encodeURIComponent(secretName)}/${op}`, {
            method: "POST",
    ...
    catch: (cause) => cause instanceof FlyKmsError
        ? cause
