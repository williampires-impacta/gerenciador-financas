...
import { createFlyAppName, matchesAlchemyPhysicalName, sanitizeFlyAppName, } from "./Metadata.js";
...
export class RedisCommandError extends Data.TaggedError("Fly.RedisCommandError") {
}
/**
 ...
 * @resource
 */
export const Redis = Resource("Fly.Redis");
...
const stringRecordOf = (value) => Object.fromEntries(Object.entries(recordOf(value)).flatMap(([key, item]) => typeof item === "string" && item.length > 0 ? [[key, item]] : []));
const evictionOf = (options) => {
...
export const findRedisAddOn = (input) => listRedisAddOns().pipe(Effect.map((rows) => rows.find((row) => {
...
const cheapestPlan = (plans) => {
    ...
    const payg = ranked.find((plan) => /pay.?as.?you.?go|free/i.test(`${plan.displayName ?? ""} ${plan.name ?? ""}`));
    return payg ?? ranked[0];
};
const resolvePlan = (plan, existingId) => Effect.gen(function* () {
    const plans = yield* listRedisPlans();
    if (plan !== undefined) {
        const found = plans.find((item) => item.id === plan || item.name === plan || item.displayName === plan);
        if (found === undefined) {
...
const resolveOrganization = (orgSlug) => addons
    .organization({ slug: orgSlug })
    .pipe(Effect.catchTag("FlyIoParseError", () => Effect.succeed(undefined)));
const ensureTos = (orgSlug, organizationId) => Effect.gen(function* () {
    ...
    yield* Effect.result(addons.createExtensionTosAgreement({
        input: {
...
const waitUntilGone = (id, name) => findRedisAddOn({ id, name }).pipe(Effect.map((row) => row === undefined), Effect.repeat({
    schedule: Schedule.spaced("2 seconds"),
    until: (gone) => gone,
    times: 10,
}));
const desiredOptions = (props, plan, observed) => {
    const options = recordOf(observed);
    if (props.eviction !== undefined)
        options.eviction = props.eviction;
 ...
 * Machines boot.
 */
export const attachRedisSecrets = Effect.fn(function* (appName, attached) {
    if (appName.length === 0 || attached.length === 0)
        return;
    for (const item of attached) {
        const name = item.name;
        const id = item.id;
        if (name.length === 0 && (id === undefined || id.length === 0))
            continue;
        const row = yield* findRedisAddOn({ id, name }).pipe(Effect.flatMap((found) => found === undefined
            ? Effect.fail(new RedisPending({
                redisId: id ?? "",
                status: "missing",
            }))
            : Effect.succeed(found)), Effect.retry({
            while: (error) => error._tag === "Fly.RedisPending",
            times: 8,
            schedule: backoff,
        }), Effect.catchTag("Fly.RedisPending", () => findRedisAddOn({ id, name })));
        ...
        if (url === undefined || url.length === 0)
            continue;
        const updated = yield* Effect.result(machines.updateSecrets({
            app_name: appName,
...
export const RedisProvider = () => Provider.succeed(Redis, {
    ...
    read: Effect.fn(function* ({ id, olds, output }) {
        ...
        const found = (output?.redisId !== undefined
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        let current = output?.redisId !== undefined
        ...
        const plan = yield* resolvePlan(props.plan, current !== undefined && "addOnPlan" in current
            ? current.addOnPlan?.id
            : output?.planId);
        if (current === undefined) {
            yield* ensureTos(orgSlug, org.id);
            const created = yield* Effect.result(addons.createAddOn({
                input: {
                    type: REDIS_ADDON_TYPE,
                    name,
        ...
        if (planChanged || regionsChanged || optionsChanged || prodPackChanged) {
            const updated = yield* Effect.result(addons.updateAddOn({
                input: {
                    addOnId: current.id,
                    planId: plan.id,
        ...
        const latest = (yield* findRedisAddOn({ id: current.id, name })) ?? current;
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        const deleted = yield* Effect.result(addons.deleteAddOn({
            input: redisId.length > 0
                ? { addOnId: redisId }
