...
import { createFlyVolumeName, matchesAlchemyPhysicalName } from "./Metadata.js";
const resolveSecretProps = (props) => Effect.gen(function* () {
...
const SecretResource = Resource("Fly.Secret");
/**
 ...
 * @resource
 */
export const Secret = Object.assign((id, props) => SecretResource(id, resolveSecretProps(props)), SecretResource);
...
const unwrapSecret = (value) => Redacted.isRedacted(value) ? Redacted.value(value) : value;
...
const toAttrs = (appName, secret, fallbackName) => ({
    appName,
...
const listSecrets = (appName) => machines
    .listSecrets({
    app_name: appName,
    show_secrets: false,
})
    .pipe(Effect.map((res) => res.secrets ?? []), Effect.catchTag(["NotFound", "Forbidden"], () => Effect.succeed([])));
const flyDigestCandidates = (plain) => Effect.sync(() => {
...
export const SecretProvider = () => Provider.succeed(Secret, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(apps, (app) => listSecrets(app.appName).pipe(Effect.map((secrets) => secrets.flatMap((secret) => {
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* getByName(output.appName, output.name).pipe(Effect.map((secret) => secret === undefined), Effect.repeat({
