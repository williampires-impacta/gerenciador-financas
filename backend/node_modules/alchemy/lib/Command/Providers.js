...
export const providers = () => Layer.mergeAll(BuildProvider(), DevProvider(), ExecProvider(), 
// TODO: Remove before v2.0.0 stable release.
Renamed({ from: "Build.Command", to: "Command.Build" }), Renamed({ from: "Build.DevServer", to: "Command.Dev" })).pipe(Layer.provide(CommandExecutorLive()));
/**
 ...
 */
const Renamed = (input) => {
    const message = (id) => `The "${input.from}" resource has been renamed to "${input.to}". Please update "${id}" to use the new resource.`;
