...
const redactedValues = (env) => [
    ...new Set(Object.values(env ?? {})
        .filter((value) => Redacted.isRedacted(value))
        .map(Redacted.value)
        .filter((value) => value.length > 0)),
].sort((left, right) => right.length - left.length);
const redactionMarker = (secrets) => {
...
const redact = (value, secrets, marker) => secrets.reduce((safe, secret) => safe.split(secret).join(marker), value);
/**
 ...
 * text is emitted.
 */
const redactStream = (stream, secrets, marker) => {
    if (secrets.length === 0)
        return stream;
    return stream.pipe(Stream.mapAccum(() => "", (pending, chunk) => {
        ...
        while (remaining.length > 0) {
            ...
            let suffixLength = 0;
            for (const secret of secrets) {
                const candidateLength = Math.min(secret.length - 1, remaining.length);
        ...
        return [remaining, output.length === 0 ? [] : [output]];
    }, {
        onHalt: (pending) => pending.length === 0 ? [] : [redact(pending, secrets, marker)],
    }));
};
export const makeCommandRedactor = (env) => {
...
export const redactPlatformReason = (reason, redactor) => {
    ...
    return new SystemError({
        ...
        description: reason.description === undefined
            ? undefined
            : redactor.redact(reason.description),
        syscall: reason.syscall === undefined
            ? undefined
            : redactor.redact(reason.syscall),
        pathOrDescriptor: typeof reason.pathOrDescriptor === "string"
