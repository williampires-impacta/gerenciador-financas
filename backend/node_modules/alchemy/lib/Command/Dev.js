...
import { CommandExecutor, UnexpectedExit, makeCommandError, } from "./Command.js";
import { makeCommandRedactor } from "./Redaction.js";
/**
 ...
 * @resource
 */
export const Dev = Resource("Command.Dev");
export const DevProvider = () => ProviderLayer.dual(Dev, {
    live: DevProviderLive,
    local: DevProviderLocal,
});
export const DevProviderLive = () => Provider.succeed(Dev, {
    list: () => Effect.succeed([]),
    diff: () => Effect.succeed({ action: "noop" }),
    reconcile: () => Effect.succeed({ url: undefined }),
    delete: () => Effect.void,
});
export const DevProviderLocal = () => LocalProvider.make(Dev, import.meta.resolve(import.meta.url.endsWith(".ts") ? "./Local.ts" : "./Local.js", import.meta.url), Effect.gen(function* () {
    ...
    return {
        ...
        start: Effect.fn(function* ({ news: props, invalidate }) {
            ...
            const mirror = (sink) => child[sink].pipe(Stream.decodeText, redactor.stream, Stream.tap((text) => Effect.sync(() => process[sink].write(text))), Stream.tap((text) => Effect.sync(() => {
            ...
            const url = yield* Effect.raceAllFirst([
                Deferred.await(deferred).pipe(Effect.timeoutOrElse({
                    duration: "5 seconds",
                ...
                child.exitCode.pipe(Effect.mapError((error) => makeCommandError(props, error.reason)), Effect.flatMap((exitCode) => makeCommandError(props, new UnexpectedExit({ exitCode, stderr: buffer })))),
            ]);
            ...
            yield* child.exitCode.pipe(Effect.exit, Effect.flatMap(() => invalidate), Effect.forkScoped);
            return { url };
        }),
    };
}));
...
const LOCAL_URL_REGEX = /https?:\/\/(?:localhost|\[[0-9a-fA-F:]+\]|(?:\d{1,3}\.){3}\d{1,3})(?::\d+)?[^\s)\],"'`]*/;
...
const isLocalUrl = (url) => LOCAL_URL_REGEX.test(url);
