...
import { MigrationError, MigrationHistoryConflictError, } from "./Format.js";
import { classifyTable, tableColumns } from "./Introspect.js";
import { quoteIdentifier, sqlLiteral, timestampPrefixMillis, } from "./Records.js";
/**
 ...
 * back strings.
 */
export const toTimestampString = (value) => value === null || value === undefined
    ? undefined
    : value instanceof Date
        ? value.toISOString()
        : String(value);
const qualify = (table, dialect, schema) => schema
 ...
 */
export const findForeignHistory = (options) => Effect.gen(function* () {
    ...
    if (table !== "__drizzle_migrations") {
        ...
        if (classifyTable(columns) === "drizzle-shaped") {
            const rows = yield* executor.query(`SELECT hash, created_at, name, applied_at FROM ${qualify("__drizzle_migrations", dialect, drizzleSchema)} ORDER BY id;`);
            return {
                tool: "drizzle",
                ...
                rows: rows
                    ...
                    hash: row.hash === null || row.hash === undefined
                        ? undefined
                        : String(row.hash),
                    createdAtMillis: row.created_at === null || row.created_at === undefined
                        ? undefined
    ...
    if (table !== "_prisma_migrations") {
        ...
        if (names.has("migration_name") && names.has("checksum")) {
            const rows = yield* executor.query(`SELECT migration_name, checksum, started_at, finished_at, rolled_back_at FROM ${quoteIdentifier("_prisma_migrations", dialect)};`);
            const failed = rows.filter((row) => (row.finished_at === null || row.finished_at === undefined) &&
                (row.rolled_back_at === null || row.rolled_back_at === undefined));
            if (failed.length > 0) {
                return yield* new MigrationError({
                    message: `_prisma_migrations records ${failed.length} failed migration(s) ` +
            ...
            return {
                ...
                rows: rows
                    ...
                    hash: row.checksum === null || row.checksum === undefined
                        ? undefined
                        : String(row.checksum),
    ...
    if (dialect === "sqlite" && table !== "d1_migrations") {
        ...
        if (shape === "wrangler" ||
            ...
            const rows = yield* executor.query(`SELECT ${nameExpr} AS name, applied_at FROM ${quoteIdentifier("d1_migrations", dialect)} ORDER BY id;`);
            return {
                tool: shape === "wrangler"
                    ? "wrangler"
                    : "legacy-alchemy",
 ...
 */
export const matchForeignRows = (options) => Effect.gen(function* () {
    const { history, records } = options;
    const byName = new Map(records.map((r) => [r.name, r]));
    const byAlias = new Map(records.flatMap((r) => [
        [`${r.name}/migration.sql`, r],
        [r.name.replace(/\/migration\.sql$/, ""), r],
    ]));
    const matched = [];
    const unmatched = [];
    ...
    if (unmatched.length > 0) {
        return yield* new MigrationHistoryConflictError({
            ...
            unmatched,
            message: `While migrating ${history.tool} bookkeeping (${history.source}) to ` +
...
export const convertedRowInsertSql = (table, dialect, row) => `INSERT INTO ${quoteIdentifier(table, dialect)} (hash, created_at, name, applied_at) VALUES (${sqlLiteral(row.hash ?? "")}, ${sqlLiteral(row.createdAtMillis ?? null)}, ${sqlLiteral(row.name)}, ${sqlLiteral(row.appliedAt ?? null)});`;
