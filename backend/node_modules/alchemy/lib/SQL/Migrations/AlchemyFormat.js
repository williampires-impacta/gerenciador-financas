...
import { convertedRowInsertSql, findForeignHistory, matchForeignRows, toTimestampString, } from "./Convert.js";
 ...
 */
const createTableSql = (table, dialect) => {
...
const insertSql = (table, dialect, record) => {
    const quoted = quoteIdentifier(table, dialect);
...
const renameSql = (from, to, dialect) => dialect === "mysql"
 ...
 */
const rebuildInPlace = (options) => Effect.gen(function* () {
    const { executor, table, records, nameExpr } = options;
    const dialect = executor.dialect;
    const quoted = quoteIdentifier(table, dialect);
    const rows = yield* executor.query(`SELECT ${nameExpr} AS name, applied_at FROM ${quoted} ORDER BY id;`);
...
const ensureTable = (options) => Effect.gen(function* () {
    ...
    switch (shape) {
        ...
        case "unknown":
            return yield* new MigrationError({
                message: `Migrations table "${table}" has an unrecognized column layout; ` +
...
const appliedNames = (executor, table) => executor
    ...
    .pipe(Effect.map((rows) => new Set(rows
    .map((row) => row.name)
