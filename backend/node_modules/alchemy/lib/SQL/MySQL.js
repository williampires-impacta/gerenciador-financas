...
const isWorkerd = () => globalThis.navigator
...
const parseMySQLUrl = (url) => Effect.try({
    try: () => {
 ...
 * detected values.
 */
export const resolveMySQLConfig = (config) => Effect.gen(function* () {
 ...
 * @binding
 */
export const MySQL = (config) => Effect.map(makeExecutionMemo(Effect.gen(function* () {
 ...
 */
export const MySQLLayer = (config) => 
