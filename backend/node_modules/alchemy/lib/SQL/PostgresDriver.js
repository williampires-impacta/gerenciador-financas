 ...
 */
export const importPg = () => import("pg")
    .then((mod) => mod.default?.Pool !== undefined
    ? mod.default
    : mod)
    .catch((cause) => {
    throw new Error("Failed to load the 'pg' driver. Install the optional peer dependency 'pg' to connect to Postgres.", { cause });
});
/**
 ...
 * connection.
 */
export const openPostgresPool = (url, config) => Effect.gen(function* () {
    const pg = yield* Effect.promise(importPg);
    const connectionString = Redacted.value(yield* url);
    return yield* Effect.acquireRelease(Effect.sync(() => new pg.Pool({ connectionString, max: 1, ...config })), (pool) => Effect.promise(() => pool.end()));
});
