 ...
 */
export const listSqlFiles = (directory) => Effect.gen(function* () {
 ...
 * resource state.
 */
export const readSqlFile = (directory, name) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const sql = yield* fs.readFileString(path.resolve(directory, name));
    const hash = crypto.createHash("sha256").update(sql).digest("hex");
    const file = { id: name, sql, hash };
    Object.defineProperty(file, "sql", { enumerable: false });
    return file;
});
export const hashMigrations = (migrationsDir) => listSqlFiles(migrationsDir).pipe(Effect.map((files) => {
    const hashes = {};
    for (const file of files)
        hashes[file.id] = file.hash;
    return hashes;
}));
export const hashImports = (importFiles, rootDir) => Effect.gen(function* () {
    const hashes = {};
 ...
 * send files whole.
 */
export const splitSqlStatements = (sql) => sql
