 ...
 * @binding
 */
export const Postgres = (config) => Effect.map(makeExecutionMemo(Effect.gen(function* () {
    const { url, ...pool } = config;
    const resolved = Effect.isEffect(url) ? yield* url : url;
    const pgCtx = yield* Layer.build(PgClient.layer({ ...pool, url: resolved }));
 ...
 */
export const PostgresLayer = (config) => 
