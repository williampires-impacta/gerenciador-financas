...
import { findCwdForBundle, resolveMainPath } from "../Bundle/TempRoot.js";
import { Self } from "../Self.js";
import { createHostRuntimeContext, } from "../Server/Process.js";
import { zipCode } from "../Util/zip.js";
import { waitForAction } from "./actions.js";
import { openSshClient } from "./Ssh.js";
...
export const renderEnvFile = (env) => Object.entries(env)
...
`;
export const createHetznerHostedSupport = ({ stackName, stage, virtualEntryPlugin, }) => {
    const alchemyEnv = {
        ALCHEMY_STACK_NAME: stackName,
        ALCHEMY_STAGE: stage,
        ALCHEMY_PHASE: "runtime",
    };
    const bundleProgram = Effect.fn(function* (_id, props) {
        ...
        const bundleOutput = props.isExternal
            ...
            : yield* buildBundle(realMain, virtualEntryPlugin(makeBunBootstrap(handler)));
        ...
        const archive = yield* zipCode(toBytes(entryFile.content), chunkFiles.map((file) => ({
            path: file.path,
            content: toBytes(file.content),
        })));
        return { archive, hash: bundleOutput.hash };
    });
    const renderUnit = (unitName, appDir) => `[Unit]
...
`;
    const waitForSsh = (ssh) => ssh.exec("true").pipe(Effect.retry({
    ...
    const attachAndMount = Effect.fn(function* (input) {
        for (const { volumeId, path } of input.volumes) {
            let volume = yield* Services.volumes.getVolume({ id: volumeId }).pipe(Effect.map(({ volume }) => volume), Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
            if (volume === undefined)
                continue;
            if (volume.server !== input.serverId) {
                if (volume.server !== null) {
                    yield* Services.volumeActions.detachVolume({ id: volumeId }).pipe(Effect.tap(({ action }) => waitForAction(action).pipe(Effect.catchTag("ActionTimeout", () => Effect.void))), Effect.catchTag(["NotFound", "UnprocessableEntity", "Locked", "Conflict"], () => Effect.void));
                }
                yield* Services.volumeActions
                    ...
                    .pipe(Effect.tap(({ action }) => waitForAction(action).pipe(Effect.catchTag("ActionTimeout", () => Effect.void))), Effect.catchTag(["UnprocessableEntity", "Locked", "Conflict"], () => Effect.void));
                volume = yield* Services.volumes.getVolume({ id: volumeId }).pipe(Effect.flatMap(({ volume }) => volume.server === input.serverId
                    ? Effect.succeed(volume)
                    : Effect.fail({ _tag: "AttachPending" })), Effect.retry({
                    while: (e) => e._tag === "AttachPending" ||
                ...
                }), Effect.catchIf((e) => e._tag === "AttachPending", () => Services.volumes.getVolume({ id: volumeId }).pipe(Effect.map(({ volume }) => volume), Effect.catchTag("NotFound", () => Effect.succeed(undefined)))), Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
            }
    ...
    const deployUnit = Effect.fn(function* (input) {
        ...
        yield* input.ssh.scp(new TextEncoder().encode(renderUnit(input.unitName, appDir)), `/etc/systemd/system/${input.unitName}.service`);
        yield* input.ssh.exec([
            `set -uo pipefail`,
