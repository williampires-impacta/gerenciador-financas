 ...
 * @resource
 */
export const FloatingIp = Resource("Hetzner.FloatingIp");
...
const createFloatingIpName = (id, name, existing) => Effect.gen(function* () {
...
const getByName = (name) => Services.floatingIps
    .listFloatingIps({ name, per_page: 50 })
    .pipe(Effect.map(({ floating_ips }) => floating_ips[0]));
const getByLabels = (labels) => Services.floatingIps
    .listFloatingIps({
...
const sameHomeLocation = (desired, attrs) => desired === attrs.homeLocation ||
    desired === attrs.homeLocationId ||
    String(desired) === String(attrs.homeLocationId);
const matchesDesired = (ip, type, location) => asType(ip.type) === type &&
    sameHomeLocation(location.name, {
        homeLocation: ip.home_location.name,
        homeLocationId: ip.home_location.id,
    });
const disableProtection = (id) => Services.floatingIpActions
...
export const FloatingIpProvider = () => Provider.succeed(FloatingIp, {
