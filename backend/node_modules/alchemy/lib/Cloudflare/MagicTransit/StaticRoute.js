...
const TypeId = "Cloudflare.MagicTransit.StaticRoute";
/**
 ...
 * @category Network
 */
export const MagicStaticRoute = Resource(TypeId);
/**
 * Returns true if the given value is a MagicStaticRoute resource.
 */
export const isMagicStaticRoute = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
const DEFAULT_PRIORITY = 100;
export const MagicStaticRouteProvider = () => Provider.succeed(MagicStaticRoute, {
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        // on no-op.
        const dirty = observed.prefix !== news.prefix ||
    ...
    list: Effect.fn(function* () {
        ...
        return yield* magicTransit.listRoutes({ accountId }).pipe(Effect.map((response) => (response.routes ?? []).map((route) => toAttributes(route, accountId))), 
 ...
 */
const getRoute = (accountId, routeId) => magicTransit.getRoute({ accountId, routeId }).pipe(Effect.map((r) => r.route ?? undefined), Effect.catchTag("RouteNotFound", () => Effect.succeed(undefined)));
/**
 * Find a route by its (prefix, nexthop, priority) identity triple.
 */
const findRoute = (accountId, prefix, nexthop, priority) => magicTransit
    .listRoutes({ accountId })
    .pipe(Effect.map((r) => (r.routes ?? []).find((route) => route.prefix === prefix &&
    route.nexthop === nexthop &&
    route.priority === priority)));
const sameList = (a, b) => [...(a ?? [])].sort().join(",") === [...(b ?? [])].sort().join(",");
const scopeDirty = (observed, desired) => {
    if (desired === undefined)
        return false;
    return (!sameList(observed?.coloNames, desired.coloNames) ||
