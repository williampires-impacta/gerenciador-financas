...
const TypeId = "Cloudflare.Rum.Site";
/**
 ...
 */
export const Site = Resource(TypeId);
/**
 * Returns true if the given value is a Site resource.
 */
export const isSite = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const SiteProvider = () => Provider.succeed(Site, {
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (!observed) {
            ...
            observed = yield* rum.getSiteInfo({ accountId, siteId: siteTag }).pipe(Effect.retry({
        ...
        const dirty = (news.host !== undefined && observed.host !== news.host) ||
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* rum.listSiteInfos.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((site) => toAttributes(site, accountId)))));
    }),
});
/**
 ...
 */
const getSite = (accountId, siteTag) => rum.getSiteInfo({ accountId, siteId: siteTag }).pipe(Effect.map((site) => site), Effect.catchTag("SiteNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findSite = (accountId, host, zoneTag) => rum.listSiteInfos.items({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    .filter((site) => host !== undefined
    ? site.host === host
