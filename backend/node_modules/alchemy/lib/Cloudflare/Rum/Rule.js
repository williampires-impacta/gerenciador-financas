...
const TypeId = "Cloudflare.Rum.Rule";
/**
...
export const RuleProvider = () => Provider.succeed(Rule, {
    ...
    list: Effect.fn(function* () {
        ...
        const sites = yield* rum.listSiteInfos.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.result ?? [])));
        const rulesetIds = Array.from(new Set(sites
            .map((site) => site.ruleset?.id ?? undefined)
            .filter((id) => id !== undefined)));
        const rows = yield* Effect.forEach(rulesetIds, (rulesetId) => listRules(accountId, rulesetId).pipe(Effect.map((rules) => (rules ?? []).map((rule) => toAttributes(rule, rulesetId, accountId))), 
    ...
    read: Effect.fn(function* ({ output, olds }) {
        ...
        const rulesetId = output?.rulesetId ??
        ...
        if (olds !== undefined && isResolved(olds)) {
            const match = rules
                .filter((rule) => (rule.host ?? undefined) === olds.host &&
                samePaths(rule.paths ?? undefined, olds.paths))
                .sort((a, b) => (a.created ?? "").localeCompare(b.created ?? ""))
                .at(0);
            if (match)
                return Unowned(toAttributes(match, rulesetId, acct));
        }
        return undefined;
    }),
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const dirty = (observed.host ?? undefined) !== desired.host ||
        ...
        return toAttributes({ ...toRule(updated), priority: updated.priority ?? observed.priority }, rulesetId, accountId);
    }),
 ...
 */
const listRules = (accountId, rulesetId) => rum.listRules({ accountId, rulesetId }).pipe(Effect.map((response) => response.rules ?? []), Effect.catchTag("RulesetNotFound", () => Effect.succeed(undefined)));
const toRule = (rule) => rule;
const samePaths = (observed, desired) => (observed ?? []).length === (desired ?? []).length &&
    (observed ?? []).join(" ") === (desired ?? []).join(" ");
const toAttributes = (rule, rulesetId, accountId) => ({
    id: rule.id ?? "",
    rulesetId,
    accountId,
