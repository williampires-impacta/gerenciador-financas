...
export const isBucket = (value) => isResourceOfType(value, "Cloudflare.R2.Bucket");
/**
 ...
 */
export const Bucket = Resource("Cloudflare.R2.Bucket", {
    aliases: ["Cloudflare.R2Bucket"],
});
export const ProviderLive = () => Provider.effect(Bucket, Effect.gen(function* () {
    const emptyBucket = Effect.fn(function* (bucketName, jurisdiction) {
        ...
        return yield* r2.listObjects
            ...
            .pipe(Stream.filter((o) => typeof o.key === "string" && o.key !== ""), Stream.map((o) => o.key), Stream.runForEachArray((chunk) => r2.deleteObjects({
            accountId,
            bucketName,
            cfR2Jurisdiction: jurisdiction,
            body: [...chunk],
    ...
    const createBucketName = (id, name) => Effect.gen(function* () {
        if (name)
            return name;
    ...
    const listCustomDomains = Effect.fn(function* (bucketName, jurisdiction, 
    ...
    options) {
        ...
        return yield* (options?.retryMissing === false
            ? fetch
            : fetch.pipe(Effect.retry({
                while: (e) => e._tag === "NoSuchBucket",
                schedule: r2BucketEndpointConsistencySchedule,
            }))).pipe(Effect.map((response) => response.domains.map(toCustomDomainAttributes)), Effect.catchTag("NoSuchBucket", () => Effect.succeed(undefined)));
    });
    const reconcileCustomDomains = (bucketName, jurisdiction, desired, previous) => Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        const observed = yield* listCustomDomains(bucketName, jurisdiction);
        if (!observed) {
            return yield* Effect.fail(new Error(`Cannot reconcile custom domains for missing R2 bucket "${bucketName}"`));
        }
        ...
        yield* Effect.forEach(previous, (previousDomain) => desiredDomains.has(previousDomain.domain)
            ? Effect.void
            : r2
                ...
                .pipe(Effect.catchTag(["DomainNotFound", "NoSuchBucket"], () => Effect.void)), { concurrency: "unbounded" });
        const applied = yield* Effect.forEach(desired, (domain) => Effect.gen(function* () {
    ...
    const listBucketsInJurisdiction = (accountId, jurisdiction) => Effect.gen(function* () {
        const all = [];
        ...
        for (;;) {
            ...
            const page = raw.filter((b) => typeof b.name === "string" && b.name !== "");
            for (const b of page) {
                all.push({
                    name: b.name,
            ...
            if (raw.length < perPage || page.length === 0)
                break;
            startAfter = page[page.length - 1].name;
        }
        return all;
    });
    const reconcileLifecycleRules = (bucketName, jurisdiction, desired) => Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
    ...
    const reconcileCorsRules = (bucketName, jurisdiction, desired) => Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        const observed = yield* r2
            .getBucketCors({
            accountId,
            ...
            .pipe(Effect.map((response) => (response.rules ?? []).map(toCorsRule)), 
        // A bucket with no CORS configuration is a typed error, not an
        // empty rule list — normalize it to [] for the diff below.
        Effect.catchTag("NoCorsConfiguration", () => Effect.succeed([])), Effect.retry({
            while: (e) => e._tag === "NoSuchBucket",
            schedule: r2BucketEndpointConsistencySchedule,
        }));
    ...
    return {
        ...
        list: () => Effect.gen(function* () {
            ...
            const perJurisdiction = yield* Effect.forEach(jurisdictions, (jurisdiction) => listBucketsInJurisdiction(accountId, jurisdiction).pipe(
            ...
            // @ts-expect-error
            Effect.catchTag(["InvalidRoute", "Forbidden"], () => Effect.succeed([]))), { concurrency: jurisdictions.length });
            ...
            return yield* Effect.forEach(buckets, (bucket) => Effect.gen(function* () {
                ...
                const [domains, lifecycleRules, cors] = yield* Effect.all([
                    ...
                    r2
                        ...
                        .pipe(Effect.map((observed) => (observed.rules ?? []).map(toLifecycleRule)), Effect.catchTag("NoSuchBucket", () => Effect.succeed([]))),
                    r2
                        .getBucketCors({
                        accountId,
                        bucketName: bucket.name,
                        ...
                        .pipe(Effect.map((observed) => (observed.rules ?? []).map(toCorsRule)), Effect.catchTag(["NoSuchBucket", "NoCorsConfiguration"], () => Effect.succeed([]))),
                ], { concurrency: 3 });
                return {
                    bucketName: bucket.name,
            ...
            // bucket down.
            Effect.catchTag("InternalServerError", () => Effect.succeed({
                bucketName: bucket.name,
                storageClass: bucket.storageClass,
                jurisdiction: bucket.jurisdiction,
                location: bucket.location,
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output }) {
            ...
            if (!observed) {
                observed = yield* r2
                    ...
                    .pipe(Effect.catchTag("BucketAlreadyExists", () => r2
                    .getBucket({
                    accountId: acct,
                    bucketName: name,
                    jurisdiction: news.jurisdiction,
            ...
            const domains = yield* reconcileCustomDomains(attrs.bucketName, attrs.jurisdiction, news.domains ?? [], output?.domains ?? []);
            const lifecycleRules = yield* reconcileLifecycleRules(attrs.bucketName, attrs.jurisdiction, news.lifecycleRules ?? []);
            const cors = yield* reconcileCorsRules(attrs.bucketName, attrs.jurisdiction, news.cors ?? []);
            return {
                ...attrs,
                domains,
                lifecycleRules,
        ...
        delete: Effect.fn(function* ({ olds = {}, output, force }) {
            yield* Effect.all((output.domains ?? []).map((domain) => r2
                .deleteBucketDomainCustom({
 ...
 */
export const ProviderLocal = () => Provider.succeed(Bucket, {
    stables: ["accountId"],
    ...
    reconcile: Effect.fn(function* ({ news = {}, output }) {
        ...
        return {
            ...
            storageClass: (news.storageClass ?? "Standard"),
...
const toCustomDomainAttributes = (domain) => ({
    domain: domain.domain,
...
const sameCustomDomainConfig = (observed, desired, zoneId) => observed !== undefined &&
...
const normalizeLifecycleRule = (rule) => ({
    id: rule.id,
...
const toLifecyclePutPayload = (rule) => ({
    id: rule.id,
    enabled: rule.enabled ?? true,
