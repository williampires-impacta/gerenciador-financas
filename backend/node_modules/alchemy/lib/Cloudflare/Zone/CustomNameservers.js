...
const TypeId = "Cloudflare.Zone.CustomNameservers";
/**
 ...
 */
export const CustomNameservers = Resource(TypeId);
/**
 * Returns true if the given value is a CustomNameservers resource.
 */
export const isCustomNameservers = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const CustomNameserversProvider = () => Provider.succeed(CustomNameservers, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => zones.getCustomNameserver({ zoneId }).pipe(Effect.map((observed) => {
            ...
            if (!(observed.enabled ?? false))
                return undefined;
            return toAttributes(zoneId, observed, observed.enabled ?? false, observed.nsSet ?? undefined);
        }), 
        ...
        Effect.catchTag(["InvalidZoneIdentifier", "Forbidden"], () => Effect.succeed(undefined))), { concurrency: 10 });
    ...
    read: Effect.fn(function* ({ output, olds }) {
        ...
        const initialEnabled = output !== undefined
            ? output.initialEnabled
            : (observed.enabled ?? false);
        const initialNsSet = output !== undefined
            ? output.initialNsSet
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const inSync = observedEnabled === news.enabled &&
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        const observedEnabled = observed.enabled ?? false;
        const observedNsSet = observed.nsSet ?? undefined;
        if (observedEnabled === initialEnabled &&
        ...
        // restore.
        Effect.catchTag(["InvalidZoneIdentifier", "CustomNameserverSetNotFound"], () => Effect.void));
    }),
});
const toAttributes = (zoneId, observed, initialEnabled, initialNsSet) => ({
    zoneId,
...
//# sourceMappingURL=CustomNameservers.js.map