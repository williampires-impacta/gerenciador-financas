...
const TypeId = "Cloudflare.Pipelines.Pipeline";
/**
 ...
 */
export const Pipeline = Resource(TypeId);
/**
 * Returns true if the given value is a Pipeline resource.
 */
export const isPipeline = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const PipelineProvider = () => Provider.succeed(Pipeline, {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (observed && (observed.sql !== sql || observed.name !== name)) {
            ...
            yield* getPipeline(accountId, observed.id).pipe(Effect.repeat({
        ...
        if (!observed) {
            ...
            }), Effect.catchTag("PipelineAlreadyExists", (error) => findPipelineByName(accountId, name).pipe(Effect.flatMap((match) => match ? Effect.succeed(match) : Effect.fail(error)))));
        }
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* pipelines.listV1Pipeline.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((p) => toAttributes(p, accountId)))));
    }),
});
// Pipelines entity names must be alphanumeric/underscore only, so swap
// the default hyphen delimiter for underscores.
const pipelineName = (id, name) => Effect.gen(function* () {
    if (name)
        return name;
 ...
 */
const getPipeline = (accountId, pipelineId) => pipelines.getV1Pipeline({ accountId, pipelineId }).pipe(Effect.map((p) => p), Effect.catchTag("PipelineNotExists", () => Effect.succeed(undefined)));
/**
 * Idempotent delete — a pipeline that is already gone is success.
 */
const deletePipeline = (accountId, pipelineId) => pipelines
    .deleteV1Pipeline({ accountId, pipelineId })
    .pipe(Effect.catchTag("PipelineNotExists", () => Effect.void));
const findPipelineByName = (accountId, name) => pipelines.listV1Pipeline.items({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((p) => p.name === name)));
