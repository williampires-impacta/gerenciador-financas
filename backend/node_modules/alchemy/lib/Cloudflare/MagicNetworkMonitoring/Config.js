...
const TypeId = "Cloudflare.MagicNetworkMonitoring.Config";
/**
 ...
 * @category Network
 */
export const Config = Resource(TypeId);
/**
 * Returns true if the given value is a Config resource.
 */
export const isConfig = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const ConfigProvider = () => Provider.succeed(Config, {
    ...
    reconcile: Effect.fn(function* ({ news }) {
        ...
        //    update below.
        if (observed === undefined) {
            const created = observe(yield* mnm
 ...
 */
const observe = (config) => {
    if (config === null)
        return undefined;
    const { name, defaultSampling } = config;
    if (name == null || defaultSampling == null)
        return undefined;
    return {
        name,
        defaultSampling,
...
const sameWarpDevices = (observed, desired) => observed.length === desired.length &&
    serializeWarpDevices(observed) === serializeWarpDevices(desired);
const serializeWarpDevices = (devices) => devices
