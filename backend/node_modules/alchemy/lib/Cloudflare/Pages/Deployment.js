...
const TypeId = "Cloudflare.Pages.Deployment";
/**
 ...
 */
export const Deployment = Resource(TypeId);
/**
 * Returns true if the given value is a Deployment resource.
 */
export const isDeployment = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const DeploymentProvider = () => Provider.succeed(Deployment, {
    stables: [
        "deploymentId",
        "shortId",
        "accountId",
    ...
    read: Effect.fn(function* ({ output }) {
        ...
        if (!output?.deploymentId)
            return undefined;
        const observed = yield* getDeployment(output.accountId, output.projectName, output.deploymentId);
        return observed
    ...
    list: Effect.fn(function* () {
        ...
        const projects = yield* pages.listProjects.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.result ?? [])), 
        // Account without a Pages entitlement can't list projects → nothing.
        Effect.catchTag("Forbidden", () => Effect.succeed([])));
        const rows = yield* Effect.forEach(projects, (project) => pages.listProjectDeployments
            .pages({ accountId, projectName: project.name })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((deployment) => toAttributes(deployment, accountId, project.name))))), { concurrency: 10 });
        return rows.flat();
    }),
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* pages
            ...
            .pipe(Effect.catchTag("DeploymentNotFound", () => Effect.void), Effect.catchTag("ProjectNotFound", () => Effect.void), Effect.catchTag("ActiveProductionDeployment", () => Effect.logWarning(`Pages deployment ${output.deploymentId} is the active production deployment of project ${output.projectName}; it will be deleted with the project.`)));
    }),
});
/**
 ...
 */
const getDeployment = (accountId, projectName, deploymentId) => pages.getProjectDeployment({ accountId, projectName, deploymentId }).pipe(Effect.catchTag("DeploymentNotFound", () => Effect.succeed(undefined)), Effect.catchTag("ProjectNotFound", () => Effect.succeed(undefined)));
const isTerminalStage = (stage) => (stage.name === "deploy" && stage.status === "success") ||
 ...
 */
const awaitDeployment = (accountId, projectName, created) => Effect.gen(function* () {
    const observed = isTerminalStage(created.latestStage)
        ? created
        : yield* pages
    ...
    if (observed.latestStage.status !== "success" ||
        observed.latestStage.name !== "deploy") {
        return yield* Effect.fail(new DeploymentFailed({
            projectName,
...
const toAttributes = (deployment, accountId, projectName) => ({
