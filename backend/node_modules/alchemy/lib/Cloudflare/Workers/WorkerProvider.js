...
import { detachQueueConsumersOfScript } from "../Queues/Consumer.js";
import { CloudflareLogs } from "../Logs.js";
import { resolveZoneId, } from "../Zone/lookup.js";
import { getAssetsPathPrefix, mergeAssetsConfigFiles, readAssets, readAssetsConfigFiles, uploadAssets, } from "./Assets.js";
import { getCompatibility } from "./Compatibility.js";
import { isDurableObjectExport } from "./DurableObject.js";
import { LocalWorkerProvider } from "./LocalWorkerProvider.js";
import { makeSourceContext, resolveSource } from "./Source.js";
import { isSelfUrl, Worker, } from "./Worker.js";
import { getCacheBinding, getCronBindings, isContainerDecl, } from "./WorkerAsyncBindings.js";
import { readPrebuiltWorkerBundle } from "./Sources/Prebuilt.js";
import { isPythonMain, readPythonWorkerBundle } from "./Sources/Python.js";
import { WorkerBundle } from "./Sources/Rolldown.js";
import { isWorkerLoader } from "./WorkerLoader.js";
import { createWorkerName } from "./WorkerName.js";
 ...
 */
export class DurableObjectTransferRequired extends Data.TaggedError("DurableObjectTransferRequired") {
    get message() {
        return (`Durable Object class '${this.className}' still lives on Worker '${this.scriptName}' but this deploy re-binds it as a cross-script reference` +
...
export class AmbiguousDurableObjectTransfer extends Data.TaggedError("AmbiguousDurableObjectTransfer") {
    get message() {
        return (`Durable Object '${this.logicalId}' (class '${this.className}') is new to Worker '${this.scriptName}' and multiple scripts match its transferredFrom declaration: ${this.sources.join(", ")}. ` +
 ...
 * @internal
 */
export const resolveNamespaceName = (namespace) => {
    if (namespace == null)
        return undefined;
 ...
 * @internal
 */
export const resolveTailConsumers = (tailConsumers) => {
    if (tailConsumers == null)
        return undefined;
    return tailConsumers.flatMap((consumer) => {
        const service = typeof consumer === "string"
            ? consumer
            : consumer.workerName;
 ...
 * @internal
 */
export const resolveVersionParentName = (version) => {
    const parent = version?.parent;
    if (parent == null)
        return undefined;
    if (typeof parent === "string")
        return parent;
    const workerName = parent.workerName;
 ...
 * @internal
 */
const getSelfRolloutTraffic = (news) => {
    if (!news.version || news.version.parent != null)
        return undefined;
    const traffic = news.version.traffic;
    return traffic === undefined || traffic >= 100 ? undefined : traffic;
};
const validateTraffic = (traffic) => traffic !== undefined &&
    (!Number.isFinite(traffic) || traffic < 0 || traffic > 100)
    ? Effect.fail(new WorkerVersionConfigError({
 ...
 */
export const resolveVersionAffinity = (affinity) => Effect.gen(function* () {
    const declared = [
    ...
    if (declared.length > 1) {
        return yield* Effect.fail(new WorkerVersionConfigError({
    ...
    if (declared.length === 0 && affinity.ip !== true) {
        return yield* Effect.fail(new WorkerVersionConfigError({
            message: "version.affinity requires a key source: set `cookie`, `header`, `key`, or `ip: true`.",
        }));
    }
    if (affinity.key !== undefined && affinity.ip === true) {
        return yield* Effect.fail(new WorkerVersionConfigError({
            message: "version.affinity: `ip` is the fallback for an absent `cookie`/`header` — a raw `key` expression has no absence condition to fall back from. Fold `ip.src` into the expression instead.",
        }));
    }
    ...
    const source = affinity.cookie !== undefined
 ...
 * @internal exported for unit testing.
 */
export const affinityHostExpression = (hosts) => {
    const dedupe = (values) => [...new Set(values)].sort();
...
const affinityRulePrefix = (scriptName) => `alchemy:worker:${scriptName}:affinity`;
/**
 ...
 * @internal exported for unit testing.
 */
export const buildAffinityZoneRules = (scriptName, affinity, hosts) => {
    ...
    const field = source.kind === "cookie"
 ...
 * @internal exported for unit testing.
 */
export const resolveWorkersDev = (workersDev) => {
...
export const resolveWorkerDomainZone = (config) => {
    if (config === undefined)
        return undefined;
    if (typeof config.zoneId === "string")
        return config.zoneId;
    if (isZoneReference(config.zone))
        return config.zone;
    if (typeof config.zoneName === "string")
        return config.zoneName;
    return undefined;
};
/** Whether an existing attachment must move to satisfy an explicit zone pin. */
export const shouldRecreateWorkerDomainAttachment = (liveZoneId, desiredZoneId) => desiredZoneId !== undefined && liveZoneId !== desiredZoneId;
...
domain) => Effect.gen(function* () {
    ...
    const config = typeof domain === "string"
        ? { name: domain }
    ...
    if (overlap.length > 0) {
        return yield* Effect.fail(new WorkerDomainConfigError({
...
const isWorkersDevHostname = (hostname) => hostname.endsWith(".workers.dev");
// Hostnames that only appear in local-dev state (the dev server's
// localhost/LAN URLs), never as attachable custom domains.
const isLocalDevHostname = (hostname) => hostname === "localhost" ||
    hostname === "::1" ||
 ...
 * @internal exported for unit testing.
 */
export const stateCustomDomains = (domains) => normalizeStateDomains(domains).filter((hostname) => !isWorkersDevHostname(hostname) && !isLocalDevHostname(hostname));
/**
 ...
 * @internal exported for unit testing.
 */
export const stateWorkerDomain = (output) => {
    const state = output;
    ...
    if (domain && typeof domain.name === "string") {
        ...
        return {
            name: domain.name,
            aliases: (domain.aliases ?? []).filter((h) => typeof h === "string"),
            redirects: (domain.redirects ?? []).filter((h) => typeof h === "string"),
            ...(zone === undefined ? {} : { zone }),
        };
    }
 ...
 * @internal
 */
const getScriptSettings = (accountId, scriptName, dispatchNamespace) => 
 ...
 * exhausted.
 */
const isBindingTargetNotFound = (e) => e._tag === "SecretsStoreBindingNotFound" ||
...
const bindingTargetNotFoundRetrySchedule = () => Schedule.max([Schedule.fixed("2 seconds"), Schedule.recurs(10)]);
/**
 ...
 */
const isScriptPutTransportError = (e) => isHttpClientError(e) && e.reason._tag === "TransportError";
const retryableScriptPut = (e) => isBindingTargetNotFound(e) || isScriptPutTransportError(e);
/**
 ...
 */
const cachedWorkerId = (value, scriptName) => value !== undefined &&
    value !== "" &&
    value !== scriptName &&
 ...
 */
const findWorkerId = (accountId, scriptName) => workers.listScripts.items({ accountId }).pipe(Stream.filter((script) => script.id === scriptName), Stream.runHead, Effect.map(Option.getOrUndefined), Effect.flatMap((script) => script?.tag != null
    ? Effect.succeed(script.tag)
    : Effect.fail(new WorkerIdNotFound({
        scriptName,
...
const putWorkerScript = (params) => Effect.gen(function* () {
 ...
 * @internal
 */
const deleteWorkerScript = (accountId, scriptName, dispatchNamespace) => Effect.gen(function* () {
    if (dispatchNamespace) {
    ...
    Effect.catchTag("QueueConsumerConflict", () => detachQueueConsumersOfScript(accountId, scriptName).pipe(Effect.andThen(workers.deleteScript({ accountId, scriptName, force: true })))));
});
/**
 ...
 * @internal exported for unit testing.
 */
export const normalizeStateDomains = (domains) => (domains ?? []).flatMap((u) => {
    if (typeof u === "string") {
        if (u.includes("://")) {
            try {
 ...
 * @internal exported for unit testing.
 */
export const shouldObserveWorkerDomains = (olds, output) => olds?.domain !== undefined || stateWorkerDomain(output) !== undefined;
/**
 ...
 * @internal exported for unit testing.
 */
export const shouldObserveWorkerRoutes = (olds, output) => olds?.routes !== undefined || (output?.routes?.length ?? 0) > 0;
/**
 ...
 * @internal exported for unit testing.
 */
export const shouldObserveWorkerCrons = (olds, output) => olds?.crons !== undefined || (output?.crons?.length ?? 0) > 0;
/**
 ...
 */
const CLOUDFLARE_WORKERS_SUBDOMAIN = Config.string("CLOUDFLARE_WORKERS_SUBDOMAIN").pipe(Config.map((value) => value.trim()), Config.option);
 ...
 */
const resolveMetadataHashValue = (value) => Effect.gen(function* () {
    ...
    if (Array.isArray(resolved)) {
        return yield* Effect.all(resolved.map((item) => resolveMetadataHashValue(item)), { concurrency: "unbounded" });
    }
    if (Predicate.isObject(resolved)) {
        ...
        const entries = yield* Effect.all(Object.entries(resolved).map(([key, nested]) => resolveMetadataHashValue(nested).pipe(Effect.map((materializedNested) => [key, materializedNested]))), { concurrency: "unbounded" });
 ...
 */
const resolveWorkerMetadataHash = ({ props, bindings, accountId, stack, selfUrl, }) => resolveMetadataHashValue({
    accountId,
    selfUrl,
...
export const WorkerProvider = () => ProviderLayer.dual(Worker, {
...
export const LiveWorkerProvider = () => Provider.effect(Worker, Effect.gen(function* () {
    ...
    const getAccountSubdomain = yield* cachedFunction((accountId) => Effect.gen(function* () {
        ...
        if (Option.isSome(configured) && configured.value !== "") {
            if (!WORKERS_SUBDOMAIN_PATTERN.test(configured.value)) {
                return yield* Effect.die(new Error(`Invalid CLOUDFLARE_WORKERS_SUBDOMAIN "${configured.value}": ` +
    ...
    const setWorkerSubdomain = Effect.fn(function* (name, desired) {
    ...
    const normalizeCrons = (crons) => Array.from(new Set(crons ?? []));
    const hasSelfUrlBinding = (bindings) => bindings.some((b) => (b.data.bindings ?? []).some((item) => item.type === "self_url"));
    ...
    const resolveSelfUrl = Effect.fn(function* (name, props, accountId) {
        const domain = yield* resolveWorkerDomain(props.domain);
        if (domain) {
            return `https://${domain.name}`;
        }
        if (!resolveWorkersDev(props.workersDev).enabled) {
            return yield* Effect.die(`Worker "${name}" binds its own URL (Worker.URL) but has none: the stable workers.dev URL is disabled (workersDev) and no custom domain is configured.`);
        }
        return `https://${name}.${yield* getAccountSubdomain(accountId)}.workers.dev`;
    });
    const getWorkerCrons = Effect.fn(function* (scriptName) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* workers
            .getScriptSchedule({
            accountId,
            ...
            .pipe(Effect.map((response) => normalizeCrons(response.schedules.map((schedule) => schedule.cron))), Effect.catchTag("WorkerNotFound", () => Effect.succeed([])));
    });
    const reconcileCrons = (scriptName, desired, previous, session) => Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        const live = yield* getWorkerCrons(scriptName);
        const desiredSorted = [...desired].sort();
        const liveSorted = [...live].sort();
        const changed = desiredSorted.length !== liveSorted.length ||
     ...
     */
    const inferZoneIdForHostname = (hostname, zoneCache, zone) => Effect.gen(function* () {
        const cacheKey = zone === undefined
            ? hostname
    ...
    const reconcileDomains = (scriptName, desired, zone) => Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        const attachDomain = Effect.fn(function* (hostname) {
            const live = liveByHostname.get(hostname);
            const desiredZoneId = zone === undefined
                ? undefined
            ...
            const otherOwner = yield* workers
                ...
                .pipe(Effect.map((r) => (r.result ?? []).find((d) => d.hostname === hostname && d.service !== scriptName)), Effect.catch(() => Effect.succeed(undefined)));
            if (otherOwner?.id) {
                return yield* Effect.die(new Error(`Cannot attach hostname '${hostname}' to Worker '${scriptName}': ` +
                    `it is already attached to Worker '${otherOwner.service ?? "<unknown>"}'. ` +
                    `Detach it from that Worker first, or pick a different hostname.`));
            }
            const zoneId = desiredZoneId ??
    ...
    const getPreviewUrl = Effect.fn(function* (scriptName, accountSubdomain) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        const deployments = yield* workers
            .listScriptDeployments({ accountId, scriptName })
            .pipe(Effect.map((response) => response.deployments ?? []), Effect.catch(() => Effect.succeed([])));
        const latest = [...deployments].sort((a, b) => b.createdOn.localeCompare(a.createdOn))[0];
        const version = latest?.versions?.reduce((max, candidate) => max === undefined || candidate.percentage > max.percentage
            ? candidate
            : max, undefined);
    ...
    const computeWorkerUrls = Effect.fn(function* (params) {
        ...
        const urls = [];
        if (domain) {
            urls.push(`https://${domain.name}`, ...domain.aliases.map((hostname) => `https://${hostname}`));
        }
            ...
            if (workersDev.previewsEnabled) {
                if (params.uploadedVersionId !== undefined) {
                    if (params.uploadedVersionAlias !== undefined) {
                        urls.push(`https://${params.uploadedVersionAlias}-${scriptName}.${accountSubdomain}.workers.dev`);
                    }
                    urls.push(`https://${params.uploadedVersionId.split("-")[0]}-${scriptName}.${accountSubdomain}.workers.dev`);
                }
    ...
    const reconcileRedirectRules = Effect.fn(function* (params) {
        ...
        for (const zoneId of zoneIds) {
            ...
            const isOurs = (rule) => (rule.description ?? "").startsWith(prefix);
            const ourExisting = existingRules.filter(isOurs);
            const converged = ourExisting.length === ourDesired.length &&
                ourDesired.every((rule) => ourExisting.some((existing) => existing.description === rule.description &&
            ...
            const foreign = existingRules
                ...
                .map((rule) => {
                const { lastUpdated: _lastUpdated, version: _version, ...rest } = rule;
                return rest;
            });
            yield* rulesets.putPhasForZone({
                zoneId,
                rulesetPhase: "http_request_dynamic_redirect",
                rules: [...foreign, ...ourDesired],
            });
        }
    });
    /**
    ...
    const reconcileAffinityRules = Effect.fn(function* (params) {
        ...
        for (const zoneId of zoneIds) {
            ...
            const ourDesired = params.desired !== undefined && hosts !== undefined
            ...
            const existingValue = (rule) => {
                const headers = "actionParameters" in rule
                    ? rule.actionParameters?.headers
                    : undefined;
                const header = headers?.[AFFINITY_HEADER];
    ...
    const routeKey = (route) => `${route.zoneId}:${route.pattern}`;
    ...
    const hostnameFromPattern = (pattern) => {
    ...
    const normalizeRoutes = (routes) => Effect.gen(function* () {
        if (!routes?.length)
            return [];
        const { accountId } = yield* yield* CloudflareEnvironment;
        const zoneCache = new Map();
    ...
    const listWorkerRoutesInZones = (scriptName, zoneIds) => {
        const uniqueZoneIds = Array.from(new Set(zoneIds));
        if (uniqueZoneIds.length === 0) {
            return Effect.succeed([]);
        }
        const routesByZone = Effect.all(uniqueZoneIds.map((zoneId) => workers.listRoutes({ zoneId }).pipe(Effect.map((response) => (response.result ?? []).flatMap((route) => route.id && route.pattern && route.script === scriptName
    ...
    const readWorkerRoutes = (scriptName, knownRoutes) => listWorkerRoutesInZones(scriptName, (knownRoutes ?? []).map((route) => route.zoneId));
    ...
    const reconcileRoutes = (scriptName, desired, previous) => Effect.gen(function* () {
        ...
        yield* Effect.all(toRemove.map((route) => workers
            .deleteRoute({ zoneId: route.zoneId, routeId: route.id })
            .pipe(Effect.catchTag("RouteNotFound", () => Effect.void))), { concurrency: "unbounded" });
        if (desired.length === 0)
            return [];
        const attachRoute = Effect.fn(function* (route) {
            const existing = liveByKey.get(routeKey(route));
            if (existing)
                return existing;
            const zoneRoutes = yield* workers
                .listRoutes({ zoneId: route.zoneId })
                .pipe(Effect.map((response) => response.result ?? []), Effect.catch(() => Effect.succeed([])));
            const otherOwner = zoneRoutes.find((candidate) => candidate.pattern === route.pattern &&
                candidate.script &&
                candidate.script !== scriptName);
            if (otherOwner) {
                return yield* Effect.die(new Error(`Cannot attach route '${route.pattern}' to Worker '${scriptName}': ` +
            ...
            }), Effect.catchTag("InvalidRoute", (originalError) => Effect.gen(function* () {
                const match = yield* workers
                    .listRoutes({ zoneId: route.zoneId })
                    .pipe(Effect.map((response) => (response.result ?? []).find((candidate) => candidate.pattern === route.pattern &&
                    candidate.script === scriptName)), Effect.catch(() => Effect.succeed(undefined)));
                if (!match?.id) {
    ...
    const createAlchemyWorkerTags = (id) => [
        `alchemy:stack:${stack.name}`,
        `alchemy:stage:${stack.stage}`,
        `alchemy:id:${id}`,
    ];
    const hasAlchemyWorkerTags = (id, tags) => {
    ...
    const resolveTransferSource = Effect.fn(function* (params) {
        ...
        const candidates = Array.from(new Set(params.observedNamespaces.flatMap((ns) => ns.class === params.className &&
        ...
        for (const script of candidates) {
            ...
            const settings = yield* getScriptSettings(params.accountId, script, undefined).pipe(Effect.catchTag("WorkerNotFound", () => Effect.succeed(undefined)), Effect.catchTag("WorkerHasNoVersions", () => Effect.succeed(undefined)));
        ...
        if (matched.length > 1) {
            return yield* Effect.fail(new AmbiguousDurableObjectTransfer({
    ...
    const getDurableObjects = (bindings) => {
        const namespaces = Object.fromEntries((bindings ?? []).flatMap((binding) => binding.type === "durable_object_namespace" &&
            binding.className &&
            binding.namespaceId
    ...
    const getExpectedDurableObjectClassNames = (bindings, workerName) => Array.from(new Set(bindings?.flatMap((binding) => binding.type === "durable_object_namespace" &&
        binding.className &&
        (binding.scriptName === undefined ||
            binding.scriptName === workerName)
        ? [binding.className]
        : []) ?? []));
    const getWorkerSettingsWithDurableObjects = Effect.fn(function* (scriptName, expectedClassNames, dispatchNamespace) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* getScriptSettings(accountId, scriptName, dispatchNamespace).pipe(Effect.map((settings) => {
            ...
            if (missing.length > 0) {
                return Effect.fail(new MissingDurableObjects({
                    scriptName,
                    expected: missing,
                }));
            }
    ...
    const prepareAssets = Effect.fn(function* (assets) {
        if (!assets) {
            return undefined;
        }
    ...
    const prepareBundle = (id, props) => (isPythonMain(props.main)
    ...
    const hashScript = (script) => Effect.sync(() => crypto.createHash("sha256").update(script).digest("hex"));
    const viteBuild = Effect.fn(function* (props, selfUrl) {
        const compatibility = getCompatibility(props);
        const Vite = yield* loadVite;
        const { clientDirectory, base, serverBundle, externalWorkspaces } = yield* Vite.viteBuild(props.vite?.rootDir, Object.fromEntries((yield* Effect.all(Object.entries(props.env ?? {}).map(Effect.fn(function* ([key, value]) {
            return [
                key,
                typeof value === "string"
                    ? value
    ...
    const loadVite = Effect.promise(() => import("./Sources/Vite.js"));
    const prepareAssetsAndBundle = (id, workerName, props, opts = {}) => Effect.gen(function* () {
        ...
        if (props.main === undefined) {
            if (!props.assets) {
                return yield* Effect.die(new Error(`Worker "${id}" has no main, script, or assets. Provide an entry module (main / script) or an assets directory to deploy an assets-only Worker.`));
            }
            return {
    ...
    }).pipe(Effect.map(({ assets, bundle, input, additionalWorkspaces }) => ({
        assets,
        bundle: {
            main: bundle?.files[0].path,
            files: bundle?.files.map((file) => new File([file.content], file.path, {
    ...
    const normalizePrebuiltAssets = (assets, output) => {
     ...
     */
    const appendAlchemyAndEnvBindings = (metadataBindings, news, accountId, workerName) => {
        ...
        if (news.env) {
            for (const [key, value] of Object.entries(news.env)) {
                if (value === undefined)
                    continue;
                if (metadataBindings.some((b) => b.name === key))
                    continue;
     ...
     */
    const resolveVersionAlias = Effect.fn(function* (id, news, parentName) {
        ...
        const hash = yield* Effect.sync(() => crypto
            .createHash("sha256")
     ...
     */
    const resolveVersionPreviewContext = Effect.fn(function* (accountId, scriptName) {
     ...
     * deploys.
     */
    const validateVersionWorkerProps = Effect.fn(function* (news, bindings, parentName) {
        const forbidden = [
            ["name", news.name],
        ...
        if (forbidden.length > 0) {
            return yield* Effect.fail(new WorkerVersionConfigError({
                message: `version.parent uploads a version of '${parentName}' — script-level settings belong to the parent Worker and cannot be set here: ${forbidden.join(", ")}. ` +
        ...
        if (hostedClasses.length > 0 || exportedClasses.length > 0) {
            return yield* Effect.fail(new WorkerVersionConfigError({
                ...
                ].join(", ")}): class migrations apply to the parent script. Host the classes on the parent Worker and reference them cross-script instead.`,
            }));
        }
    });
    /**
     ...
     */
    const putWorkerVersion = Effect.fn(function* (id, news, bindings, session, output) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        const { previewsEnabled, accountSubdomain } = yield* resolveVersionPreviewContext(accountId, parentName);
        const aliasedUrl = alias !== undefined && previewsEnabled
        ...
        yield* Effect.logInfo(`Cloudflare Worker version: preparing bundle for ${parentName} (from ${id})`);
        const { assets, bundle, hash: preparedHash, } = yield* prepareAssetsAndBundle(id, parentName, news, {
        ...
        // their own).
        const metadataBindings = bindings.flatMap((b) => (b.data.bindings ?? []).map((item) => item.type === "self_url"
            ? { type: "plain_text", name: item.name, text: selfUrl }
        ...
        const created = yield* workers
            ...
            schedule: bindingTargetNotFoundRetrySchedule(),
        }), Effect.catchTag("WorkerNotFound", () => Effect.fail(new WorkerVersionConfigError({
        ...
        if (version.affinity !== undefined || previousAffinityZoneIds.length) {
            const resolvedAffinity = version.affinity !== undefined
                ? yield* resolveVersionAffinity(version.affinity)
                : undefined;
            let hostsByZone;
            if (resolvedAffinity !== undefined) {
                hostsByZone = new Map();
                const addHost = (zoneId, host) => {
                ...
                const parentAttrs = typeof version.parent === "object" && version.parent !== null
                    ? version.parent
                    : undefined;
                ...
                if (hostsByZone.size === 0) {
                    return yield* Effect.fail(new WorkerVersionConfigError({
                        message: `version.affinity pins users via a zone Transform Rule setting the ${AFFINITY_HEADER} header, which only sees zone traffic — the parent '${parentName}' has no custom domains (or recorded zone routes) to place the rule on. ` +
            ...
            const placed = yield* reconcileAffinityRules({
                scriptName: parentName,
                desired: resolvedAffinity !== undefined && hostsByZone !== undefined
        ...
        return {
            ...
            // the parent).
            workerId: cachedWorkerId(output?.workerId, parentName) ??
    ...
    const putWorker = Effect.fn(function* (id, news, bindings, olds, output, session, existingSettings) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        const selfUrl = hasSelfUrlBinding(bindings)
            ? dispatchNamespace
                ? yield* Effect.die(`Worker "${name}" binds its own URL (Worker.URL), but a dispatch-namespace user worker is invoked via dynamic dispatch and has no URL of its own.`)
                : yield* resolveSelfUrl(name, news, accountId)
            : undefined;
        yield* Effect.logInfo(`Cloudflare Worker ${olds ? "update" : "create"}: preparing bundle for ${name}`);
        ...
        const { assets, bundle, hash: preparedHash, } = yield* prepareAssetsAndBundle(id, name, news, {
        ...
        const metadataBindings = bindings.flatMap((b) => (b.data.bindings ?? []).map((item) => {
            ...
            if (item.type === "durable_object_namespace" &&
        ...
        const expectedDurableObjectClassNames = getExpectedDurableObjectClassNames(metadataBindings, name);
        let metadataAssets;
        ...
        if (prebuiltAssets?.skip) {
            ...
            yield* Effect.logInfo(`Cloudflare Worker update: assets unchanged for ${name}, keeping existing`);
            keepAssets = true;
            ...
            metadataAssets = {
                config: mergeAssetsConfigFiles(prebuiltAssets.config, yield* readAssetsConfigFiles(prebuiltAssets.directory)),
            };
            metadataBindings.push({
                type: "assets",
                name: "ASSETS",
        ...
        yield* Effect.logInfo(`Cloudflare Worker ${olds ? "update" : "create"}: uploading script for ${name}`);
        const size = bundle.files
        ...
        const crossScriptClassTargets = new Map();
        ...
        const mayTransferIn = currentDoBindings.some((binding) => !oldDoClassNameByLogicalId[binding.logicalId] &&
            binding.transferredFrom !== undefined);
        const observedNamespaces = !dispatchNamespace &&
            (deletedClassCandidates.length > 0 || mayTransferIn)
            ? yield* listDurableObjectNamespaces(accountId)
            : [];
        const hosts = (namespaces, scriptName, className) => namespaces.some((ns) => ns.script === scriptName && ns.class === className);
        const scriptHostsClass = (scriptName, className) => hosts(observedNamespaces, scriptName, className);
        ...
        for (const className of deletedClassCandidates) {
            ...
            const namespaces = yield* listDurableObjectNamespaces(accountId).pipe(Effect.repeat({
                schedule: Schedule.spaced("2 seconds"),
                until: (observed) => !hosts(observed, name, className) ||
            ...
            return yield* Effect.fail(new DurableObjectTransferRequired({
                scriptName: name,
                className,
                targetScriptName,
            }));
        ...
        const containerClassNames = new Set(bindings.flatMap((b) => (b.data.containers ?? []).map((c) => c.className)));
        ...
        for (const binding of currentDoBindings) {
            ...
            if (!previousClassName) {
                ...
                const observed = oldBindings.find((old) => old.type === "durable_object_namespace" &&
                    "className" in old &&
                    old.className &&
        ...
        if (rolloutTraffic !== undefined &&
            ...
            const created = yield* workers
                ...
                metadata: {
                    ...
                    annotations: news.version?.alias !== undefined ||
            ...
            if (rolloutTraffic !== undefined) {
                ...
                yield* Effect.logInfo(`Cloudflare Worker ${name}: no previous live version to split traffic with; deploying at 100%`);
            }
        ...
        function putWorkerScriptWithMigrationRecovery() {
            return putWorkerScript({
                ...
                // doesn't at runtime.
                const msg = String(typeof err === "object" && err !== null && "message" in err
                    ? err.message
                    : err);
        ...
        const { settings, durableObjectNamespaces } = yield* getWorkerSettingsWithDurableObjects(name, expectedDurableObjectClassNames, dispatchNamespace);
        ...
        if (dispatchNamespace) {
            return {
                workerId: worker.tag ??
                    ...
                    (yield* Effect.fail(new WorkerIdNotFound({
                        scriptName: name,
        ...
        if (workersDev.enabled !== observedSubdomain.enabled ||
            workersDev.previewsEnabled !== observedSubdomain.previewsEnabled) {
            yield* session.note(`${workersDev.enabled || workersDev.previewsEnabled ? "Enabling" : "Disabling"} workers.dev subdomain...`);
            ...
            yield* setWorkerSubdomain(name, workersDev).pipe(Effect.retry({
                while: (error) => error._tag === "WorkerNotFound" ||
        ...
        if (!manageCustomDomains && previousDomain !== undefined) {
            ...
            const live = new Set(yield* workers.listDomains({ accountId, service: name }).pipe(Effect.map((r) => (r.result ?? []).flatMap((d) => d.hostname ? [d.hostname] : [])), Effect.catch(() => Effect.succeed([]))));
            const serving = [
        ...
        if (manageCustomDomains) {
            ...
            const liveBeforeReconcile = yield* workers
                ...
                .pipe(Effect.map((r) => (r.result ?? []).flatMap((d) => d.hostname && d.zoneId
                ? [[d.hostname, d.zoneId]]
                : [])), Effect.catch(() => Effect.succeed([])));
            const reconciled = yield* reconcileDomains(name, desiredHostnames, domainConfig?.zone);
            const zoneIdByHostname = new Map([
                ...liveBeforeReconcile,
                ...reconciled.map((d) => [d.hostname, d.zoneId]),
            ]);
        ...
        if (affinity !== undefined || previousAffinityZoneIds.length > 0) {
            const resolvedAffinity = affinity !== undefined
            ...
            if (resolvedAffinity !== undefined) {
                ...
                if (hostsByZone.size === 0) {
                    return yield* Effect.fail(new WorkerVersionConfigError({
                        message: `version.affinity pins users via a zone Transform Rule setting the ${AFFINITY_HEADER} header, which only sees zone traffic — give '${name}' a custom domain (\`domain\`) or zone \`routes\`. ` +
        ...
        const crons = desiredCrons.length > 0 || previousCrons.length > 0
    ...
    const assetsChanged = Effect.fn(function* (assets, output) {
        ...
        const read = yield* prepareAssets(assets).pipe(Effect.catchTag(["PlatformError", "AssetTooLargeError", "TooManyAssetsError"], () => Effect.succeed(undefined)));
        return read === undefined || read.hash !== output.hash?.assets;
    });
    const hasChanged = Effect.fn(function* (id, props, output, bindings, accountId) {
        ...
        if (bindings) {
            ...
            const versionParent = props.version?.parent != null
        ...
        if (props.source) {
            const source = yield* resolveSource(props);
            const slots = yield* source.hash(makeSourceContext({
                id,
            ...
            for (const slot of ["bundle", "input", "assets"]) {
        ...
        if (props.vite) {
            const Vite = yield* loadVite;
            const { hash } = yield* Vite.hashViteInput(props.vite.rootDir, props.vite.memo, Effect.succeed(output.hash?.additionalWorkspaces ?? []));
        ...
        const bundleHash = yield* prepareBundle(id, props).pipe(Effect.map((b) => b.hash));
    ...
    return Worker.Provider.of({
        ...
        list: () => Effect.gen(function* () {
            ...
            return yield* workers.listScripts.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => 
            ...
            (page.result ?? []).flatMap((script) => script.id
                ? [
                    {
                        accountId,
        ...
        diff: Effect.fn(function* ({ id, news: desired, olds, output, newBindings, }) {
            const { accountId } = yield* yield* CloudflareEnvironment;
            ...
            const routesChanged = newRouteKeys.length !== oldRouteKeys.length ||
            ...
            const effectiveDomainConfig = news.domain !== undefined ? newDomainConfig : oldDomainConfig;
            const newUrl = newIsVersion
                ? undefined
                : effectiveDomainConfig !== undefined
                    ? `https://${effectiveDomainConfig.name}`
                    : newWorkersDev.enabled
                        ? (output.urls ?? []).find((u) => isWorkersDevHostname(urlHostname(u)) &&
            ...
            const newDoClassNames = Array.isArray(newBindings)
                ? getExpectedDurableObjectClassNames(newBindings.flatMap((b) => b.data.bindings ?? []), workerName).sort()
                : [];
            ...
            if (legacyWorkerId ||
                ...
                routesChanged ||
                cronsChanged ||
                (yield* hasChanged(id, news, output, Array.isArray(newBindings)
                    ? newBindings
                    : undefined, accountId))) {
                ...
                const stables = legacyWorkerId ? [] : ["workerId"];
            ...
            if (Array.isArray(newBindings)) {
                const normalizeComparedProps = (props) => {
        ...
        precreate: Effect.fn(function* ({ id, news, session, bindings }) {
            ...
            if (news.version?.parent != null) {
                yield* Effect.logInfo(`Cloudflare Worker precreate: skipping stub for version worker ${id}`);
                return {
            ...
            if (news.namespace != null) {
                yield* Effect.logInfo(`Cloudflare Worker precreate: skipping stub for dispatch-namespace worker ${name}`);
                return {
            ...
            const durableObjects = mergeDurableObjectClasses(exportDerived, getDurableObjectBindings(bindings, name)).filter((binding) => !binding.transferredFrom);
            ...
            const containers = Array.from(new Set(bindings.flatMap((b) => (b.data.containers ?? []).map((c) => c.className)))).map((className) => ({ className }));
            ...
            yield* Effect.logInfo(`Cloudflare Worker precreate: durable objects ${JSON.stringify(durableObjects)}`);
            const existingSettings = yield* getScriptSettings(accountId, name, dispatchNamespace).pipe(
            ...
            Effect.catchTag("WorkerNotFound", () => Effect.succeed(undefined)), Effect.catchTag("WorkerHasNoVersions", () => Effect.succeed(undefined)), Effect.catchTag("DispatchNamespaceScriptNotFound", () => Effect.succeed(undefined)), Effect.catchTag("DispatchNamespaceNotFound", () => Effect.succeed(undefined)));
            ...
            if (existingSettings) {
                ...
                if (doClasses.length > 0) {
                    ({ durableObjectNamespaces } =
                        yield* getWorkerSettingsWithDurableObjects(name, doClasses, dispatchNamespace));
                }
            }
        ...
        read: Effect.fn(function* ({ id, output, olds }) {
            ...
            if (output?.versionOf !== undefined ||
                ...
                return yield* workers
                    ...
                    .pipe(Effect.map(() => output), Effect.catchTag(["WorkerNotFound", "VersionNotFound"], () => Effect.succeed(undefined)));
            }
            ...
            if (dispatchNamespace) {
                ...
                yield* Effect.logInfo(`Cloudflare Worker read: found ${workerName} in dispatch namespace ${dispatchNamespace}`);
                const attrs = {
                    accountId,
                    ...
                    workerId: cachedWorkerId(output?.workerId, workerName) ??
                        (yield* wfp
                            ...
                            .pipe(Effect.flatMap((r) => r.script?.tag != null
            ...
            // between reads.
            const declared = yield* resolveWorkerDomain(olds?.domain).pipe(Effect.orElseSucceed(() => undefined));
            const observed = new Set(domainsList.flatMap((d) => (d.hostname ? [d.hostname] : [])));
            const keptName = declared !== undefined && observed.has(declared.name)
                ? declared.name
                : undefined;
            ...
            const observedDomain = serving.length > 0
                ? {
                    name: serving[0],
                    aliases: serving.slice(1),
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, bindings, output, session, }) {
            ...
            const { accountId } = yield* yield* CloudflareEnvironment;
            const name = output?.workerName ?? (yield* createWorkerName(id, news.name));
            const durableObjects = getDurableObjectBindings(bindings, name).map(({ logicalId, className }) => ({
                logicalId,
                className,
            }));
            yield* Effect.logInfo(`Cloudflare Worker reconcile: starting ${name}`);
            yield* Effect.logInfo(`Cloudflare Worker reconcile: durable objects ${JSON.stringify(durableObjects)}`);
            const dispatchNamespace = resolveNamespaceName(news.namespace);
            ...
            // `DispatchNamespaceScriptNotFound` / `DispatchNamespaceNotFound`.
            Effect.catchTag("WorkerNotFound", () => Effect.succeed(undefined)), Effect.catchTag("WorkerHasNoVersions", () => Effect.succeed(undefined)), Effect.catchTag("DispatchNamespaceScriptNotFound", () => Effect.succeed(undefined)), Effect.catchTag("DispatchNamespaceNotFound", () => Effect.succeed(undefined)));
            yield* Effect.logInfo(`Cloudflare Worker reconcile: existing durable object tags ${JSON.stringify((existingSettings?.tags ?? []).filter(isDurableObjectTag))}`);
            yield* Effect.logInfo(`Cloudflare Worker reconcile: previous durable object tags ${JSON.stringify((output?.tags ?? []).filter(isDurableObjectTag))}`);
            return yield* putWorker(id, news, bindings, olds, output, session, existingSettings);
        }),
        delete: Effect.fn(function* ({ output }) {
            ...
            if (output.versionOf !== undefined) {
                ...
                if (!output.versionId)
                    return;
                yield* Effect.logInfo(`Cloudflare Worker delete: releasing version ${output.versionId} of ${output.versionOf}`);
                const { deployments } = yield* workers
                    .listScriptDeployments({
                    accountId: output.accountId,
                    scriptName: output.versionOf,
                })
                    .pipe(Effect.catchTag("WorkerNotFound", () => Effect.succeed({
                    deployments: [],
                })));
                ...
                if (!stable) {
                    ...
                    yield* Effect.logWarning(`Cloudflare Worker delete: version ${output.versionId} serves 100% of '${output.versionOf}' traffic; leaving the deployment as-is. Re-deploy the parent to move off it.`);
                    return;
                }
                yield* workers
            ...
            if (output.namespace) {
                yield* deleteWorkerScript(output.accountId, output.workerName, output.namespace).pipe(Effect.catchTag([
            ...
            const liveDomains = yield* workers
                ...
                .pipe(Effect.map((r) => r.result ?? []), Effect.catch(() => Effect.succeed([])));
            ...
            if (redirects.length > 0) {
                yield* reconcileRedirectRules({
                    scriptName: output.workerName,
                    domain: undefined,
                    zoneIdByHostname: new Map(liveDomains.flatMap((d) => d.hostname && d.zoneId
                        ? [[d.hostname, d.zoneId]]
                        : [])),
            ...
            if (liveDomains.length) {
                yield* Effect.all(liveDomains.flatMap((d) => d.id
                    ? [
                        workers
                            .deleteDomain({
                            accountId: output.accountId,
            ...
            if (output.routes?.length) {
                yield* Effect.all(output.routes.map((route) => workers
                    .deleteRoute({
                    zoneId: route.zoneId,
                    routeId: route.id,
                })
            ...
            yield* deleteWorkerScript(output.accountId, output.workerName, undefined).pipe(Effect.catchTag("WorkerNotFound", () => Effect.void));
        }),
        tail: ({ output }) => telemetry.tailScript({
            accountId: output.accountId,
            scriptName: output.workerName,
        }),
        logs: ({ output, options }) => telemetry.queryLogs({
            accountId: output.accountId,
            filters: [
                {
...
const contentTypeForModule = (filePath) => {
 ...
 */
const listDurableObjectNamespaces = (accountId) => durableObjectsApi.listNamespaces.items({ accountId }).pipe(Stream.runCollect, Effect.map((namespaces) => Array.from(namespaces).flatMap((ns) => ns.script && ns.class ? [{ script: ns.script, class: ns.class }] : [])));
/**
 ...
 */
const normalizeTransferSources = (value, selfScriptName) => (value === undefined
    ? []
    : Array.isArray(value)
        ? value
        : [value]).filter((source) => source !== selfScriptName);
function bumpMigrationTagVersion(oldTag) {
    if (!oldTag)
        return undefined;
 ...
 */
function mergeDurableObjectClasses(exportDerived, bindingDerived) {
    return Array.from(new Map([...exportDerived, ...bindingDerived].map((binding) => [binding.className, binding])).values());
}
function getDurableObjectBindings(bindings, workerName) {
    ...
    const seen = new Set();
    return bindings.flatMap((binding) => (binding.data.bindings ?? []).flatMap((item) => {
        if (item.type !== "durable_object_namespace" ||
            !("className" in item) ||
            !item.className) {
            return [];
        ...
        return [
            {
                ...
                transferredFrom: Array.isArray(item.transferredFrom) &&
 ...
 * @internal exported for unit testing.
 */
export function encodeDurableObjectTags(durableObjects) {
    ...
    for (const pair of pairs) {
        ...
        if (PACKED_DO_TAG_PREFIX.length + appended.length > MAX_TAG_BYTES &&
            payload !== "") {
 ...
 * @internal exported for unit testing.
 */
export function getDurableObjectTagMap(tags) {
    const map = {};
...
const isDurableObjectTag = (tag) => tag.startsWith(LEGACY_DO_TAG_PREFIX) || tag.startsWith(PACKED_DO_TAG_PREFIX);
/**
 ...
 */
const validateWorkerTags = (scriptName, tags, alchemyTagCount) => Effect.gen(function* () {
    if (tags.length > MAX_TAGS_PER_WORKER) {
        return yield* Effect.fail(new InvalidWorkerTags({
            scriptName,
            reason: `worker "${scriptName}" needs ${tags.length} script tags but Cloudflare allows at most ${MAX_TAGS_PER_WORKER}. ` +
