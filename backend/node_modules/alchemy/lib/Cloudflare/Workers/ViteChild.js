...
import { fromProcessEnv, RPC_SERVER_ENVIRONMENT_KEY, } from "../../Local/RpcServerEnvironment.js";
import { Stack } from "../../Stack.js";
import { transformTypesFlags } from "../../Util/Node.js";
import { unwrapRedacted } from "../../Util/index.js";
import { VITE_CHILD_READY_PREFIX, VITE_CHILD_READY_SUFFIX, } from "./ViteChild.shared.js";
...
const resolveRunner = (basename) => fileURLToPath(import.meta.resolve(import.meta.url.endsWith(".ts") ? `./${basename}.ts` : `./${basename}.js`));
export const startViteChild = (config, onOutput) => Effect.gen(function* () {
    ...
    const nodeExecPath = isBun && config.source?.descriptor.runtime === "node"
    ...
    const serializedConfig = nodeExecPath !== undefined
    ...
    const base = yield* fromProcessEnv.pipe(Effect.orElseSucceed(() => ({
    ...
    const childEnvironment = {
        ...
        alchemyContext: Option.getOrElse(alchemyContext, () => base.alchemyContext),
        stack: Option.getOrElse(Option.map(stack, (s) => ({ name: s.name, stage: s.stage })), () => base.stack),
    };
    const child = yield* spawner.spawn(ChildProcess.make(nodeExecPath ?? process.execPath, isBun && nodeExecPath === undefined
        ? ["run", runner]
    ...
    yield* child.stdout.pipe(Stream.decodeText, Stream.splitLines, Stream.runForEach((line) => {
    ...
    yield* child.stderr.pipe(Stream.decodeText, Stream.splitLines, Stream.runForEach((line) => Effect.sync(() => onOutput("stderr", line))), Effect.forkScoped);
    const exitCode = child.exitCode.pipe(Effect.orDie);
    const url = yield* Effect.raceAllFirst([
        Deferred.await(ready),
        exitCode.pipe(Effect.flatMap((exitCode) => Effect.die(new Error(`Vite child exited with code ${exitCode} before becoming ready`)))),
    ]);
    return { url, pid: child.pid, exitCode };
});
 ...
 */
export const runViteBuildChild = (config, onOutput) => Effect.scoped(Effect.gen(function* () {
    ...
    const child = yield* spawner.spawn(ChildProcess.make(process.execPath, isBun
        ? ["run", runner]
        : [
    ...
    const tail = [];
    const forward = (channel) => (line) => Effect.sync(() => {
        tail.push(line);
        if (tail.length > BUILD_ERROR_TAIL)
            tail.shift();
        onOutput(channel, line);
    });
    const stdoutFiber = yield* Effect.forkChild(child.stdout.pipe(Stream.decodeText, Stream.splitLines, Stream.runForEach(forward("stdout"))));
    const stderrFiber = yield* Effect.forkChild(child.stderr.pipe(Stream.decodeText, Stream.splitLines, Stream.runForEach(forward("stderr"))));
    ...
    return NodeV8.deserialize(Buffer.from(bytes));
}));
//# sourceMappingURL=ViteChild.js.map