...
import { type WorkerAccessConfig, type WorkerAccessIdentity, type WorkerExecutionContextAccess } from "./WorkerAccess.ts";
...
export interface WorkerObservability extends Exclude<workers.PutScriptRequest["metadata"]["observability"], undefined> {
}
export interface WorkerLimits extends Exclude<workers.PutScriptRequest["metadata"]["limits"], undefined> {
}
...
export type NormalizedBindings<Bindings extends WorkerBindingProps = {}, AssetsConfig extends WorkerAssetsConfig | undefined = undefined> = {
    [B in keyof Bindings]: Bindings[B] extends Effect.Effect<infer T extends WorkerBindingResource, any, any> ? T extends Redacted.Redacted<infer V> | Config.Config<infer V> ? V : Unwrap<T> : Extract<Unwrap<Bindings[B]>, WorkerBindingResource>;
} & (undefined extends AssetsConfig ? {} : {
    ASSETS: Assets;
});
...
export interface ViteOptions {
    ...
    memo?: MemoOptions & {
         ...
         * @default "auto"
         */
        workspaces?: "auto" | Array<MemoOptions & {
            /**
...
export type Worker<Bindings = any> = Resource<WorkerTypeId, WorkerProps<Bindings>, {
    /**
 ...
 * public URL.
 */
export declare const URL: URLEffect;
/**
 ...
 */
export declare const isSelfUrl: (value: unknown) => value is URLEffect;
/**
...
export declare const Self: {
    readonly "~alchemy/Kind": "Cloudflare.Workers.Self";
};
export type Self = typeof Self;
/** Returns true when the value is the {@link Self} marker. */
export declare const isSelf: (value: unknown) => value is Self;
/**
...
export declare const Worker: ResourceClassLike<Worker> & Effect.Effect<Worker & WorkerRuntimeContext & RuntimeContext, never, Worker> & {
    <Self, Shape extends WorkerShape, Deps = never>(): {
        <const Id extends string>(id: Id): Effect.Effect<Worker & Rpc<Self> & Dependencies<Deps>, never, Self | Extract<Deps, Container.Application<any>> | Providers> & Named<Id> & {
            ...
            make<PropsReq = never, InitReq = never>(props: InputProps<WorkerProps> | Effect.Effect<InputProps<WorkerProps>, ConfigError, PropsReq>, impl: Effect.Effect<Shape, ConfigError, InitReq>): Layer.Layer<Self, never, Extract<Deps, Container.Application<any>> | Providers | Exclude<PropsReq | InitReq, Self | WorkerServices | Tag<WorkerTypeId>>>;
        };
    };
    <Self>(): {
        <const Id extends string, Shape extends WorkerShape, Req extends WorkerServices | Container.Application<any> | PlatformServices | Tag, PropsReq = never>(id: Id, props: InputProps<WorkerProps> | Effect.Effect<InputProps<WorkerProps>, ConfigError, PropsReq>, impl: Effect.Effect<Shape, ConfigError, Req>): Effect.Effect<Worker & Rpc<Self>, never, Extract<Req, Container.Application<any>> | Providers | PropsReq> & Named<Id> & {
         ...
         * ```
         */
        <const Id extends string, Req = never>(id: Id, props: InputProps<WorkerProps> | Effect.Effect<InputProps<WorkerProps>, ConfigError, Req>): Effect.Effect<Worker & Rpc<{}>, never, Req | Providers> & Named<Id> & {
            new (): Named<Id> & Tag<WorkerTypeId>;
        };
    };
    <const Bindings extends WorkerBindingProps = {}, const Assets extends WorkerAssetsConfig | undefined = undefined, Req = never>(id: string, props: InputProps<WorkerProps<Bindings, Assets>> | Effect.Effect<InputProps<WorkerProps<Bindings, Assets>>, ConfigError, Req>): Effect.Effect<Worker<{
        [binding in keyof NormalizedBindings<Bindings, Assets>]: NormalizedBindings<Bindings, Assets>[binding];
    }> & Rpc<{}>, never, Req | Providers>;
    <const Id extends string, Shape extends WorkerShape, Req extends WorkerServices | Container.Application<any> | PlatformServices>(id: string, props: InputProps<WorkerProps>, impl: Effect.Effect<Shape, ConfigError, Req>): Effect.Effect<Worker & Rpc<Shape>, never, Extract<Req, Container.Application<any>> | Providers> & Named<Id>;
    /**
