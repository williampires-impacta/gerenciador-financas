...
import { RepositoryEventSource, webhookPath, webhookSecretEnvName, } from "../../GitHub/RepositoryEventSource.js";
 ...
 */
export const GitHubRepositoryEventSourceLive = Layer.effect(RepositoryEventSource, Effect.gen(function* () {
    ...
    return Effect.fn(function* (props, process) {
        ...
        if (!globalThis.__ALCHEMY_RUNTIME__) {
            yield* Namespace.push(ctx.LogicalId, Effect.gen(function* () {
...
// unverified.
secret, process) => Effect.gen(function* () {
    ...
    if (secret !== undefined) {
        ...
        const valid = yield* verifySignature(resolved ? Redacted.value(resolved) : undefined, body, signature);
        if (!valid) {
    ...
    const delivery = { id, name, payload };
    yield* process(delivery).pipe(Effect.orDie);
    return new Response(null, { status: 202 });
});
const verifySignature = (secret, body, signature) => Effect.gen(function* () {
...
//# sourceMappingURL=GitHubRepositoryEventSource.js.map