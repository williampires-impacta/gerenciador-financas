...
const TypeId = "Cloudflare.Workers.ObservabilityDestination";
/**
 ...
 */
export const ObservabilityDestination = Resource(TypeId);
/**
 * Returns true if the given value is an ObservabilityDestination resource.
 */
export const isObservabilityDestination = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const ObservabilityDestinationProvider = () => Provider.succeed(ObservabilityDestination, {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (!observed) {
            const created = yield* workers
                ...
                .pipe(Effect.catchTag("ObservabilityDestinationCreateFailed", (originalError) => Effect.gen(function* () {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* workers.listObservabilityDestinations
            .items({ accountId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).map((observed) => toAttributes(observed, accountId))));
    }),
});
const isObservedDestination = (value) => Predicate.hasProperty(value.configuration, "headers");
const createDestinationName = (id) => createPhysicalName({ id, lowercase: true });
/**
 * Locate a destination by its stable slug. There is no get-by-slug API,
 * so observation goes through the list endpoint.
 */
const findBySlug = (accountId, slug) => workers.listObservabilityDestinations.items({ accountId }).pipe(Stream.filter((d) => d.slug === slug), Stream.runCollect, Effect.map((chunk) => Array.from(chunk).at(0)));
/**
 * Locate a destination by exact name. Cloudflare enforces name
 * uniqueness, so a match identifies the destination.
 */
const findByName = (accountId, name) => workers.listObservabilityDestinations.items({ accountId }).pipe(Stream.filter((d) => d.name === name), Stream.runCollect, Effect.map((chunk) => Array.from(chunk).at(0)));
/**
 ...
 */
const freshObserved = (accountId, slug) => findBySlug(accountId, slug).pipe(Effect.flatMap((observed) => observed
    ...
    : // The destination cannot vanish between the write that just
        ...
        Effect.fail(new workers.ObservabilityDestinationNotFound({
            code: 0,
 ...
 */
const sameHeaders = (observed, desired) => {
    const normalized = Object.fromEntries(Object.entries(observed)
        .filter(([key]) => key !== "content-type" || "content-type" in desired)
        .map(([key, value]) => [key, String(value)]));
    const keys = Object.keys(normalized);
    return (keys.length === Object.keys(desired).length &&
...
//# sourceMappingURL=ObservabilityDestination.js.map