import { Runtime, } from "@alchemy.run/cloudflare-runtime/core";
...
import { unwrapRedacted } from "../../Util/index.js";
import { CloudflareEnvironment } from "../CloudflareEnvironment.js";
import { isLiveId, LOCAL_ENTRY_URL, LocalRuntimeState, localStorageDirectory, } from "../LocalRuntime.js";
import { readAssetsConfigFiles } from "./Assets.js";
import { getCompatibility } from "./Compatibility.js";
import { watchPrebuiltWorkerBundle } from "./Sources/Prebuilt.js";
import { isPythonMain, watchPythonWorkerBundle } from "./Sources/Python.js";
import { Artifacts as AlchemyArtifacts, createArtifactStore, makeScopedArtifacts, } from "../../Artifacts.js";
import { loadSource, SourceProviderError } from "./Source.js";
import { isSelfUrl, Worker } from "./Worker.js";
import { getCronBindings } from "./WorkerAsyncBindings.js";
import { WorkerBundle } from "./Sources/Rolldown.js";
import { createWorkerName } from "./WorkerName.js";
import { resolveTailConsumers } from "./WorkerProvider.js";
import { materializeRuntimeBindings, WorkerValidationError, } from "./RuntimeBindings.js";
import { startViteChild } from "./ViteChild.js";
import { DEFAULT_DEV_PORT } from "./ViteChild.shared.js";
...
const isWildcardHost = (host) => host === "0.0.0.0" || host === "::" || host === "[::]" || host === "*";
/**
 ...
 */
const resolveLocalUrls = (serverUrl) => Effect.sync(() => {
...
export const LocalWorkerProvider = () => LocalProvider.make(Worker, LOCAL_ENTRY_URL, Effect.gen(function* () {
    ...
    const toRuntimeConsumerSettings = (settings) => ({
    ...
    const getQueueConsumers = Effect.fn(function* (scriptName) {
        const consumers = [];
            ...
            if (consumer.scriptName === scriptName) {
                ...
                if (isLiveId(consumer.queueId)) {
                    if (!consumer.queueName) {
                        return yield* Effect.die(`Consumer of live queue ${consumer.queueId} is missing its resolved queueName — re-deploy the consumer`);
                    }
                    consumers.push({
                ...
                const queue = MutableHashMap.get(localRuntimeState.queues, consumer.queueId).pipe(Option.getOrUndefined);
                if (queue) {
                    consumers.push({
    ...
    const startProxy = Effect.fn(function* (id, serverOptions) {
    ...
    const stopProxy = Effect.fn(function* (id) {
    ...
    const maybeStartProxy = Effect.fn(function* (id, serverOptions) {
        const existing = proxyInstances.get(id);
        if (existing) {
    ...
    const toRuntimeModules = Effect.fn(function* (bundle) {
        ...
        for (const file of bundle.files) {
            ...
            if (file.path.startsWith("python_modules/")) {
                const isJsShim = file.path.startsWith("python_modules/workers/") &&
                    /\.m?js$/.test(file.path);
                modules.push(isJsShim
                    ? {
                        name: file.path,
                        type: "ESModule",
     ...
     */
    const resolveConfig = Effect.fn(function* ({ id, news, bindings, }) {
        const props = news;
        ...
        const dev = props.dev?.mode === "external"
            ? props.dev
            : {
                ...props.dev,
                mode: "worker",
    ...
    const serveWith = (worker, bundle, proxy) => Semaphore.withPermits(serveLock(worker.id), 1)(
    ...
    Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
        ...
        while (true) {
            ...
            MutableHashMap.set(localRuntimeState.workerRestarts, worker.name, restartWorker(worker.id));
            const currentConsumers = yield* getQueueConsumers(worker.name);
            if (JSON.stringify(currentConsumers) !==
        ...
        for (const replaced of previous
            ? [...superseded, previous]
            : superseded) {
            yield* Scope.close(replaced, Exit.void).pipe(Effect.catchCause((cause) => Effect.logWarning(`[${worker.id}] Failed to stop previous local worker instance`, Cause.squash(cause))));
        }
        return url;
    })));
    /**
     ...
     * updated state.
     */
    const logRestartFailure = (id) => Effect.catchCause((cause) => Effect.logWarning(`[${id}] Failed to restart local worker`, Cause.squash(cause)));
    const restartWorker = (id) => Effect.suspend(() => {
        const latest = latestServes.get(id);
        if (latest) {
            return serveWith(latest.worker, latest.bundle, latest.proxy).pipe(Effect.asVoid, logRestartFailure(id));
        }
        ...
        if (vite) {
            ...
            // on the serve lock).
            return serveVite(vite, { onlyIfConsumersChanged: true }).pipe(Effect.asVoid, logRestartFailure(id));
        }
        return Effect.void;
    });
    ...
    const closeWorkerd = Effect.fn(function* (id) {
     ...
     */
    const serveBundleStream = Effect.fn(function* (worker, proxy, bundles) {
        let start = Date.now();
        let status = "start";
        yield* bundles.pipe(Stream.tap((event) => {
            if (event._tag === "Start") {
                start = Date.now();
                if (status === "update") {
                    return Effect.all([
            ...
            : Result.failVoid), Stream.mapEffect((bundle) => serveWith(worker, bundle, proxy).pipe(Effect.exit, Effect.tap((exit) => {
    ...
    const runWorker = Effect.fn(function* (worker) {
        ...
        if (worker.script !== undefined) {
            yield* serveWith(worker, {
                files: [{ path: "main.js", content: worker.script, hash: "" }],
                hash: "",
            }, proxy);
            yield* Effect.log(`[${worker.id}] Started in ${Math.round(Date.now() - start)}ms → ${proxy.url}`);
            return proxy.url;
        }
        const bundles = isPythonMain(worker.bundleOptions.main)
    ...
    const runAssetsOnly = Effect.fn(function* (worker) {
        const start = Date.now();
        const proxy = yield* maybeStartProxy(worker.id, worker.dev);
        yield* serveWith(worker, assetsOnlyBundle, proxy);
        yield* Effect.log(`[${worker.id}] Started in ${Math.round(Date.now() - start)}ms`);
        return proxy.url;
    });
    ...
    const latestViteServes = new Map();
    ...
    const servedViteConsumers = new Map();
    ...
    const serveVite = (args, options) => Semaphore.withPermits(serveLock(args.worker.id), 1)(
    ...
    Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
        ...
        if (options?.onlyIfConsumersChanged) {
            ...
            if (workerdScopes.has(worker.id) &&
        ...
        while (true) {
            ...
            const scope = yield* Scope.fork(rootScope);
            const child = yield* restore(startViteChild({
                rootDir: root,
            ...
            yield* child.exitCode.pipe(Effect.flatMap((exitCode) => Effect.logWarning(`[${worker.id}] Dev server child exited unexpectedly with code ${exitCode}`)), Effect.andThen(proxy.unset().pipe(Effect.ignore)), Effect.andThen(invalidate), Effect.forkIn(scope));
            ...
            servedViteConsumers.set(worker.id, JSON.stringify(queueConsumers));
            yield* proxy.set(child.url);
            return;
        }
    })));
    const runVite = Effect.fn(function* (worker, rootDir, invalidate, source) {
        const proxy = yield* maybeStartProxy(worker.id, worker.dev);
    ...
    const runSource = Effect.fn(function* (worker) {
        const proxy = yield* maybeStartProxy(worker.id, worker.dev);
        ...
        const source = yield* loadSource(worker.source).pipe(Effect.provideService(AlchemyArtifacts, makeScopedArtifacts(createArtifactStore(), worker.id)));
        const devCtx = {
            id: worker.id,
        ...
        if (handle.mode !== "bundle") {
            return yield* Effect.fail(new SourceProviderError({
    ...
    return {
        ...
        stables: ({ output, config }) => Effect.succeed(output?.workerName === config.name
            ? ["workerName"]
            : undefined),
        ...
        start: Effect.fn(function* ({ id, config, invalidate }) {
            ...
            const needsSelfUrl = config.bindingDescriptors.some((b) => b.type === "self_url") ||
            ...
            const env = config.env && selfUrl !== undefined
                ? Object.fromEntries(Object.entries(config.env).map(([key, value]) => [
                    key,
            ...
            return {
                ...
                domain: undefined,
                tags: [],
                durableObjectNamespaces: Object.fromEntries(config.durableObjectNamespaces.map((namespace) => [
                    namespace.className,
                    namespace.uniqueKey,
                ])),
                routes: [],
...
const toRuntimeAssets = Effect.fn(function* (assets) {
    if (!assets)
        return undefined;
    ...
    const files = yield* readAssetsConfigFiles(directory).pipe(Effect.orElseSucceed(() => ({
        _headers: undefined,
...
//# sourceMappingURL=LocalWorkerProvider.js.map