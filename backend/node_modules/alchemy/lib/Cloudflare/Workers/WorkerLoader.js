...
import { asEffect } from "../../Util/index.js";
import { fromCloudflareFetcher } from "../Fetcher.js";
import { makeRpcStub } from "./Rpc.js";
import { Worker, WorkerEnvironment } from "./Worker.js";
const WorkerLoaderTypeId = "Cloudflare.DynamicWorker";
export const isWorkerLoader = (value) => typeof value === "object" &&
    value !== null &&
    "~alchemy/Kind" in value &&
    value["~alchemy/Kind"] === WorkerLoaderTypeId;
/**
 ...
 */
export const WorkerLoader = Object.assign(taggedFunction(Context.Service()("Cloudflare.WorkerLoader"), (name = "LOADER") => Object.assign(Effect.gen(function* () {
    yield* (yield* Worker).bind `${name}`({
        bindings: [{ type: "worker_loader", name }],
    });
    const loader = yield* Effect.all([
        WorkerEnvironment,
        ALCHEMY_PHASE,
    ...
    return {
        ...
        name,
        load: (options) => Effect.sync(() => wrapWorkerStub(loader.load(unwrapWorkerLoader(options)))),
        get: (name, getCode) => Effect.flatMap(Effect.context(), (context) => Effect.sync(() => wrapDynamicWorkerEntrypoint(loader.get(name, () => asEffect(getCode()).pipe(Effect.provide(context), Effect.map(unwrapWorkerLoader), Effect.runPromise))))),
    };
}), {
    "~alchemy/Name": name,
    ...
    layer: (loader) => Layer.succeed(Context.Service()("Cloudflare.WorkerLoader"), loader),
});
/**
 ...
 */
const unwrapWorkerLoader = (loader) => ({
    ...loader,
...
const wrapDynamicWorkerEntrypoint = (raw) => Object.assign(makeRpcStub(raw), fromCloudflareFetcher(raw));
const wrapWorkerStub = (raw) => {
    const defaultEntrypoint = fromCloudflareFetcher(raw.getEntrypoint());
    return {
        ...defaultEntrypoint,
        getEntrypoint: (name) => wrapDynamicWorkerEntrypoint(name ? raw.getEntrypoint(name) : raw.getEntrypoint()),
    };
};
