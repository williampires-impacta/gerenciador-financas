 ...
 */
export const cron = (expression, process) => CronEventSource.use((source) => source(expression, process));
export class CronEventSource extends Context.Service()("Cloudflare.Workers.CronEventSource") {
}
export const CronEventSourceLive = Layer.effect(CronEventSource, Effect.gen(function* () {
    const host = yield* Worker;
    return Effect.fn(function* (expression, process) {
        ...
        if (!globalThis.__ALCHEMY_RUNTIME__) {
            yield* Namespace.push(host.LogicalId, host.bind(`Cron(${expression})`, {
                crons: [expression],
            }));
        }
        const ctx = (yield* RuntimeContext);
        yield* ctx.listen((event) => {
            if (!isWorkerEvent(event) || event.type !== "scheduled")
                return;
            const controller = event.input;
            if (controller.cron !== expression)
                return;
            return process(controller).pipe(Effect.catchCause(() => Effect.void));
        });
    });
}));
//# sourceMappingURL=CronEventSource.js.map