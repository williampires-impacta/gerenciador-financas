 ...
 */
export const scheduleEvent = Effect.fn(function* (id, runAt, payload, repeatMs) {
    yield* ensureTable;
    const ctx = yield* DurableObjectState;
    yield* ctx.storage.sql.exec(`INSERT OR REPLACE INTO alchemy_scheduled_events (id, run_at, repeat_ms, payload)
     VALUES (?, ?, ?, ?)`, id, runAt.getTime(), repeatMs ?? null, JSON.stringify(payload));
    yield* reconcileAlarm;
});
/**
 * Cancel a previously scheduled event by id. No-op if the event does not exist.
 */
export const cancelEvent = Effect.fn(function* (id) {
    yield* ensureTable;
    const ctx = yield* DurableObjectState;
 ...
 */
export const listEvents = Effect.gen(function* () {
    yield* ensureTable;
    const ctx = yield* DurableObjectState;
    const cursor = yield* ctx.storage.sql.exec(`SELECT id, run_at, repeat_ms, payload FROM alchemy_scheduled_events ORDER BY run_at ASC`);
 ...
 * ```
 */
export const processScheduledEvents = Effect.gen(function* () {
    yield* ensureTable;
    const ctx = yield* DurableObjectState;
    const now = Date.now();
    const cursor = yield* ctx.storage.sql.exec(`SELECT id, run_at, repeat_ms, payload FROM alchemy_scheduled_events WHERE run_at <= ? ORDER BY run_at ASC`, now);
    const fired = yield* cursor.pipe(Stream.mapEffect((row) => (row.repeat_ms != null
        ? ctx.storage.sql.exec(`UPDATE alchemy_scheduled_events SET run_at = ? WHERE id = ?`, now + row.repeat_ms, row.id)
        : ctx.storage.sql.exec(`DELETE FROM alchemy_scheduled_events WHERE id = ?`, row.id)).pipe(Effect.as(toScheduledEvent(row)))), Stream.runCollect);
    yield* reconcileAlarm;
    return fired;
});
/**
 ...
 */
const reconcileAlarm = Effect.gen(function* () {
    const ctx = yield* DurableObjectState;
    const next = yield* (yield* ctx.storage.sql.exec(`SELECT run_at FROM alchemy_scheduled_events ORDER BY run_at ASC LIMIT 1`)).pipe(Stream.take(1), Stream.runHead);
