...
import { bundleSource, resolveMainPath, watchBundleDirectory, } from "./shared.js";
/**
 ...
 * module.
 */
export const readPrebuiltWorkerBundle = Effect.fn(function* (options) {
    ...
    const readModuleFile = Effect.fn(function* (name) {
        const file = path.join(root, name);
        const content = yield* fs.readFile(file).pipe(Effect.mapError((cause) => new Bundle.BundleError({
            message: `Failed to read prebuilt worker bundle module "${file}"`,
            cause,
        })));
        const hash = yield* sha256(content);
        return { path: name, content, hash };
    });
    return yield* readModuleFile(entryName).pipe(Effect.zipWith(Effect.tryPromise({
        try: () => fg.glob((options.rules ?? defaultModuleRules).flatMap((rule) => rule.globs), {
            cwd: root,
            onlyFiles: true,
            dot: true,
        }),
        catch: (error) => new Bundle.BundleError({
        ...
        .sort()), Effect.flatMap(Effect.forEach(readModuleFile, { concurrency: "unbounded" }))), (entryModule, additionalModules) => [entryModule, ...additionalModules], { concurrent: true }), Effect.flatMap(Bundle.bundleOutputFromFiles));
});
/**
 ...
 */
export const watchPrebuiltWorkerBundle = (options) => watchBundleDirectory({
    main: options.main,
...
export const makePrebuiltSource = (options) => {
    ...
    return bundleSource({
        build: () => build,
        watch: () => Effect.succeed(watchPrebuiltWorkerBundle({
            main: options.main,
            rules: options.rules,
        })),
    });
