...
import { isWorkflowExport, } from "../../Workflows/Workflow.js";
 ...
 */
const rebindEsmExternalRequirePlugin = (plugins, esmExternalRequirePlugin) => plugins.map((plugin) => {
...
export const WorkerBundle = Effect.gen(function* () {
    ...
    const makeOptions = Effect.fn(function* (options) {
        ...
        const [{ default: cloudflareRolldown }, { esmExternalRequirePlugin }] = yield* Effect.promise(() => Promise.all([
        ...
        const inputOptions = {
            ...
            cwd: yield* findCwdForBundle(realMain).pipe(Effect.mapError((cause) => new Bundle.BundleError({
                message: `Failed to find cwd for bundle: ${realMain}`,
                cause,
            })), Effect.provide(context)),
            plugins: [
                rebindEsmExternalRequirePlugin(cloudflareRolldown({
    ...
    const sanitizeMain = (main) => resolveMainPath(main).pipe(Effect.mapError((cause) => new Bundle.BundleError({
        message: `Failed to find real path for bundle: ${main}`,
        cause,
    })), Effect.provide(context));
    return {
        build: flow(makeOptions, Effect.flatMap((resolved) => Bundle.build(resolved.inputOptions, resolved.outputOptions, resolved.extraOptions))),
        watch: flow(makeOptions, Stream.fromEffect, Stream.flatMap((resolved) => Bundle.watch(resolved.inputOptions, resolved.outputOptions, resolved.extraOptions))),
    };
});
export const makeEffectVirtualEntry = (exports, stack) => {
    const doClasses = [];
    const wfClasses = [];
