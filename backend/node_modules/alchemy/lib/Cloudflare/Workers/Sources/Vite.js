...
import { viteBuildOutputPlugin, } from "../../../Bundle/Vite.js";
...
import { findAvailablePort, initialCwd } from "../../../Util/Node.js";
import { sha256Object } from "../../../Util/sha256.js";
import { readAssets } from "../Assets.js";
import { runViteBuildChild } from "../ViteChild.js";
...
export const viteDev = (rootDir = initialCwd, env, pluginOptions, serverOptions) => Effect.gen(function* () {
    yield* Effect.sync(() => {
        process.env[ALCHEMY_CLOUDFLARE_VITE_INJECTED] = "1";
    });
    ...
    const server = serverOptions.port === 0 && !viteSupportsPortZero(vite.version)
        ? {
            ...serverOptions,
            port: yield* findAvailablePort(typeof serverOptions.host === "string"
                ? serverOptions.host
                : undefined).pipe(Effect.orDie),
        }
        : serverOptions;
    return yield* Effect.acquireRelease(ConsoleService.consoleWith((console) => Effect.promise(async () => {
        ...
        return devServer;
    })), (devServer) => Effect.promise(async () => {
 ...
 */
export const viteBuild = (rootDir = initialCwd, env, pluginOptions) => ConsoleService.consoleWith((console) => Effect.gen(function* () {
    ...
    }, (channel, line) => channel === "stderr" ? console.error(line) : console.log(line));
    return {
 ...
 */
export const viteBuildInProcess = (rootDir, env, pluginOptions) => Effect.gen(function* () {
...
const getDefine = (env) => Object.fromEntries(Object.entries(env).flatMap(([key, raw]) => {
 ...
 */
async function loadVite(projectRoot = initialCwd) {
    try {
        ...
        return await import(__rewriteRelativeImportExtension(/* @vite-ignore */ viteUrl.href));
    }
    catch {
 ...
 */
const resolveViteEnv = (env) => Effect.gen(function* () {
    return Object.fromEntries((yield* Effect.all(Object.entries(env).map(Effect.fn(function* ([key, value]) {
        return [
            key,
            typeof value === "string"
                ? value
 ...
 */
export const hashViteInput = Effect.fn(function* (rootDir = initialCwd, options, additionalWorkspaces) {
    ...
    const relativeToRoot = (cwd) => path
        .relative(resolvedRoot, path.resolve(resolvedRoot, cwd))
        .replaceAll("\\", "/");
    const hashWorkspaceDirectory = (cwd, memo) => hashDirectory({ cwd: path.resolve(resolvedRoot, cwd), memo }).pipe(Effect.map((hash) => `${relativeToRoot(cwd)}:${hash}`));
    ...
    if (Array.isArray(options?.workspaces)) {
        ...
        ], { concurrency: "unbounded" }).pipe(Effect.flatMap(([root, ...workspaces]) => sha256Object([root, ...workspaces.sort()])), Effect.map((hash) => ({ hash, workspaces: undefined })));
    }
    const [root, workspaces] = yield* Effect.all([hashRoot, additionalWorkspaces], { concurrency: "unbounded" });
    const workspaceHashes = yield* Effect.forEach(workspaces, (cwd) => hashWorkspaceDirectory(cwd), { concurrency: "unbounded" });
 ...
 * workers.
 */
export const makeViteSource = (vite) => ({
    ownsAssets: true,
    build: Effect.fn(function* (ctx) {
        const path = yield* Path.Path;
        const env = yield* resolveViteEnv(ctx.env ?? {});
        const { clientDirectory, serverBundle, externalWorkspaces } = yield* viteBuild(vite.rootDir, env, {
            main: vite.main,
    ...
    hash: Effect.fn(function* (_ctx, previous) {
        const { hash, workspaces } = yield* hashViteInput(vite.rootDir, vite.memo, Effect.succeed(previous?.additionalWorkspaces ?? []));
        return { input: hash, additionalWorkspaces: workspaces };
    }),
    dev: Effect.fn(function* (ctx) {
        const devServer = yield* viteDev(vite.rootDir, ctx.env ?? {}, {
            main: vite.main,
            compatibilityDate: ctx.compatibility.date,
            compatibilityFlags: ctx.compatibility.flags,
            viteEnvironments: vite.viteEnvironments,
