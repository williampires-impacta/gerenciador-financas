 ...
 */
export const isPythonMain = (main) => typeof main === "string" && main.split("?")[0].endsWith(".py");
/**
...
export const pythonVersionForCompatibility = (compatibility) => {
    ...
    if (compatibility.flags.includes("python_workers_20250116") ||
...
const uvError = (message) => (cause) => new Bundle.BundleError({ message, cause });
/**
 ...
 */
const runUv = Effect.fn(function* (args, options) {
    const result = yield* exec(ChildProcess.make("uv", args, {
        cwd: options.cwd,
        env: options.env,
        extendEnv: true,
    })).pipe(Effect.mapError(uvError("Failed to run `uv` — Python Workers use uv (>= 0.8.10) to vendor " +
 ...
 */
const resolvePythonModulesDir = Effect.fn(function* (options) {
    ...
    yield* Effect.logDebug(`[${options.id}] Vendoring Python dependencies for ${pythonVersion} (emscripten-wasm32) with uv`);
    yield* fs
...
const orDefault = (effect, fallback) => Effect.catchCause(effect, () => Effect.succeed(fallback));
const globFiles = (patterns, options) => Effect.tryPromise({
    try: () => fg.glob(patterns, {
        cwd: options.cwd,
        onlyFiles: true,
 ...
 */
export const readPythonWorkerBundle = Effect.fn(function* (options) {
    const fs = yield* FileSystem.FileSystem;
    const main = yield* resolveMainPath(options.main);
    const root = path.dirname(main);
    const entryName = path.basename(main);
    const readTextModule = Effect.fn(function* (name) {
        const content = yield* fs
            .readFileString(path.join(root, name))
            .pipe(Effect.mapError(uvError(`Failed to read Python worker module "${path.join(root, name)}"`)));
    ...
    }).pipe(Effect.map((names) => names.filter((name) => name !== entryName)), Effect.flatMap(Effect.forEach(readTextModule, { concurrency: "unbounded" })));
    const vendored = resolvePythonModulesDir({ ...options, root }).pipe(Effect.flatMap((vendorDir) => {
        if (!vendorDir) {
            return Effect.succeed([]);
        }
        const readVendoredModule = Effect.fn(function* (name) {
            const content = yield* fs
                .readFile(path.join(vendorDir, name))
                .pipe(Effect.mapError(uvError(`Failed to read vendored module "${path.join(vendorDir, name)}"`)));
            const hash = yield* sha256(content);
            return {
    ...
    const [entry, additional, vendoredFiles] = yield* Effect.all([readTextModule(entryName), sources, vendored], { concurrency: "unbounded" });
 ...
 */
export const watchPythonWorkerBundle = (options) => watchBundleDirectory({
    main: options.main,
 ...
 */
export const makePythonSource = (main) => {
    const pythonOptions = (ctx) => ({
        id: ctx.id,
        main,
