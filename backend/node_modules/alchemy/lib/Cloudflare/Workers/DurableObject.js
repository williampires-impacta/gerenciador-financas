...
import { isWorker, Worker, WorkerEnvironment, } from "./Worker.js";
export const isDurableObjectExport = (value) => typeof value === "object" && value?.kind === "durableObject";
const TypeId = "Cloudflare.DurableObject";
export const isDurableObjectLike = (value) => typeof value === "object" && value?.kind === TypeId;
const resolveTransferSourceRef = (source, depth = 0) => {
    ...
    if (typeof source === "function" && depth < 8) {
        return resolveTransferSourceRef(source(), depth + 1);
    }
    throw new Error("Invalid transferredFrom entry: pass the former host's logical id or script name, its Worker class or resource, a thunk of one of those, or a string Output.");
};
/**
 ...
 * @internal
 */
export const normalizeTransferredFrom = (value) => value === undefined
    ? undefined
    : (Array.isArray(value)
        ? value
        : [value]).map((source) => resolveTransferSourceRef(source));
export class DurableObjectScope extends Context.Service()("Cloudflare.DurableObject") {
}
/**
 ...
 */
export const DurableObject = taggedFunction(DurableObjectScope, function (...args) {
    if (args.length === 0) {
        return (name, propsOrImpl) => 
        // @ts-expect-error
    ...
    const binding = (scriptName, transferredFrom) => Effect.gen(function* () {
        ...
        return {
            ...
            LogicalId: namespace,
            name: namespace,
            namespaceId: worker.durableObjectNamespaces.pipe(Output.map((durableObjectNamespaces) => durableObjectNamespaces?.[namespace])),
            getByName: (name, options) => makeRpcStub(binding.getByName(name, options)),
    ...
    const classProps = isClassForm && !Effect.isEffect(propsOrImpl)
        ? propsOrImpl
        : undefined;
    const make = Effect.fn(function* (impl) {
        ...
        const constructor = impl.pipe(Effect.provide(Layer.succeed(DurableObjectScope, self)));
        if (phase === "plan") {
            // during plan time, we evaluate the constructor with a mock DurableObjectState
            // to trigger discovery of bindings
            yield* constructor.pipe(Effect.provide(Layer.succeed(DurableObjectState, 
            // mock during plan time
            fromDurableObjectState({ storage: {} }))));
        }
    ...
    if (!isClassForm && !Effect.isEffect(args[1])) {
        ...
        return {
            kind: TypeId,
            name: namespace,
            className: args[1]?.className || namespace,
            scriptName: args[1]?.scriptName,
            transferredFrom: args[1]?.transferredFrom,
        };
    }
    else if (Effect.isEffect(propsOrImpl)) {
        // inline Effect DO
        return effectClass(Effect.tap(binding(), () => make(propsOrImpl).pipe(Effect.provideService(DurableObjectState, fromDurableObjectState(
        ...
        return class extends effectClass(tag) {
            static make = (impl) => Layer.effect(tag, make(impl));
            static from = (worker) => {
                ...
                const resolved = typeof worker === "string"
                    ? Effect.succeed(worker)
                    : asEffect(worker);
                return resolved.pipe(Effect.flatMap((w) => binding(typeof w === "string" ? w : w.workerName)));
            };
        };
    }
});
