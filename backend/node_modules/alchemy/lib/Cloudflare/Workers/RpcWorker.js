...
import { Worker as WorkerCtor, WorkerEnvironment } from "./Worker.js";
...
export class RpcWorkerScope extends Context.Service()("Cloudflare.RpcWorker") {
}
...
const bind = (workerEff) => Effect.gen(function* () {
    const schema = workerEff[SchemaSymbol];
    if (!schema) {
        return yield* Effect.die(new Error("RpcWorker.bind: passed value isn't an RpcWorker class — no schema attached."));
    }
    const worker = (yield* workerEff);
    ...
    const makeFreshClient = Effect.gen(function* () {
        ...
        const stub = env[worker.LogicalId];
        const httpClient = HttpClient.make((req) => Effect.promise((signal) => stub.fetch(new Request(req.url, {
            method: req.method,
        ...
        }).pipe(Layer.provide(RpcSerialization.layerNdjson), Layer.provide(Layer.succeed(HttpClient.HttpClient, httpClient)));
...
export const RpcWorker = (() => {
    const fn = (...args) => {
        ...
        if (args.length === 0) {
            return (...inner) => {
                if (inner.length === 2) {
                    const [id, props] = inner;
                    return buildModular(id, props);
                }
                const [id, props, impl] = inner;
    ...
    const tagged = taggedFunction(RpcWorkerScope, fn);
    tagged.bind = bind;
    return tagged;
})();
const wrapImpl = (impl) => 
...
const buildModular = (id, props) => {
    ...
    const Underlying = WorkerCtor()(id);
    ...
    const klass = class extends effectClass(Underlying) {
        static make = (props, impl) => Underlying.make(props, wrapImpl(impl));
    };
    klass[SchemaSymbol] = schema;
    return klass;
};
const build = (id, props, impl) => {
    const { schema, ...workerProps } = props;
    // Wrap the user's HttpEffect-returning Effect into the `{ fetch }`
    // shape `Cloudflare.Worker` expects.
    const wrappedImpl = Effect.map(impl, (fetch) => ({ fetch }));
    ...
    const underlying = WorkerCtor()(id, workerProps, wrappedImpl);
    ...
    const klass = effectClass(underlying);
    klass[SchemaSymbol] = schema;
    return klass;
};
