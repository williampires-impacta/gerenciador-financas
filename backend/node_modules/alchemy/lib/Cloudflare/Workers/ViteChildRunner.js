...
import { Artifacts, createArtifactStore, makeScopedArtifacts, } from "../../Artifacts.js";
import { CloudflareAuth } from "../Auth/AuthProvider.js";
import * as Credentials from "../Credentials.js";
import * as RpcServerEnvironment from "../../Local/RpcServerEnvironment.js";
import { PlatformServices, runMain } from "../../Util/PlatformServices.js";
import { materializeRuntimeBindings } from "./RuntimeBindings.js";
import { loadSource, SourceProviderError } from "./Source.js";
import * as Vite from "./Sources/Vite.js";
...
const program = Effect.scoped(Effect.gen(function* () {
    ...
    const { compatibility, main, viteEnvironments, hasAssets, bindingDescriptors, devRemote, devAccess, ...runtimeWorker } = config.worker;
    ...
    const url = source
        ...
        // module a fresh one.
        Effect.provideService(Artifacts, makeScopedArtifacts(createArtifactStore(), source.id)), Effect.flatMap((provider) => provider.dev({
            id: source.id,
            ...
            runtimeContext,
        })), Effect.flatMap((handle) => handle.mode === "server"
            ? Effect.succeed(handle.url.toString())
            : Effect.fail(new SourceProviderError({
                provider: source.descriptor.provider,
                message: "A source declared devMode 'server' but returned a bundle-mode dev handle.",
            }))))
        : yield* Vite.viteDev(config.rootDir, config.env, {
            main,
...
runMain(program.pipe(Effect.provide(RpcServerEnvironment.fromEnv().pipe(Layer.provideMerge(PlatformServices)))));
//# sourceMappingURL=ViteChildRunner.js.map