...
import {} from "../Fetcher.js";
import { DurableObjectState } from "../Workers/DurableObjectState.js";
import { ContainerCrashedError, ContainerError, ContainerRateLimitedError, ContainerTag, NoContainerInstanceError, } from "./Container.js";
/**
...
const getStartCoordination = (key) => {
...
export const layer = (container, options) => {
    ...
    return Layer.effect(ContainerTag(id), startContainer(container, options));
};
/**
 * Runs the Container in a Durable Object and monitors it, providing a durable fetch and RPC interface to it.
 */
export const startContainer = Effect.fn(function* (containerEff, options) {
    const bindEff = containerEff["~alchemy/Container/Binding"];
    const bound = yield* (bindEff ?? containerEff);
    const container = Effect.isEffect(bound)
        ? yield* bound
        : bound;
    ...
    const errString = (e) => (e instanceof Error ? e.message : String(e)).toLowerCase();
    const isNoInstanceError = (e) => errString(e).includes("there is no container instance that can be provided to this durable object");
    const isRateLimitedError = (e) => errString(e).includes("you are requesting too many containers per second");
    // Map a raw defect to a typed readiness error (native classifies the same
    // message phrases via `isErrorOfType`).
    const classifyDefect = (defect, portNumber) => isRateLimitedError(defect)
    ...
    const coordinationKey = doState.container ?? doState;
    const { startMutex, readyPorts } = getStartCoordination(coordinationKey);
    const launchMonitor = Effect.forkDetach(container.monitor().pipe(Effect.flatMap(() => Effect.logInfo("Container monitor exited")), Effect.catchTag("ContainerError", (error) => Effect.logError(`Container monitor error: ${error.message}`)), 
    ...
    const startOnce = Semaphore.withPermits(startMutex, 1)(Effect.gen(function* () {
        ...
        yield* container.start(options).pipe(Effect.catchDefect((defect) => Effect.fail(classifyDefect(defect, -1))), 
    ...
    const probePort = (portNumber) => container.getTcpPort(portNumber).pipe(Effect.andThen((port) => port.fetch(HttpClientRequest.get("http://containerstarthealthcheck"))), Effect.timeout(READINESS_PROBE_TIMEOUT), Effect.catchDefect((defect) => container.running.pipe(Effect.andThen((running) => Effect.fail(!running &&
    ...
    Effect.catchTag(["TimeoutError", "HttpClientError"], (cause) => container.running.pipe(Effect.andThen((running) => Effect.fail(!running
        ? new ContainerCrashedError({
            message: `Container exited while waiting for port ${portNumber}`,
            cause,
        })
    ...
    const isTransientReadiness = (e) => e._tag === "ContainerError" || e._tag === "NoContainerInstanceError";
    ...
    const waitForPort = (portNumber) => readyPorts.has(portNumber)
        ? Effect.void
        : probePort(portNumber).pipe(Effect.tapError((err) => Effect.logDebug(`Container not ready (will retry): ${err}`)), Effect.retry({
    ...
    const getTcpPort = (portNumber) => Effect.succeed({
        ...
        ensureReady(portNumber).pipe(Effect.andThen(() => container.getTcpPort(portNumber)), Effect.andThen((port) => port.fetch(request)), Effect.catchDefect((defect) => Effect.fail(new ContainerError({
