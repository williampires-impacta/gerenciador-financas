...
import { generateLocalId, LOCAL_ENTRY_URL } from "../LocalRuntime.js";
import { isInlineDockerfile } from "../../Docker/Dockerfile.js";
import { createContainerApplicationName, makeContainerEnv, materializeInlineDockerfileContext, prepareContainerBuildContext, validateContainerImageProps, } from "./ContainerBundle.js";
 ...
 */
export const LocalContainerProvider = () => RpcProvider.effect(ContainerPlatform, LOCAL_ENTRY_URL, Effect.gen(function* () {
    ...
    const prepareImage = (id, news) => Effect.gen(function* () {
        ...
        if (news.dockerfile !== undefined &&
            isInlineDockerfile(news.dockerfile)) {
            const content = news.dockerfile.content;
            if (typeof content !== "string") {
                return yield* Effect.die(new Error("Inline `dockerfile` content is an unresolved Output at image-build time — its dependencies have not resolved yet. Break the cycle or inline the resolved value."));
            }
    ...
    const placeholderConfiguration = (props, env) => normalizeNulls({
        image: "local",
    ...
    const makeAttributes = Effect.fn(function* ({ id, news, bindings, output, }) {
...
//# sourceMappingURL=LocalContainerProvider.js.map