...
import { getStableContextDir } from "../../Bundle/TempRoot.js";
...
import { normalizeNulls } from "../../Util/stable.js";
import { CloudflareEnvironment } from "../CloudflareEnvironment.js";
import { isLiveId } from "../LocalRuntime.js";
import { CloudflareLogs } from "../Logs.js";
import { isInlineDockerfile } from "../../Docker/Dockerfile.js";
import { buildFinalDockerfile, bundleContainerProgram, containerEnvPreamble, createContainerApplicationName, makeContainerEnv, materializeInlineDockerfileContext, validateContainerImageProps, } from "./ContainerBundle.js";
import { ContainerPlatform } from "./ContainerPlatform.js";
/**
 ...
 */
const isTargetRegistryRef = (image, registryId) => image.startsWith(`${registryId}/`);
/**
 ...
 */
const normalizePrepushedRef = (image, registryId, accountId) => {
    if (registryId !== "registry.cloudflare.com")
        return image;
...
export const LiveContainerProvider = () => Provider.effect(ContainerPlatform, Effect.gen(function* () {
    ...
    const findApplicationByName = Effect.fn(function* (name) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* Containers.listContainerApplications({ accountId }).pipe(Effect.map((apps) => apps.find((app) => app.name === name)));
    });
    const findApplicationByNamespace = Effect.fn(function* (namespaceId) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* Containers.listContainerApplications({ accountId }).pipe(Effect.map((apps) => apps.find((app) => app.durableObjects?.namespaceId === namespaceId)));
    });
    ...
    const waitForApplicationDeleted = (name, deletedId) => findApplicationByName(name).pipe(Effect.repeat({
        schedule: containerApplicationReadinessSchedule,
        until: (app) => app?.id !== deletedId,
    }), Effect.asVoid);
    const desiredConfiguration = (props, env, imageRef) => normalizeNulls({
        image: imageRef,
    ...
    const computeImage = Effect.fn(function* (id, props, env) {
        ...
        const makeRef = (imageHash) => `${registryId}/${accountId}/${repositoryName}:${imageHash}`;
        ...
        if (props.main) {
            ...
            const finalDockerfile = buildFinalDockerfile(yield* containerEnvPreamble(props), runtime, props.external, props.autoInstallExternals);
            ...
            const contextDir = yield* getStableContextDir(process.cwd(), dotAlchemy, `${id}-container`);
            return {
                build: { kind: "effectful", files },
        ...
        if (props.image) {
            ...
            if (isTargetRegistryRef(props.image, registryId)) {
                ...
                return {
                    build: { kind: "prepushed", image: prepushedRef },
            ...
            return {
                build: { kind: "remote", image: props.image },
                imageRef: makeRef(imageHash),
                imageHash,
        ...
        if (props.dockerfile !== undefined &&
            ...
            if (typeof content !== "string") {
                return yield* Effect.die(new Error("Inline `dockerfile` content is an unresolved Output at image-build time — its dependencies have not resolved yet (e.g. during precreate of a circular binding). Break the cycle or inline the resolved value."));
            }
            ...
            return {
                build: { kind: "external", context, dockerfile },
                imageRef: makeRef(imageHash),
                imageHash,
    ...
    const buildAndPushImage = Effect.fn(function* (id, props, build, imageRef, session) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        if (!username) {
            return yield* Effect.fail(new Error("Cloudflare registry credentials did not include a username."));
        }
    ...
    const maybeCreateRollout = Effect.fn(function* ({ applicationId, configuration, rollout, }) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        const strategy = rollout?.strategy ?? "immediate";
        const stepPercentage = strategy === "immediate" ? 100 : (rollout?.stepPercentage ?? 25);
        yield* retryForContainerApplicationReadiness("rollout", applicationId, Containers.createContainerApplicationRollout({
            accountId,
            applicationId,
            description: strategy === "immediate"
                ? "Immediate update"
                : "Progressive update",
            strategy: "rolling",
    ...
    const createApplication = Effect.fn(function* ({ id, news, bindings, name, configuration, durableObjects, session, }) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        if (existingByName) {
            yield* Effect.logInfo(`Cloudflare Container create: adopting existing application ${name}`);
        ...
        const adoptExistingByName = Effect.gen(function* () {
            yield* Effect.logInfo(`Cloudflare Container create: application ${name} already exists, adopting`);
            const existing = yield* findApplicationByName(name);
            if (!existing) {
                return yield* Effect.fail(new Error(`Container application "${name}" already exists but could not be found for adoption.`));
            }
            return yield* upsertApplication({
                id,
                news,
        ...
        const application = yield* Containers.createContainerApplication({
            ...
            configuration,
            durableObjects,
        }).pipe(Effect.catchTag("DurableObjectAlreadyHasApplication", () => durableObjects
            ? Effect.gen(function* () {
                ...
                if (!existing) {
                    return yield* Effect.fail(new Error(`Container application for Durable Object namespace "${durableObjects.namespaceId}" already exists but could not be found for adoption.`));
                }
                return yield* upsertApplication({
                    id,
                    news,
            ...
            : Effect.fail(new Error("Durable Object namespace already has a container application. Set AdoptPolicy to adopt it."))), Effect.catchIf((e) => "message" in e &&
            String(e.message).includes("already exists"), () => adoptExistingByName), Effect.tapError((error) => Effect.logError(`Cloudflare Container create error: ${describeError(error)}`)));
        return "applicationId" in application
            ? application
            : toAttributes(application);
    });
    const upsertApplication = Effect.fn(function* ({ id, news, bindings, existing, durableObjects, session, }) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        const application = yield* retryForContainerApplicationReadiness("update", existing.applicationId, Containers.updateContainerApplication({
            accountId,
        ...
        // does not re-collide.
        Effect.catchTag("ContainerApplicationNotFound", () => Effect.gen(function* () {
            yield* Effect.logInfo(`Cloudflare Container update: ${existing.applicationName} no longer exists, creating fresh`);
        ...
        if (!deepEqual(existing.configuration, configuration)) {
            yield* Effect.logInfo(`Cloudflare Container update: creating rollout for ${updated.applicationName}`);
    ...
    const getDurableObjects = (bindings) => {
        const dos = bindings.flatMap((b) => b.data.durableObjects ? [b.data.durableObjects] : []);
        ...
        const uniqueDos = dos.filter((d, i, arr) => arr.findIndex((other) => other.namespaceId === d.namespaceId) === i);
        ...
        return Effect.die(new Error(`A Container can only be bound to one Durable Object namespace. Found ${uniqueDos.length} unique namespaces in bindings: ${uniqueDos.map((d) => d.namespaceId).join(", ")}`));
    };
    return ContainerPlatform.Provider.of({
        ...
        diff: Effect.fn(function* ({ id, olds = {}, news = {}, output, newBindings, oldBindings, }) {
            ...
            if (!isLiveId(output.applicationId)) {
                ...
                return { action: "update", stables: ["accountId"] };
            }
            const { imageHash, dev } = yield* computeImage(id, news, makeContainerEnv(news, accountId, newBindings));
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, bindings, output, session, }) {
            ...
            const name = output?.applicationName ??
            ...
            if (existing && !deepEqual(existing.durableObjects, durableObjects)) {
                ...
                yield* Effect.logInfo(`Cloudflare Container reconcile: recreating ${name} to attach durable object binding`);
                yield* session.note(`Recreating container application ${name} with durable object binding...`);
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const readByName = (name) => Effect.gen(function* () {
            ...
            if (output?.applicationId) {
                ...
                attrs = yield* Containers.getContainerApplication({
                    ...
                    hash: output.hash,
                    dev: output.dev,
                })), Effect.catchTag("ContainerApplicationNotFound", () => readByName(output.applicationName)));
        ...
        list: () => Effect.gen(function* () {
            ...
            return yield* Containers.listContainerApplications({
                accountId,
            }).pipe(Effect.map((apps) => apps.map((app) => toAttributes(app))), 
        ...
        tail: ({ output }) => telemetry.tailStream({
            accountId: output.accountId,
...
const containerFilters = (applicationId) => [
    {
        key: "$metadata.type",
        operation: "eq",
        type: "string",
...
const resolveDurableObjectApplicationRecovery = ({ namespaceId, expectedName, existingName, }) => {
    if (!existingName) {
        return {
            canAdopt: false,
...
const isContainerApplicationNotFound = (error) => typeof error === "object" &&
    error !== null &&
    "_tag" in error &&
    error._tag === "ContainerApplicationNotFound";
export const retryForContainerApplicationReadiness = (operation, applicationId, effect) => effect.pipe(Effect.tapError((error) => isContainerApplicationNotFound(error)
    ? Effect.logDebug(`Cloudflare Container ${operation}: application ${applicationId} not found yet, retrying`)
...
const toAttributes = (application) => ({
    ...
    constraints: normalizeNulls(application.constraints),
    affinities: normalizeNulls(application.affinities),
    configuration: normalizeNulls(application.configuration),
    durableObjects: normalizeNulls(application.durableObjects),
