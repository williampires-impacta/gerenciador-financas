 ...
 */
export const makeContainerEnv = (props, accountId, bindings = []) => {
    const env = {};
 ...
 */
export const createContainerApplicationName = (id, name) => Effect.suspend(() => {
    if (name)
 ...
 */
export const validateContainerImageProps = (props) => {
    const df = props.dockerfile;
    const hasInline = df !== undefined && isInlineDockerfile(df);
    if (props.main) {
        if (props.image !== undefined && df !== undefined) {
            return Effect.die(new Error("`image` and `dockerfile` are mutually exclusive with `main` — both pick the environment for the bundled program; declare one."));
        }
        if (df !== undefined && !hasInline) {
            return Effect.die(new Error("A `dockerfile` PATH cannot be combined with `main` on Cloudflare containers — use `Dockerfile.inline` content or `image` to pick the bundled program's environment."));
        }
        if (props.context !== undefined) {
            return Effect.die(new Error("`context` cannot be combined with `main` — the build context for a bundled program is generated by Alchemy."));
        }
        return Effect.void;
    }
    if (props.image !== undefined && df !== undefined) {
        return Effect.die(new Error("`image` (pre-built remote image) and `dockerfile` (build your own) are mutually exclusive — declare one."));
    }
    if (props.image !== undefined && props.context !== undefined) {
        return Effect.die(new Error("`image` (pre-built remote image) and `context` (Dockerfile build) are mutually exclusive — declare one."));
    }
    if (hasInline && props.context !== undefined) {
        return Effect.die(new Error("Inline `dockerfile` content cannot be combined with `context` — inline content has no build context; use a `dockerfile` PATH for context-relative builds."));
    }
    return Effect.void;
};
/**
 ...
 */
export const containerEnvPreamble = (props) => {
    const df = props.dockerfile;
    if (df !== undefined && isInlineDockerfile(df)) {
        const content = df.content;
        if (typeof content !== "string") {
            return Effect.die(new Error("Inline `dockerfile` content is an unresolved Output at image-build time. " +
    ...
    if (ref) {
        if (/\s/.test(ref)) {
            ...
            // broken build.
            return Effect.die(new Error(`\`image\` must be a plain image reference (e.g. "oven/bun:latest"), got: ${JSON.stringify(props.image)}. ` +
 ...
 * entrypoint.
 */
export const buildFinalDockerfile = (envPreamble, runtime, external = [], autoInstallExternals = true) => {
    const base = envPreamble ??
    ...
    const installStep = autoInstallExternals && external.length > 0
 ...
 */
export const materializeInlineDockerfileContext = Effect.fn(function* (id, content) {
 ...
 * `docker build`).
 */
export const bundleContainerProgram = Effect.fn(function* ({ main, runtime, handler = "default", isExternal = false, external = [], outdir, build, }) {
    ...
    const bundleOutput = isExternal
        ...
        : yield* buildBundle(realMain, virtualEntryPlugin((importPath) => `
${runtime === "bun"
            ? `
 ...
 */
export const prepareContainerBuildContext = Effect.fn(function* (id, news) {
    const { dotAlchemy } = yield* AlchemyContext;
    const docker = yield* Docker;
    const path = yield* Path.Path;
    ...
    const dockerfileContent = buildFinalDockerfile(yield* containerEnvPreamble(news), runtime, news.external, news.autoInstallExternals);
...
//# sourceMappingURL=ContainerBundle.js.map