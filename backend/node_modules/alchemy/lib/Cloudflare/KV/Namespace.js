...
export const isNamespace = (value) => isResourceOfType(value, "Cloudflare.KV.Namespace");
/**
 ...
 */
export const Namespace = Resource("Cloudflare.KV.Namespace", {
    aliases: ["Cloudflare.KVNamespace"],
});
export const ProviderLive = () => Provider.succeed(Namespace, {
    ...
    reconcile: Effect.fn(function* ({ id, news = {}, output }) {
        ...
        if (!observed) {
            observed = yield* kv
                ...
                .pipe(Effect.catchTag("NamespaceTitleAlreadyExists", () => Effect.gen(function* () {
                const match = yield* findNamespaceByTitle(title);
                if (match) {
                    return match;
                }
                return yield* Effect.die(`Namespace with title "${title}" already exists but could not be found`);
            })));
        }
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* kv.listNamespaces.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((ns) => ({
            title: ns.title,
            namespaceId: ns.id,
 ...
 */
export const ProviderLocal = () => Provider.succeed(Namespace, {
    stables: ["accountId"],
...
export const NamespaceProvider = () => ProviderLayer.dual(Namespace, {
    local: () => ProviderLocal(),
    live: () => ProviderLive(),
});
const createTitle = (id, title) => Effect.gen(function* () {
...
const findNamespaceByTitle = Effect.fn(function* (title) {
    const { accountId } = yield* yield* CloudflareEnvironment;
    return yield* kv.listNamespaces.items({ accountId }).pipe(Stream.filter((ns) => ns.title === title), Stream.runHead, Effect.map(Option.getOrUndefined));
});
