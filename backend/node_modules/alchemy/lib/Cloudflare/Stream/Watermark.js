...
const TypeId = "Cloudflare.Stream.Watermark";
/**
 ...
 * @category Media
 */
export const Watermark = Resource(TypeId);
/**
 * Returns true if the given value is a Watermark resource.
 */
export const isWatermark = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const WatermarkProvider = () => Provider.succeed(Watermark, {
    stables: [
        "watermarkId",
        "accountId",
        "created",
    ...
    list: Effect.fn(function* () {
        ...
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* stream.listWatermarks.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((w) => toAttributes(w, accountId)))));
    }),
 ...
 */
const getWatermark = (accountId, watermarkId) => stream
 ...
 */
const findByName = (accountId, name) => stream.listWatermarks({ accountId }).pipe(Effect.map((list) => [...list.result]
    .filter((w) => w.name === name)
    .sort((a, b) => (a.created ?? "").localeCompare(b.created ?? ""))
    .at(0)));
const watermarkName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const toAttributes = (watermark, accountId) => ({
    ...
    position: (watermark.position ?? "upperRight"),
