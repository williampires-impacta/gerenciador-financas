...
const TypeId = "Cloudflare.Stream.SigningKey";
/**
...
export const SigningKeyProvider = () => Provider.succeed(SigningKey, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* stream.getKey.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
 ...
 */
const findKey = (accountId, keyId) => stream
    .getKey({ accountId })
