...
const TypeId = "Cloudflare.ResourceSharing.ShareResource";
/**
 ...
 */
export const ShareResource = Resource(TypeId);
/**
 * Returns true if the given value is a ShareResource resource.
 */
export const isShareResource = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const ShareResourceProvider = () => Provider.succeed(ShareResource, {
    stables: [
        "shareResourceId",
        "accountId",
        "shareId",
    ...
    diff: Effect.fn(function* ({ olds, news, output }) {
        ...
        if (typeof oldResourceId === "string" &&
    ...
    read: Effect.fn(function* ({ olds, output }) {
        ...
        const resourceType = olds?.resourceType;
        const resourceId = olds?.resourceId;
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const observed = (output?.shareResourceId
        ...
        if (!observed) {
            ...
            const created = yield* resourceSharing.createResource({
                ...
                resourceId,
                resourceAccountId: news.resourceAccountId ?? acct,
                meta: desiredMeta,
            });
    ...
    list: Effect.fn(function* () {
        ...
        const shares = yield* resourceSharing.listResourceSharings
            ...
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).filter((s) => s.status !== "deleted"))));
        const rows = yield* Effect.forEach(shares, (share) => resourceSharing.listResources
        ...
        Effect.catchTag("ShareNotFound", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
});
/**
 ...
 * `undefined`.
 */
const getEntry = (accountId, shareId, shareResourceId) => resourceSharing.getResource({ accountId, shareId, shareResourceId }).pipe(Effect.map((entry) => entry.status === "deleted" ? undefined : entry), Effect.catchTag("ShareResourceNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findEntry = (accountId, shareId, resourceType, resourceId) => resourceSharing
    .listResources({ accountId, shareId, resourceType, perPage: 50 })
    .pipe(Effect.map((list) => list.result.find((r) => r.resourceId === resourceId &&
    r.status !== "deleted" &&
    r.status !== "deleting")), Effect.catchTag("ShareNotFound", () => Effect.succeed(undefined)));
const toAttributes = (entry, accountId, shareId) => ({
