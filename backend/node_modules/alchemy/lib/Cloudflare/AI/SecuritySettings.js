...
const AiSecuritySettingsTypeId = "Cloudflare.AI.SecuritySettings";
/**
 * Returns true if the given value is a SecuritySettings resource.
 */
export const isSecuritySettings = (value) => Predicate.hasProperty(value, "Type") &&
 ...
 */
export const SecuritySettings = Resource(AiSecuritySettingsTypeId, {
    aliases: ["Cloudflare.AiSecurity.Settings"],
});
export const SecuritySettingsProvider = () => Provider.succeed(SecuritySettings, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => aiSecurity.getAiSecurity({ zoneId }).pipe(Effect.map((observed) => {
        ...
        Effect.catchTag("AiSecurityNotEntitled", () => Effect.succeed(undefined)), Effect.catchTag("ZoneNotAuthorized", () => Effect.succeed(undefined))), { concurrency: 10 });
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        const observed = yield* aiSecurity.getAiSecurity({ zoneId }).pipe(Effect.catchTag("ZoneNotAuthorized", () => Effect.succeed(undefined)), Effect.catchTag("AiSecurityNotEntitled", () => Effect.succeed(undefined)));
        if (observed === undefined)
            return;
