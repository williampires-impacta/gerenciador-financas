...
const TypeId = "Cloudflare.AI.SearchNamespace";
/**
 ...
 * @category AI
 */
export const SearchNamespace = Resource(TypeId, {
    aliases: ["Cloudflare.AiSearch.Namespace"],
});
/**
 * Returns true if the given value is a SearchNamespace resource.
 */
export const isSearchNamespace = (value) => isResourceOfType(value, TypeId);
export const SearchNamespaceProvider = () => Provider.succeed(SearchNamespace, {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (!observed) {
            ...
            const ensured = yield* aisearch
                ...
                .pipe(Effect.map((created) => ({ created: true, ns: created })), Effect.catchTag("NamespaceAlreadyExists", (originalError) => Effect.gen(function* () {
                const existing = yield* getNamespace(acct, name);
                if (!existing)
                    return yield* Effect.fail(originalError);
                return { created: false, ns: existing };
            })));
            if (ensured.created) {
        ...
        // desired shape.
        Effect.catchTag("NamespaceNotFound", () => aisearch.createNamespace({
            accountId: acct,
            name,
            description: props.description,
        })));
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* aisearch.listNamespaces.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
 ...
 */
const getNamespace = (accountId, name) => aisearch
    .readNamespace({ accountId, name })
    .pipe(Effect.catchTag("NamespaceNotFound", () => Effect.succeed(undefined)));
const createNamespaceName = (id, name) => Effect.gen(function* () {
