...
const TypeId = "Cloudflare.AI.GatewayProvider";
/**
 ...
 * @category AI
 */
export const GatewayProvider = Resource(TypeId, {
    aliases: ["Cloudflare.AiGateway.ProviderConfig"],
});
/**
 * Returns true if the given value is a GatewayProvider resource.
 */
export const isGatewayProvider = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const GatewayProviderProvider = () => Provider.succeed(GatewayProvider, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(gatewayIds, (gatewayId) => aiGateway.listProviderConfigs.pages({ accountId, gatewayId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((config) => toAttributes(config, accountId))))), { concurrency: 10 });
        return rows.flat();
    }),
});
/**
...
const reconcileProviderConfig = (desired) => {
    const { accountId, gatewayId, alias, providerSlug, secretId, defaultConfig, rateLimit, rateLimitPeriod, currentId, } = desired;
    const matchesDesired = (attrs) => attrs.alias === alias &&
    ...
    return Effect.gen(function* () {
        ...
        const observed = configs.find((c) => c.id === currentId) ??
 ...
 */
const listProviderConfigs = (accountId, gatewayId) => aiGateway
    .listProviderConfigs({ accountId, gatewayId, perPage: 50 })
    .pipe(Effect.map((page) => page.result));
const createAlias = (id, alias) => Effect.gen(function* () {
