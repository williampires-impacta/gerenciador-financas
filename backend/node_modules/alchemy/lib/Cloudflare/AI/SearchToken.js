...
const TypeId = "Cloudflare.AI.SearchToken";
/**
 ...
 * @category AI
 */
export const SearchToken = Resource(TypeId, {
    aliases: ["Cloudflare.AiSearch.Token"],
});
/**
 * Returns true if the given value is a SearchToken resource.
 */
export const isSearchToken = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const SearchTokenProvider = () => Provider.succeed(SearchToken, {
    ...
    reconcile: Effect.fn(function* ({ id, news, olds, output }) {
        ...
        const body = {
            name,
            cfApiId: news.cfApiId,
            cfApiKey: Redacted.value(news.cfApiKey),
    ...
    Effect.gen(function* () {
        ...
        return yield* aisearch.listTokens
            ...
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => 
 ...
 */
const getToken = (accountId, id) => aisearch.readToken({ accountId, id }).pipe(Effect.map((t) => ({
    id: t.id ?? id,
    name: t.name ?? "",
    cfApiId: t.cfApiId ?? "",
 ...
 */
const findTokenByName = (accountId, name) => aisearch.listTokens.items({ accountId, perPage: 50 }).pipe(Stream.filter((t) => t.name === name), Stream.runHead, Effect.map(Option.getOrUndefined));
const createTokenName = (id, name) => Effect.gen(function* () {
...
const toAttributes = (token, accountId) => ({
    id: token.id,
    accountId,
    name: token.name,
