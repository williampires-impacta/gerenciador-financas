...
const TypeId = "Cloudflare.AI.Evaluation";
/**
...
export const EvaluationProvider = () => Provider.succeed(Evaluation, {
    ...
    diff: Effect.fn(function* ({ id, olds, news, output }) {
        ...
        if (output.gatewayId !== news.gatewayId ||
            output.name !== newName ||
    ...
    read: Effect.fn(function* ({ id, olds, output }) {
        ...
        if (gatewayId === undefined)
            return undefined;
        const knownTypeIds = output?.evaluationTypeIds ??
            olds?.evaluationTypeIds ??
            [];
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        const datasetIds = news.datasetIds;
        ...
        const created = yield* aiGateway
            ...
            .pipe(Effect.catchTag("EvaluationNameAlreadyExists", (originalError) => findByName(accountId, gatewayId, name).pipe(Effect.flatMap((match) => match ? Effect.succeed(match) : Effect.fail(originalError)))));
    ...
    list: Effect.fn(function* () {
        ...
        const gatewayIds = yield* aiGateway.listAiGateways
            .pages({ accountId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((gateway) => gateway.id))));
        const rows = yield* Effect.forEach(gatewayIds, (gatewayId) => aiGateway.listEvaluations.pages({ accountId, gatewayId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((evaluation) => toAttributes(evaluation, accountId, []))))), { concurrency: 10 });
        return rows.flat();
    }),
});
/**
 ...
 * to `undefined`.
 */
const getEvaluation = (accountId, gatewayId, id) => aiGateway
 ...
 */
const findByName = (accountId, gatewayId, name) => aiGateway.listEvaluations
    .items({ accountId, gatewayId, name, perPage: 50 })
    .pipe(Stream.filter((e) => e.name === name), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    .sort((a, b) => a.createdAt.localeCompare(b.createdAt))
    .at(0)));
const createEvaluationName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const toAttributes = (evaluation, accountId, knownTypeIds) => ({
    ...
    evaluationTypeIds: evaluation.results.length > 0
