 ...
 */
export interface ReadFlags extends Binding.Service<ReadFlags, "Cloudflare.Flagship.ReadFlags", (app: App) => Effect.Effect<ReadFlagsClient>> {
}
export declare const ReadFlags: ReadFlags;
declare const FlagshipError_base: new <A extends Record<string, any> = {}>(args: import("effect/Types").VoidIfEmpty<{ readonly [P in keyof A as P extends "_tag" ? never : P]: A[P]; }>) => import("effect/Cause").YieldableError & {
    readonly _tag: "FlagshipError";
} & Readonly<A>;
export declare class FlagshipError extends FlagshipError_base<{
    message: string;
    cause: unknown;
}> {
}
...
export interface ReadFlagsClient {
     ...
     */
    get(flagKey: string, defaultValue?: unknown, context?: EvaluationContext): FlagshipEffect<unknown>;
    /**
     * Get a boolean flag value, falling back to `defaultValue` when evaluation
     * fails or the flag type does not match.
     */
    getBooleanValue(flagKey: string, defaultValue: boolean, context?: EvaluationContext): FlagshipEffect<boolean>;
    /**
     * Get a string flag value, falling back to `defaultValue` when evaluation
     * fails or the flag type does not match.
     */
    getStringValue(flagKey: string, defaultValue: string, context?: EvaluationContext): FlagshipEffect<string>;
    /**
     * Get a number flag value, falling back to `defaultValue` when evaluation
     * fails or the flag type does not match.
     */
    getNumberValue(flagKey: string, defaultValue: number, context?: EvaluationContext): FlagshipEffect<number>;
    /**
     * Get a typed object flag value, falling back to `defaultValue` when
     * evaluation fails or the flag type does not match.
     */
    getObjectValue<T extends object>(flagKey: string, defaultValue: T, context?: EvaluationContext): FlagshipEffect<T>;
    /**
     * Get a boolean flag value with full evaluation details (variant, reason,
     * error code).
     */
    getBooleanDetails(flagKey: string, defaultValue: boolean, context?: EvaluationContext): FlagshipEffect<EvaluationDetails<boolean>>;
    /**
     * Get a string flag value with full evaluation details (variant, reason,
     * error code).
     */
    getStringDetails(flagKey: string, defaultValue: string, context?: EvaluationContext): FlagshipEffect<EvaluationDetails<string>>;
    /**
     * Get a number flag value with full evaluation details (variant, reason,
     * error code).
     */
    getNumberDetails(flagKey: string, defaultValue: number, context?: EvaluationContext): FlagshipEffect<EvaluationDetails<number>>;
    /**
     * Get a typed object flag value with full evaluation details (variant,
     * reason, error code).
     */
    getObjectDetails<T extends object>(flagKey: string, defaultValue: T, context?: EvaluationContext): FlagshipEffect<EvaluationDetails<T>>;
}
export {};
//# sourceMappingURL=ReadFlags.d.ts.map