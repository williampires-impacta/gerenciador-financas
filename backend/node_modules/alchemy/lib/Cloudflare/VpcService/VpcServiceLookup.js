...
import { formatVpcService } from "./VpcService.js";
const toLookup = (attrs) => ({
    ...attrs,
 ...
 * @category Network
 */
export const lookup = (props) => Output.fromEffect(Effect.gen(function* () {
    const { accountId } = yield* yield* CloudflareEnvironment;
    if ("name" in props) {
        const match = yield* connectivity.listDirectoryServices
            .items({ accountId })
            .pipe(Stream.filter((s) => s.name === props.name), Stream.runHead, Effect.map(Option.getOrUndefined));
        if (!match) {
...
//# sourceMappingURL=VpcServiceLookup.js.map