...
const TypeId = "Cloudflare.Ssl.CertificatePack";
/**
 ...
 */
export const CertificatePack = Resource(TypeId);
/**
 * Returns true if the given value is a CertificatePack resource.
 */
export const isCertificatePack = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const CertificatePackProvider = () => Provider.succeed(CertificatePack, {
    ...
    diff: Effect.fn(function* ({ olds, news, output }) {
        ...
        if (olds.certificateAuthority !== news.certificateAuthority ||
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (news.cloudflareBranding !== undefined &&
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => ssl.listCertificatePacks
 ...
 * 7003) to `undefined`.
 */
const getPack = (zoneId, certificatePackId) => ssl.getCertificatePack({ zoneId, certificatePackId }).pipe(Effect.map((pack) => pack), Effect.catchTag(["CertificatePackNotFound", "InvalidRoute"], () => Effect.succeed(undefined)));
/**
 ...
 */
const findByHosts = (zoneId, hosts) => ssl.listCertificatePacks.items({ zoneId, status: "all" }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((pack) => pack.type === "advanced" && sameHosts(pack.hosts, hosts))), Effect.catchTag("InvalidRoute", () => Effect.succeed(undefined)));
...
const toAttributes = (zoneId, pack, validationMethod) => ({
