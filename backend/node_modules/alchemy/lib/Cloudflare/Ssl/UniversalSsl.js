...
const TypeId = "Cloudflare.Ssl.UniversalSsl";
/**
 ...
 */
export const UniversalSsl = Resource(TypeId);
/**
 * Returns true if the given value is a UniversalSsl resource.
 */
export const isUniversalSsl = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
/**
 ...
 */
const patchUniversal = (input) => ssl.patchUniversalSetting(input).pipe(Effect.retry({
    while: (e) => e._tag === "TooManyRequests",
    schedule: Schedule.spaced("20 seconds"),
    times: 6,
}));
export const UniversalSslProvider = () => Provider.succeed(UniversalSsl, {
    nuke: { singleton: true },
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => ssl.getUniversalSetting({ zoneId }).pipe(Effect.map((observed) => {
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        if (observedEnabled(observed) === initialEnabled)
            return;
        yield* patchUniversal({ zoneId, enabled: initialEnabled }).pipe(Effect.catchTag("InvalidRoute", () => Effect.void));
    }),
});
/**
 ...
 */
const observedEnabled = (setting) => setting.enabled ?? true;
//# sourceMappingURL=UniversalSsl.js.map