...
const TypeId = "Cloudflare.Iam.UserGroupMembership";
/**
 ...
 */
export const UserGroupMembership = Resource(TypeId);
/**
 * Returns true if the given value is an UserGroupMembership resource.
 */
export const isUserGroupMembership = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const UserGroupMembershipProvider = () => Provider.succeed(UserGroupMembership, {
    ...
    list: Effect.fn(function* () {
        ...
        const groups = yield* iam.listUserGroups.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => page.result ?? [])));
        const rows = yield* Effect.forEach(groups, (group) => iam.listUserGroupMembers
            .pages({ accountId, userGroupId: group.id })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((member) => toAttributes(member, group.id, accountId)))), 
        // Group removed out-of-band between enumeration and member
        // listing — skip it.
        Effect.catchTag("UserGroupNotFound", () => Effect.succeed([])), 
        ...
        Effect.catchTag("PolicyValidationFailed", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
    diff: Effect.fn(function* ({ olds, news }) {
        ...
        if (typeof olds?.userGroup === "string" &&
            olds.userGroup !== news.userGroup) {
            return { action: "replace" };
        }
        if (typeof olds?.memberId === "string" &&
    ...
    reconcile: Effect.fn(function* ({ news }) {
        ...
        const userGroupId = news.userGroup;
        const memberId = news.memberId;
        ...
        const ensure = Effect.gen(function* () {
            ...
            const observed = yield* iam
                ...
                .pipe(Effect.map((m) => m), Effect.catchTag("UserGroupMemberNotFound", () => Effect.succeed(undefined)));
            if (observed) {
        ...
        return yield* ensure.pipe(Effect.retry({
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        Effect.catchTag(["UserGroupMemberNotFound", "UserGroupNotFound", "InvalidMember"], () => Effect.void));
    }),
});
/**
 ...
 */
const getMembership = (accountId, userGroupId, memberId) => iam.getUserGroupMember({ accountId, userGroupId, memberId }).pipe(Effect.map((m) => m), Effect.catchTag(["UserGroupMemberNotFound", "UserGroupNotFound"], () => Effect.succeed(undefined)));
const toAttributes = (member, userGroupId, accountId) => ({
    userGroupId,
    memberId: member.id,
    accountId,
