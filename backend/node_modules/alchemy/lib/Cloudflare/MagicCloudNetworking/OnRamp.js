...
const TypeId = "Cloudflare.MagicCloudNetworking.OnRamp";
/**
 ...
 * @category Network
 */
export const OnRamp = Resource(TypeId);
/**
 * Returns true if the given value is an OnRamp resource.
 */
export const isOnRamp = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const OnRampProvider = () => Provider.succeed(OnRamp, {
    stables: [
        "onRampId",
        "accountId",
        "cloudType",
    ...
    diff: Effect.fn(function* ({ olds, news, output }) {
        ...
        if (oldDynamicRouting !== undefined &&
    ...
    read: Effect.fn(function* ({ id, output, olds }) {
        ...
        if (output?.onRampId) {
            ...
            if (observed)
                return toAttributes(observed, acct, destroyOnDelete);
            return undefined;
        }
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (news.manageVpcToHubAttachments !== undefined &&
    ...
    list: Effect.fn(function* () {
        ...
        const ids = yield* mcn.listOnRamps.items({ accountId }).pipe(Stream.map((onramp) => onramp.id), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("FeatureNotEnabled", () => Effect.succeed([])));
        const rows = yield* Effect.forEach(ids, (onRampId) => getOnRamp(accountId, onRampId).pipe(Effect.map((observed) => observed ? toAttributes(observed, accountId, false) : undefined)), { concurrency: 10 });
 ...
 */
const getOnRamp = (accountId, onrampId) => mcn.getOnRamp({ accountId, onrampId }).pipe(Effect.map((onramp) => onramp), Effect.catchTag("OnRampNotFound", () => Effect.succeed(undefined)));
/**
 ...
 * id for determinism.
 */
const findByName = (accountId, name) => mcn.listOnRamps.items({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    .filter((onramp) => onramp.name === name)
    .sort((a, b) => a.id.localeCompare(b.id))
    .at(0)));
const onRampName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const sameIds = (observed, desired) => observed.length === desired.length &&
    [...observed].sort().join(",") === [...desired].sort().join(",");
const toAttributes = (onramp, accountId, destroyOnDelete) => ({
    onRampId: onramp.id,
    accountId,
    name: onramp.name,
    // Distilled widens generated string enums to open unions (`string & {}`).
    cloudType: onramp.cloudType,
    type: onramp.type,
