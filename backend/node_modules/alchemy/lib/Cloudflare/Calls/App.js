...
const TypeId = "Cloudflare.Calls.App";
/**
...
export const AppProvider = () => Provider.succeed(App, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* calls.listSfus.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((app) => toAttributes(app, accountId, Redacted.make(""))))));
    }),
 ...
 */
const getApp = (accountId, appId) => calls
...
const toAttributes = (app, accountId, secret) => ({
    appId: app.uid ?? "",
    accountId,
    secret,
    name: app.name ?? "",
