...
const TypeId = "Cloudflare.Calls.TurnKey";
/**
 ...
 * @category Media
 */
export const TurnKey = Resource(TypeId);
/**
 * Returns true if the given value is a TurnKey resource.
 */
export const isTurnKey = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const TurnKeyProvider = () => Provider.succeed(TurnKey, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* calls.listTurns.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((turnKey) => toAttributes(turnKey, accountId, Redacted.make(""))))));
    }),
 ...
 */
const getTurnKey = (accountId, keyId) => calls
    .getTurn({ accountId, keyId })
    .pipe(Effect.catchTag("TurnKeyNotFound", () => Effect.succeed(undefined)));
const createTurnKeyName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const toAttributes = (turnKey, accountId, key) => ({
    keyId: turnKey.uid ?? "",
    accountId,
    key,
    name: turnKey.name ?? "",
