...
import { detachQueueConsumersOfScript } from "./Consumer.js";
import { generateLocalId, isLiveId, LOCAL_ENTRY_URL, LocalRuntimeState, localRuntimeServices, } from "../LocalRuntime.js";
export const isQueue = (value) => isResourceOfType(value, "Cloudflare.Queues.Queue");
/**
 ...
 */
export const Queue = Resource("Cloudflare.Queues.Queue", {
    aliases: ["Cloudflare.Queue"],
});
export const ProviderLive = () => Provider.succeed(Queue, {
    ...
    reconcile: Effect.fn(function* ({ id, news = {}, output }) {
        ...
        if (!observed) {
            observed = yield* queues
                ...
                .pipe(Effect.catchTag("QueueAlreadyExists", () => Effect.gen(function* () {
                const match = yield* findQueueByName(queueName);
                if (match && match.queueId && match.queueName) {
                    return match;
                }
                return yield* Effect.die(`Queue "${queueName}" already exists but could not be found`);
            })));
        }
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        Effect.catchTag("QueueInUseByWorkerBinding", (cause) => Effect.gen(function* () {
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* queues.listQueues.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
            .filter((q) => q.queueId != null && q.queueName != null)
            .map((q) => ({
            queueId: q.queueId,
...
const createQueueName = (id, name) => Effect.gen(function* () {
    if (name)
        return name;
 ...
 */
const deleteOwnedProducerScripts = Effect.fn(function* (accountId, queueId) {
    const stack = yield* Stack;
    const queue = yield* queues
        .getQueue({ accountId, queueId })
        .pipe(Effect.catchTag(["QueueNotFound", "InvalidRoute"], () => Effect.succeed(undefined)));
    const producerScripts = Array.from(new Set((queue?.producers ?? []).flatMap((producer) => producer.type === "worker" &&
    ...
    for (const scriptName of producerScripts) {
        ...
        if (!tags.has(`alchemy:stack:${stack.name}`) ||
            !tags.has(`alchemy:stage:${stack.stage}`)) {
            continue;
        }
        yield* Effect.logWarning(`Cloudflare Queue delete: removing orphaned worker script ` +
        ...
        // delete recovery.
        Effect.catchTag("QueueConsumerConflict", () => detachQueueConsumersOfScript(accountId, scriptName).pipe(Effect.andThen(workers.deleteScript({ accountId, scriptName, force: true })))), Effect.catchTag("WorkerNotFound", () => Effect.void));
        removed++;
    }
    return removed;
});
...
const findQueueByName = Effect.fn(function* (queueName) {
    const { accountId } = yield* yield* CloudflareEnvironment;
    return yield* queues.listQueues.items({ accountId }).pipe(Stream.filter((q) => q.queueName === queueName), Stream.runHead, Effect.map(Option.getOrUndefined));
});
export const ProviderLocal = () => RpcProvider.effect(Queue, LOCAL_ENTRY_URL, Effect.gen(function* () {
    ...
    return {
        ...
        read: Effect.fn(function* ({ output }) {
            if (!output?.queueId)
                return undefined;
            return MutableHashMap.get(localRuntimeState.queues, output.queueId).pipe(Option.getOrUndefined);
        }),
        reconcile: Effect.fn(function* ({ id, news = {}, output }) {
            const { accountId } = yield* yield* CloudflareEnvironment;
            const queue = {
                ...
                queueId: output?.queueId && !isLiveId(output.queueId)
                    ? output.queueId
                    : generateLocalId(),
...
export const QueueProvider = () => ProviderLayer.dual(Queue, {
