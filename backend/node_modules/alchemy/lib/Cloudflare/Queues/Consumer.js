...
import { isLiveId, LOCAL_ENTRY_URL, LocalRuntimeState, localRuntimeServices, } from "../LocalRuntime.js";
/**
 ...
 */
export const Consumer = Resource("Cloudflare.Queues.Consumer", {
 ...
 * @internal
 */
export const detachQueueConsumersOfScript = Effect.fn(function* (accountId, scriptName) {
    const pages = yield* queues.listQueues.pages({ accountId }).pipe(Stream.runCollect, Effect.catchTag("InvalidRoute", () => Effect.succeed([])));
    const targets = Array.from(pages).flatMap((page) => (page.result ?? []).flatMap((queue) => (queue.consumers ?? []).flatMap((consumer) => queue.queueId &&
        ...
        : [])));
    yield* Effect.forEach(targets, ({ queueId, consumerId }) => queues.deleteConsumer({ accountId, queueId, consumerId }).pipe(Effect.catchTag(["ConsumerNotFound", "QueueNotFound"], () => Effect.void), Effect.andThen(queues.getConsumer({ accountId, queueId, consumerId }).pipe(Effect.flatMap(() => Effect.fail("still-attached")), Effect.catchTag(["ConsumerNotFound", "QueueNotFound"], () => Effect.void), Effect.retry({
        while: (e) => e === "still-attached",
        schedule: Schedule.max([
            Schedule.spaced("1 second"),
            Schedule.recurs(30),
        ]),
    }), Effect.ignore))), { concurrency: 5, discard: true });
});
...
const findWorkerConsumer = (acct, queueId) => queues.listConsumers.items({ accountId: acct, queueId }).pipe(Stream.map(toObserved), Stream.filter((c) => c !== undefined), Stream.runHead, Effect.map(Option.getOrUndefined), 
...
export const ConsumerProviderLive = () => Provider.succeed(Consumer, {
    ...
    list: Effect.fn(function* () {
        ...
        const queueIds = yield* queues.listQueues.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
            .map((q) => q.queueId)
            .filter((id) => id != null))), 
        // Account not entitled for Queues — nothing to enumerate.
        Effect.catchTag("InvalidRoute", () => Effect.succeed([])));
        const rows = yield* Effect.forEach(queueIds, (queueId) => queues.listConsumers.pages({ accountId, queueId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).flatMap((c) => {
            if (c.type !== "worker" || !c.consumerId)
                return [];
    ...
    diff: Effect.fn(function* ({ olds, news, output }) {
        ...
        if (JSON.stringify(olds.settings ?? {}) !==
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const detachConsumer = Effect.fn(function* (consumerId) {
            yield* queues
            ...
            yield* queues
                ...
                .pipe(Effect.flatMap(() => Effect.fail("still-attached")), Effect.catchTag("ConsumerNotFound", () => Effect.void), Effect.retry({
                while: (e) => e === "still-attached",
                schedule: Schedule.max([
                    Schedule.spaced("1 second"),
                    Schedule.recurs(30),
        ...
        if (observed &&
            ...
            observed.scriptName !== undefined &&
            observed.scriptName !== news.scriptName) {
            const scriptTags = Effect.fn(function* (scriptName) {
                const settings = yield* workers
                    .getScriptScriptAndVersionSetting({
                    accountId: acct,
                    scriptName,
                })
                    .pipe(Effect.catchTag(["WorkerNotFound", "WorkerHasNoVersions"], () => Effect.succeed(undefined)));
            ...
            if (!staleGeneration) {
                return yield* Effect.die(`Cloudflare queue "${queueId}" already has a worker ` +
        ...
        if (!observed) {
            ...
            Effect.tapError((e) => e._tag === "QueueHandlerMissing" || e._tag === "QueueNotFound"
                ? Effect.logDebug(`Consumer create: not ready for worker ` +
            ...
            }), Effect.catchTag("ConsumerAlreadyExists", (cause) => Effect.gen(function* () {
                const match = yield* findWorkerConsumer(acct, queueId);
                if (!match) {
                    return yield* Effect.die(`Cloudflare reported a worker consumer already ` +
                ...
                if (match.scriptName !== undefined &&
                    match.scriptName !== news.scriptName) {
                    return yield* Effect.die(`Cloudflare queue "${queueId}" already has a ` +
        ...
        yield* queues.getConsumer({ accountId: acct, queueId, consumerId }).pipe(Effect.flatMap((fetched) => toObserved(fetched)?.scriptName === news.scriptName
            ? Effect.void
            : Effect.fail("ScriptUnbound")), Effect.catchTag("ConsumerNotFound", () => Effect.fail("ScriptUnbound")), Effect.retry({
...
export const ConsumerProviderLocal = () => RpcProvider.effect(Consumer, LOCAL_ENTRY_URL, Effect.gen(function* () {
    ...
    const restartScripts = (scriptNames) => Effect.forEach(scriptNames, (scriptName) => MutableHashMap.get(localRuntimeState.workerRestarts, scriptName).pipe(Option.match({
        onNone: () => Effect.void,
        onSome: (restart) => restart,
    })), { discard: true });
    return {
        list: () => Effect.sync(() => Array.from(MutableHashMap.values(localRuntimeState.queueConsumers))),
        diff: Effect.fn(function* ({ news, output }) {
            ...
            if (!isResolved(news))
                return undefined;
            if (output.queueId !== news.queueId ||
                output.accountId !== accountId) {
                return { action: "replace" };
            }
            if (JSON.stringify(output.settings ?? {}) !==
                JSON.stringify(news.settings ?? {}) ||
        ...
        read: Effect.fn(function* ({ output }) {
            if (!output?.consumerId)
                return undefined;
            return MutableHashMap.get(localRuntimeState.queueConsumers, output.consumerId).pipe(Option.getOrUndefined);
        }),
        reconcile: Effect.fn(function* ({ news, output }) {
            const { accountId } = yield* yield* CloudflareEnvironment;
            ...
            const consumer = {
                ...
                consumerId: output?.consumerId && !isLiveId(output.consumerId)
                    ? output.consumerId
            ...
            yield* restartScripts(output && output.scriptName !== consumer.scriptName
...
export const ConsumerProvider = () => ProviderLayer.dual(Consumer, {
