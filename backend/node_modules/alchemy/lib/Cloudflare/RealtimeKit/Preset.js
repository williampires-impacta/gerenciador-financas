...
const TypeId = "Cloudflare.RealtimeKit.Preset";
/**
 ...
 * @category Media
 */
export const Preset = Resource(TypeId);
/**
 * Returns true if the given value is a Preset resource.
 */
export const isPreset = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
/**
 ...
 */
export const defaultRealtimeKitPresetConfig = () => ({
 ...
 * dark theme.
 */
export const defaultRealtimeKitPresetUi = () => ({
    designTokens: {
...
export const PresetProvider = () => Provider.succeed(Preset, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(apps, (app) => Effect.gen(function* () {
            const appId = app.id;
            if (!appId)
                return [];
            const presets = yield* listAllPresets(accountId, appId);
            const hydrated = yield* Effect.forEach(presets, (preset) => preset.id
                ? getPreset(accountId, appId, preset.id).pipe(Effect.map((observed) => observed
 ...
 */
const listAllPresets = (accountId, appId) => Effect.gen(function* () {
    ...
    const rest = yield* Effect.forEach(Array.from({ length: pages - 1 }, (_, i) => i + 2), (pageNo) => realtimeKit
        ...
        .pipe(Effect.map((res) => [...res.data])), { concurrency: 10 });
 ...
 */
const getPreset = (accountId, appId, presetId) => realtimeKit.getPresetByIdPreset({ accountId, appId, presetId }).pipe(Effect.map((res) => res.data), Effect.catchTag("RealtimeKitPresetNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findByName = (accountId, appId, name) => realtimeKit.getPreset({ accountId, appId, perPage: 100 }).pipe(Effect.map((list) => [...list.data]
    .filter((p) => p.name === name)
    .sort((a, b) => (a.createdAt ?? "").localeCompare(b.createdAt ?? ""))
    .at(0)));
const createPresetName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const withUiDefaults = (ui) => {
 ...
 */
const withPermissionsDefaults = (permissions) => {
    const base = permissions ?? defaultRealtimeKitPresetPermissions();
    return {
        ...base,
        plugins: {
            ...base.plugins,
            config: (base.plugins.config ?? {}),
        },
    };
};
/**
...
const subsetEquals = (desired, observed) => {
    if (typeof desired === "object" &&
        ...
        return Object.entries(desired).every(([key, value]) => value === undefined ||
            subsetEquals(value, observed[key]));
    }
    if (Array.isArray(desired) && Array.isArray(observed)) {
        return (desired.length === observed.length &&
            desired.every((value, i) => subsetEquals(value, observed[i])));
    }
    return desired === observed;
};
const toAttributes = (preset, accountId, appId) => ({
    presetId: preset.id,
    accountId,
    appId,
    name: preset.name,
    ...
    config: {
        ...
        media: {
            screenshare: {
                ...
                quality: preset.config.media.screenshare.quality,
            },
            video: {
                frameRate: preset.config.media.video.frameRate,
                quality: preset.config.media.video.quality,
            },
        ...
        viewType: preset.config.viewType,
    },
    ui: {
        designTokens: {
    ...
    permissions: preset.permissions
        ? {
            ...
            media: {
                audio: {
                    canProduce: preset.permissions.media.audio.canProduce,
                },
                screenshare: {
                ...
                video: {
                    canProduce: preset.permissions.media.video.canProduce,
                },
            },
            ...
            recorderType: preset.permissions.recorderType,
            showParticipantList: preset.permissions.showParticipantList,
            waitingRoomType: preset.permissions.waitingRoomType,
