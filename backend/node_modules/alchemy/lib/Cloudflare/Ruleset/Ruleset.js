 ...
 */
export const Ruleset = Resource("Cloudflare.Ruleset.Ruleset", {
...
export const RulesetProvider = () => Provider.succeed(Ruleset, {
    ...
    read: Effect.fn(function* ({ olds, output }) {
        ...
        return yield* rulesets
            ...
            .pipe(Effect.map((ruleset) => toRulesetAttributes(zoneId, ruleset)), Effect.catchTag("RulesetNotFound", () => Effect.succeed(undefined)));
    }),
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => rulesets.listRulesetsForZone.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).filter((r) => r.kind === "zone"))), Effect.flatMap((entrypoints) => Effect.forEach(entrypoints, (entry) => rulesets
            .getPhasForZone({
            zoneId: zone.id,
            rulesetPhase: entry.phase,
        })
            .pipe(Effect.map((ruleset) => toRulesetAttributes(zone.id, ruleset)), 
        // Per-item not-found / plan-gated entrypoints are
        // skipped rather than failing the whole enumeration.
        Effect.catchTag(["RulesetNotFound", "Forbidden"], () => Effect.succeed(undefined))), { concurrency: 10 })), Effect.map((items) => items.filter((item) => 
...
const createRulesetName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id }));
});
export const toRulesetAttributes = (zoneId, ruleset) => ({
