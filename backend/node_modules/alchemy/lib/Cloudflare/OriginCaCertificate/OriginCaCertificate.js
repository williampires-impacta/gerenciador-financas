...
import { findZoneByName, listAllZones } from "../Zone/lookup.js";
const TypeId = "Cloudflare.OriginCaCertificate.OriginCaCertificate";
/**
 ...
 */
export const OriginCaCertificate = Resource(TypeId, {
    aliases: ["Cloudflare.OriginCaCertificate"],
});
/**
 * Returns true if the given value is an OriginCaCertificate resource.
 */
export const isOriginCaCertificate = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const OriginCaCertificateProvider = () => Provider.succeed(OriginCaCertificate, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => originCa.listOriginCaCertificates.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
            .filter((cert) => cert.id != null)
            .map((cert) => toAttributes(cert)))), Effect.catchTag("Forbidden", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
    ...
    read: Effect.fn(function* ({ output, olds }) {
        ...
        if (typeof olds?.hostnames?.[0] !== "string")
            return undefined;
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* originCa
            ...
            times: 6,
        }), Effect.catchTag("CertificateNotFound", () => Effect.void), Effect.catchTag("CertificateAlreadyRevoked", () => Effect.void), Effect.catchTag("CertificateRevocationFailed", (e) => getCertificate(output.certificateId).pipe(Effect.flatMap((cert) => (cert ? Effect.fail(e) : Effect.void)))));
    }),
});
 ...
 */
const getCertificate = (certificateId) => originCa.getOriginCaCertificate({ certificateId }).pipe(Effect.map((cert) => (cert.revokedAt ? undefined : cert)), Effect.catchTag("CertificateNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findByHostnames = (hostnames) => Effect.gen(function* () {
    const { accountId } = yield* yield* CloudflareEnvironment;
    ...
    const certs = yield* originCa.listOriginCaCertificates
        .items({ zoneId })
        .pipe(Stream.filter((cert) => sameHostnames([...cert.hostnames], hostnames)), Stream.runCollect);
 ...
 * account matches.
 */
const resolveZoneIdForHostname = (accountId, hostname) => Effect.gen(function* () {
...
const sameHostnames = (observed, desired) => observed.length === desired.length &&
 ...
 */
const toAttributes = (cert, fallback) => ({
