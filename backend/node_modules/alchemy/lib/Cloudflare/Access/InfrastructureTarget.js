...
const TypeId = "Cloudflare.Access.InfrastructureTarget";
/**
 ...
 */
export const InfrastructureTarget = Resource(TypeId);
/**
 * Returns true if the given value is an InfrastructureTarget resource.
 */
export const isInfrastructureTarget = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const InfrastructureTargetProvider = () => Provider.succeed(InfrastructureTarget, {
    stables: ["targetId", "accountId", "createdAt"],
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* zeroTrust.listAccessInfrastructureTargets
            .pages({ accountId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((t) => toAttributes(t, accountId)))));
    }),
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const dirty = observed.hostname !== desired.hostname ||
 ...
 */
const getTarget = (accountId, targetId) => zeroTrust
 ...
 */
const findByIdentity = (accountId, hostname, ip) => zeroTrust.listAccessInfrastructureTargets.items({ accountId, hostname }).pipe(Stream.filter((t) => t.hostname === hostname &&
    (ip === undefined || ipEquals(observedIp(t), ip))), Stream.runHead, Effect.map(Option.getOrUndefined));
/**
 ...
 */
const ipEquals = (observed, desired) => familyEquals(observed.ipv4, desired.ipv4) &&
    familyEquals(observed.ipv6, desired.ipv6);
const familyEquals = (observed, desired) => {
    if (desired === undefined)
        return observed === undefined;
    if (observed === undefined)
        return false;
    if (observed.ipAddr !== desired.ipAddr)
        return false;
    if (desired.virtualNetworkId !== undefined &&
        observed.virtualNetworkId !== desired.virtualNetworkId) {
        return false;
    }
    return true;
};
const toAttributes = (target, accountId) => ({
    targetId: target.id,
    accountId,
...
//# sourceMappingURL=InfrastructureTarget.js.map