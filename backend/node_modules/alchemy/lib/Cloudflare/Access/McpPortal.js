...
const TypeId = "Cloudflare.Access.McpPortal";
/**
 ...
 */
export const McpPortal = Resource(TypeId);
/**
 * Returns true if the given value is an McpPortal resource.
 */
export const isMcpPortal = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const McpPortalProvider = () => Provider.succeed(McpPortal, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* zeroTrust.listAccessAiControlMcpPortals
            .pages({ accountId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((portal) => toAttributes(portal, accountId)))), Effect.catchTag("Forbidden", () => Effect.succeed([])));
    }),
...
//# sourceMappingURL=McpPortal.js.map