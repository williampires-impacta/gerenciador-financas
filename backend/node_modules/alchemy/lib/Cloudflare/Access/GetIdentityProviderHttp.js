...
import { isWorker } from "../Workers/Worker.js";
import { GetIdentityProvider, } from "./GetIdentityProvider.js";
import { findFirst, toAttributes } from "./IdentityProviderLookup.js";
 ...
 */
export const GetIdentityProviderHttp = Layer.effect(GetIdentityProvider, Effect.gen(function* () {
    const Token = yield* AccountApiToken;
    const env = yield* CloudflareEnvironment;
    const ambientCredentials = yield* Effect.serviceOption(Credentials).pipe(Effect.map(Option.getOrUndefined));
    return Effect.fn(function* (options) {
        ...
        if (isWorker(host)) {
            ...
            return makeGetIdentityProviderClient({ authorize: authorizeWith(bound), accountId: bound.accountId }, options);
        }
        ...
        if (ambientCredentials === undefined) {
            return yield* Effect.die(new Error("Cloudflare.Access.GetIdentityProvider requires either a Worker " +
                "host (runtime binding) or ambient Cloudflare credentials " +
                "(plan-time data source)."));
        }
        const { accountId } = yield* env;
        return makeGetIdentityProviderClient({
            authorize: (eff) => eff.pipe(Effect.provideService(Credentials, ambientCredentials), Effect.provide(FetchHttpClient.layer)),
            accountId: Effect.succeed(accountId),
        }, options);
    });
}));
const makeGetIdentityProviderClient = (auth, options) => Effect.fn("Cloudflare.Access.GetIdentityProvider")(function* () {
    const accountId = yield* auth.accountId;
    const match = yield* auth.authorize(findFirst(options.zoneId, accountId, (idp) => (options.name === undefined || idp.name === options.name) &&
        (options.type === undefined || idp.type === options.type)));
    return match
        ? toAttributes(match, options.zoneId, accountId, undefined)
        : undefined;
});
//# sourceMappingURL=GetIdentityProviderHttp.js.map