 ...
 */
export const ServiceToken = Resource("Cloudflare.Access.ServiceToken");
export const isServiceToken = (value) => Predicate.hasProperty(value, "Type") &&
    value.Type === "Cloudflare.Access.ServiceToken";
export const ServiceTokenProvider = () => Provider.succeed(ServiceToken, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* zeroTrust.listAccessServiceTokensForAccount
            ...
            .filter((t) => t.id != null && t.clientId != null)
            .map((t) => ({
            serviceTokenId: t.id,
            accountId,
            clientId: t.clientId,
    ...
    read: Effect.fn(function* ({ id, output, olds }) {
        ...
        if (!existing || !existing.id || !existing.clientId)
            return undefined;
        return {
    ...
    reconcile: Effect.fn(function* ({ id, news = {}, olds, output, }) {
        ...
        if (!observed || !observed.id) {
            ...
            return {
                ...
                clientSecret: "clientSecret" in created &&
...
const findTokenByName = (acct, name) => zeroTrust.listAccessServiceTokensForAccount.items({ accountId: acct }).pipe(Stream.filter((t) => t.name === name), Stream.runHead, Effect.map(Option.getOrUndefined), Effect.catch(() => Effect.succeed(undefined)));
// Skip no-op PUTs while treating an unset prop as "keep whatever Cloudflare
// has" rather than a difference.
const differs = (desired, observed) => desired !== undefined && desired !== (observed ?? undefined);
