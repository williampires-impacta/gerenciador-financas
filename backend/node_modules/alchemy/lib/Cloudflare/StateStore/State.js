...
import { RandomProvider } from "../../Random.js";
...
import { CREDENTIALS_FILE, isStateStoreCredentialsStale, } from "./CredentialsFile.js";
import { AuthToken, AuthTokenSecretName, EncryptionKeySecretName, TokenValue, } from "./Token.js";
...
export const state = () => Layer.effect(State, Effect.gen(function* () {
    ...
    const autoUpdateStateStore = Option.getOrUndefined(yield* Effect.serviceOption(AlchemyContext))
    ...
    const init = Effect.gen(function* () {
        ...
        const ensureLatest = ({ url, authToken, }) => Effect.gen(function* () {
            ...
            if (observed === undefined) {
                const shouldDeploy = autoUpdateStateStore ||
                    (yield* Clank.confirm({
            ...
            if (autoUpdateStateStore) {
                ...
                return yield* Effect.die(new AuthError({
                    message: `Cloudflare State store is out of date ` +
        ...
        if (credentials) {
            ...
            if (isStateStoreCredentialsStale(credentials, accountId)) {
                yield* Clank.info(`Cloudflare State Store credentials were minted for a different ` +
...
Layer.provide(Cloudflare.CloudflareApiLive()), Layer.orDie);
 ...
 */
export const teardownStateStore = (options = {}) => Effect.gen(function* () {
    const profileName = options.profile ?? (yield* ALCHEMY_PROFILE);
    const scriptName = options.workerName ?? STATE_STORE_SCRIPT_NAME;
    ...
    yield* workers.deleteScript({ accountId, scriptName, force: true }).pipe(Effect.asVoid, Effect.catchTag("WorkerNotFound", () => Clank.info(`  Worker '${scriptName}' not found (already gone).`)));
    // 2. Delete the secrets the state store created, plus any now-empty store.
    const ourSecretNames = new Set([
        AuthTokenSecretName,
        EncryptionKeySecretName,
    ]);
    const stores = yield* SecretsStore.listStores.items({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("InvalidAccountId", () => Effect.succeed([])));
    for (const store of stores) {
        const secrets = yield* SecretsStore.listStoreSecrets
            .items({ accountId, storeId: store.id })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag(["StoreNotFound", "InvalidAccountId"], () => Effect.succeed([])));
        ...
        for (const secret of ours) {
            ...
            yield* SecretsStore.deleteStoreSecret({
                ...
                storeId: store.id,
                secretId: secret.id,
            }).pipe(Effect.asVoid, Effect.catchTag(["SecretNotFound", "StoreNotFound", "NotFound", "InvalidAccountId"], () => Effect.void));
        }
        ...
        if (deleteEmptyStore && remaining === 0) {
            ...
            yield* SecretsStore.deleteStore({
                ...
                storeId: store.id,
                force: true,
            }).pipe(Effect.asVoid, Effect.catchTag(["StoreNotFound", "NotFound", "InvalidAccountId"], () => Effect.void));
        }
        else if (remaining > 0) {
            yield* Clank.info(`Secrets store '${store.id}' still has ${remaining} other ` +
...
}).pipe(Effect.withSpan("state_store.teardown", {
    attributes: {
 ...
 * be catastrophic.
 */
const hoistBootstrapStack = Effect.fn(function* ({ source, destination, }) {
    ...
    yield* Effect.forEach(fqns, Effect.fn(function* (fqn) {
...
export const loginWithCloudflare = (profileName, force) => Effect.gen(function* () {
    ...
    const authToken = yield* readSecretViaEdge(STATE_STORE_SCRIPT_NAME, store.id, AuthTokenSecretName).pipe(Effect.retry({
        while: (error) => isWorkersPreviewConfigurationError(error) ||
...
const isStateStoreAvailable = (scriptName = "alchemy-state-store") => Effect.gen(function* () {
    ...
    const { accountId } = yield* yield* CloudflareEnvironment.CloudflareEnvironment;
    return yield* workers.getScriptSetting({ accountId, scriptName }).pipe(Effect.map((setting) => setting !== undefined), Effect.catchTag("WorkerNotFound", () => Effect.succeed(false)), Effect.catchTag("InvalidRoute", () => Effect.succeed(false)), 
 ...
 *
 */
const isStateStoreServing = (accountId) => Effect.gen(function* () {
    const url = yield* workers.getSubdomain({ accountId }).pipe(Effect.map(({ subdomain }) => subdomain
        ? `https://${STATE_STORE_SCRIPT_NAME}.${subdomain}.workers.dev`
        : undefined), Effect.catch(() => Effect.succeed(undefined)));
    if (url === undefined)
        return false;
    const { observed } = yield* checkStateStoreVersion(url);
    return observed !== undefined;
});
const makeCloudflareStateStore = Effect.fn(function* ({ url, authToken, }) {
...
class StateStoreVersionNotReady extends Error {
    expected;
    observed;
    _tag = "StateStoreVersionNotReady";
    constructor(expected, observed) {
        super(`Cloudflare State Store version not ready (expected v${expected}, observed v${observed ?? "unknown"}).`);
        this.expected = expected;
        this.observed = observed;
    }
}
const waitForStateStoreVersion = (url) => Effect.gen(function* () {
    const { matches, expected, observed } = yield* checkStateStoreVersion(url);
    if (!matches) {
...
}).pipe(Effect.withSpan("state_store.check_version", {
 ...
 */
const readSecretViaEdge = (scriptName, storeId, secretName) => Effect.gen(function* () {
    ...
    if (response.status !== 200) {
        ...
        yield* Effect.logWarning(`Secret probe failed (${response.status}) at ${session.url}\n${body}`);
    ...
    ? cause
    : new EdgeSessionError({ message: "Failed to read secret", cause })), Effect.withSpan("state_store.read_secret_via_edge", {
    attributes: {
 ...
 * @internal exported for unit testing.
 */
export const isTransientEdgeSessionError = (error) => {
    if (!(error instanceof EdgeSessionError))
        return false;
    // Non-200 probe responses are edge-propagation flakes, not client bugs.
    if (error.message.startsWith("Secret probe returned"))
        return true;
    const tag = error.cause?._tag;
