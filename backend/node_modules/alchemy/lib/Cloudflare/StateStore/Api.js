...
import { BearerTokenValidator, StateApi, StateAuthLive, } from "../../State/HttpStateApi.js";
import { ReadSecret } from "../SecretsStore/ReadSecret.js";
import { ReadSecretBinding } from "../SecretsStore/ReadSecretBinding.js";
 ...
 */
export const STATE_STORE_VERSION = 7;
/**
...
export default Worker("Api", {
    ...
    const bearerTokenValidator = Layer.succeed(BearerTokenValidator, BearerTokenValidator.of({
    ...
    const versionApi = HttpApiBuilder.group(StateApi, "version", (handlers) => handlers.handle("getVersion", () => Effect.succeed({ version: STATE_STORE_VERSION }).pipe(Effect.withSpan("state_store.getVersion", {
        attributes: { "alchemy.state_store.op": "getVersion" },
    }))));
    const stateApi = HttpApiBuilder.group(StateApi, "state", (handlers) => handlers
        .handle("listStacks", () => store
        .getByName(Store.ROOT_DO_NAME)
        .listStacks()
        .pipe(Effect.withSpan("state_store.listStacks", {
        attributes: { "alchemy.state_store.op": "listStacks" },
    })))
        .handle("listStages", ({ params }) => store
        .getByName(params.stack)
        .listStages()
        .pipe(Effect.withSpan("state_store.listStages", {
        attributes: {
            "alchemy.state_store.op": "listStages",
            "alchemy.state_store.stack": params.stack,
        },
    })))
        .handle("listResources", ({ params }) => store
        .getByName(params.stack)
        .listResources({ stage: params.stage })
        .pipe(Effect.withSpan("state_store.listResources", {
        attributes: {
        ...
        return store
            ...
            .pipe(Effect.withSpan("state_store.getState", {
            attributes: {
        ...
        return store
            .getByName(params.stack)
            .set({ stage: params.stage, fqn, value: payload })
            .pipe(Effect.tap(() => store
            .getByName(Store.ROOT_DO_NAME)
            .registerStack({ stack: params.stack })), Effect.withSpan("state_store.setState", {
            attributes: {
        ...
        return store
            ...
            .pipe(Effect.asVoid, Effect.withSpan("state_store.deleteState", {
            attributes: {
        ...
        .pipe(Effect.withSpan("state_store.getReplacedResources", {
        attributes: {
        ...
        .handle("getStackOutput", ({ params }) => store
        .getByName(params.stack)
        .getOutput({ stage: params.stage })
        .pipe(Effect.withSpan("state_store.getStackOutput", {
        attributes: {
        ...
        .handle("setStackOutput", ({ params, payload }) => store
        .getByName(params.stack)
        .setOutput({ stage: params.stage, value: payload })
        .pipe(Effect.tap(() => store
        .getByName(Store.ROOT_DO_NAME)
        .registerStack({ stack: params.stack })), Effect.withSpan("state_store.setStackOutput", {
        attributes: {
        ...
        : Effect.void), Effect.asVoid, Effect.withSpan("state_store.deleteStack", {
        attributes: {
    ...
    return {
        fetch: HttpApiBuilder.layer(StateApi).pipe(Layer.provide(stateApi), Layer.provide(versionApi), Layer.provide(StateAuthLive), Layer.provide(bearerTokenValidator), 
