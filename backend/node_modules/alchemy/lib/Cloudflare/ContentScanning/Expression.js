...
const TypeId = "Cloudflare.ContentScanning.Expression";
/**
 ...
 */
export const Expression = Resource(TypeId);
/**
 * Returns true if the given value is a Expression resource.
 */
export const isExpression = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
/**
...
export const ExpressionProvider = () => Provider.succeed(Expression, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones, (zone) => contentScanning.listPayloads.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((expression) => toAttributes(zone.id, expression)))), Effect.catchTag("ContentScanningNotEnabled", () => Effect.succeed([])), Effect.catchTag("Forbidden", () => Effect.succeed([])), Effect.catchTag("InvalidRoute", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (!observed) {
            ...
            if (!observed) {
                return yield* Effect.fail(new ExpressionCreateAnomaly({
                    zoneId,
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        if (expressions === undefined)
            return;
        if (!expressions.some((e) => e.id === expressionId))
            return;
        yield* contentScanning.deletePayload({ zoneId, expressionId }).pipe(
        // A concurrent disable between observe and delete is "gone".
        Effect.catchTag("ContentScanningNotEnabled", () => Effect.void), Effect.catchTag("InvalidRoute", () => Effect.void));
    }),
});
/**
 ...
 */
const findExpression = (zoneId, predicate) => contentScanning.listPayloads
    .items({ zoneId })
 ...
 */
const listExpressions = (zoneId) => contentScanning.listPayloads.items({ zoneId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag("ContentScanningNotEnabled", () => Effect.succeed(undefined)), Effect.catchTag("InvalidRoute", () => Effect.succeed(undefined)));
const toAttributes = (zoneId, expression) => ({
