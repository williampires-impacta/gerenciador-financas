...
const TypeId = "Cloudflare.ContentScanning.ContentScanning";
/**
 ...
 */
export const ContentScanning = Resource(TypeId, {
    aliases: ["Cloudflare.ContentScanning"],
});
/**
 * Returns true if the given value is a ContentScanning resource.
 */
export const isContentScanning = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const ContentScanningProvider = () => Provider.succeed(ContentScanning, {
    nuke: { singleton: true },
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => contentScanning.getContentScanning({ zoneId }).pipe(Effect.map((observed) => toAttributes(zoneId, observed, statusOf(observed))), 
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const desired = news.enabled === false ? "disabled" : "enabled";
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        if (statusOf(observed) === initialValue)
            return;
 ...
 * `disabled`.
 */
const statusOf = (setting) => setting.value === "enabled" ? "enabled" : "disabled";
