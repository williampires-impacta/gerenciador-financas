import type { BatchRecordError, BatchRecordRequest, BatchRecordResponse, CreateRecordError, CreateRecordRequest, CreateRecordResponse, DeleteRecordError, DeleteRecordResponse, PatchRecordError, PatchRecordRequest, PatchRecordResponse, UpdateRecordError, UpdateRecordRequest, UpdateRecordResponse } from "@distilled.cloud/cloudflare/dns";
 ...
 */
export interface WriteDns extends Binding.Service<WriteDns, "Cloudflare.DNS.WriteDns", (zone: Zone) => Effect.Effect<WriteDnsClient>> {
}
...
export interface WriteDnsClient {
    /** Create a DNS record. */
    createDnsRecord(request: CreateRecordRequestInput): Effect.Effect<CreateRecordResponse, CreateRecordError, RuntimeContext>;
    /** Overwrite (PUT) a DNS record. */
    updateDnsRecord(dnsRecordId: string, request: UpdateRecordRequestInput): Effect.Effect<UpdateRecordResponse, UpdateRecordError, RuntimeContext>;
    /** Partially update (PATCH) a DNS record. */
    patchDnsRecord(dnsRecordId: string, request: PatchRecordRequestInput): Effect.Effect<PatchRecordResponse, PatchRecordError, RuntimeContext>;
    /** Delete a DNS record by id. */
    deleteDnsRecord(dnsRecordId: string): Effect.Effect<DeleteRecordResponse, DeleteRecordError, RuntimeContext>;
    /** Apply a batch of create / update / patch / delete operations atomically. */
    batchDnsRecords(request: BatchRecordRequestInput): Effect.Effect<BatchRecordResponse, BatchRecordError, RuntimeContext>;
}
