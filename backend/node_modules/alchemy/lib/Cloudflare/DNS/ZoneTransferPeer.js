...
const TypeId = "Cloudflare.DNS.ZoneTransferPeer";
/**
 ...
 */
export const ZoneTransferPeer = Resource(TypeId, {
    aliases: ["Cloudflare.Dns.ZoneTransferPeer"],
});
/**
 * Returns true if the given value is a ZoneTransferPeer resource.
 */
export const isZoneTransferPeer = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const ZoneTransferPeerProvider = () => Provider.succeed(ZoneTransferPeer, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* dns.listZoneTransferPeers.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((peer) => toAttributes(peer, accountId)))));
    }),
...
const getPeer = (accountId, peerId) => dns
 ...
 */
const findByName = (accountId, name) => dns.listZoneTransferPeers.items({ accountId }).pipe(Stream.filter((p) => p.name === name), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    .sort((a, b) => a.id.localeCompare(b.id))
    .at(0)));
const createPeerName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const toAttributes = (peer, accountId) => ({
    peerId: peer.id,
    accountId,
    name: peer.name,
...
//# sourceMappingURL=ZoneTransferPeer.js.map