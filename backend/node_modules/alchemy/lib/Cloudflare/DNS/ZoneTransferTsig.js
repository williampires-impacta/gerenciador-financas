...
const TypeId = "Cloudflare.DNS.ZoneTransferTsig";
/**
 ...
 */
export const ZoneTransferTsig = Resource(TypeId, {
    aliases: ["Cloudflare.Dns.ZoneTransferTsig"],
});
/**
 * Returns true if the given value is a ZoneTransferTsig resource.
 */
export const isZoneTransferTsig = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const ZoneTransferTsigProvider = () => Provider.succeed(ZoneTransferTsig, {
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* dns.listZoneTransferTsigs.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((tsig) => toAttributes(tsig, accountId)))));
    }),
});
/** Read a TSIG by id, mapping "gone" (404) to `undefined`. */
const getTsig = (accountId, tsigId) => dns
 ...
 */
const findByName = (accountId, name) => dns.listZoneTransferTsigs.items({ accountId }).pipe(Stream.filter((t) => t.name === name), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    .sort((a, b) => a.id.localeCompare(b.id))
    .at(0)));
const createTsigName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const toAttributes = (tsig, accountId) => ({
    tsigId: tsig.id,
    accountId,
    name: tsig.name,
    algo: tsig.algo,
});
//# sourceMappingURL=ZoneTransferTsig.js.map