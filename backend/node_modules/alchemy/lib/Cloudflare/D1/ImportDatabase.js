 ...
 */
export const importD1Database = (options) => Effect.gen(function* () {
    ...
    const postJson = (body) => Effect.gen(function* () {
        const req = HttpClientRequest.post(url).pipe(HttpClientRequest.setHeaders(authHeaders), HttpClientRequest.bodyJsonUnsafe(body));
    ...
    const bytes = typeof options.sqlData === "string"
        ? new TextEncoder().encode(options.sqlData)
        : options.sqlData;
    const putReq = HttpClientRequest.put(init.uploadUrl).pipe(HttpClientRequest.bodyUint8Array(bytes, "application/sql"));
    ...
    const poll = (bookmark) => Effect.gen(function* () {
...
const importEndpoint = (apiBaseUrl, accountId, databaseId) => `${apiBaseUrl}/accounts/${accountId}/d1/database/${databaseId}/import`;
