...
import { localD1GatewayRuntime, withLocalD1Query } from "./LocalD1Gateway.js";
import { QueryDatabase } from "./QueryDatabase.js";
import { makeD1DatabaseFromTransport, makeHttpD1Database, makeQueryDatabaseClientFrom, } from "./QueryDatabaseHttpClient.js";
/**
 ...
 */
export const QueryDatabaseLocal = Layer.effect(QueryDatabase, Effect.gen(function* () {
    ...
    // HTTP query API.
    const localTransport = (databaseId) => (body) => withLocalD1Query(databaseId, (query) => query(body)).pipe(Effect.provide(localD1GatewayRuntime), 
    ...
    Effect.provideContext(ambient), (eff) => Effect.runPromise(eff));
    return Effect.fn(function* (database) {
        ...
        return makeQueryDatabaseClientFrom(Effect.map(databaseId, (id) => isLocalId(id)
            ? makeD1DatabaseFromTransport(localTransport(id))
            : makeHttpD1Database(auth, id)));
    });
}));
//# sourceMappingURL=QueryDatabaseLocal.js.map