...
import { diffMigrations, migrationsAttrs, migrationsInputOf, runMigrations, stampedOf, } from "../../SQL/Migrations/index.js";
import { hashImports, readSqlFile } from "../../SQL/SqlFile.js";
...
import { makeD1MigrationExecutor } from "./ApplyMigrations.js";
import { cloneDatabase } from "./CloneDatabase.js";
import { importD1Database } from "./ImportDatabase.js";
import { withLocalD1Executor } from "./LocalD1Gateway.js";
export const isDatabase = (value) => isResourceOfType(value, "Cloudflare.D1Database");
/**
 ...
 */
export const Database = Resource("Cloudflare.D1Database");
export const ProviderLive = () => Provider.succeed(Database, {
    ...
    diff: Effect.fn(function* ({ id, olds = {}, news = {}, output }) {
        ...
        const oldReplicationMode = output?.readReplication?.mode ??
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* d1.listDatabases.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
            .filter((db) => db.uuid != null)
            .map((db) => ({
            databaseId: db.uuid,
            databaseName: db.name ?? db.uuid,
            jurisdiction: (db.jurisdiction ?? "default"),
    ...
    read: Effect.fn(function* ({ id, output, olds }) {
        ...
        const match = yield* d1.listDatabases.items({ accountId, name }).pipe(Stream.filter((db) => db.name === name), Stream.runHead, Effect.map(Option.getOrUndefined));
        if (match) {
            return {
    ...
    reconcile: Effect.fn(function* ({ id, news = {}, output }) {
        ...
        if (!observed) {
            observed = yield* d1.listDatabases
                ...
                .pipe(Stream.filter((db) => db.name === name), Stream.runHead, Effect.map(Option.getOrUndefined));
        }
        ...
        if (!observed) {
            const db = yield* d1
                ...
                .pipe(Effect.catchTag("InvalidProperty", () => Effect.gen(function* () {
                ...
                return yield* Effect.die(`Database with name "${name}" already exists but could not be found`);
            })));
            databaseId = db.uuid;
        ...
        const migrations = migrationsInput
            ? yield* runMigrations({
                ...
                withExecutor: (apply) => Effect.gen(function* () {
                    const queryDb = yield* d1.queryDatabase;
                    yield* apply(makeD1MigrationExecutor((sql) => queryDb({ accountId: acct, databaseId, sql })));
                }),
            })
            : undefined;
        ...
        const importHashes = news.importFiles?.length
            ? yield* runImports(acct, databaseId, news.importFiles, yield* rootDir, output?.importHashes ?? {})
            : {};
        return {
            databaseId,
            databaseName,
            jurisdiction: (output?.jurisdiction ?? jurisdiction),
 ...
 * registration.
 */
export const ProviderLocal = () => RpcProvider.effect(Database, LOCAL_ENTRY_URL, Effect.gen(function* () {
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output }) {
            ...
            const migrations = migrationsInput
                ? yield* runMigrations({
                    input: migrationsInput,
                    stamped: stampedOf(output),
                    withExecutor: (apply) => withLocalD1Executor(databaseId, (executor) => apply(makeD1MigrationExecutor(executor))).pipe(Effect.provideContext(runtimeContext)),
                })
                : undefined;
            ...
            if (news.importFiles?.length) {
                ...
                for (const filePath of news.importFiles) {
                    ...
                    if (importHashes[filePath] === file.hash)
                        continue;
                    pending.push({ path: filePath, sql: file.sql, hash: file.hash });
                }
                if (pending.length > 0) {
                    yield* withLocalD1Executor(databaseId, (executor) => Effect.forEach(pending, (file) => executor(file.sql), {
                        discard: true,
            ...
            return {
                ...
                jurisdiction: (news.jurisdiction ?? "default"),
                readReplication: news.readReplication,
 ...
 */
const resolveCloneSource = (source, accountId) => Effect.gen(function* () {
