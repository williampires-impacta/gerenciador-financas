...
const TypeId = "Cloudflare.RiskScoring.Integration";
/**
 ...
 */
export const Integration = Resource(TypeId);
/**
 * Returns true if the given value is a Integration resource.
 */
export const isIntegration = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const IntegrationProvider = () => Provider.succeed(Integration, {
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const dirty = observed.tenantUrl !== news.tenantUrl ||
    ...
    list: Effect.fn(function* () {
        ...
        return yield* zeroTrust.listRiskScoringIntegrations
            .pages({ accountId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((integration) => toAttributes(integration, accountId)))), Effect.catchTag("Forbidden", () => Effect.succeed([])));
    }),
});
/**
 ...
 */
const findByTenantUrl = (accountId, tenantUrl) => zeroTrust
