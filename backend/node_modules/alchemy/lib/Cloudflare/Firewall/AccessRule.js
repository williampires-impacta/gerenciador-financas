...
const FirewallAccessRuleTypeId = "Cloudflare.Firewall.AccessRule";
/**
 ...
 */
export const AccessRule = Resource(FirewallAccessRuleTypeId);
/**
 * Returns true if the given value is a AccessRule resource.
 */
export const isAccessRule = (value) => Predicate.hasProperty(value, "Type") &&
    value.Type === FirewallAccessRuleTypeId;
export const AccessRuleProvider = () => Provider.succeed(AccessRule, {
    ...
    diff: Effect.fn(function* ({ olds = {}, news }) {
        ...
        if (o.configuration.target !== n.configuration.target ||
    ...
    read: Effect.fn(function* ({ output, olds }) {
        ...
        if (output?.ruleId) {
            ...
            if (observed)
                return toAttributes(observed, zoneId, acct);
        }
        ...
        if (configuration) {
            ...
            if (observed)
                return Unowned(toAttributes(observed, zoneId, acct));
        }
        return undefined;
    }),
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (!observed) {
            observed = yield* createRule(zoneId, accountId, news).pipe(Effect.catchTag("DuplicateAccessRule", (error) => findByConfiguration(zoneId, accountId, news.configuration).pipe(Effect.flatMap((existing) => existing ? Effect.succeed(existing) : Effect.fail(error)))));
        }
    ...
    list: Effect.fn(function* () {
        ...
        const accountRules = yield* firewall.listAccessRulesForAccount
            .pages({ accountId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((rule) => toAttributes(rule, undefined, accountId)))), 
        // No permission to read account-level rules — skip them.
        Effect.catchTag("Forbidden", () => Effect.succeed([])));
        const zones = yield* listAllZones(accountId);
        const zoneRuleGroups = yield* Effect.forEach(zones, (zone) => firewall.listAccessRulesForZone.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((rule) => toAttributes(rule, zone.id, accountId)))), 
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        if (!observed)
            return;
        yield* deleteRule(output.zoneId, output.accountId, output.ruleId).pipe(Effect.catchTag("AccessRuleNotFound", () => Effect.void));
    }),
});
/**
 ...
 */
const getRule = (zoneId, accountId, ruleId) => (zoneId !== undefined
    ? firewall.getAccessRuleForZone({ zoneId, ruleId })
    : firewall.getAccessRuleForAccount({ accountId, ruleId })).pipe(Effect.map((rule) => rule), Effect.catchTag("AccessRuleNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findByConfiguration = (zoneId, accountId, configuration) => (zoneId !== undefined
    ? firewall.listAccessRulesForZone.items({ zoneId })
    : firewall.listAccessRulesForAccount.items({ accountId })).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((rule) => rule.configuration.target === configuration.target &&
    rule.configuration.value === configuration.value)));
const createRule = (zoneId, accountId, news) => zoneId !== undefined
...
const patchRule = (zoneId, accountId, ruleId, news) => zoneId !== undefined
...
const toAttributes = (rule, zoneId, accountId) => ({
    ...
    configuration: {
        ...
        target: (rule.configuration.target ?? "ip"),
