...
const TypeId = "Cloudflare.Alerting.Silence";
/**
 ...
 */
export const Silence = Resource(TypeId);
/**
 * Returns true if the given value is a Silence resource.
 */
export const isSilence = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const SilenceProvider = () => Provider.succeed(Silence, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* alerting.listSilences.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
            .map(narrowSilence)
    ...
    diff: Effect.fn(function* ({ olds = {}, news, output }) {
        ...
        const o = olds;
        const n = news;
        const oldPolicyId = output?.policyId ??
            (typeof o.policyId === "string" ? o.policyId : undefined);
        if (oldPolicyId !== undefined &&
    ...
    read: Effect.fn(function* ({ output, olds }) {
        ...
        if (output?.silenceId) {
            ...
            if (observed)
                return toSilenceAttributes(observed, acct);
            return undefined;
        }
        ...
        if (policyId && o?.startTime && o?.endTime) {
            ...
            if (match)
                return Unowned(toSilenceAttributes(match, acct));
        }
        return undefined;
    }),
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (!observed) {
            yield* alerting
                ...
                .pipe(Effect.catchTag("SilenceAlreadyExists", (error) => findSilence(accountId, policyId, news.startTime, news.endTime).pipe(Effect.flatMap((existing) => existing ? Effect.void : Effect.fail(error)))));
            observed = yield* findSilence(accountId, policyId, news.startTime, news.endTime).pipe(Effect.flatMap((found) => found ? Effect.succeed(found) : Effect.fail(new SilencePending())), Effect.retry({
        ...
        //    PUT only on drift.
        if (!sameInstant(observed.startTime, news.startTime) ||
...
const narrowSilence = (raw) => raw.id == null
    ? undefined
    : {
        id: raw.id,
...
const observeSilence = (accountId, silenceId) => alerting.getSilence({ accountId, silenceId }).pipe(Effect.map((s) => narrowSilence({ ...s, id: s.id ?? silenceId })), Effect.catchTag("SilenceNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findSilence = (accountId, policyId, startTime, endTime) => alerting.listSilences.items({ accountId }).pipe(Stream.filter((s) => s.id != null &&
    s.policyId === policyId &&
    sameInstant(undef(s.startTime), startTime) &&
    sameInstant(undef(s.endTime), endTime)), Stream.runHead, Effect.map(Option.getOrUndefined), Effect.map((s) => (s === undefined ? undefined : narrowSilence(s))));
...
const toSilenceAttributes = (observed, accountId) => ({
