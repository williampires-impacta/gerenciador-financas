...
const TypeId = "Cloudflare.Images.Variant";
/**
 ...
 * @category Media
 */
export const Variant = Resource(TypeId);
/**
 * Returns true if the given value is an Variant resource.
 */
export const isVariant = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const VariantProvider = () => Provider.succeed(Variant, {
    ...
    list: Effect.fn(function* () {
        ...
        // works unchanged.
        const names = yield* images.listV1Variants({ accountId }).pipe(Effect.map(variantNamesFrom), 
        ...
        Effect.catchTag("ImagesAccessNotEnabled", () => Effect.succeed([])));
        const rows = yield* Effect.forEach(names, (name) => getVariant(accountId, name).pipe(Effect.map((variant) => variant ? toAttributes(variant, accountId) : undefined)), { concurrency: 10 });
    ...
    reconcile: Effect.fn(function* ({ id, news }) {
        ...
        if (!observed) {
            observed = yield* images
                ...
                .pipe(Effect.map((created) => created.variant ?? undefined), Effect.catchTag("VariantAlreadyExists", () => getVariant(acct, name)));
        }
        if (!observed) {
            ...
            return yield* Effect.fail(new images.VariantNotFound({
                code: 5401,
        ...
        const dirty = observed.options.fit !== desired.fit ||
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* getVariant(output.accountId, output.variantName).pipe(Effect.repeat({
 ...
 */
const getVariant = (accountId, variantId) => images.getV1Variant({ accountId, variantId }).pipe(Effect.map((response) => response.variant ?? undefined), Effect.catchTag("VariantNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const variantNamesFrom = (value) => {
    const root = Predicate.hasProperty(value, "result") &&
        typeof value.result === "object" &&
        value.result !== null
        ? value.result
        : value;
    if (Predicate.hasProperty(root, "variants") &&
...
const desiredOptions = (news) => ({
    fit: news.fit,
    width: news.width,
    height: news.height,
    metadata: news.metadata ?? "none",
});
const toAttributes = (variant, accountId) => ({
