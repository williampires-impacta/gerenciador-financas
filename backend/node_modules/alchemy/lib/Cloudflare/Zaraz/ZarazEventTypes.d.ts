 ...
 * ```
 */
export declare function defineZarazEvents<const Events extends EventMap>(options: {
    readonly ecommerce: true;
}): EventContract<Events, EcommerceEvents>;
export declare function defineZarazEvents<const Events extends EventMap>(options?: {
    readonly ecommerce?: false;
}): EventContract<Events, {}>;
export type InferZarazEvents<Contract> = Contract extends EventContract<infer Events, EventMap> ? Events : never;
export type InferZarazEcommerceEvents<Contract> = Contract extends EventContract<EventMap, infer EcommerceEvents> ? EcommerceEvents : never;
...
type ZarazEventPropertiesArgs<Properties> = [Properties] extends [undefined] ? [properties?: undefined] : undefined extends Properties ? [properties?: Exclude<Properties, undefined>] : [properties: Properties];
/**
 * Type for Cloudflare's injected `window.zaraz.track` function.
 */
export type Track<Events extends EventMap> = <const Name extends EventName<Events>>(eventName: Name, ...args: ZarazEventPropertiesArgs<Events[Name]>) => Promise<void>;
/**
...
export type ClientProperties<Events extends EventMap, Name extends EventName<Events> = EventName<Events>> = Name extends EventName<Events> ? Events[Name] extends undefined ? {
    readonly __zarazTrack: Name;
} : Simplify<Events[Name] & {
    readonly __zarazTrack: Name;
}> : never;
...
export {};
//# sourceMappingURL=ZarazEventTypes.d.ts.map