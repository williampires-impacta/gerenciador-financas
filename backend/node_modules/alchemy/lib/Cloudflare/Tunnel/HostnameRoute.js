...
const TypeId = "Cloudflare.Tunnel.HostnameRoute";
/**
 ...
 */
export const HostnameRoute = Resource(TypeId);
/**
 * Returns true if the given value is a HostnameRoute resource.
 */
export const isHostnameRoute = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const HostnameRouteProvider = () => Provider.succeed(HostnameRoute, {
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (!observed) {
            observed = yield* zeroTrust
                ...
                .pipe(Effect.catchTag("HostnameRouteAlreadyRouted", (conflict) => findByHostname(accountId, news.hostname).pipe(Effect.repeat({
        ...
        const dirty = (observed.hostname ?? "") !== news.hostname ||
 ...
 */
const observeRoute = (accountId, hostnameRouteId) => zeroTrust.getNetworkHostnameRoute({ accountId, hostnameRouteId }).pipe(Effect.map((route) => (route.deletedAt != null ? undefined : route)), Effect.catchTag("HostnameRouteNotFound", () => Effect.succeed(undefined)));
/**
 * Find a live hostname route by exact hostname (unique per account).
 */
const findByHostname = (accountId, hostname) => zeroTrust
    .listNetworkHostnameRoutes({ accountId })
    .pipe(Effect.map((list) => (list.result ?? []).find((r) => r.hostname === hostname && r.id != null && r.deletedAt == null)));
...
//# sourceMappingURL=HostnameRoute.js.map