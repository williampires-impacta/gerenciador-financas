...
const TypeId = "Cloudflare.Tunnel.VirtualNetwork";
/**
 ...
 */
export const VirtualNetwork = Resource(TypeId);
/**
 * Returns true if the given value is a VirtualNetwork resource.
 */
export const isVirtualNetwork = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const VirtualNetworkProvider = () => Provider.succeed(VirtualNetwork, {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (!observed) {
            const created = yield* zeroTrust
                ...
                .pipe(Effect.catchTag("VirtualNetworkConflict", (error) => findByName(accountId, name).pipe(Effect.flatMap((existing) => existing ? Effect.succeed(existing) : Effect.fail(error)))));
            observed = created;
        }
 ...
 */
const getVnet = (accountId, virtualNetworkId) => zeroTrust.getNetworkVirtualNetwork({ accountId, virtualNetworkId }).pipe(Effect.map((v) => (v.deletedAt ? undefined : v)), Effect.catchTag("VirtualNetworkNotFound", () => Effect.succeed(undefined)));
/**
 * Find a live virtual network by exact name. Names are unique per account
 * so at most one live network can match.
 */
const findByName = (accountId, name) => zeroTrust.listNetworkVirtualNetworks
    .items({ accountId, name, isDeleted: false })
    .pipe(Stream.filter((v) => v.name === name && !v.deletedAt), Stream.runHead, Effect.map(Option.getOrUndefined));
const resolveName = (id, name) => Effect.gen(function* () {
    if (name)
        return name;
    return yield* createPhysicalName({ id, lowercase: true });
});
const toAttributes = (vnet, accountId) => ({
