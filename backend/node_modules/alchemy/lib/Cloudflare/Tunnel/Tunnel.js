 ...
 */
export const Tunnel = Resource("Cloudflare.Tunnel.Tunnel", {
    aliases: ["Cloudflare.Tunnel"],
});
export const TunnelProvider = () => Provider.succeed(Tunnel, {
    ...
    list: Effect.fn(function* () {
        ...
        const tunnels = yield* zeroTrust.listTunnels
            ...
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).filter((t) => !!t.id && !t.deletedAt))));
        const rows = yield* Effect.forEach(tunnels, (t) => zeroTrust
        ...
        })), Effect.catchTag("TunnelTokenNotFound", () => Effect.succeed(undefined))), { concurrency: 10 });
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        if (observed === undefined || observed.deletedAt != null)
            return;
    ...
    read: Effect.fn(function* ({ id, output, olds }) {
        ...
        if (output?.tunnelId) {
            return yield* zeroTrust
                ...
                .pipe(Effect.flatMap((t) => zeroTrust
...
const createTunnelName = (id, name) => Effect.gen(function* () {
    if (name)
        return name;
    return yield* createPhysicalName({ id });
});
const writeConfiguration = (tunnelId, ingress, originRequest) => Effect.gen(function* () {
    if (!ingress && !originRequest)
        return;
    const { accountId } = yield* yield* CloudflareEnvironment;
...
const findTunnelByName = Effect.fn(function* (name) {
    const { accountId } = yield* yield* CloudflareEnvironment;
    return yield* zeroTrust.listTunnels
        ...
        .pipe(Stream.filter((t) => t.name === name && !t.deletedAt), Stream.runHead, Effect.map(Option.getOrUndefined));
});
