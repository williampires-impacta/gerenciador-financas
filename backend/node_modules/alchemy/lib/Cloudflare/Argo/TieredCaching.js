...
const TypeId = "Cloudflare.Argo.TieredCaching";
/**
 ...
 */
export const TieredCaching = Resource(TypeId);
/**
 * Returns true if the given value is a TieredCaching resource.
 */
export const isTieredCaching = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const TieredCachingProvider = () => Provider.succeed(TieredCaching, {
    nuke: { singleton: true },
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => argo.getTieredCaching({ zoneId }).pipe(Effect.map((observed) => toAttributes(zoneId, observed, toValue(observed.value))), 
        ...
        Effect.catchTag(["InvalidObjectIdentifier", "ZoneNotFound"], () => Effect.succeed(undefined))), { concurrency: 10 });
...
//# sourceMappingURL=TieredCaching.js.map