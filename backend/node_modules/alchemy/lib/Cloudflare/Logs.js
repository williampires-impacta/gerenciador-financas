...
export const CloudflareLogs = Effect.gen(function* () {
     ...
     */
    const explainMissingObservabilityScope = (effect) => effect.pipe(Effect.catchTag("Unauthorized", () => Effect.die(new Error("Cloudflare rejected the observability telemetry query (Unauthorized). " +
    ...
    const queryLogs = (opts) => Effect.gen(function* () {
        const now = Date.now();
        const limit = opts.options.limit ?? 100;
        if (opts.options.since) {
            const response = yield* explainMissingObservabilityScope(queryTelemetry({
