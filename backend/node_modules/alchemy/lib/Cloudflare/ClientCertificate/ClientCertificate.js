...
const TypeId = "Cloudflare.ClientCertificate.ClientCertificate";
/**
 ...
 */
export const ClientCertificate = Resource(TypeId, {
    aliases: ["Cloudflare.ClientCertificate"],
});
/**
 * Returns true if the given value is a ClientCertificate resource.
 */
export const isClientCertificate = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const ClientCertificateProvider = () => Provider.succeed(ClientCertificate, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => clientCertificates.listClientCertificates
            ...
            .map((cert) => toAttributes(cert, zone.id)))), Effect.catchTag("Forbidden", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        // failure.
        Effect.catchTag("ClientCertificateNotFound", () => Effect.void), Effect.catchTag("ClientCertificateAlreadyRevoked", () => Effect.void));
    }),
});
/**
 ...
 * `undefined`.
 */
const getCertificate = (zoneId, clientCertificateId) => clientCertificates.getClientCertificate({ zoneId, clientCertificateId }).pipe(Effect.map((cert) => cert), Effect.catchTag("ClientCertificateNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findByCsr = (zoneId, csr, validityDays) => clientCertificates.listClientCertificates
    .items({ zoneId, status: "all" })
    .pipe(Stream.filter((cert) => !isGone(cert.status) &&
    cert.validityDays === validityDays &&
    typeof cert.csr === "string" &&
    normalizePem(cert.csr) === normalizePem(csr)), Stream.runHead, Effect.map(Option.getOrUndefined), Effect.map((cert) => cert));
/**
 ...
 */
const isGone = (status) => status === "revoked" || status === "pending_revocation";
