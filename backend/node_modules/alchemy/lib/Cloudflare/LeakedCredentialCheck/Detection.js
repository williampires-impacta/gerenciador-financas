...
const TypeId = "Cloudflare.LeakedCredentialCheck.Detection";
/**
 ...
 */
export const LeakedCredentialDetection = Resource(TypeId);
/**
 * Returns true if the given value is a LeakedCredentialDetection resource.
 */
export const isLeakedCredentialDetection = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const LeakedCredentialDetectionProvider = () => Provider.succeed(LeakedCredentialDetection, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => lcc.listDetections.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((d) => toAttributes(zone.id, d)))), 
        ...
        Effect.catchTag("LeakedCredentialChecksDisabled", () => Effect.succeed([])), Effect.catchTag("Forbidden", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
    ...
    read: Effect.fn(function* ({ output, olds }) {
        ...
        if (username !== undefined || password !== undefined) {
            ...
            if (observed)
                return Unowned(toAttributes(zoneId, observed));
        }
        return undefined;
    }),
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const dirty = (observed.username ?? undefined) !== news.username ||
 ...
 * off) to `undefined`.
 */
const getDetection = (zoneId, detectionId) => lcc.getDetection({ zoneId, detectionId }).pipe(Effect.map((d) => d), Effect.catchTag("DetectionNotFound", () => Effect.succeed(undefined)), Effect.catchTag("LeakedCredentialChecksDisabled", () => Effect.succeed(undefined)));
/**
 ...
 */
const findByExpressions = (zoneId, username, password) => lcc.listDetections.items({ zoneId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((d) => (d.username ?? undefined) === username &&
    (d.password ?? undefined) === password)), Effect.catchTag("LeakedCredentialChecksDisabled", () => Effect.succeed(undefined)));
const toAttributes = (zoneId, detection) => ({
