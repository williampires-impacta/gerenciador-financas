...
const TypeId = "Cloudflare.LeakedCredentialCheck.LeakedCredentialCheck";
/**
 ...
 */
export const LeakedCredentialCheck = Resource(TypeId, {
    aliases: ["Cloudflare.LeakedCredentialCheck"],
});
/**
 * Returns true if the given value is a LeakedCredentialCheck resource.
 */
export const isLeakedCredentialCheck = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
const desiredEnabled = (props) => props.enabled ?? true;
export const LeakedCredentialCheckProvider = () => Provider.succeed(LeakedCredentialCheck, {
    nuke: { singleton: true },
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => lcc.getLeakedCredentialCheck({ zoneId }).pipe(Effect.map((observed) => {
