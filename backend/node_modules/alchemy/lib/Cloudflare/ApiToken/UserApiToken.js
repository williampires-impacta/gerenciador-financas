 ...
 */
export const UserApiToken = Resource("Cloudflare.ApiToken.UserApiToken", { aliases: ["Cloudflare.UserApiToken"] });
export const UserApiTokenProvider = () => Provider.succeed(UserApiToken, {
    stables: ["tokenId"],
    ...
    reconcile: Effect.fn(function* ({ id, news = {}, output, bindings }) {
        ...
        const observed = output?.tokenId
            ? yield* user.getToken({ tokenId: output.tokenId }).pipe(Effect.map((token) => token), Effect.catchTag("TokenNotFound", () => Effect.succeed(undefined)))
            : undefined;
    ...
    read: Effect.fn(function* ({ output }) {
        if (!output?.tokenId)
            return undefined;
        return yield* user.getToken({ tokenId: output.tokenId }).pipe(Effect.map((token) => buildAttributes(token, output.value)), Effect.catchTag("TokenNotFound", () => Effect.succeed(undefined)));
    }),
    ...
    list: () => user.listTokens.pages({}).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((token) => buildAttributes(token, Redacted.make(""))))), 
...
const buildAttributes = (tokenData, value) => ({
