...
import { buildConditionPayload, collectPolicies, conditionFingerprint, policyFingerprint, resolvePolicies, } from "./Common.js";
/**
 ...
 */
export const AccountApiToken = Resource("Cloudflare.ApiToken.AccountApiToken", { aliases: ["Cloudflare.AccountApiToken"] });
const resolveName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id }));
});
const buildAttributes = (tokenData, value, accountId) => ({
...
export const AccountApiTokenProvider = () => Provider.succeed(AccountApiToken, {
    ...
    reconcile: Effect.fn(function* ({ id, news = {}, output, bindings }) {
        ...
        if (collected.length === 0) {
            return yield* Effect.die(`Cloudflare requires at least one policy on token "${name}". ` +
        ...
        const observed = output?.tokenId
            ? yield* accounts
                ...
                .pipe(Effect.map((token) => token), Effect.catchTag("InvalidRoute", () => Effect.succeed(undefined)), Effect.catchTag("TokenNotFound", () => Effect.succeed(undefined)))
            : undefined;
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        Effect.catchTag("InvalidRoute", () => Effect.void), Effect.catchTag("TokenNotFound", () => Effect.void), 
    ...
    list: Effect.fn(function* () {
        ...
        return yield* accounts.listTokens.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((token) => buildAttributes(token, Redacted.make(""), accountId)))));
    }),
    read: Effect.fn(function* ({ output }) {
        ...
        return yield* accounts
            ...
            .pipe(Effect.map((token) => buildAttributes(token, output.value, output.accountId)), Effect.catchTag("InvalidRoute", () => Effect.succeed(undefined)), Effect.catchTag("TokenNotFound", () => Effect.succeed(undefined)));
    }),
});
//# sourceMappingURL=AccountApiToken.js.map