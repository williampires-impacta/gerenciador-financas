import { apiKeyCredentials, apiTokenCredentials, Credentials, oauthCredentials, } from "@distilled.cloud/cloudflare/Credentials";
...
import { CLOUDFLARE_AUTH_PROVIDER_NAME, } from "./Auth/AuthProvider.js";
...
const getRefreshAt = (credentials, now) => {
 ...
 */
export const cacheUntilExpiry = (resolve, now = () => Date.now()) => {
    const mutex = Semaphore.makeUnsafe(1);
    let cached;
    return Semaphore.withPermits(mutex, 1)(Effect.suspend(() => {
...
export const fromAuthProvider = () => Layer.effect(Credentials, Effect.gen(function* () {
    ...
    })), Match.when({ type: "oauth" }, (c) => oauthCredentials({
        accessToken: Redacted.value(c.accessToken),
        expiresAt: c.expires,
    })), Match.exhaustive)), Effect.mapError((e) => new ConfigError({
        message: `Failed to resolve Cloudflare credentials for profile '${profileName}': ${e.message ?? String(e)}`,
    })));
