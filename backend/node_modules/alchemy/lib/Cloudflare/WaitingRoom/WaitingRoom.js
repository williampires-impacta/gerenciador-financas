...
const TypeId = "Cloudflare.WaitingRoom.WaitingRoom";
/**
 ...
 */
export const WaitingRoom = Resource(TypeId, {
    aliases: ["Cloudflare.WaitingRoom"],
});
/**
 * Returns true if the given value is a WaitingRoom resource.
 */
export const isWaitingRoom = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const WaitingRoomProvider = () => Provider.succeed(WaitingRoom, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => waitingRooms.listWaitingRoomsForZone.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((room) => toAttributes(room, zone.id)))), 
        // Plan-gated / partial-permission zones reject the route, and a
        // zone deleted out-of-band has no rooms; skip both.
        Effect.catchTag("Forbidden", () => Effect.succeed([])), Effect.catchTag("InvalidRoute", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
});
/**
 ...
 * `undefined`.
 */
const getRoom = (zoneId, waitingRoomId) => waitingRooms.getWaitingRoom({ zoneId, waitingRoomId }).pipe(Effect.map((room) => room), Effect.catchTag("WaitingRoomNotFound", () => Effect.succeed(undefined)), Effect.catchTag("InvalidRoute", () => Effect.succeed(undefined)));
/**
 * Find a waiting room by exact name. Names are unique per zone, so at most
 * one room can match.
 */
const findByName = (zoneId, name) => waitingRooms.listWaitingRoomsForZone.items({ zoneId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((room) => room.name === name)), 
// Zone deleted out-of-band — nothing to find.
Effect.catchTag("InvalidRoute", () => Effect.succeed(undefined)));
/**
 * Waiting room names only allow alphanumerics, hyphens, and underscores.
 */
const createRoomName = (id, name) => Effect.gen(function* () {
 ...
 */
const isDirty = (observed, name, desired) => {
    if ((observed.name ?? "") !== name)
        return true;
    if ((observed.host ?? "") !== desired.host)
        return true;
    if (observed.totalActiveUsers !== desired.totalActiveUsers)
        return true;
    if (observed.newUsersPerMinute !== desired.newUsersPerMinute)
        return true;
    const defaulted = (key, observedValue) => {
        const want = desired[key] ?? DEFAULTS[key];
        return (observedValue ?? DEFAULTS[key]) !== want;
    };
    if (defaulted("path", observed.path))
        return true;
    if (defaulted("description", observed.description))
        return true;
    if (defaulted("sessionDuration", observed.sessionDuration))
        return true;
    if (defaulted("disableSessionRenewal", observed.disableSessionRenewal)) {
        return true;
    }
    if (defaulted("queueAll", observed.queueAll))
        return true;
    if (defaulted("queueingMethod", observed.queueingMethod))
        return true;
    if (defaulted("queueingStatusCode", observed.queueingStatusCode)) {
        return true;
    }
    if (defaulted("suspended", observed.suspended))
        return true;
    ...
    if (defaulted("turnstileMode", observed.turnstileMode))
        return true;
    if (defaulted("turnstileAction", observed.turnstileAction))
        return true;
    // Only-when-specified props (no reliable server default to reset to).
    if (desired.cookieSuffix !== undefined &&
        (observed.cookieSuffix ?? "") !== desired.cookieSuffix) {
        return true;
    }
    if (desired.customPageHtml !== undefined &&
    ...
    if (desired.additionalRoutes !== undefined) {
        ...
        if (o.length !== d.length ||
            o.some((route, i) => (route.host ?? "") !== (d[i]?.host ?? "") ||
                (route.path ?? "/") !== (d[i]?.path ?? "/"))) {
            return true;
        }
    }
    if (desired.enabledOriginCommands !== undefined) {
        ...
        if (o.length !== d.length || o.some((c, i) => c !== d[i]))
            return true;
    }
    return false;
};
const toAttributes = (room, zoneId) => ({
    ...
    queueingMethod: (room.queueingMethod ?? "fifo"),
