...
const TypeId = "Cloudflare.Logs.CmbConfig";
/**
 ...
 */
export const CmbConfig = Resource(TypeId);
/**
 * Returns true if the given value is a CmbConfig resource.
 */
export const isCmbConfig = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const CmbConfigProvider = () => Provider.succeed(CmbConfig, {
    stables: ["accountId"],
    ...
    reconcile: Effect.fn(function* ({ news }) {
        ...
        const accountId = news.accountId ?? envAccountId;
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        // no-op.
        const observed = yield* getCmbConfig(output.accountId).pipe(Effect.catchTag("InvalidRoute", () => Effect.succeed(undefined)));
        if (observed === undefined)
            return;
        yield* logs
 ...
 */
const getCmbConfig = (accountId) => logs.getControlCmbConfig({ accountId }).pipe(Effect.map((config) => config.regions === undefined || config.regions === null
    ? undefined
    : config), 
// Accounts without the Compliance/CMB entitlement get
// `LogsControlNotAuthorized` — treat as unconfigured (nothing to manage).
Effect.catchTag(["CmbConfigNotFound", "LogsControlNotAuthorized"], () => Effect.succeed(undefined)));
const toAttributes = (accountId, config) => ({
    accountId,
    regions: config.regions ?? undefined,
    allowOutOfRegionAccess: config.allowOutOfRegionAccess ?? undefined,
});
//# sourceMappingURL=CmbConfig.js.map