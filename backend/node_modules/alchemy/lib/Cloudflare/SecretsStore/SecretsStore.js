 ...
 */
export const Store = Resource("Cloudflare.SecretsStore");
export const StoreProviderLive = () => Provider.succeed(Store, {
    ...
    reconcile: Effect.fn(function* ({ output }) {
        ...
        return yield* Effect.die(new Error(`Cloudflare reported MaximumStoresExceeded for account ${acct} but no store could be listed.`));
    }),
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* secretsStore.listStores.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((store) => ({
            storeId: store.id,
 ...
 */
export const StoreProviderLocal = () => Provider.succeed(Store, {
    stables: ["accountId"],
...
export const SecretsStoreProvider = () => ProviderLayer.dual(Store, {
    local: () => StoreProviderLocal(),
    live: () => StoreProviderLive(),
});
/** First store on the account (Cloudflare permits at most one). */
const firstStore = (accountId) => secretsStore.listStores
    .items({ accountId })
    .pipe(Stream.runHead, Effect.map(Option.getOrUndefined));
/** Look a store up by id, terminating pagination on the first match. */
const findStoreById = (accountId, storeId) => secretsStore.listStores.items({ accountId }).pipe(Stream.filter((s) => s.id === storeId), Stream.runHead, Effect.map(Option.getOrUndefined));
