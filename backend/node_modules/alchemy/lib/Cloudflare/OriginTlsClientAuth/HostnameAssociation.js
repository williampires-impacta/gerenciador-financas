...
const TypeId = "Cloudflare.OriginTlsClientAuth.HostnameAssociation";
/**
...
const observeAssociation = (zoneId, hostname) => originTls.getHostname({ zoneId, hostname }).pipe(Effect.map((assoc) => assoc.enabled === null || assoc.enabled === undefined ? undefined : assoc), Effect.catchTag("HostnameAssociationNotFound", () => Effect.succeed(undefined)));
const toAttributes = (assoc, zoneId, hostname) => ({
    zoneId,
    hostname,
    certId: assoc.certId,
