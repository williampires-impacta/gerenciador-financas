...
const TypeId = "Cloudflare.OriginTlsClientAuth.Certificate";
/**
...
export const CertificateProvider = () => Provider.succeed(Certificate, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => originTls.listOriginTlsClientAuths.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
...
const observeById = (zoneId, certificateId) => originTls.getOriginTlsClientAuth({ zoneId, certificateId }).pipe(Effect.map((cert) => (isLive(cert.status) ? cert : undefined)), Effect.catchTag("CertificateNotFound", () => Effect.succeed(undefined)));
