...
const TypeId = "Cloudflare.Addressing.Prefix";
/**
 ...
 * @category Network
 */
export const Prefix = Resource(TypeId);
/**
 * Returns true if the given value is an Prefix resource.
 */
export const isPrefix = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const PrefixProvider = () => Provider.succeed(Prefix, {
    stables: [
        "prefixId",
        "accountId",
        "cidr",
    ...
    list: Effect.fn(function* () {
        ...
        return yield* addressing.listPrefixes.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((p) => toAttributes(p, accountId)))));
    }),
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (!observed) {
            const created = yield* addressing.createPrefix({
                ...
                loaDocumentId: news.loaDocumentId,
 ...
 */
const getPrefix = (accountId, prefixId) => addressing
 ...
 */
const findByCidr = (accountId, cidr) => addressing.listPrefixes.items({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((p) => p.cidr === cidr)));
const toAttributes = (prefix, accountId) => ({
    prefixId: prefix.id ?? "",
    accountId,
    cidr: prefix.cidr ?? "",
    asn: prefix.asn ?? 0,
