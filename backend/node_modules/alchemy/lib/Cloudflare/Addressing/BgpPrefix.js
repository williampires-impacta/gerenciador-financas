...
const TypeId = "Cloudflare.Addressing.BgpPrefix";
/**
 ...
 * @category Network
 */
export const BgpPrefix = Resource(TypeId);
/**
 * Returns true if the given value is an BgpPrefix resource.
 */
export const isBgpPrefix = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const BgpPrefixProvider = () => Provider.succeed(BgpPrefix, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(prefixIds, (prefixId) => addressing.listPrefixBgpPrefixes.items({ accountId, prefixId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).map((bgp) => toAttributes(bgp, prefixId, accountId))), 
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const dirty = (news.advertised !== undefined &&
        ...
        if (dirty) {
            const patched = yield* addressing
                ...
                onDemand: news.advertised !== undefined
 ...
 */
const getBgpPrefix = (accountId, prefixId, bgpPrefixId) => addressing
    .getPrefixBgpPrefix({ accountId, prefixId, bgpPrefixId })
    .pipe(Effect.catchTag(["BgpPrefixNotFound", "PrefixNotFound"], () => Effect.succeed(undefined)));
/**
 * Find a BGP prefix by exact CIDR — unique within a parent prefix. The
 * parent prefix being gone reads as "no match".
 */
const findByCidr = (accountId, prefixId, cidr) => addressing.listPrefixBgpPrefixes.items({ accountId, prefixId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((p) => p.cidr === cidr)), Effect.catchTag("PrefixNotFound", () => Effect.succeed(undefined)));
const toAttributes = (bgp, prefixId, accountId) => ({
    bgpPrefixId: bgp.id ?? "",
    prefixId,
    accountId,
    cidr: bgp.cidr ?? "",
...
//# sourceMappingURL=BgpPrefix.js.map