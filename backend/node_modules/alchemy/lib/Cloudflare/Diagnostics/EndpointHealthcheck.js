...
const TypeId = "Cloudflare.Diagnostics.EndpointHealthcheck";
/**
 ...
 */
export const EndpointHealthcheck = Resource(TypeId);
/**
 * Returns true if the given value is an EndpointHealthcheck resource.
 */
export const isEndpointHealthcheck = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const EndpointHealthcheckProvider = () => Provider.succeed(EndpointHealthcheck, {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        const dirty = observed.endpoint !== desired.endpoint ||
    ...
    list: Effect.fn(function* () {
        ...
        return yield* diagnostics.listEndpointHealthchecks({ accountId }).pipe(Effect.map((checks) => checks.map((hc) => toAttributes(hc, accountId))), Effect.catchTag("Forbidden", () => Effect.succeed([])));
    }),
 ...
 */
const getHealthcheck = (accountId, id) => diagnostics.getEndpointHealthcheck({ accountId, id }).pipe(Effect.map((hc) => ({ ...hc, accountId })), Effect.catchTag("EndpointHealthcheckNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const observeExisting = (accountId, id) => diagnostics.getEndpointHealthcheck({ accountId, id }).pipe(Effect.map((hc) => ({ ...hc, accountId })), Effect.retry({
 ...
 */
const findByName = (accountId, name) => diagnostics.listEndpointHealthchecks({ accountId }).pipe(Effect.map((list) => list
    .filter((hc) => hc.name === name && typeof hc.id === "string")
    .sort((a, b) => (a.id ?? "").localeCompare(b.id ?? ""))
    .at(0)), Effect.map((hc) => hc && { ...hc, accountId }));
const createHealthcheckName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const toAttributes = (hc, accountId) => ({
    healthcheckId: hc.id ?? "",
    accountId,
...
//# sourceMappingURL=EndpointHealthcheck.js.map