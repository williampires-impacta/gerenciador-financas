...
const TypeId = "Cloudflare.LoadBalancer.Pool";
/**
 ...
 */
export const Pool = Resource(TypeId);
/**
 * Returns true if the given value is a Pool resource.
 */
export const isPool = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const PoolProvider = () => Provider.succeed(Pool, {
    ...
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* loadBalancers.listPools.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((pool) => ({
            poolId: pool.id ?? "",
            accountId,
            name: pool.name ?? "",
 ...
 * to `undefined`.
 */
const getPool = (accountId, poolId) => loadBalancers
 ...
 */
const findByName = (accountId, name) => loadBalancers.listPools.items({ accountId }).pipe(Stream.filter((p) => p.name === name), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    .sort((a, b) => (a.createdOn ?? "").localeCompare(b.createdOn ?? ""))
    .at(0)));
const createPoolName = (id, name) => Effect.gen(function* () {
    return name ?? (yield* createPhysicalName({ id, lowercase: true }));
});
const buildBody = (news, name) => ({
    name,
    origins: news.origins.map((o) => ({
        ...
        weight: o.weight,
        port: o.port,
        header: o.header === undefined
            ? undefined
            : {
                host: o.header.host === undefined
                    ? undefined
    ...
    monitorGroup: news.monitorGroup,
...
const poolDirty = (observed, body) => {
    ...
    const observedOrigins = (observed.origins ?? []).map((o) => normalizeOrigin({
        address: o.address ?? "",
...
const toAttributes = (pool, accountId) => ({
    poolId: pool.id ?? "",
    accountId,
    name: pool.name ?? "",
    enabled: pool.enabled ?? true,
