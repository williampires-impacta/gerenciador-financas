...
const TypeId = "Cloudflare.DdosProtection.TcpFlowProtectionFilter";
/**
 ...
 * @category Network
 */
export const TcpFlowProtectionFilter = Resource(TypeId);
/**
 * Returns true if the given value is a TcpFlowProtectionFilter resource.
 */
export const isTcpFlowProtectionFilter = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const TcpFlowProtectionFilterProvider = () => Provider.succeed(TcpFlowProtectionFilter, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* ddos.listAdvancedTcpProtectionTcpFlowProtectionFilters
            .pages({ accountId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((filter) => toAttributes(filter, accountId)))), 
        ...
        Effect.catchTag(["AdvancedTcpProtectionNotEntitled", "Forbidden"], () => Effect.succeed([])));
    }),
...
const getFilter = (accountId, filterId) => ddos
    ...
    .pipe(Effect.map((filter) => filter), Effect.catchTag("TcpFlowProtectionFilterNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findByExpression = (accountId, expression) => ddos.listAdvancedTcpProtectionTcpFlowProtectionFilters
    .items({ accountId })
...
//# sourceMappingURL=TcpFlowProtectionFilter.js.map