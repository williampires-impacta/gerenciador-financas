 ...
 * @category Email
 */
export interface Send extends Binding.Service<Send, "Cloudflare.Email.SendEmail", (sender: SendEmail) => Effect.Effect<SendClient>> {
}
...
declare const SendEmailError_base: new <A extends Record<string, any> = {}>(args: import("effect/Types").VoidIfEmpty<{ readonly [P in keyof A as P extends "_tag" ? never : P]: A[P]; }>) => import("effect/Cause").YieldableError & {
    readonly _tag: "SendEmailError";
} & Readonly<A>;
export declare class SendEmailError extends SendEmailError_base<{
    message: string;
    cause?: unknown;
}> {
}
export interface SendClient {
     ...
     */
    send(message: SendEmailMessage): Effect.Effect<runtime.EmailSendResult, SendEmailError, RuntimeContext>;
    /**
     * Send a raw `EmailMessage` (constructed via `cloudflare:email`).
     */
    sendRaw(message: runtime.EmailMessage): Effect.Effect<runtime.EmailSendResult, SendEmailError, RuntimeContext>;
}
export {};
