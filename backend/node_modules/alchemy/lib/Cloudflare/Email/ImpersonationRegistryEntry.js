...
const EmailSecurityImpersonationRegistryEntryTypeId = "Cloudflare.Email.ImpersonationRegistryEntry";
/**
 ...
 * @category Email
 */
export const ImpersonationRegistryEntry = Resource(EmailSecurityImpersonationRegistryEntryTypeId, { aliases: ["Cloudflare.EmailSecurity.ImpersonationRegistryEntry"] });
/**
 * Returns true if the given value is an
 * ImpersonationRegistryEntry resource.
 */
export const isImpersonationRegistryEntry = (value) => Predicate.hasProperty(value, "Type") &&
    value.Type === EmailSecurityImpersonationRegistryEntryTypeId;
export const ImpersonationRegistryEntryProvider = () => Provider.succeed(ImpersonationRegistryEntry, {
    ...
    list: Effect.fn(function* () {
        ...
        return yield* emailSecurity.listSettingImpersonationRegistries
            .pages({ accountId })
            .pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((entry) => toAttributes(entry, accountId)))), Effect.catchTag("EmailSecurityNotEntitled", () => Effect.succeed([])));
    }),
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        const dirty = (observed.name ?? "") !== news.name ||
 ...
 */
const getEntry = (accountId, impersonationRegistryId) => emailSecurity
    .getSettingImpersonationRegistry({ accountId, impersonationRegistryId })
    .pipe(Effect.map((entry) => entry), Effect.catchTag("ImpersonationRegistryEntryNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findByIdentity = (accountId, name, email) => emailSecurity.listSettingImpersonationRegistries
...
//# sourceMappingURL=ImpersonationRegistryEntry.js.map