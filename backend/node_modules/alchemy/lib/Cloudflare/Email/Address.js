 ...
 * @category Email
 */
export const Address = Resource("Cloudflare.Email.Address", {
    aliases: ["Cloudflare.EmailAddress"],
});
const toAttrs = (accountId, result) => ({
    addressId: result.id ?? "",
    email: result.email ?? "",
    accountId,
...
const findByEmail = (accountId, email) => emailRouting.listAddresses.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk)
    .flatMap((page) => page.result ?? [])
    .map((addr) => toAttrs(accountId, addr))
    .find((a) => a.email === email)));
export const AddressProvider = () => Provider.succeed(Address, {
    stables: ["addressId", "accountId", "email"],
    // Account collection: destination addresses are enumerable account-wide.
    list: Effect.fn(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        return yield* emailRouting.listAddresses.pages({ accountId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((addr) => toAttrs(accountId, addr)))));
    }),
    ...
    read: Effect.fn(function* ({ output, olds }) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        const identifier = output?.addressId ??
        ...
        return yield* emailRouting
            ...
            .pipe(Effect.map((r) => toAttrs(acct, r)), Effect.catch(() => Effect.succeed(undefined)));
    }),
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (!observed) {
            observed = yield* emailRouting
                ...
                .pipe(Effect.map((created) => toAttrs(acct, created)), 
            ...
            Effect.catchTag("TooManyRequests", (error) => findByEmail(acct, email).pipe(Effect.flatMap((found) => found ? Effect.succeed(found) : Effect.fail(error)))));
        }
        return observed;
    }),
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* emailRouting
            ...
            .pipe(Effect.catchTag("EmailAddressNotFound", () => Effect.void), Effect.retry({
