...
import { DispatchNamespaceError, Get, } from "./Get.js";
/**
 * Implementation of the {@link Get} binding that uses a native
 * `dispatch_namespace` Worker binding.
 */
export const GetBinding = Layer.effect(Get, Effect.gen(function* () {
    ...
    return Effect.fn(function* (namespace) {
        ...
        const self = {
            raw,
            get: (name, args, options) => raw.pipe(Effect.flatMap((ns) => Effect.try({
                try: () => ns.get(name, args, options),
                catch: (error) => new DispatchNamespaceError({
