...
export const toCloudflareFetcher = Effect.fn(function* (fetcher) {
    const context = yield* Effect.context();
    return {
        fetch: (input, init) => fetcher
            .fetch(HttpServerRequest.fromWeb(new Request(input, init)))
            .pipe(Effect.map((response) => HttpServerResponse.toWeb(response, {
            context,
        })), Effect.provideContext(context), Effect.runPromise),
        connect() {
            // TODO
...
export const fromCloudflareFetcher = (fetcher) => {
    const fetch = (request) => Effect.suspend(() => {
        ...
        return Effect.promise((signal) => fetcher.fetch(attempt, {
            signal: signal,
        }));
    }).pipe(
    ...
    Effect.catchCause((cause) => {
        ...
        times: 8,
    }), Effect.catch((error) => error instanceof HandlerNotReady
    ...
    return {
        ...
        fetch: (request) => HttpClientRequest.isHttpClientRequest(request)
            ? pipe(HttpServerRequest.toWeb(HttpServerRequest.fromClientRequest(request)), Effect.flatMap(fetch), Effect.map((response) => HttpClientResponse.fromWeb(request, response)), Effect.catch((error) => Effect.succeed(HttpClientResponse.fromWeb(request, new Response(error.message, {
                status: error._tag === "InternalError"
                    ? 500
                    : error._tag === "RequestParseError"
                        ? 400
                        : 404,
            })))))
            : pipe(HttpServerRequest.toWeb(request), Effect.flatMap(fetch), Effect.map((response) => {
                if (response.status === 101) {
                    return HttpServerResponse.setBody(HttpServerResponse.empty({ status: 101 }), HttpBody.raw(response));
                }
...
class HandlerNotReady {
    cause;
    _tag = "HandlerNotReady";
    constructor(cause) {
        this.cause = cause;
    }
}
export const toHttpClient = (fetcher) => HttpClient.make((request) => {
    ...
    fetcher
        ...
        .pipe(Effect.map((response) => HttpClientResponse.fromWeb(request, HttpServerResponse.toWeb(response))))).pipe(
    ...
    Effect.catchCause((cause) => {
        const squashed = Cause.squash(cause);
        return Effect.fail(new HttpClientError({
...
export const fromCloudflareSocket = (cfSocket) => {
    const latch = Latch.makeUnsafe(false);
    let currentFiberSet;
    let writerRef;
    const encoder = new TextEncoder();
    const closeError = (code, closeReason) => new Socket.SocketError({
        reason: new Socket.SocketCloseError({ code, closeReason }),
    });
    const runRaw = (handler, opts) => Effect.scopedWith(Effect.fn(function* (scope) {
        ...
        yield* Effect.tryPromise({
            ...
            catch: (cause) => Socket.isSocketError(cause)
                ? cause
        ...
        if (opts?.onOpen)
            yield* opts.onOpen;
        return yield* Effect.catchFilter(FiberSet.join(fiberSet), Socket.SocketCloseError.filterClean((code) => code === 1000 || code === 1006), () => Effect.void);
    ...
    const runString = (handler, opts) => runRaw((data) => typeof data === "string"
        ? handler(data)
        : handler(decoder.decode(data)), opts);
