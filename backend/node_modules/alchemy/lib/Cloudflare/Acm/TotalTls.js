...
const TypeId = "Cloudflare.Acm.TotalTls";
/**
 ...
 */
export const TotalTls = Resource(TypeId);
/**
 * Returns true if the given value is a TotalTls resource.
 */
export const isTotalTls = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const TotalTlsProvider = () => Provider.succeed(TotalTls, {
    nuke: { singleton: true },
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => acm.getTotalTl({ zoneId }).pipe(Effect.map((observed) => toAttributes(zoneId, observed, initialStateOf(undefined, observed))), 
        // Zone deleted out-of-band between enumeration and read.
        Effect.catchTag("InvalidObjectIdentifier", () => Effect.succeed(undefined))), { concurrency: 10 });
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* acm
            ...
            .pipe(Effect.catchTag("InvalidObjectIdentifier", () => Effect.void), Effect.catchTag("NoStateChange", () => Effect.void), 
        // The zone lost its ACM entitlement since we configured it —
        // Cloudflare already refuses Total TLS writes, so there is
        // nothing left to restore.
        Effect.catchTag("AdvancedCertificateManagerRequired", () => Effect.void), Effect.retry({
            while: (e) => e._tag === "PreviousJobInProgress",
            schedule: Schedule.spaced("3 seconds"),
            times: 10,
        }), Effect.asVoid);
 ...
 */
const initialStateOf = (output, observed) => output !== undefined
    ? {
 ...
 */
const matchesDesired = (observed, news) => (observed.enabled ?? false) === news.enabled &&
