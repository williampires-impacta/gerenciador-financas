...
const TypeId = "Cloudflare.Acm.CustomTrustStore";
/**
 ...
 */
export const CustomTrustStore = Resource(TypeId);
/**
 * Returns true if the given value is a CustomTrustStore resource.
 */
export const isCustomTrustStore = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const CustomTrustStoreProvider = () => Provider.succeed(CustomTrustStore, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => acm.listCustomTrustStores.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? [])
        ...
        Effect.catchTag("AdvancedCertificateManagerRequired", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* acm
            ...
            .pipe(Effect.catchTag(["CustomTrustStoreNotFound", "InvalidObjectIdentifier"], () => Effect.void));
    }),
});
/**
 ...
 */
const isGoneStatus = (status) => status === "pending_deletion" || status === "deleted";
/**
 * Read a trust store certificate by id, mapping "gone"
 * (`CustomTrustStoreNotFound` / `InvalidObjectIdentifier`) to `undefined`.
 */
const getTrustStore = (zoneId, id) => acm.getCustomTrustStore({ zoneId, customOriginTrustStoreId: id }).pipe(Effect.map((cert) => cert), Effect.catchTag(["CustomTrustStoreNotFound", "InvalidObjectIdentifier"], () => Effect.succeed(undefined)));
/**
 ...
 */
const findByCertificate = (zoneId, certificate) => acm.listCustomTrustStores.items({ zoneId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((cert) => !isGoneStatus(cert.status) &&
    normalizePem(cert.certificate) === normalizePem(certificate))));
const normalizePem = (pem) => pem.replace(/\r\n/g, "\n").trim();
const toAttributes = (zoneId, cert) => ({
    id: cert.id,
    zoneId,
...
//# sourceMappingURL=CustomTrustStore.js.map