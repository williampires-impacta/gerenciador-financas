...
const TypeId = "Cloudflare.GoogleTagGateway.GoogleTagGateway";
/**
 ...
 */
export const GoogleTagGateway = Resource(TypeId, {
    aliases: ["Cloudflare.GoogleTagGateway"],
});
/**
 * Returns true if the given value is a GoogleTagGateway resource.
 */
export const isGoogleTagGateway = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const GoogleTagGatewayProvider = () => Provider.succeed(GoogleTagGateway, {
    nuke: { singleton: true },
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => googleTagGateway.getConfig({ zoneId }).pipe(Effect.map((observed) => {
        ...
        Effect.catchTag(["InvalidRoute", "Forbidden"], () => Effect.succeed(undefined))), { concurrency: 10 });
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        // crashed run).
        if (configEquals(toConfig(observed), target))
            return;
...
const configEquals = (a, b) => a.enabled === b.enabled &&
