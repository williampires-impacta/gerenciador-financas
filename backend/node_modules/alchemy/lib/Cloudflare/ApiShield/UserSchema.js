...
const TypeId = "Cloudflare.ApiShield.UserSchema";
/**
 ...
 */
export const UserSchema = Resource(TypeId);
/**
 * Returns true if the given value is an UserSchema resource.
 */
export const isUserSchema = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const UserSchemaProvider = () => Provider.succeed(UserSchema, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => apiGateway.listUserSchemas.pages({ zoneId: zone.id }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((schema) => toAttributes(schema, zone.id)))), 
        ...
        Effect.catchTag("ZonePurged", () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
});
/**
 ...
 */
const getSchema = (zoneId, schemaId) => apiGateway.getUserSchema({ zoneId, schemaId }).pipe(Effect.map((schema) => schema), Effect.catchTag("SchemaNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const findByName = (zoneId, name) => apiGateway.listUserSchemas.items({ zoneId }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).find((schema) => schema.name === name)));
/**
...
const toAttributes = (schema, zoneId) => ({
