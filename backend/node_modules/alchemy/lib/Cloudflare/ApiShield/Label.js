...
const TypeId = "Cloudflare.ApiShield.Label";
/**
 * Returns true if the given value is an Label resource.
 */
export const isLabel = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
/**
 ...
 */
export const Label = Resource(TypeId);
export const LabelProvider = () => Provider.succeed(Label, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => apiGateway.listLabels.pages({ zoneId: zone.id, source: "user" }).pipe(Stream.runCollect, Effect.map((chunk) => Array.from(chunk).flatMap((page) => (page.result ?? []).map((label) => toAttributes(label, zone.id)))), 
        ...
        Effect.catchTag(["ZonePurged", "InvalidRoute", "NotFound"], () => Effect.succeed([]))), { concurrency: 10 });
        return rows.flat();
    }),
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (!observed) {
            observed = yield* apiGateway
                ...
                .pipe(Effect.map((response) => response.result[0]), Effect.catchTag("LabelAlreadyExists", (error) => getLabel(zoneId, name).pipe(Effect.flatMap((existing) => existing ? Effect.succeed(existing) : Effect.fail(error)))));
        }
 ...
 */
const getLabel = (zoneId, name) => apiGateway.getLabelUser({ zoneId, name }).pipe(Effect.map((label) => label), Effect.catchTag("LabelNotFound", () => Effect.succeed(undefined)));
/**
 ...
 */
const createLabelName = (id, name) => Effect.gen(function* () {
    return (name ??
...
const toAttributes = (label, zoneId) => ({
    zoneId,
    name: label.name,
