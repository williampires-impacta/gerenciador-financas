...
const TypeId = "Cloudflare.SecurityTxt.SecurityTxt";
/**
 ...
 */
export const SecurityTxt = Resource(TypeId, {
    aliases: ["Cloudflare.SecurityTxt"],
});
/**
 * Returns true if the given value is a SecurityTxt resource.
 */
export const isSecurityTxt = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const SecurityTxtProvider = () => Provider.succeed(SecurityTxt, {
    stables: ["zoneId"],
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => securityTxt
            .getSecurityTxt({ zoneId })
            .pipe(Effect.map((observed) => typeof observed === "string"
            ? undefined
            : toAttributes(zoneId, observed))), { concurrency: 10 });
...
const desiredAttributes = (zoneId, news) => ({
    zoneId,
...
const toAttributes = (zoneId, observed) => ({
    ...
    preferredLanguages: observed.preferredLanguages === ""
        ? undefined
        : (observed.preferredLanguages ?? undefined),
});
/** Null/empty list fields normalize to `undefined` (not configured). */
const nonEmpty = (values) => values === null || values === undefined || values.length === 0
    ? undefined
    : [...values];
...
const attributesEqual = (a, b) => a.zoneId === b.zoneId &&
    a.enabled === b.enabled &&
    a.expires === b.expires &&
...
//# sourceMappingURL=SecurityTxt.js.map