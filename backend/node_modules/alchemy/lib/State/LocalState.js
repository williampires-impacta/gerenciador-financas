...
import { encodeState, reviveState } from "./StateEncoding.js";
/**
...
export const makeLocalState = () => Effect.gen(function* () {
    ...
    const recover = (effect) => effect.pipe(Effect.catchTag("PlatformError", (e) => e.reason._tag === "NotFound" ? Effect.void : fail(e)));
    ...
    const recoverMissingDir = (dir, effect) => effect.pipe(Effect.catchTag("PlatformError", (e) => {
        if (e.reason._tag !== "NotFound")
            return fail(e);
        return Effect.flatMap(Effect.sync(() => existsSync(dir)), (exists) => exists
            ? Effect.fail(new StateStoreError({
                message: `state store reported NotFound for '${dir}', but the directory exists — ` +
    ...
    const stageDir = ({ stack, stage }) => path.join(stateDir, stack, stage);
    const resource = ({ stack, stage, fqn, }) => path.join(stateDir, stack, stage, `${encodeFqn(fqn)}.json`);
    const outputFile = ({ stack, stage }) => path.join(stateDir, stack, stage, `__stack_output__.json`);
    ...
    const writeAtomic = (file, contents) => Effect.suspend(() => {
        const tmp = `${file}.${process.pid}.${Math.random()
            .toString(36)
            .slice(2)}.tmp`;
        return fs.writeFileString(tmp, contents).pipe(Effect.flatMap(() => fs.rename(tmp, file)), Effect.tapError(() => fs.remove(tmp).pipe(Effect.ignore)));
    });
    ...
    const parseState = (contents) => contents.trim().length === 0
        ? undefined
        : JSON.parse(contents, reviveState);
    const created = new Set();
    const ensure = (dir) => created.has(dir)
    ...
    const state = {
        ...
        listStacks: () => fs.readDirectory(stateDir).pipe(recover, Effect.map((files) => files ?? [])),
        listStages: (stack) => fs.readDirectory(path.join(stateDir, stack)).pipe(recover, Effect.map((files) => files ?? [])),
        get: (request) => fs.readFile(resource(request)).pipe(Effect.map((file) => parseState(file.toString())), recover),
        ...
        set: (request) => ensure(stageDir(request)).pipe(Effect.flatMap(() => writeAtomic(resource(request), JSON.stringify(encodeState(request.value), null, 2))), recover, Effect.map(() => request.value)),
        delete: (request) => fs.remove(resource(request)).pipe(recover),
        deleteStack: ({ stack, stage }) => Effect.suspend(() => {
            const dir = stage === undefined
                ? path.join(stateDir, stack)
                : stageDir({ stack, stage });
            return fs.remove(dir, { recursive: true }).pipe((eff) => recoverMissingDir(dir, eff), 
            ...
            Effect.tap(() => {
                ...
                return fs.readDirectory(stackDir).pipe(Effect.flatMap((entries) => entries.length === 0
                    ? fs
        ...
        list: (request) => fs.readDirectory(stageDir(request)).pipe((eff) => recoverMissingDir(stageDir(request), eff), Effect.map((files) => (files ?? [])
        ...
        getOutput: (request) => fs.readFile(outputFile(request)).pipe(Effect.map((file) => parseState(file.toString())), recover),
        setOutput: (request) => ensure(stageDir(request)).pipe(Effect.flatMap(() => writeAtomic(outputFile(request), JSON.stringify(encodeState(request.value), null, 2))), recover, Effect.map(() => request.value)),
    };
    return state;
});
