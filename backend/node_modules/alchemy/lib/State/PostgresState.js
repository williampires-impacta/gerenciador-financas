...
const importPgClient = () => import("@effect/sql-pg/PgClient").catch((cause) => {
    throw new Error("Failed to load '@effect/sql-pg'. Install the optional peer dependency '@effect/sql-pg' to use postgresState with a `url`.", { cause });
});
 ...
 */
const amortizeCheck = (checkLive, ttlMs) => {
    let lastOkAt;
 ...
 * ```
 */
export const postgresState = (options) => Layer.effect(State, Effect.gen(function* () {
    const scope = yield* Effect.scope;
    // The layer carries the `url` Effect's requirements; the service it
    // builds must not, so they are provided here, once.
    const context = yield* Effect.context();
    const make = makePostgresState(options, scope).pipe(recordStateStoreInit, Effect.provideContext(context));
 ...
 */
export const makePostgresState = (options, scope) => Effect.gen(function* () {
    ...
    const context = yield* Effect.context();
    const toError = (cause) => cause instanceof StateStoreError
        ? cause
        : new StateStoreError({
    ...
    const stateError = (effect) => Effect.mapError(effect, toError);
    /**
     ...
     */
    const openPool = (url) => Effect.gen(function* () {
        ...
        const resolved = Effect.isEffect(url)
            ...
            : url;
        const built = yield* Layer.build(PgClient.layer({ url: resolved })).pipe(Scope.provide(scope), stateError);
    ...
    const ready = yield* Effect.cached(Effect.gen(function* () {
        const { client, url } = options;
        const resolved = client !== undefined && url === undefined
            ? client
            : url !== undefined && client === undefined
                ? yield* openPool(url)
                : yield* Effect.fail(new StateStoreError({
    ...
    const run = (f) => ready.pipe(Effect.flatMap((sql) => stateError(f(sql))));
    const lockKey = (stack, stage) => `${prefix}:${stack}/${stage}`;
    /**
     ...
     */
    const acquireLease = (key) => Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
        ...
        const reserved = yield* restore(sql.reserve.pipe(Scope.provide(lockScope), stateError)).pipe(Effect.onError(() => releaseReserved));
        ...
        const onReserved = (effect) => Effect.provideService(effect, sql.transactionService, [
            reserved,
            0,
        ]);
        const acquired = yield* onReserved(sql `select pg_try_advisory_lock(hashtextextended(${key}, 0)) as acquired, pg_backend_pid() as pid`).pipe(stateError, Effect.map((rows) => rows[0]), Effect.onError(() => releaseReserved));
        ...
        yield* Scope.addFinalizer(scope, onReserved(sql `select pg_advisory_unlock(hashtextextended(${key}, 0))`).pipe(
        // If the connection already dropped, Postgres auto-released
        // the session-scoped lock; there is nothing left to unlock.
        Effect.ignore, Effect.andThen(releaseReserved)));
        ...
        // for itself.
        const checkLive = run((poolSql) => poolSql `
              select
                exists (
            ...
            if (Number(row?.checker_pid) === lockPid) {
                return Effect.fail(new StateStoreError({
            ...
            return row?.live === true
                ? Effect.void
                : Effect.fail(new StateStoreError({
    ...
    const leaseFor = (stack, stage) => Semaphore.withPermits(leaseMutex, 1)(Effect.gen(function* () {
     ...
     */
    const guarded = (request, op) => leaseFor(request.stack, request.stage).pipe(Effect.flatMap((lease) => lease.checkLive), Effect.andThen(op));
    // Operations without a stage cannot name a single lease, so they
    // re-verify every lease this store currently holds instead.
    const verifyHeldLeases = Effect.suspend(() => Effect.forEach(Array.from(leases.values()), (lease) => lease.pipe(Effect.flatMap((held) => held.checkLive)), { discard: true }));
    const jsonParam = (value) => JSON.stringify(encodeState(value));
    const deleteStage = (stack, stage) => run((sql) => sql `delete from alchemy_resource_state where stack = ${stack} and stage = ${stage}`).pipe(Effect.andThen(run((sql) => sql `delete from alchemy_stack_output where stack = ${stack} and stage = ${stage}`)), Effect.asVoid);
    const service = {
        id: options.id ?? "postgres",
        getVersion: () => Effect.succeed(STATE_STORE_VERSION),
        listStacks: () => verifyHeldLeases.pipe(Effect.andThen(run((sql) => sql `select stack from alchemy_resource_state union select stack from alchemy_stack_output order by stack`)), Effect.map((rows) => rows.map((row) => String(row.stack)))),
        listStages: (stack) => verifyHeldLeases.pipe(Effect.andThen(run((sql) => sql `select stage from alchemy_resource_state where stack = ${stack} union select stage from alchemy_stack_output where stack = ${stack} order by stage`)), Effect.map((rows) => rows.map((row) => String(row.stage)))),
        get: (request) => guarded(request, run((sql) => sql `select value from alchemy_resource_state where stack = ${request.stack} and stage = ${request.stage} and fqn = ${request.fqn}`).pipe(Effect.map((rows) => {
            const row = rows[0];
        ...
        // N+1 round-trips.
        getReplacedResources: (request) => guarded(request, run((sql) => sql `select value from alchemy_resource_state where stack = ${request.stack} and stage = ${request.stage} and value ->> 'status' = 'replaced'`).pipe(Effect.map((rows) => rows.map((row) => reviveStateRecursive(row.value))))),
        set: (request) => guarded(request, run((sql) => sql `
        ...
        delete: (request) => guarded(request, run((sql) => sql `delete from alchemy_resource_state where stack = ${request.stack} and stage = ${request.stage} and fqn = ${request.fqn}`).pipe(Effect.asVoid)),
        ...
        deleteStack: ({ stack, stage }) => stage === undefined
            ? verifyHeldLeases.pipe(Effect.andThen(service.listStages(stack)), Effect.flatMap((stages) => Effect.forEach(stages, (found) => guarded({ stack, stage: found }, deleteStage(stack, found)), { discard: true })), Effect.andThen(run((sql) => sql `delete from alchemy_resource_state where stack = ${stack}`)), Effect.andThen(run((sql) => sql `delete from alchemy_stack_output where stack = ${stack}`)), Effect.asVoid)
            : guarded({ stack, stage }, deleteStage(stack, stage)),
        list: (request) => guarded(request, run((sql) => sql `select fqn from alchemy_resource_state where stack = ${request.stack} and stage = ${request.stage} order by fqn`).pipe(Effect.map((rows) => rows.map((row) => String(row.fqn))))),
        getOutput: (request) => guarded(request, run((sql) => sql `select value from alchemy_stack_output where stack = ${request.stack} and stage = ${request.stage}`).pipe(Effect.map((rows) => {
            const row = rows[0];
            return row === undefined
                ? undefined
                : reviveStateRecursive(row.value);
        }))),
        setOutput: (request) => guarded(request, run((sql) => sql `
...
//# sourceMappingURL=PostgresState.js.map